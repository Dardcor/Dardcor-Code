---
title: "Dardcor Code Codebase Documentation"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code Codebase Documentation

> **Version:** v3.8.0
> **Last updated:** 2026-06-28
> **Audience:** Engineers contributing to Dardcor Code or building integrations on top of it.
>
...
- `@/*` → `src/*`
- `@Dardcor Code/open-sse` → `open-sse/index.ts`
- `@Dardcor Code/open-sse/*` → `open-sse/*`

Default HTTP port: **`20128`** (API and dashboard share the same process). Data
directory is `DATA_DIR` env var, defaulting to `~/.Dardcor Code/`.
...
├── open-sse/             Streaming engine workspace (@Dardcor Code/open-sse)
├── electron/             Desktop wrapper (Electron 41 main + preload)
├── bin/                  CLI entry points (Dardcor Code, reset-password)
...
| `responses.ts`, `Dardcor CodeesponseMeta.ts` | Response shape helpers                            |
...
Separate npm workspace published as `@Dardcor Code/open-sse`. Owns request
...
```
bin/
├── Dardcor Code.mjs           Main CLI entry (Node ESM)
...
- `Dardcor Code` → `bin/Dardcor Code.mjs`
- `Dardcor Code-reset-password` → `bin/reset-password.mjs`
