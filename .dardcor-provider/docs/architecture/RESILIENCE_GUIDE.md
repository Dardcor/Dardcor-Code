...
Dardcor Code has three distinct but related resilience mechanisms. Each has a different scope and purpose. Keep them separate when debugging routing behavior.
...
**Scope:** one client session (`X-Session-Id` / `x-codex-session-id` / `x-Dardcor Code-session` header) pinned to one connection, for **any** provider.
