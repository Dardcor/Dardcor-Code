...
> (typed registry — code lands with [#5868](https://github.com/diegosouzapw/Dardcor Code/pull/5868))
...
1. **Lifecycle** (`RouterBackendLifecycle`) — _how the engine runs_:
   - `in-process` — runs inside the Dardcor Code Node process (the native TS pipeline).
   - `supervised` — a local child process Dardcor Code installs/starts/stops/health-checks
     via `ServiceSupervisor`, then consumes as a provider connection.
   - `external` — an HTTP endpoint Dardcor Code dispatches to but does **not** manage
...
[#5868](https://github.com/diegosouzapw/Dardcor Code/pull/5868)) declares every engine once, with its
...
[#5817](https://github.com/diegosouzapw/Dardcor Code/pull/5817); until it merges,
