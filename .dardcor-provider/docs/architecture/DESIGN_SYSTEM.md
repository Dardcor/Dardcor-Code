...
# Dardcor Code — Design System & Visual Identity
...
> **Scope:** the Dardcor Code dashboard (`src/`) and the marketing site (`_mono_repo/Dardcor CodeSite/`) share **one visual identity** — same graph-paper grid background (32px), same color tokens, standardized components.
>
...
The marketing site (`viral.Dardcor Code.online`, `why.Dardcor Code.online`, `Dardcor Code.online`) and the product dashboard should look like **one product**. The site already borrowed its palette from the dashboard — its `css/tokens.css` even says _"Palette mirrors the Dardcor Code dashboard (src/app/globals.css)"_. So the two are already ~80% aligned at the color level. What's missing on the dashboard:
...
The exact recipe from the site (`_mono_repo/Dardcor CodeSite/css/base.css`): a **fixed, full-viewport pseudo-element** painting two 1px line gradients, sitting at `z-index:-1` behind all content.
...
| Site reference                    | `_mono_repo/Dardcor CodeSite/css/tokens.css`, `css/base.css`                                                            |
