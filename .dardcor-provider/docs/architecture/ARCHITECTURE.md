---
title: "Dardcor Code Architecture"
version: 3.8.40
...
Dardcor Code can install, supervise, and route to locally-running AI tool processes
...
- Dardcor Code response metadata builder: `src/domain/Dardcor CodeesponseMeta.ts`
...
- file: `${DATA_DIR}/storage.sqlite` (or `$XDG_CONFIG_HOME/Dardcor Code/storage.sqlite` when set, else `~/.Dardcor Code/storage.sqlite`)
...
> Dardcor Code v3.8.0. For the canonical and continuously-updated list, refer to
...
- final client response returned by Dardcor Code; streamed responses are stored in the same compact summary form
...
1. `usageDb` and `localDb` share the same base directory policy (`DATA_DIR` -> `XDG_CONFIG_HOME/Dardcor Code` -> `~/.Dardcor Code`) with legacy file migration.
...
5. The `open-sse/` directory is published as the `@Dardcor Code/open-sse` **npm workspace package**. Source code imports it via `@Dardcor Code/open-sse/...` (resolved by Next.js `transpilePackages`). File paths in this document still use the directory name `open-sse/` for consistency.
...
- Build Docker image: `docker build -t Dardcor Code .`
