...
This document is the authoritative reference for all CI quality gates in Dardcor Code.
...
| `nightly-schemathesis` | OpenAPI contract fuzzing (schemathesis) against a live Dardcor Code using `docs/openapi.yaml` — surfaces spec violations / unhandled 500s (Fase 8 B.4) | **Advisory** |
...
Beyond the CI gates, Dardcor Code ships an **opt-in** `agent-lsp` scaffold
