...
Embedded services are locally-installed process sidecar tools that Dardcor Code installs, supervises, and
exposes as first-class routing targets. Unlike external providers (which are reached over the internet
via API keys), embedded services run on the same machine as Dardcor Code and communicate over loopback.
...
| --------------- | ----------------------------------------------- | :----------: | ------------------------------------------------------------------------------------------------------------------ |
| **9Router**     | `9router`                                      |    20130     | AI router that Dardcor Code can use as a sub-provider. Models exposed as `9router/{sub}/{model}`                     |
...
- Dardcor Code installs them under `DATA_DIR/services/{name}/` (isolated from Dardcor Code's own `package.json`)
- Dardcor Code spawns and monitors them as child processes
- Dardcor Code injects an ephemeral API key into the child's environment and rotates it without downtime (where applicable)
...
| API key management                    | Dardcor Code generates, encrypts at-rest (AES-256-GCM), and injects via env |
...
`package.json` and `node_modules/`. Does not conflict with Dardcor Code's own deps.
...
the next time Dardcor Code boots (if the service is installed).
...
- Strips incoming `cookie` and `authorization` headers (no leakage of Dardcor Code session)
...
2. Verify `npm` is in PATH: `which npm` from the same user account that runs Dardcor Code.
...
Dardcor Code's default port 20128.
...
- `DATA_DIR` or its parent is not writable by the Dardcor Code process.
- Running inside Docker rootless without write access to the mapped volume.

**Fix:**

1. Check `DATA_DIR` (default: `~/.Dardcor Code/`): `ls -la ~/.Dardcor Code/`
2. Ensure the Dardcor Code process user owns the directory: `chown -R $USER ~/.Dardcor Code/`
...
3. Try the install manually: `npm install {package}@latest --prefix ~/.Dardcor Code/services/{name}/`.
...
Yes. Both services follow the same local-first model as Dardcor Code itself. They run
on the same machine and communicate over loopback. "Production" here means the VPS
or local server where Dardcor Code is deployed, not a remote cloud provider.
...
2. Check structured logs in Dardcor Code's pino output filtered by
   `service:supervisor` namespace.
3. Inspect the DB row: `sqlite3 ~/.Dardcor Code/Dardcor Code.db "SELECT * FROM version_manager WHERE tool='9router'"`.
