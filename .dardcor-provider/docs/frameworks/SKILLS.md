...
Dardcor Code exposes an extensible Skills framework that lets language models (and operators) compose reusable capabilities — from filesystem reads and HTTP requests to sandboxed code execution and curated marketplace skills.

A skill is a versioned, schema-defined unit of work. Dardcor Code can inject skills as tool definitions into outbound requests, intercept tool calls coming back from the model, run the matching handler, and feed the result back to the model so the conversation can continue. The model never sees the implementation — only the tool interface.

---

## Agent Skills vs Omni Skills

Dardcor Code has two distinct but complementary skill systems:
...
| Who uses it     | Dardcor Code itself (combo routing, inbound LLM calls)           | External agents, MCP clients, A2A orchestrators                                             |
...
| Discovery       | `/api/skills/*` REST + `Dardcor Code_skills_*` MCP tools         | `/api/agent-skills/*` REST + `Dardcor Code_agent_skills_*` MCP tools + A2A `list-capabilities` |

**Omni Skills** are the execution engine — they define what Dardcor Code _can do_ when an LLM invokes a tool.

**Agent Skills** are the documentation catalog — they explain to external agents _how to use_ Dardcor Code's REST API and CLI, with structured SKILL.md files that can be fed directly into agent prompts.
...
1. **Built-in skills** (`src/lib/skills/builtins.ts`) — shipped with Dardcor Code. Cover the common cases:
...
2. **SkillsMP** (the Dardcor Code Marketplace) — fetched from `https://skillsmp.com/api/v1/skills/search`. Requires `skillsmpApiKey` in Settings.
...
2. Built-in tool aliases (e.g. `Dardcor Code_web_search` → `web_search`) are resolved first. Built-in handlers run inline.
...
`file_read` and `file_write` resolve every path relative to a per-API-key workspace at `<DATA_DIR>/skills/workspaces/<sha256(apiKeyId).slice(0,24)>/`. Path traversal (`..`) and forbidden segments (`.env`, `.git`, `.ssh`, `.Dardcor Code`, `.codex`, `secrets`) are rejected before any disk I/O.
...
| Tool                          | Description                                                  |
| ----------------------------- | ------------------------------------------------------------ |
| `Dardcor Code_skills_list`       | List skills, optional filters: `apiKeyId`, `name`, `enabled` |
| `Dardcor Code_skills_enable`     | Enable/disable a skill by `skillId`                          |
| `Dardcor Code_skills_execute`    | Execute a skill with an input payload                        |
| `Dardcor Code_skills_executions` | Recent execution history (default 50, max 100)               |
...
- **Audit executions:** `/dashboard/skills/executions` and `Dardcor Code_skills_executions` both query `skill_executions`. Successful runs include `durationMs`; failures include `errorMessage`.
...
```ts
import { skillExecutor } from "Dardcor Code/skills/executor";
...
Dardcor Code ships with a curated set of built-in skills in `src/lib/skills/builtin/`. The most common ones:
