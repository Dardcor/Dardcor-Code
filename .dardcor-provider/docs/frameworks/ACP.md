...
> **TL;DR**: ACP lets Dardcor Code spawn CLI agents (like Claude Code, Codex) as child processes instead of using HTTP APIs. This gives you "CLI-as-backend" transport.

---

## What Is ACP?

ACP (Agent Client Protocol) is a **"CLI-as-backend" transport** for Dardcor Code. Instead of intercepting HTTP API calls to AI providers, ACP **spawns CLI agents as child processes** and feeds prompts through their native interface.
...
Once detected, ACP can be used as a transport for any supported provider. Dardcor Code will automatically use ACP when the CLI is available.
...
2. **Check ownership**: Ensure Dardcor Code has read/execute permissions
