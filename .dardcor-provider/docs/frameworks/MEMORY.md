...
Dardcor Code provides persistent conversational memory keyed by API key (and
...
> A client can opt a single request out with the `x-Dardcor Code-no-memory`
...
- `searchSemanticMemory(query, topK, scope)` — embed the query, search the
  collection filtered by `kind = "Dardcor Code_memory"` and optionally by
...
`Dardcor Code_memory_search` tool.
...
`qdrantApiKey`, `qdrantCollection` default `"Dardcor Code_memory"`,
...
- `Dardcor Code_memory_clear` — `{apiKeyId, type?, olderThan?}` → lists matching
...
- For hard deletion, use `DELETE /api/memory/[id]` or `Dardcor Code_memory_clear`.
...
Dardcor Code's memory engine supports **four embedding sources** (`src/lib/memory/embedding/`). Each has different trade-offs in **latency, cost, model quality, and setup complexity**.
