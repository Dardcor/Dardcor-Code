...
Dardcor Code exposes three different agent-related surfaces. They look similar at first glance but solve different problems. Use this page to pick the right one.
...
| **ACP — CLI Agents Registry** | Detecting / registering / launching CLI coding agents installed on the user's machine (Cursor, Cline, Codex CLI, Claude Code, Aider, etc.) | HTTP REST                   | Dardcor Code-specific   |
| **Cloud Agents**              | Submitting long-running coding tasks to external cloud services (Codex Cloud, Devin, Jules, Cursor Cloud)                                  | HTTP REST + DB-backed tasks | Dardcor Code-specific   |
...
- Building a multi-agent system where Dardcor Code is one of the peers
- Exposing Dardcor Code's routing intelligence (smart-routing, quota-management, etc.) to agents in frameworks like Google ADK or generic agent meshes
- Wrapping Dardcor Code behind a standard discovery + invocation surface
...
ACP is Dardcor Code's **local CLI agent inventory**. It detects which coding CLIs are installed on the host (Cursor, Cline, Claude Code, Codex CLI, Continue, etc.), resolves their versions, and surfaces them to the dashboard so the user can wire each CLI to point at Dardcor Code.
...
- Dashboard "CLI Tools" page lists what's installed and helps you point each at Dardcor Code
- Custom agents let power users register internal/proprietary CLIs that Dardcor Code doesn't know about by default
- Detection result fuels the `cli-tools` fingerprint matrix

### When NOT to use ACP

- ACP doesn't _run_ tasks. It only detects + configures CLIs. To actually invoke a CLI, you launch it yourself with the env vars Dardcor Code provides (`OPENAI_BASE_URL`, `OPENAI_API_KEY`, etc.).
...
A uniform interface over third-party cloud coding agents. You submit a prompt + repo URL, Dardcor Code dispatches to the right cloud agent, polls status, returns results.
...
| Where compute runs | Inside Dardcor Code (uses configured combos)                                         | External (Codex / Devin / Jules servers) |
...
### Discover Dardcor Code's A2A capabilities
...
### Call Dardcor Code as an A2A agent
