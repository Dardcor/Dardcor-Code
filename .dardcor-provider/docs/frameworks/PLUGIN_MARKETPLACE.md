...
Dardcor Code ships a WordPress-style plugin system. Plugins are self-contained
...
Plugins live under the Dardcor Code data directory:
...
`<home>/.Dardcor Code/plugins`, where `<home>` is taken from the `HOME` /
