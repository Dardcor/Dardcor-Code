...
Dardcor Code can fire HTTP webhooks on platform events. Use them to integrate with
...
When a webhook has a `secret`, Dardcor Code signs the JSON body and sends:
...
> Header names use the `X-Webhook-*` prefix (not `X-Dardcor Code-*`). The signature
