...
Traffic Inspector is Dardcor Code's built-in HTTPS traffic debugger — a Charles Proxy / mitmweb / HTTP Toolkit-like tool that is **LLM-aware** and **agent-aware**. It lives at `/dashboard/tools/traffic-inspector` and receives live traffic from up to 5 simultaneous capture sources.
...
| Feature                                                             | mitmweb | Charles | Fiddler | **Dardcor Code Traffic Inspector** |
| ------------------------------------------------------------------- | :-----: | :-----: | :-----: | :-----------------------------: |
| Web-based                                                           |    ✓    |    ✗    |    ✗    |                ✓                |
| Open-source                                                         |    ✓    |    ✗    | partial |                ✓                |
...
| **Integrated with Dardcor Code** routing, fallback, cost               |    ✗    |    ✗    |    ✗    |                ✓                |
...
interface LlmMetadata {
  ...
  mappedTo: string | null; // x-Dardcor Code-mapped header
  costEstimateUsd: number | null; // estimated cost based on Dardcor Code pricing
}
```
...
If Dardcor Code crashes while system-wide proxy mode is active:
...
| POST   | `/requests/{id}/replay`     | Re-execute the same request through Dardcor Code router                               |
