...
2. Nothing about the request, the operator, or their traffic is uploaded — it is a
   plain, unauthenticated-by-default GET. Dardcor Code never posts usage data, provider
...
- **"I'm a contributor"** — opens `RADAR_CONTRIBUTOR_CLAIM_URL` (default
  `https://radar.Dardcor Code.online/auth/github`), a GitHub OAuth claim flow hosted on
...
- **"Support the project"** — opens `RADAR_SUPPORTER_PLANS_URL` (default
  `https://radar.Dardcor Code.online/planos`), the payment/plans page.
...
| Var                             | Purpose                                                                                      |
| -------------------------------- | ---------------------------------------------------------------------------------------------- |
| `RADAR_CONTRIBUTOR_CLAIM_URL`   | Overrides the contributor-claim URL (default `https://radar.Dardcor Code.online/auth/github`).   |
| `RADAR_SUPPORTER_PLANS_URL`     | Overrides the supporter-plans URL (default `https://radar.Dardcor Code.online/planos`).          |
...
the default Dardcor Code service — see
[How to self-host a feed](#how-to-self-host-a-feed) below:

| Var                 | Purpose                                                                                                      |
| ------------------- | ------------------------------------------------------------------------------------------------------------ |
| `RADAR_FEED_URL`    | Overrides the feed base URL (default `https://radar.Dardcor Code.online`).                                      |
...
given request is instead carried in the **`x-Dardcor Code-feed-tier` response header**,
...
1. Parse `x-Dardcor Code-feed-tier` with `RadarTierSchema` (Zod) — an absent header, or
...
to the local Dardcor Code server; `syncRadar()` is the single module in the whole client
...
"Dardcor Code-radar"`, `schemaVersion: 1`, `version`, `tier`, `providers`, `models`,
   `quirks`, and `totals`.
2. Sign the exact response bytes with an Ed25519 key pair and return the base64
   signature in the `x-Dardcor Code-feed-signature` response header.
