---
title: "Dardcor Code MCP Server Documentation"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code MCP Server Documentation
...
Dardcor Code MCP is built-in. Start it with:
...
| :------------------------------ | :-------------------- | :------------------------------------------------------------ |
| `Dardcor Code_get_health`          | `read:health`         | Uptime, memory, circuit breakers, rate limits, cache stats    |
| `Dardcor Code_list_combos`         | `read:combos`         | All configured combos with strategies (optional metrics)      |
| `Dardcor Code_get_combo_metrics`   | `read:combos`         | Performance metrics for a specific combo                      |
| `Dardcor Code_switch_combo`        | `write:combos`        | Activate or deactivate a combo                                |
| `Dardcor Code_check_quota`         | `read:quota`          | Quota used/total, percent remaining, reset time, token health |
| `Dardcor Code_route_request`       | `execute:completions` | Send a chat completion through Dardcor Code routing              |
| `Dardcor Code_cost_report`         | `read:usage`          | Cost report by period (session/day/week/month)                |
| `Dardcor Code_list_models_catalog` | `read:models`         | Full model catalog with capabilities, status, pricing         |

## Phase 1 — Search

| Tool                   | Scopes           | Description                                                                                                                        |
| :--------------------- | :--------------- | :--------------------------------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_web_search` | `execute:search` | Web search through Dardcor Code search gateway (Serper/Brave/Perplexity/Exa/Tavily/Google PSE/Linkup/SearchAPI/SearXNG) with failover |

## Advanced Tools (11) — Phase 2

| Tool                               | Scopes                               | Description                                                                               |
| :--------------------------------- | :----------------------------------- | :---------------------------------------------------------------------------------------- |
| `Dardcor Code_simulate_route`         | `read:health`, `read:combos`         | Dry-run routing simulation with fallback tree                                             |
| `Dardcor Code_set_budget_guard`       | `write:budget`                       | Session budget with degrade/block/alert action                                            |
| `Dardcor Code_set_routing_strategy`   | `write:combos`                       | Update combo strategy at runtime (priority/weighted/auto/etc.)                            |
| `Dardcor Code_set_resilience_profile` | `write:resilience`                   | Apply `aggressive` / `balanced` / `conservative` resilience preset                        |
| `Dardcor Code_test_combo`             | `execute:completions`, `read:combos` | Live test of every provider in a combo using a real upstream call                         |
| `Dardcor Code_get_provider_metrics`   | `read:health`                        | Per-provider metrics with p50/p95/p99 latency and circuit breaker state                   |
| `Dardcor Code_best_combo_for_task`    | `read:combos`, `read:health`         | Recommend combo by task type with budget/latency constraints                              |
| `Dardcor Code_explain_route`          | `read:health`, `read:usage`          | Explain why a request was routed to a provider (scoring factors + fallbacks)              |
| `Dardcor Code_get_session_snapshot`   | `read:usage`                         | Full session snapshot: cost, tokens, top models/providers, errors, budget guard           |
| `Dardcor Code_db_health_check`        | `read:health`, `write:resilience`    | Diagnose (and optionally auto-repair) database drift like broken combo refs / orphan rows |
| `Dardcor Code_sync_pricing`           | `pricing:write`                      | Sync pricing data from external sources (LiteLLM); supports `dryRun`                      |

## Cache Tools (2)

| Tool                    | Scopes        | Description                                         |
| :---------------------- | :------------ | :-------------------------------------------------- |
| `Dardcor Code_cache_stats` | `read:cache`  | Semantic cache, prompt-cache, and idempotency stats |
| `Dardcor Code_cache_flush` | `write:cache` | Flush cache globally or by signature/model          |

## Compression Tools (13)

| Tool                                | Scopes              | Description                                                                                                              |
| :---------------------------------- | :------------------ | :----------------------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_compression_status`      | `read:compression`  | Compression settings, analytics summary, and cache-aware stats (includes `analytics.mcpDescriptionCompression` metadata) |
| `Dardcor Code_compression_configure`   | `write:compression` | Configure compression mode, threshold, target ratio, system-prompt preservation, MCP description compression toggle      |
| `Dardcor Code_set_compression_engine`  | `write:compression` | Pick the active engine (off/caveman/rtk/stacked) and Caveman/RTK intensity                                               |
| `Dardcor Code_list_compression_combos` | `read:compression`  | List named compression combos and their engine pipelines                                                                 |
| `Dardcor Code_compression_combo_stats` | `read:compression`  | Analytics grouped by compression combo and engine                                                                        |
| `Dardcor Code_ccr_store`               | `write:compression` | Store caller-isolated content in the bounded in-memory CCR store and return a marker plus `ccr://` reference             |
| `Dardcor Code_ccr_retrieve`            | `read:compression`  | Retrieve CCR content in full or with head, tail, lines, grep, and stats modes                                            |
| `Dardcor Code_ccr_inspect`             | `read:compression`  | Inspect caller-owned CCR metadata without returning content                                                              |
| `Dardcor Code_ccr_list`                | `read:compression`  | List paginated metadata for caller-owned CCR blocks                                                                      |
| `Dardcor Code_ccr_delete`              | `write:compression` | Delete a caller-owned CCR block                                                                                          |
| `Dardcor Code_ccr_stats`               | `read:compression`  | Report caller-scoped memory usage, lifecycle counters, and store limits                                                  |
| `Dardcor Code_rtk_discover`            | `read:compression`  | Discover recurring noise in opt-in RTK output samples                                                                    |
| `Dardcor Code_rtk_learn`               | `read:compression`  | Generate a reviewable RTK filter draft from opt-in samples                                                               |
...
`Dardcor Code_compression_status` reports MCP description compression separately under
...
Separate from the compression tools above, Dardcor Code includes a post-execution filter that
...
| Tool                        | Scopes         | Description                                                                             |
| :-------------------------- | :------------- | :-------------------------------------------------------------------------------------- |
| `Dardcor Code_oneproxy_fetch`  | `read:proxies` | Fetch free proxies from the 1proxy marketplace (protocol/country/quality/limit filters) |
| `Dardcor Code_oneproxy_rotate` | `read:proxies` | Get the next available proxy by strategy (`random` / `quality` / `sequential`)          |
| `Dardcor Code_oneproxy_stats`  | `read:proxies` | Pool stats, sync status, distribution by protocol and country                           |
...
| Tool                      | Scopes         | Description                                                                         |
| :------------------------ | :------------- | :---------------------------------------------------------------------------------- |
| `Dardcor Code_memory_search` | `read:memory`  | Search memories by query / type / API key with token-budget enforcement             |
| `Dardcor Code_memory_add`    | `write:memory` | Add a new memory entry (`factual` / `episodic` / `procedural` / `semantic`)         |
| `Dardcor Code_memory_clear`  | `write:memory` | Clear memories for an API key, optionally filtered by type or `olderThan` timestamp |
...
| Tool                          | Scopes           | Description                                                                       |
| :---------------------------- | :--------------- | :-------------------------------------------------------------------------------- |
| `Dardcor Code_skills_list`       | `read:skills`    | List registered skills with optional filtering by API key, name, or enabled state |
| `Dardcor Code_skills_enable`     | `write:skills`   | Enable or disable a specific skill by ID                                          |
| `Dardcor Code_skills_execute`    | `execute:skills` | Execute a skill with provided input and return the execution record               |
| `Dardcor Code_skills_executions` | `read:skills`    | List recent skill execution history                                               |
...
| Tool                              | Scopes         | Description                                                                                                      |
| :-------------------------------- | :------------- | :--------------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_agent_skills_list`     | `read:catalog` | List all 42 agent skills with optional `category` (api\|cli) and `area` filters; returns metadata + coverage     |
| `Dardcor Code_agent_skills_get`      | `read:catalog` | Get full metadata + SKILL.md content for a single skill by canonical `id`                                        |
| `Dardcor Code_agent_skills_coverage` | `read:catalog` | Coverage stats: how many of the 22 API and 20 CLI skills have SKILL.md files on the filesystem vs catalog totals |
...
Dardcor Code through the same connection model used for LLM providers. They are exposed via
...
| :-------------------------------------- | :--------------------------------- | :----------------------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_BASE_URL`                    | `http://localhost:20128`           | Base URL the MCP server uses when calling Dardcor Code internal APIs                                                        |
...
| `DATA_DIR`                              | `~/.Dardcor Code`                     | Heartbeat file is written to `${DATA_DIR}/runtime/mcp-heartbeat.json`                                                    |
...
- Realtime stats are surfaced via `Dardcor Code_compression_status` under `analytics.mcpDescriptionCompression` and tagged `source: "mcp_metadata_estimate"` to disambiguate from real provider usage receipts.
