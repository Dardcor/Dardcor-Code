...
Dardcor Code connects to an **Obsidian** vault as a **context source** — a local Markdown
...
vault sync (driven by `src/lib/obsidianSync.ts`). Enabling it points Dardcor Code at a
...
per call (per-API-key first, then global). Tools that hit the Dardcor Code **sync server**
(the four `obsidian_sync_*` tools) additionally require the sync auth token configured
in Dardcor Code settings.
...
| `obsidian_sync_status`       | Dardcor Code sync server status: running, vault name, port, uptime, last sync.          |
