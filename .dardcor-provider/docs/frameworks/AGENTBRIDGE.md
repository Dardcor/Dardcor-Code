...
AgentBridge is Dardcor Code's MITM (Man-in-the-Middle) proxy that intercepts HTTPS traffic from IDE AI agents and reroutes it through Dardcor Code's unified routing engine. It supports **9 IDE agents** — Antigravity, Kiro, GitHub Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code, and Trae (investigating) — making Dardcor Code the broadest-coverage MITM proxy for AI coding assistants on the market.
...
- **Apply Dardcor Code resilience**: combo routing, circuit breakers, fallbacks, and cost tracking work for IDE agent traffic too.
...
> into the same `Dardcor Code-mitm.crt` trust-store slot the old leaf used
...
AgentBridge intercepts credentials (OAuth tokens, API keys) that the IDE uses to authenticate with upstream providers. These are **masked before logging** (see §2.7) but are visible to Dardcor Code's MITM layer. First activation of each agent shows a dismissible risk notice modal.
...
The bypass list ensures that financial institutions, OAuth/SSO providers, and other sensitive hosts are **never decrypted**. Their TLS traffic passes through as a transparent TCP tunnel — Dardcor Code never sees the plaintext.
...
1. Verify the cert was installed: `security find-certificate -c "Dardcor Code AgentBridge"` (macOS) or `certutil -L -d sql:$HOME/.pki/nssdb` (Linux/NSS)
...
2. Check Dardcor Code server logs: `APP_LOG_LEVEL=debug` in `.env`
