...
> **TL;DR**: `open-sse/` is the core streaming engine that powers every LLM request in Dardcor Code. It contains ~900 files implementing the request pipeline, executors, services, MCP server, and translation layer. This guide explains how the pieces fit together.
...
`open-sse/` is a **standalone workspace** in the Dardcor Code monorepo for several reasons:

1. **Reusability** — `open-sse` is published as `@Dardcor Code/open-sse` on npm, so other projects can use it independently
2. **Clean boundaries** — the streaming engine is decoupled from the Dardcor Code-specific UI/DB layer
...
The Responses API is OpenAI's new format with **stateful conversations** (`previous_response_id`). When a client sends a Responses request, Dardcor Code:
