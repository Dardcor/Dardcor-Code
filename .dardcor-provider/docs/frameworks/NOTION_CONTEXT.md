...
Dardcor Code can connect to a **Notion** workspace as a **context source** — a read/write
...
blocks — all proxied through Dardcor Code (with retry, timeout, and error classification)
...
> Dardcor Code to access with that integration (Notion's permission model is share-based,
> not workspace-wide).
...
Dardcor Code saves the token and immediately runs a 1-result test search; if Notion
