...
Dardcor Code compression is built around engine contracts. A mode can run one engine directly
...
actually reach `Dardcor Code_ccr_retrieve`** (`callerSupportsCcrRetrieve()` in
...
with a `22-87%` range, and a `~46%` input-compression tool. Dardcor Code uses the Caveman input-side
...
Dardcor Code install; when the optional stack is absent, LLMLingua still fail-opens. Only the three
...
- **Global npm (`npm i -g Dardcor Code`) / standalone** — run the install command above inside
...
| Tool                                | Scope               | Purpose                          |
| ----------------------------------- | ------------------- | -------------------------------- |
| `Dardcor Code_compression_status`      | `read:compression`  | Settings, analytics, cache stats |
| `Dardcor Code_compression_configure`   | `write:compression` | Update global settings           |
| `Dardcor Code_set_compression_engine`  | `write:compression` | Set mode and optional pipeline   |
| `Dardcor Code_list_compression_combos` | `read:compression`  | List compression combos          |
| `Dardcor Code_compression_combo_stats` | `read:compression`  | Read combo/engine analytics      |
