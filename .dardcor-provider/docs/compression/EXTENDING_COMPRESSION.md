...
> **TL;DR**: Dardcor Code's compression engine is **pluggable** — you can register custom engines, ship language packs for new languages, and compose stacked pipelines. This guide shows how.
...
Caveman-style compression uses **language-specific rule packs** to handle fillers, hedging, and verbose patterns in each natural language. Dardcor Code ships with **6 language packs**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
Dardcor Code selects **ONE mode per request** based on configuration, auto-trigger thresholds, and combo overrides.
...
Dardcor Code's compression engines credit several upstream projects in the README
...
pack, how does that reach Dardcor Code?** This section is the authoritative answer.
...
rule/filter in Dardcor Code's own format** (see
...
Dardcor Code's own `smartcrusher` / `toon` / `tabular` layers. There is no live
...
1. **Don't vendor** — re-express the upstream rule/filter in Dardcor Code's format.
