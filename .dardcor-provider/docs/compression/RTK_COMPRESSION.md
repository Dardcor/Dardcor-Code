...
RTK compression is Dardcor Code's command-aware compression engine for terminal and tool output. It is
...
`~118,000` standard tokens to `~23,900` RTK tokens, which is `79.7%` saved (`~80%`). Dardcor Code uses
...
Within the same scope, RTK TOML schema v1 filters take precedence over Dardcor Code JSON filters. TOML
...
Dardcor Code can parse, validate, test, and install declarative filter files using RTK TOML schema v1.
...
Dardcor Code receives tool output after the client has already captured it, so `filter_stderr = true`
...
`rtkEngine` (`@Dardcor Code/open-sse/services/compression/engines/rtk`) is a
...
- User directory: `~/.Dardcor Code/rtk/filters/`
