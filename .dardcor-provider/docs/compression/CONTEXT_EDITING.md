...
Delegated **Context Editing** is a Claude-only context-management feature. Unlike Dardcor Code's local
...
tool-use / tool-result blocks from its own running context window. Dardcor Code only attaches a body
parameter (`context_management.edits[]`); Claude does the actual clearing against its own tokenizer.

This is a delegated capability by nature: other providers reject the parameter, so Dardcor Code scopes
...
Dardcor Code injects a single edit into the outbound Anthropic Messages body:
...
Dardcor Code already emits on Claude requests.
...
left as-is. If a `clear_thinking_20251015` edit is also present, Dardcor Code stable-sorts the
...
After a Claude response, Dardcor Code records how much context the provider actually cleared. This is
...
| ----------------- | --------------------------------------------------- | ------------------------------------------- |
| Where it runs     | In Dardcor Code, before the request leaves the proxy   | In the provider (Claude), server-side       |
...
The two are complementary: local engines compress the bytes Dardcor Code sends; Context Editing lets
