...
> Feature design + implementation notes for Dardcor Code's operator-level proxy
> subscription flow. This is the v1 cut: a single operator pastes subscription
> links, picks a mode (global or rule), and Dardcor Code binds the resulting proxy
...
Today, Dardcor Code's proxy pool is hand-curated: every node lives in
...
into Dardcor Code and re-bind them whenever the upstream list changes.

The goal of v1 is to make Dardcor Code first-class for **operator-supplied**
...
| U5 | Operator | supply a local sing-box/clash SOCKS5 endpoint | SS/VMess/Trojan/VLESS nodes (which Dardcor Code's dispatcher can't speak natively) become usable through a local kernel bridge |
