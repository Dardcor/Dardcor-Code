---
title: "CLI Tools — Dardcor Code"
version: 3.8.40
lastUpdated: 2026-06-28
---

# CLI Tools — Dardcor Code

Last updated: 2026-06-28

Dardcor Code integrates with three categories of CLI tools spread across three dedicated dashboard pages:

| Page           | Route                   | Concept                                                                   | Count        |
| -------------- | ----------------------- | ------------------------------------------------------------------------- | ------------ |
| **CLI Code's** | `/dashboard/cli-code`   | Coding tools you point at Dardcor Code (Client → CLI → Dardcor Code → Provider) | 21           |
| **CLI Agents** | `/dashboard/cli-agents` | Autonomous agents you point at Dardcor Code (same flow, broader scope)       | 6            |
| **ACP Agents** | `/dashboard/acp-agents` | CLIs that Dardcor Code spawns as backend via stdio/ACP (reverse flow)        | see registry |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
           │
           ▼  (all point to Dardcor Code)
    http://YOUR_SERVER:20128/v1
           │
           ▼  (Dardcor Code routes to the right provider)
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP Agents (reverse spawn flow):
    Client request → Dardcor Code → spawns CLI via stdio/ACP → response
```
...
You do not have to write each tool's config by hand. Dardcor Code ships a `setup-*`
command per supported CLI that reads the **live** model catalog from a running
Dardcor Code (local or remote) and writes the tool's own config on your machine:

```bash
Dardcor Code setup-codex        Dardcor Code setup-claude       Dardcor Code setup-opencode
Dardcor Code setup-cline        Dardcor Code setup-kilo         Dardcor Code setup-continue
Dardcor Code setup-cursor       Dardcor Code setup-roo          Dardcor Code setup-crush
Dardcor Code setup-goose        Dardcor Code setup-qwen         Dardcor Code setup-aider
```

Each accepts `--remote <url> --api-key <key>` (configure a local tool against a
remote Dardcor Code), `--dry-run` (preview without writing), and `--port`. Tools
...
`Dardcor Code launch` (Claude Code) and `Dardcor Code launch-codex` (Codex) spawn the CLI
...
This page (renamed from `/dashboard/agents`) shows CLIs that Dardcor Code can **spawn** as backend execution engines via stdio/ACP protocol. The catalog is maintained separately in `src/lib/acp/registry.ts` and is **not** the same as `CLI_TOOLS`.
...
| `POST /api/cli-tools/grok-build-settings`   | Grok Build (~/.grok/config.toml, `[model.Dardcor Code]`) |
...
### Step 1 — Get an Dardcor Code API Key
...
```bash
# Dardcor Code Universal Endpoint
export OPENAI_BASE_URL="http://localhost:20128/v1"
export OPENAI_API_KEY="sk-your-Dardcor Code-key"
export ANTHROPIC_BASE_URL="http://localhost:20128"
export ANTHROPIC_AUTH_TOKEN="sk-your-Dardcor Code-key"
export GEMINI_BASE_URL="http://localhost:20128/v1"
export GEMINI_API_KEY="sk-your-Dardcor Code-key"
```
...
mkdir -p ~/.claude && cat > ~/.claude/settings.json << EOF
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "sk-your-Dardcor Code-key"
  }
}
EOF
```
...
model: auto
apiKey: sk-your-Dardcor Code-key
...
> Use `opencode run "your prompt" --model Dardcor Code/claude-sonnet-4-5-thinking --variant high`
...
mkdir -p ~/.cline/data && cat > ~/.cline/data/globalState.json << EOF
{
  ...
  "openAiApiKey": "sk-your-Dardcor Code-key"
}
EOF
```
...
Or use the Dardcor Code dashboard → **CLI Tools → Cline → Apply Config**.
...
```bash
kilocode --api-base http://localhost:20128/v1 --api-key sk-your-Dardcor Code-key
```
...
```json
{
  ...
  "kilo-code.apiKey": "sk-your-Dardcor Code-key"
}
```

Or use the Dardcor Code dashboard → **CLI Tools → KiloCode → Apply Config**.
...
Use this when VS Code Insiders is configured for custom endpoint models and you want Dardcor Code to work without a custom header field.
...
**Example using the tokenized Dardcor Code alias:**
...
[
  {
    ...
    "id": "auto",
    "name": "Dardcor Code Auto",
    "family": "gpt-4",
    "version": "1.0.0",
    "url": "http://localhost:20128/api/v1/vscode/sk-your-Dardcor Code-key/chat/completions",
    "modelsUrl": "http://localhost:20128/api/v1/vscode/sk-your-Dardcor Code-key/models",
...
- Replace `sk-your-Dardcor Code-key` with an API key created in Dardcor Code.
...
# The CLI uses its own auth — Dardcor Code is not needed as backend for Kiro CLI itself.
# Use kiro-cli alongside Dardcor Code for other tools.
kiro-cli status
```

For the **Kiro IDE** desktop app, use the MITM endpoint exposed by Dardcor Code
under `/dashboard/cli-tools → Kiro`.

---

## 10. Internal Dardcor Code CLI

The `Dardcor Code` binary provides commands for server lifecycle, setup, diagnostics, and provider management. Entry point: `bin/Dardcor Code.mjs`.

```bash
Dardcor Code                              # Start server (default port 20128)
Dardcor Code setup                        # Interactive setup wizard
Dardcor Code doctor                       # Check config, DB, ports, runtime
Dardcor Code providers list               # Configured provider connections
Dardcor Code providers test-all           # Test every active connection
Dardcor Code reset-password               # Reset the admin password
Dardcor Code logs                         # Stream request logs
Dardcor Code health                       # Detailed health (breakers, cache, memory)
Dardcor Code --version                    # Print version
Dardcor Code --help                       # Show all commands
```
...
```bash
Dardcor Code setup                        # Interactive setup wizard
Dardcor Code setup --non-interactive      # CI/automation mode (reads env vars + flags)
Dardcor Code setup --password '<value>'   # Set admin password directly
Dardcor Code setup --add-provider \
  --provider openai \
  --api-key '<value>' \
...
| `DATA_DIR`          | Override the Dardcor Code data directory                          |

All other non-interactive inputs are passed as flags, not environment variables:
`--password`, `--provider`, `--provider-name`, `--provider-base-url`, `--default-model`
(see the `Dardcor Code setup` options above).

### Diagnostics

```bash
Dardcor Code doctor                       # Check config, DB, ports, runtime, memory, liveness
Dardcor Code doctor --json                # Machine-readable JSON
Dardcor Code doctor --no-liveness         # Skip the HTTP health probe
Dardcor Code doctor --host 0.0.0.0        # Override liveness host
Dardcor Code doctor --liveness-url <url>  # Full health endpoint URL override
```
...
```bash
Dardcor Code providers available                       # Dardcor Code provider catalog
Dardcor Code providers available --search openai       # Filter catalog by id/name/alias/category
Dardcor Code providers available --category api-key    # Filter by category (api-key, oauth, free, ...)
...
Dardcor Code providers test-all                        # Test every active connection
Dardcor Code providers validate                        # Local-only structural validation
```

> `providers available` reads the Dardcor Code catalog; `providers list/test/test-all/validate`
> read the local SQLite database directly and do not require the server to be running.

### Recovery & Reset

```bash
Dardcor Code reset-password                # Reset the admin password (also: Dardcor Code-reset-password)
Dardcor Code reset-encrypted-columns       # Show warning + dry-run for encrypted credential reset
Dardcor Code reset-encrypted-columns --force  # Actually null out encrypted credentials in SQLite
```

### Credential Export (⚠ handle with care)

```bash
Dardcor Code auth export                                 # Show warning + confirmation gate — no DB access
Dardcor Code auth export --force                          # Export ALL connections' DECRYPTED credentials to stdout as JSON
Dardcor Code auth export --force --id <id>                 # Export only the matching connection
Dardcor Code auth export --force --format env               # Emit OMNIROUTE_<PROVIDER>_<FIELD>=<value> lines
Dardcor Code auth export --force --out creds.json           # Write to a file (created with 0600 permissions)
```
...
These assume a running Dardcor Code server, unless noted otherwise:

```bash
Dardcor Code status                       # Comprehensive runtime status
Dardcor Code logs                         # Stream request logs (--json, --search, --follow)
Dardcor Code config show                  # Display current configuration

Dardcor Code provider list                # List available providers (alias of providers list)
Dardcor Code provider add                 # Register Dardcor Code as a provider on a tool
Dardcor Code keys add | list | remove     # Manage API keys
Dardcor Code models [provider]            # List models (--json, --search)
Dardcor Code combo list | switch | create | delete

Dardcor Code backup                       # Snapshot config + DB
Dardcor Code restore                      # Restore from a previous snapshot

Dardcor Code health                       # Detailed health (breakers, cache, memory)
Dardcor Code quota                        # Provider quota usage
Dardcor Code cache                        # Cache status
Dardcor Code cache clear                  # Clear semantic + signature caches

Dardcor Code mcp status | restart         # MCP server status / restart
Dardcor Code a2a status | card            # A2A server status / agent card

Dardcor Code tunnel list | create | stop  # Manage tunnels (cloudflare/tailscale/ngrok)
Dardcor Code env show | get <k> | set <k> <v>  # Inspect / set env vars (temporary)

Dardcor Code test                         # Provider connectivity smoke test
Dardcor Code update                       # Check for updates
Dardcor Code completion                   # Generate shell completion
```
...
Ready-to-paste examples with a tokenized Dardcor Code URL:
...
| Error                                        | Cause                   | Fix                                              |
| -------------------------------------------- | ----------------------- | ------------------------------------------------ |
| `Connection refused`                         | Dardcor Code not running   | `Dardcor Code serve`                                |
