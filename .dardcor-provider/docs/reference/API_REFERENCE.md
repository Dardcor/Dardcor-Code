...
Complete reference for all Dardcor Code API endpoints.
...
| ------------------------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `X-Dardcor Code-No-Cache`   | Request   | Set to `true` to bypass cache                                                                                                                                                     |
| `x-Dardcor Code-no-memory`  | Request   | Set to `true` to skip memory + skills injection for this request (mirrors no-cache; avoids the per-call token/cost overhead)                                                      |
| `X-Dardcor Code-Progress`   | Request   | Set to `true` for progress events                                                                                                                                                 |
| `X-Session-Id`           | Request   | Sticky session key for external session affinity                                                                                                                                  |
| `x_session_id`           | Request   | Underscore variant also accepted (direct HTTP)                                                                                                                                    |
| `X-Dardcor Code-Session-Id` | Request   | Caller-supplied session/conversation tag (also feeds memory). When present, persisted verbatim to `call_logs.session_tag` for per-session cost attribution (#8249) — never synthesized when absent |
| `Idempotency-Key`        | Request   | Dedup key (5s window)                                                                                                                                                             |
| `X-Request-Id`           | Request   | Alternative dedup key                                                                                                                                                             |
| `X-Dardcor Code-Cache`      | Response  | `HIT` or `MISS` (non-streaming)                                                                                                                                                   |
| `X-Dardcor Code-Idempotent` | Response  | `true` if deduplicated                                                                                                                                                            |
| `X-Dardcor Code-Progress`   | Response  | `enabled` if progress tracking on                                                                                                                                                 |
| `X-Dardcor Code-Session-Id` | Response  | Effective session ID used by Dardcor Code                                                                                                                                            |
| `X-Dardcor Code-Request-Id` | Response  | Request correlation id (when known)                                                                                                                                               |
| `X-Dardcor Code-Version`    | Response  | Dardcor Code build version (always present)                                                                                                                                          |
| `X-Dardcor Code-Cost-Saved` | Response  | USD the cache avoided on a HIT (cache hits only)                                                                                                                                  |
| `X-Dardcor Code-Decision`   | Response  | Routing trace: `strategy=<name>; provider=<alias>; latency_ms=<n>` (`<name>` is the combo strategy, or `single` for a non-combo request) — always present on completion responses |

> Nginx note: if you rely on underscore headers (for example `x_session_id`), enable `underscores_in_headers on;`.

> **Cost telemetry headers:** non-streaming success responses also carry the `X-Dardcor Code-*` cost-telemetry set — `X-Dardcor Code-Response-Cost` (USD, fixed 10 decimals; `0.0000000000` for free/unpriced), `X-Dardcor Code-Tokens-In` / `X-Dardcor Code-Tokens-Out`, `X-Dardcor Code-Model`, `X-Dardcor Code-Provider`, `X-Dardcor Code-Latency-Ms`, `X-Dardcor Code-Cache-Hit`, and `X-Dardcor Code-Fallback-Attempts` (only when > 0), plus `X-Dardcor Code-Request-Id` and `X-Dardcor Code-Version`. These are emitted by chat completions, `/v1/responses`, `/v1/messages`, **and the media endpoints** — `/v1/embeddings`, `/v1/images/generations`, `/v1/audio/speech`, `/v1/audio/transcriptions`, `/v1/rerank`, `/v1/videos/generations`, `/v1/music/generations`, and `/v1/moderations` (always cost `0`). Media cost is computed per modality (per-image, per-second, per-character, per search-unit) when pricing is available, otherwise `0` (fail-open).

> **Cache-hit cost semantics:** on a semantic-cache HIT (`X-Dardcor Code-Cache-Hit: true`) no upstream call is made, so `X-Dardcor Code-Response-Cost` is `0.0000000000` (the **incremental** cost of serving the hit). The original/would-have-been cost is reported separately in `X-Dardcor Code-Cost-Saved`. Billing consumers should sum `X-Dardcor Code-Response-Cost` (hits cost nothing); cache analytics can aggregate `X-Dardcor Code-Cost-Saved`.

### `x-Dardcor Code-compression`
...
```
X-Dardcor Code-Compression: <mode>; source=<source>
```
...
- Remote media URLs must be public HTTPS. Dardcor Code fetches them server-side with redirect
...
For thinking-capable Claude models, `/v1/models` also advertises a **no-thinking** variant whose id is prefixed with `claude-3-Dardcor Code-no-thinking/`:

```
claude-3-Dardcor Code-no-thinking/<provider>/<model>
```
...
For clients that cannot attach `Authorization: Bearer ...`, Dardcor Code also accepts API keys in the URL via either query-string compatibility (`?token=...`, `?apiKey=...`, `?api_key=...`, `?key=...`) or the dedicated `/api/v1/vscode/{token}/...` endpoints documented below.
...
a ChatGPT account`). Send the **bare** id (e.g. `gpt-5.5`). Dardcor Code's bridge is
...
Point the Codex CLI at Dardcor Code by adding a custom provider with WebSocket
...
env_key = "OMNIROUTE_API_KEY"             # holds the Dardcor Code API key (Bearer)
```

```bash
export OMNIROUTE_API_KEY=sk-...           # an Dardcor Code API key (any key if REQUIRE_API_KEY=false)
codex exec "Responda apenas: PONG"
```

The CLI upgrades `base_url + /responses` to a WebSocket and Dardcor Code tunnels it
...
```json
{
  ...
  "backupPath": "/home/user/.Dardcor Code/backups/env-repair-2026-04-11.bak"
}
```
...
- URL-based tokens may appear in reverse-proxy logs, browser history, and telemetry outside Dardcor Code. Treat them as a compatibility option, not the default authentication mode.
...
Outbound webhook subscriptions for Dardcor Code events (request completion, quota exhaustion, key rotation, etc.).
...
Cloud agent tasks (Claude Code, Codex Cloud, OpenHands, etc.) executed remotely on behalf of Dardcor Code users.
...
Dardcor Code exposes three independent temporary-failure mechanisms; the management endpoints below let operators read and override them:
...
Skill framework for extending Dardcor Code with custom executable handlers, plus marketplace integrations.
...
Dardcor Code ships an embedded Model Context Protocol server with 3 transports (stdio, SSE, streamable-http) and scoped tools. The dashboard endpoints below read status/audit data and proxy the HTTP transports.
...
Dardcor Code exposes an A2A (Agent-to-Agent) JSON-RPC 2.0 endpoint plus a REST wrapper for inspection/dashboard use.
...
Manage CLI tools that integrate with Dardcor Code (antigravity, chipotle, commandCode,
...
Manage Dardcor Code plugins (third-party extensions).
...
Inspect the runtime guardrails (PII detection, prompt injection detection, vision bridging). Guardrails run on every request; per-call opt-out is via the `x-Dardcor Code-disabled-guardrails` request header — there is no persisted enable/disable surface.
