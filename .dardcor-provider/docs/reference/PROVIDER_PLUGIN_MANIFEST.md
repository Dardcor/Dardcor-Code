...
Dardcor Code advertises that URL to Bifrost and CLIProxyAPI via the
`X-Dardcor Code-Provider-Manifest-Url` request header. Set
...
eventually be owned by a lower-latency sidecar while Dardcor Code keeps the
