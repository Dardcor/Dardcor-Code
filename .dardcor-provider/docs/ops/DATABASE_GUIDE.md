...
> **TL;DR**: Dardcor Code uses **SQLite with WAL journaling** as its primary store, with **AES-256-GCM** encryption at rest for sensitive fields. This guide covers the schema, migrations, backup/recovery, and operational runbooks.
...
Dardcor Code chose SQLite over PostgreSQL/MySQL for several reasons:
...
For **single-user, single-instance** deployments (the primary Dardcor Code use case), SQLite is simpler and faster.
...
Dardcor Code's database has **94 domain modules** in `src/lib/db/`. Each module:
...
Dardcor Code has **94 module files** in `src/lib/db/`. Below is a sampling of core modules; see the directory listing for the complete list:
...
Dardcor Code uses **versioned, idempotent migrations** in `src/lib/db/migrations/`. Each migration is a single SQL file named `NNN_description.sql`.
...
1. Creates `_Dardcor Code_migrations` table if not exists
...
> **Backwards-incompatible changes** (e.g., dropping columns) are tricky. Dardcor Code does NOT support downgrade — once a migration is applied, the schema change is permanent. Plan accordingly.
...
Dardcor Code uses **`migrateLegacyEncryptedString()`** to handle two encryption schemes transparently:
...
This uses SQLite's online backup API — safe to run while Dardcor Code is running.
...
Long-running Dardcor Code instances benefit from occasional `VACUUM`:
...
- If persistent, restart Dardcor Code
...
1. **Stop Dardcor Code** (prevent further attempts)
2. **Check the DB state** with `sqlite3`
3. **Manually fix** the partial migration
4. **Re-run** Dardcor Code (the migration will be retried)
