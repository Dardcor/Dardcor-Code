...
1. `npm stage list Dardcor Code` — find the stage id (also printed in the workflow summary).
...
`Dardcor Code` in stage-only mode so a leaked long-lived token cannot `npm publish`
directly from anywhere — CI can only stage; only the owner's 2FA releases.

**Broken-artifact playbook (unchanged):** `npm deprecate Dardcor Code@<bad> "<reason> — use <fixed>"`
...
- [ ] Test on fresh install: delete `~/.Dardcor Code/Dardcor Code.db` and run `npm run dev`
...
> **Operator note:** the remote VPS image directory remains `/usr/lib/node_modules/Dardcor Code/app/`.
...
- [ ] `Dardcor Code --tray` boots on macOS (systray2 installed into `~/.Dardcor Code/runtime/`)
- [ ] `Dardcor Code --tray` boots on Linux (requires DISPLAY; graceful error if not set)
- [ ] `Dardcor Code --tray` boots on Windows (PowerShell NotifyIcon, no extra binaries)
- [ ] `Dardcor Code config tray enable` creates autostart entry; disable removes it
- [ ] `npm install -g Dardcor Code@<this-version>` runs postinstall without fatal exit
- [ ] Update path keeps optional deps: `Dardcor Code update --apply` and the auto-updater
...
- [ ] `Dardcor Code status` works with no `.env` (CLI token path, loopback only)
...
- [ ] All 10 `skills/Dardcor Code*/SKILL.md` files are publicly fetchable via raw GitHub URL
