...
> **TL;DR**: Dardcor Code ships with built-in health monitoring, provider autopilot, quota tracking, and observability hooks. This guide covers the dashboard, alerts, and troubleshooting.
...
Dardcor Code has **3 layers of monitoring**:
...
│  └─ MCP tools: Dardcor Code_get_health, Dardcor Code_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
Dardcor Code supports **3 alert channels**:
...
Create a `~/.Dardcor Code/dashboard.json`:
...
- Dardcor Code's quota tracking may be **stale** — the provider's truth is upstream
...
- Run `sqlite3 ~/.Dardcor Code/storage.sqlite "PRAGMA integrity_check;"`
- If "ok" — false alarm, the health check is being too strict
- If anything else — **stop Dardcor Code** and follow the [disaster recovery guide](./DATABASE_GUIDE.md#disaster-recovery)
