---
title: "Dardcor Code — Deployment Guide on VM with Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code — Deployment Guide on VM with Cloudflare
...
Complete guide to install and configure Dardcor Code on a VM (VPS) with domain managed via Cloudflare.
...
## 2. Install Dardcor Code

### 2.1 Create configuration directory

```bash
mkdir -p /opt/Dardcor Code
```

### 2.2 Create environment variables file

```bash
cat > /opt/Dardcor Code/.env << 'EOF'
# === Security ===
...
# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.Dardcor Code.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.Dardcor Code.online
EOF
```
...
```bash
docker pull diegosouzapw/Dardcor Code:latest

docker run -d \
  --name Dardcor Code \
  --restart unless-stopped \
  --env-file /opt/Dardcor Code/.env \
  -p 20128:20128 \
...
```bash
docker ps | grep Dardcor Code
docker logs Dardcor Code --tail 20
```
...
```bash
cat > /etc/nginx/sites-available/Dardcor Code << 'NGINX'
...
Keep reverse-proxy stream timeouts aligned with your Dardcor Code timeout env vars. If you raise
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, raise `proxy_read_timeout` / `proxy_send_timeout`
above the same threshold.

Dardcor Code uses `NEXT_PUBLIC_BASE_URL` as the canonical browser-facing origin for OAuth
...
`OMNIROUTE_TRUST_PROXY` if Dardcor Code is not directly reachable by clients and your proxy
...
# Enable Dardcor Code
ln -sf /etc/nginx/sites-available/Dardcor Code /etc/nginx/sites-enabled/Dardcor Code
...
docker run -d --name Dardcor Code --restart unless-stopped \
  --env-file /opt/Dardcor Code/.env \
  -p 20128:20128 \
  -v Dardcor Code-data:/app/data \
  diegosouzapw/Dardcor Code:latest
...
```bash
docker logs -f Dardcor Code          # Real-time stream
docker logs Dardcor Code --tail 50   # Last 50 lines
```
...
docker cp Dardcor Code:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v Dardcor Code-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/Dardcor Code-data-$(date +%F).tar.gz /data
```
...
docker run --rm -v Dardcor Code-data:/data -v $(pwd):/backup \
  alpine sh -c "rm -rf /data/* && tar xzf /backup/Dardcor Code-data-YYYY-MM-DD.tar.gz -C /"
docker start Dardcor Code
```
...
cd Dardcor CodeCloud
npm install
npx wrangler login
npx wrangler deploy
```

See also [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) for the in-repo Cloudflare Tunnel walkthrough. The standalone `Dardcor CodeCloud/` worker lives in a separate companion repo.
...
| 20128 | Dardcor Code   | Localhost only (via nginx) |
...
- **Monitor with `top` / `free -m`** — Dardcor Code typically uses 200-400 MB RSS at idle on a 1 GB VM.
