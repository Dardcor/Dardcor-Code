...
Dardcor Code can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — Dardcor Code starts/stops the underlying
...
- **Auto-install.** On first use, Dardcor Code downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  Dardcor Code. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, Dardcor Code uses it instead of downloading. |
...
must be configured in the ngrok dashboard — Dardcor Code itself only forwards the
...
1. Install Tailscale (or let Dardcor Code do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via Dardcor Code's `login` endpoint).
...
Dardcor Code's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose Dardcor Code through a tunnel, the dashboard and OAuth flows must
...
and restart Dardcor Code before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; Dardcor Code will track each
independently.
...
Dardcor Code attempts to auto-install on first use. If the install is blocked
