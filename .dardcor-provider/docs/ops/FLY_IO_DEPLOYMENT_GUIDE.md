---
title: "Dardcor Code Fly.io Deployment Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code Fly.io Deployment Guide

This document describes the actual deployment process for Dardcor Code on Fly.io, covering two scenarios:
...
This guide is based on a verified working configuration for the current project. The application name is `Dardcor Code`.
...
- Access URL: `https://Dardcor Code.fly.dev/`
...
- `app = 'Dardcor Code'` determines which Fly application the deployment targets
...
```toml
app = 'Dardcor Code-yourname'
```

Note:

- Make sure the application you see in the console matches the `app` value in `fly.toml`
- If you previously used a different name, such as `oroute`, do not confuse it with `Dardcor Code`
...
```powershell
flyctl apps create Dardcor Code
```

If you changed the application name, replace `Dardcor Code` with your chosen name.
...
These parameters have been used in actual deployments on the current `Dardcor Code` application:
...
| ---------------------- | --------------------------- |
| `DATA_DIR`             | `/data`                     |
| `NEXT_PUBLIC_BASE_URL` | `https://Dardcor Code.fly.dev` |
...
1. **Set `NEXT_PUBLIC_BASE_URL` to your public HTTPS domain**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://Dardcor Code.fly.dev -a Dardcor Code
   ```

   If you are using a custom domain, replace it with the corresponding domain (e.g. `https://Dardcor Code.yourdomain.com`).
...
2. **Configure the callback URL on the provider console**
   ...
   - `https://Dardcor Code.fly.dev/callback`
...
- Intended for the current project `Dardcor Code`
...
```bash
flyctl secrets set OMNIROUTE_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a Dardcor Code
```
...
```powershell
flyctl secrets set INITIAL_PASSWORD=your-strong-password -a Dardcor Code
```
...
```powershell
flyctl secrets list -a Dardcor Code
```

If the `Secrets` page in the console does not show the expected variables, check:

- That you are viewing the `Dardcor Code` application
...
```powershell
flyctl secrets set KEY=value -a Dardcor Code
```
...
If the current repository is a fork and you want to sync updates from the upstream `https://github.com/diegosouzapw/Dardcor Code`, follow the workflow below.
...
```powershell
git remote add upstream https://github.com/diegosouzapw/Dardcor Code.git
```
...
5. `flyctl deploy`
6. `flyctl status -a Dardcor Code`
7. `flyctl logs --no-tail -a Dardcor Code`
...
```powershell
flyctl status -a Dardcor Code
```

### 10.2 View Startup Logs

```powershell
flyctl logs --no-tail -a Dardcor Code
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://Dardcor Code.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- You are viewing a different application (e.g. `oroute` instead of `Dardcor Code`)
...
1. `flyctl auth login`
2. `flyctl apps create Dardcor Code`
3. `flyctl secrets set ... -a Dardcor Code`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a Dardcor Code`
