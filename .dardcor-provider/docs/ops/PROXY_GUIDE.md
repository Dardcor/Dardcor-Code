---
title: "🌐 Dardcor Code Proxy Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🌐 Dardcor Code Proxy Guide

> **Bypass geographic blocks, protect your identity, and route AI traffic through any proxy — with zero configuration complexity.**

Dardcor Code includes a full-featured proxy management system that lets you route upstream AI provider traffic through HTTP, HTTPS, or SOCKS5 proxies. Whether you're in a blocked region, need IP rotation, or want stealth fingerprinting — this guide covers everything.
...
Dardcor Code supports proxy configuration at **four independent scopes**, resolved in priority order:
...
When Dardcor Code sends a request to an upstream provider, it calls `resolveProxyForConnectionFromRegistry()` which checks each level in order:
...
Proxies are included in the **Backup/Restore** system. When you export your Dardcor Code configuration:
...
If you configured proxies in an older version (pre-registry), Dardcor Code automatically migrates them:
...
> 🆕 **Contributed by [@oyi77](https://github.com/oyi77)** — PR [#1847](https://github.com/diegosouzapw/Dardcor Code/pull/1847) (Issue [#1788](https://github.com/diegosouzapw/Dardcor Code/issues/1788))

Dardcor Code integrates with the **[1proxy](https://1proxy-api.aitradepulse.com)** community platform to provide access to **hundreds of free, validated proxies** from around the world. This is perfect for users who don't have their own proxy infrastructure.
...
1. **Sync** — Dardcor Code fetches validated proxies from the 1proxy API
...
Dardcor Code doesn't just route traffic through a proxy — it makes the traffic look legitimate:
...
For providers that use the CLIProxyAPI pattern, Dardcor Code supports three upstream proxy modes:

| Mode          | Description                                        |
| ------------- | -------------------------------------------------- |
| `native`      | Dardcor Code handles proxy routing directly (default) |
...
This is normal with cheap proxies that drop idle connections. Dardcor Code already handles this by:
...
Make sure the proxy is configured **before** starting the OAuth flow. Dardcor Code routes OAuth token exchange through the configured proxy. Set a global or provider-level proxy first, then connect.
...
Dardcor Code's **proxy fast-fail** mechanism (`src/lib/proxyHealth.ts`) detects dead proxies in <2s via a quick TCP connection check, then **caches the result** to avoid per-request overhead.
...
Dardcor Code tracks per-proxy usage to help operators diagnose routing patterns, latency spikes, and recurring failures.

### What's Tracked

For every request through a configured proxy, Dardcor Code records:
...
When multiple proxies are assigned to a scope, Dardcor Code uses a **rotation strategy** to pick which one to use for each request. The strategy is configured at the scope level (global, per-provider, per-account, per-combo).
