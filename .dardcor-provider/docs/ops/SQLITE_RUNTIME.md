...
Dardcor Code resolves its SQLite driver at startup through a 5-step fallback chain:
...
2. **Runtime-installed `better-sqlite3`** (in `~/.Dardcor Code/runtime/`)
...
- **Windows EBUSY**: `npm install -g Dardcor Code@latest` can fail if the previous
  version's `better_sqlite3.node` is locked by a running process. The runtime
  install in `~/.Dardcor Code/runtime/` sidesteps the global npm cache.
- **No build tools**: Some environments (corporate Windows without VS Build
  ...
  drivers ensure Dardcor Code still boots even if that fails.
- **Air-gapped systems**: If the npm registry is unreachable, `node:sqlite`
  or `sql.js` guarantee baseline functionality.

## Magic-byte validation

Before loading a runtime-installed `.node` file, Dardcor Code reads the first 8
