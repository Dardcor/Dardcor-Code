...
Dardcor Code CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-Dardcor Code-cli-token`
request header.

This allows CLI subcommands (`Dardcor Code status`, `Dardcor Code providers`, etc.)
...
3. The CLI sends the token as `x-Dardcor Code-cli-token` on every request to
...
Default salt: `Dardcor Code-cli-auth-v1`
