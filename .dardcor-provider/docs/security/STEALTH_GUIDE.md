...
Dardcor Code integrates with providers whose edges actively fingerprint non-official clients (TLS JA3/JA4, header ordering, JSON body shape, integrity tokens). This page documents the stealth surfaces Dardcor Code exposes and where they are implemented.

## Legal and Ethical Notice

Stealth features exist so Dardcor Code can act as a compatibility layer between user-owned official accounts (Claude Code CLI, ChatGPT Desktop/Web, Antigravity, Cursor, etc.) and Dardcor Code's unified API. They are **not** for evading fraud detection, sharing credentials, or violating provider Terms of Service. The maintainers expect operators to comply with the upstream ToS they signed when creating accounts.
...
When `cliCompatMode` is on, Dardcor Code reshapes outgoing Claude requests so they are indistinguishable from `claude-cli` traffic. Three modules collaborate:
...
Server-side integrity check the official Claude Code CLI computes via Bun/Zig. Dardcor Code reimplements with `xxhash-wasm`:
...
Antigravity requests preserve caller text byte-for-byte. Dardcor Code does not insert zero-width characters into prompts or rename/inject tools to imitate an IDE client.
...
The upstream enforcement is on **Google's side**, not anything Dardcor Code can prevent. The env var name and the existing docs make it sound like a safe knob to flip; it isn't.
...
- Multiple Dardcor Code users hitting overage credits in parallel from the same external IP compounds the signal.
...
- Original incident report: Discussion [#1183](https://github.com/diegosouzapw/Dardcor Code/discussions/1183)
...
For CLIs whose binaries cannot be redirected via `OPENAI_BASE_URL`, Dardcor Code runs a local TLS-terminating proxy. Endpoints live under `src/app/api/cli-tools/antigravity-mitm/`.
...
Cert filename: `Dardcor Code-mitm.crt`. Fingerprint match via `getCertFingerprint()` (SHA-1 of DER).

Additionally, `updateNssDatabases()` installs into per-user NSS DBs when `certutil` is available: `~/.pki/nssdb`, `~/snap/chromium/.../nssdb`, all Firefox profiles (including snap), under the nickname **`Dardcor Code MITM Root CA`**.
...
Dardcor Code scrubs inbound client headers before forwarding so a request that arrives from Cursor doesn't leak `User-Agent: Cursor/X.Y.Z` to a Claude upstream. See `src/shared/constants/upstreamHeaders.ts` for the denylist, kept in lockstep with the Zod schemas and unit tests.
