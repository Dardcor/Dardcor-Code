...
description: "Maintainer attestation for the AI-detected potential-malware findings raised against Dardcor Code and the v3.8.6 mitigations applied at each flagged call site."
---

# Socket.dev / supply-chain finding attestation

This document is the maintainer-authored attestation for the six
`AI-detected potential malware` findings raised against `Dardcor Code@3.8.5` and
the mitigations applied in `Dardcor Code@3.8.6`. It exists so:
...
Caddy. The fact that they exist in Dardcor Code is documented at
...
who already use Zed and want to mirror their provider keys into Dardcor Code
without re-pasting.
...
toolkit." Within Dardcor Code they are only used by the MITM cert install path
...
## §5 — Dardcor Code Cloud Sync credential write-back (`api/keys/[id]/route.js`)
...
**Why we keep it**: Cloud Sync is the only way for an Dardcor Code Cloud tenant
...
The resulting bundle is intended to be published as `Dardcor Code-secure`. See
