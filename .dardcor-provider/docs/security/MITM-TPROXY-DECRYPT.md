...
TPROXY transparent decrypt is Dardcor Code's **5th capture mode** for the
...
configuration change** — but the process must trust the dynamic CA Dardcor Code
...
> and installs under the pre-existing `Dardcor Code-mitm.crt` trust-store slot
> (superseding the old single leaf there — no dual-trust cleanup needed),
> kept fully separate from TPROXY's own `Dardcor Code-tproxy-ca.crt` slot below.
...
- Generates a long-lived CA via `generateMitmCa()` (CN `"Dardcor Code MITM CA"`,
...
`Dardcor Code-tproxy-ca.crt` (constant `TPROXY_CA_CERT_NAME`) — kept separate from
the static MITM cert's slot (`Dardcor Code-mitm.crt`) so the two never clobber each
other.
...
host means anything Dardcor Code intercepts can be decrypted. It is gated behind the
...
- Confirm the process trusts the dynamic CA. The CA is installed under
  `Dardcor Code-tproxy-ca.crt`; apps with their own trust store (Firefox/Chrome NSS)
...
mode reverts the rules; if Dardcor Code was killed mid-session, use the AgentBridge
