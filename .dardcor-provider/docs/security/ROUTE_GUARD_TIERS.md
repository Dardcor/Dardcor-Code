...
All Dardcor Code management API routes are classified into one of three protection
...
the operator put Dardcor Code behind an nginx/Cloudflare/Tailscale tunnel and a JWT
leaked, or auth was misconfigured — the attacker turns "call an API" into "run a
command on the host" (remote code execution). Dardcor Code closes this by enforcing a
...
If you run Dardcor Code behind a reverse proxy or tunnel (nginx, Caddy, Cloudflare
...
- A `403 LOCAL_ONLY` in Dardcor Code's logs for one of these paths is the guard
