...
between Dardcor Code and upstream providers. Each guardrail can inspect (and
...
`x-Dardcor Code-modality-bridge: image->text;model=<visionModel>;parts=<n>`
...
**Self-loop admission bypass:** when the describe call routes through Dardcor Code's
own `/v1` self-loop (non-standard provider model), the sub-request sends
`x-Dardcor Code-admission-bypass: internal` and is authenticated with the resolved
self-loop credential — the local `sk_Dardcor Code` sentinel in local mode, or the
...
- Header `x-Dardcor Code-disabled-guardrails` (or legacy
...
- **`garak` (advisory)** — runs garak `--probes promptinject,dan,leakreplay`
  against a local Dardcor Code instance (`http://localhost:20128/v1`). Gated on a
