...
Dardcor Code controls which **browser origins** may read cross-origin responses
...
taking seriously, but Dardcor Code's exposure is **narrower than the generic case**,
...
> `Access-Control-Allow-Credentials: true`. Dardcor Code's shared CORS path never
> does.
...
- If Dardcor Code runs behind a reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
## Example: reverse proxy in front of Dardcor Code

CORS is enforced by Dardcor Code itself, so the proxy generally should **not** add or
rewrite `Access-Control-*` headers (double headers break browsers). Terminate TLS
and forward — let Dardcor Code answer preflight:
...
Set the allowed browser origins in Dardcor Code (`CORS_ALLOWED_ORIGINS` or the
