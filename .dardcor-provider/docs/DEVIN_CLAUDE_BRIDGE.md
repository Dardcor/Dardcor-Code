...
`devin-cli-agentic` lets the real Claude Code runtime use Dardcor Code's local Anthropic
...
Claude Code 2.1.220 (isolated non-root Linux container)
  -> http://Dardcor Code:20128/v1/messages
...
- run as UID/GID `10001:10001`, with a read-only root filesystem, dropped capabilities, and
  `no-new-privileges`;
- use a private `/home/bridge`, a dedicated Claude config volume, isolated Dardcor Code data,
...
- direct Claude Code inference only to `http://Dardcor Code:20128` with a local-only key.

The offline profile uses an internal network. In the live profile, Dardcor Code reaches the
official Devin endpoints only through `network-guard`; unrelated destinations are denied.
Claude Code has a separate deny-all egress guard and can reach only the local Dardcor Code
...
- `docker compose -f docker/devin-bridge/compose.yml --profile offline logs Dardcor Code`
