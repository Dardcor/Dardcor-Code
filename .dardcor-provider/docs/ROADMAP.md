---
title: "Dardcor Code Roadmap"
version: 3.8.50
lastUpdated: 2026-08-06
---

# Dardcor Code Roadmap
...
Dardcor Code is heading from a monolithic router to a **modular AI platform**: a lightweight
...
- **`stable/v3`** — the LTS line (3.9.x). Receives fixes, security patches, and provider
  updates. `npm install Dardcor Code` (aka `latest`) stays on v3 during the whole v4 cycle.
...
- **`@Dardcor Code/core`** (npm name stays `Dardcor Code`) — just the engine: `/v1/*`, routing,
  combo/fallback, providers.
- **`@Dardcor Code/sdk`** — one typed contract: hooks, extension points, two-phase lifecycle,
...
- **Modules** (`@Dardcor Code/mod-*`) — cloud agents, traffic inspection (MITM), evals,
