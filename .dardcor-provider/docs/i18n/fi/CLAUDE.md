...
**Dardcor Code** — yhtenäinen AI-proxy/reititin. Yksi päätepiste, yli 160 LLM-toimittajaa, automaattinen varajärjestelmä.
...
Dardcor Code:lla on kolme liittyvää mutta erilaista tilapäisen epäonnistumisen mekanismia. Pidä niiden
...
- **Tuonnit**: ulkoiset → sisäiset (`@/`, `@Dardcor Code/open-sse`) → suhteelliset
...
- **Polkualias**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Oletusportti**: 20128 (API + dashboard samalla portilla)
- **Tietohakemisto**: `DATA_DIR` ympäristömuuttuja, oletuksena `~/.Dardcor Code/`
