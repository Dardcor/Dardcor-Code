...
title: "Arquitetura do Dardcor Code"
version: 3.8.2
lastUpdated: 2026-05-13

---

# Arquitetura do Dardcor Code
...
Dardcor Code é um gateway de roteamento de IA local e um painel construído sobre Next.js.  
...
flowchart LR
    ...
    subgraph Router[Processo Local Dardcor Code]
...
- Construtor de metadados de resposta Dardcor Code: `src/domain/Dardcor CodeesponseMeta.ts`
...
- arquivo: `${DATA_DIR}/storage.sqlite` (ou `$XDG_CONFIG_HOME/Dardcor Code/storage.sqlite` quando definido, caso contrário `~/.Dardcor Code/storage.sqlite`)
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Runtime do Dardcor Code]
...
> Dardcor Code v3.8.0. Para a lista canônica e continuamente atualizada, consulte
...
- resposta final do cliente retornada pelo Dardcor Code; respostas transmitidas são armazenadas na mesma forma de resumo compacto
...
1. `usageDb` e `localDb` compartilham a mesma política de diretório base (`DATA_DIR` -> `XDG_CONFIG_HOME/Dardcor Code` -> `~/.Dardcor Code`) com migração de arquivos legados.
...
5. O diretório `open-sse/` é publicado como o pacote de **workspace npm** `@Dardcor Code/open-sse`. O código-fonte o importa via `@Dardcor Code/open-sse/...` (resolvido pelo Next.js `transpilePackages`). Os caminhos de arquivo neste documento ainda usam o nome do diretório `open-sse/` para consistência.
...
- Construir imagem Docker: `docker build -t Dardcor Code .`
