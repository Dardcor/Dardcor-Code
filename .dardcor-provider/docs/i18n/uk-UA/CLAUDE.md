...
**Dardcor Code** — єдиний AI проксі/маршрутизатор. Один кінцевий пункт, 160+ постачальників LLM, автоматичне резервування.
...
Dardcor Code має три пов'язані, але різні механізми тимчасових збоїв. Тримайте їх
...
- **Імпорти**: зовнішні → внутрішні (`@/`, `@Dardcor Code/open-sse`) → відносні
...
- **Псевдоніми шляхів**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Порт за замовчуванням**: 20128 (API + панель управління на одному порту)
- **Директорія даних**: `DATA_DIR` змінна середовища, за замовчуванням `~/.Dardcor Code/`
