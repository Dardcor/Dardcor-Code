...
- before the active account is exhausted, Dardcor Code generates a compact structured summary
- after authentication selects a different account for the same session, Dardcor Code injects
...
If the active provider is enabled in `handoffProviders`, Dardcor Code generates a structured
...
Dardcor Code prepends the stored handoff as a system message. Injection happens only after the
...
At injection time, Dardcor Code converts that payload into a `<context_handoff>` system
