...
# 🚀 Dardcor Code — Darmowa bramka AI

<img src="../../../docs/diagrams/readme-hero.svg" width="100%" alt="Dardcor Code — Nigdy nie przestawaj kodować. Każde narzędzie AI → 268 dostawców — ponad 90 darmowych — przez jeden punkt końcowy. Claude Code, Codex, Cursor, Cline, Copilot i Antigravity w DARMOWE Claude / GPT / Gemini z automatycznym przekierowaniem (auto-fallback). Kaskadowa kompresja RTK + Caveman oszczędza 15–95% tokenów (średnio ~89%) — nigdy nie przekraczaj limitów. 268 dostawców AI · ponad 90 darmowych pakietów · ~1,4 mld darmowych tokenów/mies. · 18 strategii routingu · 0 USD na start."/>
...
**Pytania, wskazówki dotyczące dostawców, plany rozwoju (roadmap) i wsparcie → [Discord](https://discord.gg/U47eFqAXCn) · [Telegram](https://t.me/Dardcor CodeOficial) · WhatsApp [🌍 Global](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 Brasil](https://chat.whatsapp.com/LTSpdFhXTxjH4R6CCNiKWz)**
...
[**🚀 Szybki start**](#-szybki-start) • [**🎯 Komba**](#-komba-combos--flagowa-funkcja) • [**🌐 Dostawcy**](#-268-dostawc%C3%B3w-ai--ponad-90-darmowych) • [**🔌 CLI & MCP**](#-pe%C5%82ne-cli--a2a-i-mcp) • [**🗜️ Kompresja**](#%EF%B8%8F-oszcz%C4%99dzaj-1595-token%C3%B3w--automatycznie) • [**🌍 Strona WWW**](https://Dardcor Code.online)

[💥 Obietnica](#-obietnica) • [🤔 Dlaczego](#-dlaczego-Dardcor Code) • [🏆 Co wyróżnia Dardcor Code](#-co-wyr%C3%B3%C5%BCnia-Dardcor Code) • [🤖 Zgodne CLI](#-zgodne-cli-i-agenci-koduj%C4%85cy) • [🖥️ Gdzie to działa](#%EF%B8%8F-gdzie-dzia%C5%82a-Dardcor Code--wsz%C4%99dzie) • [🔒 Prywatność](#-prywatno%C5%9B%C4%87-i-lokalne-dzia%C5%82anie-local-first) • [🎬 W akcji](#-Dardcor Code-w-akcji) • [📸 Zrzuty ekranu](#-zrzuty-ekranu-z-panelu) • [📧 Wsparcie](#-wsparcie-i-spo%C5%82eczno%C5%9B%C4%87)
...
> **Kombo** to łańcuch modeli, po których Dardcor Code nawiguje **automatycznie**. Wybucha limit, dostawca ulega awarii lub koszty gwałtownie rosną — kombo bezgłośnie przełącza się na kolejny model. **To właśnie sprawia, że Dardcor Code jest niezawodny.** 🛡️

### ⚡ Zero konfiguracji — po prostu użyj `auto`

Nie musisz tworzyć żadnego komba. Ustaw swój model na `auto` (lub jego wariant), a Dardcor Code zbuduje wirtualne kombo z Twoich połączonych dostawców, oceniane na żywo:
...
# 🏆 Co wyróżnia Dardcor Code

</div>

| Funkcja | Dardcor Code | Inne routery |
| -------------------------------------- | ------------------------------------------------------------------- | ------------- |
| 🌐 Dostawcy | **268** | 20–100 |
...
- **🤖 Konfiguracja CLI/agenta jednym poleceniem** — `setup-*` konfiguruje ponad 12 narzędzi programistycznych; `Dardcor Code launch` / `launch-codex` działają bez konfiguracji. → [Integracje CLI](../../../docs/guides/CLI-INTEGRATIONS.md)
- **🛰️ Tryb zdalny** — steruj zdalną instancją Dardcor Code za pomocą tokenów o ograniczonym zakresie (`connect` / `contexts` / `tokens`) + pomocnika OAuth `antigravity` dla instalacji na VPS. → [Tryb zdalny](../../../docs/guides/REMOTE-MODE.md)
...
- **💸 Telemetria kosztów wszędzie** — nagłówki kosztu/użycia `X-Dardcor Code-*` w każdym punkcie końcowym, nagłówek oszczędności z trafień w cache (cache-HIT), limity wydatków USD na klucz. → [Referencja API](docs/reference/API_REFERENCE.md)
- **🧠 Pamięć pod Twoją kontrolą** — domyślnie wyłączona, opcjonalna kwantyzacja wektorowa int8 + stopniowe wygaszanie (typed decay), nagłówek `x-Dardcor Code-no-memory` na żądanie. → [Pamięć](../../../docs/frameworks/MEMORY.md)
...
<sub>📖 Konfiguracja per narzędzie dla wszystkich 33 narzędzi (25 z CLI Code + 8 z CLI Agents) → [`docs/reference/CLI-TOOLS.md`](docs/reference/CLI-TOOLS.md) · 🧩 Wtyczka OpenCode → [`@Dardcor Code/opencode-provider`](https://www.npmjs.com/package/@Dardcor Code/opencode-provider)</sub>
...
# 🖥️ Gdzie działa Dardcor Code — wszędzie
...
| ------------------------- | ---------------------------------------- | --------------------------------------------------------- |
| 📦 **npm (globalnie)** | `npm install -g Dardcor Code` | Jedno polecenie, dowolny system operacyjny |
| 🐳 **Docker** | `docker run … diegosouzapw/Dardcor Code` | Wielonatywność architektur **AMD64 + ARM64** |
...
| 📱 **Android (Termux)** | `pkg install nodejs && npx -y Dardcor Code` | Działa **na Twoim telefonie**, 24/7, bez roota |
| 📲 **PWA** | "Dodaj do ekranu głównego" | Pełny ekran, offline, instalacja z poziomu przeglądarki |
| 🧩 **Wtyczka OpenCode** | `@Dardcor Code/opencode-provider` | Natywna integracja z OpenCode |
...
> Dardcor Code to nie tylko serwer — to **kompletny kokpit w wierszu poleceń** z **ponad 80 poleceniami**, plus otwarte protokoły agentów, dzięki którym agent AI może samodzielnie sterować Dardcor Code.

### ⌨️ Prawdziwe CLI (nie tylko `start`)

```bash
Dardcor Code               # uruchom bramkę + panel (port 20128)
Dardcor Code chat          # interaktywny klient czatu TUI (polecenia ukośnika: /model /combo /skill /memory)
Dardcor Code setup         # kreator pierwszej konfiguracji
Dardcor Code doctor        # diagnozuj dostawców, porty, natywne zależności
```

### 🛰️ Tryb zdalny — uruchom CLI lokalnie, Dardcor Code na VPS

Dardcor Code na serwerze? Steruj nim ze swojego laptopa za pomocą **tego samego CLI**. Zaloguj się raz
za pomocą tokenu dostępu o ograniczonym zakresie; każde kolejne polecenie będzie skierowane do zdalnej maszyny.

```bash
Dardcor Code connect 192.168.0.15            # hasło → token o ograniczonym zakresie, zapisany jako kontekst
Dardcor Code models list                     # ← działa na ZDALNYM serwerze
Dardcor Code configure codex                 # ← wybiera zdalny model, zapisuje lokalny profil Codex
Dardcor Code tokens create --name ci --scope read   # generuj węższe tokeny dla innych maszyn
Dardcor Code contexts use default            # ← przełącz z powrotem na serwer lokalny
```
...
<img src="../../../docs/diagrams/cli-terminal.svg" width="100%" alt="Animowany terminal demonstrujący CLI Dardcor Code — Dardcor Code providers list, Dardcor Code combo list, Dardcor Code health — przechodzący przez obszar ponad 80 poleceń: dostawcy · oauth · klucze · kombo · węzły · modele · pamięć podręczna · kompresja · koszt · użycie · limity · stan techniczny · odporność · telemetria · logi · audyt · mcp · a2a · chmura · pamięć · umiejętności · ewaluacja · tunel · kopia zapasowa · synchronizacja · webhooki · polityka · cennik · tłumacz · symulacja …"/>

</div>

### 🤝 Połącz agenta — i pozwól mu kontrolować samo Dardcor Code

Udostępnij Dardcor Code przez **MCP** lub **A2A**, a każdy zdolny do tego agent autonomicznie otrzyma klucze do całej bramki — routingu, dostawców, kombinacji (combos), pamięci podręcznej, kompresji i pamięci.

| Protokół | Punkt końcowy | Zastosowanie |
| ------------------ | ----------------------------------------------- | ------------------------------------------------------- |
| 🧰 **MCP (stdio)** | `Dardcor Code --mcp`                               | Podłącz do Claude Desktop, Cursor, dowolnego klienta MCP |
...
```bash
# Daj Claude Code pełny zestaw narzędzi Dardcor Code przez MCP:
...
<img src="../../../docs/diagrams/compression-pipeline.svg" width="100%" alt="Potok kompresji Dardcor Code: żądanie klienta o rozmiarze 10 000 tokenów przechodzi przez 11 ułożonych kaskadowo silników — Session-Dedup, CCR, RTK, Headroom, Relevance, Caveman, LLMLingua-2, Omniglyph, Lite, Aggressive, Ultra — i dociera do dostawcy mając około 1080 tokenów, co daje do 95% oszczędności. Kod, adresy URL i JSON są zawsze zachowywane z dokładnością co do bajtu."/>
...
- **🛞 Miejsce decyzji o kompresji** (priorytet od najwyższego do najniższego) — nagłówek `x-Dardcor Code-compression` w żądaniu › nadpisanie w kombie routingu › aktywny profil nazwany › adaptacyjny / automatyczny wyzwalacz › domyślne ustawienie panelu › wyłączone. Zastosowany plan jest zwracany w nagłówku odpowiedzi `X-Dardcor Code-Compression: <tryb>; source=<źródło>`.
...
Powinieneś zobaczyć listę połączonych modeli. 🎉 To wszystko — zacznij kodować, a Dardcor Code automatycznie zajmie się routingiem i przełączaniem awaryjnym.

Jeśli Twój klient nie może wysyłać niestandardowych nagłówków, Dardcor Code udostępnia również stokenizowane aliasy zgodności:
...
docker run -d --name Dardcor Code --restart unless-stopped --stop-timeout 40 \
  -p 20128:20128 -v Dardcor Code-data:/app/data diegosouzapw/Dardcor Code:latest
```
...
```bash
OMNIROUTE_SKIP_POSTINSTALL=1 npm install -g Dardcor Code   # CI=1 również to pomija
```
...
> 🎬 **Nagrałeś film o Dardcor Code?** Otwórz [zgłoszenie (issue)](https://github.com/diegosouzapw/Dardcor Code/issues/new) lub [dyskusję (discussion)](https://github.com/diegosouzapw/Dardcor Code/discussions) z linkiem — umieścimy go tutaj.
...
- 🌍 **Strona internetowa**: [Dardcor Code.online](https://Dardcor Code.online)
- 🐙 **GitHub**: [github.com/diegosouzapw/Dardcor Code](https://github.com/diegosouzapw/Dardcor Code)
- 🐛 **Zgłoszenia (Issues)**: [zgłoś błąd](https://github.com/diegosouzapw/Dardcor Code/issues) (dołącz wynik działania komendy `npm run system-info`)
...
- **Strona WWW**: [Dardcor Code.online](https://Dardcor Code.online)
- **Pakiet**: [npmjs.com/package/Dardcor Code](https://www.npmjs.com/package/Dardcor Code)
- **Docker**: [hub.docker.com/r/diegosouzapw/Dardcor Code](https://hub.docker.com/r/diegosouzapw/Dardcor Code)
...
| [Tryb zdalny](../../../docs/guides/REMOTE-MODE.md) | Steruj zdalnym Dardcor Code (VPS) z poziomu CLI na swoim laptopie za pomocą tokenów o ograniczonym zakresie |
| [Konfiguracja Claude Code](../../../docs/guides/CLAUDE-CODE-CONFIGURATION.md) | Skieruj Claude Code na Dardcor Code (lokalnie/zdalnie) za pomocą polecenia launch + profili dla poszczególnych modeli |
...
| [Podręcznik Termux](../../../docs/guides/TERMUX_GUIDE.md) | Uruchamianie Dardcor Code na systemie Android za pomocą Termux |
...
> Dardcor Code jest kształtowane przez pasjonatów ze społeczności open-source. Te osoby wniosły wyjątkowy wkład, który bezpośrednio wpływa na jakość, stabilność i zasięg projektu. **Dziękujemy.**
...
> 🙏 Funkcje, poprawki błędów i ulepszenia infrastruktury wprowadzone przez tych autorów stanowią **kluczową część** tego, co czyni Dardcor Code niezawodnym i bogatym w funkcje. Każde żądanie ściągnięcia (pull request), każdy przypadek testowy i każdy plik tłumaczenia i18n ma znaczenie. Open source tworzą ludzie tacy jak oni.
...
Dardcor Code stoi na barkach gigantów. Projekt powstał jako fork **[9router](https://github.com/decolua/9router)** oraz port na TypeScript projektu w Go **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** — a stamtąd każdy z poniższych podsystemów był inspirowany projektem open-source, który powstał wcześniej. Każdy z nich ukształtował konkretny element Dardcor Code. To jest nasze podziękowanie dla nich wszystkich. 🙏

> ⭐ liczba gwiazdek na lipiec 2026 r. — zachęcamy do dodania gwiazdki tym projektom.

### 🧬 Rodowód i bramka (gateway)

| Projekt | ⭐ | Jak zainspirował Dardcor Code |
| ------------------------------------------------------------------------------- | ----: | ------------------------------------------------------------------------------------------------------------------------------------- |
...
| Projekt | Jak współgra z Dardcor Code |
| ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **[CodeWebChat](https://github.com/robertpiosik/CodeWebChat)** · robertpiosik | Towarzysz po stronie edytora — rozszerzenie VS Code + przeglądarki, które automatycznie uzupełnia ponad 15 interfejsów webowych chatbotów kontekstem z edytora. Obsługuje ścieżkę darmowego UI webowego obok ścieżki API Dardcor Code; pozwala skierować swój tryb API na Dardcor Code. |

## 💖 Sponsorzy

> **Chcesz się tutaj pojawić?** Dardcor Code prezentuje sponsorów przed jedną z najszybciej rozwijających się społeczności bramki AI na GitHubie — i publicznie informujemy, dokąd trafia każdy sponsorowany token. Skontaktuj się z nami: [diegosouza.pw@outlook.com](mailto:diegosouza.pw@outlook.com)
...
Dardcor Code jest darmowym projektem o otwartym kodzie źródłowym, tworzonym i utrzymywanym jawnie. Jeśli pozwala Ci zaoszczędzić czas lub pieniądze, rozważ wsparcie rozwoju:
...
- 🐛 **Zgłaszaj błędy i dziel się opiniami** w [Dyskusjach](https://github.com/diegosouzapw/Dardcor Code/discussions)
...
**[⬆ Powrót do góry](#-Dardcor Code--darmowa-bramka-ai)** · Stworzono z ❤️ dla społeczności AI open-source.

<sub>Dardcor Code v3.8.49 · Node ≥22.22.2 · Licencja MIT · <a href="https://Dardcor Code.online">Dardcor Code.online</a></sub>
