...
Usługi wbudowane to lokalnie instalowane procesy sidecar, które Dardcor Code instaluje, nadzoruje i
udostępnia jako pełnoprawne cele routingu. W przeciwieństwie do zewnętrznych providerów (osiąganych przez internet
przez klucze API), usługi wbudowane działają na tej samej maszynie co Dardcor Code i komunikują się przez loopback.
...
| --------------- | ---------------------------------------------- | :-----------: | -------------------------------------------------------------------------------------------------------------------------- |
| **9Router**     | `9router`                                      |     20130     | Router AI, którego Dardcor Code może używać jako sub-providera. Modele jako `9router/{sub}/{model}`                           |
...
- Dardcor Code instaluje je pod `DATA_DIR/services/{name}/` (odizolowane od własnego `package.json` Dardcor Code)
- Dardcor Code uruchamia je i monitoruje jako procesy potomne
- Dardcor Code wstrzykuje efemeryczny klucz API do środowiska potomka i rotuje go bez przestoju (gdzie dotyczy)
...
| Zarządzanie kluczem API                   | Dardcor Code generuje, szyfruje at-rest (AES-256-GCM) i wstrzykuje przez env |
...
`package.json` i `node_modules/`. Nie koliduje z zależnościami Dardcor Code.
...
przy następnym boot Dardcor Code (jeśli jest zainstalowana).
...
- Usuwa przychodzące nagłówki `cookie` i `authorization` (brak wycieku sesji Dardcor Code)
...
2. Zweryfikuj, że `npm` jest w PATH: `which npm` z tego samego konta użytkownika, które uruchamia Dardcor Code.
...
domyślnym portem Dardcor Code 20128.
...
- `DATA_DIR` lub jego rodzic nie jest zapisywalny przez proces Dardcor Code.
- Uruchomienie w Docker rootless bez zapisu do zamapowanego volume.

**Naprawa:**

1. Sprawdź `DATA_DIR` (domyślnie: `~/.Dardcor Code/`): `ls -la ~/.Dardcor Code/`
2. Upewnij się, że użytkownik procesu Dardcor Code jest właścicielem katalogu: `chown -R $USER ~/.Dardcor Code/`
...
3. Spróbuj instalacji ręcznie: `npm install {package}@latest --prefix ~/.Dardcor Code/services/{name}/`.
...
Tak. Obie usługi idą tym samym modelem local-first co sam Dardcor Code. Działają
na tej samej maszynie i komunikują się przez loopback. „Production” oznacza tu VPS
lub lokalny serwer, na którym wdrożono Dardcor Code, a nie zdalnego providera chmurowego.
...
2. Sprawdź strukturalne logi w output pino Dardcor Code filtrowane po
   namespace `service:supervisor`.
3. Podejrzyj wiersz DB: `sqlite3 ~/.Dardcor Code/Dardcor Code.db "SELECT * FROM version_manager WHERE tool='9router'"`.
