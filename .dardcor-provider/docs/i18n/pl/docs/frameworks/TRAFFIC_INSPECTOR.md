...
Traffic Inspector to wbudowany debugger ruchu HTTPS w Dardcor Code — narzędzie w stylu Charles Proxy / mitmweb / HTTP Toolkit, które jest **świadome LLM** (**LLM-aware**) i **świadome agentów** (**agent-aware**). Znajduje się pod `/dashboard/tools/traffic-inspector` i odbiera ruch na żywo z maksymalnie 5 jednoczesnych źródeł przechwytywania.
...
| **Zintegrowany z Dardcor Code** routing, fallback, cost                   |    ✗    |    ✗    |    ✗    |                ✓                |
...
Jeśli Dardcor Code crashnie przy aktywnym trybie proxy systemowego:
...
| POST   | `/requests/{id}/replay`     | Ponownie wykonuje to samo żądanie przez router Dardcor Code                          |
