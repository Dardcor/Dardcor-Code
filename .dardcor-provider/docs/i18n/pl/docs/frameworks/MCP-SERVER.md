---
title: "Dokumentacja serwera MCP Dardcor Code"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dokumentacja serwera MCP Dardcor Code
...
Dardcor Code MCP jest wbudowany. Uruchom go poleceniem:
...
Dardcor Code --dev  # MCP auto-starts on /mcp endpoint
```
...
| :------------------------------ | :-------------------- | :----------------------------------------------------------------- |
| `Dardcor Code_get_health`          | `read:health`         | Uptime, pamięć, circuit breakery, limity rate, statystyki cache    |
| `Dardcor Code_list_combos`         | `read:combos`         | Wszystkie skonfigurowane combo ze strategiami (opcjonalne metryki) |
| `Dardcor Code_get_combo_metrics`   | `read:combos`         | Metryki wydajności dla konkretnego combo                           |
| `Dardcor Code_switch_combo`        | `write:combos`        | Aktywacja lub deaktywacja combo                                    |
| `Dardcor Code_check_quota`         | `read:quota`          | Quota used/total, procent pozostały, czas resetu, health tokenów   |
| `Dardcor Code_route_request`       | `execute:completions` | Wysłanie chat completion przez routing Dardcor Code                   |
| `Dardcor Code_cost_report`         | `read:usage`          | Raport kosztów wg okresu (session/day/week/month)                  |
| `Dardcor Code_list_models_catalog` | `read:models`         | Pełny katalog modeli z capabilities, statusem i pricing            |

## Phase 1 — Search

| Tool                   | Scopes           | Description                                                                                                                            |
| :--------------------- | :--------------- | :------------------------------------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_web_search` | `execute:search` | Wyszukiwanie w sieci przez bramkę search Dardcor Code (Serper/Brave/Perplexity/Exa/Tavily/Google PSE/Linkup/SearchAPI/SearXNG) z failover |

## Advanced Tools (11) — Phase 2

| Tool                               | Scopes                               | Description                                                                                   |
| :--------------------------------- | :----------------------------------- | :-------------------------------------------------------------------------------------------- |
| `Dardcor Code_simulate_route`         | `read:health`, `read:combos`         | Symulacja routingu dry-run z drzewem fallback                                                 |
| `Dardcor Code_set_budget_guard`       | `write:budget`                       | Budżet sesji z akcją degrade/block/alert                                                      |
| `Dardcor Code_set_routing_strategy`   | `write:combos`                       | Aktualizacja strategii combo w runtime (priority/weighted/auto itd.)                          |
| `Dardcor Code_set_resilience_profile` | `write:resilience`                   | Zastosowanie presetu resilience `aggressive` / `balanced` / `conservative`                    |
| `Dardcor Code_test_combo`             | `execute:completions`, `read:combos` | Live test każdego providera w combo przez realne wywołanie upstream                           |
| `Dardcor Code_get_provider_metrics`   | `read:health`                        | Metryki per provider z latencją p50/p95/p99 i stanem circuit breakera                         |
| `Dardcor Code_best_combo_for_task`    | `read:combos`, `read:health`         | Rekomendacja combo wg typu zadania z ograniczeniami budżetu/latencji                          |
| `Dardcor Code_explain_route`          | `read:health`, `read:usage`          | Wyjaśnienie, dlaczego request poszedł do providera (czynniki scoringu + fallbacki)            |
| `Dardcor Code_get_session_snapshot`   | `read:usage`                         | Pełny snapshot sesji: koszt, tokeny, top modele/providerzy, błędy, budget guard               |
| `Dardcor Code_db_health_check`        | `read:health`, `write:resilience`    | Diagnoza (i opcjonalna auto-naprawa) driftu bazy, np. uszkodzone refy combo / wiersze-sieroty |
| `Dardcor Code_sync_pricing`           | `pricing:write`                      | Synchronizacja danych cenowych ze źródeł zewnętrznych (LiteLLM); obsługuje `dryRun`           |

## Cache Tools (2)

| Tool                    | Scopes        | Description                                           |
| :---------------------- | :------------ | :---------------------------------------------------- |
| `Dardcor Code_cache_stats` | `read:cache`  | Statystyki semantic cache, prompt-cache i idempotency |
| `Dardcor Code_cache_flush` | `write:cache` | Flush cache globalnie albo wg signature/model         |

## Compression Tools (13)

| Tool                                | Scopes              | Description                                                                                                                       |
| :---------------------------------- | :------------------ | :-------------------------------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_compression_status`      | `read:compression`  | Ustawienia kompresji, podsumowanie analytics oraz statystyki cache-aware (zawiera metadane `analytics.mcpDescriptionCompression`) |
| `Dardcor Code_compression_configure`   | `write:compression` | Konfiguracja trybu kompresji, progu, target ratio, zachowania system-prompt oraz przełącznika kompresji opisów MCP                |
| `Dardcor Code_set_compression_engine`  | `write:compression` | Wybór aktywnego silnika (off/caveman/rtk/stacked) oraz intensywności Caveman/RTK                                                  |
| `Dardcor Code_list_compression_combos` | `read:compression`  | Lista nazwanych compression combo i ich pipeline'ów silników                                                                      |
| `Dardcor Code_compression_combo_stats` | `read:compression`  | Analytics pogrupowane wg compression combo i silnika                                                                              |
| `Dardcor Code_ccr_store`               | `write:compression` | Zapis izolowanej per-caller treści w ograniczonym in-memory magazynie CCR i zwrot markera plus referencji `ccr://`                |
| `Dardcor Code_ccr_retrieve`            | `read:compression`  | Odczyt treści CCR w całości albo w trybach head, tail, lines, grep i stats                                                        |
| `Dardcor Code_ccr_inspect`             | `read:compression`  | Podgląd metadanych CCR należących do callera bez zwracania treści                                                                 |
| `Dardcor Code_ccr_list`                | `read:compression`  | Stronicowana lista metadanych bloków CCR należących do callera                                                                    |
| `Dardcor Code_ccr_delete`              | `write:compression` | Usunięcie bloku CCR należącego do callera                                                                                         |
| `Dardcor Code_ccr_stats`               | `read:compression`  | Raport użycia pamięci w zakresie callera, liczników cyklu życia i limitów magazynu                                                |
| `Dardcor Code_rtk_discover`            | `read:compression`  | Wykrywanie powtarzającego się szumu w opt-in próbkach wyjścia RTK                                                                 |
| `Dardcor Code_rtk_learn`               | `read:compression`  | Generowanie draftu filtra RTK do przeglądu na podstawie opt-in próbek                                                             |
...
`Dardcor Code_compression_status` raportuje kompresję opisów MCP osobno pod
...
Osobno od powyższych narzędzi kompresji Dardcor Code zawiera filtr post-execution, który
...
| Tool                        | Scopes         | Description                                                                            |
| :-------------------------- | :------------- | :------------------------------------------------------------------------------------- |
| `Dardcor Code_oneproxy_fetch`  | `read:proxies` | Pobranie darmowych proxy z marketplace 1proxy (filtry protocol/country/quality/limit)  |
| `Dardcor Code_oneproxy_rotate` | `read:proxies` | Pobranie kolejnego dostępnego proxy wg strategii (`random` / `quality` / `sequential`) |
| `Dardcor Code_oneproxy_stats`  | `read:proxies` | Statystyki poola, status sync, rozkład wg protocol i country                           |
...
| Tool                      | Scopes         | Description                                                                                    |
| :------------------------ | :------------- | :--------------------------------------------------------------------------------------------- |
| `Dardcor Code_memory_search` | `read:memory`  | Wyszukiwanie pamięci po query / type / API key z egzekwowaniem token-budget                    |
| `Dardcor Code_memory_add`    | `write:memory` | Dodanie nowego wpisu pamięci (`factual` / `episodic` / `procedural` / `semantic`)              |
| `Dardcor Code_memory_clear`  | `write:memory` | Czyszczenie pamięci dla klucza API, opcjonalnie filtrowane po type lub timestampie `olderThan` |
...
| Tool                          | Scopes           | Description                                                                             |
| :---------------------------- | :--------------- | :-------------------------------------------------------------------------------------- |
| `Dardcor Code_skills_list`       | `read:skills`    | Lista zarejestrowanych skills z opcjonalnym filtrem po API key, name lub stanie enabled |
| `Dardcor Code_skills_enable`     | `write:skills`   | Włączenie lub wyłączenie konkretnego skilla po ID                                       |
| `Dardcor Code_skills_execute`    | `execute:skills` | Wykonanie skilla z podanym inputem i zwrot rekordu wykonania                            |
| `Dardcor Code_skills_executions` | `read:skills`    | Lista niedawnej historii wykonań skills                                                 |
...
| Tool                              | Scopes         | Description                                                                                                         |
| :-------------------------------- | :------------- | :------------------------------------------------------------------------------------------------------------------ |
| `Dardcor Code_agent_skills_list`     | `read:catalog` | Lista wszystkich 42 agent skills z opcjonalnymi filtrami `category` (api\|cli) i `area`; zwraca metadata + coverage |
| `Dardcor Code_agent_skills_get`      | `read:catalog` | Pełne metadata + treść SKILL.md dla pojedynczego skilla po kanonicznym `id`                                         |
| `Dardcor Code_agent_skills_coverage` | `read:catalog` | Statystyki coverage: ile z 22 API i 20 CLI skills ma pliki SKILL.md na filesystemie względem sum katalogu           |
...
Dardcor Code przez ten sam model połączeń co providery LLM. Są wystawione przez
...
| :-------------------------------------- | :--------------------------------- | :------------------------------------------------------------------------------------------------------------ |
| `OMNIROUTE_BASE_URL`                    | `http://localhost:20128`           | Bazowy URL używany przez serwer MCP przy wywołaniach wewnętrznych API Dardcor Code                               |
...
| `DATA_DIR`                              | `~/.Dardcor Code`                     | Plik heartbeat jest zapisywany do `${DATA_DIR}/runtime/mcp-heartbeat.json`                                    |
...
- Statystyki realtime są dostępne przez `Dardcor Code_compression_status` pod `analytics.mcpDescriptionCompression` i oznaczone `source: "mcp_metadata_estimate"`, aby odróżnić je od rzeczywistych rachunków użycia providera.
...
MCP_TOOL_DENY="Dardcor Code_get_health,Dardcor Code_list_combos" Dardcor Code --mcp

# Announce only the routing + quota tools (allow-list mode)
MCP_TOOL_ALLOW="Dardcor Code_route_request,Dardcor Code_check_quota" Dardcor Code --mcp
```
