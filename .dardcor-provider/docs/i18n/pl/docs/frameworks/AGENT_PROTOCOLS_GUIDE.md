...
Dardcor Code udostępnia trzy różne powierzchnie związane z agentami. Na pierwszy rzut oka wyglądają podobnie, ale rozwiązują inne problemy. Ta strona pomaga wybrać właściwą.
...
| **ACP — CLI Agents Registry** | Wykrywanie / rejestracja / uruchamianie agentów CLI do kodowania zainstalowanych na maszynie użytkownika (Cursor, Cline, Codex CLI, Claude Code, Aider itd.) | HTTP REST                   | Dardcor Code-specific   |
| **Cloud Agents**              | Przesyłanie długotrwałych zadań kodowania do zewnętrznych usług chmurowych (Codex Cloud, Devin, Jules, Cursor Cloud)                                         | HTTP REST + DB-backed tasks | Dardcor Code-specific   |
...
**Dardcor Code endpoint:** `POST /a2a` (JSON-RPC 2.0)
**Agent Card:** `GET /.well-known/agent.json`

### Kiedy używać

- Budowa systemu multi-agent, w którym Dardcor Code jest jednym z peerów
- Udostępnianie inteligencji routingu Dardcor Code (smart-routing, quota-management itd.) agentom w frameworkach takich jak Google ADK lub generycznych siatkach agentów
- Opakowanie Dardcor Code za standardową powierzchnią discovery + invocation
...
**Dardcor Code endpoint:** `GET /api/acp/agents`
**Source:** `src/lib/acp/{index,manager,registry}.ts`

### Czym jest

ACP to **lokalny inwentarz agentów CLI** Dardcor Code. Wykrywa, które CLI do kodowania są zainstalowane na hoście (Cursor, Cline, Claude Code, Codex CLI, Continue itd.), ustala ich wersje i udostępnia je w dashboardzie, aby użytkownik mógł skonfigurować każde CLI tak, by wskazywało na Dardcor Code.
...
- Strona dashboardu „CLI Tools” listuje, co jest zainstalowane, i pomaga wskazać każde CLI na Dardcor Code
- Custom agenci pozwalają power userom rejestrować wewnętrzne/własnościowe CLI, o których Dardcor Code domyślnie nie wie
- Wynik detekcji zasila macierz fingerprintów `cli-tools`

### Kiedy NIE używać ACP

- ACP nie _uruchamia_ tasków. Tylko wykrywa + konfiguruje CLI. Aby faktycznie wywołać CLI, uruchamiasz je sam z env vars, które dostarcza Dardcor Code (`OPENAI_BASE_URL`, `OPENAI_API_KEY` itd.).

## 3. Cloud Agents

**Dardcor Code endpoints:** `/api/v1/agents/tasks/*` (lifecycle) + `/api/cloud/*` (plumbing)
**Source:** `src/lib/cloudAgent/`

### Czym jest

Jednolity interfejs nad zewnętrznymi chmurowymi agentami do kodowania. Przesyłasz prompt + URL repozytorium, Dardcor Code dysponuje do właściwego cloud agenta, odpytuje status i zwraca wyniki.
...
| ------------------ | ------------------------------------------------------------------------------------ | ------------------------------------------- |
| Standard           | Open A2A v0.3                                                                        | Dardcor Code-specific                          |
| Where compute runs | Wewnątrz Dardcor Code (używa skonfigurowanych combos)                                   | Zewnętrznie (serwery Codex / Devin / Jules) |
...
### Odkryj capabilities A2A Dardcor Code
...
### Wywołaj Dardcor Code jako agenta A2A
