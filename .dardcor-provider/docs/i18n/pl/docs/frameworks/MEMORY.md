...
Dardcor Code zapewnia trwałą pamięć konwersacyjną powiązaną z kluczem API (oraz
...
> Klient może wyłączyć pamięć dla pojedynczego żądania nagłówkiem `x-Dardcor Code-no-memory`
...
- `searchSemanticMemory(query, topK, scope)` — embeduje zapytanie, przeszukuje
  kolekcję filtrowaną po `kind = "Dardcor Code_memory"` oraz opcjonalnie po
...
`qdrantApiKey`, `qdrantCollection` domyślnie `"Dardcor Code_memory"`,
...
- `Dardcor Code_memory_search` — `{apiKeyId, query?, type?, maxTokens?, limit?}`
...
- `Dardcor Code_memory_add` — `{apiKeyId, sessionId?, type, key, content,
metadata?}` → opakowuje `createMemory()`. Przyjmuje tylko 4 kanoniczne typy:
  `factual`, `episodic`, `procedural`, `semantic` (D17).
- `Dardcor Code_memory_clear` — `{apiKeyId, type?, olderThan?}` → listuje pasujące
...
- Do twardego usunięcia użyj `DELETE /api/memory/[id]` lub `Dardcor Code_memory_clear`.
...
Silnik memory Dardcor Code wspiera **cztery źródła embeddingów** (`src/lib/memory/embedding/`). Każde ma inne kompromisy w **latencji, koszcie, jakości modelu i złożoności setupu**.
