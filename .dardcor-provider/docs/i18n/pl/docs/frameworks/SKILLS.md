...
Dardcor Code udostępnia rozszerzalny framework Skills, który pozwala modelom językowym (oraz operatorom) składać wielokrotnego użytku możliwości — od odczytów systemu plików i żądań HTTP po wykonywanie kodu w sandboxie oraz wyselekcjonowane skille z marketplace.

Skill to wersjonowana, zdefiniowana schematem jednostka pracy. Dardcor Code może wstrzykiwać skille jako definicje narzędzi do żądań wychodzących, przechwytywać wywołania narzędzi wracające od modelu, uruchamiać pasujący handler i zwracać wynik do modelu, aby rozmowa mogła być kontynuowana. Model nigdy nie widzi implementacji — tylko interfejs narzędzia.

---

## Agent Skills vs Omni Skills

Dardcor Code ma dwa odrębne, ale komplementarne systemy skilli:
...
| Who uses it     | Sam Dardcor Code (routing combo, przychodzące wywołania LLM)     | Zewnętrzni agenci, klienci MCP, orkiestratory A2A                                               |
...
| Discovery       | REST `/api/skills/*` + narzędzia MCP `Dardcor Code_skills_*`     | REST `/api/agent-skills/*` + narzędzia MCP `Dardcor Code_agent_skills_*` + A2A `list-capabilities` |

**Omni Skills** to silnik wykonania — definiują, _co Dardcor Code potrafi zrobić_, gdy LLM wywoła narzędzie.

**Agent Skills** to katalog dokumentacji — wyjaśniają zewnętrznym agentom, _jak używać_ REST API i CLI Dardcor Code, ze ustrukturyzowanymi plikami SKILL.md, które można bezpośrednio wstawić do promptów agenta.
...
1. **Built-in skills** (`src/lib/skills/builtins.ts`) — dostarczane z Dardcor Code. Pokrywają typowe przypadki:
...
2. **SkillsMP** (Dardcor Code Marketplace) — pobierany z `https://skillsmp.com/api/v1/skills/search`. Wymaga `skillsmpApiKey` w Settings.
...
2. Aliasys wbudowanych narzędzi (np. `Dardcor Code_web_search` → `web_search`) są rozwiązywane najpierw. Handlery wbudowane działają inline.
...
`file_read` i `file_write` rozwiązują każdą ścieżkę względem workspace per klucz API w `<DATA_DIR>/skills/workspaces/<sha256(apiKeyId).slice(0,24)>/`. Path traversal (`..`) oraz zabronione segmenty (`.env`, `.git`, `.ssh`, `.Dardcor Code`, `.codex`, `secrets`) są odrzucane przed jakimkolwiek I/O dyskowym.
...
- **Audytuj wykonania:** `/dashboard/skills/executions` oraz `Dardcor Code_skills_executions` odpytują `skill_executions`. Udane runy zawierają `durationMs`; nieudane — `errorMessage`.
...
Dardcor Code dostarcza wyselekcjonowany zestaw wbudowanych skilli w `src/lib/skills/builtin/`. Najczęstsze:
