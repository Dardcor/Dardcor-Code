---
title: "Katalog Agent Skills Dardcor Code"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Katalog Agent Skills Dardcor Code

> **Source of truth:** `src/lib/agentSkills/` (catalog, generator, parsers) + `skills/` directory (SKILL.md files)
> **Last updated:** 2026-06-28 — v3.8.40

Agent Skills to ustrukturyzowane pliki SKILL.md, które uczą zewnętrzne agenty, klientów MCP oraz orkiestratory A2A, jak korzystać z REST API i CLI Dardcor Code. W przeciwieństwie do [Omni Skills](./SKILLS.md) (które są definicjami narzędzi LLM wykonywanymi wewnątrz Dardcor Code), Agent Skills to _katalog dokumentacji_ — statyczny markdown, który można wstrzyknąć bezpośrednio do kontekstu agenta.
...
- **Surowy URL GitHub** do pobrania na żywo: `https://raw.githubusercontent.com/diegosouzapw/Dardcor Code/refs/heads/main/skills/{id}/SKILL.md`
...
| Tool                              | Description                                        |
| :-------------------------------- | :------------------------------------------------- |
| `Dardcor Code_agent_skills_list`     | List skills (optional `category` / `area` filters) |
| `Dardcor Code_agent_skills_get`      | Get metadata + SKILL.md for one skill by `id`      |
| `Dardcor Code_agent_skills_coverage` | Coverage stats (API/CLI have/total)                |
...
| :------------------- | :----------------- | :---------------------- |
| `cli-serve`          | cli-serve          | `Dardcor Code serve`       |
| `cli-health`         | cli-health         | `Dardcor Code health`      |
| `cli-providers`      | cli-providers      | `Dardcor Code providers`   |
| `cli-keys`           | cli-keys           | `Dardcor Code keys`        |
| `cli-models`         | cli-models         | `Dardcor Code models`      |
| `cli-chat`           | cli-chat           | `Dardcor Code chat`        |
| `cli-routing`        | cli-routing        | `Dardcor Code routing`     |
| `cli-resilience`     | cli-resilience     | `Dardcor Code resilience`  |
| `cli-compression`    | cli-compression    | `Dardcor Code compression` |
| `cli-contexts`       | cli-contexts       | `Dardcor Code contexts`    |
| `cli-cost-usage`     | cli-cost-usage     | `Dardcor Code cost`        |
| `cli-mcp`            | cli-mcp            | `Dardcor Code mcp`         |
| `cli-a2a`            | cli-a2a            | `Dardcor Code a2a`         |
| `cli-tunnel`         | cli-tunnel         | `Dardcor Code tunnel`      |
| `cli-backup-sync`    | cli-backup-sync    | `Dardcor Code backup`      |
| `cli-policy-audit`   | cli-policy-audit   | `Dardcor Code policy`      |
| `cli-batches`        | cli-batches        | `Dardcor Code batch`       |
| `cli-eval`           | cli-eval           | `Dardcor Code eval`        |
| `cli-plugins-skills` | cli-plugins-skills | `Dardcor Code plugins`     |
| `cli-setup`          | cli-setup          | `Dardcor Code setup`       |
...
curl "http://your-Dardcor Code/api/agent-skills" | jq '.skills[] | {id, name, category}'

# Get SKILL.md for context injection
curl "http://your-Dardcor Code/api/agent-skills/omni-providers/raw" > omni-providers.md
```
...
const result = await client.callTool("Dardcor Code_agent_skills_list", { category: "api" });
...
resp = requests.post("http://your-Dardcor Code/a2a", json={
    "jsonrpc": "2.0", "id": "1",
    "method": "message/send",
...
```bash
BASE="https://raw.githubusercontent.com/diegosouzapw/Dardcor Code/refs/heads/main/skills"
...
- [MCP-SERVER.md](./MCP-SERVER.md) — katalog narzędzi MCP (narzędzia `Dardcor Code_agent_skills_*`)
