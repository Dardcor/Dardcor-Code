...
Dardcor Code dostarcza system wtyczek w stylu WordPressa. Wtyczki to samodzielne
...
Wtyczki mieszkają w katalogu danych Dardcor Code:

```
~/.Dardcor Code/plugins/<plugin-name>/
  ├─ plugin.json
  └─ index.js          # (or whatever manifest.main points to)
```

`getDefaultPluginDir()` (`src/lib/plugins/scanner.ts`) rozwiązuje to do
`<home>/.Dardcor Code/plugins`, gdzie `<home>` pochodzi ze zmiennych środowiskowych `HOME` /
...
| `requires`         | object?   | `{ Dardcor Code?, permissions[] }`                             |
...
install (POST /api/plugins, path)
  ...
  → atomic rename into ~/.Dardcor Code/plugins/<name> → insert DB row
