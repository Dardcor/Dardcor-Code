...
Dardcor Code może wysyłać HTTP webhooki na zdarzenia platformy. Użyj ich do integracji ze
...
Gdy webhook ma `secret`, Dardcor Code podpisuje ciało JSON i wysyła:

```
Content-Type: application/json
User-Agent: Dardcor Code-Webhook/1.0
X-Webhook-Event: <event>
X-Webhook-Timestamp: <ISO-8601>
X-Webhook-Signature: sha256=<hex HMAC-SHA256(secret, body)>
```

> Nazwy nagłówków używają prefiksu `X-Webhook-*` (nie `X-Dardcor Code-*`). Wartość podpisu
...
```json
{
  ...
  "data": {
    "message": "Test webhook delivery from Dardcor Code",
    "webhookId": "<uuid>"
  }
}
```
