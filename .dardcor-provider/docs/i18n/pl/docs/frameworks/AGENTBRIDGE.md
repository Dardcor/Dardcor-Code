...
AgentBridge to proxy MITM (Man-in-the-Middle) Dardcor Code, który przechwytuje ruch HTTPS z agentów AI w IDE i przekierowuje go przez ujednolicony silnik routingu Dardcor Code. Obsługuje **9 agentów IDE** — Antigravity, Kiro, GitHub Copilot, OpenAI Codex, Cursor, Zed, Claude Code, Open Code oraz Trae (w badaniu) — co czyni Dardcor Code proxy MITM o najszerszym pokryciu asystentów AI do kodowania na rynku.
...
Gdy agent IDE (np. GitHub Copilot, Cursor, Claude Code) wykonuje wywołanie API, łączy się bezpośrednio z upstreamowym dostawcą AI (OpenAI, Anthropic itd.). AgentBridge przechwytuje to połączenie w sposób przezroczysty na poziomie TLS — bez konieczności zmiany konfiguracji agenta — i przepisuje żądanie przez Dardcor Code.

Dzięki temu możesz:

- **Przekierować dowolnego agenta do dowolnego providera**: Copilot rozmawia z OpenAI? Przekieruj go na Anthropic Claude, Gemini lub dowolnego z 226+ providerów Dardcor Code.
...
- **Stosować odporność Dardcor Code**: combo routing, circuit breakery, fallbacki i śledzenie kosztów działają także dla ruchu agentów IDE.

### Pozycjonowanie względem rynku

| Feature           | 9router | anti-api | llm-interceptor | **Dardcor Code AgentBridge** |
| ----------------- | :-----: | :------: | :-------------: | :-----------------------: |
| Antigravity       |    ✓    |    ✓     |        —        |             ✓             |
| GitHub Copilot    |    ✓    |    ✓     |        —        |             ✓             |
| Kiro (AWS)        |    ✓    |    ✓     |        —        |             ✓             |
...
src/mitm/server.cjs  (port 443, CJS child process)
    ...
    ├── Target match? → fetch → Dardcor Code router (port 20128)
...
> w tym samym slocie trust-store `Dardcor Code-mitm.crt`, którego używał stary leaf
...
```bash
certutil -A -d sql:$HOME/.pki/nssdb -n "Dardcor Code AgentBridge" -t CT,, -i ~/.Dardcor Code/mitm/ca.crt
```
...
sudo security add-trusted-cert -d -r trustRoot \
  -k /Library/Keychains/System.keychain ~/.Dardcor Code/mitm/ca.crt
```

**Windows (certmgr):**

```powershell
certutil -addstore -f Root $env:USERPROFILE\.Dardcor Code\mitm\ca.crt
```
...
1. Wskaż runtime jawnie na CA:
   ```bash
   export NODE_EXTRA_CA_CERTS=/path/to/Dardcor Code-agentbridge-ca.crt
   ```
...
| Source model (agent native) | Target model (Dardcor Code) |
| --------------------------- | ------------------------ |
...
AgentBridge przechwytuje poświadczenia (tokeny OAuth, klucze API), których IDE używa do uwierzytelnienia u upstreamowych providerów. Są one **maskowane przed logowaniem** (zob. §2.7), ale są widoczne dla warstwy MITM Dardcor Code. Pierwsza aktywacja każdego agenta pokazuje zamykalny modal z ostrzeżeniem o ryzyku.
...
Lista bypass gwarantuje, że instytucje finansowe, providery OAuth/SSO i inne wrażliwe hosty **nigdy nie są deszyfrowane**. Ich ruch TLS przechodzi jako przezroczysty tunel TCP — Dardcor Code nigdy nie widzi plaintextu.
...
1. Sprawdź, czy cert został zainstalowany: `security find-certificate -c "Dardcor Code AgentBridge"` (macOS) lub `certutil -L -d sql:$HOME/.pki/nssdb` (Linux/NSS)
...
```bash
grep "Dardcor Code\|127.0.0.1.*github\|127.0.0.1.*cursor" /etc/hosts
```
...
2. Sprawdź logi serwera Dardcor Code: `APP_LOG_LEVEL=debug` w `.env`
