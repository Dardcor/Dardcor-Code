...
Dardcor Code łączy się z vaultem **Obsidian** jako **źródłem kontekstu** — lokalną bazą
...
wskazuje Dardcor Code na lokalny katalog vaultu i generuje losową parę username/password WebDAV:
...
w **serwer sync** Dardcor Code (cztery narzędzia `obsidian_sync_*`) dodatkowo wymagają
tokenu auth sync skonfigurowanego w ustawieniach Dardcor Code.
...
| `obsidian_sync_status`       | Status serwera sync Dardcor Code: running, nazwa vaultu, port, uptime, last sync.               |
