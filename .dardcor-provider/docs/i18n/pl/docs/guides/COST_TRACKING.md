...
Jak Dardcor Code szacuje, rejestruje i raportuje koszt każdego żądania — oraz dlaczego
...
Dardcor Code przypisuje koszt w USD do każdego completion, mnożąc liczbę tokenów przez
stawki cenowe modelu. Te liczby zasilają dashboard **Costs**, CLI
`Dardcor Code cost` / `Dardcor Code usage`, eksporty CSV/JSON oraz budżety per klucz API.

> **„Koszt” na dashboardzie to tracker oszczędności, a nie rachunek.** Dardcor Code nigdy
...
- Zależy od tabeli cen Dardcor Code dla każdego modelu. Model bez wpisu cenowego
...
3. **Zakodowane na stałe domyślne** — dostarczane z Dardcor Code.
...
CLI Dardcor Code udostępnia komendy cost, usage i pricing (zarejestrowane w
...
- **Wszystkie koszty pokazują $0 / „Legacy / Free”.** Używane modele nie mają wpisu
  ...
  `Dardcor Code pricing sync`, albo ustaw ceny ręcznie na stronie Pricing / przez
  `PATCH /api/pricing`.
