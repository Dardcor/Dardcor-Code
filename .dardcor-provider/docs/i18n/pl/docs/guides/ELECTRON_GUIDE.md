...
Dardcor Code dostarcza wieloplatformową aplikację desktopową (Windows / macOS / Linux) opartą na
...
| Id aplikacji       | `online.Dardcor Code.desktop` |
| Nazwa produktu     | `Dardcor Code`                |
...
- Linux: `$XDG_CONFIG_HOME/Dardcor Code` lub `~/.Dardcor Code`
- macOS: `~/.Dardcor Code`
...
- Przycisk zamknięcia minimalizuje do trayu; menu trayu ma **Open Dardcor Code**, **Open Dashboard** (zewnętrzna przeglądarka), podmenu **Server Port**, **Check for Updates**, **Quit**.
...
- `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*.Dardcor Code.online https://*.Dardcor Code.dev`
- Tryb dev dodaje `'unsafe-eval'` tylko do `script-src`

## Auto-aktualizacja

Używa `electron-updater` z providerem GitHub (`diegosouzapw/Dardcor Code`).
...
- `Dardcor Code Setup X.Y.Z.exe`, `Dardcor Code-X.Y.Z-portable.exe` (Windows)
- `Dardcor Code-X.Y.Z-mac.dmg`, `Dardcor Code-X.Y.Z-arm64-mac.dmg` (macOS)
- `Dardcor Code-X.Y.Z.AppImage`, `Dardcor Code-desktop_X.Y.Z_amd64.deb` (Linux)

Wydania są publikowane w GitHub Releases (`diegosouzapw/Dardcor Code`), skąd `electron-updater` sprawdza też nowe wersje.
