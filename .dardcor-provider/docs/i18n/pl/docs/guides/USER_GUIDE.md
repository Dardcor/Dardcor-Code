...
Kompletny przewodnik po konfiguracji providerów, tworzeniu combo, integracji narzędzi CLI i wdrażaniu Dardcor Code.
...
**Wskazówka:** Używaj Opus do złożonych zadań, Sonnet dla szybkości. Dardcor Code śledzi limit per model!

Trasy zgodne z Claude i Claude Code zachowują poziom myślenia `max` dla modeli Opus i Sonnet
Modele Haiku nie akceptują poziomu wysiłku `max`, więc Dardcor Code obniża to
...
1. Subskrybuj: [Moonshot AI](https://platform.kimi.ai?aff=Dardcor Code)
2. Pobierz klucz API → Dashboard → Add API Key
...
CLI automatycznie ładuje `.env` z `~/.Dardcor Code/.env` lub `./.env`.

### Odinstalowywanie

Gdy nie potrzebujesz już Dardcor Code, mamy dwa szybkie skrypty do czystego usunięcia:
...
> Uwaga: Aby uruchomić te polecenia, przejdź do folderu projektu Dardcor Code (jeśli klonowałeś) i je wykonaj. Alternatywnie, przy instalacji globalnej możesz po prostu uruchomić `npm uninstall -g Dardcor Code`.
...
Użytkownicy Void Linux mogą spakować i zainstalować Dardcor Code natywnie przez framework cross-kompilacji `xbps-src`. Automatyzuje to build standalone Node.js wraz z wymaganymi natywnymi bindingami `better-sqlite3`.
...
| --------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------ |
| `JWT_SECRET`                            | `Dardcor Code-default-secret-change-me` | Sekret podpisu JWT (**zmień w produkcji**)                                                                               |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | Hasło pierwszego logowania                                                                                               |
| `DATA_DIR`                              | `~/.Dardcor Code`                       | Katalog danych (db, usage, logi)                                                                                         |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://Dardcor Code.dev`              | Bazowy URL endpointu cloud sync (zastępuje legacy `CLOUD_URL`)                                                           |
...
### Łączenie peerów Dardcor Code

Inną bramę Dardcor Code możesz dodać jako provider **Custom OpenAI-compatible**. Użyj
...
`X-Dardcor Code-Peer-Trace`. Brama odrzuca powtórzone ID instancji lub wyczerpany budżet hopów
...
do wielu uruchomionych instancji Dardcor Code.
...
- Quick Tunnels nie są automatycznie przywracane po restarcie Dardcor Code lub kontenera; włącz je ponownie z dashboardu w razie potrzeby
...
- **Semantic Cache** — automatycznie cache'uje odpowiedzi non-streaming z temperature=0 (pomiń przez `X-Dardcor Code-No-Cache: true`)
- **Idempotencja żądań** — deduplikuje żądania w ciągu 5s przez nagłówek `Idempotency-Key` lub `X-Request-Id`
- **Śledzenie postępu** — opcjonalne zdarzenia SSE `event: progress` przez nagłówek `X-Dardcor Code-Progress: true`

---

### Translator Playground

Dostęp: **Dashboard → Translator**. Debuguj i wizualizuj, jak Dardcor Code tłumaczy żądania API między providerami.
...
Dardcor Code akceptuje też `x_session_id` i zwraca efektywny klucz sesji w `X-Dardcor Code-Session-Id`.
...
Dardcor Code implementuje odporność na poziomie providera w pięciu komponentach:
...
4. **Wait For Cooldown** — jeśli każde połączenie-kandydat jest już w cooldown, Dardcor Code może poczekać na najwcześniejszy cooldown i automatycznie ponowić to samo żądanie klienta.
...
- Migruj Dardcor Code między maszynami
...
Dardcor Code obsługuje transkrypcję audio przez endpoint zgodny z OpenAI:
...
Dardcor Code obcina go do tego limitu przed wysłaniem żądania upstream.
...
Dardcor Code ma wbudowany **auto-router oparty na score**, który wybiera najlepszy model dla każdego żądania spośród podłączonych providerów — bez utrzymywania combo. Wystarczy wysłać żądanie z jednym z prefiksów `auto/*`, a Dardcor Code złoży wirtualne combo w locie, oceniając kandydatów pod kątem opóźnienia, kosztu, success rate, dopasowania kontekstu, przydatności modelu do zadania, ostatnich błędów, quoty i stanu circuit breakera.
...
Dardcor Code jest jednocześnie **serwerem MCP** (Model Context Protocol) i **serwerem A2A** (Agent-to-Agent JSON-RPC 2.0). Każde IDE lub host agentów zgodny z MCP może wywoływać narzędzia Dardcor Code bezpośrednio — bez dodatkowego wrappera.

### Transporty MCP

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `Dardcor Code --mcp` (dla wtyczek IDE preferujących stdio)
...
Dardcor Code udostępnia rozszerzalny **framework skills** (`src/lib/skills/`), dzięki czemu agenci i endpoint A2A mogą uruchamiać rutyny domenowe (np. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
Dardcor Code przechowuje **długoterminową pamięć konwersacyjną** z hybrydowym retrieval:
...
Subskrybuj zdarzenia Dardcor Code do monitoringu i automatyzacji w czasie rzeczywistym.
...
- Każdy payload zawiera `X-Dardcor Code-Signature` (HMAC-SHA256) do weryfikacji
...
Dardcor Code integruje się z cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**), dzięki czemu możesz wysyłać długotrwałe zadania z tego samego dashboardu, który obsługuje lokalny routing.

- Twórz zadania w **Dashboard → Cloud Agents** lub przez `POST /api/v1/agents/tasks`
- Śledź status, logi i artefakty per zadanie
- Własny klucz API per provider — credentials nigdy nie opuszczają instancji Dardcor Code
...
Możesz zarządzać każdym zasobem Dardcor Code (providerzy, combo, klucze, ustawienia) przez HTTP, używając klucza zarządzania.
...
Dardcor Code dostarcza wewnętrzne CLI (`Dardcor Code …`) do setupu, diagnostyki i kontroli runtime.
...
Wskazówka: połącz `Dardcor Code doctor --json` z narzędziem monitoringu, by alertować o niezdrowych providerach.

---

## 🖥️ Aplikacja desktopowa (Electron)

Dardcor Code jest dostępny jako natywna aplikacja desktopowa na Windows, macOS i Linux.
