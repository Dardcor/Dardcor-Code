...
Ten przewodnik opisuje dodawanie kont Kiro (hostowany w AWS asystent AI do kodowania) do Dardcor Code,
...
## Jak Dardcor Code to rozwiązuje (v3.8.0+)

Od v3.8.0 Dardcor Code wywołuje `registerClient()` (AWS SSO OIDC) przy każdym
imporcie połączenia Kiro. Dzięki temu każde połączenie Dardcor Code ma własną, dedykowaną
...
- Dardcor Code v3.8.0 lub nowszy.
...
2. Wybierz jedną z opcji:
   ...
   - **Auto-Import** — kliknij przycisk; Dardcor Code odczyta poświadczenia z
...
4. Dardcor Code waliduje klucz i zapisuje połączenie.
...
(`clientSecretExpiresAt`). Dardcor Code przechowuje ten znacznik czasu w `providerSpecificData`
...
- Upewnij się, że Dardcor Code może dotrzeć do `https://oidc.us-east-1.amazonaws.com` (lub
...
- Klucz musi móc wywołać `ListAvailableProfiles`; w przeciwnym razie Dardcor Code nie może
