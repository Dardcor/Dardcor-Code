...
Typowe problemy i rozwiązania dla Dardcor Code.

---

## Szybki przewodnik

**Nowy w Dardcor Code?** Zacznij tutaj — te wskazówki rozwiązują 90% problemów:

| Widzę to                | Co to oznacza                      | Co zrobić                                                                             |
| ----------------------- | ---------------------------------- | ------------------------------------------------------------------------------------- |
| "Can't connect"         | Dardcor Code nie działa               | Uruchom `Dardcor Code` lub `docker restart Dardcor Code`                                    |
...
| EACCES: permission denied                                  | Ustaw `DATA_DIR=/path/to/writable/dir`, by nadpisać `~/.Dardcor Code`                                                                                                         |
...
| `dlopen` / `slice is not valid mach-o file` (macOS)        | Uruchom `cd $(npm root -g)/Dardcor Code/app && npm rebuild better-sqlite3 && Dardcor Code` — zobacz [przebudowa natywnego modułu na macOS](#macos-native-module-rebuild) poniżej |
...
linków wyglądających jak żądania HTTP. `README.md` Dardcor Code jest w paczce npm (jest
wymieniony w `package.json` → `files`), więc ląduje w `node_modules/Dardcor Code/README.md` przy
...
1. **Zatrzymaj powiadomienia** — wyklucz katalog instalacji w antywirusie
   ...
   danych Dardcor Code (`~/.Dardcor Code/`).
...
`%LOCALAPPDATA%\Programs\Dardcor Code` (w tym katalogi paczek z sufiksem hasha z
...
1. **Najpierw zweryfikuj pobranie** (wyklucza spreparowany plik). Każde wydanie publikuje
   `latest.yml`, którego pole `sha512` (base64) obejmuje instalator `Dardcor Code.Setup.<version>.exe`.
   W PowerShell, z folderu z instalatorem:
   ```powershell
   $b = [System.Security.Cryptography.SHA512]::Create().ComputeHash(
     [System.IO.File]::ReadAllBytes("$PWD\Dardcor Code.Setup.<version>.exe"))
   [Convert]::ToBase64String($b)
   ```
   Wynik musi pasować do `latest.yml` → `sha512`. Jeśli nie, usuń plik i
   pobierz ponownie wyłącznie ze [strony wydań GitHub](https://github.com/diegosouzapw/Dardcor Code/releases).
2. **Przywróć + wyklucz** — przywróć cofnięte elementy z kwarantanny i dodaj wykluczenie
   dla `%LOCALAPPDATA%\Programs\Dardcor Code` (Kaspersky → Settings → Threats and Exclusions),
...
**Przyczyna:** Uruchamiasz wersję Node.js poza zatwierdzonym bezpiecznym progiem runtime Dardcor Code. Najczęstszy przypadek to starszy patch Node 22 lub 24 poniżej wymaganego przez Dardcor Code zabezpieczonego progu.
...
3. Zainstaluj ponownie Dardcor Code: `npm install -g Dardcor Code`
4. Uruchom ponownie: `Dardcor Code`
...
**Przyczyna:** Po globalnym `npm install -g Dardcor Code` natywne binarium `better-sqlite3` w paczce mogło zostać skompilowane pod inną architekturę lub ABI Node.js niż lokalnie uruchomione. To częste na macOS (Apple Silicon i Intel), gdy prebuilt nie pasuje do środowiska.
...
```
dlopen(/Users/<user>/.nvm/versions/node/v24.14.1/lib/node_modules/Dardcor Code/app/node_modules/better-sqlite3/build/Release/better_sqlite3.node, 0x0001): tried: '...' (slice is not valid mach-o file)
```

**Naprawa — przebuduj pod lokalne środowisko (bez obniżania Node.js):**

```bash
cd $(npm root -g)/Dardcor Code/app
...
**Naprawa (v3.5.5+):** Dardcor Code używa teraz własnego `fetch()` z undici, gdy aktywny jest dispatcher proxy, co daje spójne zachowanie. Zaktualizuj do v3.5.5+.

### Proxy MITM pod WSL: aplikacje desktop na hoście Windows nie są przechwytywane

**Przyczyna:** Proxy MITM i jego certyfikat CA instalują się w środowisku, w którym działa Dardcor Code. Pod WSL to gość Linux, a aplikacje AI desktop (Kiro, Trae, Copilot, Zed, …) działają na hoście Windows. Aplikacje hosta nie ufają magazynowi certyfikatów gościa i nie idą przez systemowe proxy gościa, więc przechwytywanie desktop tam nie działa.

**Rekomendacja:** Uruchamiaj Dardcor Code natywnie na tym samym OS co aplikacje desktop, które chcesz przechwytywać (Windows dla aplikacji Windows; analogicznie macOS/Linux). Trzymanie Dardcor Code w WSL przy celowaniu w aplikacje hosta wymaga ręcznego zaufania wygenerowanemu certyfikatowi CA na hoście Windows i wskazania ustawień sieci/proxy każdej aplikacji hosta na endpoint proxy WSL — to nieobsługiwana, krucha konfiguracja.
...
Dardcor Code automatycznie odświeża tokeny. Jeśli problemy trwają:
...
2. Sprawdź, czy `CLOUD_URL` wskazuje na endpoint chmury (np. `https://Dardcor Code.dev`)
...
Gdy wiele równoległych żądań trafia w rate-limitowanego providera, Dardcor Code używa mutex + auto rate-limiting, by serializować żądania i zapobiec kaskadowym awariom. To automatyczne dla providerów z kluczem API.

---

## Opcjonalna taksonomia awarii RAG / LLM (16 problemów)

Część użytkowników Dardcor Code stawia gateway przed stackami RAG lub agentów. W takich setupach często widać dziwny wzorzec: Dardcor Code wygląda na zdrowy (providerzy w górze, profile routingu OK, brak alertów rate limit), a końcowa odpowiedź i tak jest zła.
...
1. Przy badaniu złej odpowiedzi zbierz:
   ...
   - route lub combo providera w Dardcor Code
...
3. Zapisz numer we własnym dashboardzie, runbooku lub trackerze incydentów obok logów Dardcor Code.
...
Możesz pominąć tę sekcję, jeśli nie uruchamiasz pipeline’ów RAG ani agentów za Dardcor Code.
...
2. Zrestartuj Dardcor Code, by nowe wartości env weszły w życie
...
3. Zrestartuj Dardcor Code i przetestuj ponownie z **Dashboard → CLI Tools**
...
- Uruchom `Dardcor Code providers` z CLI, by ponownie odpalić flow OAuth, albo
...
- **GitHub Issues**: [github.com/diegosouzapw/Dardcor Code/issues](https://github.com/diegosouzapw/Dardcor Code/issues)
