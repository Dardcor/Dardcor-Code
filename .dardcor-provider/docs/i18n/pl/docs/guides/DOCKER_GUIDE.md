---
title: "🐳 Przewodnik Docker — Dardcor Code"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Przewodnik Docker — Dardcor Code
...
Dardcor Code dostarcza cztery profile Compose. Wybierz ten, który pasuje do Twojego środowiska.

| Profil            | Usługa           | Kiedy używać                                                                                                                              | Polecenie                                    |
| ----------------- | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (domyślny) | `Dardcor Code-base` | Serwer headless / minimalne runtime, bez dołączonych CLI providerów                                                                       | `docker compose --profile base up -d`        |
| `cli`             | `Dardcor Code-cli`  | Przepływy agentowe wywołujące `Dardcor Code providers/setup/doctor` oraz dołączone CLI (Codex, Claude Code, Droid, OpenClaw)                 | `docker compose --profile cli up -d`         |
| `host`            | `Dardcor Code-host` | Hosty Linux z dostępem do CLI hosta w stylu `network_mode` przez montowanie `~/.local/bin`, `~/.codex`, `~/.claude` itd. tylko do odczytu | `docker compose --profile host up -d`        |
...
Dardcor Code korzysta z Redis jako zaplecza rozproszonego rate limitera i współdzielonej pamięci podręcznej. Usługa `redis` jest **zawsze zdefiniowana** w `docker-compose.yml` (bez bramki profilu) i startuje razem z każdym innym profilem.

| Szczegół             | Wartość                           |
| -------------------- | --------------------------------- |
| Image                | `redis:7-alpine`                  |
| Container name       | `Dardcor Code-redis`                 |
| Internal port        | `6379`                            |
| Host port (override) | `REDIS_PORT` (defaults to `6379`) |
| Volume               | `Dardcor Code-redis-data` → `/data`  |
...
| Image                  | `Dardcor Code:prod` (built from `runner-cli` target)                                  |
| Redis container        | `Dardcor Code-redis-prod` (`redis:8.6.2`, dedicated `redis-prod-data` volume)         |
| Data volume            | `Dardcor Code-prod-data` (named, persisted across rebuilds)                           |
...
```bash
docker build --target runner-base -t Dardcor Code:base .
docker build --target runner-cli  -t Dardcor Code:cli  .
```
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Ścieżka hosta montowana w profilu `cli` pod `/workspace/Dardcor Code` na potrzeby self-update   | `.` (current directory)  |
...
| `OMNIROUTE_BASE_PATH`         | Podścieżka URL, gdy aplikacja jest publikowana za reverse proxy (np. `/Dardcor Code`)           | _(empty = root)_         |
| `NEXT_PUBLIC_BASE_URL`        | Publiczny origin przeglądarki wraz z podścieżką (np. `https://host/Dardcor Code`)               | unset                    |
...
Next.js `basePath` jest kompilowany do bundla standalone. Dardcor Code zapisuje wbudowaną
...
```bash
# .env
OMNIROUTE_BASE_PATH=/Dardcor Code
NEXT_PUBLIC_BASE_URL=https://myhostname.example.com/Dardcor Code
```
...
Opublikowane obrazy `diegosouzapw/Dardcor Code:*` są budowane pod root domeny. Nadal możesz
...
prefiksu). Traefik powinien routować `PathPrefix(`/Dardcor Code`)` do kontenera bez
`StripPrefix`, żeby Next.js otrzymywał `/Dardcor Code/...` i serwował assety z
`/Dardcor Code/_next/...`.
...
Dardcor Code można bezpiecznie udostępnić dzięki automatycznemu provisionowaniu SSL w Caddy. Upewnij się, że rekord DNS A domeny wskazuje na IP Twojego serwera.
...
services:
  Dardcor Code:
    ...
    environment:
      ...
      - BASE_URL=http://Dardcor Code:20128
...
Caddy ustawia standardowe nagłówki forwardingu dla kontenera upstream. Dardcor Code używa
...
chcesz, by Dardcor Code wyprowadzał publiczny origin z zaufanych nagłówków forwarded zamiast z jawnej
konfiguracji.
...
- Quick Tunnels nie są automatycznie przywracane po restarcie Dardcor Code ani kontenera. Włącz je ponownie z dashboardu, gdy potrzeba.
...
- Ustaw `CLOUDFLARED_BIN=/absolute/path/to/cloudflared`, jeśli chcesz, by Dardcor Code używał istniejącego binarium zamiast pobierać własne.

## Tagi obrazów

| Obraz                    | Tag      | Rozmiar | Opis                       |
| ------------------------ | -------- | ------- | -------------------------- |
| `diegosouzapw/Dardcor Code` | `latest` | ~250MB  | Najnowsze stabilne wydanie |
| `diegosouzapw/Dardcor Code` | `3.8.0`  | ~250MB  | Bieżąca wersja             |
...
- **Tryb SQLite WAL:** `docker stop` powinien móc się dokończyć, żeby Dardcor Code mógł zrobić checkpoint najnowszych zmian z powrotem do `storage.sqlite`. Dołączone pliki Compose ustawiają już 40s grace period stopu. Przy bezpośrednim uruchomieniu obrazu zachowaj `--stop-timeout 40`.
