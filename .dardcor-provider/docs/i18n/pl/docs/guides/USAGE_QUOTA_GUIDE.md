...
> **TL;DR**: Dardcor Code śledzi zużycie tokenów każdego żądania, liczy koszt, egzekwuje limity per klucz API i pokazuje analitykę na dashboardzie. Ten przewodnik wyjaśnia, jak to działa.
...
Każde żądanie przechodzące przez Dardcor Code generuje **rekord użycia**, który rejestruje:
...
Dla providerów, które nie zwracają usage (niektóre providery web-cookie), Dardcor Code **szacuje** tokeny heurystyką `~4 chars per token` (zob. `open-sse/services/autoCombo/pipelineRouter.ts`).

### Tokeny z cache

Dardcor Code śledzi `cached_tokens` osobno od `prompt_tokens`, ponieważ:
...
Dla modeli bez danych cenowych Dardcor Code przechodzi na **szacowanie kosztu** wewnętrznymi średnimi stawkami (źródło: dane cenowe LiteLLM).
...
| Narzędzie               | Opis                                            |
| ----------------------- | ----------------------------------------------- |
| `Dardcor Code_cost_report` | Generuje raport kosztów per klucz za dany okres |
| `Dardcor Code_check_quota` | Zwraca bieżący status limitu dla klucza API     |
