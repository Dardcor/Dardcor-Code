---
title: "Claude Code CLI — konfiguracja z Dardcor Code"
version: 3.8.40
lastUpdated: 2026-07-24
---

# Claude Code CLI — konfiguracja z Dardcor Code

Skieruj CLI **Claude Code** (`claude`) na Dardcor Code — lokalnie lub na zdalny VPS —
...
```bash
# Launch Claude Code against a local Dardcor Code (auto-detects the active context)
Dardcor Code launch

# Against a remote Dardcor Code (after `Dardcor Code connect <host>`, this is automatic)
Dardcor Code launch --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Generate per-model profiles, then launch one
Dardcor Code setup-claude            # writes ~/.claude/profiles/<name>/settings.json
Dardcor Code launch --profile glm52  # Claude Code using glm/glm-5.2 via Dardcor Code
```
...
| `ANTHROPIC_AUTH_TOKEN`                       | Wysyłany jako `Authorization: Bearer …` — użyj tokenu dostępu / klucza API Dardcor Code       |
...
`Dardcor Code launch` ustawia to wszystko za Ciebie: rozwiązuje bazowy URL + token
z aktywnego kontekstu (więc `Dardcor Code connect <vps>`, a potem `Dardcor Code launch`
...
picker `/model` zwykle pokazuje **tylko** modele Claude/Anthropic z Dardcor Code — `kimi/kimi-k2.6`
lub `glm/glm-5.2` się nie pojawią, mimo że routują poprawnie.

Dardcor Code może odzwierciedlić dowolny włączony model (i combo) pod identyfikatorem `claude/…`,
aby przeszedł ten filtr i pojawił się w pickerze:

```
kimi/kimi-k2.6            →  claude/kimi/kimi-k2.6      "Kimi K2.6 (Dardcor Code)"
glm/glm-5.2              →  claude/glm/glm-5.2         "GLM 5.2 (Dardcor Code)"
<combo "custo-otimizado"> →  claude/combo/custo-otimizado
```

Gdy wybierzesz jeden z nich w Claude Code, Dardcor Code zdejmuje opakowanie `claude/`
...
wygrywa), więc zwykły Dardcor Code nie podwaja katalogu dla klientów, które nie używają
Claude Code:
...
{
  "env": {
    "ANTHROPIC_BASE_URL": "http://<your Dardcor Code>:20128",
...
`Dardcor Code setup-claude` pobiera żywy katalog `/v1/models` i zapisuje jeden
...
> `Dardcor Code launch --profile <name>` (wstrzykuje `ANTHROPIC_AUTH_TOKEN` z
> aktywnego kontekstu) albo sam wyeksportuj `ANTHROPIC_AUTH_TOKEN` i uruchom
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync po discovery modeli (opt-in).** Dardcor Code może regenerować te same
...
Dardcor Code setup-claude --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Only some providers
Dardcor Code setup-claude --only glm,kimi

# Preview without writing
Dardcor Code setup-claude --dry-run

# Launch a profile
Dardcor Code launch --profile kimi-k27
```
...
Claude Code routuje do poziomów możliwości. Zmapuj każdy na model Dardcor Code przez env /
...
Po uruchomieniu `Dardcor Code connect <host>` (zobacz
[Tryb zdalny](./REMOTE-MODE.md)), `Dardcor Code launch` i `Dardcor Code setup-claude`
...
`/v1`**, i zrestartuj `claude` (env jest odczytywane raz przy starcie). `Dardcor Code launch`
robi to za Ciebie.
...
Dardcor Code odmawia zgadywania. Napraw na jeden z dwóch sposobów: przypnij prefiksowane id przez
...
**Błędy auth** — profil nie przechowuje tokenu. Użyj `Dardcor Code launch --profile`
