---
title: "Codex CLI — konfiguracja z Dardcor Code"
version: 3.8.49
lastUpdated: 2026-07-26
---

# Codex CLI — konfiguracja z Dardcor Code

Kompletny przewodnik po używaniu Codex CLI skierowanego na Dardcor Code jako backend zgodny z OpenAI.
...
**Dardcor Code rozwiązuje to w sposób przezroczysty:**
...
Przy Dardcor Code nie potrzebujesz osobnego proxy tłumaczącego. **Wszystkie modele używają `wire_api = "responses"`** — resztę obsługuje Dardcor Code.
...
| Model                                | ID Dardcor Code                         | Okno kontekstu           | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ------------------------ | -------------- | ------------------- |
...
Wszystkie modele Codex w Dardcor Code używają prefiksu `cx/`:

| Nazwa w Codex CLI       | Model Dardcor Code    |
| ----------------------- | ------------------ |
...
Inni providerzy używają własnego prefiksu (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — prefiks odpowiada aliasowi providera w Dardcor Code.
...
## Automatyczne generowanie profili przez `Dardcor Code setup-codex`

Gdy Dardcor Code działa na VPS, możesz automatycznie wygenerować pliki profili z żywego katalogu modeli:
...
Dwa domyślne ustawienia Dardcor Code mogą cicho psuć wielogodzinne sesje Codex CLI. Żadne nie jest ustawieniem Codex CLI — oba leżą po stronie Dardcor Code. Użytkownicy migrujący konfigurację z upstreamowych proxy, które pinują konta i wyłączają idle cutoff, często trafiają na oba i wnioskują, że Dardcor Code „nie utrzyma długiej sesji”.
...
Powiązane dyskusje: [#7126](https://github.com/diegosouzapw/Dardcor Code/discussions/7126) (zrywanie długich zadań), [#5718](https://github.com/diegosouzapw/Dardcor Code/discussions/5718) (dlaczego affinity jest domyślnie wyłączone). Tracking: [#7287](https://github.com/diegosouzapw/Dardcor Code/issues/7287).
...
Krytyczny szczegół: syntetyczny SSE **heartbeat Dardcor Code nie resetuje** zegara idle. Tylko prawdziwy chunk body z upstream aktualizuje `lastChunkTime`. Cichy model, który nadal „myśli”, wygląda dla watchdogu tak samo jak zawieszony upstream.
...
W środowisku procesu Dardcor Code (`.env` / compose / systemd):
...
Zrestartuj Dardcor Code po zmianie tych zmiennych env.
...
2. **Podnieś / wyłącz idle cutoff** w środowisku Dardcor Code:
...
4. Zrestartuj Dardcor Code, potem uruchom długie zadanie Codex.
...
Gdy odpala się idle watchdog, Dardcor Code loguje linię w stylu:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grepuj `Idle timeout: no data from` (albo kod `stream_idle_timeout` / nazwę błędu `StreamIdleTimeoutError`). Segment providera to to, czego Dardcor Code użyło dla tego requestu (`codex`, inny id providera albo `provider`, gdy nieznany) — nie zawsze jest to dosłowny string `codex`.
...
Sprawdź, czy model istnieje w Dardcor Code z poprawnym prefiksem. Użyj `Dardcor Code models list` albo otwórz `/dashboard/providers/<provider>`.

**`Authentication error`**
Potwierdź, że `OMNIROUTE_API_KEY` jest wyeksportowany: `echo $OMNIROUTE_API_KEY`.

**`Connection refused`**
Sprawdź, czy Dardcor Code działa i że host/port w `base_url` jest poprawny dla Twojej sieci (lokalnie vs Tailscale vs VPS).
...
Zobacz [Zadania długotrwałe](#zadania-długotrwałe). Włącz session affinity (TTL powyżej długości zadania) i podnieś lub wyłącz `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grepuj logi Dardcor Code pod kątem `Idle timeout: no data from`.
