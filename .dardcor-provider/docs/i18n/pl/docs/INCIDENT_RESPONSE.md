# Runbook reagowania na incydenty — Dardcor Code (2026-06-18)
...
| **SEV-3** | Uśpiony błąd lub near-miss; brak bieżącego wpływu na użytkownika, ale budżet błędów zagrożony.               | Wyciek pamięci w trendzie wzrostowym; circuit breaker wyłącza jednego dostawcę. | Slack `#Dardcor Code-ops` (następny standup) | 7 d                          |
...
| Panele SLO w Grafana              | Panele burn-rate SLO                 | Slack `#Dardcor Code-ops`                |
...
Alerty **nie** idą na prywatne DM. Domyślny kanał to `#Dardcor Code-ops`; PagerDuty
...
2. **Zadeklaruj**. Opublikuj w `#Dardcor Code-ops`:
   ```
...
3. **Stabilizuj** przed diagnozą główną przyczyny. Preferowana kolejność:
   - Odetnij zły deploy: `kubectl rollout undo deploy/Dardcor Code` (lub
...
- **Wewnętrzna**: `#Dardcor Code-ops` jest źródłem prawdy. Wątek na incydent na
  deklarację; wszystkie aktualizacje w wątku.
- **Zewnętrzna** (gdy dotyczy użytkowników zewnętrznych): status page
  (status.Dardcor Code.example — zastąp prawdziwym URL, gdy będzie live). SEV-1
