---
title: "Architektura Dardcor Code"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Architektura Dardcor Code
...
Dardcor Code to lokalna brama routingu AI i panel (dashboard) zbudowane na Next.js.
...
Dardcor Code może instalować, nadzorować i routować do lokalnie działających procesów narzędzi AI
...
- Builder metadanych odpowiedzi Dardcor Code: `src/domain/Dardcor CodeesponseMeta.ts`
...
- plik: `${DATA_DIR}/storage.sqlite` (lub `$XDG_CONFIG_HOME/Dardcor Code/storage.sqlite` gdy ustawione, w przeciwnym razie `~/.Dardcor Code/storage.sqlite`)
...
> Dardcor Code v3.8.0. Kanoniczna i stale aktualizowana lista: zob.
...
- końcowa odpowiedź klienta zwrócona przez Dardcor Code; odpowiedzi streamowane są przechowywane w tej samej zwartej formie podsumowania
...
1. `usageDb` i `localDb` współdzielą tę samą politykę katalogu bazowego (`DATA_DIR` -> `XDG_CONFIG_HOME/Dardcor Code` -> `~/.Dardcor Code`) z migracją plików legacy.
...
5. Katalog `open-sse/` jest publikowany jako **pakiet npm workspace** `@Dardcor Code/open-sse`. Kod źródłowy importuje go przez `@Dardcor Code/open-sse/...` (rozwiązywane przez Next.js `transpilePackages`). Ścieżki plików w tym dokumencie nadal używają nazwy katalogu `open-sse/` dla spójności.
...
- Build obrazu Docker: `docker build -t Dardcor Code .`
