...
Dardcor Code ma potok autoryzacji zależny od trasy, który bramkuje każde żądanie API. Klasyfikacja jest **deterministyczna** i **fail-closed** — wszystko, czego nie da się sklasyfikować, trafia do `MANAGEMENT` i wymaga sesji albo tokenu klasy management. Ta strona opisuje model dla inżynierów utrzymujących trasy lub projektujących nowe endpointy.
...
Incoming request → src/proxy.ts
  → runAuthzPipeline() in src/server/authz/pipeline.ts
    1. Strip trusted internal headers (x-Dardcor Code-auth-*, x-Dardcor Code-route-class)
    ...
    8. Otherwise: POLICIES[routeClass].evaluate(ctx)
       - allow  → stamp x-Dardcor Code-auth-{kind,id,label,scopes} → NextResponse.next()
       - reject → JSON error w/ correlation_id (dashboard pages → 302 /login)
```

Zaufane nagłówki wewnętrzne (zdefiniowane w `src/server/authz/headers.ts`) są **usuwane z przychodzących żądań** przed klasyfikacją — klienci nie mogą wcześniej ustawić `x-Dardcor Code-auth-*`, aby podszyć się pod subject.
...
x-Dardcor Code-route-class:    PUBLIC | CLIENT_API | MANAGEMENT
```

Dla uwierzytelnionych żądań nagłówki żądania upstream (po stronie handlera) zawierają także:

```
x-Dardcor Code-auth-kind:      client_api_key | dashboard_session | management_key | anonymous
x-Dardcor Code-auth-id:        key_<last-4> | "dashboard" | "anonymous"
x-Dardcor Code-auth-label:     (optional)
x-Dardcor Code-auth-scopes:    comma-separated list
```
