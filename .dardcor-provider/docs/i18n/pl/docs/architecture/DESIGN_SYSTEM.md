...
# Dardcor Code — System designu i tożsamość wizualna
...
> **Zakres:** dashboard Dardcor Code (`src/`) i strona marketingowa (`_mono_repo/Dardcor CodeSite/`) dzielą **jedną tożsamość wizualną** — ta sama tapeta siatki w kratkę (32px), te same tokeny kolorów, ustandaryzowane komponenty.
>
...
Strona marketingowa (`viral.Dardcor Code.online`, `why.Dardcor Code.online`, `Dardcor Code.online`) i dashboard produktu powinny wyglądać jak **jeden produkt**. Strona już pożyczyła paletę z dashboardu — jej `css/tokens.css` wręcz mówi _"Palette mirrors the Dardcor Code dashboard (src/app/globals.css)"_. Zatem obie są już zsynchronizowane w ~80% na poziomie kolorów. Czego brakuje na dashboardzie:
...
Dokładna receptura ze strony (`_mono_repo/Dardcor CodeSite/css/base.css`): **stały, pełnoekranowy pseudo-element** malujący dwa gradienty linii 1px, siedzący na `z-index:-1` za całą treścią.
...
| Referencja strony                    | `_mono_repo/Dardcor CodeSite/css/tokens.css`, `css/base.css`                                                            |
