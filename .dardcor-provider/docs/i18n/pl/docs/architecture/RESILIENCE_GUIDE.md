...
Dardcor Code ma trzy odrębne, ale powiązane mechanizmy odporności. Każdy ma inny zakres i cel. Trzymaj je osobno przy debugowaniu zachowania routingu.
...
**Zakres:** jedna sesja klienta (nagłówek `X-Session-Id` / `x-codex-session-id` / `x-Dardcor Code-session`) przypięta do jednego połączenia, dla **dowolnego** providera.
