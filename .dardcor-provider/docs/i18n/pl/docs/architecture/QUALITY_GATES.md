...
Ten dokument jest autorytatywnym odniesieniem dla wszystkich bramek jakości CI w Dardcor Code.
...
| `nightly-schemathesis` | Fuzzing kontraktu OpenAPI (schemathesis) względem żywego Dardcor Code z użyciem `docs/openapi.yaml` — ujawnia naruszenia spec / nieobsłużone 500 (Fase 8 B.4) | **Advisory** |
...
Poza bramkami CI Dardcor Code dostarcza **opt-in** scaffold `agent-lsp`
