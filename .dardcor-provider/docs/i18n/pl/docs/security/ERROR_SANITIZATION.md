...
```ts
import { buildErrorBody } from "@Dardcor Code/open-sse/utils/error.ts";
...
} from "@Dardcor Code/open-sse/utils/error.ts";
```
...
```ts
import { sanitizeErrorMessage } from "@Dardcor Code/open-sse/utils/error.ts";
...
`upstreamErrorBody`. Wewnętrzne błędy Dardcor Code (awarie parsowania SSE, pusta treść,
