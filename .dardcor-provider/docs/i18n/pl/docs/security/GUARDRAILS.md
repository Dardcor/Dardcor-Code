...
między Dardcor Code a upstream providerami. Każdy guardrail może sprawdzać (oraz
...
- nagłówek `x-Dardcor Code-disabled-guardrails` (lub legacy
...
- **`garak` (advisory)** — uruchamia garak `--probes promptinject,dan,leakreplay`
  przeciw lokalnej instancji Dardcor Code (`http://localhost:20128/v1`). Bramkowany
