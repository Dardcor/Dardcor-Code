...
Komendy CLI Dardcor Code uwierzytelniają się względem lokalnego management API za pomocą
tokenu `HMAC-SHA256(machine-id, salt)` wysyłanego w nagłówku żądania
`x-Dardcor Code-cli-token`.

Dzięki temu podkomendy CLI (`Dardcor Code status`, `Dardcor Code providers` itd.)
...
3. CLI wysyła token jako `x-Dardcor Code-cli-token` w każdym żądaniu do
...
Domyślna sól: `Dardcor Code-cli-auth-v1`
