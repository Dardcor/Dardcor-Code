...
TPROXY transparent decrypt to **5. tryb przechwytywania** Dardcor Code w stosie MITM
...
zmiany konfiguracji** — ale musi ufać dynamicznemu CA instalowanemu przez Dardcor Code
...
"TPROXY decrypt requires Linux + root + the native addon", a reszta Dardcor Code
działa normalnie.
...
> i instaluje pod istniejącym slotem trust-store `Dardcor Code-mitm.crt`
> (zastępując stary pojedynczy leaf — bez potrzeby dual-trust cleanup),
> w pełni oddzielony od slotu TPROXY `Dardcor Code-tproxy-ca.crt` opisanego poniżej.
...
- Generuje długowieczne CA przez `generateMitmCa()` (CN `"Dardcor Code MITM CA"`,
...
`Dardcor Code-tproxy-ca.crt` (stała `TPROXY_CA_CERT_NAME`) — oddzielonym od
slotu statycznego certu MITM (`Dardcor Code-mitm.crt`), żeby te dwa nigdy się nie nadpisywały.
...
| **Dedicated CA slot**            | The dynamic CA installs to `Dardcor Code-tproxy-ca.crt`, never clobbering the static MITM cert.                                                                                                                                                                                                                                                                       |
...
host, oznacza, że wszystko przechwycone przez Dardcor Code może zostać odszyfrowane. Jest bramkowane przez
...
- Potwierdź, że proces ufa dynamicznemu CA. CA jest instalowane pod
  `Dardcor Code-tproxy-ca.crt`; aplikacje z własnym trust store (Firefox/Chrome NSS)
...
trybu cofa reguły; jeśli Dardcor Code zostało zabite w trakcie sesji, użyj akcji AgentBridge
