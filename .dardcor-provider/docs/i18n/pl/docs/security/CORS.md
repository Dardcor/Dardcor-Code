...
Dardcor Code kontroluje, które **origin przeglądarki** mogą odczytywać odpowiedzi cross-origin
...
traktować poważnie, ale ekspozycja Dardcor Code jest **węższa niż w przypadku ogólnym**,
...
> `Access-Control-Allow-Credentials: true`. Wspólna ścieżka CORS Dardcor Code nigdy
> tego nie robi.
...
- Jeśli Dardcor Code działa za reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
```bash
# Vite (5173) + Next.js (3000) dev servers calling a local Dardcor Code
...
## Przykład: reverse proxy przed Dardcor Code

CORS jest egzekwowany przez samo Dardcor Code, więc proxy generalnie **nie powinno** dodawać ani
przepisywać nagłówków `Access-Control-*` (podwójne nagłówki psują przeglądarki). Terminuj TLS
i forwarduj — niech Dardcor Code odpowiada na preflight:

```nginx
# nginx — forward to Dardcor Code; do NOT inject Access-Control-* here
location / {
...
Ustaw dozwolone originy przeglądarki w Dardcor Code (`CORS_ALLOWED_ORIGINS` lub
