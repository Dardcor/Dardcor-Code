...
Wszystkie trasy management API Dardcor Code są klasyfikowane do jednego z trzech
...
operator wystawił Dardcor Code za tunelem nginx/Cloudflare/Tailscale i wyciekł JWT,
albo auth był źle skonfigurowany — atakujący zamienia „wywołaj API” na „uruchom
komendę na hoście” (remote code execution). Dardcor Code zamyka to, egzekwując
...
Jeśli uruchamiasz Dardcor Code za reverse proxy lub tunelem (nginx, Caddy, Cloudflare
...
- `403 LOCAL_ONLY` w logach Dardcor Code dla jednej z tych ścieżek to strażnik
