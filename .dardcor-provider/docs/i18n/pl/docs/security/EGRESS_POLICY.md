...
Dardcor Code pozwala każdemu proxy nieść **dyrektywę egress rodziny adresów**. Domyślnie system operacyjny wybiera IPv4 lub IPv6 (dual-stack, „Happy Eyeballs”). Gdy ustawisz dyrektywę na `ipv4` lub `ipv6`, Dardcor Code przypina każde połączenie przez to proxy do wybranej rodziny i **fails closed** (odmawia), zamiast robić fallback na drugą rodzinę.
...
Wprowadzone w PR [#3777](https://github.com/diegosouzapw/Dardcor Code/pull/3777). Problemy motywujące:
...
Gdy `family` to `auto`, Dardcor Code **nie** dokleja żadnej dyrektywy — URL proxy jest używany as-is, a rodzina połączenia wynika wewnętrznie.
...
Gdy budowany jest dispatcher, marker jest odczytywany i zamieniany na konkretną rodzinę połączenia. Jeśli host to literał IP **przeciwnej** rodziny, Dardcor Code rzuca wyjątek (sprzeczność = fail-closed):
...
Pin rodziny działa z proxy SOCKS5, ale stockowe `fetch-socks` nie udostępnia opcji socket potrzebnych do przypięcia rodziny hopu proxy. Dardcor Code dostarcza własny connector do tego:
