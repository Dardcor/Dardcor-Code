...
Dardcor Code integruje się z providerami, których edge aktywnie fingerprintuje nieoficjalne klienty (TLS JA3/JA4, kolejność nagłówków, kształt body JSON, tokeny integralności). Ta strona dokumentuje powierzchnie stealth, które Dardcor Code udostępnia, oraz miejsca ich implementacji.

## Uwaga prawna i etyczna

Funkcje stealth istnieją po to, by Dardcor Code mógł działać jako warstwa kompatybilności między oficjalnymi kontami użytkownika (Claude Code CLI, ChatGPT Desktop/Web, Antigravity, Cursor itd.) a ujednoliconym API Dardcor Code. **Nie** służą do omijania fraud detection, współdzielenia poświadczeń ani naruszania Terms of Service providera. Maintainerzy oczekują, że operatorzy będą przestrzegać upstream ToS, które zaakceptowali przy tworzeniu kont.
...
Gdy `cliCompatMode` jest włączony, Dardcor Code przekształca wychodzące żądania Claude tak, by były nieodróżnialne od ruchu `claude-cli`. Współpracują trzy moduły:
...
Server-side check integralności, który oficjalne Claude Code CLI liczy przez Bun/Zig. Dardcor Code reimplementuje to z `xxhash-wasm`:
...
Żądania Antigravity zachowują tekst wywołującego bajt po bajcie. Dardcor Code nie wstawia zero-width characters do promptów ani nie zmienia nazw/nie wstrzykuje tooli, by udawać klienta IDE.
...
Egzekucja upstream jest po **stronie Google**, nie ma nic, co Dardcor Code mógłby temu zapobiec. Nazwa zmiennej env i istniejąca dokumentacja sprawiają wrażenie bezpiecznego przełącznika; nim nie jest.
...
- Wielu użytkowników Dardcor Code uderzających w overage credits równolegle z tego samego zewnętrznego IP wzmacnia sygnał.
...
- Oryginalny raport incydentu: Discussion [#1183](https://github.com/diegosouzapw/Dardcor Code/discussions/1183)
...
Dla CLI, których binarów nie da się przekierować przez `OPENAI_BASE_URL`, Dardcor Code uruchamia lokalny proxy z terminacją TLS. Endpointy są pod `src/app/api/cli-tools/antigravity-mitm/`.
...
Nazwa pliku cert: `Dardcor Code-mitm.crt`. Dopasowanie fingerprintu przez `getCertFingerprint()` (SHA-1 z DER).

Dodatkowo `updateNssDatabases()` instaluje do per-user NSS DB, gdy dostępne jest `certutil`: `~/.pki/nssdb`, `~/snap/chromium/.../nssdb`, wszystkie profile Firefox (w tym snap), pod nickiem **`Dardcor Code MITM Root CA`**.
...
Dardcor Code czyści inbound nagłówki klienta przed forwardem, by żądanie przychodzące z Cursor nie wyciekało `User-Agent: Cursor/X.Y.Z` do upstream Claude. Zobacz `src/shared/constants/upstreamHeaders.ts` — denylist utrzymywany w lockstep ze schematami Zod i testami jednostkowymi.
