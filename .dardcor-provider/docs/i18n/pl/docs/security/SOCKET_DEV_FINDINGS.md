...
description: "Atestacja maintainerów dla znalezisk AI-detected potential-malware zgłoszonych wobec Dardcor Code oraz mitigacji z v3.8.6 zastosowanych w każdym oznaczonym call site."
---

# Socket.dev / atestacja znalezisk supply-chain

Ten dokument to atestacja napisana przez maintainerów dla sześciu
znalezisk `AI-detected potential malware` zgłoszonych wobec `Dardcor Code@3.8.5` oraz
mitigacji wprowadzonych w `Dardcor Code@3.8.6`. Istnieje po to, aby:
...
Caddy. Fakt, że istnieją w Dardcor Code, jest udokumentowany w
...
którzy już używają Zed i chcą odzwierciedlić klucze providerów w Dardcor Code
...
uprawnień”. W Dardcor Code są używane wyłącznie przez ścieżkę instalacji certyfikatu MITM
...
## §5 — Zapis zwrotny credentiali Dardcor Code Cloud Sync (`api/keys/[id]/route.js`)
...
**Dlaczego to zostawiamy**: Cloud Sync to jedyny sposób, by tenant Dardcor Code Cloud
...
Wynikowy bundle ma być publikowany jako `Dardcor Code-secure`. Zob.
