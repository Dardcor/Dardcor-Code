...
> proxy w Dardcor Code. To jest cięcie v1: pojedynczy operator wkleja linki
> subskrypcji, wybiera tryb (global lub rule), a Dardcor Code wiąże wynikową pulę
...
Dziś pula proxy Dardcor Code jest ręcznie kuratorowana: każdy węzeł żyje w
...
Dardcor Code i ponownie je wiązać przy każdej zmianie listy upstream.

Celem v1 jest uczynienie Dardcor Code first-class dla subskrypcji
...
| U5  | Operator | podać lokalny endpoint SOCKS5 sing-box/clash                          | węzły SS/VMess/Trojan/VLESS (których dispatcher Dardcor Code nie mówi natywnie) stawały się używalne przez lokalny mostek kernela |
