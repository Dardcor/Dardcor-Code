---
title: "Silnik Dardcor Code Auto-Combo"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Silnik Dardcor Code Auto-Combo
...
1. Dardcor Code wykrywa prefiks `auto/` w `src/sse/handlers/chat.ts`
...
per-API-key (tabela `auto_candidate_overrides`, migracja `128`) — Dardcor Code jest
...
[#6940](https://github.com/diegosouzapw/Dardcor Code/issues/6940). Ponieważ
...
[#3227/#3233](https://github.com/diegosouzapw/Dardcor Code/issues/3227) i jest
...
| `X-Dardcor Code-Budget`          | a positive number (max USD per request)                                                                                                                                                 | Twardy sufit kosztu: kandydaci, których szacowany koszt go przekracza, są filtrowani przed selekcją. Co się dzieje, gdy **każdy** kandydat przekracza limit, kontroluje `X-Dardcor Code-Budget-Fallback` poniżej.                        |
| `X-Dardcor Code-Budget-Fallback` | `cheapest` (default, aliases: `cheapest-viable`, `soft`) or `strict` (aliases: `block`, `hard`)                                                                                         | `cheapest`: fallback do globalnie najtańszego kandydata nawet jeśli nadal przekracza cap (zachowanie legacy). `strict`: odmawia selekcji — żądanie fail-fast z `HTTP 402` zamiast cichego overspend. Nieznane wartości są ignorowane. |
...
Silnik combo Dardcor Code obsługuje **18 strategii routingu** (zadeklarowanych w `src/shared/constants/routingStrategies.ts` → `ROUTING_STRATEGY_VALUES`). Sam silnik Auto Combo jest wystawiony pod strategią `auto`; pozostałe są dostępne dla persystowanych combo.
...
| Komenda                                | Co robi                                                                         |
| :------------------------------------- | :------------------------------------------------------------------------------ |
| `npm run test:combo:live`              | In-process real routing z `RUN_COMBO_LIVE=1`; snapshot żywej DB Dardcor Code       |
| `npm run test:combo:live:vps`          | Wywołania HTTP przeciw żywemu serwerowi Dardcor Code (ustaw `COMBO_LIVE_BASE_URL`) |
