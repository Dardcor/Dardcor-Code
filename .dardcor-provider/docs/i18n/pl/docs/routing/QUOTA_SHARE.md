...
**Problem, który rozwiązuje:** Dardcor Code proxy’uje wiele kluczy API względem
...
- Cała persystencja w istniejącej bazie SQLite Dardcor Code (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("Dardcor Code:quota-share:pools")` jest niepuste.
