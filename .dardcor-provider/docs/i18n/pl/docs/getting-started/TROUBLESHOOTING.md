...
Po `npm install -g Dardcor Code` możesz zobaczyć lawinę ostrzeżeń typu `npm warn ERESOLVE`, komunikaty o peer-dependency oraz `deprecated`. **Są one oczekiwane i nieszkodliwe.** Instalacja się powiodła, jeśli w wyniku widać `added <N> packages`.

Ostrzeżenia pochodzą z przestarzałych zakresów peer-dependency w pakietach firm trzecich, których Dardcor Code nie kontroluje:
...
| EACCES: permission denied                           | Ustaw `DATA_DIR=/path/to/writable/dir`, aby nadpisać `~/.Dardcor Code`                                                                                                        |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Uruchom `cd $(npm root -g)/Dardcor Code/app && npm rebuild better-sqlite3 && Dardcor Code` — zobacz [przebudowa modułu natywnego na macOS](#macos-native-module-rebuild) poniżej |
...
**Przyczyna:** Uruchamiasz wersję Node.js poniżej zatwierdzonego bezpiecznego poziomu runtime Dardcor Code. Najczęstszy przypadek to starszy patch Node 22 lub 24 poniżej wymaganego przez Dardcor Code poziomu bezpieczeństwa.
...
**Przyczyna:** Po globalnym `npm install -g Dardcor Code` natywna binarka `better-sqlite3` w pakiecie mogła zostać skompilowana pod inną architekturę lub ABI Node.js niż ta, która działa lokalnie. To częste na macOS (Apple Silicon i Intel), gdy prebuilt nie pasuje do środowiska.
...
**Naprawa (v3.5.5+):** Dardcor Code używa teraz własnej funkcji `fetch()` z undici, gdy aktywny jest dispatcher proxy, co zapewnia spójne zachowanie. Zaktualizuj do v3.5.5+.
...
Gdy wiele równoległych żądań trafia w providera z limitem zapytań, Dardcor Code używa mutexa + auto rate-limiting, aby serializować żądania i zapobiegać awariom kaskadowym. Działa to automatycznie dla providerów z kluczem API.

---

## Opcjonalna taksonomia awarii RAG / LLM (16 problemów)

Część użytkowników Dardcor Code stawia bramkę przed stackami RAG lub agentów. W takich setupach często widać dziwny wzorzec: Dardcor Code wygląda na zdrowe (providery w górze, profile routingu OK, brak alertów rate limit), a ostateczna odpowiedź i tak jest błędna.
...
1. Gdy badziesz złą odpowiedź, zbierz:
   ...
   - trasę lub combo providerów w Dardcor Code
...
Możesz zignorować tę sekcję, jeśli nie uruchamiasz pipeline'ów RAG ani agentów za Dardcor Code.
...
2. Zrestartuj Dardcor Code, aby nowe wartości env zostały wczytane
...
3. Zrestartuj Dardcor Code i przetestuj ponownie w **Dashboard → CLI Tools**
...
- Uruchom `Dardcor Code providers` z CLI, aby ponownie wywołać przepływ OAuth, albo
