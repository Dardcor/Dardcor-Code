...
> **TL;DR**: Dardcor Code agreguje darmowe plany od 50+ providerów. Podłącz wielu darmowych providerów, aby uzyskać nielimitowane darmowe AI z automatycznym fallbackiem.
...
Dardcor Code **agreguje** te darmowe plany w jeden endpoint. Zamiast rejestrować się w 10 różnych serwisach, podłączasz je wszystkie do Dardcor Code i używasz `model: "auto"`, aby automatycznie wybrać najlepszą darmową opcję dla każdego żądania.
...
Siła Dardcor Code to **stackowanie darmowych planów**. Zamiast polegać na jednym providerze, podłączasz wielu darmowych providerów i pozwalasz Dardcor Code automatycznie wybrać najlepszego dla każdego żądania.
...
Następnie użyj `model: "auto"`, a Dardcor Code:
...
## Jak Dardcor Code usprawnia darmowe plany

### 1. Automatyczny fallback

Jeśli jeden darmowy provider jest zajęty lub niedostępny, Dardcor Code automatycznie próbuje następnego. Nie musisz nic robić.

### 2. Inteligentny routing

Dardcor Code wybiera **najlepszego darmowego providera** dla każdego żądania na podstawie:
...
Funkcja **compression** w Dardcor Code oszczędza 15–95% tokenów. Dzięki temu darmowy limit starcza **5–20× dłużej**.

### 4. Obsługa wielu kont

Jeśli masz wiele kont u tego samego providera, Dardcor Code traktuje każde jako osobnego kandydata. To podwaja lub potroja Twój darmowy limit.
...
**Tak!** To oficjalne darmowe plany providerów. Dardcor Code tylko ułatwia korzystanie z nich wszystkich naraz.
...
Nie ma haczyka! Providerzy oferują darmowe plany, żeby przyciągnąć użytkowników. Dardcor Code tylko ułatwia korzystanie z nich wszystkich naraz.
...
- **[Przewodnik Auto-Combo](./AUTO-COMBO-GUIDE.md)** — pozwól Dardcor Code wybrać najlepsze AI
