# Przewodnik po providerach: podłącz modele AI do Dardcor Code

> **TL;DR**: Provider to połączenie z usługą AI (np. OpenAI, Anthropic, Google). Potrzebujesz co najmniej jednego providera, aby korzystać z Dardcor Code.

---

## Czym jest provider?

Traktuj providera jak **operatora komórkowego**. Tak jak potrzebujesz operatora, by dzwonić, tak potrzebujesz providera AI, by korzystać z modeli AI. Dardcor Code jest jak telefon, który działa z **wszystkimi operatorami** — możesz między nimi przełączać się automatycznie.
...
Dardcor Code działa najlepiej z **wieloma providerami**. Daje to:

- **Automatyczny fallback** — jeśli jeden provider zawiedzie, Dardcor Code próbuje kolejnego
- **Optymalizacja kosztów** — Dardcor Code wybiera najtańszego providera dla każdego żądania
- **Optymalizacja szybkości** — Dardcor Code wybiera najszybszego providera dla każdego żądania
- **Optymalizacja jakości** — Dardcor Code wybiera najlepszego providera do danego zadania
...
Następnie używaj `model: "auto"`, a Dardcor Code automatycznie wybierze najlepszy wariant dla każdego żądania.
...
2. W Dardcor Code: Providers → Add Provider → OpenAI
3. Wklej klucz API → Connect

### Anthropic

1. Pobierz klucz API: https://console.anthropic.com/
2. W Dardcor Code: Providers → Add Provider → Anthropic
3. Wklej klucz API → Connect

### Google (Gemini)

1. Pobierz klucz API: https://aistudio.google.com/apikey
2. W Dardcor Code: Providers → Add Provider → Gemini
3. Wklej klucz API → Connect

### DeepSeek

1. Pobierz klucz API: https://platform.deepseek.com/
2. W Dardcor Code: Providers → Add Provider → DeepSeek
3. Wklej klucz API → Connect

### Groq

1. Pobierz klucz API: https://console.groq.com/
2. W Dardcor Code: Providers → Add Provider → Groq
3. Wklej klucz API → Connect
...
### „Czy muszę płacić, żeby korzystać z Dardcor Code?”

**Nie!** Dardcor Code jest darmowy i open-source. Możesz używać darmowych providerów (Kiro, OpenCode Free, Pollinations) bez żadnych opłat. Płacisz tylko wtedy, gdy zdecydujesz się na płatnych providerów.
...
**Tak!** Na tym właśnie polega Dardcor Code. Podłącz wielu providerów i używaj `model: "auto"`, aby Dardcor Code wybierał najlepszy wariant dla każdego żądania.

### „Co jeśli provider padnie?”

Dardcor Code automatycznie pomija niedziałających providerów i próbuje kolejnego. Nic nie musisz robić.
...
**Tak!** Jeśli masz już klucze API do OpenAI, Anthropic, Google itd., możesz ich użyć w Dardcor Code. Wystarczy wkleić je przy podłączaniu providera.
