# Auto-Combo: niech Dardcor Code wybierze najlepsze AI za Ciebie

> **TL;DR**: Ustaw model na `auto`, a Dardcor Code automatycznie wybierze najlepszego providera AI dla każdego żądania. Nie wymaga konfiguracji.

---

## Co robi

Zamiast wybierać konkretny model AI (np. GPT-4o lub Claude), możesz pozwolić Dardcor Code **automatycznie wybrać najlepszy** dla każdego żądania. Bierze pod uwagę:
...
Dardcor Code ocenia wszystkich podłączonych providerów i wybiera najlepszego. Jeśli ten zawiedzie, automatycznie próbuje kolejnego.
...
**Krok 2**: To wszystko! Dardcor Code zajmie się resztą.
...
Gdy wysyłasz żądanie z `model: "auto"`, Dardcor Code:
...
4. **Automatycznie odzyskuje się** — jeśli ten zawiedzie, Dardcor Code automatycznie próbuje kolejnego
...
Dardcor Code ma **trzy warstwy ochrony**:

### 1. Auto-Fallback

Jeśli najlepszy provider zawiedzie, Dardcor Code automatycznie próbuje kolejnego. Nie musisz nic robić.
...
Jeśli wszyscy providerzy zawiodą, Dardcor Code jako ostateczność kieruje ruch do stabilnych darmowych providerów (np. Kiro lub Qoder).

---

## Obsługa wielu kont

Jeśli masz wiele kont u tego samego providera (np. dwa klucze OpenAI), Dardcor Code traktuje każde jako **osobnego kandydata**. To oznacza:
...
Dardcor Code od czasu do czasu **eksploruje** nowych providerów, żeby odkryć lepsze opcje:
...
Dzięki temu Dardcor Code uczy się, którzy providerzy najlepiej pasują do Twojego wzorca użycia.
...
Dardcor Code automatycznie go pomija i próbuje kolejnego. Jeśli provider wciąż zawodzi, jest tymczasowo wykluczany (5–30 minut). Nie musisz nic robić.

### „Czy mogę zobaczyć, który provider został użyty?”

Sprawdź nagłówki odpowiedzi — Dardcor Code dołącza użytego providera i model w każdej odpowiedzi.
