...
Dla większości użytkowników **nie jest wymagana żadna specjalna konfiguracja**. Dardcor Code dostarcza wbudowany
...
Dardcor Code obsługuje też AgentRouter (i podobne przekaźniki) przez typ providera **Claude Code
...
- Dardcor Code uruchomione z włączoną flagą funkcji `ENABLE_CC_COMPATIBLE_PROVIDER`
  (patrz poniżej).
...
ustawiając zmienną środowiskową przed uruchomieniem Dardcor Code:
...
```bash
docker run -d --name Dardcor Code \
...
**`Invalid JSON response from provider (reset after Ns)` w logach Dardcor Code** —
...
providera `agentrouter`, który już ma poprawny obraz sieciowy. Sprawdź w logach Dardcor Code,
