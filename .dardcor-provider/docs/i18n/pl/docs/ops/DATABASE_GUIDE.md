...
> **TL;DR**: Dardcor Code używa **SQLite z journalingiem WAL** jako głównego magazynu, z szyfrowaniem **AES-256-GCM** w spoczynku dla wrażliwych pól. Ten przewodnik obejmuje schemat, migracje, kopie zapasowe/odzyskiwanie oraz runbooki operacyjne.
...
Dardcor Code wybrał SQLite zamiast PostgreSQL/MySQL z kilku powodów:
...
Dla wdrożeń **jednoużytkownikowych, jednainstancyjnych** (główny przypadek użycia Dardcor Code) SQLite jest prostszy i szybszy.
...
| OS      | Ścieżka                                                      |
| ------- | ------------------------------------------------------------ |
| Linux   | `~/.Dardcor Code/storage.sqlite`                                |
| macOS   | `~/.Dardcor Code/storage.sqlite`                                |
| Windows | `%USERPROFILE%\.Dardcor Code\storage.sqlite`                    |
...
```bash
DATA_DIR=/custom/path Dardcor Code
```
...
Baza Dardcor Code ma **94 moduły domenowe** w `src/lib/db/`. Każdy moduł:
...
Dardcor Code ma **94 pliki modułów** w `src/lib/db/`. Poniżej próbka kluczowych modułów; pełna lista w listingu katalogu:
...
Dardcor Code używa **wersjonowanych, idempotentnych migracji** w `src/lib/db/migrations/`. Każda migracja to pojedynczy plik SQL o nazwie `NNN_description.sql`.
...
1. Tworzy tabelę `_Dardcor Code_migrations`, jeśli nie istnieje
...
> **Zmiany niekompatybilne wstecz** (np. usuwanie kolumn) są trudne. Dardcor Code NIE wspiera downgrade — po zastosowaniu migracji zmiana schematu jest trwała. Planuj odpowiednio.
...
Dardcor Code używa **`migrateLegacyEncryptedString()`**, aby przezroczyście obsługiwać dwa schematy szyfrowania:
...
Dardcor Code backup create --name pre-migration
...
```bash
# Via CLI
Dardcor Code restore pre-migration
...
Dardcor Code backup auto enable --cron "0 2 * * *" --retention 7
```
...
```bash
sqlite3 ~/.Dardcor Code/storage.sqlite ".backup /backups/Dardcor Code-hot.db"
```

Używa online backup API SQLite — bezpieczne podczas działania Dardcor Code.
...
Długo działające instancje Dardcor Code zyskują na okazjonalnym `VACUUM`:

```bash
sqlite3 ~/.Dardcor Code/storage.sqlite "VACUUM;"
```
...
```bash
sqlite3 ~/.Dardcor Code/storage.sqlite "PRAGMA integrity_check;"
# Should print: ok
```
...
```bash
sqlite3 ~/.Dardcor Code/storage.sqlite ".recover" > recovered.sql
sqlite3 recovered.db < recovered.sql
mv recovered.db ~/.Dardcor Code/storage.sqlite
```
...
```bash
Dardcor Code sync pull --merge   # or: Dardcor Code backup restore <backup-id>
```
...
sqlite3 ~/.Dardcor Code/storage.sqlite "PRAGMA wal_checkpoint(TRUNCATE);"
```
...
```bash
sqlite3 ~/.Dardcor Code/storage.sqlite "SELECT * FROM api_keys LIMIT 5;"
```

### Liczba wierszy we wszystkich tabelach

```bash
sqlite3 ~/.Dardcor Code/storage.sqlite <<EOF
...
```bash
# Stop Dardcor Code first
Dardcor Code stop

# Delete the DB file
rm ~/.Dardcor Code/storage.sqlite*
...
- Zabij drugi proces
- Jeśli problem się utrzymuje, zrestartuj Dardcor Code
...
1. **Zatrzymaj Dardcor Code** (zapobiegaj dalszym próbom)
2. **Sprawdź stan DB** przez `sqlite3`
3. **Napraw ręcznie** częściową migrację
4. **Uruchom ponownie** Dardcor Code (migracja zostanie ponowiona)
