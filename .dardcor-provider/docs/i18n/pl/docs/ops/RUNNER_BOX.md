...
```bash
sudo mkdir -p /opt/Dardcor Code-ops
sudo cp scripts/ops/runner-janitor.sh /opt/Dardcor Code-ops/
sudo chmod +x /opt/Dardcor Code-ops/runner-janitor.sh
( sudo crontab -l 2>/dev/null; echo '*/30 * * * * /opt/Dardcor Code-ops/runner-janitor.sh >> /var/log/runner-janitor.log 2>&1' ) | sudo crontab -
```
