---
title: "Dardcor Code — Przewodnik wdrożenia na VM z Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code — Przewodnik wdrożenia na VM z Cloudflare
...
Kompletny przewodnik instalacji i konfiguracji Dardcor Code na VM (VPS) z domeną zarządzaną przez Cloudflare.
...
Utrzymuj limity czasu strumienia reverse proxy w zgodzie ze zmiennymi timeout Dardcor Code. Jeśli podniesiesz
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, podnieś też `proxy_read_timeout` / `proxy_send_timeout`
powyżej tego samego progu.

Dardcor Code używa `NEXT_PUBLIC_BASE_URL` jako kanonicznego, przeglądarkowego originu dla callbacków OAuth
...
`OMNIROUTE_TRUST_PROXY` tylko wtedy, gdy Dardcor Code nie jest bezpośrednio osiągalny przez klientów, a Twój proxy
...
Zobacz też [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) — przewodnik po Cloudflare Tunnel w tym repozytorium. Samodzielny worker `Dardcor CodeCloud/` znajduje się w osobnym repozytorium towarzyszącym.
...
| 20128 | Dardcor Code   | Tylko localhost (przez nginx) |
