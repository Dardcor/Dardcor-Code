...
> **TL;DR**: Dardcor Code dostarcza wbudowane monitorowanie kondycji, autopilota providerów, śledzenie limitów (quota) oraz haki obserwowalności. Ten przewodnik obejmuje dashboard, alerty i rozwiązywanie problemów.
...
Dardcor Code ma **3 warstwy monitorowania**:
...
│  └─ Narzędzia MCP: Dardcor Code_get_health, Dardcor Code_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
Dardcor Code obsługuje **3 kanały alertów**:
...
Utwórz `~/.Dardcor Code/dashboard.json`:
...
- Śledzenie quota w Dardcor Code może być **nieaktualne** — prawda providera jest upstream
...
- Uruchom `sqlite3 ~/.Dardcor Code/storage.sqlite "PRAGMA integrity_check;"`
- Jeśli "ok" — fałszywy alarm, health check jest zbyt rygorystyczny
- Jeśli cokolwiek innego — **zatrzymaj Dardcor Code** i postępuj według [przewodnika disaster recovery](./DATABASE_GUIDE.md#disaster-recovery)
