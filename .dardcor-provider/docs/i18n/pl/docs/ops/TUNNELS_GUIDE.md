...
Dardcor Code może udostępnić swój lokalny serwer (`http://localhost:20128`) w publicznym
...
Wszystkie trzy backendy są zarządzane w procesie — Dardcor Code uruchamia/zatrzymuje
...
- **Auto-install.** Przy pierwszym użyciu Dardcor Code pobiera najnowszą binarkę
...
login` + `cloudflared tunnel route dns ...`) nie są orkiestrowane przez
  Dardcor Code. URL-e są efemeryczne i zmieniają się przy każdym restarcie.
...
| Variable                                             | Purpose                                                                                           |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Nadpisuje ścieżkę do binarki. Jeśli ustawiona i poprawna, Dardcor Code używa jej zamiast pobierania. |
...
trzeba skonfigurować w dashboardzie ngrok — sam Dardcor Code tylko przekazuje
...
1. Zainstaluj Tailscale (albo pozwól Dardcor Code to zrobić — patrz endpoint `install` poniżej).
2. Zaloguj się (`tailscale login` lub przez endpoint `login` Dardcor Code).
...
cache haseł MITM Dardcor Code (`getCachedPassword` / `setCachedPassword`) na
...
Gdy udostępniasz Dardcor Code przez tunel, dashboard i przepływy OAuth muszą
...
i zrestartuj Dardcor Code przed rozpoczęciem OAuth. Dla efemerycznych Cloudflare Quick
...
uruchomienie więcej niż jednego backendu jest dozwolone; Dardcor Code śledzi każdy
niezależnie.
...
Dardcor Code próbuje auto-instalacji przy pierwszym użyciu. Jeśli instalacja jest zablokowana
