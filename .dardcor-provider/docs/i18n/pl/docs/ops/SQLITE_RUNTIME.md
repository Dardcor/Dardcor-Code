...
Dardcor Code wybiera sterownik SQLite przy starcie w 5-stopniowym łańcuchu fallback:
...
2. **`better-sqlite3` zainstalowany w runtime** (w `~/.Dardcor Code/runtime/`)
...
- **Windows EBUSY**: `npm install -g Dardcor Code@latest` może się nie udać, jeśli
  `better_sqlite3.node` poprzedniej wersji jest zablokowany przez działający proces. Instalacja
  runtime w `~/.Dardcor Code/runtime/` omija globalną pamięć podręczną npm.
- **Brak narzędzi do budowania**: Niektóre środowiska (korporacyjny Windows bez VS Build
  ...
  gwarantują, że Dardcor Code i tak się uruchomi, nawet gdy to się nie uda.
...
Przed załadowaniem pliku `.node` zainstalowanego w runtime Dardcor Code odczytuje pierwsze 8
...
OMNIROUTE_SKIP_POSTINSTALL=1 npm install -g Dardcor Code

# Force-reinstall runtime better-sqlite3
rm -rf ~/.Dardcor Code/runtime
Dardcor Code  # will reinstall on next start

# Check what driver is active
Dardcor Code config db-info  # (if CLI command exists)
```
