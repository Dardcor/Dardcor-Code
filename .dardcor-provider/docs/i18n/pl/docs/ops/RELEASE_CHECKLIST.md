...
1. `npm stage list Dardcor Code` — znajdź stage id (wypisywany też w podsumowaniu workflow).
...
`Dardcor Code` w trybie stage-only, żeby wycieknięty długotrwały token nie mógł `npm publish`
bezpośrednio skądkolwiek — CI może tylko stage'ować; tylko 2FA właściciela wypuszcza.

**Playbook zepsutego artefaktu (bez zmian):** `npm deprecate Dardcor Code@<bad> "<reason> — use <fixed>"`
...
- [ ] Test na świeżej instalacji: usuń `~/.Dardcor Code/Dardcor Code.db` i uruchom `npm run dev`
...
> **Notatka operatorska:** zdalny katalog obrazu VPS pozostaje `/usr/lib/node_modules/Dardcor Code/app/`.
...
- [ ] `Dardcor Code --tray` bootuje na macOS (systray2 instalowany do `~/.Dardcor Code/runtime/`)
- [ ] `Dardcor Code --tray` bootuje na Linux (wymaga DISPLAY; graceful error jeśli nie ustawione)
- [ ] `Dardcor Code --tray` bootuje na Windows (PowerShell NotifyIcon, bez dodatkowych binarek)
- [ ] `Dardcor Code config tray enable` tworzy wpis autostart; disable go usuwa
- [ ] `npm install -g Dardcor Code@<this-version>` uruchamia postinstall bez fatalnego wyjścia
- [ ] Ścieżka update zachowuje optional deps: `Dardcor Code update --apply` i auto-updater
...
- [ ] `Dardcor Code status` działa bez `.env` (ścieżka tokenu CLI, tylko loopback)
...
- [ ] Wszystkie 10 plików `skills/Dardcor Code*/SKILL.md` są publicznie pobieralne przez raw GitHub URL
