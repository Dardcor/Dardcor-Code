---
title: "Przewodnik wdrażania Dardcor Code na Fly.io"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Przewodnik wdrażania Dardcor Code na Fly.io

Ten dokument opisuje rzeczywisty proces wdrażania Dardcor Code na Fly.io i obejmuje dwa scenariusze:
...
Przewodnik opiera się na zweryfikowanej, działającej konfiguracji bieżącego projektu. Nazwa aplikacji to `Dardcor Code`.
...
- Adres dostępu: `https://Dardcor Code.fly.dev/`
...
- `app = 'Dardcor Code'` określa, do której aplikacji Fly kierowane jest wdrożenie
...
- Jeśli wcześniej używałeś innej nazwy, np. `oroute`, nie myl jej z `Dardcor Code`
...
Jeśli zmieniłeś nazwę aplikacji, zastąp `Dardcor Code` wybraną nazwą.
...
Te parametry były używane w rzeczywistych wdrożeniach bieżącej aplikacji `Dardcor Code`:
...
1. **Ustaw `NEXT_PUBLIC_BASE_URL` na publiczną domenę HTTPS**
   ...
   Jeśli używasz własnej domeny, zastąp ją odpowiednią domeną (np. `https://Dardcor Code.yourdomain.com`).
...
- Przeznaczone dla bieżącego projektu `Dardcor Code`
...
- Czy przeglądasz aplikację `Dardcor Code`
...
Jeśli bieżące repozytorium jest forkiem i chcesz synchronizować aktualizacje z upstream `https://github.com/diegosouzapw/Dardcor Code`, postępuj według poniższego przepływu.
...
- Przeglądasz inną aplikację (np. `oroute` zamiast `Dardcor Code`)
