---
title: "🌐 Przewodnik po proxy Dardcor Code"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🌐 Przewodnik po proxy Dardcor Code

> **Omijaj blokady geograficzne, chroń tożsamość i kieruj ruch AI przez dowolne proxy — bez złożonej konfiguracji.**

Dardcor Code zawiera pełny system zarządzania proxy, który pozwala kierować ruch do upstreamowych dostawców AI przez proxy HTTP, HTTPS lub SOCKS5. Niezależnie od tego, czy jesteś w zablokowanym regionie, potrzebujesz rotacji IP, czy fingerprintingu stealth — ten przewodnik obejmuje wszystko.
...
Dardcor Code obsługuje konfigurację proxy w **czterech niezależnych zakresach**, rozwiązywanych w kolejności priorytetu:
...
Gdy Dardcor Code wysyła żądanie do upstreamowego dostawcy, wywołuje `resolveProxyForConnectionFromRegistry()`, które sprawdza kolejne poziomy w kolejności:
...
Proxy są uwzględnione w systemie **Backup/Restore**. Gdy eksportujesz konfigurację Dardcor Code:
...
Jeśli skonfigurowałeś proxy w starszej wersji (przed rejestrem), Dardcor Code migruje je automatycznie:
...
> 🆕 **Wkład [@oyi77](https://github.com/oyi77)** — PR [#1847](https://github.com/diegosouzapw/Dardcor Code/pull/1847) (Issue [#1788](https://github.com/diegosouzapw/Dardcor Code/issues/1788))

Dardcor Code integruje się z platformą społecznościową **[1proxy](https://1proxy-api.aitradepulse.com)**, by dać dostęp do **setek darmowych, zwalidowanych proxy** z całego świata. Idealne dla użytkowników bez własnej infrastruktury proxy.
...
1. **Sync** — Dardcor Code pobiera zwalidowane proxy z API 1proxy
...
Dardcor Code nie tylko kieruje ruch przez proxy — sprawia, że ruch wygląda na legalny:
...
Dla dostawców używających wzorca CLIProxyAPI Dardcor Code obsługuje trzy tryby upstream proxy:

| Tryb          | Opis                                              |
| ------------- | ------------------------------------------------- |
| `native`      | Dardcor Code sam obsługuje routing proxy (domyślnie) |
...
To normalne przy tanich proxy zrywających idle connections. Dardcor Code już to obsługuje przez:
...
Upewnij się, że proxy jest skonfigurowane **przed** startem flow OAuth. Dardcor Code kieruje wymianę tokenów OAuth przez skonfigurowane proxy. Najpierw ustaw globalne lub provider-level proxy, potem połącz.
...
Mechanizm **proxy fast-fail** Dardcor Code (`src/lib/proxyHealth.ts`) wykrywa martwe proxy w <2s szybkim sprawdzeniem połączenia TCP, potem **cache’uje wynik**, by uniknąć narzutu na każde żądanie.
...
```ts
import { getAllProxyHealthStatuses, invalidateProxyHealth } from "Dardcor Code/proxyHealth";
...
Dardcor Code śledzi użycie per-proxy, by operatorzy mogli diagnozować wzorce routingu, skoki latencji i powtarzające się awarie.

### Co jest śledzone

Dla każdego żądania przez skonfigurowane proxy Dardcor Code zapisuje:
...
Gdy do zakresu przypisano wiele proxy, Dardcor Code używa **strategii rotacji**, by wybrać, którego użyć na każde żądanie. Strategia jest konfigurowana na poziomie zakresu (global, per-provider, per-account, per-combo).
...
```ts
import { rotateOneproxyProxy } from "Dardcor Code/oneproxyRotator";
...
```ts
import { resetSequentialIndex } from "Dardcor Code/oneproxyRotator";
...
```ts
import { failOneproxyProxy } from "Dardcor Code/oneproxyRotator";
