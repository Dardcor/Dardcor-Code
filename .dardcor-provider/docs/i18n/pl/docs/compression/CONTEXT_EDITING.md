...
W przeciwieństwie do lokalnych silników kompresji Dardcor Code (Caveman, RTK, LLMLingua, stacked
...
kontekstu. Dardcor Code dołącza jedynie parametr w body (`context_management.edits[]`); faktyczne
czyszczenie wykonuje Claude względem własnego tokenizera.

Z natury jest to zdolność delegowana: inni providerzy odrzucają ten parametr, więc Dardcor Code
...
Dardcor Code wstrzykuje pojedynczą edycję do wychodzącego body Anthropic Messages:
...
Beta jest reklamowana nagłówkiem `anthropic-beta: context-management-2025-06-27`, który Dardcor Code
...
Dardcor Code stabilnie sortuje edycję `clear_thinking` na początek, bo Anthropic wymaga, by
...
Po odpowiedzi Claude Dardcor Code zapisuje, ile kontekstu provider faktycznie usunął. To **nie** jest
...
| ----------------------- | ----------------------------------------------------- | -------------------------------------------- |
| Gdzie działa            | W Dardcor Code, zanim żądanie opuści proxy               | U providera (Claude), po stronie serwera     |
...
Te dwa podejścia są komplementarne: lokalne silniki kompresują bajty wysyłane przez Dardcor Code;
