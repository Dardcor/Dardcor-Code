...
> **TL;DR**: Silnik kompresji Dardcor Code jest **podłączalny (pluggable)** — możesz rejestrować własne silniki, dostarczać language packi dla nowych języków i składać stacked pipelines. Ten przewodnik pokazuje jak.
...
````ts
import type { CompressionEngine } from "Dardcor Code/compression/engines/types";
import { registerCompressionEngine } from "Dardcor Code/compression/engines/registry";
...
```
~/.Dardcor Code/compression/engines/my-engine.ts    # User-level
...
} from "@Dardcor Code/open-sse/services/compression/engines/registry";
...
Kompresja w stylu Caveman używa **pakietów reguł zależnych od języka**, aby obsługiwać wypełniacze (fillers), hedging i rozwlekłe wzorce w każdym języku naturalnym. Dardcor Code dostarcza **6 language packów**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
```ts
import { loadRulePack } from "Dardcor Code/compression/ruleLoader";
...
```
~/.Dardcor Code/compression/rules/hi/filler.json  # User-level
...
Dardcor Code wybiera **JEDEN tryb na żądanie** na podstawie konfiguracji, progów auto-trigger i override'ów combo.
...
`@Dardcor Code/open-sse/services/compression/strategySelector`):

```ts
import { applyStackedCompression } from "@Dardcor Code/open-sse/services/compression/strategySelector";
...
Silniki kompresji Dardcor Code w README przypisują zasługi kilku projektom upstream
(„inspired by RTK, Caveman, LLMLingua-2, Troglodita”). Częste pytanie kontrybutorów:
**gdy upstream RTK doda nowy filtr toola albo Caveman doda rule pack, jak to trafia do Dardcor Code?**
...
reguła/filtr w formacie Dardcor Code** (zob.
...
własne warstwy Dardcor Code `smartcrusher` / `toon` / `tabular`. Nie ma żywego
...
1. **Nie vendoruj** — wyraź regułę/filtr upstream w formacie Dardcor Code.
