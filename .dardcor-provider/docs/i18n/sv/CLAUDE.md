...
**Dardcor Code** — enad AI-proxy/router. En slutpunkt, 160+ LLM-leverantörer, automatisk återkoppling.
...
Dardcor Code har tre relaterade men distinkta mekanismer för tillfälliga fel. Håll deras
...
- **Importeringar**: extern → intern (`@/`, `@Dardcor Code/open-sse`) → relativ
...
- **Sökvägsalias**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + dashboard på samma port)
- **Data katalog**: `DATA_DIR` miljövariabel, standard till `~/.Dardcor Code/`
...
16. Inkludera aldrig `Co-Authored-By`-trailers som krediterar en AI-assistent, LLM eller automatiseringskonto (t.ex. namn som innehåller "Claude", "GPT", "Copilot", "Bot"; e-postmeddelanden på `anthropic.com` / `openai.com` / bot-ägda `noreply.github.com`-adresser). Sådana trailers dirigerar commit-attribution till bot-kontot på GitHub, vilket döljer den verkliga författaren (`diegosouzapw`) i PR-historiken. Mänskliga medarbetare — inklusive upstream PR-författare och issue-rapporterare som portas till Dardcor Code — KAN och BÖR krediteras med standard `Co-authored-by: Name <email>`-trailers; upstream-port arbetsflöden (`/port-upstream-features`, `/port-upstream-issues`) beror på detta.
