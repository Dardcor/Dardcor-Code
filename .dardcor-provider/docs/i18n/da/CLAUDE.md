...
**Dardcor Code** — samlet AI proxy/router. Én endpoint, 160+ LLM udbydere, auto-fallback.
...
Dardcor Code har tre relaterede, men distinkte mekanismer til midlertidige fejl. Hold deres
...
- **Imports**: ekstern → intern (`@/`, `@Dardcor Code/open-sse`) → relativ
...
- **Sti aliaser**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Standardport**: 20128 (API + dashboard på samme port)
- **Data katalog**: `DATA_DIR` miljøvariabel, standard til `~/.Dardcor Code/`
