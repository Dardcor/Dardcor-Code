...
**Dardcor Code** — پروکسی/روتر AI یکپارچه. یک نقطه انتهایی، بیش از 160 ارائه‌دهنده LLM، بازگشت خودکار.
...
Dardcor Code سه مکانیزم موقت شکست مرتبط اما متمایز دارد. دامنه آن‌ها را هنگام اشکال‌زدایی رفتار مسیریابی جدا نگه دارید. برای یک نمای کلی، به
...
- **واردات**: خارجی → داخلی (`@/`, `@Dardcor Code/open-sse`) → نسبی
...
- **آلیاس‌های مسیر**: `@/*` → `src/`، `@Dardcor Code/open-sse` → `open-sse/`، `@Dardcor Code/open-sse/*` → `open-sse/*`
- **پورت پیش‌فرض**: 20128 (API + داشبورد در همان پورت)
- **دایرکتوری داده**: متغیر محیطی `DATA_DIR`، به طور پیش‌فرض به `~/.Dardcor Code/`
