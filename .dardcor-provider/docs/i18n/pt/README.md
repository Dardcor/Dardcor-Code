# 🚀 Dardcor Code — The Free AI Gateway (Português (Portugal))
...
[🌐 Website](https://Dardcor Code.online) • [🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Docs](#-documentation) • [💰 Pricing](#-pricing-at-a-glance) • [💬 WhatsApp](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t)
...
_Connect any AI-powered IDE or CLI tool through Dardcor Code — free API gateway for unlimited coding._
...
<sub>📡 All agents connect via <code>http://localhost:20128/v1</code> or <code>http://cloud.Dardcor Code.online/v1</code> — one config, unlimited models and quota</sub>
...
This generates a `system-info.txt` with your Node.js version, Dardcor Code version, OS details, installed CLI tools (qoder, gemini, claude, codex, antigravity, droid, etc.), Docker/PM2 status, and system packages — everything we need to reproduce your issue quickly. Attach the file directly to your GitHub issue.
...
## 🎯 What Dardcor Code Solves — 30 Real Pain Points & Use Cases

> **Every developer using AI tools faces these problems daily.** Dardcor Code was built to solve them all — from cost overruns to regional blocks, from broken OAuth flows to protocol operations and enterprise observability.
...
- **System Info Report** — `npm run system-info` generates `system-info.txt` with your full environment (Node version, Dardcor Code version, OS, CLI tools, Docker/PM2 status). Attach it when reporting issues for instant triage.
...
- **npm global install** — `npm install -g Dardcor Code && Dardcor Code` — done
...
- Built-in quick-start for `Dardcor Code --mcp` and client onboarding
...
Arch Linux users can install the [AUR package](https://aur.archlinux.org/packages/Dardcor Code-bin), which installs Dardcor Code and provides a systemd user service:
...
| Command                 | Description                                                 |
| ----------------------- | ----------------------------------------------------------- |
| `Dardcor Code`             | Start server (`PORT=20128`, API and dashboard on same port) |
| `Dardcor Code --port 3000` | Set canonical/API port to 3000                              |
| `Dardcor Code --mcp`       | Start MCP server (stdio transport)                          |
...
When you no longer need Dardcor Code, we provide two quick scripts for a clean removal:

| Command                  | Action                                                                              |
| ------------------------ | ----------------------------------------------------------------------------------- |
| `npm run uninstall`      | Removes the system app but **keeps your DB and configurations** in `~/.Dardcor Code`.  |
| `npm run uninstall:full` | Removes the app AND permanently **erases all configurations, keys, and databases**. |

> Note: To run these commands, navigate to the Dardcor Code project folder (if you cloned it) and run them. Alternatively, if globally installed, you can simply run `npm uninstall -g Dardcor Code`.
...
| `STREAM_IDLE_TIMEOUT_MS` | inherits `REQUEST_TIMEOUT_MS` | Maximum gap between streaming chunks before Dardcor Code aborts the SSE stream                                                                  |

Backward compatibility is preserved: existing `FETCH_TIMEOUT_MS`, `API_BRIDGE_PROXY_TIMEOUT_MS`, and other per-layer timeout vars still work and override the shared baseline.

For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), Dardcor Code also derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, Dardcor Code keeps the default
...
`cache_control`, Dardcor Code does not inject bridge-owned markers.
...
For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, Dardcor Code will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

If you run Dardcor Code behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy
timeouts are also higher than your Dardcor Code stream/fetch timeouts.
...
### 3) Point your coding tool to Dardcor Code
...
For Void Linux users, you can build a native package using `xbps-src`. Save this block as `srcpkgs/Dardcor Code/template`:
...
Dardcor Code is available as a public Docker image on [Docker Hub](https://hub.docker.com/r/diegosouzapw/Dardcor Code).
...
- Quick Tunnels are not auto-restored after an Dardcor Code or container restart. Re-enable them from the dashboard when needed.
- Managed install currently supports Linux, macOS, and Windows on `x64` / `arm64`.
- Managed Quick Tunnels default to HTTP/2 transport to avoid noisy QUIC UDP buffer warnings in constrained container environments. Set `CLOUDFLARED_PROTOCOL=quic` or `auto` if you want a different transport.
- Docker images bundle system CA roots and pass them to managed `cloudflared`, which avoids TLS trust failures when the tunnel bootstraps inside the container.
- SQLite runs in WAL mode. `docker stop` should be allowed to finish so Dardcor Code can checkpoint the latest changes back into `storage.sqlite`.
...
- Set `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` if you want Dardcor Code to use an existing binary instead of downloading one.

**Using Docker Compose with Caddy (HTTPS Auto-TLS):**

Dardcor Code can be securely exposed using Caddy's automatic SSL provisioning. Ensure your domain's DNS A record points to your server's IP.
...
> 🆕 **NEW!** Dardcor Code is now available as a **native desktop application** for Windows, macOS, and Linux.

Run Dardcor Code as a standalone desktop app — no terminal, no browser, no internet required for local models. The Electron-based app includes:

- 🖥️ **Native Window** — Dedicated app window with system tray integration
- 🔄 **Auto-Start** — Launch Dardcor Code on system login
...
When minimized, Dardcor Code lives in your system tray with quick actions:
...
**Zero cost. Never stops coding.** Configure this as one Dardcor Code combo and all fallbacks happen automatically — no manual switching ever.
...
> All models below are **100% free with zero credit card required**. Dardcor Code auto-routes between them when one quota runs out — combine them all for an unbreakable $0 combo.
...
Dardcor Code v3.6 is built as an operational platform, not just a relay proxy.
...
When quota, rate, or health fails, Dardcor Code automatically moves to the next candidate without manual switching.
...
Dardcor Code includes a built-in evaluation framework to test LLM response quality against a golden set. Access it via **Analytics → Evals** in the dashboard.

### Built-in Golden Set

The pre-loaded "Dardcor Code Golden Set" contains test cases for:
...
2. Run `Dardcor Code_get_health`.
3. Run `Dardcor Code_list_combos`.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. Dardcor Code tracks quota per model!
...
- Rotation behavior: Dardcor Code routes to the next eligible Codex account automatically.
...
**Pro Tip:** OpenAI-compatible API — works seamlessly with Dardcor Code's format translation!
...
> **Note:** OpenClaw only works with local Dardcor Code. Use `127.0.0.1` instead of `localhost` to avoid IPv6 resolution issues.
...
**Step 1:** Add Dardcor Code as a custom provider:
...
> **Tip:** Add any model available in your Dardcor Code `/v1/models` endpoint to the `models` section. Use the format `provider/model-id` from your Dardcor Code dashboard.
...
- Auto-refreshed by Dardcor Code
...
- Dardcor Code v1.0.6+ includes fallback validation via chat completions
...
> **⚠️ Important for users running Dardcor Code on a VPS, Docker, or any remote server**



The OAuth credentials bundled in Dardcor Code are registered **for `localhost` only**. When you access Dardcor Code on a remote server (e.g. `https://Dardcor Code.myserver.com`), Google rejects the authentication with:
...
- Name: anything you like (e.g. `Dardcor Code Remote`)
...
1. Dardcor Code opens the Google authorization URL
...
4. Paste that URL into the field shown in the Dardcor Code connection modal
5. Click **"Connect"**
...
As credenciais OAuth embutidas no Dardcor Code estão cadastradas **apenas para `localhost`**. Quando você acessa o Dardcor Code em um servidor remoto (ex: `https://Dardcor Code.meuservidor.com`), o Google rejeita a autenticação com:
...
- Nome: escolha qualquer nome (ex: `Dardcor Code Remote`)
...
1. O Dardcor Code abrirá a URL de autorização do Google
2. Após você autorizar, o Google tentará redirecionar para `localhost` (que falha no servidor remoto)
3. **Copie a URL completa** da barra de endereço do seu browser (mesmo que a página não carregue)
4. Cole essa URL no campo que aparece no modal de conexão do Dardcor Code
5. Clique em **"Connect"**
...
- **Website**: [Dardcor Code.online](https://Dardcor Code.online)
- **Package**: [npmjs.com/package/Dardcor Code](https://www.npmjs.com/package/Dardcor Code)
...
Dardcor Code has **218+ features planned** across multiple development phases. Here are the key areas:
