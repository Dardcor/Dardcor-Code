# Dardcor Code MCP Server Documentation (Português (Portugal))
...
| Tool                            | Description                              |
| :------------------------------ | :--------------------------------------- |
| `Dardcor Code_get_health`          | Gateway health, circuit breakers, uptime |
| `Dardcor Code_list_combos`         | All configured combos with models        |
| `Dardcor Code_get_combo_metrics`   | Performance metrics for a specific combo |
| `Dardcor Code_switch_combo`        | Switch active combo by ID/name           |
| `Dardcor Code_check_quota`         | Quota status per provider or all         |
| `Dardcor Code_route_request`       | Send a chat completion through Dardcor Code |
| `Dardcor Code_cost_report`         | Cost analytics for a time period         |
| `Dardcor Code_list_models_catalog` | Full model catalog with capabilities     |

## Advanced Tools (8)

| Tool                               | Description                                                 |
| :--------------------------------- | :---------------------------------------------------------- |
| `Dardcor Code_simulate_route`         | Dry-run routing simulation with fallback tree               |
| `Dardcor Code_set_budget_guard`       | Session budget with degrade/block/alert actions             |
| `Dardcor Code_set_resilience_profile` | Apply conservative/balanced/aggressive preset               |
| `Dardcor Code_test_combo`             | Live-test all models in a combo via a real upstream request |
| `Dardcor Code_get_provider_metrics`   | Detailed metrics for one provider                           |
| `Dardcor Code_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `Dardcor Code_explain_route`          | Explain a past routing decision                             |
| `Dardcor Code_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
