...
> **VPS 部署注意：** 遠端映像檔目錄 `/usr/lib/node_modules/Dardcor Code/app/` 維持不變。部署技能會將 `dist/` 的內容 rsync 到其中。只有儲存庫內的建置輸出路徑改變了（`app/` → `dist/`）。
...
open-sse/                   # @Dardcor Code/open-sse 工作區
...
├── comparison/              # Dardcor Code 與替代方案比較
├── compression/             # 壓縮指南與規則
├── dev/                     # 開發指南
├── diagrams/                # 架構圖
...
- **問題回報**：[github.com/diegosouzapw/Dardcor Code/issues](https://github.com/diegosouzapw/Dardcor Code/issues)
