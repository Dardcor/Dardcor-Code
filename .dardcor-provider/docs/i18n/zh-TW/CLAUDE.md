...
**Dardcor Code** — 統一的 AI 代理/路由器。一個端點，160+ LLM 提供者，自動回退。
...
Dardcor Code 有三種相關但不同的臨時故障機制。在調試路由行為時，請保持它們的範圍分開。請參見
...
- **導入**：外部 → 內部（`@/`，`@Dardcor Code/open-sse`）→ 相對
...
- **路徑別名**：`@/*` → `src/`，`@Dardcor Code/open-sse` → `open-sse/`，`@Dardcor Code/open-sse/*` → `open-sse/*`
- **預設埠**：20128（API + 儀表板在同一埠）
- **數據目錄**：`DATA_DIR` 環境變量，預設為 `~/.Dardcor Code/`
...
16. 切勿在提交消息中包含將 AI 助手、LLM 或自動化帳戶作為作者的 `Co-Authored-By` 尾部（例如包含 "Claude"、"GPT"、"Copilot"、"Bot" 的名稱；`anthropic.com` / `openai.com` / 機器人擁有的 `noreply.github.com` 地址上的電子郵件）。這類尾部會將 commit 歸屬路由到 GitHub 上的機器人帳戶，從而在 PR 歷史中隱藏真正的作者 (`diegosouzapw`)。人類協作者——包括 upstream PR 作者和被移植到 Dardcor Code 的 issue 報告者——可以並且應該使用標準的 `Co-authored-by: Name <email>` 尾部進行署名；upstream-port 工作流（`/port-upstream-features`、`/port-upstream-issues`）依賴於此。
