...
<img src="../../screenshots/MainDardcor Code.png" alt="Dardcor Code 儀表板" width="820"/>
...
**~1.53B 有記錄的免費 Token/月** — 首月透過註冊獎勵最高可達 **~2.15B** — 聚合所有免費層配額，加上永久免費、無上限的提供者，再輔以智慧壓縮進一步延長每一分 Token 開銷。([統計方法 →](../../reference/FREE_TIERS.md#tldr--how-much-free-inference-does-Dardcor Code-actually-aggregate))
...
**問題、提供者技巧、路線圖與支援 → [Discord](https://discord.gg/U47eFqAXCn) · [Telegram](https://t.me/Dardcor CodeOficial) · WhatsApp [🌍 全球](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 巴西](https://chat.whatsapp.com/LTSpdFhXTxjH4R6CCNiKWz)**
...
[**🚀 快速開始**](#-quick-start) • [**🎯 Combo**](#-combos--the-flagship) • [**🌐 提供者**](#-290-ai-providers--90-free) • [**🔌 CLI 與 MCP**](#-full-cli--a2a--mcp) • [**🗜️ 壓縮**](#%EF%B8%8F-save-1595-tokens--automatically) • [**🌍 網站**](https://Dardcor Code.online)

[💥 承諾](#-the-promise) • [🤔 為什麼](#-why-Dardcor Code) • [🏆 優勢](#-what-sets-Dardcor Code-apart) • [🤖 相容 CLI](#-compatible-clis--coding-agents) • [🖥️ 執行平台](#%EF%B8%8F-where-Dardcor Code-runs--anywhere) • [🔒 隱私](#-private--local-first) • [🎬 實際展示](#-Dardcor Code-in-action) • [📚 探索更多](#-explore-more) • [📧 支援](#-support--community)
...
> 手動堆疊免費層很痛苦 — 數十個 SDK、數十個速率限制，而且你不清楚自己到底有多少配額。Dardcor Code 將 **43 個提供者池 / 516 個模型**的**有記錄**免費層聚合為一個真實數字，並在儀表板上即時展示 (`/dashboard/free-tiers`)。
...
> 單一端點。**290 家提供者。** 讓開發流程暢行無阻 — 由 Dardcor Code 自動挑選最划算且可行的最佳方案。
...
> **Combo** 是 Dardcor Code 的**自動路由模型鏈結機制**。無論是配額用盡、提供者斷線或成本飆升，Combo 都會自動無縫切換至下一個候選模型。**讓您的 AI 開發流程永遠不中斷！** 🛡️

### ⚡ 零設定 — 只需將模型設為 `auto`

無需手動建立 Combo。只要將模型名稱設為 `auto`（或其衍生變體），Dardcor Code 就會根據您已連線的提供者進行即時動態評分，為您建立虛擬 Combo：
...
- **🛰️ 遠端模式** — 透過範圍存取令牌從任何機器驅動遠端 Dardcor Code（`Dardcor Code connect` / `Dardcor Code contexts` / `Dardcor Code tokens`）。→ [Remote Mode](../../guides/REMOTE-MODE.md)
...
- **⚡ 本地效能與基礎設施** — 一鍵本地 Redis 啟動器（`Dardcor Code redis up`，加上儀表板 Redis 面板）、一鍵 **Cloudflare Workers** 和 **Deno Deploy** 中繼部署器接入代理池，以及可選的 Bifrost Go sidecar，用於卸載最熱門的中繼路徑（`BIFROST_BASE_URL`，逾時時自動備援到 TypeScript 路徑）。→ [Environment](../../reference/ENVIRONMENT.md)
...
<sub>📖 所有 16+ 工具的個別設定 → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · 🧩 OpenCode 插件 → [`@Dardcor Code/opencode-provider`](https://www.npmjs.com/package/@Dardcor Code/opencode-provider)</sub>
...
| 平台                     | 安裝方式                                     | 亮點                                            |
| ------------------------ | -------------------------------------------- | ----------------------------------------------- |
| 📦 **npm（全域）**       | `npm install -g Dardcor Code`                   | 一條命令，任何作業系統                          |
| 🐳 **Docker**            | `docker run … diegosouzapw/Dardcor Code`        | 多架構 **AMD64 + ARM64**                        |
| 🖥️ **桌面（Electron）**  | `npm run electron:build`                     | 原生視窗 + 系統匣 — **Windows / macOS / Linux** |
| 💪 **ARM**               | 原生 `arm64`                                 | Raspberry Pi、ARM 伺服器、Apple Silicon         |
| 📱 **Android（Termux）** | `pkg install nodejs-lts && npx -y Dardcor Code` | **在手機上**執行，24/7，無需 root               |
| 📲 **PWA**               | "新增到主畫面"                               | 全螢幕、離線、可從瀏覽器安裝                    |
| 🧩 **OpenCode 插件**     | `@Dardcor Code/opencode-provider`               | 原生 OpenCode 整合                              |
...
> 您的金鑰、您的機器、您的資料。Dardcor Code 是**本地代理** — 絕不向外回報。

- 🏠 **100% 在您的硬體上執行** — npm、Docker、桌面或手機。Dardcor Code 雲端絕不介入請求路徑。
...
> Dardcor Code 不僅僅是伺服器 — 它擁有 **60+ 命令**的**完整命令列控制台**，以及開放的代理協定，讓 AI 代理可以**自行**驅動 Dardcor Code。

### ⌨️ 真正的 CLI（不僅僅是 `start`）

```bash
Dardcor Code               # 啟動閘道 + 儀表板（埠口 20128）
Dardcor Code chat          # 互動式 TUI 聊天客戶端（斜線指令：/model /combo /skill /memory）
Dardcor Code setup         # 引導式首次執行精靈
Dardcor Code doctor        # 診斷提供者、埠口、原生依賴
```

### 🛰️ 遠端模式 — 在此執行 CLI，Dardcor Code 在 VPS 上

Dardcor Code 在伺服器上？從筆記型電腦使用**相同 CLI** 驅動。登入一次
使用範圍存取令牌；之後每個命令都指向遠端。

```bash
Dardcor Code connect 192.168.0.15            # 密碼 → 範圍令牌，儲存為上下文
Dardcor Code models list                     # ← 針對遠端伺服器執行
Dardcor Code configure codex                 # ← 選擇遠端模型，寫入本地 Codex 設定檔
Dardcor Code tokens create --name ci --scope read   # 為其他機器建立更窄的令牌
Dardcor Code contexts use default            # ← 切換回本地伺服器
```
...
### 🤝 連接代理 — 代理自行控制 Dardcor Code

透過 **MCP** 或 **A2A** 公開 Dardcor Code，任何有能力的代理即可取得整個閘道的金鑰 — 路由、提供者、Combo、快取、壓縮、記憶 — 自主運作。

| 協定                | 端點                                            | 用途                                           |
| ------------------- | ----------------------------------------------- | ---------------------------------------------- |
| 🧰 **MCP（stdio）** | `Dardcor Code --mcp`                               | 接入 Claude Desktop、Cursor 等 MCP 客戶端      |
...
```bash
# 將完整 Dardcor Code 工具集透過 MCP 提供給 Claude Code：
...
```
Client (10,000 tok) ──▶ Dardcor Code Compression (9 engines) ──▶ Provider (~1,080 tok, up to 95% saved)
```
...
- **🎛️ 壓縮決策位置**（_優先級，高 → 低_）— 每請求 `x-Dardcor Code-compression` 標頭 › 路由 Combo 覆寫 › 活動命名設定檔 › 自適應/自動觸發 › 面板預設 › 關閉。應用的計畫在 `X-Dardcor Code-Compression: <mode>; source=<source>` 回應標頭中回顯。
...
您應該會看到已連接的模型列表。🎉 就是這樣 — 開始編碼，Dardcor Code 會自動路由和備援。

如果您的客戶端無法傳送自訂標頭，Dardcor Code 也提供令牌化的相容別名：
...
> 🎬 **製作了關於 Dardcor Code 的影片？** 透過連結開啟 [issue](https://github.com/diegosouzapw/Dardcor Code/issues/new) 或 [discussion](https://github.com/diegosouzapw/Dardcor Code/discussions) — 我們會在此展示。
...
> 💡 儀表板上的"成本"是**節省追蹤器**，不是帳單 — Dardcor Code 從不向您收費。使用免費模型顯示的"$290 總成本"意味著**節省了 $290**。
...
🇷🇺 🇨🇳 🇮🇷 🇨🇺 🇹🇷 在被封鎖的地區？Dardcor Code 的 **3 層代理**（全域 / 每提供者 / 每連線）代理 API 請求、OAuth 流程、連線測試、Token 重新整理和模型同步。
...
| `DATA_DIR`        | `~/.Dardcor Code` | 資料庫和設定儲存位置      |

**Dardcor Code 會向我收費嗎？** 不會 — 它是免費的開源軟體，在您的機器上執行。您只直接向付費提供者付費。Dardcor Code 沒有帳單系統。
...
- 🌍 **網站**：[Dardcor Code.online](https://Dardcor Code.online)
- 🐙 **GitHub**：[github.com/diegosouzapw/Dardcor Code](https://github.com/diegosouzapw/Dardcor Code)
- 🐛 **Issue**：[回報錯誤](https://github.com/diegosouzapw/Dardcor Code/issues)（請附上 `npm run system-info` 輸出）
...
- **網站**：[Dardcor Code.online](https://Dardcor Code.online)
- **套件**：[npmjs.com/package/Dardcor Code](https://www.npmjs.com/package/Dardcor Code)
...
| [Remote Mode](../../guides/REMOTE-MODE.md)                      | 從筆記型電腦 CLI 透過範圍存取令牌驅動遠端 Dardcor Code（VPS）                |
| [Claude Code Config](../../guides/CLAUDE-CODE-CONFIGURATION.md) | 將 Claude Code 指向 Dardcor Code（本地/遠端），附 `launch` + 每模型設定檔    |
...
| [Termux Guide](../../guides/TERMUX_GUIDE.md)              | 在 Android 上透過 Termux 執行 Dardcor Code                  |
...
> Dardcor Code 由充滿熱情的開源社群塑造。這些個人做出了卓越貢獻，直接影響了專案的品質、穩定性和影響力。**謝謝你們。**
...
> 🙏 這些貢獻者的功能、錯誤修復和基礎設施改進是 Dardcor Code 可靠且功能豐富的**核心部分**。每一個 Pull Request、每一個測試案例、每一個 i18n 翻譯檔案都很重要。開源是由像他們這樣的人建構的。
...
Dardcor Code 站在巨人的肩膀上。它始於 **[9router](https://github.com/decolua/9router)** 的分支和 Go 專案 **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** 的 TypeScript 移植 — 從那裡開始，下面的每個子系統都受到先行開源專案的啟發。每一個都塑造了 Dardcor Code 的具體部分。這是我們對它們所有人的感謝。🙏

> ⭐ 星星數截至 2026 年 6 月 — 去給這些專案一顆星。

### 🧬 淵源與閘道

| 專案                                                                            | ⭐    | 啟發 Dardcor Code 的方式                                                                       |
| ------------------------------------------------------------------------------- | ----- | ------------------------------------------------------------------------------------------- |
...
Dardcor Code 是免費且開源的，在公開環境中建構和維護。如果它為您節省了時間或金錢，請考慮支援其開發：

- ⭐ **為倉庫加星** — 這確實有助於提高能見度
- 💖 **[GitHub Sponsors](https://github.com/sponsors/diegosouzapw)** — 資助持續維護和新提供者
- 🐛 **在 [Discussions](https://github.com/diegosouzapw/Dardcor Code/discussions) 中回報錯誤和分享意見回饋**
...
**[⬆ 回到頂部](#-Dardcor Code)** · 為開源 AI 社群用心建構。
