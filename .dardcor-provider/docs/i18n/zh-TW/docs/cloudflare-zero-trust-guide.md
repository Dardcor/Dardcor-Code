...
本指南記錄了保護 **Dardcor Code** 並將應用程式安全地暴露到網際網路的網路基礎設施黃金標準，**無需開放任何連接埠（Zero Inbound）**。

## 您的虛擬機器上做了什麼？

我們透過 PM2 以 **Split-Port** 模式啟動了 Dardcor Code：
...
3. 點選 **Add a Tunnel**，選擇 **Cloudflared**，命名為 `Dardcor Code-VM`。
...
- **Subdomain：** `Dardcor Code` 或 `panel`
...
2. 選擇 **Self-hosted**。
3. 在 **Application name** 中，填入 `Dardcor Code Panel`。
4. 在 **Application domain** 中，填入 `Dardcor Code.yourdomain.com`（與"路由 2"中設定的一致）。
5. 點選 **Next**。
...
> **效果：** 如果您嘗試開啟 `Dardcor Code.yourdomain.com`，將不再直接進入您的 Dardcor Code 應用程式！而是跳轉到一個精美的 Cloudflare 頁面，要求輸入電子郵件地址。只有您（或您填寫的電子郵件）輸入後，Outlook/Gmail 會收到一個 6 位數臨時驗證碼，驗證通過後才會解除隧道限制，允許存取 `20129` 連接埠。
...
3. 點選 **Create rule**。
4. **Name：** `Anti-Abuse Dardcor Code API`
...
2. Dardcor Code 僅透過 `cloudflared` 進行 HTTPS 對外通訊，不直接接收來自外部的 TCP 連線。
