---
title: "Dardcor Code Fly.io 部署指南（繁體中文）"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code Fly.io 部署指南（繁體中文）
...
本文件說明 Dardcor Code 在 Fly.io 上的實際部署流程，涵蓋兩種情境：

- 首次將目前專案部署到 Fly.io
- 發布後續程式碼更新
- 新專案遵循相同的部署工作流程

本指南基於目前專案經過驗證的有效配置。應用程式名稱為 `Dardcor Code`。
...
- 存取 URL：`https://Dardcor Code.fly.dev/`
...
- `app = 'Dardcor Code'` 決定部署目標是哪個 Fly 應用程式
...
- 如果您之前使用過不同的名稱（例如 `oroute`），請不要與 `Dardcor Code` 混淆
...
如果您變更了應用程式名稱，請將 `Dardcor Code` 替換為您選擇的名稱。
...
以下參數已在目前 `Dardcor Code` 應用程式的實際部署中使用：
...
1. **將 `NEXT_PUBLIC_BASE_URL` 設定為您的公開 HTTPS 網域**
   ...
   如果您使用自訂網域，請替換為對應的網域（例如 `https://Dardcor Code.yourdomain.com`）。
...
如果目前儲存庫是 fork，且您想同步上游 `https://github.com/diegosouzapw/Dardcor Code` 的更新，請遵循以下工作流程。
...
- 您正在檢視不同的應用程式（例如 `oroute` 而非 `Dardcor Code`）
