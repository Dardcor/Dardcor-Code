...
1. `npm stage list Dardcor Code` — 找到 stage id（也會列印在工作流程摘要中）。
...
**一次性強化措施（擁有者，npmjs.com）：**為 `Dardcor Code` 設定僅限階段式發布的
Trusted Publisher，這樣即使長期權杖外洩，也無法從任何地方直接 `npm publish`——
CI 只能暫存；只有擁有者的 2FA 才能真正發布。

**成品損壞應對手冊（未變更）：** `npm deprecate Dardcor Code@<bad> "<reason> — use <fixed>"`
...
- [ ] 若 `src/lib/db/migrations/` 中有新檔案：
  ...
  - [ ] 遷移包含在交易中
  - [ ] 編號正確（序列中無間隙）
- [ ] 在全新安裝上測試：刪除 `~/.Dardcor Code/Dardcor Code.db` 並執行 `npm run dev`
...
> **操作注意：** 遠端 VPS 映像目錄仍為 `/usr/lib/node_modules/Dardcor Code/app/`。
...
- [ ] `Dardcor Code --tray` 在 macOS 上啟動（systray2 已安裝到 `~/.Dardcor Code/runtime/`）
- [ ] `Dardcor Code --tray` 在 Linux 上啟動（需要 DISPLAY；若未設定則優雅報錯）
- [ ] `Dardcor Code --tray` 在 Windows 上啟動（PowerShell NotifyIcon，無需額外二進位檔）
- [ ] `Dardcor Code config tray enable` 建立開機自啟項目；disable 則移除
- [ ] `npm install -g Dardcor Code@<此版本>` 執行 postinstall 而不會致命退出
- [ ] 更新路徑保留選擇性依賴：`Dardcor Code update --apply` 和自動更新器
      ...
      導致 SLM 層靜默地失敗但仍保持運作。
- [ ] `Dardcor Code status` 在無 `.env` 的情況下正常運作（僅限 CLI 權杖路徑，迴環介面）
...
- [ ] 所有 10 個 `skills/Dardcor Code*/SKILL.md` 檔案均可透過原始 GitHub URL 公開獲取
