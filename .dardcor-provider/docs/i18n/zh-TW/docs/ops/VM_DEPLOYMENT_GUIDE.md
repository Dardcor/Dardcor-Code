---
title: "Dardcor Code — VM 部署指南（搭配 Cloudflare）"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code — VM 部署指南（搭配 Cloudflare）
...
在 VM（VPS）上安裝並設定 Dardcor Code 的完整指南，搭配經由 Cloudflare 管理的網域。
...
```bash
docker logs -f Dardcor Code          # 即時串流
docker logs Dardcor Code --tail 50   # 最後 50 行
```
...
另請參閱 [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) 以了解儲存庫內的 Cloudflare Tunnel 逐步說明。獨立的 `Dardcor CodeCloud/` worker 位於另一個配套儲存庫中。
...
| 20128  | Dardcor Code   | 僅限本機（經由 nginx） |
