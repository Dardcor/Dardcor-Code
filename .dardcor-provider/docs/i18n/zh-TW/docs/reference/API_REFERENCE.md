...
| 標頭                           | 方向     | 說明                                                                                          |
| ------------------------------ | -------- | --------------------------------------------------------------------------------------------- |
| `X-Dardcor Code-No-Cache`         | 請求     | 設為 `true` 以略過快取                                                                        |
| `x-Dardcor Code-no-memory`        | 請求     | 設為 `true` 以跳過此請求的記憶與技能注入（等同於 no-cache；避免每次呼叫的權杖/成本開銷）        |
| `X-Dardcor Code-Progress`         | 請求     | 設為 `true` 以取得進度事件                                                                     |
| `X-Session-Id`                 | 請求     | 用於外部工作階段親和性的黏性工作階段金鑰                                                         |
| `x_session_id`                 | 請求     | 也接受底線變體（直接 HTTP）                                                                     |
| `Idempotency-Key`              | 請求     | 去重金鑰（5 秒窗口）                                                                           |
| `X-Request-Id`                 | 請求     | 替代去重金鑰                                                                                   |
| `X-Dardcor Code-Cache`            | 回應     | `HIT` 或 `MISS`（非串流）                                                                      |
| `X-Dardcor Code-Idempotent`       | 回應     | `true` 表示已去重                                                                              |
| `X-Dardcor Code-Progress`         | 回應     | `enabled` 表示進度追蹤已啟用                                                                    |
| `X-Dardcor Code-Session-Id`       | 回應     | Dardcor Code 使用的有效工作階段 ID                                                                 |
| `X-Dardcor Code-Request-Id`       | 回應     | 請求關聯 ID（已知時）                                                                          |
| `X-Dardcor Code-Version`          | 回應     | Dardcor Code 建置版本（始終存在）                                                                  |
| `X-Dardcor Code-Cost-Saved`       | 回應     | 快取命中避免的美金成本（僅限快取命中）                                                           |
| `X-Dardcor Code-Decision`         | 回應     | 路由追蹤：`strategy=<名稱>; provider=<別名>; latency_ms=<n>`（`<名稱>` 為組合策略，非組合請求則為 `single`）—— 完成回應時始終存在 |

> Nginx 注意：如果您依賴底線標頭（例如 `x_session_id`），請啟用 `underscores_in_headers on;`。

> **成本遙測標頭：** 非串流成功回應也會攜帶 `X-Dardcor Code-*` 成本遙測組 — `X-Dardcor Code-Response-Cost`（美金，固定 10 位小數；免費/未定價為 `0.0000000000`）、`X-Dardcor Code-Tokens-In` / `X-Dardcor Code-Tokens-Out`、`X-Dardcor Code-Model`、`X-Dardcor Code-Provider`、`X-Dardcor Code-Latency-Ms`、`X-Dardcor Code-Cache-Hit` 以及 `X-Dardcor Code-Fallback-Attempts`（僅大於 0 時），加上 `X-Dardcor Code-Request-Id` 和 `X-Dardcor Code-Version`。這些由聊天補全、`/v1/responses`、`/v1/messages` **以及媒體端點** — `/v1/embeddings`、`/v1/images/generations`、`/v1/audio/speech`、`/v1/audio/transcriptions`、`/v1/rerank`、`/v1/videos/generations`、`/v1/music/generations` 和 `/v1/moderations`（成本始終為 `0`）發出。媒體成本按模態計算（每張圖片、每秒、每字元、每個搜尋單位），有定價時適用，否則為 `0`（故障開放）。

> **快取命中的成本語義：** 當語義快取命中時（`X-Dardcor Code-Cache-Hit: true`），不會進行上游呼叫，因此 `X-Dardcor Code-Response-Cost` 為 `0.0000000000`（服務命中的**增量**成本）。原始/原本的成本單獨在 `X-Dardcor Code-Cost-Saved` 中回報。計費消費者應加總 `X-Dardcor Code-Response-Cost`（命中不計費）；快取分析可匯總 `X-Dardcor Code-Cost-Saved`。
...
對於支援思考的 Claude 模型，`/v1/models` 也會提供一個**無思考**變體，其 ID 前綴為 `claude-3-Dardcor Code-no-thinking/`：
...
對於無法附加 `Authorization: Bearer ***` 的客戶端，Dardcor Code 也接受 URL 中的 API 金鑰，可透過查詢字串相容性（`?token=...`、`?apiKey=...`、`?api_key=...`、`?key=...`）或下方記錄的專用 `/api/v1/vscode/{token}/...` 端點。
...
OpenAI **Codex CLI** 在 `supports_websockets = true` 時會於用戶端驗證模型名稱，並**拒絕帶提供者前綴的 ID**，例如 `codex/gpt-5.5`（`使用 ChatGPT 帳戶使用 Codex 時不支援 'codex/gpt-5.5' 模型`）。請傳送**裸** ID（例如 `gpt-5.5`）。Dardcor Code 的橋接器僅限 codex，因此它會將裸 ID 重新解析為 codex 模型（`resolveCodexWsModelInfo`），然後再隧道至上游 — 即使裸 `gpt-5.5` 在其他情況下會透過 HTTP 路由至另一個提供者。

#### 配置 OpenAI Codex CLI

將 Codex CLI 指向 Dardcor Code，方法是在 `~/.codex/config.toml` 中新增一個支援 WebSocket 的自訂提供者（使用單獨的 `CODEX_HOME` 以避免觸碰現有配置）：
...
env_key = "OMNIROUTE_API_KEY"             # 存放 Dardcor Code API 金鑰（Bearer）
```

```bash
export OMNIROUTE_API_KEY=sk-...           # Dardcor Code API 金鑰（若 REQUIRE_API_KEY=false 則任意金鑰）
codex exec "Responda apenas: PONG"
```

CLI 將 `base_url + /responses` 升級為 WebSocket，而 Dardcor Code 將其隧道至選取的 codex OAuth 連線。已針對本機伺服器進行端到端驗證：ChatGPT 回傳 `codex.rate_limits` + `response.created` 並串流完成內容。
...
- URL 中的權杖可能會出現於反向代理日誌、瀏覽器歷史記錄和 Dardcor Code 外的遙測資料中。請將其視為相容性選項，而非預設驗證模式。
...
Dardcor Code 事件（請求完成、配額耗盡、金鑰輪換等）的外送 Webhook 訂閱。
...
雲端代理程式任務（Claude Code、Codex Cloud、OpenHands 等），代表 Dardcor Code 使用者遠端執行。
...
Dardcor Code 公開三種獨立的暫時性故障機制；下面的管理端點讓運算子可以讀取和覆寫它們：
...
用於以自訂可執行處理常式擴充 Dardcor Code 的技能框架，以及市集整合。
...
Dardcor Code 內嵌了一個模型上下文協定伺服器，具有 3 種傳輸方式（stdio、SSE、streamable-http）和範圍限定的工具。下方的儀表板端點可讀取狀態/稽核資料並代理 HTTP 傳輸。
...
Dardcor Code 公開一個 A2A（代理程式對代理程式）JSON-RPC 2.0 端點，以及用於檢視/儀表板使用的 REST 包裝器。
...
管理與 Dardcor Code 整合的 CLI 工具（antigravity、chipotle、commandCode、devin-cli 等）。請參閱[提供者參考](./PROVIDER_REFERENCE.md)以取得完整清單。
...
檢查執行時期防護機制（PII 偵測、提示注入偵測、視覺橋接）。防護機制在每個請求上執行；每次呼叫的選擇退出方式是透過 `x-Dardcor Code-disabled-guardrails` 請求標頭 — 沒有持久的啟用/停用表面。
