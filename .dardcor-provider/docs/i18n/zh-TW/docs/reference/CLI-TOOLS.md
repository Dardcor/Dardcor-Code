...
Dardcor Code 整合了三類 CLI 工具，分別對應三個專屬儀表板頁面：

| 頁面               | 路由                    | 概念                                                             | 數量       |
| ------------------ | ----------------------- | ---------------------------------------------------------------- | ---------- |
| **CLI 程式碼工具** | `/dashboard/cli-code`   | 指向 Dardcor Code 的程式碼工具（客戶端 → CLI → Dardcor Code → 提供者） | 21         |
| **CLI 代理工具**   | `/dashboard/cli-agents` | 指向 Dardcor Code 的自動代理工具（相同流程，範圍更廣）              | 6          |
| **ACP 代理**       | `/dashboard/acp-agents` | Dardcor Code 透過 stdio/ACP 以反向流程衍生的 CLI                    | 參見註冊表 |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
    ...
    http://YOUR_SERVER:20128/v1
           │
           ▼  （Dardcor Code 路由至對應提供者）
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP 代理（反向衍生流程）：
    客戶端請求 → Dardcor Code → 透過 stdio/ACP 衍生 CLI → 回應
```
...
您無需手動編寫每個工具的設定檔。Dardcor Code 為每個受支援的 CLI 提供了 `setup-*` 指令，可讀取執行中 Dardcor Code（本機或遠端）的**即時**模型目錄，並在您的機器上寫入該工具的設定檔：
...
每個指令都接受 `--remote <url> --api-key <key>`（針對遠端 Dardcor Code 設定本機工具）、`--dry-run`（預覽不寫入）和 `--port`。不支援模型自動探索的工具（Cline、Kilo、Roo、Goose、Aider、Gemini）需要 `--model <id>`（以及用於非互動執行的 `--yes`）。啟動器 `Dardcor Code launch`（Claude Code）和 `Dardcor Code launch-codex`（Codex）會以正確的環境變數注入來衍生 CLI，完全不寫入設定檔。
...
此頁面（從 `/dashboard/agents` 重新命名而來）顯示 Dardcor Code 可以**衍生**為後端執行引擎（透過 stdio/ACP 協定）的 CLI。目錄獨立維護於 `src/lib/acp/registry.ts`，**不同於** `CLI_TOOLS`。
...
或使用 Dardcor Code 儀表板 → **CLI 工具 → Cline → 套用設定**。
...
或使用 Dardcor Code 儀表板 → **CLI 工具 → KiloCode → 套用設定**。
...
當 VS Code Insiders 設定為使用自訂端點模型，且您希望 Dardcor Code 在無需自訂標頭欄位的情況下運作時使用。
...
- 將 `«redacted:sk-…»` 替換為在 Dardcor Code 中建立的 API 金鑰。
...
# CLI 使用自己的認證機制 — Kiro CLI 本身不需要 Dardcor Code 作為後端。
# 請將 kiro-cli 與 Dardcor Code 搭配使用於其他工具。
kiro-cli status
```

至於 **Kiro IDE** 桌面應用程式，請使用 Dardcor Code 在 `/dashboard/cli-tools → Kiro` 提供的 MITM 端點。

---

## 10. 內部 Dardcor Code CLI

`Dardcor Code` 二進位檔提供用於伺服器生命週期管理、設定、診斷和提供者管理的指令。進入點：`bin/Dardcor Code.mjs`。

```bash
Dardcor Code                              # 啟動伺服器（預設通訊埠 20128）
Dardcor Code setup                        # 互動式設定精靈
Dardcor Code doctor                       # 檢查設定、資料庫、通訊埠、執行環境
Dardcor Code providers list               # 已設定的提供者連線
Dardcor Code providers test-all           # 測試每個作用中連線
Dardcor Code reset-password               # 重設管理員密碼
Dardcor Code logs                         # 串流要求日誌
Dardcor Code health                       # 詳細健康狀態（斷路器、快取、記憶體）
Dardcor Code --version                    # 顯示版本
Dardcor Code --help                       # 顯示所有指令
```
...
```bash
Dardcor Code setup                        # 互動式設定精靈
Dardcor Code setup --non-interactive      # CI/自動化模式（讀取環境變數 + 旗標）
Dardcor Code setup --password '<value>'   # 直接設定管理員密碼
...
| `DATA_DIR`          | 覆寫 Dardcor Code 資料目錄                                       |

所有其他非互動式輸入皆以旗標傳遞（非環境變數）：
`--password`、`--provider`、`--provider-name`、`--provider-base-url`、`--default-model`
（請參閱上方 `Dardcor Code setup` 選項）。

### 診斷

```bash
Dardcor Code doctor                       # 檢查設定、資料庫、通訊埠、執行環境、記憶體、運作狀態
Dardcor Code doctor --json                # 機器可讀的 JSON
Dardcor Code doctor --no-liveness         # 跳過 HTTP 健康狀態探測
Dardcor Code doctor --host 0.0.0.0        # 覆寫運作狀態主機
Dardcor Code doctor --liveness-url <url>  # 完整健康端點 URL 覆寫
```
...
```bash
Dardcor Code providers available                       # Dardcor Code 提供者目錄
Dardcor Code providers available --search openai       # 依 ID/名稱/別名/類別過濾目錄
Dardcor Code providers available --category api-key    # 依類別過濾（api-key、oauth、free 等）
Dardcor Code providers available --json                # 機器可讀的 JSON

Dardcor Code providers list                            # 已設定的提供者連線
Dardcor Code providers list --json

Dardcor Code providers test <id|name>                  # 測試一個已設定的連線
Dardcor Code providers test-all                        # 測試每個作用中連線
Dardcor Code providers validate                        # 僅限本機的結構驗證
```

> `providers available` 讀取 Dardcor Code 目錄；`providers list/test/test-all/validate`
> 直接讀取本機 SQLite 資料庫，無需伺服器執行中。

### 復原與重設

```bash
Dardcor Code reset-password                # 重設管理員密碼（亦可使用：Dardcor Code-reset-password）
Dardcor Code reset-encrypted-columns       # 顯示警告 + 加密憑證重設的試執行
Dardcor Code reset-encrypted-columns --force  # 實際將 SQLite 中的加密憑證設為 null
```

### 憑證匯出（⚠ 請謹慎處理）

```bash
Dardcor Code auth export                                 # 顯示警告 + 確認閘道 — 不會存取資料庫
Dardcor Code auth export --force                          # 將所有連線的**解密後**憑證匯出至 stdout 為 JSON
Dardcor Code auth export --force --id <id>                 # 僅匯出符合條件的連線
Dardcor Code auth export --force --format env               # 輸出為 OMNIROUTE_<PROVIDER>_<FIELD>=<value> 格式
Dardcor Code auth export --force --out creds.json           # 寫入檔案（以 0600 權限建立）
```
...
以下指令假設 Dardcor Code 伺服器正在執行中，除非另有說明：

```bash
Dardcor Code status                       # 完整的執行時期狀態
Dardcor Code logs                         # 串流要求日誌（--json、--search、--follow）
Dardcor Code config show                  # 顯示目前設定

Dardcor Code provider list                # 列出可用提供者（providers list 的別名）
Dardcor Code provider add                 # 將 Dardcor Code 註冊為工具上的提供者
Dardcor Code keys add | list | remove     # 管理 API 金鑰
Dardcor Code models [provider]            # 列出模型（--json、--search）
Dardcor Code combo list | switch | create | delete

Dardcor Code backup                       # 快照設定 + 資料庫
Dardcor Code restore                      # 從先前的快照還原

Dardcor Code health                       # 詳細健康狀態（斷路器、快取、記憶體）
Dardcor Code quota                        # 提供者配額使用情況
Dardcor Code cache                        # 快取狀態
Dardcor Code cache clear                  # 清除語意 + 簽章快取

Dardcor Code mcp status | restart         # MCP 伺服器狀態 / 重新啟動
Dardcor Code a2a status | card            # A2A 伺服器狀態 / 代理卡片

Dardcor Code tunnel list | create | stop  # 管理通道（cloudflare/tailscale/ngrok）
Dardcor Code env show | get <k> | set <k> <v>  # 檢查 / 設定環境變數（暫時性）

Dardcor Code test                         # 提供者連線冒煙測試
Dardcor Code update                       # 檢查更新
Dardcor Code completion                   # 產生 Shell 補全
```
...
可直接貼上的 Token 化 Dardcor Code URL 範例：
...
| 錯誤                               | 原因                 | 解決方式                                      |
| ---------------------------------- | -------------------- | --------------------------------------------- |
| `Connection refused`               | Dardcor Code 未執行     | `Dardcor Code serve`                             |
