...
Dardcor Code 的常見問題與解決方案。

---

## 快速參考

**剛接觸 Dardcor Code？** 從這裡開始 — 這些能解決 90% 的問題：

| 我看見這個                | 代表什麼                | 該怎麼做                                                                      |
| ------------------------- | ----------------------- | ----------------------------------------------------------------------------- |
| 「無法連線」              | Dardcor Code 未在執行      | 執行 `Dardcor Code` 或 `docker restart Dardcor Code`                                |
...
| EACCES：權限被拒                                     | 設定 `DATA_DIR=/path/to/writable/dir` 以覆蓋 `~/.Dardcor Code`                                                                                |
| 路由策略未儲存                                       | 更新至最新的 v3.x 版本（早期版本已修復 Zod schema 以確保設定持續性）                                                                       |
| 登入崩潰／空白頁面                                   | 檢查 Node.js 版本 — 請參閱下方的 [Node.js 相容性](#nodejs-相容性)                                                                          |
| `dlopen` / `slice is not valid mach-o file`（macOS） | 執行 `cd $(npm root -g)/Dardcor Code/app && npm rebuild better-sqlite3 && Dardcor Code` — 請參閱下方的 [macOS 原生模組重建](#macos-原生模組重建) |
...
Avast 和 AVG 執行啟發式掃描，會將包含大量類似 HTTP 請求連結的純文字/Markdown 檔案標記為可疑。Dardcor Code 的 `README.md` 隨 npm 套件一起發布（它列在 `package.json` → `files` 中），因此在全域安裝時會出現在 `node_modules/Dardcor Code/README.md` — 而其中包含約 15 個 `http://localhost:20128/...` 的範例（MCP HTTP/SSE 端點、A2A `.well-known` URL 以及 `curl` 程式碼片段）。這樣的連結密度足以觸發啟發式掃描。
...
1. **停止通知** — 在防毒軟體中排除安裝目錄（Avast：設定 → 例外），加入您的全域 `node_modules` 路徑和/或 Dardcor Code 資料目錄（`~/.Dardcor Code/`）。
...
**為什麼會觸發：** Windows 安裝程式**尚未進行程式碼簽署**，因此未簽署的 NSIS 安裝程式沒有信譽，行為啟發式掃描會以最大強度執行。加上捆綁的原生 DLL 和數百個寫入 `%LOCALAPPDATA%\Programs\Dardcor Code` 的 `.js` 檔案（包括 Next.js 獨立建置的雜湊後綴套件目錄），這就足以觸發啟發式掃描。程式碼簽署已規劃中；在完成之前，新版本可能會重複觸發此問題。

**該怎麼做：**

1. **先驗證您的下載**（排除檔案被竄改的可能性）。每個版本都會發布 `latest.yml`，其 `sha512` 欄位（base64）涵蓋 `Dardcor Code.Setup.<version>.exe` 安裝程式。在 PowerShell 中，從包含安裝程式的目錄執行：
   ```powershell
   ...
   ```
   輸出必須與 `latest.yml` → `sha512` 相符。如果不符，請刪除檔案並僅從 [GitHub 發布頁面](https://github.com/diegosouzapw/Dardcor Code/releases) 重新下載。
2. **還原 + 排除** — 從隔離區還原被回滾的項目，並為 `%LOCALAPPDATA%\Programs\Dardcor Code` 加入排除規則（Kaspersky → 設定 → 威脅與排除），然後重新安裝。
...
**原因：** 您執行的 Node.js 版本低於 Dardcor Code 核准的安全執行環境最低版本。最常見的情況是執行較舊的 Node 22 或 24 修補版本，低於 Dardcor Code 所需的修補安全門檻。
...
3. 重新安裝 Dardcor Code：`npm install -g Dardcor Code`
4. 重新啟動：`Dardcor Code`
...
**原因：** 在全域 `npm install -g Dardcor Code` 之後，套件內的 `better-sqlite3` 原生二進位檔案可能是為與本地執行環境不同的架構或 Node.js ABI 所編譯。這在 macOS（Apple Silicon 和 Intel 皆適用）上很常見，當預先編譯的二進位檔案與您的環境不符時就會發生。
...
**修復方式（v3.5.5+）：** Dardcor Code 現在在啟用 Proxy 分派器時使用 undici 自己的 `fetch()` 函式，確保行為一致。請更新至 v3.5.5+。

### WSL 下的 MITM Proxy：Windows 主機上的桌面應用程式未被攔截

**原因：** MITM Proxy 及其 CA 憑證會安裝在 Dardcor Code 執行的環境中。在 WSL 下，該環境是 Linux 客體，而 AI 桌面應用程式（Kiro、Trae、Copilot、Zed 等）則在 Windows 主機上執行。主機應用程式不信任客體的憑證儲存區，也不會透過客體的系統 Proxy 路由，因此桌面攔截無法在該處生效。

**建議：** 在與您要攔截的桌面應用程式相同的作業系統上原生執行 Dardcor Code（Windows 應用程式用 Windows；macOS/Linux 同理）。在 WSL 內執行 Dardcor Code 同時鎖定主機應用程式，需要手動在 Windows 主機上信任所產生的 CA 憑證，並將每個主機應用程式的網路/Proxy 設定指向 WSL Proxy 端點 — 這是不受支援且脆弱的設定。
...
Dardcor Code 會自動刷新 Token。如果問題持續存在：
...
2. 確認 `CLOUD_URL` 指向您的雲端端點（例如 `https://Dardcor Code.dev`）
...
當大量並發請求湧入一個已達速率限制的提供者時，Dardcor Code 會使用互斥鎖 + 自動速率限制來序列化請求，防止連鎖失敗。這對 API 金鑰提供者是自動生效的。

---

## 選用：RAG / LLM 失敗分類（16 種問題）

部分 Dardcor Code 使用者將閘道器部署在 RAG 或 Agent 堆疊之前。在這些設定中，常會看到一種奇怪的現象：Dardcor Code 看起來正常（提供者正常、路由設定檔無誤、無速率限制警示），但最終答案仍然錯誤。
...
1. 當您調查一個錯誤回應時，記錄：
   - 使用者的任務與請求
   - Dardcor Code 中的路由或提供者組合
   - 下游使用的任何 RAG 上下文（檢索的文件、工具呼叫等）
2. 將事件對應到一或兩個 WFGY ProblemMap 編號（`No.1` … `No.16`）。
3. 將編號儲存在您自己的儀表板、Runbook 或事件追蹤器中，放在 Dardcor Code 日誌旁邊。
...
如果您沒有在 Dardcor Code 後方執行 RAG 或 Agent 管線，可以忽略本節。
...
2. 重新啟動 Dardcor Code 以載入新的環境變數值
...
3. 重新啟動 Dardcor Code 並從**儀表板 → CLI 工具**重新測試
...
- 從 CLI 執行 `Dardcor Code providers` 以重新觸發 OAuth 流程，或
