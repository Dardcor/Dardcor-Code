...
本指南說明如何從系統中徹底移除 Dardcor Code。

---

## 快速解除安裝（v3.6.2+）

Dardcor Code 提供兩個內建指令碼來進行乾淨的移除：
...
此指令會移除 Dardcor Code 應用程式，但**保留**您的資料庫、設定檔、API 金鑰及提供者設定於 `~/.Dardcor Code/`。若您日後打算重新安裝並保留既有設定，請使用此方式。
...
- 開啟 `設定 → 應用程式 → Dardcor Code → 解除安裝`
- 或從安裝目錄執行 NSIS 解除安裝程式

**macOS：**

- 將 `/Applications` 中的 `Dardcor Code.app` 拖入垃圾桶
- 移除資料：`rm -rf ~/Library/Application Support/Dardcor Code`

**Linux：**

- 刪除 AppImage 檔案
- 移除資料：`rm -rf ~/.Dardcor Code`
...
Dardcor Code 預設將資料存放於以下位置：

| 平台         | 預設路徑                      | 覆蓋方式                   |
| ------------ | ----------------------------- | -------------------------- |
| Linux        | `~/.Dardcor Code/`               | `DATA_DIR` 環境變數        |
| macOS        | `~/.Dardcor Code/`               | `DATA_DIR` 環境變數        |
| Windows      | `%APPDATA%/Dardcor Code/`        | `DATA_DIR` 環境變數        |
| Docker       | `/app/data/`（掛載資料卷）    | `DATA_DIR` 環境變數        |
| XDG 相容模式 | `$XDG_CONFIG_HOME/Dardcor Code/` | `XDG_CONFIG_HOME` 環境變數 |
