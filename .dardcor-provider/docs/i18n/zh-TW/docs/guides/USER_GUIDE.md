...
設定提供者、建立組合、整合 CLI 工具及部署 Dardcor Code 的完整指南。
...
**小撇步：** 複雜任務使用 Opus，追求速度使用 Sonnet。Dardcor Code 會追蹤每個模型的額度！

Claude 及與 Claude Code 相容的路由會保留 Opus 和 Sonnet 模型的 `max` 思考強度設定。
Haiku 模型不接受 `max` 思考強度層級，因此 Dardcor Code 會在將請求發送給上游之前，
將該設定降級為較高的思考預算。
...
設定 → 模型 → 進階：
  ...
  OpenAI API 金鑰：［取自 Dardcor Code 控制台］
...
CLI 會自動從 `~/.Dardcor Code/.env` 或 `./.env` 載入 `.env`。

### 解除安裝

當您不再需要 Dardcor Code 時，我們提供兩個快速腳本進行乾淨移除：

| 指令                    | 動作                                                                                 |
| ------------------------ | ------------------------------------------------------------------------------------ |
| `npm run uninstall`      | 移除系統應用程式，但**保留您的資料庫和配置**於 `~/.Dardcor Code`。                      |
| `npm run uninstall:full` | 移除應用程式並永久**清除所有配置、金鑰和資料庫**。                                    |

> 注意：若要執行這些指令，請導航至 Dardcor Code 專案資料夾（如果您是透過複製倉庫的方式）並執行。或者，若是全域安裝，您可以直接執行 `npm uninstall -g Dardcor Code`。
...
npm run start
# 或：pm2 start npm --name Dardcor Code -- start
```
...
Void Linux 使用者可以使用 `xbps-src` 跨編譯框架，以原生方式封裝並安裝 Dardcor Code。這會自動化 Node.js 獨立建置以及所需的 `better-sqlite3` 原生繫結。
...
| 變數                                   | 預設值                                | 說明                                                                                       |
| --------------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------ |
| `JWT_SECRET`                            | `Dardcor Code-default-secret-change-me` | JWT 簽署密鑰（**生產環境務必修改**）                                                        |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | 首次登入密碼                                                                                |
| `DATA_DIR`                              | `~/.Dardcor Code`                       | 資料目錄（資料庫、用量、日誌）                                                                |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://Dardcor Code.dev`              | 雲端同步端點基礎網址（取代舊的 `CLOUD_URL`）                                                  |
...
**關於模型 ID 的說明：** Dardcor Code 使用提供者原生 ID（`claude-opus-4-8`, `gpt-5.5`, `glm-5.1`, `MiniMax-M2.7`, `kimi-k2.5`, `grok-4.20-0309-reasoning`）。部分 ID 包含帶點號的版本號，因為上游 API 期望如此。如果以上未列出某個模型，請執行 `Dardcor Code models --search <term>` 或存取 `GET /api/models/catalog` 以確認可用性。
...
### 串聯 Dardcor Code 對等節點

另一個 Dardcor Code 閘道可以作為**自訂 OpenAI 相容**提供者加入。使用
...
`X-Dardcor Code-Peer-Trace` 標頭。閘道在遇到重複的執行個體 ID 或用盡跳數
...
- 快速隧道在 Dardcor Code 或容器重啟後不會自動恢復；請在需要時從控制台重新啟用
...
- **語意快取** — 自動快取非串流、temperature=0 的回應（可透過 `X-Dardcor Code-No-Cache: true` 繞過）
- **請求冪等性** — 透過 `Idempotency-Key` 或 `X-Request-Id` 標頭在 5 秒內去重複請求
- **進度追蹤** — 選擇性 SSE `event: progress` 事件，透過 `X-Dardcor Code-Progress: true` 標頭啟用

---

### 翻譯器測試平台

透過**控制台 → 翻譯器**存取。偵錯並視覺化 Dardcor Code 如何在提供者之間轉換 API 請求。
...
Dardcor Code 也接受 `x_session_id`，並在 `X-Dardcor Code-Session-Id` 中回傳有效的會話金鑰。
...
4. **等待冷卻** — 如果所有候選連線都處於冷卻狀態，Dardcor Code 可以等待最早的冷卻結束，並自動重試相同的用戶端請求。
...
Dardcor Code 透過 OpenAI 相容端點支援音訊轉錄：
...
Dardcor Code 會在發送上游請求前將其限制在該上限內。
...
Dardcor Code 內建**評分驅動的自動路由器**，會為每次請求在已連線的所有提供者中選取最佳模型 — 無需維護組合。只需使用 `auto/*` 前綴發送請求，Dardcor Code 就會即時組裝一個虛擬組合，依據延遲、成本、成功率、上下文適應性、模型對任務的適合度、近期失敗、額度和斷路器狀態對候選者進行評分。
...
Dardcor Code 既是 **MCP 伺服器**（模型上下文協定），也是 **A2A 伺服器**（代理間 JSON-RPC 2.0）。任何相容 MCP 的 IDE 或代理主機都可以直接呼叫 Dardcor Code 工具 — 無需額外的包裝器。

### MCP 傳輸方式

- **SSE**：`http://localhost:20128/api/mcp/sse`
- **可串流 HTTP**：`http://localhost:20128/api/mcp/stream`
- **stdio**：`Dardcor Code --mcp`（適用於偏好 stdio 的 IDE 外掛程式）
...
Dardcor Code 提供可擴充的**技能框架**（`src/lib/skills/`），讓代理和 A2A 端點可以執行領域特定的常式（例如 `code-review`、`summarize`、`extract-facts`、`web-research`）。
...
Dardcor Code 使用混合檢索來持久保存**長期對話記憶**：
...
- 每個承載都包含 `X-Dardcor Code-Signature`（HMAC-SHA256）以供驗證
...
Dardcor Code 與雲端編碼代理（**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**）整合，讓您可以從處理本地路由的同一個控制台分派長時間執行的任務。

- 在**控制台 → 雲端代理**或透過 `POST /api/v1/agents/tasks` 建立任務
- 追蹤每個任務的狀態、日誌和產出物
- 自備每個提供者的 API 金鑰 — 憑證絕不會離開 Dardcor Code 實例
...
您可以使用具有 `manage` 範圍的 **Bearer 金鑰**，透過 HTTP 管理所有 Dardcor Code 資源（提供者、組合、金鑰、設定）。
...
Dardcor Code 內建一個內部 CLI（`Dardcor Code …`），用於設定、診斷和執行時期控制。這**與控制台中的「CLI 工具」頁面不同**，後者用於配置第三方 CLI（Claude Code, Cursor, Codex, Cline, …）使其能夠與 Dardcor Code 通訊。

```bash
Dardcor Code setup                    # 互動式精靈（密碼、提供者、組合）
Dardcor Code setup --non-interactive  # CI 友善模式
Dardcor Code doctor                   # 健康診斷（資料目錄、資料庫、提供者、連接埠）
Dardcor Code providers available      # 列出支援的提供者
Dardcor Code providers list           # 列出已配置的連線
Dardcor Code providers test <id>      # 即時測試提供者連線
Dardcor Code combos list              # 列出組合
Dardcor Code combos switch <name>     # 設定預設組合
Dardcor Code models                   # 列出可用模型（--json, --search）
Dardcor Code keys add | list | remove # 從終端機管理 API 金鑰
Dardcor Code backup                   # 快照配置 + 資料庫
Dardcor Code restore [<timestamp>]    # 從快照還原
Dardcor Code health                   # 詳細健康狀態（斷路器、快取、記憶體）
Dardcor Code quota                    # 提供者額度使用情況
Dardcor Code mcp status               # MCP 伺服器狀態
Dardcor Code a2a status               # A2A 伺服器狀態
Dardcor Code tunnel list|create|stop  # Cloudflare/Tailscale/ngrok 隧道
Dardcor Code reset-password           # 重設管理員密碼
Dardcor Code --mcp                    # 透過 stdio 啟動 MCP 伺服器
Dardcor Code --port 3000              # 在自訂連接埠上啟動伺服器
```

提示：將 `Dardcor Code doctor --json` 與您的監控工具搭配使用，可在提供者連線異常時發出警報。

---

## 🖥️ 桌面應用程式（Electron）

Dardcor Code 也可作為 Windows、macOS 和 Linux 的原生桌面應用程式使用。
