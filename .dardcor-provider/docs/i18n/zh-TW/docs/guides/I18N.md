...
Dardcor Code 支援 **43 種語言**，包含完整的儀表板 UI 翻譯、翻譯文件，以及阿拉伯語和希伯來語的 RTL（從右至左）支援。
...
Dardcor Code 使用基於雜湊的增量翻譯器來處理說明文件，後端採用與 OpenAI 相容的 LLM 端點（通常是透過 Dardcor Code Cloud 的 `cx/gpt-5.4-mini`）：
...
**次要翻譯器** — 使用任何與 OpenAI 相容的 LLM API（包括 Dardcor Code 本身）來翻譯現有的 `docs/i18n/` Markdown 檔案。最適合用於潤飾或重新翻譯文件，品質優於 Google 翻譯。
...
`Dardcor Code` CLI 有獨立於 Next.js 儀表板之外的 i18n 層。
...
| 優先順序 | 來源                     | 範例                                       |
| -------- | ------------------------ | ------------------------------------------ |
| 1        | `--lang` 旗標            | `Dardcor Code --lang de status`               |
| 2        | `OMNIROUTE_LANG` 環境變數| `OMNIROUTE_LANG=ja Dardcor Code providers`    |
...
```bash
# 設定語言並儲存至 ~/.Dardcor Code/.env（跨工作階段持續有效）
...
儲存的偏好設定會以原子方式寫入 `~/.Dardcor Code/.env`，並在 CLI 啟動時、任何指令執行前載入。
