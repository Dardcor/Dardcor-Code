...
Dardcor Code 是一個建構於 Next.js 上的本地 AI 路由閘道與儀表板。
...
flowchart LR
    ...
    subgraph Router[Dardcor Code 本地程序]
        API[V1 相容性 API\n/v1/*]
        DASH[儀表板 + 管理 API\n/api/*]
        CORE[SSE + 轉換核心\nopen-sse + src/sse]
        DB[(storage.sqlite)]
...
Dardcor Code 可以安裝、監督並路由至本地運行的 AI 工具程序，
...
- Dardcor Code 回應中繼資料建置器：`src/domain/Dardcor CodeesponseMeta.ts`
...
- 檔案：`${DATA_DIR}/storage.sqlite`（或設定了 `$XDG_CONFIG_HOME/Dardcor Code/storage.sqlite` 時使用該路徑，否則為 `~/.Dardcor Code/storage.sqlite`）
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Dardcor Code 執行時期]
        Next[Next.js 伺服器\nPORT=20128]
        Core[SSE 核心 + 執行器]
...
> **注意：** 以下矩陣為 Dardcor Code v3.8.0 中 237 個已註冊提供者的代表性樣本。
> 完整且持續更新的清單，請參閱
...
- Dardcor Code 回傳的最終客戶端回應；串流回應以相同壓縮摘要形式儲存
...
1. `usageDb` 與 `localDb` 共用相同的基礎目錄政策（`DATA_DIR` → `XDG_CONFIG_HOME/Dardcor Code` → `~/.Dardcor Code`），附舊版檔案遷移。
...
5. `open-sse/` 目錄以 `@Dardcor Code/open-sse` **npm workspace 套件**形式發布。原始碼透過 `@Dardcor Code/open-sse/...` 匯入（由 Next.js `transpilePackages` 解析）。本文件中的檔案路徑為求一致仍使用目錄名稱 `open-sse/`。
...
- 建置 Docker 映像：`docker build -t Dardcor Code .`
- 啟動服務並驗證：
- `GET /api/settings`
- `GET /api/v1/models`
