---
title: "Dardcor Code 程式碼基礎文件"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code 程式碼基礎文件

> **版本：** v3.8.0
> **最後更新：** 2026-06-28
> **讀者對象：** 為 Dardcor Code 貢獻程式碼或在其之上建立整合的工程師。
>
...
目錄由 `DATA_DIR` 環境變數指定，預設為 `~/.Dardcor Code/`）。
...
├── open-sse/             串流引擎 workspace（@Dardcor Code/open-sse）
├── electron/             桌面包裝程式（Electron 41 main + preload）
├── bin/                  CLI 進入點（Dardcor Code、reset-password）
...
| `responses.ts`、`Dardcor CodeesponseMeta.ts` | 回應形狀輔助程式             |
...
獨立的 npm workspace，發布為 `@Dardcor Code/open-sse`。負責請求
...
```
bin/
├── Dardcor Code.mjs           主要 CLI 進入點（Node ESM）
...
- **匯入順序**：外部 → 內部（`@/`、`@Dardcor Code/open-sse`）→ 相對。
