...
- 在活躍帳戶耗盡之前，Dardcor Code 會產生一個精簡的結構化摘要
- 在認證為同一個工作階段選取不同帳戶後，Dardcor Code 會將該摘要作為系統訊息注入到下一個請求中
- 交接成功消耗後，會從儲存中移除
...
如果活躍提供者在 `handoffProviders` 中啟用，Dardcor Code 會在帳戶完全耗盡之前在背景產生結構化的交接摘要。
...
當同一個工作階段的下一個請求解析到不同的已認證帳戶時，Dardcor Code 會將儲存的交接作為系統訊息預先加入。只有在實際帳戶切換已知後才會進行注入。
...
在注入時，Dardcor Code 會將該酬載轉換為 `<context_handoff>` 系統訊息，以便下一個帳戶能以正確的本地上下文繼續。
