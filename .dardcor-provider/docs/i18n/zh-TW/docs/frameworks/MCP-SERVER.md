---
title: "Dardcor Code MCP Server 文件"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code MCP Server 文件
...
Dardcor Code MCP 為內建功能。透過以下指令啟動：
...
Dardcor Code --dev  # MCP 會自動在 /mcp 端點啟動
```
...
| 工具                              | 範圍                   | 說明                                                              |
| :-------------------------------- | :--------------------- | :---------------------------------------------------------------- |
| `Dardcor Code_get_health`            | `read:health`          | 運作時間、記憶體、斷路器、速率限制、快取統計                      |
| `Dardcor Code_list_combos`           | `read:combos`          | 所有已設定的組合及策略（可選指標）                                |
| `Dardcor Code_get_combo_metrics`     | `read:combos`          | 特定組合的效能指標                                                |
| `Dardcor Code_switch_combo`          | `write:combos`         | 啟用或停用組合                                                    |
| `Dardcor Code_check_quota`           | `read:quota`           | 已用配額／總配額、剩餘百分比、重置時間、代幣健康狀態              |
| `Dardcor Code_route_request`         | `execute:completions`  | 透過 Dardcor Code 路由發送聊天完成請求                               |
| `Dardcor Code_cost_report`           | `read:usage`           | 按期間（工作階段／日／週／月）的成本報告                          |
| `Dardcor Code_list_models_catalog`   | `read:models`          | 完整模型目錄，包含功能、狀態、定價                                |

## 第一階段 — 搜尋

| 工具                     | 範圍             | 說明                                                                                                                       |
| :----------------------- | :--------------- | :------------------------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_web_search`   | `execute:search` | 透過 Dardcor Code 搜尋閘道（Serper/Brave/Perplexity/Exa/Tavily/Google PSE/Linkup/SearchAPI/SearXNG）進行網路搜尋，支援容錯轉移 |

## 進階工具（11 個）— 第二階段

| 工具                                 | 範圍                                  | 說明                                                                                  |
| :----------------------------------- | :------------------------------------ | :------------------------------------------------------------------------------------ |
| `Dardcor Code_simulate_route`           | `read:health`、`read:combos`          | 乾執行路由模擬，含備援樹                                                              |
| `Dardcor Code_set_budget_guard`         | `write:budget`                        | 工作階段預算，可設為降級／封鎖／警示動作                                               |
| `Dardcor Code_set_routing_strategy`     | `write:combos`                        | 於執行階段更新組合策略（優先／加權／自動等）                                          |
| `Dardcor Code_set_resilience_profile`   | `write:resilience`                    | 套用 `aggressive`／`balanced`／`conservative` 復原能力預設                            |
| `Dardcor Code_test_combo`               | `execute:completions`、`read:combos`  | 使用真實上游呼叫，對組合中的每個提供者進行即時測試                                    |
| `Dardcor Code_get_provider_metrics`     | `read:health`                         | 各提供者指標，含 p50/p95/p99 延遲與斷路器狀態                                         |
| `Dardcor Code_best_combo_for_task`      | `read:combos`、`read:health`          | 依任務類型推薦組合，考量預算與延遲限制                                                |
| `Dardcor Code_explain_route`            | `read:health`、`read:usage`           | 解釋為何請求被路由至某提供者（評分因素＋備援）                                        |
| `Dardcor Code_get_session_snapshot`     | `read:usage`                          | 完整工作階段快照：成本、代幣、熱門模型／提供者、錯誤、預算守衛                        |
| `Dardcor Code_db_health_check`          | `read:health`、`write:resilience`     | 診斷（並可選自動修復）資料庫漂移，如中斷的組合參考／孤立資料列                        |
| `Dardcor Code_sync_pricing`             | `pricing:write`                       | 從外部來源（LiteLLM）同步定價資料；支援 `dryRun`                                      |

## 快取工具（2 個）

| 工具                      | 範圍           | 說明                                              |
| :------------------------ | :------------- | :------------------------------------------------ |
| `Dardcor Code_cache_stats`   | `read:cache`   | 語意快取、提示快取與冪等性統計                    |
| `Dardcor Code_cache_flush`   | `write:cache`  | 全域或依簽章／模型清除快取                        |

## 壓縮工具（13 個）

| 工具                                  | 範圍                | 說明                                                                                                                       |
| :------------------------------------ | :------------------ | :------------------------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_compression_status`        | `read:compression`  | 壓縮設定、分析摘要與快取感知統計（包含 `analytics.mcpDescriptionCompression` 元資料）                                      |
| `Dardcor Code_compression_configure`     | `write:compression` | 設定壓縮模式、閾值、目標比率、系統提示保留、MCP 描述壓縮開關                                                              |
| `Dardcor Code_set_compression_engine`    | `write:compression` | 選取作用中引擎（off/caveman/rtk/stacked）與 Caveman/RTK 強度                                                              |
| `Dardcor Code_list_compression_combos`   | `read:compression`  | 列出已命名的壓縮組合及其引擎管線                                                                                           |
| `Dardcor Code_compression_combo_stats`   | `read:compression`  | 依壓縮組合與引擎分組的分析資料                                                                                             |
| `Dardcor Code_ccr_store`                 | `write:compression` | 將呼叫者隔離的內容儲存至有界限的記憶體內 CCR 存放區，並回傳標記與 `ccr://` 參考                                               |
| `Dardcor Code_ccr_retrieve`              | `read:compression`  | 以完整、開頭、結尾、行數、grep 及統計模式擷取 CCR 內容                                                                       |
| `Dardcor Code_ccr_inspect`               | `read:compression`  | 檢查呼叫者擁有的 CCR 元資料，不回傳內容                                                                                    |
| `Dardcor Code_ccr_list`                  | `read:compression`  | 列出呼叫者擁有的 CCR 區塊之分頁元資料                                                                                      |
| `Dardcor Code_ccr_delete`                | `write:compression` | 刪除呼叫者擁有的 CCR 區塊                                                                                                  |
| `Dardcor Code_ccr_stats`                 | `read:compression`  | 回報呼叫者範圍的記憶體使用量、生命週期計數器與存放區限制                                                                   |
| `Dardcor Code_rtk_discover`              | `read:compression`  | 在選擇性加入的 RTK 輸出樣本中發現重複出現的雜訊                                                                             |
| `Dardcor Code_rtk_learn`                 | `read:compression`  | 從選擇性加入的樣本產生可供審查的 RTK 過濾器草稿                                                                              |
...
`Dardcor Code_compression_status` 會將 MCP 描述壓縮分別回報於 `analytics.mcpDescriptionCompression` 之下。這些數值是對 MCP 可列出描述（`tools`、`prompts`、`resources` 與 `resourceTemplates`）的元資料大小估計值，並非提供者使用收據，並標記有 `source: "mcp_metadata_estimate"`。

### MCP 無障礙樹過濾器（v3.8.0）

與上述壓縮工具不同，Dardcor Code 包含一個執行後過濾器，可在 MCP 瀏覽器／無障礙工具的**工具結果**回傳給代理之前對其進行壓縮。此過濾器本身不是一個工具 — 它會透明地作用於任何包含冗長無障礙樹或瀏覽器快照文字（≥2000 字元）的工具結果。
...
| 工具                          | 範圍            | 說明                                                                                 |
| :---------------------------- | :-------------- | :----------------------------------------------------------------------------------- |
| `Dardcor Code_oneproxy_fetch`    | `read:proxies`  | 從 1proxy 市集取得免費代理（協定／國家／品質／數量過濾器）                            |
| `Dardcor Code_oneproxy_rotate`   | `read:proxies`  | 依策略（`random`／`quality`／`sequential`）取得下一個可用代理                         |
| `Dardcor Code_oneproxy_stats`    | `read:proxies`  | 池統計、同步狀態、依協定與國家的分佈                                                 |
...
| 工具                        | 範圍            | 說明                                                                           |
| :-------------------------- | :-------------- | :----------------------------------------------------------------------------- |
| `Dardcor Code_memory_search`   | `read:memory`   | 依查詢／類型／API 金鑰搜尋記憶，並執行代幣預算限制                             |
| `Dardcor Code_memory_add`      | `write:memory`  | 新增記憶條目（`factual`／`episodic`／`procedural`／`semantic`）                |
| `Dardcor Code_memory_clear`    | `write:memory`  | 清除某 API 金鑰的記憶，可選擇依類型或 `olderThan` 時間戳過濾                    |
...
| 工具                            | 範圍              | 說明                                                                           |
| :------------------------------ | :---------------- | :----------------------------------------------------------------------------- |
| `Dardcor Code_skills_list`         | `read:skills`     | 列出已註冊的技能，可依 API 金鑰、名稱或啟用狀態過濾                            |
| `Dardcor Code_skills_enable`       | `write:skills`    | 依 ID 啟用或停用特定技能                                                       |
| `Dardcor Code_skills_execute`      | `execute:skills`  | 以提供的輸入執行技能，並回傳執行記錄                                            |
| `Dardcor Code_skills_executions`   | `read:skills`     | 列出近期技能執行歷史                                                            |
...
| 工具                                | 範圍            | 說明                                                                                                             |
| :---------------------------------- | :-------------- | :--------------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_agent_skills_list`       | `read:catalog`  | 列出全部 42 個 agent 技能，可選 `category`（api\|cli）與 `area` 過濾器；回傳元資料＋覆蓋率                        |
| `Dardcor Code_agent_skills_get`        | `read:catalog`  | 依標準 `id` 取得單一技能的完整元資料＋SKILL.md 內容                                                               |
| `Dardcor Code_agent_skills_coverage`   | `read:catalog`  | 覆蓋率統計：22 個 API 與 20 個 CLI 技能中有多少個在檔案系統上擁有 SKILL.md 檔案，與目錄總數比較                   |
...
Cloud Agents 是行程外的 AI 編碼代理（codex-cloud、devin、jules），透過與 LLM 提供者相同的連線模型接入 Dardcor Code。它們透過自己的 REST 介面（`/api/v1/agents/*`）暴露，且**不屬於** MCP 工具目錄的一部分 — 呼叫 Cloud Agent 不會消耗 MCP 範圍。
...
| 變數                                      | 預設值                             | 用途                                                                                                                        |
| :---------------------------------------- | :--------------------------------- | :-------------------------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_BASE_URL`                      | `http://localhost:20128`           | MCP 伺服器在呼叫 Dardcor Code 內部 API 時使用的基礎 URL                                                                        |
...
| `DATA_DIR`                                | `~/.Dardcor Code`                     | 心跳檔案寫入至 `${DATA_DIR}/runtime/mcp-heartbeat.json`                                                                      |
...
- 即時統計資料透過 `Dardcor Code_compression_status` 在 `analytics.mcpDescriptionCompression` 下呈現，並標記為 `source: "mcp_metadata_estimate"`，以與真實的提供者使用收據區分。
