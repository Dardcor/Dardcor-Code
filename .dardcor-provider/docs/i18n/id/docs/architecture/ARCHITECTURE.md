# Arsitektur Dardcor Code (Bahasa Indonesia)
...
Dardcor Code adalah gateway routing AI lokal dan dasbor yang dibangun di atas Next.js.
...
- File: `${DATA_DIR}/storage.sqlite` (atau `$XDG_CONFIG_HOME/Dardcor Code/storage.sqlite` jika diatur, jika tidak `~/.Dardcor Code/storage.sqlite`)
...
- respons klien akhir yang dikembalikan oleh Dardcor Code; tanggapan yang dialirkan disimpan dalam bentuk ringkasan ringkas yang sama
...
1. `usageDb` dan `localDb` berbagi kebijakan direktori dasar yang sama (`DATA_DIR` -> `XDG_CONFIG_HOME/Dardcor Code` -> `~/.Dardcor Code`) dengan migrasi file lama.
...
5. Direktori `open-sse/` diterbitkan sebagai `@Dardcor Code/open-sse` **paket ruang kerja npm**. Kode sumber mengimpornya melalui `@Dardcor Code/open-sse/...` (diselesaikan oleh Next.js `transpilePackages`). Jalur file dalam dokumen ini masih menggunakan nama direktori `open-sse/` untuk konsistensi.
...
- Bangun gambar Docker: `docker build -t Dardcor Code .`
