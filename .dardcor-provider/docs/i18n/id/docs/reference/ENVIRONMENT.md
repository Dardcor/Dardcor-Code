...
> Referensi lengkap untuk setiap variabel lingkungan yang dikenali oleh Dardcor Code.
...
Dardcor Code menggunakan **SQLite** (melalui `better-sqlite3`) untuk semua persistensi data. Variabel-variabel ini mengontrol lokasi data, enkripsi, dan siklus hidup data.

| Variable                         | Default              | Source File                                     | Deskripsi                                                                                                              |
| -------------------------------- | -------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `DATA_DIR`                       | `~/.Dardcor Code/`      | `src/lib/db/core.ts`                            | Direktori utama untuk DB SQLite, cadangan, dan file data. Override untuk volume Docker atau path khusus.               |
...
| --------------------------- | -------------------------------------------------------------------------------------------- |
| **Pengembangan lokal**      | Biarkan semua nilai default. DB berada di `~/.Dardcor Code/Dardcor Code.db`.                       |
...
| **CI/Testing**              | `DATA_DIR=/tmp/Dardcor Code-test` — bersifat sementara, tidak perlu enkripsi.                   |
...
Dardcor Code menyediakan pertahanan dua lapis: pemindaian injeksi di sisi permintaan dan penghapusan PII di sisi respons.
...
> [!IMPORTANT]
> Saat melakukan deployment di balik reverse proxy (nginx, Caddy), `NEXT_PUBLIC_BASE_URL` **harus** diatur ke URL publik Anda (misalnya, `https://Dardcor Code.example.com`). Tanpa ini, callback OAuth akan gagal karena redirect_uri tidak akan cocok.
...
Mengontrol bagaimana Dardcor Code menemukan dan menjalankan sidecar CLI (Claude Code, Codex, dll.).
...
| `CLI_ALLOW_CONFIG_WRITES` | `false`    | `src/shared/services/cliRuntime.ts` | Izinkan Dardcor Code menulis file konfigurasi CLI (penyegaran token, data sesi).    |
...
```bash
# Mount biner host ke kontainer dan beri tahu Dardcor Code lokasinya:
...
| Variable                                | Default          | Source File                                 | Deskripsi                                                                                                                         |
| --------------------------------------- | ---------------- | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_BASE_URL`                    | deteksi otomatis | `open-sse/mcp-server/server.ts`             | URL eksplisit agar alat MCP/A2A dapat menjangkau Dardcor Code. Menimpa deteksi otomatis localhost.                                   |
...
| Variable            | Default          | Source File                     | Deskripsi                                            |
| ------------------- | ---------------- | ------------------------------- | ---------------------------------------------------- |
| `OMNIROUTE_SERVER`  | deteksi otomatis | `src/lib/oauth/config/index.ts` | URL server untuk jembatan autentikasi CLI↔Dardcor Code. |
...
Saat diaktifkan, Dardcor Code mengatur ulang urutan header HTTP dan field body JSON agar cocok dengan tanda tangan persis dari alat CLI resmi. Hal ini mengurangi risiko pemblokiran akun sambil mempertahankan IP proxy Anda.
...
| Variable                   | Default              | Deskripsi                                                                                                                 |
| -------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_MEMORY_MB`      | `512`                | Batas heap V8 saat runtime. Docker standalone dan `Dardcor Code serve` menggunakannya untuk mengatur `--max-old-space-size`. |
...
| `INSTANCE_NAME=Dardcor Code`                             | Ada di template docs/env lama tetapi tidak digunakan saat runtime. Mungkin kembali dalam fitur multi-instansi di masa depan. |
