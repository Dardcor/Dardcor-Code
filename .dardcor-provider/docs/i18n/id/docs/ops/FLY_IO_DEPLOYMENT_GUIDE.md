# Panduan Deployment Dardcor Code di Fly.io
...
Dokumen ini menjelaskan metode deployment Dardcor Code di Fly.io yang telah terbukti berhasil, mencakup dua skenario utama:
...
Dokumen ini disusun berdasarkan konfigurasi yang telah diverifikasi pada proyek saat ini, dengan nama aplikasi `Dardcor Code`.
...
- Alamat akses: `https://Dardcor Code.fly.dev/`
...
- `app = 'Dardcor Code'` menentukan aplikasi Fly mana yang menjadi target deployment
...
- Jika sebelumnya Anda menggunakan nama lain seperti `oroute`, jangan sampai tertukar dengan `Dardcor Code`
...
Jika Anda sudah mengganti nama aplikasi, ganti `Dardcor Code` dengan nama Anda.
...
Parameter-parameter berikut telah digunakan secara nyata pada aplikasi `Dardcor Code` saat ini:
...
- Berlaku untuk proyek saat ini yaitu `Dardcor Code`
...
```powershell
flyctl secrets set INITIAL_PASSWORD=kata-sandi-kuat-anda -a Dardcor Code
```
...
- Apakah aplikasi yang sedang dilihat adalah `Dardcor Code`
...
Jika repositori saat ini adalah fork dan Anda ingin menyinkronkan pembaruan dari upstream `https://github.com/diegosouzapw/Dardcor Code`, ikuti alur berikut.
...
- Anda membuka aplikasi yang salah, misalnya `oroute`, bukan `Dardcor Code`
