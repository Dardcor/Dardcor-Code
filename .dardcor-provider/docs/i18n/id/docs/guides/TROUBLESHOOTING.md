...
Masalah umum dan solusinya untuk Dardcor Code.
...
| EACCES: permission denied                           | Atur `DATA_DIR=/path/to/writable/dir` untuk mengganti `~/.Dardcor Code`                                                                                            |
...
| `dlopen` / `slice is not valid mach-o file` (macOS) | Jalankan `cd $(npm root -g)/Dardcor Code/app && npm rebuild better-sqlite3 && Dardcor Code` — lihat [Pembangunan ulang modul native macOS](#macos-native-module-rebuild) di bawah |
...
**Penyebab:** Anda menjalankan versi Node.js di luar batas runtime aman yang disetujui Dardcor Code. Kasus paling umum adalah menjalankan Node 20, 22, atau 24 versi patch lama yang berada di bawah batas keamanan yang diperlukan Dardcor Code.
...
3. Instal ulang Dardcor Code: `npm install -g Dardcor Code`
4. Mulai ulang: `Dardcor Code`
...
**Solusi (v3.5.5+):** Dardcor Code sekarang menggunakan fungsi `fetch()` milik undici sendiri ketika dispatcher proxy aktif, memastikan perilaku yang konsisten. Perbarui ke v3.5.5+.
...
Dardcor Code memperbarui token secara otomatis. Jika masalah berlanjut:
...
2. Pastikan `CLOUD_URL` mengarah ke endpoint cloud Anda (misalnya, `https://Dardcor Code.dev`)
...
Ketika banyak permintaan bersamaan mengenai penyedia yang dibatasi lajunya, Dardcor Code menggunakan mutex + auto rate-limiting untuk membuat serialisasi permintaan dan mencegah kegagalan berantai. Ini berjalan otomatis untuk penyedia dengan API key.

---

## Taksonomi Kegagalan RAG / LLM Opsional (16 masalah)

Beberapa pengguna Dardcor Code menempatkan gateway di depan tumpukan RAG atau agen. Dalam pengaturan tersebut, umum terjadi pola yang aneh: Dardcor Code terlihat sehat (penyedia aktif, profil routing baik, tidak ada peringatan batas laju) tetapi jawaban akhir masih salah.
...
1. Saat Anda menyelidiki respons yang buruk, kumpulkan:
   ...
   - route atau combo penyedia di Dardcor Code
...
3. Simpan nomornya di dashboard, runbook, atau pelacak insiden Anda sendiri di samping log Dardcor Code.
...
Anda dapat mengabaikan bagian ini jika Anda tidak menjalankan pipeline RAG atau agen di belakang Dardcor Code.
