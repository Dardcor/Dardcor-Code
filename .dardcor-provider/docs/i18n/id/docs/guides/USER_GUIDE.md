...
Panduan lengkap untuk mengonfigurasi penyedia, membuat combo, mengintegrasikan alat CLI, dan menerapkan Dardcor Code.
...
**Tips Pro:** Gunakan Opus untuk tugas kompleks, Sonnet untuk kecepatan. Dardcor Code melacak kuota per model!
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [dari dasbor Dardcor Code]
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "Dardcor Code/if/glm-4.7" }
    }
  },
  "models": {
    "providers": {
...
CLI secara otomatis memuat `.env` dari `~/.Dardcor Code/.env` atau `./.env`.
...
| Perintah                 | Tindakan                                                                                           |
| ------------------------ | -------------------------------------------------------------------------------------------------- |
| `npm run uninstall`      | Menghapus aplikasi dari sistem tetapi **menyimpan DB dan konfigurasi** di `~/.Dardcor Code`.          |
| `npm run uninstall:full` | Menghapus aplikasi DAN secara permanen **menghapus semua konfigurasi, kunci, dan basis data**. |

> Catatan: Untuk menjalankan perintah ini, navigasikan ke folder proyek Dardcor Code (jika Anda telah meng-clone-nya) dan jalankan. Atau, jika diinstal secara global, Anda cukup menjalankan `npm uninstall -g Dardcor Code`.
...
Pengguna Void Linux dapat mengemas dan menginstal Dardcor Code secara native menggunakan framework kompilasi silang `xbps-src`. Ini mengotomasi build standalone Node.js beserta binding native `better-sqlite3` yang diperlukan.
...
| --------------------------------------- | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `Dardcor Code-default-secret-change-me` | Rahasia penandatanganan JWT (**ubah di produksi**)                                                                         |
| `INITIAL_PASSWORD`                      | `123456`                             | Kata sandi login pertama                                                                                                   |
| `DATA_DIR`                              | `~/.Dardcor Code`                       | Direktori data (db, penggunaan, log)                                                                                       |
...
| `CLOUD_URL`                             | `https://Dardcor Code.dev`              | Cloud sinkronisasi titik akhir berbasis URL                                                                                      |
...
- Quick Tunnel tidak dipulihkan otomatis setelah Dardcor Code atau container di-restart; aktifkan kembali dari dasbor bila diperlukan
...
- **Cache Semantik** — Otomatis menyimpan respons non-streaming, temperature=0 (lewati dengan `X-Dardcor Code-No-Cache: true`)
- **Idempotensitas Permintaan** — Mendeduplikasi permintaan dalam 5 detik melalui header `Idempotency-Key` atau `X-Request-Id`
- **Pelacakan Progres** — Event SSE `event: progress` yang bisa diaktifkan melalui header `X-Dardcor Code-Progress: true`

---

### Translator Playground

Akses melalui **Dashboard → Translator**. Debug dan visualisasikan bagaimana Dardcor Code menerjemahkan permintaan API antar penyedia.
...
Dardcor Code juga menerima `x_session_id` dan mengembalikan kunci sesi efektif di `X-Dardcor Code-Session-Id`.
...
Dardcor Code mengimplementasikan ketahanan tingkat penyedia dengan lima komponen:
...
4. **Tunggu Cooldown** — Jika setiap kandidat koneksi sudah cooldown, Dardcor Code dapat menunggu cooldown paling awal dan mencoba kembali permintaan klien yang sama secara otomatis.
...
- Migrasi Dardcor Code antar mesin
...
Dardcor Code mendukung transkripsi audio melalui titik akhir yang kompatibel dengan OpenAI:
...
Dardcor Code tersedia sebagai aplikasi desktop asli untuk Windows, macOS, dan Linux.
