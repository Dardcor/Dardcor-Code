...
**Dardcor Code** — proxy/router de IA unificado. Un punto final, más de 160 proveedores de LLM, retroceso automático.
...
Dardcor Code tiene tres mecanismos de falla temporal relacionados pero distintos. Mantenga su
...
- **Importaciones**: externo → interno (`@/`, `@Dardcor Code/open-sse`) → relativo
...
- **Alias de ruta**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Puerto predeterminado**: 20128 (API + dashboard en el mismo puerto)
- **Directorio de datos**: variable de entorno `DATA_DIR`, por defecto `~/.Dardcor Code/`
...
16. Nunca incluyas trailers `Co-Authored-By` que acrediten a un asistente de IA, LLM o cuenta automatizada (p. ej. nombres que contengan "Claude", "GPT", "Copilot", "Bot"; correos en `anthropic.com` / `openai.com` / direcciones `noreply.github.com` propiedad de bots). Tales trailers redirigen la atribución del commit a la cuenta del bot en GitHub, ocultando al autor real (`diegosouzapw`) en el historial del PR. Los colaboradores humanos — incluyendo autores de PRs upstream y reporteros de issues que se portan a Dardcor Code — PUEDEN y DEBEN ser acreditados con trailers estándar `Co-authored-by: Name <email>`; los flujos de trabajo de port upstream (`/port-upstream-features`, `/port-upstream-issues`) dependen de esto.
