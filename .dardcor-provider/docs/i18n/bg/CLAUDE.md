...
**Dardcor Code** — обединен AI прокси/рутер. Една крайна точка, 160+ LLM доставчици, автоматично резервиране.
...
Dardcor Code има три свързани, но различни механизма за временно неуспех. Дръжте обхвата им отделен при отстраняване на проблеми с маршрутизацията. Вижте
...
- **Импорти**: външни → вътрешни (`@/`, `@Dardcor Code/open-sse`) → относителни
...
- **Пътни псевдоними**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Порт по подразбиране**: 20128 (API + табло на същия порт)
- **Директория за данни**: `DATA_DIR` env var, по подразбиране `~/.Dardcor Code/`
...
16. Никога не включвайте `Co-Authored-By` трейлъри, които кредитират AI асистент, LLM или автоматизиран акаунт (напр. имена, съдържащи "Claude", "GPT", "Copilot", "Bot"; имейли на `anthropic.com` / `openai.com` / `noreply.github.com` адреси, притежавани от ботове). Такива трейлъри пренасочват атрибуцията към бот акаунта в GitHub, скривайки истинския автор (`diegosouzapw`) в историята на PR. Човешките сътрудници — включително авторите на upstream PR и докладвачите на issues, които се пренасят в Dardcor Code — МОГАТ и ТРЯБВА да бъдат кредитирани със стандартни `Co-authored-by: Name <email>` трейлъри; upstream-port работните потоци (`/port-upstream-features`, `/port-upstream-issues`) зависят от това.
