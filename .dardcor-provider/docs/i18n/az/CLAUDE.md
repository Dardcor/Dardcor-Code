...
**Dardcor Code** — birləşdirilmiş AI proxy/router. Bir uç nöqtə, 160+ LLM təminatçısı, avtomatik geri dönmə.
...
Dardcor Code-un üç əlaqəli, lakin fərqli müvəqqəti uğursuzluq mexanizmi var. Marşrut davranışını düzəldərkən onların
...
- **İdxallar**: xarici → daxili (`@/`, `@Dardcor Code/open-sse`) → nisbət
...
- **Yol aliasları**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Default port**: 20128 (API + dashboard eyni portda)
- **Məlumat qovluğu**: `DATA_DIR` env var, varsayılan `~/.Dardcor Code/`
