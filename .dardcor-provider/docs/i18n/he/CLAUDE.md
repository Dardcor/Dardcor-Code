...
**Dardcor Code** — פרוקסי/נתב AI מאוחד. נקודת קצה אחת, 160+ ספקי LLM, חזרה אוטומטית.
...
Dardcor Code יש שלושה מנגנוני כישלון זמניים הקשורים אך שונים. שמור על התחום שלהם נפרד כאשר אתה מדבג התנהגות נתיב. ראה את
...
- **ייבוא**: חיצוני → פנימי (`@/`, `@Dardcor Code/open-sse`) → יחסי
...
- **Alias נתיב**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **פורט ברירת מחדל**: 20128 (API + לוח מחוונים באותו פורט)
- **ספריית נתונים**: משתנה סביבה `DATA_DIR`, ברירת מחדל ל-`~/.Dardcor Code/`
