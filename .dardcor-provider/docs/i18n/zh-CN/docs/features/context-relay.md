...
- 在活跃账户耗尽之前，Dardcor Code 生成一份紧凑的结构化摘要
- 身份认证为同一会话选择了不同账户后，Dardcor Code 将该摘要作为系统消息注入下一次请求
- 交接被成功消费后，将从存储中移除
...
如果活跃服务商在 `handoffProviders` 中已启用，Dardcor Code 将在账户完全耗尽前在后台生成一份结构化的交接摘要。
...
当同一会话的下一次请求解析到不同的认证账户时，Dardcor Code 将存储的交接作为系统消息前置注入。注入仅在实际账户切换被确认后发生。
...
在注入时，Dardcor Code 将该载荷转换为 `<context_handoff>` 系统消息，使下一个账户能够在正确的本地上下文中继续。
