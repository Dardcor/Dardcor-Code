---
title: "Dardcor Code Fly.io 部署指南"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code Fly.io 部署指南

本文档记录 Dardcor Code 在 Fly.io 上的实际部署方法，适用于两类场景：

- 首次把当前项目部署到 Fly.io
- 后续代码更新后继续发布
- 新项目参考同样流程部署

本文基于当前项目已经验证通过的配置整理，应用名为 `Dardcor Code`。
...
- 访问地址：`https://Dardcor Code.fly.dev/`
...
- `app = 'Dardcor Code'` 决定实际部署到哪个 Fly 应用
...
- 以前如果用过别的名字，例如 `oroute`，不要和 `Dardcor Code` 混淆
...
如果你已经改成别的应用名，把 `Dardcor Code` 替换成你的名字。
...
这些参数已经在当前 `Dardcor Code` 应用上实际部署：
...
1. **设置 `NEXT_PUBLIC_BASE_URL` 指向你公开的 HTTPS 域名 (set `NEXT_PUBLIC_BASE_URL` to the public HTTPS domain / defina `NEXT_PUBLIC_BASE_URL` para o domínio HTTPS público)**
   ...
   如果你使用了自定义域名 (if using a custom domain / se usar um domínio personalizado)，请替换为对应域名 (e.g. `https://Dardcor Code.yourdomain.com`)。
...
```powershell
flyctl secrets set INITIAL_PASSWORD=你的强密码 -a Dardcor Code
```
...
如果当前仓库是 fork，并且你要同步上游 `https://github.com/diegosouzapw/Dardcor Code` 的更新，推荐按下面流程执行。
...
- 你打开的是另一个应用，例如 `oroute`，不是 `Dardcor Code`
