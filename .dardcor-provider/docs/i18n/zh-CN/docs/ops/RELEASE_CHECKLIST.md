...
- [ ] 若 `src/lib/db/migrations/` 有新增文件：
  ...
  - [ ] 迁移包裹在事务中
  - [ ] 编号正确（序列中无间隙）
- [ ] 全新安装测试：删除 `~/.Dardcor Code/Dardcor Code.db` 并运行 `npm run dev`
...
> **运维说明：** 远程 VPS 镜像目录仍为 `/usr/lib/node_modules/Dardcor Code/app/`。
...
- [ ] `Dardcor Code --tray` 在 macOS 上启动（systray2 安装到 `~/.Dardcor Code/runtime/`）
- [ ] `Dardcor Code --tray` 在 Linux 上启动（需要 DISPLAY；未设置时优雅报错）
- [ ] `Dardcor Code --tray` 在 Windows 上启动（PowerShell NotifyIcon，无额外二进制文件）
- [ ] `Dardcor Code config tray enable` 创建自启动条目；disable 则移除
- [ ] `npm install -g Dardcor Code@<this-version>` 运行 postinstall 无致命退出
- [ ] 更新路径保留可选依赖：`Dardcor Code update --apply` 以及自动更新器
      ...
      SLM 层将静默失效。
- [ ] `Dardcor Code status` 在无 `.env` 的情况下正常工作（CLI Token 路径，仅 loopback）
...
- [ ] 全部 10 个 `skills/Dardcor Code*/SKILL.md` 文件可通过 GitHub raw URL 公开访问
