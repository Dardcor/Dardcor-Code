...
本指南记录了保护 **Dardcor Code** 并将应用安全暴露到互联网的网络基础设施黄金标准，**无需开放任何端口（Zero Inbound）**。

## 您的虚拟机上做了什么？

我们通过 PM2 以 **Split-Port** 模式启动了 Dardcor Code：
...
3. 点击 **Add a Tunnel**，选择 **Cloudflared**，命名为 `Dardcor Code-VM`。
...
4. 在 **Application domain** 中，填入 `Dardcor Code.yourdomain.com`（与"路由 2"中设置的一致）。
5. 点击 **Next**。
...
> **效果：** 如果您尝试打开 `Dardcor Code.yourdomain.com`，将不再直接进入您的 Dardcor Code 应用！而是跳转到一个精美的 Cloudflare 页面，要求输入邮箱地址。只有您（或您填写的邮箱）输入后，Outlook/Gmail 会收到一个 6 位临时验证码，验证通过后才会解除隧道限制，允许访问 `20129` 端口。
...
2. Dardcor Code 仅通过 `cloudflared` 进行 HTTPS 出站通信，不直接接收来自外部的 TCP 连接。
