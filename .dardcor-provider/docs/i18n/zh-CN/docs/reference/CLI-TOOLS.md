...
Dardcor Code 与三类 CLI 工具集成，分布在三个专用的 dashboard 页面中：

| 页面             | 路由                     | 概念                                                                        | 数量         |
| ---------------- | ------------------------ | --------------------------------------------------------------------------- | ------------ |
| **CLI Code's**   | `/dashboard/cli-code`    | 指向 Dardcor Code 的编程工具（客户端 → CLI → Dardcor Code → 服务商）               | 19           |
| **CLI Agents**   | `/dashboard/cli-agents`  | 指向 Dardcor Code 的自主代理（相同流程，更广泛的范围）                           | 6            |
| **ACP Agents**   | `/dashboard/acp-agents`  | Dardcor Code 通过 stdio/ACP 作为后端启动的 CLI（反向流程）                       | 见注册表      |
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
    ...
    http://YOUR_SERVER:20128/v1
           │
           ▼  （Dardcor Code 路由到合适的服务商）
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP Agents（反向启动流程）：
    客户端请求 → Dardcor Code → 通过 stdio/ACP 启动 CLI → 响应
```
...
无需手动编写每个工具的配置。Dardcor Code 为每个支持的 CLI 提供对应的 `setup-*` 命令，该命令从运行中的 Dardcor Code（本地或远程）读取**实时**模型目录并将工具的配置写入你的机器：
...
每个命令接受 `--remote <url> --api-key <key>`（针对远程 Dardcor Code 配置本地工具）、`--dry-run`（预览不写入）和 `--port`。不支持模型自动发现的工具（Cline、Kilo、Roo、Goose、Qwen、Aider、Gemini）接受 `--model <id>`（以及用于非交互式运行的 `--yes`）。启动器 `Dardcor Code launch`（Claude Code）和 `Dardcor Code launch-codex`（Codex）在注入正确的环境变量后启动 CLI，不写入任何配置。
...
此页面（从 `/dashboard/agents` 重命名而来）显示 Dardcor Code 可以通过 stdio/ACP 协议**启动**为后端执行引擎的 CLI。目录在 `src/lib/acp/registry.ts` 中单独维护，**不同于** `CLI_TOOLS`。
...
> 使用 `opencode run "your prompt" --model Dardcor Code/claude-sonnet-4-5-thinking --variant high`
> 发送 thinking 变体。
...
或使用 Dardcor Code dashboard → **CLI Tools → Cline → 应用配置**。
...
或使用 Dardcor Code dashboard → **CLI Tools → KiloCode → 应用配置**。
...
当 VS Code Insiders 配置了自定义端点模型，且你希望 Dardcor Code 在不使用自定义请求头字段的情况下工作时使用。
...
- 将 `sk-your-Dardcor Code-key` 替换为在 Dardcor Code 中创建的 API Key。
...
# CLI 使用自己的认证 — Kiro CLI 本身不需要 Dardcor Code 作为后端。
# 将 kiro-cli 与 Dardcor Code 配合用于其他工具。
kiro-cli status
```

对于 **Kiro IDE** 桌面应用，使用 Dardcor Code 通过 `/dashboard/cli-tools → Kiro` 暴露的 MITM 端点。
...
> Qwen OAuth 免费层已于 2026-04-15 停用。改为使用 Dardcor Code 搭配
...
OPENAI_API_KEY="sk-your-Dardcor Code-key"
...
## 10. Dardcor Code 内置 CLI

`Dardcor Code` 二进制文件提供服务端生命周期、设置、诊断和服务商管理的命令。入口点：`bin/Dardcor Code.mjs`。

```bash
Dardcor Code                              # 启动服务器（默认端口 20128）
Dardcor Code setup                        # 交互式设置向导
Dardcor Code doctor                       # 检查配置、数据库、端口、运行时
Dardcor Code providers list               # 已配置的服务商连接
Dardcor Code providers test-all           # 测试所有活跃连接
Dardcor Code reset-password               # 重置管理员密码
Dardcor Code logs                         # 流式输出请求日志
Dardcor Code health                       # 详细健康状态（熔断器、缓存、内存）
Dardcor Code --version                    # 输出版本号
Dardcor Code --help                       # 显示所有命令
```
...
```bash
Dardcor Code setup                        # 交互式设置向导
Dardcor Code setup --non-interactive      # CI/自动化模式（读取环境变量 + 标志）
Dardcor Code setup --password '<value>'   # 直接设置管理员密码
...
| `DATA_DIR`          | 覆盖 Dardcor Code 数据目录                                      |

其他所有非交互式输入通过标志传入，而非环境变量：
`--password`、`--provider`、`--provider-name`、`--provider-base-url`、`--default-model`
（参见上述 `Dardcor Code setup` 选项）。

### 诊断

```bash
Dardcor Code doctor                       # 检查配置、数据库、端口、运行时、内存、存活状态
Dardcor Code doctor --json                # 机器可读的 JSON
Dardcor Code doctor --no-liveness         # 跳过 HTTP 健康探测
Dardcor Code doctor --host 0.0.0.0        # 覆盖存活探测的主机
Dardcor Code doctor --liveness-url <url>  # 完全覆盖健康端点 URL
```
...
```bash
Dardcor Code providers available                       # Dardcor Code 服务商目录
Dardcor Code providers available --search openai       # 按 id/name/alias/category 过滤目录
Dardcor Code providers available --category api-key    # 按分类过滤（api-key、oauth、free 等）
Dardcor Code providers available --json                # 机器可读的 JSON

Dardcor Code providers list                            # 已配置的服务商连接
Dardcor Code providers list --json

Dardcor Code providers test <id|name>                  # 测试一个已配置的连接
Dardcor Code providers test-all                        # 测试所有活跃连接
Dardcor Code providers validate                        # 仅本地结构校验
```

> `providers available` 读取 Dardcor Code 目录；`providers list/test/test-all/validate`
> 直接读取本地 SQLite 数据库，不需要服务器运行。

### 恢复与重置

```bash
Dardcor Code reset-password                # 重置管理员密码（旧别名仍然可用）
Dardcor Code reset-encrypted-columns       # 显示加密凭证重置的警告 + 干运行
Dardcor Code reset-encrypted-columns --force  # 实际在 SQLite 中将加密凭证设为 null
```

### 其他子命令

以下命令假定 Dardcor Code 服务器正在运行（另有说明除外）：

```bash
Dardcor Code status                       # 全面的运行时状态
Dardcor Code logs                         # 流式输出请求日志（--json、--search、--follow）
Dardcor Code config show                  # 显示当前配置

Dardcor Code provider list                # 列出可用服务商（providers list 的别名）
Dardcor Code provider add                 # 将 Dardcor Code 注册为某个工具的服务商
Dardcor Code keys add | list | remove     # 管理 API Key
Dardcor Code models [provider]            # 列出模型（--json、--search）
Dardcor Code combo list | switch | create | delete

Dardcor Code backup                       # 快照配置 + 数据库
Dardcor Code restore                      # 从之前的快照恢复

Dardcor Code health                       # 详细健康状态（熔断器、缓存、内存）
Dardcor Code quota                        # 服务商配额用量
Dardcor Code cache                        # 缓存状态
Dardcor Code cache clear                  # 清除语义 + 签名缓存

Dardcor Code mcp status | restart         # MCP 服务器状态 / 重启
Dardcor Code a2a status | card            # A2A 服务器状态 / agent card

Dardcor Code tunnel list | create | stop  # 管理隧道（cloudflare/tailscale/ngrok）
Dardcor Code env show | get <k> | set <k> <v>  # 检查 / 设置环境变量（临时）

Dardcor Code test                         # 服务商连通性冒烟测试
Dardcor Code update                       # 检查更新
Dardcor Code completion                   # 生成 shell 补全
```
...
可直接粘贴的示例（使用 Token 化 Dardcor Code URL）：
...
| 错误                                          | 原因                     | 修复方法                                          |
| --------------------------------------------- | ------------------------ | ------------------------------------------------- |
| `Connection refused`                          | Dardcor Code 未运行          | `Dardcor Code serve`                                 |
