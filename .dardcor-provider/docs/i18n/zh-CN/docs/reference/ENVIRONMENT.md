...
> Dardcor Code 全部环境变量的完整参考。如需快速上手模板，请参阅 [`.env.example`](../../../.env.example)。
...
| `OMNIROUTE_WS_BRIDGE_SECRET` | **是**（生产环境） | _(未设置)_           | `src/app/api/internal/codex-responses-ws/route.ts` | 内部 Codex Responses WebSocket 桥接的共享密钥。用于认证 Electron/浏览器 WS 中继与 Dardcor Code 之间的桥接请求。⚠️ **生产环境必须设置 — 未设置时所有 WS 桥接请求都会被拒绝。** 使用 `openssl rand -base64 32` 生成。                                                                        |
| `OMNIROUTE_PEER_STAMP_TOKEN` | 否（自动）         | _(每次启动自动生成)_ | `src/server/authz/policies/management.ts`          | 每个进程的密钥，用于证明可信的对等 IP 戳记来自 Dardcor Code 自身的 HTTP 服务器（`scripts/dev/peer-stamp.mjs`）。authz 中间件仅在戳记携带此 Token 时才信任请求的本地性（对 LOCAL_ONLY 路由进行 loopback/LAN 门控）。每次启动自动生成 — 保持未设置；仅在必须共享戳记的多进程场景下固定此值。 |
...
Dardcor Code 使用 **SQLite**（通过 `better-sqlite3`）进行所有持久化存储。以下变量控制数据位置、加密和生命周期。

| 变量                                   | 默认值               | 源文件                                                | 说明                                                                                                                                                     |
| -------------------------------------- | -------------------- | ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DATA_DIR`                             | `~/.Dardcor Code/`      | `src/lib/db/core.ts`                                  | SQLite 数据库、备份和数据文件的根目录。在 Docker 卷或自定义路径中可覆盖。                                                                                |
...
| 场景         | 配置                                                            |
| ------------ | --------------------------------------------------------------- |
| **本地开发** | 保留所有默认值。数据库位于 `~/.Dardcor Code/Dardcor Code.db`。        |
| **Docker**   | `DATA_DIR=/data` + 挂载卷到 `/data`。                           |
| **静态加密** | 设置 `STORAGE_ENCRYPTION_KEY` + 备份密钥！丢失密钥 = 丢失数据。 |
| **CI/测试**  | `DATA_DIR=/tmp/Dardcor Code-test` — 临时目录，无需加密。           |
...
| `OMNIROUTE_CLI_SALT`                    | `Dardcor Code-cli-auth-v1` | `src/lib/machineToken.ts`                        | 用于派生本地 CLI 认证 Token 的 HMAC 盐值。修改此值将轮换机器上的所有 CLI Token。参阅 `docs/security/CLI_TOKEN.md`。                                                                                                                                        |
...
Dardcor Code 提供两层防护：请求侧的注入扫描和响应侧的 PII 脱敏。
...
| `OMNIROUTE_PUBLIC_BASE_URL`               | _(未设置)_                                      | 公共源解析器、图片 URL                      | 最高优先级的浏览器侧 Dardcor Code 源，用于公共 URL 生成和源校验（例如 `/v1/chatgpt-web/image/<id>`）。当 OpenWebUI 或其他中继通过内部 URL 访问 Dardcor Code，但用户浏览器必须从 LAN、隧道或公共源获取图片时设置。**不要**包含 `/v1`。                                   |
...
| `OMNIROUTE_CGPT_WEB_PRO_TIMEOUT_MS`       | `1200000`（20 分钟）                            | `open-sse/executors/chatgpt-web.ts`         | chatgpt-web GPT-5.5 Pro 后台轮询交接的总体等待预算。Pro 推理在带外完成，Dardcor Code 轮询直到结果到达或预算耗尽。如果 Pro 请求完成前超时，请提高此值。                                                                                                               |
...
> [!IMPORTANT]
> 当部署在反向代理（nginx、Caddy）之后时，**必须**将 `NEXT_PUBLIC_BASE_URL` 设置为你的公共 URL（例如 `https://Dardcor Code.example.com`）。否则 OAuth 回调可能因 redirect_uri 不匹配而失败，生成的公共链接可能指向内部容器源，同源 Dashboard 变更可能被浏览器源检查拒绝。
>
> 对于服务器到服务器的任务，保持 `BASE_URL` 为内部 loopback/容器 URL。不要为携带凭证的内部自调用使用浏览器 `Origin` 或公共主机名。
>
> Dardcor Code 集中处理公共源校验：显式的公共 URL 环境变量优先被信任；原始的 `Forwarded` / `X-Forwarded-*` 头默认被忽略，除非启用了 `OMNIROUTE_TRUST_PROXY` 且直接代理对端经过 Token 戳记为可信。不要用 CORS 设置来修复同源 Dashboard 请求；CORS 仅用于跨源浏览器客户端。
...
控制 Dardcor Code 如何发现和启动 CLI sidecar（Claude Code、Codex 等）。
...
| `CLI_ALLOW_CONFIG_WRITES` | `false`     | `src/shared/services/cliRuntime.ts`                 | 允许 Dardcor Code 写入 CLI 配置文件（Token 刷新、会话数据）。                                                                                                |
...
| `HERMES_HOME`             | `~/.hermes` | `src/lib/cli-helper/config-generator/hermesHome.ts` | Hermes Agent 主目录，Dardcor Code 从此处读取/写入 Hermes CLI 配置。与 Hermes PowerShell 安装程序在 Windows 上设置的环境变量（`%LOCALAPPDATA%\hermes`）一致。 |

### Docker 示例

```bash
# 将主机二进制文件挂载到容器中，并告诉 Dardcor Code 它们的位置：
...
### CLI 二进制文件（`Dardcor Code`）辅助变量

以下变量调优 `Dardcor Code` CLI 二进制文件自身的行为（而非上面的 sidecar 检测）。

| 变量                           | 默认值     | 源文件                                  | 说明                                                                                                    |
| ------------------------------ | ---------- | --------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_LANG`               | _(系统)_   | `bin/cli/i18n.mjs`                      | 强制 CLI 输出语言。BCP-47 locale（如 `en`、`pt-BR`）。覆盖系统 locale 环境变量（LC_ALL, LC_MESSAGES）。 |
| `OMNIROUTE_SHOW_LOG`           | _(未设置)_ | `bin/cli/runtime/processSupervisor.mjs` | 设为 `1` 可在受监管模式下将服务器 stdout/stderr 转发到终端。等同于 `Dardcor Code serve` 的 `--log` 标志。  |
| `OMNIROUTE_CLI_TOKEN`          | _(未设置)_ | `bin/cli/api.mjs`                       | 作为 `x-Dardcor Code-cli-token` 头注入的机器认证 Token。在任务 8.12 中自动生成。                           |
...
| `OMNIROUTE_PLUGIN_PATH`        | _(未设置)_ | `bin/cli/plugins.mjs`                   | CLI 插件发现的自定义目录（`Dardcor Code-cmd-*` 包）。未设置时默认为 `~/.Dardcor Code/plugins/`。              |
...
| 变量                                            | 默认值                                              | 源文件                                                      | 说明                                                                                                                     |
| ----------------------------------------------- | --------------------------------------------------- | ----------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `OMNIROUTE_BASE_URL`                            | 自动检测                                            | `open-sse/mcp-server/server.ts`                             | MCP/A2A 工具访问 Dardcor Code 的显式 URL。覆盖 localhost 自动检测。                                                         |
...
| `OMNIROUTE_CONTEXT`                             | _(活跃上下文)_                                      | `bin/cli/program.mjs`, `bin/cli/api.mjs`                    | `Dardcor Code` 命令的 CLI 远程模式上下文/配置文件；覆盖本地上下文存储中的活跃上下文。等同于 `--context <name>`。            |
...
| 变量                | 默认值     | 源文件                          | 说明                                 |
| ------------------- | ---------- | ------------------------------- | ------------------------------------ |
| `OMNIROUTE_SERVER`  | 自动检测   | `src/lib/oauth/config/index.ts` | CLI↔Dardcor Code 认证桥接的服务器 URL。 |
...
| `BLACKBOX_WEB_VALIDATED_TOKEN`    | Blackbox Web            | 作为 `validated` 发送到 `/api/chat` 的前端 `tk` Token。当 Blackbox 强制 Token 匹配时必需；否则 Dardcor Code 回退到随机 UUID。参阅 issue #2252。                                                                                    |
| `VISION_BRIDGE_BASE_URL`          | Vision Bridge 安全护栏  | 非 Anthropic 视觉桥接调用的 OpenAI 兼容基础 URL。默认为旧版 OpenAI URL 或 api.openai.com。指向 Dardcor Code 的 `/v1` 自循环或任意 OpenAI 兼容端点（Gemini OpenAI-compat、OpenRouter）。Issue #2232。                               |
...
启用后，Dardcor Code 会重排 HTTP 头和 JSON body 字段以匹配官方 CLI 工具的精确签名。在保留代理 IP 的同时，降低账户被标记的风险。
...
| 变量                       | 默认值               | 说明                                                                                                                                                                                                       |
| -------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_MEMORY_MB`      | _自动_               | 运行时 V8 堆限制（MB）。未设置时动态校准（约 35% 系统内存，限制在 `[512, 4096]`）；`512` 仅是总内存不可读取时的下限。显式设置以覆盖。Docker 独立运行和 `Dardcor Code serve` 用它设置 `--max-old-space-size`。 |
...
| `NEXT_PUBLIC_CLOUDFLARE_RELAY_DEFAULT_PROJECT` | `Dardcor Code-relay`                      | `src/app/(dashboard)/dashboard/settings/components/proxy/CloudflareRelayModal.tsx` | 代理池"Deploy Relay"弹窗中建议的默认 worker 项目名。                                       |
...
| `NEXT_PUBLIC_DENO_RELAY_DEFAULT_PROJECT`       | `Dardcor Code-deno-relay`                 | `src/app/(dashboard)/dashboard/settings/components/proxy/DenoRelayModal.tsx`       | 代理池"Deploy Relay"弹窗中建议的默认 Deno Deploy 应用名。                                  |
...
Dardcor Code 会重写请求以使这些中继接受。如果你只想使用
...
NEXT_PUBLIC_BASE_URL=https://Dardcor Code.example.com
...
CORS_ORIGIN=https://Dardcor Code.example.com
...
| `NEXT_PUBLIC_VERCEL_RELAY_DEFAULT_PROJECT`  | `Dardcor Code-relay`                                                           | `src/app/(dashboard)/…/VercelRelayModal.tsx`                              | Vercel Relay 部署弹窗中预填的默认项目名。                                                                                                                                                                                                                                                            |
...
| `BIFROST_API_KEY`                           | _(未设置)_                                                                  | `src/app/api/v1/relay/chat/completions/bifrost/route.ts`                  | Bifrost 网关的 API key（作为 `Authorization: Bearer ...` 发送）。未设置时，路由期望请求携带有效的 Dardcor Code API key；此 key 仅用于网关侧认证。                                                                                                                                                       |
...
| `OMNIROUTE_TLS_CERT`                        | _(未设置)_                                                                  | `bin/cli/commands/serve.mjs`                                              | PEM TLS 证书路径，用于 `Dardcor Code serve` 通过 HTTPS 提供服务（等同于 `--tls-cert`）。必须与 `OMNIROUTE_TLS_KEY` 配对；独立服务器随后在同一监听器上终止 TLS（`wss://` 工作不变）。未设置 → 纯 HTTP。仅提供证书或密钥中的一个，或路径不可读，会记录警告并保持 HTTP。                                   |
| `OMNIROUTE_TLS_KEY`                         | _(未设置)_                                                                  | `bin/cli/commands/serve.mjs`                                              | `Dardcor Code serve` HTTPS 的 PEM TLS 私钥路径（等同于 `--tls-key`）。必须与 `OMNIROUTE_TLS_CERT` 配对。参阅 `OMNIROUTE_TLS_CERT`。                                                                                                                                                                     |
...
| `OMNIROUTE_REDIS_CONTAINER_NAME`            | `Dardcor Code-redis`                                                           | `bin/cli/commands/redis.mjs`                                              | 一键 Redis 启动器（`Dardcor Code redis up`）的容器名。CLI 和 `RedisLauncherPanel` GUI 均使用。                                                                                                                                                                                                          |
...
| `QDRANT_COLLECTION`                         | `Dardcor Code-memory`                                                          | _(可选集群配置文件)_                                                      | Dardcor Code 的对话记忆嵌入的 collection 名称。首次运行时以 `QDRANT_VECTOR_SIZE` 维度创建。                                                                                                                                                                                                             |
| `QDRANT_EMBEDDING_MODEL`                    | `text-embedding-3-small`                                                    | _(可选集群配置文件)_                                                      | Qdrant collection 元数据中记录的默认嵌入模型名称。实际嵌入由 Dardcor Code 设置中 `embeddingModel` 字段指向的任意服务商生成。                                                                                                                                                                            |
...
| `OMNIROUTE_SKIP_UNINSTALL_HOOK`        | `0`                            | `scripts/build/uninstall.mjs`             | 跳过 Dardcor Code 卸载钩子（CI 用，保持 `node_modules` 完整）。                                                                                             |
...
| `INSTANCE_NAME=Dardcor Code`                                                                                                                                                       | 存在于旧文档/环境模板中但运行时未使用。可能在未来多实例功能中回归。                                                 |
...
其中包含从运行中的 Dardcor Code 实例拉取的准确的 `limit.context` 和 `limit.output` 值。
这些变量都不是正常运行所需的 — 该脚本仅供开发者工具使用。

| 变量               | 默认值                   | 源文件                                    | 说明                                                                                                 |
| ------------------ | ------------------------ | ----------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_URL`    | `http://localhost:20128` | `scripts/ad-hoc/regen-opencode-config.ts` | 查询 `/v1/models` 的 Dardcor Code 实例基础 URL。                                                        |
| `OMNIROUTE_KEY`    | _(未设置)_               | `scripts/ad-hoc/regen-opencode-config.ts` | 认证 Dardcor Code `/v1/models` 端点的 API key。未设置时回退到 `OPENCODE_API_KEY`。                      |
