...
| 请求头                     | 方向   | 说明                                                                                                                            |
| -------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------- |
| `X-Dardcor Code-No-Cache`     | 请求   | 设为 `true` 以绕过缓存                                                                                                          |
| `x-Dardcor Code-no-memory`    | 请求   | 设为 `true` 以跳过本请求的记忆 + 技能注入（与 no-cache 镜像；避免每次调用的 Token/成本开销）                                     |
| `X-Dardcor Code-Progress`     | 请求   | 设为 `true` 以接收进度事件                                                                                                      |
...
| `X-Dardcor Code-Cache`        | 响应   | 缓存 `HIT` 或 `MISS`（非流式）                                                                                                  |
| `X-Dardcor Code-Idempotent`   | 响应   | 去重命中时为 `true`                                                                                                             |
| `X-Dardcor Code-Progress`     | 响应   | 进度跟踪开启时为 `enabled`                                                                                                      |
| `X-Dardcor Code-Session-Id`   | 响应   | Dardcor Code 使用的有效会话 ID                                                                                                     |
| `X-Dardcor Code-Request-Id`   | 响应   | 请求关联 ID（已知时）                                                                                                          |
| `X-Dardcor Code-Version`      | 响应   | Dardcor Code 构建版本号（始终返回）                                                                                                 |
| `X-Dardcor Code-Cost-Saved`   | 响应   | 缓存命中时节省的 USD 金额（仅缓存命中时）                                                                                        |

> Nginx 提示：如果依赖下划线请求头（如 `x_session_id`），请启用 `underscores_in_headers on;`。

> **成本遥测请求头：** 非流式成功响应还会携带 `X-Dardcor Code-*` 成本遥测系列 — `X-Dardcor Code-Response-Cost`（USD，固定 10 位小数；免费/无定价时为 `0.0000000000`）、`X-Dardcor Code-Tokens-In` / `X-Dardcor Code-Tokens-Out`、`X-Dardcor Code-Model`、`X-Dardcor Code-Provider`、`X-Dardcor Code-Latency-Ms`、`X-Dardcor Code-Cache-Hit` 以及 `X-Dardcor Code-Fallback-Attempts`（仅在 >0 时返回），外加 `X-Dardcor Code-Request-Id` 和 `X-Dardcor Code-Version`。这些请求头由 chat completions、`/v1/responses`、`/v1/messages` **以及媒体端点**发出 — `/v1/embeddings`、`/v1/images/generations`、`/v1/audio/speech`、`/v1/audio/transcriptions`、`/v1/rerank`、`/v1/videos/generations`、`/v1/music/generations` 和 `/v1/moderations`（成本始终为 `0`）。媒体成本按模态计算（按图片、按秒、按字符、按搜索单元），仅在定价可用时计算，否则为 `0`（fail-open）。

> **缓存命中成本语义：** 语义缓存命中时（`X-Dardcor Code-Cache-Hit: true`），不会发起上游调用，因此 `X-Dardcor Code-Response-Cost` 为 `0.0000000000`（即命中的**增量**成本）。原始/本应产生的成本单独在 `X-Dardcor Code-Cost-Saved` 中报告。计费消费者应累加 `X-Dardcor Code-Response-Cost`（命中成本为零）；缓存分析可聚合 `X-Dardcor Code-Cost-Saved`。
...
对于支持 thinking 的 Claude 模型，`/v1/models` 还会列出一个 **no-thinking** 变体，其 id 前缀为 `claude-3-Dardcor Code-no-thinking/`：
...
对于无法附加 `Authorization: Bearer ...` 的客户端，Dardcor Code 也接受通过 URL 传入 API Key：查询字符串兼容方式（`?token=...`、`?apiKey=...`、`?api_key=...`、`?key=...`）或下文介绍的专用 `/api/v1/vscode/{token}/...` 端点。
...
OpenAI **Codex CLI** 在 `supports_websockets = true` 时会在客户端侧校验模型名称，并**拒绝带服务商前缀的 id**，如 `codex/gpt-5.5`（`The 'codex/gpt-5.5' model is not supported when using Codex with a ChatGPT account`）。请发送**裸** id（如 `gpt-5.5`）。Dardcor Code 的桥接仅限 codex，因此会通过 `resolveCodexWsModelInfo` 将裸 id 重新解析为 codex 模型后隧道化到上游 — 尽管裸的 `gpt-5.5` 在 HTTP 下会路由到其他服务商。

#### 配置 OpenAI Codex CLI

通过在 `~/.codex/config.toml` 中添加支持 WebSocket 的自定义服务商，将 Codex CLI 指向 Dardcor Code（使用单独的 `CODEX_HOME` 以避免覆盖已有配置）：
...
env_key = "OMNIROUTE_API_KEY"             # 持有 Dardcor Code API Key（Bearer）
```

```bash
export OMNIROUTE_API_KEY=sk-...           # 一个 Dardcor Code API Key（若 REQUIRE_API_KEY=false 则任意 Key）
codex exec "Responda apenas: PONG"
```

CLI 将 `base_url + /responses` 升级为 WebSocket，Dardcor Code 将其隧道化到选定的 codex OAuth 连接。已对本地服务器完成端到端验证：ChatGPT 返回 `codex.rate_limits` + `response.created` 并流式传输补全结果。
...
- 基于 URL 的 Token 可能出现在反向代理日志、浏览器历史和 Dardcor Code 之外的遥测中。将其作为兼容选项而不是默认的认证方式。
...
Dardcor Code 事件（请求完成、配额耗尽、Key 轮换等）的出站 Webhook 订阅。
...
Cloud Agent 任务（Claude Code、Codex Cloud、OpenHands 等）代表 Dardcor Code 用户远程执行。
...
Dardcor Code 公开三个独立的临时故障机制；以下管理端点允许管理员读取和覆盖它们：
...
用于通过自定义可执行处理器扩展 Dardcor Code 的技能框架，以及市场集成。
...
Dardcor Code 内置一个 Model Context Protocol 服务器，支持 3 种传输方式（stdio、SSE、streamable-http）及权限域划分的工具。以下 dashboard 端点用于读取状态/审计数据并代理 HTTP 传输。
...
Dardcor Code 暴露一个 A2A（Agent-to-Agent）JSON-RPC 2.0 端点，并提供 REST 包装以供检查/dashboard 使用。
...
管理与 Dardcor Code 集成的 CLI 工具（antigravity、chipotle、commandCode、devin-cli 等）。完整列表参见 [Provider Reference](./PROVIDER_REFERENCE.md)。
...
检查运行时安全护栏（PII 检测、提示注入检测、视觉桥接）。安全护栏在每次请求中运行；按调用退出通过 `x-Dardcor Code-disabled-guardrails` 请求头实现 — 没有持久化的启用/禁用地表。
