...
配置服务商、创建 Combo、集成 CLI 工具和部署 Dardcor Code 的完整指南。
...
**技巧：** 复杂任务用 Opus，追求速度用 Sonnet。Dardcor Code 为每个模型单独追踪配额！

Claude 和 Claude Code 兼容路由对 Opus 和 Sonnet 模型保留 `max` 思考级别。Haiku 模型不接受 `max` 级别，Dardcor Code 会在发送到上游之前将该请求降级为较高的思考预算。
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "Dardcor Code/if/kimi-k2" }
    }
  },
  "models": {
    "providers": {
...
CLI 自动从 `~/.Dardcor Code/.env` 或 `./.env` 加载环境变量。

### 卸载

当你不再需要 Dardcor Code 时，我们提供了两个快速脚本来干净地移除：

| 命令                      | 作用                                                             |
| ------------------------- | ---------------------------------------------------------------- |
| `npm run uninstall`       | 移除系统应用，但**保留 `~/.Dardcor Code` 中的数据库和配置**         |
| `npm run uninstall:full`  | 移除应用并**永久删除���有配置、密钥和数据库**                     |

> 注意：运行这些命令需要进入 Dardcor Code 项目目录（如果你 clone 了项目）。如果全局安装，直接运行 `npm uninstall -g Dardcor Code` 即可。
...
Void Linux 用户可使用 `xbps-src` 交叉编译框架原生打包安装 Dardcor Code。这种方式可自动化完成 Node.js 独立构建及所需的 `better-sqlite3` 原生绑定。
...
| 变量                                    | 默认值                               | 说明                                                                                |
| --------------------------------------- | ------------------------------------ | ----------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `Dardcor Code-default-secret-change-me` | JWT 签名密钥（**生产环境必须修改**）                                                |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | 首次登录密码                                                                        |
| `DATA_DIR`                              | `~/.Dardcor Code`                       | 数据目录（数据库、用量、日志）                                                      |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://Dardcor Code.dev`              | Cloud Sync 端点基础 URL（替代旧版 `CLOUD_URL`）                                      |
...
**模型 ID 说明：** Dardcor Code 使用服务商原生的 ID（`claude-opus-4-8`、`gpt-5.5`、`glm-5.1`、`MiniMax-M2.7`、`kimi-k2.5`、`grok-4.20-0309-reasoning`）。部分 ID 带有带点版本号，这是因为上游 API 要求如此。如果某模型未在上方列出，运行 `Dardcor Code models --search <term>` 或调用 `GET /api/models/catalog` 确认可用性。
...
- Quick Tunnel 在 Dardcor Code 或容器重启后不会自动恢复；需要时从 Dashboard 重新启用
...
- **语义缓存** — 自动缓存非流式、temperature=0 的响应（通过 `X-Dardcor Code-No-Cache: true` 绕过）
- **请求幂等** — 通过 `Idempotency-Key` 或 `X-Request-Id` 头在 5 秒内对请求去重
- **进度追踪** — 通过 `X-Dardcor Code-Progress: true` 头选择加入 SSE `event: progress` 事件

---

### 翻译器实验场

通过 **Dashboard → Translator** 访问。调试和可视化 Dardcor Code 如何在服务商之间转换 API 请求。
...
Dardcor Code 也接受 `x_session_id`，并在 `X-Dardcor Code-Session-Id` 中返回生效的会话 Key。
...
4. **等待冷却** — 如果所有候选连接都已在冷却中，Dardcor Code 可以等待最早完成的冷却，然后自动重试同一个客户端请求。
...
Dardcor Code 通过 OpenAI 兼容端点支持音频转录：
...
当上游服务商要求严格的 `max_tokens`/`maxOutputTokens` 限制时，禁用 **Reasoning token buffer**。启用后，Combo 路由仅对已知输出上限的模型添加推理模型 Headroom，当安全的缓冲值超出客户端 Token 限制时保持其不变。如果客户端限制已高于已知上限，Dardcor Code 在发送上游请求前会将其限制到该上限。
...
Dardcor Code 内置了一个**得分驱动的自动路由器**，可跨所有已连接的服务商为每个请求选择最佳模型 — 无需维护 Combo。只需使用 `auto/*` 前缀发送请求，Dardcor Code 即可即时构建虚拟 Combo，按延时、费用、成功率、上下文适配度、任务匹配度、近期故障、配额和熔断器状态对候选模型进行评分。
...
Dardcor Code 同时是一个 **MCP 服务端**（Model Context Protocol）和一个 **A2A 服务端**（Agent-to-Agent JSON-RPC 2.0）。任何兼容 MCP 的 IDE 或代理主机都可以直接调用 Dardcor Code 工具 — 无需额外包装。

### MCP 传输方式

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `Dardcor Code --mcp`（适用于偏好 stdio 的 IDE 插件）
...
Dardcor Code 暴露一个可扩展的**技能框架** (`src/lib/skills/`)，使代理和 A2A 端点可以运行领域特定的例程（如 `code-review`、`summarize`、`extract-facts`、`web-research`）。
...
- 每个载荷包含 `X-Dardcor Code-Signature`（HMAC-SHA256）供验证
...
Dardcor Code 集成了云编程代理（**OpenAI Codex Cloud**、**Devin**、**Jules**、**Antigravity**），使你能够在处理本地路由的同一 Dashboard 中派发长时间运行的任务。

- 在 **Dashboard → Cloud Agents** 中创建任务，或通过 `POST /api/v1/agents/tasks`
- 按任务追踪状态、日志和产物
- 每个服务商使用自备 API Key — 凭据永不离开 Dardcor Code 实例
...
你可以通过 HTTP，使用具有 `manage` 权限域的 **Bearer Key** 来管理 Dardcor Code 的每一项资源（服务商、Combo、Key、设置）。
...
Dardcor Code 内置了 CLI 工具（`Dardcor Code …`），用于设置、诊断和运行时控制。这与 Dashboard 中的「CLI Tools」页面是**分开的**，后者用于配置第三方 CLI（Claude Code、Cursor、Codex、Cline 等）使之能够对接 Dardcor Code。

```bash
Dardcor Code setup                    # 交互式向导（密码、服务商、Combo）
Dardcor Code setup --non-interactive  # 适合 CI 环境
Dardcor Code doctor                   # 健康诊断（数据目录、数据库、服务商、端口）
Dardcor Code providers available      # 列出支持的服务商
Dardcor Code providers list           # 列出已配置的连接
Dardcor Code providers test <id>      # 实时测试服务商连接
Dardcor Code combos list              # 列出 Combo
Dardcor Code combos switch <name>     # 设置默认 Combo
Dardcor Code models                   # 列出可用模型（--json、--search）
Dardcor Code keys add | list | remove # 从终端管理 API Key
Dardcor Code backup                   # 快照配置 + 数据库
Dardcor Code restore [<timestamp>]    # 从快照恢复
Dardcor Code health                   # 详细健康信息（熔断器、缓存、内存）
Dardcor Code quota                    # 服务商配额用量
Dardcor Code mcp status               # MCP 服务端状态
Dardcor Code a2a status               # A2A 服务端状态
Dardcor Code tunnel list|create|stop  # Cloudflare/Tailscale/ngrok 隧道
Dardcor Code reset-password           # 重置管理员密码
Dardcor Code --mcp                    # 通过 stdio 启动 MCP 服务端
Dardcor Code --port 3000              # 在自定义端口启动服务端
```

提示：将 `Dardcor Code doctor --json` 与你的监控工具结合，用于对不健康的服务商连接发出告警。

---

## 🖥️ 桌面应用 (Electron)

Dardcor Code 提供适用于 Windows、macOS 和 Linux 的原生桌面应用。
