...
Dardcor Code 支持 **42 种语言**，提供完整的控制台 UI 翻译、文档翻译，以及阿拉伯语和希伯来语的 RTL 支持。
...
Dardcor Code 使用基于哈希的增量文档翻译器，后端为 OpenAI 兼容的 LLM 端点（通常通过 Dardcor Code Cloud 使用 `cx/gpt-5.4-mini`）：
...
**辅助翻译器** — 使用任意 OpenAI 兼容的 LLM API（包括 Dardcor Code 自身）翻译已有的 `docs/i18n/` Markdown 文件。适用于优化或重新翻译文档，质量优于 Google Translate。
...
`Dardcor Code` CLI 拥有独立的 i18n 层，与 Next.js 控制台分离。
...
| 优先级 | 来源                     | 示例                                   |
| ------ | ------------------------ | -------------------------------------- |
| 1      | `--lang` 标志            | `Dardcor Code --lang de status`           |
| 2      | `OMNIROUTE_LANG` 环境变量 | `OMNIROUTE_LANG=ja Dardcor Code providers` |
...
```bash
# 设置语言并保存到 ~/.Dardcor Code/.env（跨会话持久化）
...
保存的偏好以原子方式写入 `~/.Dardcor Code/.env`，并在 CLI 引导阶段加载，先于任何命令执行。
