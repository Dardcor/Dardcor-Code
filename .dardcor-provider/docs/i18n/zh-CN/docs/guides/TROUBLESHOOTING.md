...
Dardcor Code 常见问题及解决方案。

---

## 快速参考

**刚接触 Dardcor Code？** 从这里开始 —— 以下内容可解决 90% 的问题：

| 你看到的提示            | 含义                     | 解决方法                                                                                          |
| ----------------------- | ------------------------ | ------------------------------------------------------------------------------------------------- |
| "Can't connect"         | Dardcor Code 未运行          | 运行 `Dardcor Code` 或 `docker restart Dardcor Code`                                                     |
...
| EACCES: permission denied                           | 设置 `DATA_DIR=/path/to/writable/dir` 覆盖 `~/.Dardcor Code`                                                                                             |
| 路由策略无法保存                                    | 更新至最新 v3.x 版本（早期版本中已修复 settings 持久化的 Zod Schema 问题）                                                                            |
| 登录崩溃 / 空白页面                                 | 检查 Node.js 版本 —— 参见下方的 [Node.js 兼容性](#nodejs-兼容性)                                                                                      |
| `dlopen` / `slice is not valid mach-o file` (macOS) | 运行 `cd $(npm root -g)/Dardcor Code/app && npm rebuild better-sqlite3 && Dardcor Code` —— 参见下方的 [macOS 原生模块重新编译](#macos-原生模块重新编译)         |
...
**原因：** 你使用的 Node.js 版本不在 Dardcor Code 批准的安全运行时范围内。最常见的情况是运行的 Node.js 22 或 24 补丁版本过低，未达到 Dardcor Code 所需的安全补丁基线。
...
3. 重新安装 Dardcor Code：`npm install -g Dardcor Code`
4. 重启：`Dardcor Code`
...
**原因：** 执行全局 `npm install -g Dardcor Code` 后，包内的 `better-sqlite3` 原生二进制文件可能被编译为与你本地运行环境不同的架构或 Node.js ABI。这在 macOS 上很常见（Apple Silicon 和 Intel 均如此），当预编译的二进制文件与你的环境不匹配时就会发生。
...
**修复（v3.5.5+）：** Dardcor Code 现在在代理调度器激活时使用 undici 自身的 `fetch()` 函数，确保行为一致。请更新至 v3.5.5+。

### WSL 下的 MITM 代理：Windows 主机上的桌面应用未被拦截

**原因：** MITM 代理及其 CA 证书安装在 Dardcor Code 运行的环境中。在 WSL 下，该环境是 Linux 客户机，而 AI 桌面应用（Kiro、Trae、Copilot、Zed 等）运行在 Windows 主机上。主机应用不信任客户机的证书存储，也不会通过客户机的系统代理路由，因此桌面拦截不会在那里生效。

**建议：** 将 Dardcor Code 以原生方式运行在与你想要拦截的桌面应用相同的操作系统上（Windows 应用在 Windows 上运行；macOS/Linux 同理）。将 Dardcor Code 留在 WSL 内同时针对主机应用，需要手动在 Windows 主机上信任生成的 CA 证书，并将每个主机应用的网络/代理设置指向 WSL 代理端点 —— 这是一种不受支持、脆弱的配置方案。
...
Dardcor Code 会自动刷新 Token。如果问题持续：
...
2. 验证 `CLOUD_URL` 指向你的云端端点（例如 `https://Dardcor Code.dev`）
...
当大量并发请求同时打到一个受速率限制的服务商时，Dardcor Code 使用互斥锁 + 自动速率限制来串行化请求，防止级联失败。这对 API 密钥类服务商是自动生效的。

---

## 可选的 RAG / LLM 失败分类法（16 种问题）

部分 Dardcor Code 用户将网关部署在 RAG 或代理栈之前。在这些场景中，经常会出现一种奇怪的现象：Dardcor Code 看起来一切正常（服务商在线、路由配置正常、无线速告警），但最终答案仍然不对。
...
1. 当你调查一个错误响应时，捕获：
   - 用户任务和请求
   - Dardcor Code 中的路由或服务商 Combo
   - 下游使用的任何 RAG 上下文（检索到的文档、工具调用等）
2. 将该事件映射到一个或两个 WFGY ProblemMap 编号（`No.1` … `No.16`）。
3. 将编号存储在你自己的仪表盘、runbook 或事件追踪器中，紧邻 Dardcor Code 日志。
...
如果你不在 Dardcor Code 后面运行 RAG 或代理流水线，可以忽略本节。
...
3. 重启 Dardcor Code 并从 **仪表盘 → CLI Tools** 重新测试
...
- 从 CLI 运行 `Dardcor Code providers` 重新触发 OAuth 流程，或
