...
本指南介绍如何从系统中彻底移除 Dardcor Code。

---

## 快速卸载（v3.6.2+）

Dardcor Code 提供两个内置脚本用于干净移除：
...
此命令移除 Dardcor Code 应用程序，但**保留** `~/.Dardcor Code/` 中的数据库、配置、API Key 和服务商设置。如果你计划稍后重装并希望保留现有配置，请使用此方式。
...
- 打开 `设置 → 应用 → Dardcor Code → 卸载`
- 或从安装目录运行 NSIS 卸载程序

**macOS：**

- 将 `Dardcor Code.app` 从 `/Applications` 拖入废纸篓
- 删除数据：`rm -rf ~/Library/Application Support/Dardcor Code`

**Linux：**

- 删除 AppImage 文件
- 删除数据：`rm -rf ~/.Dardcor Code`
...
Dardcor Code 默认将数据存储在以下位置：

| 平台          | 默认路径                      | 覆盖方式                   |
| ------------- | ----------------------------- | ------------------------- |
| Linux         | `~/.Dardcor Code/`               | `DATA_DIR` 环境变量        |
| macOS         | `~/.Dardcor Code/`               | `DATA_DIR` 环境变量        |
| Windows       | `%APPDATA%/Dardcor Code/`        | `DATA_DIR` 环境变量        |
| Docker        | `/app/data/`（挂载卷）         | `DATA_DIR` 环境变量        |
| XDG 兼容      | `$XDG_CONFIG_HOME/Dardcor Code/` | `XDG_CONFIG_HOME` 环境变量 |
