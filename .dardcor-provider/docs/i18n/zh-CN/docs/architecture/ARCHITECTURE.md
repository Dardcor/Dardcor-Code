...
Dardcor Code 是基于 Next.js 构建的本地 AI 路由网关和控制台。
...
Dardcor Code 可以安装、监管并路由到本地运行的 AI 工具进程，称为**嵌入式服务**。
...
- Dardcor Code 响应元数据构建器：`src/domain/Dardcor CodeesponseMeta.ts`
...
- 文件：`${DATA_DIR}/storage.sqlite`（或设置了 `$XDG_CONFIG_HOME` 时为 `$XDG_CONFIG_HOME/Dardcor Code/storage.sqlite`，否则为 `~/.Dardcor Code/storage.sqlite`）
...
> **注意：** 下表是 Dardcor Code v3.8.0 中 237 个已注册服务商的代表性样本。
> 完整且持续更新的列表请参阅
...
- Dardcor Code 返回的最终客户端响应；流式响应以相同压缩摘要形式存储
...
1. `usageDb` 和 `localDb` 共享相同的基础目录策略（`DATA_DIR` -> `XDG_CONFIG_HOME/Dardcor Code` -> `~/.Dardcor Code`），并有旧文件迁移机制。
...
5. `open-sse/` 目录作为 `@Dardcor Code/open-sse` **npm workspace 包**发布。源码通过 `@Dardcor Code/open-sse/...` 导入（由 Next.js 的 `transpilePackages` 解析）。本文档中的文件路径为了一致性仍使用目录名 `open-sse/`。
...
- 从源码构建：`npm run build`
- 构建 Docker 镜像：`docker build -t Dardcor Code .`
- 启动服务并验证：
- `GET /api/settings`
- `GET /api/v1/models`
