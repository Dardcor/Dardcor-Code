...
> Dardcor Code 代码库工程参考文档，面向贡献者和集成开发者。
...
默认 HTTP 端口：**`20128`**（API 和仪表盘共享同一进程）。数据目录由 `DATA_DIR` 环境变量指定，默认为 `~/.Dardcor Code/`。
...
├── open-sse/             流式传输引擎工作空间（@Dardcor Code/open-sse）
├── electron/             桌面端封装（Electron 41 主进程 + preload）
├── bin/                  CLI 入口点（Dardcor Code、reset-password）
...
| `responses.ts`、`Dardcor CodeesponseMeta.ts`   | 响应格式辅助工具                  |
...
独立的 npm workspace，发布为 `@Dardcor Code/open-sse`。负责请求处理、执行器、翻译器、服务、转换器和 MCP 服务器。
...
```
bin/
├── Dardcor Code.mjs           主 CLI 入口（Node ESM）
...
- **导入**：外部 → 内部（`@/`、`@Dardcor Code/open-sse`）→ 相对路径。
