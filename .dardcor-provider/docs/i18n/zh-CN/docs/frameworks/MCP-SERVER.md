...
Dardcor Code --dev  # MCP 自动在 /mcp 端点启动
```
...
| 工具                              | 权限域                  | 描述                                                         |
| :-------------------------------- | :---------------------- | :----------------------------------------------------------- |
| `Dardcor Code_get_health`            | `read:health`           | 运行时间、内存、熔断器、速率限制、缓存统计                   |
| `Dardcor Code_list_combos`           | `read:combos`           | 所有已配置 Combo 及其策略（可选指标）                        |
| `Dardcor Code_get_combo_metrics`     | `read:combos`           | 指定 Combo 的性能指标                                        |
| `Dardcor Code_switch_combo`          | `write:combos`          | 激活或停用 Combo                                             |
| `Dardcor Code_check_quota`           | `read:quota`            | 已用/总配额、剩余百分比、重置时间、Token 健康状态            |
| `Dardcor Code_route_request`         | `execute:completions`   | 通过 Dardcor Code 路由发送聊天补全请求                          |
| `Dardcor Code_cost_report`           | `read:usage`            | 按时间段（会话/天/周/月）的费用报告                          |
| `Dardcor Code_list_models_catalog`   | `read:models`           | 完整模型目录，包含能力、状态、定价                           |

## 阶段 1 — 搜索

| 工具                   | 权限域             | 描述                                                                                  |
| :--------------------- | :----------------- | :------------------------------------------------------------------------------------ |
| `Dardcor Code_web_search` | `execute:search`   | 通过 Dardcor Code 搜索网关进行 Web 搜索（Serper/Brave/Perplexity/Exa/Tavily/Google PSE/Linkup/SearchAPI/SearXNG），支持容灾 |

## 高级工具（11）— 阶段 2

| 工具                                 | 权限域                                 | 描述                                                                 |
| :----------------------------------- | :------------------------------------- | :------------------------------------------------------------------- |
| `Dardcor Code_simulate_route`           | `read:health`、`read:combos`           | 包含容灾树的演习路由仿真                                             |
| `Dardcor Code_set_budget_guard`         | `write:budget`                         | 会话预算，支持降级/阻断/告警动作                                     |
| `Dardcor Code_set_routing_strategy`     | `write:combos`                         | 在运行时更新 Combo 策略（priority/weighted/auto 等）                 |
| `Dardcor Code_set_resilience_profile`   | `write:resilience`                     | 应用 `aggressive` / `balanced` / `conservative` 容灾预设文件         |
| `Dardcor Code_test_combo`               | `execute:completions`、`read:combos`   | 使用真实上游调用对 Combo 中的每个服务商进行实时测试                  |
| `Dardcor Code_get_provider_metrics`     | `read:health`                          | 每个服务商的指标，含 p50/p95/p99 延迟和熔断器状态                    |
| `Dardcor Code_best_combo_for_task`      | `read:combos`、`read:health`           | 根据任务类型推荐 Combo，含预算/延迟约束                             |
| `Dardcor Code_explain_route`            | `read:health`、`read:usage`            | 解释某次请求的路由决策依据（评分因子 + 容灾路径）                   |
| `Dardcor Code_get_session_snapshot`     | `read:usage`                           | 完整会话快照：费用、Token、热门模型/服务商、错误、预算守卫           |
| `Dardcor Code_db_health_check`          | `read:health`、`write:resilience`      | 诊断（并可自动修复）数据库异常，如损坏的 Combo 引用/孤儿行          |
| `Dardcor Code_sync_pricing`             | `pricing:write`                        | 从外部源（LiteLLM）同步定价数据；支持 `dryRun`                      |

## 缓存工具（2）

| 工具                      | 权限域          | 描述                                     |
| :------------------------ | :-------------- | :--------------------------------------- |
| `Dardcor Code_cache_stats`   | `read:cache`    | 语义缓存、提示缓存和幂等性统计           |
| `Dardcor Code_cache_flush`   | `write:cache`   | 全局或按签名/模型刷新缓存                |

## 压缩工具（5）

| 工具                                  | 权限域                | 描述                                                                                                       |
| :------------------------------------ | :-------------------- | :--------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_compression_status`        | `read:compression`    | 压缩设置、分析摘要和缓存感知统计（含 `analytics.mcpDescriptionCompression` 元数据）                         |
| `Dardcor Code_compression_configure`     | `write:compression`   | 配置压缩模式、阈值、目标比例、系统提示保留、MCP 描述压缩开关                                               |
| `Dardcor Code_set_compression_engine`    | `write:compression`   | 选择活跃引擎（off/caveman/rtk/stacked）及 Caveman/RTK 强度                                                 |
| `Dardcor Code_list_compression_combos`   | `read:compression`    | 列出已命名的压缩 Combo 及其引擎流水线                                                                      |
| `Dardcor Code_compression_combo_stats`   | `read:compression`    | 按压缩 Combo 和引擎分组的分析数据                                                                          |

`Dardcor Code_compression_status` 将 MCP 描述压缩数据单独报告在 `analytics.mcpDescriptionCompression` 下。这些值是 MCP 可列表描述（`tools`、`prompts`、`resources` 和 `resourceTemplates`）的元数据大小估算；不是服务商用量记录，标记为 `source: "mcp_metadata_estimate"`。

### MCP 无障碍树过滤器（v3.8.0）

除了上述 5 个压缩工具，Dardcor Code 还包含一个执行后过滤器，在 MCP 浏览器/无障碍工具的**工具结果**返回给代理之前对其进行压缩。此过滤器本身不是工具——它对任何包含冗长的无障碍树或浏览器快照文本（≥2000 字符）的工具结果透明运行。
...
| 工具                          | 权限域           | 描述                                                                               |
| :---------------------------- | :--------------- | :--------------------------------------------------------------------------------- |
| `Dardcor Code_oneproxy_fetch`    | `read:proxies`   | 从 1proxy 市场获取免费代理（支持协议/国家/质量/数量筛选）                          |
| `Dardcor Code_oneproxy_rotate`   | `read:proxies`   | 按策略获取下一个可用代理（`random` / `quality` / `sequential`）                    |
| `Dardcor Code_oneproxy_stats`    | `read:proxies`   | 连接池统计、同步状态、按协议和国家分布                                             |
...
| 工具                        | 权限域           | 描述                                                                       |
| :-------------------------- | :--------------- | :------------------------------------------------------------------------- |
| `Dardcor Code_memory_search`   | `read:memory`    | 按查询/类型/API Key 搜索记忆，强制 Token 预算                              |
| `Dardcor Code_memory_add`      | `write:memory`   | 添加新的记忆条目（`factual` / `episodic` / `procedural` / `semantic`）     |
| `Dardcor Code_memory_clear`    | `write:memory`   | 清除某 API Key 的记忆，可选按类型或 `olderThan` 时间戳过滤                 |
...
| 工具                            | 权限域             | 描述                                                                     |
| :------------------------------ | :----------------- | :----------------------------------------------------------------------- |
| `Dardcor Code_skills_list`         | `read:skills`      | 列出已注册的技能，支持按 API Key、名称或启用状态过滤                     |
| `Dardcor Code_skills_enable`       | `write:skills`     | 按 ID 启用或禁用某个技能                                                |
| `Dardcor Code_skills_execute`      | `execute:skills`   | 以给定输入执行技能，返回执行记录                                         |
| `Dardcor Code_skills_executions`   | `read:skills`      | 列出近期的技能执行历史                                                   |
...
| 工具                                | 权限域           | 描述                                                                                                      |
| :---------------------------------- | :--------------- | :-------------------------------------------------------------------------------------------------------- |
| `Dardcor Code_agent_skills_list`       | `read:catalog`   | 列出全部 42 个代理技能，支持可选的 `category`（api\|cli）和 `area` 过滤；返回元数据 + 覆盖情况             |
| `Dardcor Code_agent_skills_get`        | `read:catalog`   | 按规范 `id` 获取单个技能的完整元数据 + SKILL.md 内容                                                      |
| `Dardcor Code_agent_skills_coverage`   | `read:catalog`   | 覆盖统计：22 个 API 技能和 20 个 CLI 技能中，哪些在文件系统上有 SKILL.md 文件 vs 目录总计                 |
...
云代理是外部 AI 编程代理（codex-cloud、devin、jules），通过与大语言模型服务商相同的连接模型接入 Dardcor Code。它们通过自己的 REST 接口（`/api/v1/agents/*`）暴露，**不**属于 MCP 工具目录——调用云代理不消耗 MCP 权限域。
...
| 变量                                      | 默认值                              | 用途                                                                                                                       |
| :---------------------------------------- | :---------------------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_BASE_URL`                      | `http://localhost:20128`            | MCP 服务器调用 Dardcor Code 内部 API 时使用的基础 URL                                                                          |
...
| `DATA_DIR`                                | `~/.Dardcor Code`                      | 心跳文件写入 `${DATA_DIR}/runtime/mcp-heartbeat.json`                                                                       |
...
- 实时统计通过 `Dardcor Code_compression_status` 在 `analytics.mcpDescriptionCompression` 下呈现，标记为 `source: "mcp_metadata_estimate"` 以区别于实际服务商用量记录。
