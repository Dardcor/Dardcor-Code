...
<img src="../../screenshots/MainDardcor Code.png" alt="Dardcor Code 控制台" width="820"/>
...
**约 1.6B 可统计免费 Token / 月** — 计入注册奖励后，首月最高可达 **~2.1B** — 聚合各家免费层配额，外加一众永久免费、不限量的服务商；再叠加上述压缩引擎，每一枚 Token 都物超所值。([统计方法 →](../../reference/FREE_TIERS.md#tldr--how-much-free-inference-does-Dardcor Code-actually-aggregate))
...
**疑难解答、服务商攻略、路线图与支持 → [Discord](https://discord.gg/U47eFqAXCn) · [Telegram](https://t.me/Dardcor CodeOficial) · WhatsApp [🌍 全球](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 巴西](https://chat.whatsapp.com/LTSpdFhXTxjH4R6CCNiKWz)**
...
[![npm](https://img.shields.io/npm/v/Dardcor Code?logo=npm&style=flat-square)](https://www.npmjs.com/package/Dardcor Code)
...
[**🚀 快速开始**](#-quick-start) • [**🎯 Combo**](#-combos--the-flagship) • [**🌐 服务商**](#-231-ai-providers--50-free) • [**🔌 CLI 与 MCP**](#-full-cli--a2a--mcp) • [**🗜️ 压缩**](#%EF%B8%8F-save-1595-tokens--automatically) • [**🌍 官网**](https://Dardcor Code.online)

[💥 我们的承诺](#-the-promise) • [🤔 为什么选择 Dardcor Code](#-why-Dardcor Code) • [🏆 核心优势](#-what-sets-Dardcor Code-apart) • [🤖 兼容的编程工具](#-compatible-clis--coding-agents) • [🖥️ 运行平台](#%EF%B8%8F-where-Dardcor Code-runs--anywhere) • [🔒 隐私优先](#-private--local-first) • [🎬 实机演示](#-Dardcor Code-in-action) • [📚 探索更多](#-explore-more) • [📧 支持](#-support--community)
...
> 手动凑各家免费额度有多痛苦 — 数十套 SDK、数十个速率限制，根本搞不清到底还剩多少。Dardcor Code 将 **40+ 服务商池 / 500+ 模型**的**可核实**免费层聚合为一个真实的统一数字，并在控制台实时展示 (`/dashboard/free-tiers`)。
>
...
> 一个端点。**236 家服务商。** 编码不止步 — 让 Dardcor Code 帮你选出最便宜且可用的那个。
...
> **Combo** 是 Dardcor Code **自动**路由的模型接力链路。配额耗尽、服务商宕机或成本飙升 — Combo 自动滑向下一个模型，无声无息。**正是它让 Dardcor Code 坚不可摧。** 🛡️

### ⚡ 零配置 — 只需设为 `auto`

无需预先配置 Combo。将模型 ID 设为 `auto`（或其变体），Dardcor Code 会基于你已连接的服务商实时评分，自动构建虚拟 Combo：
...
- **🤖 一键 CLI/Agent 配置** — 专用 `setup-*` 命令为各编程工具一键配置 Dardcor Code 路由（Claude Code、Codex、Cline、Continue、Cursor、Roo Code、Kilo Code、Crush、Goose、Qwen Code、Aider、OpenCode）；`Dardcor Code launch` / `Dardcor Code launch-codex` 为零配置启动器。→ [CLI 集成](../../guides/CLI-INTEGRATIONS.md)
- **🛰️ 远程模式** — 通过授权范围 Token 从任意机器操控远程 Dardcor Code（`Dardcor Code connect` / `Dardcor Code contexts` / `Dardcor Code tokens`）；另附 `Dardcor Code login antigravity` 辅助命令，在你的本机运行 Google "native/desktop" OAuth 后将凭证 blob 粘贴至远程/VPS 安装实例（因远程环境无法接收 loopback 回调）。→ [远程模式](../../guides/REMOTE-MODE.md)
...
- **🗜️ 可插拔压缩体系** — **9 大可组合引擎**的异步流水线，含 Compression Studios、LLMLingua-2 ONNX 引擎和启发式/SLM 双层 **Ultra**、RTK、委托式 Anthropic 上下文编辑、**输出风格**（输出轴调控：简洁文章 / 少代码 / 简洁文言）、**自适应上下文预算旋钮**（仅推进到刚好适应上下文窗口的程度）、每请求 `x-Dardcor Code-compression` 控制、可选离线评估套件、控制台一键 **Headroom** 代理生命周期管理（支持 Docker 边车）、合成**压缩演练场**（Play 通道 + A/B 对比，附 USD 上限保真度判定）、可选**每步保真度门控**（在有损引擎降低 Prompt 质量前将其拦截）、**Best-of-N 候选编码器**（GCF vs TOON — 取更短者，Studio 中附 A/B 字节/Token 对照表）、**CCR 范围/grep/统计检索**（直接拉取储存块的精确字节/行切片或摘要而无需全量展开），以及统一面板含命名配置文件 + 活动配置文件选择器。→ [压缩](../../compression/COMPRESSION_ENGINES.md)
- **🕵️ 透明 MITM 解密（TPROXY）** — 捕获并翻译忽略代理环境变量的 CLI 流量，含每 SNI 证书颁发机构和信任存储安装器。→ [MITM/TPROXY](../../security/MITM-TPROXY-DECRYPT.md)
- **💸 全方位成本遥测** — 每个端点上的 `X-Dardcor Code-*` 成本/用量响应头（含媒体端点）、非 Token 成本引擎、缓存命中 `X-Dardcor Code-Cost-Saved` 响应头，以及每密钥美元消费配额。→ [API 参考](../../reference/API_REFERENCE.md)
- **🧠 完全可控的记忆系统** — 可选 int8 向量量化（Qdrant + sqlite-vec）、默认关闭记忆、每请求 `x-Dardcor Code-no-memory` 响应头。→ [记忆系统](../../frameworks/MEMORY.md)
...
- **⚡ 本地性能与基础设施** — 一键本地 Redis 启动器（`Dardcor Code redis up`，含控制台 Redis 面板）、一键 **Cloudflare Workers** 和 **Deno Deploy** 中继部署器（接入代理池），以及可选 Bifrost Go 边车将最热中继路径卸载至 Go 侧（`BIFROST_BASE_URL`，超时自动回退 TypeScript 路径）— 现支持中继后端选择器（`OMNIROUTE_RELAY_BACKEND=ts|bifrost|auto`），`/v1/relay` 端点保持对外稳定接口的同时内部自动择取最快后端。→ [环境配置](../../reference/ENVIRONMENT.md)
...
<sub>📖 16+ 款工具的逐项配置指南 → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · 🧩 OpenCode 插件 → [`@Dardcor Code/opencode-provider`](https://www.npmjs.com/package/@Dardcor Code/opencode-provider)</sub>
...
| 平台 | 安装方式 | 亮点 |
|---|---|---|
| 📦 **npm（全局）** | `npm install -g Dardcor Code` | 一行命令，任意 OS |
| 🐳 **Docker** | `docker run … diegosouzapw/Dardcor Code` | 多架构 **AMD64 + ARM64** |
| 🖥️ **桌面（Electron）** | `npm run electron:build` | 原生窗口 + 系统托盘 — **Windows / macOS / Linux** |
| 💪 **ARM** | 原生 `arm64` | 树莓派、ARM 服务器、Apple Silicon |
| 📱 **Android（Termux）** | `pkg install nodejs && npx -y Dardcor Code` | **在手机上** 7×24 运行，无需 Root |
| 📲 **PWA** | "添加到主屏幕" | 全屏、离线、可从浏览器安装 |
| 🧩 **OpenCode 插件** | `@Dardcor Code/opencode-provider` | 原生 OpenCode 集成 |
...
> 你的密钥、你的机器、你的数据。Dardcor Code 是**本地代理** — 绝不会向外回传。

- 🏠 **100% 运行在本地硬件上** — npm、Docker、桌面端或你的手机。请求链路中不存在任何 Dardcor Code 云端节点。
...
> Dardcor Code 不只是一台服务器 — 它是拥有 **60+ 命令**的**全功能命令行驾驶舱**，外加开放的代理协议，让 AI 代理**自主**操控 Dardcor Code。

### ⌨️ 真正的 CLI（不止 `start`）

```bash
Dardcor Code               # 启动网关 + 控制台（端口 20128）
Dardcor Code chat          # 交互式 TUI 聊天客户端（斜杠命令：/model /combo /skill /memory）
Dardcor Code setup         # 引导式首次设置向导
Dardcor Code doctor        # 诊断服务商、端口、原生依赖
```

### 🛰️ 远程模式 — CLI 在本地，Dardcor Code 在远端的 VPS

Dardcor Code 跑在服务器上？用**同一套 CLI** 从笔记本远程操控。登录一次，绑定授权范围 Token；后续所有命令自动指向远端。

```bash
Dardcor Code connect 192.168.0.15            # 密码 → 范围 Token，保存为上下文
Dardcor Code models list                     # ← 在远端服务器上执行
Dardcor Code configure codex                 # ← 选择远端模型，写入本地 Codex 配置文件
Dardcor Code tokens create --name ci --scope read   # 为其他机器签发更窄范围的 Token
Dardcor Code contexts use default            # ← 切回本机服务器
```
...
### 🤝 接入 AI 代理 — 让代理自主操控 Dardcor Code

通过 **MCP** 或 **A2A** 协议暴露 Dardcor Code，任何智能代理都能获得网关的完整控制权 — 路由、服务商、Combo、缓存、压缩、记忆 — 全自主运行。

| 协议 | 端点 | 用途 |
|---|---|---|
| 🧰 **MCP（stdio）** | `Dardcor Code --mcp` | 接入 Claude Desktop、Cursor 等各种 MCP 客户端 |
...
```bash
# 通过 MCP 将 Dardcor Code 完整工具集赋予 Claude Code：
...
> **Token 够用就好，何必铺张浪费？** 每个请求**透明地**通过 Dardcor Code 压缩流水线 — 客户端无需任何改动。现已升级为 **9 大可组合引擎**的级联体系，按 Combo 自由排列组合 — 凝聚了 [RTK](https://github.com/rtk-ai/rtk)、[Caveman](https://github.com/JuliusBrussee/caveman)（⭐ 51K+）、[LLMLingua-2](https://github.com/microsoft/LLMLingua) 和 [Troglodita](https://github.com/leninejunior/troglodita)（PT-BR）的技术精华。
...
```
Client (10,000 tok) ──▶ Dardcor Code Compression (9 engines) ──▶ Provider (~1,080 tok, 节省高达 95%)
```
...
- **🎛️ 压缩决策的优先链路** _（从高到低）_ — 逐请求 `x-Dardcor Code-compression` 头 › Combo 覆写 › 活动命名配置 › 自适应/自动触发 › 面板默认 › 关闭。最终采用的压缩方案会通过 `X-Dardcor Code-Compression: <mode>; source=<source>` 响应头回显。
...
你应该能看到已连接模型的列表。🎉 至此大功告成 — 开始编码，Dardcor Code 自动路由、自动容灾。

如果你的客户端无法发送自定义请求头，Dardcor Code 也提供 Token 化兼容别名：
...
> 🎬 **制作了关于 Dardcor Code 的视频？** 通过链接创建 [issue](https://github.com/diegosouzapw/Dardcor Code/issues/new) 或 [discussion](https://github.com/diegosouzapw/Dardcor Code/discussions) — 我们将在本节予以展示。
...
> 💡 控制台上的"费用"是**节省追踪器**，而非账单 — Dardcor Code 从不向你收费。显示"$290 总费用"意味着你使用免费模型**省下了 $290**。
...
🇷🇺 🇨🇳 🇮🇷 🇨🇺 🇹🇷 身处受限地区？Dardcor Code 的**三级代理体系**（全局 / 按服务商 / 按连接）代理 API 请求、OAuth 流程、连通性测试、Token 刷新和模型同步。
...
| `DATA_DIR` | `~/.Dardcor Code` | 数据库与配置存储路径 |

**Dardcor Code 会向我收费吗？** 不会 — 它是运行在你本机的免费开源软件。你只直接向付费服务商付款。Dardcor Code 不含任何计费系统。
...
- 🌍 **官网**：[Dardcor Code.online](https://Dardcor Code.online)
- 🐙 **GitHub**：[github.com/diegosouzapw/Dardcor Code](https://github.com/diegosouzapw/Dardcor Code)
- 🐛 **Issues**：[报告 Bug](https://github.com/diegosouzapw/Dardcor Code/issues)（请附上 `npm run system-info` 的输出结果）
...
- **官网**：[Dardcor Code.online](https://Dardcor Code.online)
- **npm 包**：[npmjs.com/package/Dardcor Code](https://www.npmjs.com/package/Dardcor Code)
...
| [远程模式](../../guides/REMOTE-MODE.md) | 通过授权范围 Token 从笔记本 CLI 操控远端 Dardcor Code（VPS） |
| [Claude Code 配置](../../guides/CLAUDE-CODE-CONFIGURATION.md) | 使用 `launch` + 按模型配置文件将 Claude Code 指向 Dardcor Code（本地/远程） |
...
| [Termux 指南](../../guides/TERMUX_GUIDE.md) | 通过 Termux 在 Android 上运行 Dardcor Code |
...
> Dardcor Code 由充满热情的开源社区共同塑造。以下同仁做出了卓越贡献，直接影响着项目的质量、稳定性与影响力。**衷心感谢。**
...
> 🙏 这些贡献者的功能、Bug 修复和基础设施改进，是 Dardcor Code 可靠且功能丰富的**核心支柱**。每一个 Pull Request、每一个测试用例、每一个 i18n 翻译文件都意义重大。开源正是由他们这样的人建造的。
...
Dardcor Code 是站在巨人肩膀上的作品。它始于 **[9router](https://github.com/decolua/9router)** 的一个 Fork 以及 Go 项目 **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** 的 TypeScript 移植 — 自此，以下每个子系统均受惠于先行者的开源成果。每一个项目都在 Dardcor Code 中留下了具体印记。这是我们对所有项目的由衷感谢。🙏

> ⭐ 星标数为 2026 年 6 月数据 — 请给这些项目点颗星。

### 🧬 渊源与网关

| 项目 | ⭐ | 对 Dardcor Code 的启发 |
|---|---|---|
...
Dardcor Code 是免费开源项目，在公开环境中持续构建与维护。如果它帮你节省了时间或金钱，请考虑以以下方式支持开发：

- ⭐ **为本仓库加颗 Star** — 这确确实实能帮我们提升可见度
- 💖 **[GitHub Sponsors](https://github.com/sponsors/diegosouzapw)** — 资助持续维护和新服务商接入
- 🐛 **在 [Discussions](https://github.com/diegosouzapw/Dardcor Code/discussions) 中反馈 Bug 和分享意见**
...
**[⬆ 返回顶部](#-Dardcor Code)** · 用 ❤️ 为开源 AI 社区构建。

<sub>Dardcor Code v3.8.24 · Node ≥22.0.0 · MIT License · <a href="https://Dardcor Code.online">Dardcor Code.online</a></sub>
