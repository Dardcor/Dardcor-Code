...
**Dardcor Code** — verenigde AI proxy/router. Eén eindpunt, 160+ LLM-providers, automatische fallback.
...
Dardcor Code heeft drie gerelateerde maar verschillende mechanismen voor tijdelijke fouten. Houd hun
...
- **Imports**: extern → intern (`@/`, `@Dardcor Code/open-sse`) → relatief
...
- **Padaliassen**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Standaardpoort**: 20128 (API + dashboard op dezelfde poort)
- **Gegevensdirectory**: `DATA_DIR` omgevingsvariabele, standaard `~/.Dardcor Code/`
...
16. Neem nooit `Co-Authored-By`-trailers op die een AI-assistent, LLM of automatiseringsaccount crediteren (bijv. namen met "Claude", "GPT", "Copilot", "Bot"; e-mails op `anthropic.com` / `openai.com` / `noreply.github.com`-adressen die eigendom zijn van bots). Dergelijke trailers leiden commit-attributie naar het botaccount op GitHub, waardoor de werkelijke auteur (`diegosouzapw`) in de PR-geschiedenis verborgen blijft. Menselijke medewerkers — inclusief upstream PR-auteurs en issue-rapporteurs die naar Dardcor Code worden geport — MOGEN en MOETEN worden gecrediteerd met standaard `Co-authored-by: Name <email>`-trailers; de upstream-port workflows (`/port-upstream-features`, `/port-upstream-issues`) zijn hiervan afhankelijk.
