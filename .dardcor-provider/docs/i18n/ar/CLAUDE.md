...
**Dardcor Code** — وكيل/موجه AI موحد. نقطة نهاية واحدة، أكثر من 160 مزود LLM، تراجع تلقائي.
...
يمتلك Dardcor Code ثلاث آليات فشل مؤقتة مرتبطة ولكن متميزة. حافظ على نطاقها منفصلًا عند تصحيح سلوك التوجيه. راجع
...
- **الاستيرادات**: خارجي → داخلي (`@/`, `@Dardcor Code/open-sse`) → نسبي
...
- **أسماء المسارات**: `@/*` → `src/`، `@Dardcor Code/open-sse` → `open-sse/`، `@Dardcor Code/open-sse/*` → `open-sse/*`
- **المنفذ الافتراضي**: 20128 (API + لوحة التحكم على نفس المنفذ)
- **دليل البيانات**: متغير البيئة `DATA_DIR`، الافتراضي هو `~/.Dardcor Code/`
...
16. لا تضمن أبدًا ملحقات `Co-Authored-By` التي تنسب لمساعد ذكاء اصطناعي أو LLM أو حساب آلي (مثل الأسماء التي تحتوي على "Claude" أو "GPT" أو "Copilot" أو "Bot"؛ والبريد الإلكتروني على `anthropic.com` / `openai.com` / عناوين `noreply.github.com` المملوكة للبوتات). تلك الملحقات توجه نسبة الالتزامات إلى حساب البوت على GitHub، مما يخفي المؤلف الحقيقي (`diegosouzapw`) في تاريخ PR. المساهمون البشريون — بما في ذلك مؤلفو PRs upstream ومُبلغو الـ issues الذين يتم نقلهم إلى Dardcor Code — يجوز ويجب أن يُنسبوا باستخدام ملحقات `Co-authored-by: Name <email>` القياسية؛ تعتمد سير عمل النقل (`/port-upstream-features` و `/port-upstream-issues`) على ذلك.
