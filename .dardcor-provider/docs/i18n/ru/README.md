# 🚀 Dardcor Code — Бесплатный AI-шлюз (Русский)
...
<img src="../../screenshots/MainDardcor Code.png" alt="Панель Dardcor Code" width="820"/>

<br/>

# 🚀 Dardcor Code — Бесплатный AI-шлюз
...
**Вопросы, советы по провайдерам, roadmap и поддержка → [Discord](https://discord.gg/U47eFqAXCn) · [Telegram](https://t.me/Dardcor CodeOficial) · WhatsApp [🌍 Global](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 Brasil](https://chat.whatsapp.com/LTSpdFhXTxjH4R6CCNiKWz)**
...
[**🚀 Быстрый старт**](#-быстрый-старт) • [**🎯 Комбо**](#-комбо--главная-фича) • [**🌐 Провайдеры**](#-278-ai-провайдеров--90-бесплатных) • [**🔌 CLI и MCP**](#-полный-cli--a2a-и-mcp) • [**🗜️ Сжатие**](#️-экономьте-1595-токенов--автоматически) • [**🌍 Сайт**](https://Dardcor Code.online)

[💥 Обещание](#-обещание) • [🤔 Зачем](#-зачем-Dardcor Code) • [🏆 Чем отличается](#-чем-Dardcor Code-отличается) • [🤖 Совместимые CLI](#-совместимые-cli-и-агенты) • [🖥️ Где запускать](#️-где-запускается-Dardcor Code--везде) • [🔒 Приватность](#-приватно-и-local-first) • [🎬 В деле](#-Dardcor Code-в-деле) • [📚 Дальше](#-узнать-больше) • [📧 Поддержка](#-поддержка-и-сообщество)
...
> Собирать free-tier вручную — боль: десятки SDK, лимиты и непонятный остаток. Dardcor Code сводит **документированные** free-tier **43 пулов / 460+ моделей** в одно честное число и показывает его live на `/dashboard/free-tiers`.
>
...
> Один endpoint. **278 провайдеров.** Код не останавливается — Dardcor Code сам выбирает самый дешёвый рабочий вариант.
...
| ❌ Боль каждый день | ✅ Как решает Dardcor Code |
|---|---|
...
> **Combo** — цепочка моделей, по которой Dardcor Code ходит **сам**. Квота кончилась, провайдер упал, цена взлетела — комбо тихо уходит на следующий шаг. **Именно это делает Dardcor Code «неубиваемым».** 🛡️

### ⚡ Без настройки — просто `auto`

Комбо создавать не обязательно. Поставьте модель `auto` (или вариант) — Dardcor Code соберёт виртуальное комбо из подключённых провайдеров:
...
# 🏆 Чем Dardcor Code отличается

</div>

| Фича | Dardcor Code | Другие роутеры |
|---|---|---|
...
- **🤖 One-command CLI setup** — `setup-*` для 12+ tools; `Dardcor Code launch` / `launch-codex`.
...
- **💸 Cost telemetry** — `X-Dardcor Code-*` headers, cache-HIT savings, per-key USD quotas.
- **🧠 Memory** — opt-in, int8 quantization, `x-Dardcor Code-no-memory`.
...
<sub>📖 Setup 33 tools → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · OpenCode plugin → [`@Dardcor Code/opencode-provider`](https://www.npmjs.com/package/@Dardcor Code/opencode-provider)</sub>
...
# 🖥️ Где запускается Dardcor Code — везде

</div>

| Платформа | Установка | Плюсы |
|---|---|---|
| 📦 **npm (global)** | `npm install -g Dardcor Code` | Одна команда, любая ОС |
| 🐳 **Docker** | `docker run … diegosouzapw/Dardcor Code` | **AMD64 + ARM64** |
| 🖥️ **Desktop (Electron)** | `npm run electron:build` | Окно + tray — Win/macOS/Linux |
| 💪 **ARM** | native `arm64` | Pi, ARM servers, Apple Silicon |
| 📱 **Android (Termux)** | `pkg install nodejs && npx -y Dardcor Code` | На телефоне 24/7, без root |
| 📲 **PWA** | «Add to Home Screen» | Fullscreen, offline |
| 🧩 **OpenCode plugin** | `@Dardcor Code/opencode-provider` | Нативная интеграция |
...
> Ваши ключи, ваша машина, ваши данные. Dardcor Code — **локальный прокси**, без «звонков домой».

- 🏠 **100% на вашем железе** — npm, Docker, desktop или телефон. Нет cloud-hop Dardcor Code.
...
> Это не «просто сервер» — **CLI-кокит** с **80+ командами** и открытыми agent-протоколами, чтобы AI управлял Dardcor Code **сам**.

### ⌨️ Настоящий CLI

```bash
Dardcor Code               # gateway + dashboard (порт 20128)
Dardcor Code chat          # TUI-чат (/model /combo /skill /memory)
Dardcor Code setup         # мастер первого запуска
Dardcor Code doctor        # диагностика провайдеров, портов, native deps
```

### 🛰️ Remote mode — CLI здесь, Dardcor Code на VPS

```bash
Dardcor Code connect 192.168.0.15            # пароль → scoped token
Dardcor Code models list                     # ← на REMOTE
Dardcor Code configure codex                 # remote model → local Codex profile
Dardcor Code tokens create --name ci --scope read
Dardcor Code contexts use default            # ← обратно на local
```
...
|---|---|---|
| 🧰 **MCP (stdio)** | `Dardcor Code --mcp` | Claude Desktop, Cursor, любой MCP client |
...
Precedence (high → low): header `x-Dardcor Code-compression` › combo override › named profile › adaptive › panel default › off.
...
Должны появиться подключённые модели. 🎉 Дальше пишите код — Dardcor Code сам роутит и делает fallback.
...
| Команда | Описание |
|---|---|
| `Dardcor Code` | Сервер (`PORT=20128`, API + dashboard) |
| `Dardcor Code --port 3000` | Порт 3000 |
| `Dardcor Code --mcp` | MCP server (stdio) |
| `Dardcor Code --no-open` | Не открывать браузер |
| `Dardcor Code --help` | Справка |
...
| Команда | Действие |
|---|---|
| `npm run uninstall` | Убирает app, **сохраняет** `~/.Dardcor Code` |
| `npm run uninstall:full` | Удаляет app **и** все ключи/БД |
| `npm uninstall -g Dardcor Code` | Глобальный npm uninstall |
...
> 🎬 **Сняли видео про Dardcor Code?** Откройте [issue](https://github.com/diegosouzapw/Dardcor Code/issues/new) или [discussion](https://github.com/diegosouzapw/Dardcor Code/discussions) — добавим в этот раздел.
...
> 💡 «Cost» в дашборде — **tracker экономии**, не счёт Dardcor Code. Dardcor Code вам **не** выставляет счета.
...
| `DATA_DIR` | `~/.Dardcor Code` | БД и конфиги |
| `REQUEST_TIMEOUT_MS` | `600000` | Базовый timeout |
| `STREAM_IDLE_TIMEOUT_MS` | inherits | Idle gap SSE |

**Dardcor Code берёт деньги?** Нет — open-source на вашей машине. Платите только платным провайдерам.  
...
- 🌍 **Сайт:** [Dardcor Code.online](https://Dardcor Code.online)
- 🐙 **GitHub:** [github.com/diegosouzapw/Dardcor Code](https://github.com/diegosouzapw/Dardcor Code)
- 🐛 **Issues:** [сообщить о баге](https://github.com/diegosouzapw/Dardcor Code/issues) (с `npm run system-info`)
...
| [Remote Mode](../../guides/REMOTE-MODE.md) | CLI с ноутбука → Dardcor Code на VPS |
...
**Сделано с ❤️ open-source сообществом · MIT License · [Dardcor Code.online](https://Dardcor Code.online)**

⭐ Если Dardcor Code помог сэкономить — поставьте star репозиторию.
