...
> Complete reference for every environment variable recognized by Dardcor Code.
...
Dardcor Code uses **SQLite** (via `better-sqlite3`) for all persistence. These variables control data location, encryption, and lifecycle.

| Variable                         | Default              | Source File                                     | Description                                                                                                        |
| -------------------------------- | -------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `DATA_DIR`                       | `~/.Dardcor Code/`      | `src/lib/db/core.ts`                            | Root directory for SQLite DB, backups, and data files. Override for Docker volumes or custom paths.                |
...
| --------------------- | -------------------------------------------------------------------------------- |
| **Local development** | Leave all defaults. DB lives at `~/.Dardcor Code/Dardcor Code.db`.                     |
...
| **CI/Testing**        | `DATA_DIR=/tmp/Dardcor Code-test` — ephemeral, no encryption needed.                |
...
Dardcor Code provides a two-layer defense: request-side injection scanning and response-side PII stripping.
...
> [!IMPORTANT]
> When deploying behind a reverse proxy (nginx, Caddy), `NEXT_PUBLIC_BASE_URL` **must** be set to your public URL (e.g., `https://Dardcor Code.example.com`). Without this, OAuth callbacks will fail because the redirect_uri won't match.
...
Controls how Dardcor Code discovers and launches CLI sidecars (Claude Code, Codex, etc.).
...
| `CLI_ALLOW_CONFIG_WRITES` | `false`    | `src/shared/services/cliRuntime.ts` | Allow Dardcor Code to write CLI config files (token refresh, session data).   |
...
```bash
# Mount host binaries into the container and tell Dardcor Code where they are:
...
| Variable                                | Default     | Source File                                 | Description                                                                                                                   |
| --------------------------------------- | ----------- | ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_BASE_URL`                    | auto-detect | `open-sse/mcp-server/server.ts`             | Explicit URL for MCP/A2A tools to reach Dardcor Code. Overrides localhost auto-detection.                                        |
...
| Variable            | Default     | Source File                     | Description                               |
| ------------------- | ----------- | ------------------------------- | ----------------------------------------- |
| `OMNIROUTE_SERVER`  | auto-detect | `src/lib/oauth/config/index.ts` | Server URL for CLI↔Dardcor Code auth bridge. |
...
When enabled, Dardcor Code reorders HTTP headers and JSON body fields to match the exact signature of official CLI tools. This reduces the risk of account flagging while preserving your proxy IP.
...
| Variable                   | Default            | Description                                                                                          |
| -------------------------- | ------------------ | ---------------------------------------------------------------------------------------------------- |
| `OMNIROUTE_MEMORY_MB`      | `512`              | Runtime V8 heap limit. Docker standalone and `Dardcor Code serve` use it to set `--max-old-space-size`. |
...
| `INSTANCE_NAME=Dardcor Code`                             | Present in old docs/env templates but unused at runtime. May return in a future multi-instance feature. |
