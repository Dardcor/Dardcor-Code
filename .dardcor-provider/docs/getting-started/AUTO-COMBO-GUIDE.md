---
title: "Auto-Combo: Let Dardcor Code Pick the Best AI for You"
version: 3.8.50
lastUpdated: 2026-08-06
---

# Auto-Combo: Let Dardcor Code Pick the Best AI for You

> **TL;DR**: Set your model to `auto` and Dardcor Code automatically picks the best AI provider for each request. No configuration needed.

---

## What It Does

Instead of choosing a specific AI model (like GPT-4o or Claude), you can let Dardcor Code **automatically pick the best one** for each request. It considers:
...
Dardcor Code scores all your connected providers and picks the best one. If it fails, it automatically tries the next one.
...
**Step 2**: That's it! Dardcor Code handles the rest.
...
When you send a request with `model: "auto"`, Dardcor Code:
...
4. **Auto-recovers** — If it fails, Dardcor Code tries the next one automatically
...
Dardcor Code has **three layers of protection**:

### 1. Auto-Fallback
If the best provider fails, Dardcor Code automatically tries the next one. You don't need to do anything.
...
If all providers fail, Dardcor Code routes to stable free providers (like Kiro or Qoder) as a last resort.

---

## Multi-Account Support

If you have multiple accounts for the same provider (e.g., two OpenAI keys), Dardcor Code treats each as a **separate candidate**. This means:
...
Dardcor Code occasionally **explores** new providers to discover better options:
...
This helps Dardcor Code learn which providers work best for your usage patterns.
...
Dardcor Code automatically skips it and tries the next one. If a provider keeps failing, it's excluded temporarily (5-30 minutes). You don't need to do anything.

### "Can I see which provider was used?"

Check the response headers — Dardcor Code includes the provider and model used in each response.
