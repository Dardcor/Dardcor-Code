...
Web Cookie providers let Dardcor Code use an AI service through your existing browser session instead of an API key. They are useful when you already have access to a service through its website and want Dardcor Code to use the same authenticated session.
...
For most Web Cookie providers, this is the value that should be pasted into Dardcor Code.
...
## Cookie Works in Browser but Not in Dardcor Code
