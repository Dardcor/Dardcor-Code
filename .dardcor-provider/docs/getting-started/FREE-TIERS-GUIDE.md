...
> **TL;DR**: Dardcor Code aggregates free tiers from 50+ providers. Connect multiple free providers for unlimited free AI with automatic fallback.
...
Dardcor Code **aggregates** these free tiers into one endpoint. Instead of signing up for 10 different services, you connect them all to Dardcor Code and use `model: "auto"` to automatically pick the best free option for each request.
...
The magic of Dardcor Code is **stacking free tiers**. Instead of relying on one provider, you connect multiple free providers and let Dardcor Code automatically pick the best one for each request.
...
Then use `model: "auto"` and Dardcor Code will:
...
## How Dardcor Code Makes Free Tiers Better

### 1. Automatic Fallback

If one free provider is busy or down, Dardcor Code automatically tries the next one. You don't need to do anything.

### 2. Smart Routing

Dardcor Code picks the **best free provider** for each request based on:
...
Dardcor Code's **compression** feature saves 15-95% of tokens. This means your free quota lasts **5-20x longer**.

### 4. Multi-Account Support

If you have multiple accounts for the same provider, Dardcor Code treats each as a separate candidate. This doubles or triples your free quota.
...
**Yes!** These are official free tiers from the providers. Dardcor Code just makes it easier to use them all at once.
...
No catch! Providers offer free tiers to attract users. Dardcor Code just makes it easier to use them all at once.
