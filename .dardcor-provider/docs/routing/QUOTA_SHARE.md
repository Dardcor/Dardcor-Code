...
**Problem it solves:** Dardcor Code proxies many API keys against the same upstream
...
- All persistence is in the existing Dardcor Code SQLite DB (`DATA_DIR/storage.sqlite`).
...
1. `localStorage.getItem("Dardcor Code:quota-share:pools")` is non-empty.
