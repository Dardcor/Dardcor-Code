...
| RES-003     | `src/lib/issueAgent/execution.ts`                | provider and model are resolved into the chat request; policy is only encoded as `X-Dardcor Code-Mode`                                                | an implementation review must establish that this header is a consumed routing-policy contract     |
