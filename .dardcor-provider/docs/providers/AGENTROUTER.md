...
For most users, **no special setup is required**. Dardcor Code ships a built-in
...
Dardcor Code also supports AgentRouter (and similar relays) through the **Claude Code
...
- Dardcor Code running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting Dardcor Code:
...
**`Invalid JSON response from provider (reset after Ns)` from the Dardcor Code logs** —
...
model actually resolves in the Dardcor Code logs (`ROUTING` tag shows
