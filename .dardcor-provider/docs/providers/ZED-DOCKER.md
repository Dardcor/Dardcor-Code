...
When Dardcor Code runs inside Docker, the standard "Import from Zed Keychain" flow fails
...
Dardcor Code detects the Docker environment via two heuristics:
...
2. The **Manual Token Import** panel appears below the keychain import card. When
   Dardcor Code detects Docker, this panel expands automatically after the first failed
