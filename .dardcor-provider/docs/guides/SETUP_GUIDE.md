---
title: "📖 Setup Guide — Dardcor Code"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 📖 Setup Guide — Dardcor Code

> Complete setup reference for Dardcor Code. For the quick version, see the [Quick Start in README](../README.md#-quick-start).
...
The [AUR package](https://aur.archlinux.org/packages/Dardcor Code-bin) installs Dardcor Code and provides a systemd user service.
...
Dardcor Code ships a desktop wrapper built on Electron 41 + electron-builder 26.10. Available scripts (workspace root):
...
Combined with env vars (`INITIAL_PASSWORD`, `OMNIROUTE_WS_BRIDGE_SECRET`, etc.), this lets you spin up an Dardcor Code instance fully scriptable.
...
| `Dardcor Code setup`       | Guided CLI onboarding for password and first provider          |
| `Dardcor Code doctor`      | Run local health checks without starting the server            |
| `Dardcor Code providers`   | Discover, list, validate, and test providers from CLI          |
| `Dardcor Code config`      | CLI tool configuration — list, get, set, validate configs      |
| `Dardcor Code status`      | Offline status dashboard — version, DB, tools, config          |
| `Dardcor Code logs`        | Stream usage logs from the API (supports `--follow`)           |
| `Dardcor Code update`      | Check for or apply Dardcor Code updates                           |
| `Dardcor Code provider`    | Manage provider connections — add, list, remove, test, default |
...
Instead of pasting the base URL and key by hand, let Dardcor Code write each tool's
...
**remote** Dardcor Code, plus `--dry-run` to preview. The launchers
...
For Claude Code-compatible upstreams (`anthropic-compatible-cc-*`), Dardcor Code derives the outbound `X-Stainless-Timeout` header from the resolved fetch timeout so provider-side read timeouts stay aligned with your env configuration.

For third-party Claude Code-compatible reverse proxies, Dardcor Code keeps the default `anthropic-beta` set conservative and, when `Client Cache Control` is left on `Auto`, only forwards client-provided `cache_control` markers. Enable the per-connection "Enable redact-thinking beta" toggle only when the upstream specifically requires redacted Claude thinking streams.
...
> **Note:** For streaming requests, `FETCH_TIMEOUT_MS` only covers connection setup / waiting for the first upstream response. Once the stream is active, Dardcor Code will only abort on an actual stall (`STREAM_IDLE_TIMEOUT_MS`) or Undici body inactivity (`FETCH_BODY_TIMEOUT_MS`).

### Reverse Proxy Compatibility

If you run Dardcor Code behind Nginx, Caddy, Cloudflare, or another reverse proxy, make sure the proxy timeouts are also higher than your Dardcor Code stream/fetch timeouts.
