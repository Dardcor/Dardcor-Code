---
title: "🐳 Docker Guide — Dardcor Code"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Docker Guide — Dardcor Code
...
Dardcor Code ships four Compose profiles. Pick the one that matches your environment.

| Profile          | Service          | When to use                                                                                                                       | Command                                      |
| ---------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (default) | `Dardcor Code-base` | Headless server / minimal runtime, no provider CLIs bundled                                                                       | `docker compose --profile base up -d`        |
| `cli`            | `Dardcor Code-cli`  | Agentic workflows that call `Dardcor Code providers/setup/doctor` and bundled CLIs (Codex, Claude Code, Droid, OpenClaw)             | `docker compose --profile cli up -d`         |
| `host`           | `Dardcor Code-host` | Linux hosts that want `network_mode`-like access to host CLIs by mounting `~/.local/bin`, `~/.codex`, `~/.claude`, etc. read-only | `docker compose --profile host up -d`        |
...
Dardcor Code relies on Redis to back the distributed rate limiter and shared cache. The `redis` service is **always defined** in `docker-compose.yml` (it has no profile gate) and starts alongside any other profile.
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Host path mounted into `cli` profile at `/workspace/Dardcor Code` for self-update workflows            | `.` (current directory)  |
...
| `OMNIROUTE_BASE_PATH`         | URL subpath when the app is published behind a reverse proxy (e.g. `/Dardcor Code`)                    | _(empty = root)_         |
| `NEXT_PUBLIC_BASE_URL`        | Public browser origin including the subpath (e.g. `https://host/Dardcor Code`)                         | unset                    |
...
Next.js `basePath` is compiled into the standalone bundle. Dardcor Code records the baked
...
Published `diegosouzapw/Dardcor Code:*` images are built for the domain root. You can still
...
prefix). Traefik should route `PathPrefix(`/Dardcor Code`)` to the container without
`StripPrefix`, so Next.js receives `/Dardcor Code/...` and serves assets from
`/Dardcor Code/_next/...`.
...
Caddy sets the standard forwarding headers for the upstream container. Dardcor Code uses
...
want Dardcor Code to derive the public origin from trusted forwarded headers instead of explicit
configuration.
...
| `diegosouzapw/Dardcor Code` | `3.8.0`  | ~250MB | Current version       |
...
- **SQLite WAL Mode:** `docker stop` should be allowed to finish so Dardcor Code can checkpoint the latest changes back into `storage.sqlite`. The bundled Compose files already set a 40s stop grace period. If you run the image directly, keep `--stop-timeout 40`.
