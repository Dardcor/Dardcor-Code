...
This guide covers adding Kiro (AWS-hosted AI coding assistant) accounts to Dardcor Code,
...
## How Dardcor Code Solves This (v3.8.0+)

Starting with v3.8.0, Dardcor Code calls `registerClient()` (AWS SSO OIDC) during every
Kiro connection import. This gives each Dardcor Code connection its own dedicated OIDC
...
2. Choose one of:
   ...
   - **Auto-Import** — click the button; Dardcor Code reads credentials from the
...
4. Dardcor Code validates the key and saves the connection.
...
(`clientSecretExpiresAt`). Dardcor Code stores this timestamp in `providerSpecificData`
...
- Ensure Dardcor Code can reach `https://oidc.us-east-1.amazonaws.com` (or the configured
...
- The key must be able to call `ListAvailableProfiles`; otherwise Dardcor Code cannot
