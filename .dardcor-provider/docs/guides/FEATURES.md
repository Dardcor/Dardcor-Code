---
title: "Dardcor Code — Dashboard Features Gallery"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dardcor Code — Dashboard Features Gallery
...
Visual guide to every section of the Dardcor Code dashboard.
...
- 💻 **CLI enhancement suite** — 20+ commands including `Dardcor Code providers`, `Dardcor Code combos`, `Dardcor Code doctor`, `Dardcor Code setup`
...
OpenRouter connections can store a per-connection `preset` in Advanced Settings. When set, Dardcor Code sends it as the OpenRouter top-level request field, for example `"preset": "email-copywriter"`, unless the client request already supplied its own `preset`.
...
A combo strategy that preserves session continuity when account rotation happens mid-conversation. Before the active account is exhausted, Dardcor Code generates a structured handoff summary in the background. After the next request resolves to a different account, the summary is injected as a system message so the new account continues with full context.
...
Native Electron desktop app for Windows, macOS, and Linux. Run Dardcor Code as a standalone application with system tray integration, offline support, auto-update, and one-click install.
...
Dardcor Code now supports **OpenAI-compatible WebSocket clients** via the `/v1/ws` upgrade endpoint. The custom `scripts/dev/v1-ws-bridge.mjs` server wraps Next.js and upgrades WS connections to full bidirectional streaming sessions. Authentication uses the same API key or session cookie as HTTP requests.
...
**Hybrid token counting** also lands in v3.6.6: when a Claude-compatible provider exposes `/messages/count_tokens`, Dardcor Code calls it before large requests with graceful estimation fallback.
