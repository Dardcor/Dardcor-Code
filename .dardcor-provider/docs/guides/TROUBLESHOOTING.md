...
Common problems and solutions for Dardcor Code.

---

## Quick Reference

**New to Dardcor Code?** Start here — these solve 90% of problems:

| I see this              | What it means                       | What to do                                                                                        |
| ----------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------- |
| "Can't connect"         | Dardcor Code isn't running             | Run `Dardcor Code` or `docker restart Dardcor Code`                                                     |
...
| EACCES: permission denied                                  | Set `DATA_DIR=/path/to/writable/dir` to override `~/.Dardcor Code`                                                                                           |
...
| `dlopen` / `slice is not valid mach-o file` (macOS)        | Run `cd $(npm root -g)/Dardcor Code/app && npm rebuild better-sqlite3 && Dardcor Code` — see [macOS native module rebuild](#macos-native-module-rebuild) below  |
...
HTTP-request-looking links. Dardcor Code's `README.md` ships inside the npm package (it is
listed in `package.json` → `files`), so it lands at `node_modules/Dardcor Code/README.md` on
...
1. **Stop the notifications** — exclude the install directory in your antivirus
   ...
   Dardcor Code data dir (`~/.Dardcor Code/`).
...
`%LOCALAPPDATA%\Programs\Dardcor Code` (including hash-suffixed package directories from the
...
1. **Verify your download first** (rules out a tampered file). Every release publishes
   `latest.yml`, whose `sha512` field (base64) covers the `Dardcor Code.Setup.<version>.exe`
   ...
   re-download only from the [GitHub releases page](https://github.com/diegosouzapw/Dardcor Code/releases).
2. **Restore + exclude** — restore the rolled-back items from quarantine and add an exclusion
   for `%LOCALAPPDATA%\Programs\Dardcor Code` (Kaspersky → Settings → Threats and Exclusions),
   then reinstall.
...
**Cause:** You are running a Node.js version outside Dardcor Code's approved secure runtime floor. The most common case is running an older Node 22 or 24 patch level that falls below the patched security floor Dardcor Code requires.
...
3. Reinstall Dardcor Code: `npm install -g Dardcor Code`
4. Restart: `Dardcor Code`
...
**Fix (v3.5.5+):** Dardcor Code now uses undici's own `fetch()` function when a proxy dispatcher is active, ensuring consistent behavior. Update to v3.5.5+.

### MITM proxy under WSL: desktop apps on the Windows host are not intercepted

**Cause:** The MITM proxy and its CA certificate install into the environment where Dardcor Code runs. Under WSL that environment is the Linux guest, while the AI desktop apps (Kiro, Trae, Copilot, Zed, …) run on the Windows host. The host apps do not trust the guest's certificate store and do not route through the guest's system proxy, so desktop interception does not engage there.

**Recommendation:** Run Dardcor Code natively on the same OS as the desktop apps you want to intercept (Windows for Windows apps; macOS/Linux likewise). Keeping Dardcor Code inside WSL while targeting host apps requires manually trusting the generated CA certificate on the Windows host and pointing each host app's network/proxy settings at the WSL proxy endpoint — an unsupported, fragile setup.
...
Dardcor Code auto-refreshes tokens. If issues persist:
...
2. Verify `CLOUD_URL` points to your cloud endpoint (e.g., `https://Dardcor Code.dev`)
...
When many concurrent requests hit a rate-limited provider, Dardcor Code uses mutex + auto rate-limiting to serialize requests and prevent cascading failures. This is automatic for API key providers.
...
**Cause:** This is deliberate load shedding inside Dardcor Code, not an upstream-provider failure.
...
2. If normal deployment traffic repeatedly exhausts the guard, you can cautiously raise
   ...
   restart Dardcor Code after each change, and observe memory headroom under representative load.
...
Some Dardcor Code users place the gateway in front of RAG or agent stacks. In those setups it is common to see a strange pattern: Dardcor Code looks healthy (providers up, routing profiles ok, no rate limit alerts) but the final answer is still wrong.
...
1. When you investigate a bad response, capture:
   ...
   - route or provider combo in Dardcor Code
   - any RAG context used downstream (retrieved documents, tool calls, etc)
2. Map the incident to one or two WFGY ProblemMap numbers (`No.1` … `No.16`).
3. Store the number in your own dashboard, runbook, or incident tracker next to the Dardcor Code logs.
...
You can ignore this section if you do not run RAG or agent pipelines behind Dardcor Code.
...
2. Restart Dardcor Code so the new env values are picked up
...
3. Restart Dardcor Code and re-test from **Dashboard → CLI Tools**
...
- Run `Dardcor Code providers` from the CLI to re-trigger the OAuth flow, or
