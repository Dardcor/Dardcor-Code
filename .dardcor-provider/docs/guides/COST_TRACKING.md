...
How Dardcor Code estimates, records, and reports the cost of every request — and why the
...
Dardcor Code attributes a per-request USD cost to every completion by multiplying token
counts by a model's pricing rates. These numbers power the **Costs** dashboard, the
`Dardcor Code cost` / `Dardcor Code usage` CLI, CSV/JSON exports, and per-API-key budgets.

> **The dashboard "cost" is a savings tracker, not a bill.** Dardcor Code never charges you
...
- It depends on the pricing table Dardcor Code has for each model. A model with no pricing
...
3. **Hardcoded defaults** — shipped with Dardcor Code.
...
Dardcor Code's CLI exposes cost, usage, and pricing commands (registered in
...
```bash
Dardcor Code cost                          # last 30d, grouped by provider
Dardcor Code cost --period 7d              # last 7 days
Dardcor Code cost --group-by model         # group by provider | model | combo | api-key | day
Dardcor Code cost --since 2026-06-01 --until 2026-06-13
Dardcor Code cost --api-key <key> --limit 50
```
...
```bash
Dardcor Code usage analytics --period 30d [--provider <id>]   # per-provider cost summary
Dardcor Code usage logs [--limit 100] [--follow] [--api-key <k>] [--search <q>]
Dardcor Code usage quota [--provider <id>] [--check]
Dardcor Code usage utilization [--api-key <k>]
Dardcor Code usage history [--limit 100]
Dardcor Code usage proxy-logs [--limit 100]

# Budgets
Dardcor Code usage budget list
Dardcor Code usage budget get [scope]
Dardcor Code usage budget set <amount> [--scope global] [--period monthly]
Dardcor Code usage budget reset [scope]
```

### `Dardcor Code pricing`

```bash
Dardcor Code pricing list [--provider <p>] [--model <m>] [--limit 200]
Dardcor Code pricing get <model>
Dardcor Code pricing sync [--provider <p>] [--force]   # POST /api/pricing/sync
Dardcor Code pricing diff [--model <m>]
Dardcor Code pricing defaults show
Dardcor Code pricing defaults set [--input <p>] [--output <p>] [--cache-read <p>] [--cache-write <p>]
```
...
- **All costs show $0 / "Legacy / Free".** The models in use have no pricing entry.
  Enable external sync (`PRICING_SYNC_ENABLED=true`) and run `Dardcor Code pricing sync`, or
