---
title: "Antigravity (Google One AI) — Onboarding with Dardcor Code"
version: 3.8.50
lastUpdated: 2026-07-31
---

# Dardcor Code Antigravity (Google One AI) Onboarding Guide

> **What you get**: Access to Gemini 3.1 Pro, Gemini 3.5 Flash, Claude Sonnet 4.6, and other models through your Google One AI Pro subscription — routed through Dardcor Code as a unified gateway.
...
| **Model catalog**    | Static curated list (Dardcor Code hardcoded) | Live-probed from Google via `:fetchAvailableModels` |
...
- Quota is **account-level shared** — the same Google account in Antigravity IDE, CLI, and Dardcor Code shares one quota pool
...
**When your account is exhausted**: Dardcor Code automatically retries with the next available account in the combo route. No manual intervention needed.
...
Dardcor Code handles this automatically. When you add a new Google account via Dashboard OAuth:

1. Dardcor Code refreshes the token
...
After `agy login` succeeds, refresh the token in Dardcor Code Dashboard. The projectId will be discovered automatically.
...
```
podman logs Dardcor Code 2>&1 | grep "projectId discovered"
```
...
Google OAuth requires a valid redirect URI. Dardcor Code's default uses `http://127.0.0.1:20128/callback` (loopback). This works for local builds but **fails for remote deployments** (e.g., a server accessed via LAN IP).
...
- Works when you access Dardcor Code from `localhost` or `127.0.0.1`
...
- Run `agy login` on the machine that will access Dardcor Code
- The OAuth flow completes locally, tokens are stored
- Import the connection into Dardcor Code via Dashboard
...
Dardcor Code handles projectId discovery and onboarding automatically for most accounts. When it fails, the root cause is usually one of these:
...
**Fix**: Run `agy login` manually to trigger Cloud Code registration, then refresh the token in Dardcor Code Dashboard.
...
| Check logs            | `podman logs Dardcor Code 2>&1 \| grep projectId`                                          |
| Refresh token         | Dashboard → Providers → agy → Click refresh icon                                        |

---

_Last updated: 2026-07-31. Based on Dardcor Code v3.8.50._
