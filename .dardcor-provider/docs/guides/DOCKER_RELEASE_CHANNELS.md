...
Dardcor Code publishes separate Docker channels for stable releases, active release-branch testing, and development builds.
...
```bash
docker pull diegosouzapw/Dardcor Code:next
docker pull diegosouzapw/Dardcor Code:next-web
```
...
services:
  Dardcor Code:
    image: diegosouzapw/Dardcor Code:next
```
...
```bash
docker pull diegosouzapw/Dardcor Code:next
docker image inspect diegosouzapw/Dardcor Code:next --format '{{index .RepoDigests 0}}'
```

Before testing, back up the Dardcor Code data volume or bind-mounted data directory. To roll back, restore the previously used stable version or digest and recreate the container:

```bash
docker pull diegosouzapw/Dardcor Code:<stable-version>
docker compose up -d
```
