...
Dardcor Code uses four distinct credential families for management access. This guide
...
| CLI machine-id token    | Full management    | Per-machine    | `Dardcor Code` CLI commands          |
...
Created by `Dardcor Code auth login` on first use. Stored in `~/.Dardcor Code/auth.json`.
...
Cookie: Dardcor Code_session=<jwt-token>
```
