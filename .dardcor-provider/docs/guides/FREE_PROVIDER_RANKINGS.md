...
> **TL;DR**: Dardcor Code ranks its **free** providers by model quality using **Arena AI
...
Dardcor Code aggregates 160+ providers, many of which expose a **free tier** (no-auth,
...
Leaderboard categories map to Dardcor Code task categories:

| Arena leaderboard | Dardcor Code task categories                         |
| ----------------- | ------------------------------------------------- |
...
- It is **non-blocking and never fatal** — if the upstream fetch fails, Dardcor Code keeps
...
stripped and a small alias map expands canonical names into the variants Dardcor Code uses
