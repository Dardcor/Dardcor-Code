...
Dardcor Code ships a cross-platform desktop app (Windows / macOS / Linux) built on
...
| App version        | `3.8.0`                    |
| App id             | `online.Dardcor Code.desktop` |
| Product name       | `Dardcor Code`                |
...
- Linux: `$XDG_CONFIG_HOME/Dardcor Code` or `~/.Dardcor Code`
- macOS: `~/.Dardcor Code`
...
- Close button minimizes to tray; the tray menu has **Open Dardcor Code**, **Open Dashboard** (external browser), **Server Port** submenu, **Check for Updates**, **Quit**.
...
Uses `electron-updater` with the GitHub provider (`diegosouzapw/Dardcor Code`).
...
Releases are published to GitHub Releases (`diegosouzapw/Dardcor Code`), which is also where `electron-updater` checks for new versions.
