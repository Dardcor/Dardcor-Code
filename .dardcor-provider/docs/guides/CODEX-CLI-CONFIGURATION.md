---
title: "Codex CLI — Configuration with Dardcor Code"
version: 3.8.49
lastUpdated: 2026-07-26
---

# Codex CLI — Configuration with Dardcor Code

Complete guide for using the Codex CLI pointed at Dardcor Code as an OpenAI-compatible backend.
...
**Dardcor Code solves this transparently:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (Dardcor Code)
    → Dardcor Code Responses ↔ Chat Completions transformer
    → POST /chat/completions (DeepSeek / Mistral / GLM / Kimi / any provider)
```

You never need a separate translation proxy when using Dardcor Code. **All models use `wire_api = "responses"`** — Dardcor Code handles the rest.
...
| Model                                | Dardcor Code ID                         | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in Dardcor Code use the `cx/` prefix:

| Codex CLI name          | Dardcor Code model    |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the Dardcor Code provider alias.
...
## Generating profiles automatically with `Dardcor Code setup-codex`

If you run Dardcor Code on a VPS, you can auto-generate profile files from the live model catalog:

```bash
# From a VPS (uses local Dardcor Code on port 20128)
Dardcor Code setup-codex

# From any machine — point at your VPS
Dardcor Code setup-codex --remote http://100.x.x.x:20128 --api-key sk-xxx

# Preview without writing files
Dardcor Code setup-codex --remote http://100.x.x.x:20128 --dry-run

# Only generate GLM and Kimi profiles
Dardcor Code setup-codex --only glm,kimi

# Write to a custom directory
Dardcor Code setup-codex --codex-home /path/to/.codex
```
...
Dardcor Code can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `OMNIROUTE_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `Dardcor Code launch-codex`

Health-checks your Dardcor Code instance before launching Codex:

```bash
# Launch against local Dardcor Code (default port 20128)
Dardcor Code launch-codex

# Launch with a specific profile
Dardcor Code launch-codex --profile kimi-k27

# Launch against a remote VPS
Dardcor Code launch-codex --remote http://100.x.x.x:20128/v1 --api-key sk-xxx

# Pass extra args to codex
Dardcor Code launch-codex --profile glm52 -- --yolo "fix this bug"
```
...
[model_providers.Dardcor Code.http_headers]
"X-Custom-Header" = "value"

# Headers read from env vars
[model_providers.Dardcor Code.env_http_headers]
"X-Trace-Id" = "TRACE_ID"

# Extra URL query params (useful for Azure api-version)
[model_providers.Dardcor Code.query_params]
...
```toml
[model_providers.Dardcor Code-main]
base_url = "http://192.168.0.1:20128/v1"
env_key  = "OMNIROUTE_API_KEY"

[model_providers.Dardcor Code-tailscale]
base_url = "http://100.x.x.x:20128/v1"
env_key  = "OMNIROUTE_API_KEY"
```
...
Two Dardcor Code defaults can silently sabotage multi-hour Codex CLI sessions. Neither is a Codex CLI setting — both live on the Dardcor Code side. Users migrating a config from upstream proxies that pin accounts and disable idle cutoffs often hit both and conclude Dardcor Code “cannot sustain a long session.”
...
Related discussions: [#7126](https://github.com/diegosouzapw/Dardcor Code/discussions/7126) (long task drops), [#5718](https://github.com/diegosouzapw/Dardcor Code/discussions/5718) (why affinity defaults off). Tracking: [#7287](https://github.com/diegosouzapw/Dardcor Code/issues/7287).
...
Every turn of a multi-turn Codex conversation is routed independently by the active combo strategy and can land on a **different account per turn**. That breaks upstream session / prompt-cache continuity. Dardcor Code only consults Codex session headers (`x-codex-session-id` / `x-session-id` / `x-Dardcor Code-session`) and body fields such as `prompt_cache_key` / `session_id` when the TTL is greater than `0` (`extractSessionAffinityKey` in `src/sse/services/auth.ts`).
...
Critical detail: Dardcor Code’s synthetic SSE **heartbeat does not reset** the idle clock. Only a real upstream body chunk updates `lastChunkTime`. A quiet model that is still “thinking” looks identical to a stalled upstream from the watchdog’s point of view.
...
In the Dardcor Code process environment (`.env` / compose / systemd):
...
Restart Dardcor Code after changing these env vars.
...
2. **Raise / disable idle cutoffs** in Dardcor Code’s environment:
...
4. Restart Dardcor Code, then start the long Codex task.
...
When the idle watchdog fires, Dardcor Code logs a line shaped like:

```text
[STREAM] Idle timeout: no data from codex for 600000ms (model: cx/gpt-5.5)
```

Grep for `Idle timeout: no data from` (or the code `stream_idle_timeout` / error name `StreamIdleTimeoutError`). The provider segment is whatever Dardcor Code used for that request (`codex`, another provider id, or `provider` if unknown) — it is not always the literal string `codex`.
...
Verify the model exists in Dardcor Code with the correct prefix. Use `Dardcor Code models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `OMNIROUTE_API_KEY` is exported: `echo $OMNIROUTE_API_KEY`.

**`Connection refused`**
Verify Dardcor Code is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
...
See [Long-running tasks](#long-running-tasks). Enable session affinity (TTL above task length) and raise or disable `STREAM_IDLE_TIMEOUT_MS` / `FETCH_BODY_TIMEOUT_MS`. Grep Dardcor Code logs for `Idle timeout: no data from`.
