---
title: "Claude Code CLI — Configuration with Dardcor Code"
version: 3.8.40
lastUpdated: 2026-07-24
---

# Claude Code CLI — Configuration with Dardcor Code

Point the **Claude Code** CLI (`claude`) at Dardcor Code — local or a remote VPS —
...
| `ANTHROPIC_AUTH_TOKEN`                       | Sent as `Authorization: Bearer …` — use your Dardcor Code access token / API key          |
...
`Dardcor Code launch` sets all of these for you: it resolves the base URL + token
from the active context (so `Dardcor Code connect <vps>` then `Dardcor Code launch`
...
`/model` picker normally shows **only** Dardcor Code's Claude/Anthropic models — a
`kimi/kimi-k2.6` or `glm/glm-5.2` never appears, even though it routes fine.

Dardcor Code can mirror any enabled model (and combo) under a `claude/…` id so it
...
When you pick one of these in Claude Code, Dardcor Code strips the `claude/` wrapper
...
wins), so a plain Dardcor Code never doubles its catalog for clients that don't use
Claude Code:
...
`Dardcor Code setup-claude` fetches the live `/v1/models` catalog and writes one
...
> `Dardcor Code launch --profile <name>` (it injects `ANTHROPIC_AUTH_TOKEN` from the
> active context), or export `ANTHROPIC_AUTH_TOKEN` yourself and run
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync after model discovery (opt-in).** Dardcor Code can regenerate these same
...
Claude Code routes to capability tiers. Map each to an Dardcor Code model via env /
...
Once you've run `Dardcor Code connect <host>` (see
[Remote Mode](./REMOTE-MODE.md)), `Dardcor Code launch` and `Dardcor Code setup-claude`
...
`/v1`** and restart `claude` (env is read once at startup). `Dardcor Code launch`
...
Dardcor Code refuses to guess. Fix it either way: pin a prefixed id with
...
**Auth errors** — the profile holds no token. Use `Dardcor Code launch --profile`
