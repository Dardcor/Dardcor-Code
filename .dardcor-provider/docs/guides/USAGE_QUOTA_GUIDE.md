...
> **TL;DR**: Dardcor Code tracks every request's token usage, computes cost, enforces per-API-key quota, and surfaces analytics in the dashboard. This guide explains how it all works.
...
Every request that flows through Dardcor Code generates a **usage record** that captures:
...
For providers that don't return usage (some web-cookie providers), Dardcor Code **estimates** tokens using a `~4 chars per token` heuristic (see `open-sse/services/autoCombo/pipelineRouter.ts`).

### Cached Tokens

Dardcor Code tracks `cached_tokens` separately from `prompt_tokens` because:
...
For models with no pricing data, Dardcor Code falls back to **estimating cost** using internal average rates (sourced from LiteLLM's pricing data).
...
| Tool                    | Description                                        |
| ----------------------- | -------------------------------------------------- |
| `Dardcor Code_cost_report` | Generates a per-key cost report for a given period |
| `Dardcor Code_check_quota` | Returns current quota status for an API key        |

Example agent invocation:

```json
{
  "tool": "Dardcor Code_cost_report",
  "args": { "period": "week" }
}
```
