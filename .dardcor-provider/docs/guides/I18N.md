...
Dardcor Code supports **43 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
Dardcor Code uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through Dardcor Code
Cloud):
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including Dardcor Code itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `Dardcor Code` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | ------------------------ | --------------------------------------- |
| 1        | `--lang` flag            | `Dardcor Code --lang de status`            |
| 2        | `OMNIROUTE_LANG` env var | `OMNIROUTE_LANG=ja Dardcor Code providers` |
...
The saved preference is written atomically to `~/.Dardcor Code/.env` and is loaded by the
...
(`Dardcor Code`, `OAuth`, `MCP`, `A2A`, `DATA_DIR`, …) that must appear verbatim
