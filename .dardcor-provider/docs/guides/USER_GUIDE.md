...
Complete guide for configuring providers, creating combos, integrating CLI tools, and deploying Dardcor Code.
...
models. Haiku models do not accept the `max` effort tier, so Dardcor Code downgrades that
...
1. Subscribe: [Moonshot AI](https://platform.kimi.ai?aff=Dardcor Code)
2. Get API key → Dashboard → Add API Key
...
```json
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "your-Dardcor Code-api-key"
  }
}
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "Dardcor Code/if/kimi-k2.7-code" }
    }
  },
  "models": {
    "providers": {
...
mkdir -p ~/.Dardcor Code

# Create .env file (see .env.example)
cp .env.example ~/.Dardcor Code/.env
...
The CLI automatically loads `.env` from `~/.Dardcor Code/.env` or `./.env`.
...
```bash
git clone https://github.com/diegosouzapw/Dardcor Code.git
cd Dardcor Code && npm install && npm run build
...
npm run start
# Or: pm2 start npm --name Dardcor Code -- start
```
...
OMNIROUTE_MEMORY_MB=512 pm2 start npm --name Dardcor Code -- start
...
docker build -t Dardcor Code:cli .

# Portable mode (recommended)
docker run -d --name Dardcor Code -p 20128:20128 --env-file ./.env -v Dardcor Code-data:/app/data Dardcor Code:cli
```
...
Void Linux users can package and install Dardcor Code natively using the `xbps-src` cross-compilation framework. This automates the Node.js standalone build along with the required `better-sqlite3` native bindings.
...
| --------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `Dardcor Code-default-secret-change-me` | JWT signing secret (**change in production**)                                                             |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | First login password                                                                                      |
| `DATA_DIR`                              | `~/.Dardcor Code`                       | Data directory (db, usage, logs)                                                                          |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://Dardcor Code.dev`              | Cloud sync endpoint base URL (replaces legacy `CLOUD_URL`)                                                |
...
### Chaining Dardcor Code Peers

Another Dardcor Code gateway can be added as a **Custom OpenAI-compatible** provider. Use the
...
`X-Dardcor Code-Peer-Trace` header. A gateway rejects a repeated instance ID or exhausted hop
...
into multiple running Dardcor Code instances.
...
- **Semantic Cache** — Auto-caches non-streaming, temperature=0 responses (bypass with `X-Dardcor Code-No-Cache: true`)
- **Request Idempotency** — Deduplicates requests within 5s via `Idempotency-Key` or `X-Request-Id` header
- **Progress Tracking** — Opt-in SSE `event: progress` events via `X-Dardcor Code-Progress: true` header

---

### Translator Playground

Access via **Dashboard → Translator**. Debug and visualize how Dardcor Code translates API requests between providers.
...
Dardcor Code also accepts `x_session_id` and returns the effective session key in `X-Dardcor Code-Session-Id`.
...
Dardcor Code implements provider-level resilience with five components:
...
4. **Wait For Cooldown** — If every candidate connection is already cooling down, Dardcor Code can wait for the earliest cooldown and retry the same client request automatically.
...
- Migrate Dardcor Code between machines
...
Dardcor Code supports audio transcription via the OpenAI-compatible endpoint:
...
Dardcor Code clamps it down to that cap before sending the upstream request.
...
Dardcor Code is both an **MCP server** (Model Context Protocol) and an **A2A server** (Agent-to-Agent JSON-RPC 2.0). Any MCP-compatible IDE or agent host can call Dardcor Code tools directly — no extra wrapper required.

### MCP transports

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `Dardcor Code --mcp` (for IDE plugins that prefer stdio)
...
Dardcor Code exposes an extensible **skill framework** (`src/lib/skills/`) so agents and the A2A endpoint can run domain-specific routines (e.g. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
Dardcor Code persists **long-term conversational memory** with hybrid retrieval:
...
Subscribe to Dardcor Code events for real-time monitoring and automation.
...
- Every payload includes `X-Dardcor Code-Signature` (HMAC-SHA256) for verification
...
Dardcor Code integrates with cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**) so you can dispatch long-running tasks from the same dashboard that handles your local routing.

- Create tasks in **Dashboard → Cloud Agents** or via `POST /api/v1/agents/tasks`
- Track status, logs, and artifacts per task
- Bring-your-own API key per provider — credentials never leave the Dardcor Code instance
...
You can manage every Dardcor Code resource (providers, combos, keys, settings) over HTTP using a **Bearer key with the `manage` scope**.
...
Dardcor Code ships an internal CLI (`Dardcor Code …`) for setup, diagnostics, and runtime control. This is **separate from the "CLI Tools" page in the dashboard**, which configures third-party CLIs (Claude Code, Cursor, Codex, Cline, …) so they can talk to Dardcor Code.

```bash
Dardcor Code setup                    # Interactive wizard (password, providers, combos)
Dardcor Code setup --non-interactive  # CI-friendly
Dardcor Code doctor                   # Health diagnostics (data dir, DB, providers, ports)
Dardcor Code providers available      # List supported providers
Dardcor Code providers list           # List configured connections
Dardcor Code providers test <id>      # Live test a provider connection
Dardcor Code combos list              # List combos
Dardcor Code combos switch <name>     # Set default combo
Dardcor Code models                   # List available models (--json, --search)
Dardcor Code keys add | list | remove # Manage API keys from the terminal
Dardcor Code backup                   # Snapshot config + DB
Dardcor Code restore [<timestamp>]    # Restore from a snapshot
Dardcor Code health                   # Detailed health (breakers, cache, memory)
Dardcor Code quota                    # Provider quota usage
Dardcor Code mcp status               # MCP server status
Dardcor Code a2a status               # A2A server status
Dardcor Code tunnel list|create|stop  # Cloudflare/Tailscale/ngrok tunnels
Dardcor Code reset-password           # Reset the admin password
Dardcor Code --mcp                    # Start MCP server over stdio
Dardcor Code --port 3000              # Start the server on a custom port
```

Tip: pair `Dardcor Code doctor --json` with your monitoring tool to alert on unhealthy provider connections.

---

## 🖥️ Desktop Application (Electron)

Dardcor Code is available as a native desktop application for Windows, macOS, and Linux.
