...
const RUNTIME_DIR = join(homedir(), ".dardcorCode", "runtime");
...
export async function loadSystray(): Promise<(new (...args: unknown[]) => unknown) | null> {
  ...
  if (!isInstalled()) {
    try {
      ...
      console.warn(`[dardcorCode] tray runtime install failed: ${(err as Error).message}`);
      return null;
    }
  }
  ...
  try {
    ...
    console.warn(`[dardcorCode] tray runtime import failed: ${(err as Error).message}`);
    return null;
  }
}
...
function ensureRuntimeDir(): void {
  ...
  if (!existsSync(pkg)) {
    writeFileSync(pkg, JSON.stringify({ name: "dardcorCode-runtime", private: true }), "utf-8");
  }
}
