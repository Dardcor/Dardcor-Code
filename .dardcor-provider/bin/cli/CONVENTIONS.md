# Dardcor Code CLI — Internal Conventions
...
```
Dardcor Code keys add openai sk-xxx
Dardcor Code combo switch fastest
Dardcor Code memory search "react hooks"
```

**Not allowed**:

```
Dardcor Code --add-key openai sk-xxx     # ❌ flag-as-verb
Dardcor Code add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `OMNIROUTE_BASE_URL` env or `~/.Dardcor Code/config.json`
  (active profile).
- Injects `Authorization: Bearer ${OMNIROUTE_API_KEY}` when available.
- Injects `x-Dardcor Code-cli-token` when applicable (see task 8.12).
...
- Locale persisted via `config lang set <code>` — saves `OMNIROUTE_LANG` to `~/.Dardcor Code/.env`.
