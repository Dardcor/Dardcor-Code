# bin/cli — Dardcor Code CLI internals

This directory contains the CLI runtime, helpers, and commands for the `Dardcor Code` binary.
...
├── sqlite.mjs              ← openDardcor CodeDb() — DB bootstrap
...
All HTTP calls to the Dardcor Code server must go through this wrapper.
...
```bash
Dardcor Code config lang set pt-BR       # saves to ~/.Dardcor Code/.env
Dardcor Code config lang list            # show all 42 available locales
Dardcor Code config lang get             # show currently active locale
```

**One-time override:**

```bash
Dardcor Code --lang de providers list    # run in German, not persisted
OMNIROUTE_LANG=ja Dardcor Code status    # same effect via env
```
