...
// Minimal 16x16 DardcorCode icon as base64 PNG (fallback when file missing)
...
async function initWindowsTrayInstance(options: TrayOptions): Promise<TrayInstance | null> {
  ...
  handle = initWindowsTray({
    iconPath,
    tooltip: `DardcorCode :${options.port}`,
    onEvent: async (evt) => {
