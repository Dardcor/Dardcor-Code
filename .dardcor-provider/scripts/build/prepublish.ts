...
/**
 * DardcorCode — Prepublish Build Script
 *
...
console.log("🔨 DardcorCode — Building for npm publish...\n");
...
const cliSrcFile = join(ROOT, "bin", "dardcorCode.ts");
const cliDestFile = join(ROOT, "bin", "dardcorCode.mjs");
  ...
  try {
    runBuildTool(
      ...
      [
        ...
        "--packages=external",
        "--format=esm",
        "--outfile=bin/dardcorCode.mjs",
      ],
      { cwd: ROOT, stdio: "inherit" }
    );
    chmodSync(cliDestFile, 0o755);
    console.log("  ✅ CLI Entrypoint bundled to bin/dardcorCode.mjs");
  } catch (err: any) {
    console.warn("  ⚠️  CLI bundle error:", err.message);
  }
}

// ── Step 8.8: Build @dardcorCode/opencode-plugin ──────────────
// The plugin ships bundled inside the dardcorCode npm package (see root
// package.json "files": ["@dardcorCode/", ...]). Its built `dist/` MUST be
// present in the publish tarball so `dardcorCode setup opencode` can copy it
...
const opencodePluginSrc = join(ROOT, "@dardcorCode", "opencode-plugin");
...
if (existsSync(opencodePluginSrc) && existsSync(join(opencodePluginSrc, "package.json"))) {
  ...
  if (!pluginAlreadyBuilt) {
    console.log("\n  🔨 Building @dardcorCode/opencode-plugin (tsup)...");
    try {
      ...
      console.log("  ✅ @dardcorCode/opencode-plugin bundled to @dardcorCode/opencode-plugin/dist/");
    } catch (err: any) {
      console.error("  ❌ Failed to build @dardcorCode/opencode-plugin:", err.message);
      console.error("     The published package would be missing the plugin dist.");
      console.error(
        "     Run `cd @dardcorCode/opencode-plugin && npm install && npm run build` to debug."
      );
      process.exit(1);
    }
  } else {
    console.log("  ✅ @dardcorCode/opencode-plugin dist/ already present (skipping rebuild)");
  }
  ...
  if (existsSync(pluginNodeModules)) {
    ...
    console.log("  🧹 Removed @dardcorCode/opencode-plugin/node_modules (hard link guard)");
  }
} else {
  console.log("  ⏭️  @dardcorCode/opencode-plugin not found in workspace (skipping build)");
}
