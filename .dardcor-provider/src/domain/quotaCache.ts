...
import { getUsageForProvider } from "@dardcorCode/open-sse/services/usage.ts";
...
import { getCodexQuotaWindowFilterForModel } from "@dardcorCode/open-sse/config/codexQuotaScopes.ts";
import { getAntigravityQuotaFamily } from "@dardcorCode/open-sse/services/antigravityQuotaFamily.ts";
...
declare global {
  var __dardcorCodeQuotaCacheState: QuotaCacheState | undefined;
}

function getState(): QuotaCacheState {
  if (!globalThis.__dardcorCodeQuotaCacheState) {
    globalThis.__dardcorCodeQuotaCacheState = {
      cache: new Map(),
  ...
  return globalThis.__dardcorCodeQuotaCacheState;
}
