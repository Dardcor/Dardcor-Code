...
export function getDardcorCodeTokenCounts(usage: UsageLike): { input: number; output: number } {
...
export function formatDardcorCodeCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

/**
 * Build the `X-DardcorCode-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-DardcorCode-Provider` header already applies)
 ...
 */
export function buildDardcorCodeDecisionHeaderValue({
  strategy = null,
  provider = null,
  latencyMs = 0,
}: {
...
export function buildDardcorCodeResponseMetaHeaders({
  cacheHit = false,
  costUsd = 0,
   ...
   * Emitted as `X-DardcorCode-Cost-Saved` only when provided (omitted on normal
   ...
   * request) surfaced via `X-DardcorCode-Decision`. See #6022.
   */
  strategy?: string | null;
  usage?: UsageLike;
}): Record<string, string> {
  const tokens = getDardcorCodeTokenCounts(usage);
  const headers: Record<string, string> = {
    ...
    [OMNIROUTE_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatDardcorCodeCost(costUsd)),
  ...
  if (costSavedUsd != null) {
    headers[OMNIROUTE_RESPONSE_HEADERS.costSaved] = toHeaderValue(
      formatDardcorCodeCost(costSavedUsd)
    );
  }
  ...
  const decisionValue = buildDardcorCodeDecisionHeaderValue({ strategy, provider, latencyMs });
...
export function buildDardcorCodeSseMetadataComment(
  options: Parameters<typeof buildDardcorCodeResponseMetaHeaders>[0]
): string {
  const headers = buildDardcorCodeResponseMetaHeaders(options);
...
/**
 * Single choke-point for attaching the X-DardcorCode-* response meta headers.
 ...
 */
export function attachDardcorCodeMetaHeaders(
  headers: Headers | Record<string, string>,
  meta: Parameters<typeof buildDardcorCodeResponseMetaHeaders>[0]
): void {
  const built = buildDardcorCodeResponseMetaHeaders(meta);
...
/**
 * Attach the X-DardcorCode-* meta headers onto an already-built Response, ADDING
 ...
 */
export function attachDardcorCodeMetaToResponse(
  response: Response,
  meta: Parameters<typeof buildDardcorCodeResponseMetaHeaders>[0]
): Response {
  ...
  try {
    attachDardcorCodeMetaHeaders(response.headers, meta);
    return response;
  } catch {
    ...
    attachDardcorCodeMetaHeaders(cloned.headers, meta);
    return cloned;
  }
}
