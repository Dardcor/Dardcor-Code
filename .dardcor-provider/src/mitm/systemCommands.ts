 ...
 *
 * Slim Docker images (e.g. `node:24-trixie-slim` used by DardcorCode's runtime
...
export async function runElevatedPowerShell(script: string): Promise<string> {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-elevate-"));
  const scriptName = `dardcorCode-elevate-${crypto.randomUUID()}.ps1`;
