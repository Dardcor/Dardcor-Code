...
const INSTRUCTIONS = [
  "1. Install DardcorCode's root certificate (Dashboard → AgentBridge → Cert)",
  "2. Start the MITM proxy: `dardcorCode mitm start --target kiro`",
  "3. Set your system HTTP proxy to 127.0.0.1:20130 (or use transparent MITM via DNS override)",
  "4. Open Kiro IDE — API calls will be automatically routed through DardcorCode.",
  "5. Verify: check the Proxy Logs in DardcorCode dashboard and look for provider=anthropic source=mitm",
];
...
export const KIRO_MITM_PROFILE: MitmTarget & {
  ...
  ...KIRO_TARGET,
  description:
    "Intercepts Kiro IDE requests to api.anthropic.com and routes them through DardcorCode.",
  targetHost: HOSTS[0],
  targetPort: 443,
  localPort: 20130,
