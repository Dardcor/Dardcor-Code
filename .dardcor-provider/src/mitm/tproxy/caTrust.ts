 ...
 * trust store under a DEDICATED slot (`dardcorCode-tproxy-ca.crt`), separate from
 * the static MITM cert (`dardcorCode-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "dardcorCode-tproxy-ca.crt";
