...
export function interpretMitmStartupError(stderr: string, port: number): string {
  ...
  if (lower.includes("router_api_key")) {
    return "MITM server failed to start: no API key was provided (ROUTER_API_KEY is required). Set a router API key in DardcorCode and retry.";
  }
...
/**
 * Start MITM proxy
 * @param {string} apiKey - DardcorCode API key
...
async function startMitmInternal(
  ...
  // is shared with the DardcorCode process: getIngestTokenForBootstrap() returns
