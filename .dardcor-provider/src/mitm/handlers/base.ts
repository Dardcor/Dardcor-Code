 ...
 *   - forwarding to the DardcorCode router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@dardcorCode/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@dardcorCode/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the DardcorCode router round-trip.
  ...
  /**
   * Forward the prepared body to the DardcorCode router (Next.js API).
   * Adds AgentBridge correlation headers (`x-dardcorCode-source`, `x-dardcorCode-agent`)
  ...
  protected async fetchRouter(
    ...
    return fetch(url, {
      ...
      headers: {
        ...
        "x-dardcorCode-source": "agent-bridge",
        "x-dardcorCode-agent": this.agentId,
