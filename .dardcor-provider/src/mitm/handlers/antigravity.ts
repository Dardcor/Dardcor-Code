 ...
 * `thinkingConfig`, …). The DardcorCode router endpoint `/v1/chat/completions`
 ...
 *   - forward to `/v1/chat/completions` on the DardcorCode router,
 ...
 * @param model      resolved DardcorCode model string
  ...
  async intercept(
    ...
    try {
      ...
      if (!upstream.ok) {
        ...
        throw new Error(`DardcorCode ${upstream.status}: ${errText}`);
      }
