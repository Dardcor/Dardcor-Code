...
const LINUX_CERT_NAME = "dardcorCode-mitm.crt";
  ...
  return new Promise((resolve) => {
    exec(
      script,
      {
        ...
        env: {
          ...process.env,
          CERT_NAME: "DardcorCode MITM Root CA",
          CERT_PATH: certPath || "",
          ACTION: action,
        },
      },
 ...
 * cert-path-agnostic and keep writing to the same `dardcorCode-mitm.crt`
 ...
 * cleanup needed. Distinct from TPROXY's own `dardcorCode-tproxy-ca.crt` slot
...
// They install / remove the DardcorCode MITM root CA from the OS trust store and
