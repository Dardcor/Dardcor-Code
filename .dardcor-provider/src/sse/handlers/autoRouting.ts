...
import type { AutoVariant } from "@dardcorCode/open-sse/services/autoCombo/autoPrefix.ts";
import {
  AUTO_TEMPLATE_VARIANTS,
  VALID_AUTO_VARIANTS,
} from "@dardcorCode/open-sse/services/autoCombo/builtinCatalog.ts";
import {
  parseAutoSuffix,
  type AutoCategory,
  type AutoTier,
} from "@dardcorCode/open-sse/services/autoCombo/suffixComposition.ts";
import {
  isValidModelFamily,
  type ModelFamily,
} from "@dardcorCode/open-sse/services/autoCombo/modelFamily.ts";
...
async function applyAutoPrefix(
  ...
  try {
    const { parseAutoPrefix } =
      await import("@dardcorCode/open-sse/services/autoCombo/autoPrefix.ts");
...
export async function createVirtualAutoCombo(
  ...
  try {
    const { createVirtualAutoCombo: createVirtual } =
      await import("@dardcorCode/open-sse/services/autoCombo/virtualFactory.ts");
