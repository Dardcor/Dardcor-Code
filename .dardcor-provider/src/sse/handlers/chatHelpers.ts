...
import { createBuiltinAutoCombo } from "@dardcorCode/open-sse/services/autoCombo/builtinCatalog.ts";
import * as log from "../utils/logger";
import { updateProviderCredentials } from "../services/tokenRefresh";
import { detectFormatFromEndpoint } from "@dardcorCode/open-sse/services/provider.ts";
import { resolveChatCoreTargetFormat } from "@dardcorCode/open-sse/handlers/chatCore/targetFormat.ts";
import { handleChatCore } from "@dardcorCode/open-sse/handlers/chatCore.ts";
import {
  checkResourcePressureGuard,
  type ResourcePressureGuardResult,
} from "@dardcorCode/open-sse/utils/resourcePressure.ts";
import {
  errorResponse,
  modelCooldownResponse,
  providerCircuitOpenResponse,
  unavailableResponse,
} from "@dardcorCode/open-sse/utils/error.ts";
import { HTTP_STATUS } from "@dardcorCode/open-sse/config/constants.ts";
import {
  runWithProxyContext,
...
} from "@dardcorCode/open-sse/utils/proxyFetch.ts";
...
import { getRuntimeProviderProfile } from "@dardcorCode/open-sse/services/accountFallback.ts";
...
export async function safeLogEvents({
  ...
  try {
    ...
    try {
      ...
      const { proxyConfigToUrl } = await import("@dardcorCode/open-sse/utils/proxyDispatcher.ts");
...
export function withSessionHeader(response: Response, sessionId: string | null): Response {
  ...
  try {
    response.headers.set("X-DardcorCode-Session-Id", sessionId);
    return response;
  } catch {
    ...
    cloned.headers.set("X-DardcorCode-Session-Id", sessionId);
    return cloned;
  }
}
 ...
 * `x-dardcorCode-modality-bridge` header on responses whose request payload was
...
export function withModalityBridgeHeader(response: Response, value: string | null): Response {
  ...
  try {
    response.headers.set("x-dardcorCode-modality-bridge", value);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
    ...
    cloned.headers.set("x-dardcorCode-modality-bridge", value);
    return cloned;
  }
}
...
export function withSelectedConnectionHeader(
  ...
  try {
    response.headers.set("X-DardcorCode-Selected-Connection-Id", connectionId);
    return response;
  } catch {
    const cloned = new Response(response.body, {
      status: response.status,
    ...
    cloned.headers.set("X-DardcorCode-Selected-Connection-Id", connectionId);
    return cloned;
  }
}
