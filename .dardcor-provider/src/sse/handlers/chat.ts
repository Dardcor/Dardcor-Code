...
} from "@dardcorCode/open-sse/services/accountFallback.ts";
import { getCombo, getComboForModel, getModelInfo } from "../services/model";
import { stripContextWindowSuffix } from "@dardcorCode/open-sse/services/model.ts";
import { resolveBareModelToConnectionDefault } from "@dardcorCode/open-sse/services/model.ts";
import { errorResponse } from "@dardcorCode/open-sse/utils/error.ts";
import { getImageModelEntry } from "@dardcorCode/open-sse/config/imageRegistry.ts";
import { acceptHeaderForcesStream } from "@dardcorCode/open-sse/utils/aiSdkCompat.ts";
import { applyNoThinkingAlias } from "@dardcorCode/open-sse/utils/noThinkingAlias.ts";
import { resolveCcDiscoveryAliasStrip } from "@/lib/ccDiscoveryAliasResolve";
import { handleComboChat, shouldSkipConnDisable } from "@dardcorCode/open-sse/services/combo.ts";
import { mergeAbortSignals } from "@dardcorCode/open-sse/executors/base.ts";
import { resolveRequestAutoControls } from "@dardcorCode/open-sse/services/autoCombo/requestControls.ts";
import { resolveComboConfig } from "@dardcorCode/open-sse/services/comboConfig.ts";
import { injectHandoffIntoBody } from "@dardcorCode/open-sse/services/contextHandoff.ts";
import {
  HTTP_STATUS,
  ANTIGRAVITY_PRE_RESPONSE_TIMEOUT_CODE,
} from "@dardcorCode/open-sse/config/constants.ts";
import { getTargetFormat, detectFormatFromUrl } from "@dardcorCode/open-sse/services/provider.ts";
import {
  getModelsByProviderId,
  getModelTargetFormat,
  PROVIDER_ID_TO_ALIAS,
} from "@dardcorCode/open-sse/config/providerModels.ts";
...
import { connectionHasExtraKeys } from "@dardcorCode/open-sse/services/apiKeyRotator.ts";
import {
...
import { isSubscriptionQuotaText } from "@dardcorCode/open-sse/services/quotaTextCooldowns.ts";
...
import { cloneBoundedForLog } from "@dardcorCode/open-sse/utils/requestLogger.ts";
import { handleInternalUsageCommand } from "@/lib/usage/internalUsageCommand";
import {
  applyTaskAwareRouting,
  getTaskRoutingConfig,
} from "@dardcorCode/open-sse/services/taskAwareRouter.ts";
import {
  hasNativeWebSearchTool,
  resolveWebSearchRouteOverride,
} from "@dardcorCode/open-sse/services/webSearchRouting.ts";
import {
...
import { startQuotaMonitor } from "@dardcorCode/open-sse/services/quotaMonitor.ts";
import {
  isFallbackDecision,
  shouldUseFallback,
} from "@dardcorCode/open-sse/services/emergencyFallback.ts";
import {
  registerCodexConnection,
  registerCodexQuotaFetcher,
} from "@dardcorCode/open-sse/services/codexQuotaFetcher.ts";
import { registerBailianCodingPlanQuotaFetcher } from "@dardcorCode/open-sse/services/bailianQuotaFetcher.ts";
import { registerCrofUsageFetcher } from "@dardcorCode/open-sse/services/crofUsageFetcher.ts";
import { registerDeepseekQuotaFetcher } from "@dardcorCode/open-sse/services/deepseekQuotaFetcher.ts";
import { registerOpenrouterQuotaFetcher } from "@dardcorCode/open-sse/services/openrouterQuotaFetcher.ts";
import { registerOpencodeQuotaFetcher } from "@dardcorCode/open-sse/services/opencodeQuotaFetcher.ts";
import { registerGrokWebQuotaFetcher } from "@dardcorCode/open-sse/services/grokQuotaFetcher.ts";
import { registerGenericQuotaFetchers } from "@dardcorCode/open-sse/services/genericQuotaFetcher.ts";
import "@dardcorCode/open-sse/services/quotaTrackersBatch.ts";
import {
...
async function handleChatImplementation(
  ...
  // DardcorCode-level 400 before any routing or upstream call (#5110, #6402).
  ...
  const requestedConnectionId = request.headers.get("x-dardcorCode-connection")?.trim() || null;
  if (sessionId) {
  ...
  requestAttemptLoop: while (true) {
    ...
    while (true) {
      ...
      if (
        ...
        body?._dardcorCodeSkipContextRelay !== true
      ) {
      ...
      if (runtimeOptions.sessionId && body?._dardcorCodeInternalRequest !== "context-handoff") {
