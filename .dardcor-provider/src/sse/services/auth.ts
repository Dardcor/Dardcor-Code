...
import { getQuotaScopeLabelForProvider } from "@dardcorCode/open-sse/services/antigravityQuotaFamily.ts";
import { getCreditsMode } from "@dardcorCode/open-sse/services/antigravityCredits.ts";
import {
...
import { isLocalProvider } from "@dardcorCode/open-sse/config/providerRegistry.ts";
import { COOLDOWN_MS, RateLimitReason } from "@dardcorCode/open-sse/config/constants.ts";
import {
  preflightQuota,
  isQuotaPreflightEnabled,
} from "@dardcorCode/open-sse/services/quotaPreflight.ts";
import { resolveResilienceSettings } from "@/lib/resilience/settings";
import { resolveModelLockoutSettings } from "@/lib/resilience/modelLockoutSettings";
import { syncHealthFromDB, type KeyHealth } from "@dardcorCode/open-sse/services/apiKeyRotator.ts";
import {
  classifyProviderError,
  PROVIDER_ERROR_TYPES,
} from "@dardcorCode/open-sse/services/errorClassifier.ts";
...
} from "@dardcorCode/open-sse/config/codexQuotaScopes.ts";
import {
  getProviderById,
  getProviderAlias,
  resolveProviderId,
...
export function extractSessionAffinityKey(
  ...
  const headerKey = normalizeSessionKey(
    readHeaderValue(headers, "x-codex-session-id") ??
      ...
      readHeaderValue(headers, "x-dardcorCode-session"),
    "header"
  );
 ...
 * - `Authorization: Bearer <key>` (OpenAI / DardcorCode / Codex CLI / Bearer clients)
 ...
 * rejected by per-route gates that compare against DardcorCode keys.
 *
