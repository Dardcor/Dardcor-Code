...
import { validateQoderCliPat } from "@dardcorCode/open-sse/services/qoderCli.ts";
...
export async function validateAuggieProvider() {
  const { checkAuggieCliVersion } = await import("@dardcorCode/open-sse/executors/auggie.ts");
