 ...
 *   model: string;                    // the model name the user chose from DardcorCode
 * }
 *
 * interface HermesAgentConfigPayload {
 *   baseUrl: string;                  // usually the DardcorCode base URL
...
export async function generateHermesAgentConfig(
  ...
  // Build the providers.dardcorCode entry (shared)
  const normalizedBase = normalizeBaseUrl(baseUrl);
  const dardcorCodeProvider = {
  ...
  const next: any = {
    ...
    providers: {
      ...(existing.providers || {}),
      dardcorCode: dardcorCodeProvider,
    },
  };
