 ...
 * source is the user's OWN DardcorCode instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from DardcorCode. The catalog is the
 ...
 */
export async function fetchDardcorCodeCatalog(
  baseUrl: string,
  apiKey: string,
  timeoutMs = 5_000
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`DardcorCode /v1/models returned ${response.status} ${response.statusText}`);
    }
...
/**
 * Generate a full `opencode.json` document for DardcorCode. The catalog is the
...
export async function generateOpencodeConfig(options: GenerateOpencodeOptions): Promise<string> {
  ...
  const providerId = options.providerId?.trim() || "dardcorCode";
  ...
  if (fetchCatalog) {
    const result = await fetchDardcorCodeCatalog(baseURL, options.apiKey, timeoutMs);
  ...
  const provider: Record<string, unknown> = {
    name: existingProvider?.name ?? "DardcorCode",
