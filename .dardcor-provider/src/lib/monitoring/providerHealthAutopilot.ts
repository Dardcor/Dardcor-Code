...
import { clearProviderFailure, clearModelLock } from "@dardcorCode/open-sse/services/accountFallback";
...
export async function buildProviderHealthAutopilotReport(
  ...
  const [{ getAllCircuitBreakerStatuses }, { getAllModelLockouts }, quotaMonitor] =
    await Promise.all([
      ...
      import("@dardcorCode/open-sse/services/quotaMonitor.ts").catch(() => null),
    ]);
