...
// ─── Provider mapping: LiteLLM provider → DardcorCode aliases ─────
...
/**
 * Transform LiteLLM raw data → DardcorCode PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (DardcorCode format).
 ...
 */
export function transformToDardcorCode(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToDardcorCode(raw);
