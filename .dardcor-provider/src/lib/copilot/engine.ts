/**
 * DardcorCode Copilot — Chat Engine
 *
...
function getKnowledgeResponse(query: string): string | null {
  ...
  if (
    ...
    return `## DardcorCode Architecture
...
Use \`createCombo\` tool or \`runDardcorCodeCli\` to create them.`;
  }
...
DardcorCode supports 212+ providers across categories:
...
I can use CodeGraph to explore the DardcorCode codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## DardcorCode Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`dardcorCode health\`
- "CLI list-combos" → ejecuta \`dardcorCode list-combos\`
- "CLI set-budget 10" → ejecuta \`dardcorCode set-budget 10\`

### Conocimiento
- "Cómo funciona DardcorCode?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with DardcorCode.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runDardcorCodeCli") {
    ...
    return {
      message: result,
      toolCalls: [{ name: "runDardcorCodeCli", args: intent.args, result }],
    };
  }
