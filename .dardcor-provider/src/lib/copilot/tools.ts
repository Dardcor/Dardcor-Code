/**
 * DardcorCode Copilot — Tool definitions
 *
 * Tools the copilot can execute to configure DardcorCode on behalf of the user,
...
// ── Helper: check if dardcorCode CLI is available ──────────────────────────────

function getDardcorCodeCliPath(): string | null {
  try {
    const result = execSync("which dardcorCode 2>/dev/null || command -v dardcorCode 2>/dev/null", {
      encoding: "utf-8",
      timeout: 3000,
    }).trim();
    return result || null;
...
export const COPILOT_TOOLS: CopilotTool[] = [
  ...
  // ── CodeGraph Tools ──
  {
    ...
    description:
      "Search for symbols in the DardcorCode codebase by name (functions, classes, types, variables). Use this to understand how the app works internally.",
    parameters: [
      {
        name: "query",
        type: "string",
  ...
  },
  {
    name: "findCallees",
    description:
      "Find all functions/symbols that a specific function calls. Useful for understanding dependencies and code flow within DardcorCode.",
    parameters: [
      {
        name: "symbol",
        type: "string",
  ...
  // ── CLI Execution Tool ──
  {
    name: "runDardcorCodeCli",
    description:
      "Execute an 'dardcorCode' CLI command to configure or query the DardcorCode app. Gives complete control over the app — use for advanced operations not covered by other tools. Common commands: dardcorCode list-keys, dardcorCode switch-combo [id], dardcorCode set-budget 10, dardcorCode set-strategy [id] priority, dardcorCode health, dardcorCode mcp (starts MCP server), dardcorCode db-health, dardcorCode reset-password.",
    parameters: [
      {
        name: "command",
        type: "string",
        description:
          "CLI command arguments (everything after 'dardcorCode'). Example: 'list-keys', 'switch-combo abc123', 'health'",
        required: true,
      },
    ],
    handler: async (args) => {
      ...
      const cliPath = getDardcorCodeCliPath();
      if (!cliPath) return "dardcorCode CLI not found in PATH. Install DardcorCode first.";
