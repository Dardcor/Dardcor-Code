...
import { isObsoleteKiroModelAlias } from "@dardcorCode/open-sse/services/kiroModels.ts";
...
// expose (as opposed to the flat `supportedThinkingEfforts` field DardcorCode's own
...
// Maps common upstream synonyms onto DardcorCode's canonical effort vocabulary
...
export function normalizeDiscoveredModels(models: unknown): SyncedAvailableModel[] {
  ...
  for (const item of items) {
    ...
    deduped.set(id, {
      ...
      ...(() => {
        // #7694/#8347: the flat field (DardcorCode's own import format) wins verbatim when
