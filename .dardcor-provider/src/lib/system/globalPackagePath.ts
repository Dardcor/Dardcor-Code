...
/**
 * Resolve the real install directory of the globally-installed `dardcorCode` package — the directory
 * that owns `node_modules/better-sqlite3` and so is the correct cwd for `npm rebuild`.
 *
 * Replaces the hardcoded `${globalRoot}/dardcorCode/app` assumption (Bug 3, security-report v3.8.15):
 * the global package root is `${npm root -g}/dardcorCode`, not `/dardcorCode/app`. We probe the real
 ...
 * global install.
 */
export async function resolveGlobalDardcorCodePath(
  ...
  const packageRoot = path.join(globalRoot, "dardcorCode");
