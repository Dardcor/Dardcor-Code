...
export function getAutoUpdateConfig(env: NodeJS.ProcessEnv = process.env): AutoUpdateConfig {
  const dataDir = env.DATA_DIR || "/tmp/dardcorCode";
  const repoDir = env.AUTO_UPDATE_REPO_DIR || "/workspace/dardcorCode";
  ...
  return {
    ...
    composeService: env.AUTO_UPDATE_SERVICE || "dardcorCode-cli",
...
export async function validateAutoUpdateRuntime(
  ...
  if (!(await existsImpl("/var/run/docker.sock"))) {
    return {
      supported: false,
      reason: "Docker socket is not mounted into the DardcorCode container.",
      composeCommand: null,
    };
  }
  ...
  try {
    ...
    return {
      supported: false,
      reason: "git is not available inside the DardcorCode container.",
      composeCommand: null,
    };
  }
  ...
  if (!composeCommand) {
    return {
      supported: false,
      reason:
        "Neither docker compose nor docker-compose is available inside the DardcorCode container.",
      composeCommand: null,
    };
  }
...
export function buildNpmUpdateScript(latest: string): string {
  return [
    ...
    `npm install -g dardcorCode@${latest} --include=optional --ignore-scripts --legacy-peer-deps`,
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart dardcorCode || true",
    "fi",
...
export function buildSourceUpdateScript(latest: string, gitRemote = "origin"): string {
  ...
  return [
    ...
    "npm run build",
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart dardcorCode --update-env || true",
    "fi",
