 ...
 * ONLY from `npm info dardcorCode version --json` (the `npm` CLI binary). When that binary
...
const NPM_REGISTRY_LATEST_URL = "https://registry.npmjs.org/dardcorCode/latest";
...
const GITHUB_RELEASES_LATEST_URL =
  "https://api.github.com/repos/diegosouzapw/DardcorCode/releases/latest";
  ...
  try {
    const res = await fetchImpl(GITHUB_RELEASES_LATEST_URL, {
      ...
      headers: {
        ...
        "User-Agent": "dardcorCode-version-check",
