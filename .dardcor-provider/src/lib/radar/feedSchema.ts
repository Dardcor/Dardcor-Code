/**
 * feedSchema.ts — Zod schema for the DardcorCode Radar feed payload.
 *
...
/**
 * Exported so `sync.ts` can validate the `x-dardcorCode-feed-tier` response
...
export const RadarFeedSchema = z.object({
  feed: z.literal("dardcorCode-radar"),
  ...
  // the Authorization key, and is surfaced via the `x-dardcorCode-feed-tier`
