 ...
 */
const DEFAULT_FEED_BASE_URL = "https://radar.dardcorCode.online";
...
/**
 * Parse & validate the `x-dardcorCode-feed-tier` response header.
 *
...
export async function syncRadar(deps: SyncDeps = {}): Promise<SyncStatus> {
  ...
  try {
    ...
    const signature = res.headers.get("x-dardcorCode-feed-signature") ?? "";
    ...
    // The `x-dardcorCode-feed-tier` header reflects the tier ACTUALLY served
    ...
    const servedTier = parseServedTierHeader(res.headers.get("x-dardcorCode-feed-tier")) ?? feed.tier;
