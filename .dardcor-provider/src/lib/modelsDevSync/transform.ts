 ...
 * types, the provider-id mapping table, and the raw→DardcorCode transform
...
// ─── Provider mapping: models.dev provider ID → DardcorCode provider IDs/aliases ──
//
// models.dev uses canonical provider IDs (e.g. "openai", "anthropic", "google").
// DardcorCode uses both full IDs and short aliases (e.g. "cc" for claude, "cx" for codex).
// We map each models.dev provider to ALL DardcorCode identifiers that should receive
...
export const MODELS_DEV_PROVIDER_MAP: Record<string, string[]> = {
  ...
  // that is the historical alias pair. But DardcorCode's catalog & combo targets
  ...
  // Additional providers that may overlap with DardcorCode
  alibaba: ["ali", "alibaba"],
...
/**
 * Map a models.dev provider ID to DardcorCode provider IDs.
...
/**
 * Transform models.dev raw data → DardcorCode PricingByProvider format.
 *
 * models.dev costs are already in $/1M tokens (same as DardcorCode format).
...
export function transformModelsDevToPricing(raw: ModelsDevData): PricingByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      // Write to ALL mapped DardcorCode providers
