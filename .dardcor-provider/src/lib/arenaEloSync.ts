...
/**
 * Maps Arena leaderboard categories to DardcorCode task-type categories.
 *
...
/**
 * DardcorCode model aliases: canonical name → known aliases.
 * Creates additional DB entries for each alias so that models
 * are findable under any name DardcorCode uses internally.
 */
 ...
 *
 * Leaderboard categories are mapped to DardcorCode task types:
 * - "text" → default, review, documentation, debugging
 * - "code" → coding
 *
 * Known DardcorCode model aliases are also expanded into additional entries.
 *
