...
const G_KEY = "__dardcor-codeMcpBridges";
...
function smartFilterText(text) {
  ...
  if (out.length > MAX_TEXT_CHARS) {
    ...
    out = `${head}\n\n... [truncated ${text.length - head.length} chars by dardcor-code bridge. Page is large; ask user to scroll/navigate to a specific section, or click an element with the refs shown above]`;
  }
  return out;
}
...
function collapseRepeated(text) {
  ...
  while (i < lines.length) {
    ...
    if (groupLen >= COLLAPSE_THRESHOLD) {
      ...
      out.push(`${indent}... [${groupLen - COLLAPSE_KEEP_HEAD - COLLAPSE_KEEP_TAIL} similar "${role}" items omitted by dardcor-code bridge]`);
