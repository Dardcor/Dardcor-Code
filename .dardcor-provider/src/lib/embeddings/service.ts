import { handleEmbedding } from "@dardcorCode/open-sse/handlers/embeddings.ts";
import {
  parseEmbeddingModel,
...
} from "@dardcorCode/open-sse/config/embeddingRegistry.ts";
import { errorResponse, unavailableResponse } from "@dardcorCode/open-sse/utils/error.ts";
...
import { handleComboChat } from "@dardcorCode/open-sse/services/combo.ts";
...
import { attachDardcorCodeMetaHeaders } from "@/domain/dardcorCodeResponseMeta";
...
export async function createEmbeddingResponse(
  ...
  if (result.success) {
    ...
    attachDardcorCodeMetaHeaders(responseHeaders, {
      provider,
