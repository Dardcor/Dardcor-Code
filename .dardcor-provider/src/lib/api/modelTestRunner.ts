...
import { withRateLimit } from "@dardcorCode/open-sse/services/rateLimitManager";
import {
  isCreditsExhausted,
  isDailyQuotaExhausted,
} from "@dardcorCode/open-sse/services/accountFallback";
import { looksLikeQuotaExhausted } from "@/shared/utils/classify429";

const INTERNAL_ORIGIN = "http://dardcorCode.internal";
...
export function buildInternalChatRequest(
  ...
  return new Request(`${INTERNAL_ORIGIN}/v1/chat/completions`, {
    ...
    headers: {
      ...
      "X-DardcorCode-Compression": "off",
      "X-Request-Id": `model-test-${randomUUID()}`,
      ...(connectionId ? { "X-DardcorCode-Connection": connectionId } : {}),
    },
...
export async function runSingleModelTest(
  ...
  const testBody = isRerank
    ? {
        model: fullModelStr,
        query: "What is DardcorCode?",
        documents: [
          "DardcorCode routes AI requests across configured providers.",
