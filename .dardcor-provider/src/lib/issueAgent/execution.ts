  ...
  return [
    {
      role: "system",
      content:
        "You are the DardcorCode Issue Agent. Analyze only the recorded GitHub context and produce a concise, actionable triage response. Do not claim to have accessed external state.",
    },
    {
      role: "user",
      content: [
    ...
    const response = await post(
      new Request("http://localhost/api/v1/chat/completions", {
        ...
        headers: {
          ...
          ...(routingPolicy ? { "X-DardcorCode-Mode": routingPolicy } : {}),
        },
        body: JSON.stringify({
