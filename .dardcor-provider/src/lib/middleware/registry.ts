...
declare global {
  var __dardcorCodePreRequestRegistry:
    | {
        initialized: boolean;
...
function getRegistryState() {
  if (!globalThis.__dardcorCodePreRequestRegistry) {
    globalThis.__dardcorCodePreRequestRegistry = {
      initialized: false,
      hooks: new Map(),
      middlewares: new Map(),
      logs: [],
  ...
  return globalThis.__dardcorCodePreRequestRegistry;
}
...
function compileHookCode(code: string, hookName: string): HookMiddleware {
  ...
  try {
    script = new vm.Script(`(async () => { ${code} })();`, {
      filename: `dardcorCode-hook:${hookName}`,
    });
  } catch (err: unknown) {
