 ...
 * Priority: system daemon socket → DardcorCode custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to DardcorCode custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `dardcorCode-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
