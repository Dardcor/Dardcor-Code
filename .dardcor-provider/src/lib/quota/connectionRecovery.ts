...
import { cooldownUntilMs } from "@dardcorCode/open-sse/services/accountFallback.ts";
...
declare global {
  var __dardcorCodeConnRecovery:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getRecoveryState() {
  if (!globalThis.__dardcorCodeConnRecovery) {
    globalThis.__dardcorCodeConnRecovery = { initialized: false, interval: null };
  }
  return globalThis.__dardcorCodeConnRecovery;
}
