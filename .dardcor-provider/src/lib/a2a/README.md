# Dardcor Code A2A Server

> **Agent-to-Agent Protocol v0.3** — Enables any AI agent to use Dardcor Code as an intelligent routing agent via JSON-RPC 2.0.

The A2A Server exposes Dardcor Code as a **first-class agent** that other agents can discover, delegate tasks to, and collaborate with using the [A2A Protocol](https://google.github.io/A2A/).
  ...
  "skills": [
    {
      "id": "smart-routing",
      "name": "Smart Routing",
      "description": "Routes prompts through Dardcor Code intelligent pipeline",
...
Routes prompts through Dardcor Code's intelligent pipeline with full observability.
...
Discovers Dardcor Code agent, sends a task, and processes the result.
"""
import requests
import json
...
```python
"""
LangChain integration — Use Dardcor Code A2A as a custom LLM.
"""
...
An orchestrator agent delegates code generation to Dardcor Code, then passes the output to a review agent.

```python
def coding_pipeline(task: str):
    # Step 1: Generate code via Dardcor Code A2A
    ...
    # Step 2: Review the code via Dardcor Code A2A (different model)
...
Multiple agents share quota through Dardcor Code, using the quota skill to coordinate.
