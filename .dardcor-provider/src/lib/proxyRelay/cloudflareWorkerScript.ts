/**
 * Cloudflare Worker source emitter for the DardcorCode proxy relay.
 *
 ...
 *
 * The DardcorCode variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerUploadRequest(
  ...
  const boundary = `----DardcorCodeCFWorker${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
  const parts: Buffer[] = [
    Buffer.from(
      `--${boundary}${CRLF}` +
...
export function buildCloudflareWorkerScript(relayAuth: string): string {
  ...
  return `// DardcorCode Cloudflare Worker proxy relay — generated at deploy time.
