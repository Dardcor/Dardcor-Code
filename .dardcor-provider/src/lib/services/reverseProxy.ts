 ...
 * can be iframed inside the DardcorCode dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between DardcorCode and the embedded service.
 ...
 * Prevents DardcorCode session cookies and Authorization from leaking upstream.
 */
 ...
 *   DardcorCode origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the DardcorCode iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (DardcorCode controls framing via its own CSP).
