...
/**
 * Providers the `dardcorCode login <provider>` helper can mint a blob for.
 *
...
export function buildGoogleLoopbackHint(
  ...
  return {
    ...
    helperCommand: LOGIN_HELPER_PROVIDERS.has(provider) ? `npx dardcorCode login ${provider}` : null,
  };
}
