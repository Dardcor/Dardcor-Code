 ...
 * browser, not on whatever host serves the DardcorCode dashboard.
 *
 ...
 *   DardcorCode *server's* loopback while the provider redirects the *browser's* loopback.
 */
...
export function buildPkceLoopbackMismatchWarning(provider: string): string {
  ...
  return (
    `DardcorCode is being accessed from a LAN IP, not true localhost. ${provider}'s OAuth app ` +
    `is registered with a fixed loopback redirect (${redirect}) that only resolves on the ` +
    "machine running this browser tab, not on the DardcorCode server — the login will silently " +
    "fail on the provider's side. Open the DardcorCode dashboard from true localhost instead " +
    "(SSH port-forward: ssh -L <port>:127.0.0.1:<port> <user>@<dardcorCode-host>, then browse to " +
