...
import { splitClaudeEffortSuffix } from "@dardcorCode/open-sse/config/providerModels.ts";
...
export function attachReasoningRuleDirective(
  ...
  body._dardcorCodeReasoningRule = {
    id: decision.rule.id,
  ...
  body._dardcorCodeReasoningRouteTrace = {
...
export function applyReasoningRuleDirective(bodyInput: unknown): unknown {
  const source = asRecord(bodyInput);
  const directive = asRecord(source._dardcorCodeReasoningRule);
  if (!directive.id) return bodyInput;
  const body = { ...source };
  delete body._dardcorCodeReasoningRule;
