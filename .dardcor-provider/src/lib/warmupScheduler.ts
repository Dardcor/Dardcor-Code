...
import { runWithProxyContext } from "@dardcorCode/open-sse/utils/proxyFetch";
import { logger } from "@dardcorCode/open-sse/utils/logger";
...
declare global {
  var __dardcorCodeWarmupScheduler: {
    timer: NodeJS.Timeout | null;
    executing: boolean;
    lastFireMinute: number;
  };
}
const STATE = (globalThis.__dardcorCodeWarmupScheduler ??= {
  timer: null,
  executing: false,
  lastFireMinute: -1,
});
...
async function getWarmupHeaders(): Promise<Record<string, string>> {
  const { getClaudeCliHeaders } = await import("@dardcorCode/open-sse/config/providers/shared");
