...
export function getOmpCredentials(providerId: string) {
  ...
  if (!Database) return { hasDardcorCode: false, baseUrl: null, apiKey: null };
  ...
  try {
    ...
    if (row?.data) {
      ...
      return { hasDardcorCode: true, baseUrl: parsed.baseUrl || null, apiKey: parsed.apiKey || null };
    }
    return { hasDardcorCode: false, baseUrl: null, apiKey: null };
  } catch {
    return { hasDardcorCode: false, baseUrl: null, apiKey: null };
  }
}
