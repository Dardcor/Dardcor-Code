...
function ensureMigrationsTable(db: SqliteAdapter): void {
  db.exec(`
    CREATE TABLE IF NOT EXISTS _dardcorCode_migrations (
...
function supportsFts5(db: SqliteAdapter): boolean {
  ...
  try {
    const probeTable = `__dardcorCode_fts5_probe_${crypto.randomUUID().replace(/-/g, "_")}`;
    db.transaction(() => {
...
function getMigrationFiles(): Array<{ version: string; name: string; path: string }> {
  ...
  // would record version=NNN in _dardcorCode_migrations; the rest would never run).
...
function getAppliedVersions(db: SqliteAdapter): Set<string> {
  const rows = db.prepare("SELECT version FROM _dardcorCode_migrations").all() as Array<{
    version: string;
  }>;
...
function getAppliedRecords(db: SqliteAdapter): Array<{ version: string; name: string }> {
  return db
    .prepare("SELECT version, name FROM _dardcorCode_migrations ORDER BY version")
    .all() as Array<{
    version: string;
    name: string;
  }>;
...
function reconcileRenumberedMigrations(
  ...
  for (const compatibility of RENAMED_MIGRATION_COMPATIBILITY) {
    ...
    const legacyRow = db
      .prepare("SELECT version, name FROM _dardcorCode_migrations WHERE version = ? AND name = ?")
    ...
    const targetRow = db
      .prepare("SELECT version FROM _dardcorCode_migrations WHERE version = ?")
      .get(compatibility.toVersion) as { version: string } | undefined;

    const applyRepair = db.transaction(() => {
      if (targetRow) {
        db.prepare("DELETE FROM _dardcorCode_migrations WHERE version = ? AND name = ?").run(
          compatibility.fromVersion,
          compatibility.fromName
        );
      } else {
        db.prepare(
          "UPDATE _dardcorCode_migrations SET version = ?, name = ? WHERE version = ? AND name = ?"
        ).run(
    ...
    const residualRow = db
      .prepare("SELECT version, name FROM _dardcorCode_migrations WHERE version = ?")
    ...
    if (residualRow) {
      ...
      db.prepare("DELETE FROM _dardcorCode_migrations WHERE version = ?").run(
        compatibility.fromVersion
      );
    }
  }
...
function rehomeLegacyVersionSlotMigrations(
  ...
  for (const legacy of LEGACY_VERSION_SLOT_MIGRATIONS) {
    ...
    const applyRepair = db.transaction(() => {
      ...
      db.prepare("UPDATE _dardcorCode_migrations SET version = ? WHERE version = ? AND name = ?").run(
        legacyVersion,
        legacy.version,
        legacy.name
      );
...
export function runMigrations(db: SqliteAdapter, options?: { isNewDb?: boolean }): number {
  ...
  if (mismatches.length > 0) {
    ...
    console.error(
      `[Migration] The version-only tracking will skip these (version already applied), ` +
        `but please report this to the DardcorCode maintainers.`
    );
  }
  ...
  // from the _dardcorCode_migrations table.
  ...
  for (const migration of pending) {
    ...
    const applyMigration = db.transaction(() => {
      ...
      db.prepare("INSERT INTO _dardcorCode_migrations (version, name) VALUES (?, ?)").run(
        migration.version,
        migration.name
      );
    });
    ...
    try {
      ...
      if (message.includes("duplicate column name")) {
        const applyMarkerOnly = db.transaction(() => {
          db.prepare(
            "INSERT OR IGNORE INTO _dardcorCode_migrations (version, name) VALUES (?, ?)"
...
export function getMigrationStatus(db: SqliteAdapter): {
  ...
  const appliedRows = db
    .prepare("SELECT version, name, applied_at FROM _dardcorCode_migrations ORDER BY version")
