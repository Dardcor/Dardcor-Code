...
export function isSqliteDriverUnavailableError(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("Call ensureDbInitialized() at startup") ||
...
function openSqliteDatabase(sqliteFile: string, options?: Record<string, unknown>): SqliteDatabase {
  ...
  throw new Error(
    `[DB] No SQLite driver available for '${sqliteFile}'. ` +
      "Call ensureDbInitialized() at startup. " +
      `Drivers tested: ${syncDrivers}. ` +
  ...
  try {
    ...
    console.warn(
      `[DB] Cannot create data directory '${DATA_DIR}': ${msg}\n` +
        ...
        `[DB]   DATA_DIR=/path/to/writable/dir dardcorCode`
    );
  }
}
...
declare global {
  var __dardcorCodeDb: SqliteAdapter | undefined;
  ...
  var __dardcorCodeDbProbeRestoreCount: number | undefined;
  ...
  var __dardcorCodeDbOomFailureCount: number | undefined;
}

function getDb(): SqliteDatabase | null {
  return globalThis.__dardcorCodeDb ?? null;
}

function setDb(db: SqliteDatabase | null): void {
  if (db) {
    globalThis.__dardcorCodeDb = db;
  } else {
...
export function getDbInstance(): SqliteDatabase {
  ...
  if (!fs.existsSync(sqliteFile) && probeFailureBackups.length > 0) {
    ...
    if (
      (globalThis.__dardcorCodeDbProbeRestoreCount =
        (globalThis.__dardcorCodeDbProbeRestoreCount || 0) + 1) > 3
    ) {
      throw new Error(
  ...
  if (fs.existsSync(sqliteFile)) {
    try {
      ...
      if (
        ...
        if (
          (globalThis.__dardcorCodeDbOomFailureCount =
            (globalThis.__dardcorCodeDbOomFailureCount || 0) + 1) > 3
        ) {
          throw new Error(
  ...
  db.exec(`
    ...
    INSERT OR IGNORE INTO _dardcorCode_migrations (version, name)
 ...
 * can place it in ~/.dardcorCode/runtime/ without blocking a synchronous caller.
 *
