...
const LOCAL_USAGE_MODEL = "dardcorCode/local-usage";
  ...
  try {
    ...
    return {
      preferredConnectionId:
        ...
        readHeader(request, "x-dardcorCode-connection")?.trim() ||
        null,
      preferredProvider: url.searchParams.get("provider")?.trim() || null,
    };
  } catch {
    return {
      preferredConnectionId: readHeader(request, "x-dardcorCode-connection")?.trim() || null,
