 ...
 * Token Plan), where the upstream exposes no API-key usage endpoint — DardcorCode
 * counts the tokens it routed and compares against the known monthly limit.
 * Only reflects traffic that went THROUGH DardcorCode (not the provider's panel).
 */
...
/**
 * Total USD spend DardcorCode has recorded for a single provider connection, across all time
 ...
 * total. Only reflects traffic that went THROUGH DardcorCode, not the provider's own dashboard. Used
