 ...
 */
import { detectMediaParts, type MediaPart } from "@dardcorCode/open-sse/utils/mediaParts";
 ...
 *      DardcorCode itself, OpenRouter, a Gemini-OpenAI-compat URL).
 ...
 *      DardcorCode's own `/v1` so the vision model can be any provider
 *      registered in DardcorCode (`google/gemini-2.0-flash`,
 *      `openrouter/...`, etc.) instead of being limited to OpenAI/Anthropic.
 *   2. `OPENAI_API_URL` env var (legacy)
 *   3. DardcorCode self-loop (`http://localhost:20128/v1`) — auto-detected when
 *      the model uses a known DardcorCode-internal provider (e.g. `kr/`, `if/`,
 ...
 *                that require DardcorCode self-loop routing.
 */
export function resolveVisionBridgeBaseUrl(model?: string): string {
  ...
  // anthropic/), it can only be resolved through DardcorCode's own router,
...
/**
 * Unwrap the detailed-log/diagnostics envelope that some DardcorCode paths attach
 ...
 *      DardcorCode's self-loop when the `stream` default kicks in)
 ...
 * DardcorCode's own self-loop and forceStream providers) reply with a `data:`
...
async function readVisionResponseBody(response: Response): Promise<unknown> {
  try {
    ...
    // that some DardcorCode capture paths attach to provider responses.
...
async function callVisionModelSingle(
  ...
  try {
    ...
    if (isAnthropic) {
      ...
      // DardcorCode itself or any other OpenAI-compatible endpoint instead of
      // hardcoded api.openai.com.
      const baseUrl = resolveVisionBridgeBaseUrl(config.model);

      // When routing through the DardcorCode self-loop (non-standard provider),
      // keep the full provider-prefixed model ID so DardcorCode can resolve the
      ...
      // When routing through DardcorCode's own API, omit the vision-bridge
      // guardrail on the sub-request to prevent infinite recursion.
      // Use sk_dardcorCode as fallback for self-loop if no API key is resolved.
      const selfLoopApiKey = resolvedApiKey || "sk_dardcorCode";
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        // Explicit JSON opt-in: without `Accept: application/json` DardcorCode's
      ...
      if (useFullModelId) {
        headers["x-dardcorCode-disabled-guardrails"] = "vision-bridge";
        ...
        // `sk_dardcorCode` sentinel OR the operator-configured env key), so
        // external clients cannot use it to bypass admission.
        headers["x-dardcorCode-admission-bypass"] = "internal";
        // The admission bypass honors the env key when set (REQUIRE_API_KEY=true
        // deployments) and the `sk_dardcorCode` sentinel otherwise. Force the same
