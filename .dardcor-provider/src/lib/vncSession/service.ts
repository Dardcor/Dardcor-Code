...
const LABEL = "com.dardcorCode.browser-login";
...
export function sessionKey(sessionId: string): string {
  return `dardcorCode-browser-login-${sessionId.replace(/[^a-zA-Z0-9]/g, "").slice(0, 20)}`;
}
...
export async function startSession(connectionId: string): Promise<VncSession> {
  ...
  try {
    ...
    if (result.code !== 0) {
      ...
      const buildHint =
        VNC_CONFIG.image === "dardcorCode-vnc-chromium:local" &&
        /pull access denied|unable to find image|not found/i.test(message)
          ? " Build it with: docker build -t dardcorCode-vnc-chromium:local docker/vnc-browser/chromium"
          : "";
