...
export interface VncProviderEntry {
  ...
  url: string;
  /** Canonical DardcorCode credential contract for this provider. */
...
const profileRoot =
  ...
  `${process.env.HOME || "/tmp"}/.dardcorCode/browser-login-profiles`;
...
export const VNC_CONFIG = {
   ...
   */
  image: process.env.OMNIROUTE_VNC_IMAGE || "dardcorCode-vnc-chromium:local",
