...
import {
  ...
  refreshCopilotToken,
} from "@dardcorCode/open-sse/services/tokenRefresh.ts";
...
declare global {
  var __dardcorCodeTokenHC:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null; sweeping: boolean }
    | undefined;
}
function getHCState() {
  if (!globalThis.__dardcorCodeTokenHC) {
    globalThis.__dardcorCodeTokenHC = {
      initialized: false,
      interval: null,
      initTimeout: null,
      sweeping: false,
  ...
  return globalThis.__dardcorCodeTokenHC;
}
...
export async function checkConnection(conn) {
  ...
  // Codex). 9router did not have this background sweep; it was introduced in DardcorCode
