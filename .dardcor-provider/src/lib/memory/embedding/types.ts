...
export interface EmbeddingResolution {
  /** Active source after resolveEmbeddingSource(settings). null = none available → degrades to FTS5. */
  ...
  dimensions: number | null;
  /** Unique signature used as vectorStore key to detect model changes. */
  signature: string; // ${source}:${model}:${dim}
  /** Reason for the choice (UI displays in Engine status). */
  reason: string; // e.g. "provider openai with configured key"
}
