 ...
 * Enforcing one here rejected conversations before DardcorCode's own compression pipeline — the
 ...
 * entries, so the metric an operator caps is partly manufactured by DardcorCode itself.
 *
 ...
 * synchronous JavaScript turn, making acquisition atomic within an DardcorCode process.
...
function isInternalAdmissionBypass(request: Request): boolean {
  ...
  // the local `sk_dardcorCode` sentinel OR the operator-configured env key
  // (`OMNIROUTE_API_KEY` / `ROUTER_API_KEY`, #1350) so REQUIRE_API_KEY=true
