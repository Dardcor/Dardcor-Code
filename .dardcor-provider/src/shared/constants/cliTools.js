...
export const CLI_TOOLS = {
  ...
  amp: {
    ...
    notes: [
      { type: "info", text: "Use DRouter model aliases to keep Amp shorthand mappings stable across provider updates." },
    ...
    guideSteps: [
      ...
      { step: 5, title: "Add Shorthands", desc: "Map Amp shorthand names such as g25p or cs45 to DRouter aliases in your local config." },
    ],
    codeBlock: {
      language: "bash",
...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Alibaba Qwen Code CLI — supports OpenAI, Anthropic & Gemini providers via DRouter",
    docsUrl: "https://qwenlm.github.io/qwen-code-docs/en/users/configuration/model-providers/",
    configType: "guide",
    defaultCommand: "qwen",
    notes: [
      { type: "info", text: "Qwen Code supports multiple provider types (openai, anthropic, gemini) via modelProviders in settings.json. DRouter works as an OpenAI-compatible endpoint." },
      { type: "info", text: "Any model available in DRouter can be used — not just Qwen models. Select from Qwen, Claude, Gemini, GPT, and more." },
      { type: "warning", text: "Config path: Linux/macOS ~/.qwen/settings.json • Windows %USERPROFILE%\\.qwen\\settings.json" },
      { type: "error", text: "Qwen OAuth free tier was discontinued on 2026-04-15. Use DRouter with alicode/openrouter/anthropic/gemini providers instead." },
    ],
  ...
  "deepseek-tui": {
    ...
    notes: [
      { type: "info", text: "DeepSeek TUI uses ~/.deepseek/config.toml for configuration. DRouter will update the provider to 'openai' mode with your base_url, api_key, and model." },
  ...
  "grok-build": {
    ...
    notes: [
      {
        type: "info",
        text: "Grok Build uses ~/.grok/config.toml. DRouter writes a [model.dardcor-code] custom model and sets it as the default.",
      },
      {
        type: "info",
...
devin --version`,
  ...
  opendesign: {
    ...
    configType: "guide",
    notes: [
      { type: "info", text: "OpenDesign ships as a plugin/skills pack installed into Claude Code, Cursor, OpenAI Codex, Gemini CLI, or OpenCode. It inherits the host agent's model config, so once your host points at DRouter, /opendesign design sessions route through DRouter automatically — no extra env vars needed." },
    ...
    guideSteps: [
      ...
      { step: 2, title: "No config needed", desc: "OpenDesign runs inside your host agent and uses its model config. If the host already routes through DRouter, /opendesign traffic does too." },
