...
import { getClaudeCodeDefaultModels } from "@dardcorCode/open-sse/config/providerRegistry";
...
export const CLI_TOOLS: Record<string, CliCatalogEntry> = {
  claude: {
    ...
    color: "#D97757",
    description: "Anthropic Claude Code CLI — ANTHROPIC_BASE_URL points to DardcorCode",
  ...
  codex: {
    ...
    color: "#10A37F",
    description: "OpenAI Codex CLI — OpenAI-compatible base URL targets DardcorCode",
  ...
  opencode: {
    ...
    notes: [
      ...
      {
        type: "warning",
        text: 'Thinking variant example: opencode run "implement this feature" --model dardcorCode/claude-sonnet-4-5-thinking --variant high',
      },
    ],
    guideSteps: [
  ...
  hermes: {
    ...
    guideSteps: [
      ...
      { step: 4, title: "Select Model", type: "modelSelector" },
      {
        ...
        desc: "Use the JSON block below as the OpenAI-compatible provider definition for DardcorCode.",
      },
    ],
    codeBlock: {
      language: "json",
  ...
  qwen: {
    ...
    color: "#10B981",
    description: "Qwen Code CLI — current V4 OpenAI-compatible model provider via DardcorCode",
    ...
    notes: [
      {
        type: "info",
        text: "DardcorCode is registered under modelProviders.openai using Qwen Code's current bare-array V4 format.",
      },
      {
        type: "info",
...
OPENAI_API_KEY: "{{apiKey}}"`,
  ...
  omp: {
    ...
    description: "Oh My Pi terminal coding agent via DardcorCode",
    configType: "custom",
    category: "agent",
    vendor: "OSS",
    acpSpawnable: true,
    ...
    notes: [
      {
        type: "info",
        text: "Oh My Pi reads custom OpenAI-compatible providers from ~/.omp/agent/models.yml. DardcorCode adds itself as a provider with auto-discovery — models appear automatically in omp's /model menu.",
      },
      {
        type: "warning",
  ...
  letta: {
    ...
    notes: [
      {
        type: "info",
        text: "Letta CLI uses pi-ai which sends OpenAI-compatible requests. DardcorCode configures it as an OpenAI provider with custom base URL.",
      },
      {
        type: "info",
        text: "CLI (Local Mode): DardcorCode auto-configures ~/.letta/lc-local-backend/providers/auth.json. Use 'letta --info' to check if local mode is enabled.",
      },
      {
        type: "warning",
