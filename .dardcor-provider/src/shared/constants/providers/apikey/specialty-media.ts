...
export const APIKEY_PROVIDERS_SPECIALTY = {
  nlpcloud: {
    ...
    authHint:
      "Use your NLP Cloud API key in Authorization: Token <key>. DardcorCode targets the chatbot endpoint on https://api.nlpcloud.io/v1/gpu/<model>/chatbot by default.",
    apiHint:
      "NLP Cloud uses a proprietary chatbot API instead of OpenAI chat/completions. DardcorCode adapts OpenAI messages to input/context/history and exposes a local catalog of supported chatbot models.",
    hasFree: true,
  ...
  runwayml: {
    ...
    authHint:
      "Use your Runway API key in Authorization: Bearer <key>. DardcorCode targets the current Runway API at https://api.dev.runwayml.com/v1 and sends the required X-Runway-Version header automatically.",
    apiHint:
      "Runway video generation is task-based. DardcorCode submits text-to-video or image-to-video jobs, polls /v1/tasks/{id}, and normalizes the finished video outputs back into the OpenAI-like /v1/videos/generations response.",
  },
  kie: {
    id: "kie",
    alias: "kie",
  ...
  segmind: {
    ...
    authHint:
      "Use your Segmind API key in the x-api-key header. DardcorCode targets https://api.segmind.com/v1/<model> and returns the generated image/video bytes directly.",
    apiHint:
      "Segmind exposes 200+ hosted image and video models (Flux, SDXL, SD3, Kandinsky, Wan, Hunyuan, LTX, Kling, ...) under a single POST /v1/<model> REST call per model. DardcorCode ships a curated starter subset for each modality; more models can be routed by using their Segmind slug as the model id.",
    hasFree: true,
  ...
  tinyfish: {
    ...
    notice: {
      text: "Fetch does not use TinyFish credits. Submit up to 10 URLs per request (DardcorCode fetches one URL per call).",
