...
export const NOAUTH_PROVIDERS = {
  ...
  auggie: {
    ...
    isLocalCli: true,
    freeNote:
      "Local passthrough — runs the Augment CLI (`auggie`) on this machine. Auth is handled by `auggie login`, not DardcorCode.",
    authHint:
      "No API key stored by DardcorCode. Install the Auggie CLI and run `auggie login` on this machine, then DardcorCode spawns it locally for each request.",
    notice: {
      text: "Augment (Auggie CLI) requires the `auggie` binary installed and authenticated locally (`auggie login`). DardcorCode spawns it as a subprocess and never sees or stores your Augment credentials.",
    },
  },
  aihorde: {
    id: "aihorde",
...
// upstream path runs through DardcorCode's proxy-aware global fetch. Providers
