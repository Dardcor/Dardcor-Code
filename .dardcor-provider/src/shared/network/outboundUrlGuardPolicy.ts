...
// of `./outboundUrlGuard.ts` so the CLI (`dardcorCode setup-opencode`, loaded via tsx with no
...
// cloud-metadata endpoints stay blocked. Defaults ON (DardcorCode is local-first); operators
 ...
 * (DardcorCode is local-first — local OpenAI-compatible providers should validate out of the
