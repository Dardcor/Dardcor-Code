...
const globalState = globalThis as typeof globalThis & {
  __dardcorCodeModelSyncInternalAuthToken?: string;
};
...
function getInternalAuthToken(): string {
  if (!internalAuthToken) {
    internalAuthToken = globalState.__dardcorCodeModelSyncInternalAuthToken || randomUUID();
    globalState.__dardcorCodeModelSyncInternalAuthToken = internalAuthToken;
  }
...
export function isModelSyncInternalRequest(request: { headers: Headers }): boolean {
  if (!internalAuthToken && globalState.__dardcorCodeModelSyncInternalAuthToken) {
    internalAuthToken = globalState.__dardcorCodeModelSyncInternalAuthToken;
  }
 ...
 * @param apiBaseUrl — internal base URL to call DardcorCode's own API
