...
export interface ClaudeDiscoverySnippetInput {
  /** DardcorCode root — Claude Code appends `/v1/messages` itself, so no `/v1` suffix. */
  baseUrl: string;
...
/**
 * Build the `settings.json` fragment that points Claude Code at this DardcorCode and
