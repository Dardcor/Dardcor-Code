...
/**
 * Identifies only entries owned by DardcorCode. Generic custom endpoints are not
...
export const isManagedQwenCodeModel = (value: unknown): boolean => {
  ...
  if (value.envKey === QWEN_CODE_ENV_KEY || value.id === "dardcorCode") return true;

  return (
    typeof value.name === "string" &&
    value.name.endsWith(" (DardcorCode)") &&
...
export const buildQwenCodeModel = ({
  ...
  return {
    id: normalizedModel,
    name: `${String(modelName || normalizedModel).trim()} (DardcorCode)`,
...
/**
 * Merge one DardcorCode model into Qwen Code's current V4 settings contract.
...
export const mergeQwenCodeSettings = (
  ...
  if (next.selectedProvider === "dardcorCode") delete next.selectedProvider;

  return next;
};

export const findDardcorCodeQwenCodeModel = (settings: unknown): JsonRecord | undefined => {
...
export const hasDardcorCodeQwenCodeConfig = (settings: unknown): boolean =>
  findDardcorCodeQwenCodeModel(settings) !== undefined;

/** Remove only DardcorCode-owned Qwen Code entries and selection state. */
...
export const mergeQwenCodeEnv = (existing: unknown, apiKey: unknown): string => {
  ...
  lines.push(`${QWEN_CODE_ENV_KEY}=${JSON.stringify(String(apiKey || "sk_dardcorCode"))}`);
  return `${lines.join("\n")}\n`;
};
