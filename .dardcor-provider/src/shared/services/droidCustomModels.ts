/**
 * Build the DardcorCode `customModels` entries for Factory Droid's `settings.json`.
 *
 * Ported from upstream PR decolua/9router#618 (author Anurag Saxena) — multi-model
 * support for the Factory Droid CLI tool. Adapted to DardcorCode branding
 * (`custom:DardcorCode-<i>`) and extracted as a pure helper so the route-handler
...
/** True when a `customModels` entry was written by DardcorCode (any index). */
export function isDardcorCodeCustomModel(entry: { id?: unknown } | null | undefined): boolean {
  return typeof entry?.id === "string" && entry.id.startsWith("custom:DardcorCode");
}
