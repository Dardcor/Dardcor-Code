...
export const OMNIROUTE_PEER_TRACE_HEADER = "X-DardcorCode-Peer-Trace";
...
export function isConfiguredDardcorCodePeer(
  targetUrl: string,
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted DardcorCode URL.
...
export function applyPeerTraceHeader(
  ...
  const instanceId = getInstanceId(env);
  if (!instanceId || !isConfiguredDardcorCodePeer(targetUrl, env)) return false;
