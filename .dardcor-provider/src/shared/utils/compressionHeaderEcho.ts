 ...
 *
 * When a request carries `x-dardcorCode-compression`, docs promise the response echoes
 * `X-DardcorCode-Compression: <mode>; source=<source>`. Internal paths (idempotency
...
const COMPRESSION_REQUEST_HEADER = "x-dardcorCode-compression";
...
/**
 * Wrap a Response so it carries `X-DardcorCode-Compression: <mode>; source=request-header`
 * when the request supplied `x-dardcorCode-compression` and the inner pipeline did not
