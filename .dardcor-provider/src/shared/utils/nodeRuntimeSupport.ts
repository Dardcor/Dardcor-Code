/**
 * Shared Node.js runtime support policy for DardcorCode.
 *
...
export function getNodeRuntimeWarning(version: string = process.versions.node): string | null {
  ...
  if (support.reason === "unreleased-major") {
    return `Node.js ${support.nodeVersion} is outside the supported LTS lines. DardcorCode currently supports Node.js 22.x, 24.x, 25.x, and 26.x.`;
  }

  return `Node.js ${support.nodeVersion} is outside DardcorCode's approved secure runtime policy.`;
}
