...
export async function GET(request) {
  try {
    ...
    const { getQuotaMonitorSummary } = await import("@dardcorCode/open-sse/services/quotaMonitor.ts");
    const { getActiveSessions } = await import("@dardcorCode/open-sse/services/sessionManager.ts");
