...
import { buildErrorBody, sanitizeErrorMessage } from "@dardcorCode/open-sse/utils/error.ts";
  ...
  try {
    if (second === "start" && !action) {
      ...
      return NextResponse.json({
        ...
        note:
          "The viewer is loopback-only. Open it on the DardcorCode host or forward its port over SSH, then harvest the session.",
      });
    }
