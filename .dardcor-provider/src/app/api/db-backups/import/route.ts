...
import { setSystemPromptConfig } from "@dardcorCode/open-sse/services/systemPrompt.ts";
...
// Required tables that must exist in a valid DardcorCode database
...
export async function POST(request: Request) {
  ...
  try {
    ...
    tmpPath = path.join(os.tmpdir(), `dardcorCode-import-${Date.now()}.sqlite`);
    ...
    try {
      ...
      if (missingTables.length > 0) {
        return NextResponse.json(
          {
            error: `Invalid DardcorCode database. Missing tables: ${missingTables.join(", ")}`,
          },
          { status: 400 }
        );
      }
