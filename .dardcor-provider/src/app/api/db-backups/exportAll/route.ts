...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `dardcorCode-export-${timestamp}`);
    const zipPath = path.join(os.tmpdir(), `dardcorCode-full-backup-${timestamp}.zip`);
    ...
    try {
      ...
      const metadata = {
        ...
        format: "dardcorCode-full-backup-v1",
        contents: [
      ...
      return new NextResponse(archiveBuffer, {
        ...
        headers: {
          ...
          "Content-Disposition": `attachment; filename="dardcorCode-full-backup-${timestamp}.tar.gz"`,
