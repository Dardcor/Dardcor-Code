/**
 * GET/POST /api/services/dario/admin/import-from-dardcorCode
 *
 * Imports an existing DardcorCode `claude` provider connection's OAuth tokens
 * directly into Dario's account store, skipping the interactive browser OAuth
 * flow entirely. This works because DardcorCode's native `claude` provider and
 ...
 * PKCE/token-exchange ourselves, since DardcorCode's own decrypt() already
 ...
 * the file we stop+start the DardcorCode-managed supervisor to force a clean
...
function safeAliasFromSource(email: string | null | undefined, connectionId: string): string {
  const base = (email || connectionId || "dardcorCode").toLowerCase();
  const cleaned = base.replace(/[^a-z0-9_.-]/g, "-").replace(/^[^a-z0-9]+/, "");
  const alias = cleaned || "dardcorCode";
  return `dardcorCode-${alias}`.slice(0, 64);
}
