  ...
  try {
    ...
    if (ADOBE_FIREFLY_SLUGS.has(providerSlug)) {
      const { startAdobeFireflyBrowserLogin } =
        await import("@dardcorCode/open-sse/services/adobeFireflyBrowserLogin.ts");
    ...
    const { inAppLoginService } = await import("@dardcorCode/open-sse/services/inAppLoginService.ts");
