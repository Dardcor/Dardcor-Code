...
import { getRegistryEntry } from "@dardcorCode/open-sse/config/providerRegistry.ts";
...
import { getStaticQoderModels } from "@dardcorCode/open-sse/services/qoderCli.ts";
import { deriveConfigFromRegistryModelsUrl } from "./discoveryConfig";
import { resolveZedModels } from "@dardcorCode/open-sse/shared/zedAuth.ts";
import {
  fetchGitHubCopilotModels,
  fetchGheCopilotModels,
} from "@dardcorCode/open-sse/services/githubCopilotModels.ts";
import { fetchKiroAvailableModels } from "@dardcorCode/open-sse/services/kiroModels.ts";
import {
  buildGlmCodingHeaders,
  buildGlmModelsUrl,
} from "@dardcorCode/open-sse/config/glmProvider.ts";
import { getImageProvider } from "@dardcorCode/open-sse/config/imageRegistry.ts";
import { getVideoProvider } from "@dardcorCode/open-sse/config/videoRegistry.ts";
import {
  discoverBedrockNativeModels,
  isBedrockNativeApiError,
} from "@dardcorCode/open-sse/services/bedrock.ts";
import {
  discoverPromptQlModels,
  PROMPTQL_FALLBACK_MODELS,
} from "@dardcorCode/open-sse/services/promptqlModels.ts";
import {
  discoverNotionWebModels,
  NOTION_WEB_FALLBACK_MODELS,
} from "@dardcorCode/open-sse/services/notionWebModels.ts";
import {
  AZURE_AI_DEFAULT_BASE_URL,
  buildAzureAiModelsUrl,
} from "@dardcorCode/open-sse/config/azureAi.ts";
import {
  DATAROBOT_DEFAULT_BASE_URL,
  buildDataRobotCatalogUrl,
  isDataRobotDeploymentUrl,
} from "@dardcorCode/open-sse/config/datarobot.ts";
import { OCI_DEFAULT_BASE_URL, buildOciModelsUrl } from "@dardcorCode/open-sse/config/oci.ts";
import {
  SAP_DEFAULT_BASE_URL,
  buildSapModelsUrl,
  getSapResourceGroup,
} from "@dardcorCode/open-sse/config/sap.ts";
import {
  WATSONX_DEFAULT_BASE_URL,
  buildWatsonxModelsUrl,
} from "@dardcorCode/open-sse/config/watsonx.ts";
import { getEmbeddingProvider } from "@dardcorCode/open-sse/config/embeddingRegistry.ts";
import { getRerankProvider } from "@dardcorCode/open-sse/config/rerankRegistry.ts";
import {
  getSpeechProvider,
  getTranscriptionProvider,
} from "@dardcorCode/open-sse/config/audioRegistry.ts";
import {
...
import { fetchRaycastModels } from "@dardcorCode/open-sse/services/raycast.ts";
import { runWithProxyContext } from "@dardcorCode/open-sse/utils/proxyFetch.ts";
import {
  type JsonRecord,
  asRecord,
  toNonEmptyString,
...
export async function GET(
  ...
  try {
    ...
    const buildDiscoveryErrorFallbackResponse = (
      ...
      // the local/cached catalog DardcorCode ships instead of surfacing a raw 503.
    ...
    if (provider === "vertex" || provider === "vertex-partner") {
      ...
      try {
        const { parseSAFromApiKey, getAccessToken } =
          await import("@dardcorCode/open-sse/executors/vertex.ts");
        if (accessToken) {
