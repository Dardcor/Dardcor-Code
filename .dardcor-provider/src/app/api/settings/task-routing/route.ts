...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const config =
      validation.data as import("@dardcorCode/open-sse/services/taskAwareRouter.ts").TaskRoutingConfig;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (actionRequest.action === "detect") {
      const { detectTaskType } = await import("@dardcorCode/open-sse/services/taskAwareRouter.ts");
