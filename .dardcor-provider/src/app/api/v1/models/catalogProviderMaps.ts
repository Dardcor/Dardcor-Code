...
import { parseModel, resolveCanonicalProviderModel } from "@dardcorCode/open-sse/services/model";
...
/** True when `${prefix}/__dardcorCode_probe__` parses back to the given providerId. */
export function prefixRoutesToProvider(prefix: string, providerId: string): boolean {
  const parsed = parseModel(`${prefix}/__dardcorCode_probe__`);
