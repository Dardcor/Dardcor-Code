...
import { handleAudioTranscription } from "@dardcorCode/open-sse/handlers/audioTranscription.ts";
import {
...
import { attachDardcorCodeMetaToResponse } from "@/domain/dardcorCodeResponseMeta";
...
import { log } from "@dardcorCode/open-sse/utils/logger.ts";
...
async function transcribeWithModel(
  ...
  if (response?.ok) {
    ...
    response = attachDardcorCodeMetaToResponse(response, {
      provider,
      model: resolvedModel,
      costUsd: 0,
