...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasDardcorCode: !!(ymlProvider || creds.hasDardcorCode),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const keyRef = apiKey || "sk_dardcorCode";
    ...
    // 2. Write auth_credentials — so omp sees dardcorCode as "logged in"
    saveOmpCredentials(PROVIDER_ID, keyRef, normalizedBaseUrl);

    return NextResponse.json({
      success: true,
      message:
        "Oh My Pi settings applied! Run omp and all DardcorCode models appear under dardcorCode in /model.",
      configPath: getOmpModelsYmlPath(),
    });
  } catch (error) {
    return NextResponse.json(
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "DardcorCode removed from Oh My Pi",
    });
  } catch (error) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(error) } },
