...
// Check if DardcorCode OpenAI-compatible provider is configured
const hasDardcorCodeConfig = (auth) => {
  if (!auth) return false;
  const routerEntry = auth["openai-compatible"] || auth["dardcorCode"];
  if (!routerEntry) return false;
  const baseUrl = routerEntry.baseUrl || routerEntry.baseURL || "";
  return (
    baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("dardcorCode")
  );
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      hasDardcorCode: hasDardcorCodeConfig(auth),
      authPath: AUTH_PATH,
    });
  } catch (error) {
...
// POST - Configure Kilo Code to use DardcorCode as OpenAI-compatible provider
export async function POST(request) {
  ...
  try {
    ...
    // Add/update DardcorCode as openai-compatible provider
    auth["openai-compatible"] = {
      type: "api-key",
      apiKey: apiKey || "sk_dardcorCode",
...
// DELETE - Remove DardcorCode config from Kilo
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DardcorCode provider
    delete auth["openai-compatible"];
    delete auth["dardcorCode"];
    ...
    return NextResponse.json({
      success: true,
      message: "DardcorCode settings removed from Kilo Code",
    });
  } catch (error) {
