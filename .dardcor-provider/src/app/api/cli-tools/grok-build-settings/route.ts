...
// [model.dardcorCode] ... until the next [section] header or EOF
...
const PREV_DEFAULT_RE = /^# dardcorCode-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
 ...
 * only the flat string fields DardcorCode itself writes.
 */
...
const buildModelSection = (model: string, baseUrl: string, apiKey: string): string => {
  const lines = [
    ...
    `name = "DardcorCode"`,
    `description = "Routed via DardcorCode gateway"`,
...
/** Insert/replace the `[model.dardcorCode]` section, preserving the rest of the file. */
...
const rememberPrevDefault = (toml: string): string => {
  ...
  const marker = `# dardcorCode-prev-default = "${current}"\n`;
...
const hasDardcorCodeConfig = (modelCfg: GrokModelSection | null): boolean =>
...
// GET — check Grok Build CLI and return current [model.dardcorCode] config
...
// POST — write the [model.dardcorCode] section into ~/.grok/config.toml and set it default
...
// DELETE — remove the [model.dardcorCode] section and restore the previous default
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "DardcorCode model slot removed from Grok Build",
    });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
