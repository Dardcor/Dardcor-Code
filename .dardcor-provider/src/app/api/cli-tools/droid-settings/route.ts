...
// Check if settings has DardcorCode customModels.
// Multi-model entries are stored as `custom:DardcorCode-0`, `custom:DardcorCode-1`, …
// (Ported from upstream PR decolua/9router#618.)
const hasDardcorCodeConfig = (settings: any) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(isDardcorCodeCustomModel);
};
...
// POST - Update DardcorCode customModels (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Remove every existing DardcorCode config (multi-model: index 0..N)
    settings.customModels = settings.customModels.filter((m) => !isDardcorCodeCustomModel(m));

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Build and prepend DardcorCode entries (one per requested model)
...
// DELETE - Remove DardcorCode customModels only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DardcorCode customModels (every index, multi-model)
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter((m) => !isDardcorCodeCustomModel(m));
