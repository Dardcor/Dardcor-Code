...
// POST — write DardcorCode settings to Pi config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge DardcorCode settings (pi uses OpenAI-compatible config)
...
// DELETE — remove DardcorCode settings from Pi config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Pi DardcorCode settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
