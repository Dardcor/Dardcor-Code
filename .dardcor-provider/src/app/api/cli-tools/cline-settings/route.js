...
const hasDardcor CodeConfig = (globalState) => {
  if (!globalState) return false;
  const isOpenAi =
    globalState.actModeApiProvider === "openai" || globalState.planModeApiProvider === "openai";
  const baseUrl = globalState.openAiBaseUrl || "";
  return isOpenAi && (baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("dardcor-code"));
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasDardcor Code: hasDardcor CodeConfig(globalState),
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Dardcor Code settings removed from Cline" });
  } catch (error) {
