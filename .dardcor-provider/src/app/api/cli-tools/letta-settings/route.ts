...
const getBackupPath = () =>
  path.join(getLocalBackendDir(), "providers", "auth.json.dardcorCode-backup");
...
// ── Check if a base_url points to DardcorCode ──────────────────────────────
const isDardcorCodeUrl = (baseUrl) => {
  if (!baseUrl) return false;
  return baseUrl.includes(":20128") || baseUrl.includes(":3000") || baseUrl.includes("dardcorCode");
};

// ── Check if DardcorCode is configured ─────────────────────────────────────
const hasDardcorCodeConfig = (authFile) => {
  if (!authFile?.providers) return false;
  const provider = authFile.providers[PROVIDER_NAME];
  if (!provider) return false;
  return isDardcorCodeUrl(provider.base_url);
};
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Detect if lmstudio is already configured for a non-DardcorCode endpoint
    let lmstudioConflict = false;
    if (provider && !isDardcorCodeUrl(provider.base_url)) {
    ...
    return NextResponse.json({
      installed: true,
      config: authFile,
      hasDardcorCode: hasDardcorCodeConfig(authFile),
      lmstudioConflict,
...
// ── POST - Apply DardcorCode as LM Studio provider + switch to local mode ──
/**
 ...
 * up a non-DardcorCode provider before it is overwritten. Extracted to keep POST under
 * the complexity gate.
 */
...
> {
  ...
  if (existingProvider && !isDardcorCodeUrl(existingProvider.base_url) && !overwrite) {
  ...
  if (existingProvider && !isDardcorCodeUrl(existingProvider.base_url)) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ── 1-2. Read auth.json, guard non-DardcorCode conflicts, back up before overwrite ──
    ...
    // Clean up legacy lc-dardcorCode provider if present
    if (authFile.providers?.["lc-dardcorCode"]) {
      delete authFile.providers["lc-dardcorCode"];
    }
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta CLI, then use /model to select a DardcorCode model.",
      needsRestart: true,
    });
  } catch (error) {
    return NextResponse.json(
...
// ── DELETE - Remove DardcorCode configuration ──────────────────────────────
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const message = restored
      ? "DardcorCode config removed. Your original LM Studio provider has been restored. Restart Letta CLI to take effect."
      : "DardcorCode config removed. Restart Letta CLI to take effect.";
