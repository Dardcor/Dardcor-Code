...
// POST — write DardcorCode settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge DardcorCode settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove DardcorCode settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Smelt DardcorCode settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
