...
// Check if settings has DardcorCode config
const hasDardcorCodeConfig = (settings: any) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["dardcorCode"];
};
...
// POST - Update DardcorCode settings (merge with existing settings)
export async function POST(request: Request) {
  ...
  try {
    ...
    settings.agents.defaults.model.primary = `dardcorCode/${model}`;

    // Update models.providers.dardcorCode
    settings.models.providers["dardcorCode"] = {
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DardcorCode from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["dardcorCode"];
    ...
    // Reset agents.defaults.model.primary if it uses dardcorCode
    if (settings.agents?.defaults?.model?.primary?.startsWith("dardcorCode/")) {
