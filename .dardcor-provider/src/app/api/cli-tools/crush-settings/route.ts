...
import { sanitizeErrorMessage } from "@dardcorCode/open-sse/utils/error.ts";
...
// the `dardcorCode setup-crush` CLI command agree on one canonical location.
...
/**
 * Crush's config uses a `providers.<id>` map. DardcorCode is registered under
 * the `dardcorCode` provider id as an `openai-compat` provider — same shape
...
const hasDardcorCodeConfig = (settings: Record<string, unknown> | null): boolean => {
  if (!settings) return false;
  const providers = settings.providers as Record<string, unknown> | undefined;
  const dardcorCode = providers?.dardcorCode as Record<string, unknown> | undefined;
  return (
    !!dardcorCode &&
    dardcorCode.type === "openai-compat" &&
    typeof dardcorCode.base_url === "string" &&
    dardcorCode.base_url.length > 0
  );
};
...
// POST — write DardcorCode settings to crush.json (providers.dardcorCode)
export async function POST(request: Request) {
  ...
  try {
    ...
    const provider: CrushProvider = {
      ...
      api_key: apiKey,
      models: [{ id: model, name: `DardcorCode: ${model}`, context_window: DEFAULT_CONTEXT_WINDOW }],
    };
...
// DELETE — remove DardcorCode provider from Crush config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Crush DardcorCode settings removed" });
  } catch (err) {
    return NextResponse.json({ error: { message: sanitizeErrorMessage(err) } }, { status: 500 });
  }
}
