  ...
  try {
    switch (toolId) {
      ...
      case "opencode":
        ...
        // OpenCode reads ~/.config/opencode/opencode.json — write the DardcorCode settings there.
...
async function saveContinueConfig({ baseUrl, apiKey, model }) {
  ...
  // Build the DardcorCode model entry
  ...
  // Check if DardcorCode entry already exists and update it, or add new
  const existingIdx = models.findIndex(
    (m) =>
      m &&
      (m.dardcorCodeManaged === true ||
        normalizeApiBase(m.apiBase) === normalizedBaseUrl.toLowerCase() ||
        normalizeApiBase(m.apiBase).includes("dardcorCode") ||
 ...
 * keys, but make sure the DardcorCode provider entry is present and selected.
 */
async function saveHermesConfig({ baseUrl, apiKey, model }) {
  ...
  const nextConfig = {
    ...
    providers: {
      ...
      dardcorCode: {
        ...((existingConfig.providers && existingConfig.providers.dardcorCode) || {}),
        base_url: providerBaseUrl,
        api_key:
          apiKey ||
          (existingConfig.providers &&
            existingConfig.providers.dardcorCode &&
            existingConfig.providers.dardcorCode.api_key) ||
          "",
      },
    },
  };
