...
/**
 * Render the DardcorCode config block in DeepSeek TUI TOML format.
...
function renderDeepseekTuiConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# DeepSeek TUI config — managed by DardcorCode (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
...
// POST — write DardcorCode settings to DeepSeek TUI config.toml
...
// DELETE — remove DeepSeek TUI DardcorCode config
