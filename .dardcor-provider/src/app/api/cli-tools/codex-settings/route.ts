...
const parseToml = (content: string) => {
  ...
  content.split("\n").forEach((line) => {
    ...
    // Section header like [model_providers.dardcorCode]
...
// Check if config has DardcorCode settings
const hasDardcorCodeConfig = (config: string | null) => {
  if (!config) return false;
  return (
    config.includes("openai_base_url") ||
    config.includes('model_provider = "dardcorCode"') ||
    config.includes("[model_providers.dardcorCode]")
  );
};
...
// POST - Update DardcorCode settings (merge with existing config)
export async function POST(request: Request) {
  ...
  try {
    ...
    // Update only DardcorCode related fields (api_key goes to auth.json, not config.toml)
    ...
    parsed._root.model_provider = "dardcorCode";
    parsed._sections["model_providers.dardcorCode"] = {
      name: "DardcorCode",
...
// DELETE - Remove DardcorCode settings only (keep other settings)
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DardcorCode related root fields
    delete parsed._root.openai_base_url;

    if (parsed._root.model_provider === "dardcorCode") {
      delete parsed._root.model;
      delete parsed._root.model_provider;
    }

    // Remove dardcorCode provider section
    delete parsed._sections["model_providers.dardcorCode"];
    ...
    return NextResponse.json({
      success: true,
      message: "DardcorCode settings removed successfully",
    });
  } catch (error) {
