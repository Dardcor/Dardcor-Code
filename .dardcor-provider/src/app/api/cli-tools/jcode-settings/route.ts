...
/**
 * Check if the config file contains DardcorCode settings.
 */
const hasDardcorCodeConfig = (settings: Record<string, unknown> | null): boolean => {
  ...
  return (
    ...
    settings._managedBy === "dardcorCode"
  );
};
...
// POST — write DardcorCode settings to jcode config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge DardcorCode settings (jcode uses OpenAI-compatible config)
...
// DELETE — remove DardcorCode settings from jcode config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    // Remove DardcorCode-managed fields
    ...
    return NextResponse.json({ success: true, message: "jcode DardcorCode settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
