...
/**
 * Render the DardcorCode provider block in Forge TOML format.
...
function renderForgeConfig(baseUrl: string, apiKey: string, model: string): string {
  ...
  return [
    "# Forge config — managed by DardcorCode (plan 14)",
    "",
    "[openai]",
    `api_key = "${apiKey}"`,
 ...
 * Looks for the managed-by-DardcorCode marker comment.
 */
...
// POST — write DardcorCode settings to Forge config.toml
...
// DELETE — remove Forge DardcorCode config
