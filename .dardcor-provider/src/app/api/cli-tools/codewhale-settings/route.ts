...
/**
 * Render the DardcorCode config block in CodeWhale TOML format.
...
function renderCodewhaleConfig(baseUrl: string, apiKey: string, model: string): string {
  return [
    "# CodeWhale config — managed by DardcorCode (plan 14)",
    "",
    "[openai]",
    `base_url = "${baseUrl}"`,
    `api_key = "${apiKey}"`,
 ...
 */
const hasDardcorCodeConfig = (content: string | null): boolean => {
  if (!content) return false;
  return content.includes("managed by DardcorCode");
};
...
// POST — write DardcorCode settings to CodeWhale's config.toml (primary), and
...
// DELETE — remove DardcorCode CodeWhale config (primary + legacy, if present)
