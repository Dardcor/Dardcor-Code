 ...
 * If the operator drives the OAuth flow from a different machine (DardcorCode on
...
export function buildRemoteOAuthHint(hostHeader: string | null, port: number): RemoteOAuthHint {
  ...
  return {
    remoteHost: true,
    tunnelCommand: `ssh -L ${port}:127.0.0.1:${port} <user>@<dardcorCode-host>`,
    message:
      `DardcorCode appears to be running on a remote host (${hostHeader}). ` +
