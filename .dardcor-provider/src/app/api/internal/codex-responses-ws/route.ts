...
import { z } from "zod";
import { CodexExecutor } from "@dardcorCode/open-sse/executors/codex.ts";
...
import { logger } from "@dardcorCode/open-sse/utils/logger.ts";
import { resolveProxy } from "@dardcorCode/open-sse/utils/networkProxy.ts";
import { proxyConfigToUrl } from "@dardcorCode/open-sse/utils/proxyDispatcher.ts";
import {
...
function getAuthRequest(body: JsonRecord): Request {
  ...
  const url = new URL(requestUrl, "http://dardcorCode.local");
...
async function prepare(body: JsonRecord) {
  ...
  if (reasoningDecision) {
    ...
    reasoningRouting = isRecord(withDirective._dardcorCodeReasoningRouteTrace)
      ? withDirective._dardcorCodeReasoningRouteTrace
      : null;
    responseBodyWithMemory = applyReasoningRuleDirective(withDirective) as JsonRecord;
    delete responseBodyWithMemory._dardcorCodeReasoningRouteTrace;
  }
  ...
  // Responses WebSocket too. The downstream client→DardcorCode hop works, but the
...
export async function POST(request: Request) {
  ...
  const receivedSecret = request.headers.get("x-dardcorCode-ws-bridge-secret") || "";
