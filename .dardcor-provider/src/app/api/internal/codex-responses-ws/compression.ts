...
import { estimateTokens } from "@dardcorCode/open-sse/services/contextManager.ts";
import { adaptBodyForCompression } from "@dardcorCode/open-sse/services/compression/bodyAdapter.ts";
import type {
  CompressionConfig,
  CompressionResult,
} from "@dardcorCode/open-sse/services/compression/types.ts";
import { resolveCompressionSettings } from "@dardcorCode/open-sse/handlers/chatCore/compressionSettings.ts";
import {
  writeCompressionAnalytics,
  writeCompressionSkip,
} from "@dardcorCode/open-sse/handlers/chatCore/compressionAnalyticsWrite.ts";
...
export async function applyResponsesWsCompression(
  ...
  try {
    ...
    const { selectCompressionStrategy, applyCompressionAsync } = await import(
      "@dardcorCode/open-sse/services/compression/strategySelector.ts"
    );
