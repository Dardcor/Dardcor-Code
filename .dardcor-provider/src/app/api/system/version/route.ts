...
import { resolveGlobalDardcorCodePath } from "@/lib/system/globalPackagePath";
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          try {
            await execFileAsync("pm2", ["restart", "dardcorCode", "--update-env"], {
              timeout: 30_000,
              cwd: PROJECT_ROOT,
            });
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        send({ step: "install", status: "running", message: `Installing dardcorCode@${latest}...` });
          await execFileAsync(
            "npm",
            ["install", "-g", `dardcorCode@${latest}`, "--ignore-scripts", "--legacy-peer-deps"],
            buildNpmExecOptions(process.platform, { cwd: PROJECT_ROOT, timeoutMs: 300_000 })
          );
        send({ step: "install", status: "done", message: `Installed dardcorCode@${latest}` });
        ...
        const omniPath = await resolveGlobalDardcorCodePath();
        await execFileAsync(
          "npm",
