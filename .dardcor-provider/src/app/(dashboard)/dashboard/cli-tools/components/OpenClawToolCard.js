...
export default function OpenClawToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentProvider = openclawStatus.settings?.models?.providers?.["dardcor-code"] || openclawStatus.settings?.models?.providers?.["9router"];
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = openclawStatus.settings?.models?.providers?.["dardcor-code"] || openclawStatus.settings?.models?.providers?.["9router"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) setSelectedModel(primaryModel.replace(/^(dardcor-code|9router)\//, ""));
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `dardcor-code/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
          ...
          {!checkingOpenclaw && openclawStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {(openclawStatus?.settings?.models?.providers?.["dardcor-code"]?.baseUrl || openclawStatus?.settings?.models?.providers?.["9router"]?.baseUrl) && (
