...
export default function OpenCodeToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus, tunnelEnabled, tunnelPublicUrl, tailscaleEnabled, tailscaleUrl }) {
  ...
  useEffect(() => {
    ...
    if (status?.config?.agent?.explorer?.model?.startsWith("dardcor-code/") || status?.config?.agent?.explorer?.model?.startsWith("9router/")) {
      setSubagentModel(status.config.agent.explorer.model.replace(/^(dardcor-code|9router)\//, ""));
    }
  }, [status]);
  ...
  const getConfigStatus = () => {
    ...
    const url = status.config?.provider?.["dardcor-code"]?.options?.baseURL || status.config?.provider?.["9router"]?.options?.baseURL || "";
          ...
          {!checking && status?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {((status?.config?.provider?.["dardcor-code"]?.options?.baseURL) || (status?.config?.provider?.["9router"]?.options?.baseURL)) && (
