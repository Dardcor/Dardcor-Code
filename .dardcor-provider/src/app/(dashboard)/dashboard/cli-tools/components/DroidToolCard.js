...
export default function DroidToolCard({
  ...
  // Check for any Dardcor Code model entry (support multi-model: custom:Dardcor Code-0, custom:Dardcor Code-1, ...; legacy custom:9Router-*)
  const currentConfig = droidStatus.settings?.customModels?.find(m => m.id?.startsWith("custom:Dardcor Code") || m.id?.startsWith("custom:9Router"));
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const existingModels = (droidStatus.settings?.customModels || [])
        .filter(m => m.id?.startsWith("custom:Dardcor Code") || m.id?.startsWith("custom:9Router"))
        .sort((a, b) => (a.index || 0) - (b.index || 0))
        .map(m => m.model);
      if (existingModels.length > 0) {
        setModelList(existingModels);
      } else {
  // Legacy: single model stored as custom:9Router-0 (or current custom:Dardcor Code-0)
  const legacy = droidStatus.settings?.customModels?.find(m => m.id === "custom:Dardcor Code-0" || m.id === "custom:9Router-0");
        if (legacy?.model) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                ...
                {/* Current configured */}
                {((droidStatus?.settings?.customModels?.find(m => m.id?.startsWith("custom:Dardcor Code") || m.id?.startsWith("custom:9Router"))?.baseUrl)) && (
