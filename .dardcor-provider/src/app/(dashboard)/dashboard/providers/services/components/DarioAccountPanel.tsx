 ...
 * DARIO_ADMIN_TOKEN never reaches this component; the DardcorCode routes attach it.
 *
 ...
 *
 * Also offers "Import from DardcorCode": lists any existing DardcorCode `claude`
 ...
 * /api/services/dario/admin/import-from-dardcorCode/route.ts for why this is
...
export function DarioAccountPanel() {
  ...
  const refreshOmniConnections = useCallback(async () => {
    setOmniLoading(true);
    try {
      const res = await fetch("/api/services/dario/admin/import-from-dardcorCode");
      const json = (await res.json().catch(() => null)) as {
  ...
  return (
    <Card padding="md">
      <div className="space-y-4">
        ...
        {/* Import from DardcorCode */}
        <div className="space-y-2 border-t border-border pt-3">
          <p className="text-xs font-medium">Import from DardcorCode</p>
          <p className="text-xs text-text-muted">
            Reuse an existing DardcorCode Claude connection&apos;s OAuth tokens instead of logging in
          ...
          {omniLoading && omniConnections.length === 0 ? (
            ...
            <div className="flex items-center justify-between gap-3 rounded-lg border border-border px-3 py-2">
              <p className="text-xs text-text-muted">
                No eligible DardcorCode Claude connections found.
              </p>
              <Tooltip content="Will be active once a Claude connection exists in DardcorCode.">
