...
export default function SystemStorageTab() {
  ...
  const [storageHealth, setStorageHealth] = useState({
    driver: "sqlite",
    dbPath: "~/.dardcorCode/storage.sqlite",
    sizeBytes: 0,
    retentionDays: {
      app: 7,
      call: 7,
  ...
  const handleExportJson = async () => {
    ...
    try {
      await fetchAndDownload(
        ...
        `dardcorCode-legacy-backup-${new Date().toISOString().replace(/[:.]/g, "-")}.json`,
        t("jsonExportFailed")
      );
    } catch (err) {
  ...
  const handleExport = async () => {
    ...
    try {
      await fetchAndDownload(
        ...
        `dardcorCode-backup-${new Date().toISOString().replace(/[:.]/g, "-")}.sqlite`,
        t("exportFailed")
      );
    } catch (err) {
      ...
      <div className="grid grid-cols-1 gap-3 mb-4">
        <div className="p-3 rounded-lg bg-bg border border-border">
          ...
          <p className="text-sm font-mono text-text-main break-all">
            {storageHealth.dbPath || "~/.dardcorCode/storage.sqlite"}
          </p>
        </div>
      </div>
            ...
            onClick={async () => {
              ...
              try {
                await fetchAndDownload(
                  ...
                  "dardcorCode-full-backup.tar.gz",
                  t("exportFailed")
                );
              } catch (err) {
                setImportStatus({
