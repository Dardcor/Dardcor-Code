...
export function SkillPreviewPane({
  ...
  const handleCopyRawUrl = useCallback(async () => {
    if (!skillId) return;
    const rawUrl = `https://raw.githubusercontent.com/diegosouzapw/DardcorCode/refs/heads/main/skills/${skillId}/SKILL.md`;
    try {
  ...
  const githubUrl = skillId
    ? `https://github.com/diegosouzapw/DardcorCode/blob/main/skills/${skillId}/SKILL.md`
    : null;
