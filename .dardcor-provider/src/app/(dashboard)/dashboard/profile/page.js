...
export default function ProfilePage() {
  ...
  const handlePasswordChange = async (e) => {
    ...
    if (passwords.new.length < 8) {
      setPassStatus({ type: "error", message: "New password must be at least 8 characters" });
      return;
    }
  ...
  const handleExportDatabase = async (password) => {
    ...
    try {
      const res = await fetch("/api/settings/database", {
        headers: { "x-dardcor-password": password },
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
          ...
          <div className="flex flex-col gap-4">
            ...
            {settings.requireLogin === true && (
              <form onSubmit={handlePasswordChange} className="flex flex-col gap-4 pt-4 border-t border-border/50">
                ...
                {passStatus.message && (
                  <p role="status" className={`text-xs sm:text-sm ${passStatus.type === "error" ? "text-red-500" : "text-green-500"}`}>
                    {passStatus.message}
                  </p>
                )}
