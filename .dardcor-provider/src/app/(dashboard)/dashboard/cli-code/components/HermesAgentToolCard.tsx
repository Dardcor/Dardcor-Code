...
export default function HermesAgentToolCard({
  ...
  const setRoleSelection = (roleId: string, model: string, provider = "DardcorCode") => {
    setSelections((prev) => ({ ...prev, [roleId]: { model, provider } }));
  };

  const applyToAll = (model: string) => {
    const newSel: Record<string, RoleSelection> = {};
    HERMES_ROLES.forEach((r) => (newSel[r.id] = { model, provider: "DardcorCode" }));
  ...
  const effectiveRoles = React.useMemo(() => {
    // If user has pending changes, treat selected roles as DardcorCode
    if (Object.keys(selections).length > 0) {
      const map: Record<string, any> = {};
      HERMES_ROLES.forEach((r) => {
        if (selections[r.id]) {
          map[r.id] = { usingDardcorCode: true };
  ...
  // Count of roles that are (or will be) routed via DardcorCode
  const configuredRolesCount = HERMES_ROLES.filter((role) => {
    // Pending selection always counts as DardcorCode intent
    if (selections[role.id]) return true;

    const info = effectiveRoles[role.id];
    if (!info) return false;

    // Support both shapes: detector shape (usingDardcorCode) and settings shape (provider + base_url)
    if (typeof info.usingDardcorCode === "boolean") {
      return info.usingDardcorCode;
    }
    return (
      info?.provider === "dardcorCode" ||
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {HERMES_ROLES.map((role) => {
              ...
              // - If user has selected something in this session (pending): show as via DardcorCode
              // - Else if current from disk: show real provider name + "(not DardcorCode)" or "DardcorCode"
              let badge: { label: string; pending: boolean; outsideDardcorCode: boolean } | null =
                null;

              if (sel) {
                // pending change made via the Select modal / quick apply → will be routed via DardcorCode
                const prov = sel.provider || "DardcorCode";
                badge = {
                  label: t("hermesViaDardcorCode", { provider: prov }),
                  pending: true,
                  outsideDardcorCode: false,
                };
              } else if (current) {
                const isOmni =
                  current?.provider === "dardcorCode" ||
                  (current?.base_url || "").includes("20128") ||
                  (current?.base_url || "").includes("localhost");

                if (isOmni) {
                  badge = { label: "DardcorCode", pending: false, outsideDardcorCode: false };
                } else {
                  const realProvider = current.provider || t("other");
                  badge = {
                    label: t("hermesNotDardcorCode", { provider: realProvider }),
                    pending: false,
                    outsideDardcorCode: true,
                  };
                }
              }
        ...
        onSelect={(model: any) => {
          if (modalRole) {
            ...
            if (modelValue) {
              ...
              const prov =
                (model && (model.provider || model.providerId || model.group)) || "DardcorCode";
