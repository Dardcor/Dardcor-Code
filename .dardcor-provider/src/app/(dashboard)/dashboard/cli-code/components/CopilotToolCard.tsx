 ...
 *
 * Feature request: https://github.com/diegosouzapw/DardcorCode/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("dardcorCode-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("dardcorCode-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "dardcorCode-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "DardcorCode",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.dardcorCode}`,
      models,
    };

    const responsesConfig = {
      name: "DardcorCode-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.dardcorCode}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("dardcorCode-cli-key-copilot", value);
  };
