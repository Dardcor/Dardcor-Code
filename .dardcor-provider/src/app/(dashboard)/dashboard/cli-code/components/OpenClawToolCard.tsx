...
export default function OpenClawToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentProvider = openclawStatus.settings?.models?.providers?.["dardcorCode"];
  ...
  useEffect(() => {
    if (openclawStatus?.installed && !hasInitializedModel.current) {
      ...
      const provider = openclawStatus.settings?.models?.providers?.["dardcorCode"];
      if (provider) {
        const primaryModel = openclawStatus.settings?.agents?.defaults?.model?.primary;
        if (primaryModel) {
          const modelId = primaryModel.replace("dardcorCode/", "");
  ...
  const getManualConfigs = () => {
    ...
    const settingsContent = {
      agents: {
        defaults: {
          model: {
            primary: `dardcorCode/${selectedModel || "provider/model-id"}`,
          },
        },
      },
      models: {
          ...
          {!checkingOpenclaw && openclawStatus && !cliReady && (
            <div className="flex items-center gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              ...
              {/*
                ...
                detected locally — typical of remote DardcorCode
          ...
          {!checkingOpenclaw && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL */}
                {openclawStatus?.settings?.models?.providers?.["dardcorCode"]?.baseUrl && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!openclawStatus?.hasDardcorCode}
                  loading={restoring}
                >
