...
export default function CustomCliCard({
  ...
  const keyToUse =
    ...
    (!cloudEnabled
      ? "sk_dardcorCode"
      : translateOrFallback("yourApiKeyPlaceholder", "sk-your-dardcorCode-key"));
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-text-muted">
                  {translateOrFallback(
                    ...
                    "Use any DardcorCode model ID or combo. Most OpenAI-compatible CLIs only need the /v1 base URL plus a model string."
                  )}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-text-muted">
                  {translateOrFallback(
                    "customCliKeyHelper",
                    "For local installs DardcorCode can use sk_dardcorCode. In cloud mode, pick one of your management API keys."
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="rounded-lg border border-border/60 bg-black/[0.02] dark:bg-white/[0.02] p-3 text-xs text-text-muted">
            ...
            <p className="mt-1">
              {translateOrFallback(
                ...
                "Point any OpenAI-compatible client to the DardcorCode /v1 base URL. The raw chat completions endpoint is {endpoint}. Use the JSON block when the tool wants a provider object, or the env script when it reads OPENAI_* variables.",
