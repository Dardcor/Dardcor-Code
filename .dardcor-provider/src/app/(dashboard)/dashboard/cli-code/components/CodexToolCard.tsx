...
export default function CodexToolCard({
  ...
  const handleApplySettings = async () => {
    ...
    setMessage(null);
    try {
      // Use sk_dardcorCode for localhost if no key, otherwise use selected key
      const keyToUse =
  ...
  const getManualConfigs = () => {
    const keyToUse = !cloudEnabled ? "sk_dardcorCode" : "<YOUR_OMNIROUTE_API_KEY>";

    let configContent = `# DardcorCode Configuration for Codex CLI
model = "${selectedModel || CODEX_DEFAULT_MODELS[0]}"`;
    ...
    if (wireApi === "responses") {
      configContent += `
model_provider = "dardcorCode"

[model_providers.dardcorCode]
name = "DardcorCode"
...
# Utilize the built-in OpenAI provider pointed to DardcorCode
