...
export default function DroidToolCard({
  ...
  // (#618) Match any custom:DardcorCode-<i> entry (multi-model).
  const isDardcorCodeEntry = (m) => typeof m?.id === "string" && m.id.startsWith("custom:DardcorCode");

  const getConfigStatus = () => {
    if (!cliReady) return null;
    const currentConfig = droidStatus.settings?.customModels?.find(isDardcorCodeEntry);
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      // (#618) Pre-fill the multi-model list from every custom:DardcorCode-<i>
      // entry, preserving the original index order.
      const existing = (droidStatus.settings?.customModels || [])
        .filter(isDardcorCodeEntry)
        .slice()
    ...
    try {
      ...
      const res = await fetch("/api/cli-tools/droid-settings", {
        ...
        body: JSON.stringify({
          ...
          apiKey: !cloudEnabled ? "sk_dardcorCode" : null,
          keyId: selectedKeyId,
  ...
  const getManualConfigs = () => {
    ...
    const keyToDisplay =
      selectedKeyObj?.key || (!cloudEnabled ? "sk_dardcorCode" : "<API_KEY_FROM_DASHBOARD>");
          ...
          {!checkingDroid && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL — first DardcorCode entry, any index (#618) */}
                {droidStatus?.settings?.customModels?.find(isDardcorCodeEntry)?.baseUrl && (
                  <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {apiKeys.length > 0 ? (
                    ...
                    <span className="flex-1 text-xs text-text-muted px-2 py-1.5">
                      {cloudEnabled ? t("noApiKeysCreateOne") : t("defaultDardcorCodeKey")}
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!droidStatus?.hasDardcorCode}
                  loading={restoring}
                >
