 ...
 * `selectedApiKey` was empty — but the default `sk_dardcorCode` key is a valid
 * local-mode default and should not block Apply. The Reset button was disabled
 * whenever `codexStatus.hasDardcorCode` was false — but a user should always be
 * able to reset, even when Codex was never configured against DardcorCode.
 */
 ...
 * `sk_dardcorCode` default kicks in, so an empty `selectedApiKey` must NOT
 * disable Apply.
 */
