...
export default function DefaultToolCard({
  ...
  const resolveApiKeyValue = useCallback(
    () => selectedKeyObj?.rawKey || (!cloudEnabled ? "sk_dardcorCode" : t("yourApiKeyPlaceholder")),
  ...
  useEffect(() => {
    const restoreTimer = window.setTimeout(() => {
      const savedModel = localStorage.getItem(`dardcorCode-cli-model-${toolId}`);
      if (savedModel) {
        if (isMultiModelTool) {
          try {
      ...
      const savedKey = localStorage.getItem(`dardcorCode-cli-key-${toolId}`);
  ...
  const handleModelChange = useCallback(
    (value) => {
      setModelValue(value);
      if (value) {
        localStorage.setItem(`dardcorCode-cli-model-${toolId}`, value);
      } else {
        localStorage.removeItem(`dardcorCode-cli-model-${toolId}`);
      }
    },
    [toolId]
  );
  ...
  const handleModelValuesChange = useCallback(
    (values) => {
      ...
      if (normalized.length > 0) {
        localStorage.setItem(`dardcorCode-cli-model-${toolId}`, JSON.stringify(normalized));
      } else {
        localStorage.removeItem(`dardcorCode-cli-model-${toolId}`);
      }
    },
  ...
  const handleApiKeyChange = useCallback(
    (value) => {
      ...
      if (value) {
        ...
        localStorage.setItem(`dardcorCode-cli-key-${toolId}`, value);
      }
    },
    [toolId]
  );
      ...
      <div className="mt-2 flex items-center gap-2">
        {apiKeys && apiKeys.length > 0 ? (
          ...
          <span className="text-sm text-text-muted">
            {cloudEnabled ? t("noApiKeysCreateOne") : "sk_dardcorCode"}
          </span>
        )}
      </div>
    );
