...
/**
 * Editable table: source model → target DardcorCode model.
 */
      ...
      {rows.length === 0 ? (
        <div className="rounded-lg border border-border/40 bg-surface/30 px-4 py-6 text-center">
          <p className="text-xs text-text-muted mb-3">
            {t("noMappingsDesc") ||
              "No model mappings configured yet. Add mappings to route agent requests through DardcorCode."}
          </p>
          <button
            type="button"
            onClick={addMapping}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border/40 bg-surface/60">
                  ...
                  <th className="px-3 py-2 text-left text-xs font-medium text-text-muted">
                    {t("targetModel") || "Target model (DardcorCode)"}
                  </th>
