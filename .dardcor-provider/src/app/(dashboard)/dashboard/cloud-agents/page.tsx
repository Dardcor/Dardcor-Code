...
export default function CloudAgentsPage() {
  ...
  useEffect(() => {
    const loadSettings = setTimeout(() => {
      try {
        const stored = localStorage.getItem("dardcorCode-cloud-agents-settings");
  ...
  const updateSetting = (key: keyof typeof settings, value: boolean) => {
    ...
    setSettings(next);
    try {
      localStorage.setItem("dardcorCode-cloud-agents-settings", JSON.stringify(next));
    } catch {
      // ignore
    }
  };
