...
/**
 * Rename a Node process title so DardcorCode is identifiable in `ps`/`htop`
...
export function renameProcessTitle(currentTitle: string): string {
  ...
  return `dardcorCode${currentTitle.slice("next-server".length)}`;
}
...
export async function registerNodejs(): Promise<void> {
  ...
  // Rename the process title so DardcorCode is identifiable in ps/htop instead
  ...
  await import("@dardcorCode/open-sse/index.ts");
  ...
  // See: https://github.com/diegosouzapw/DardcorCode/issues/3625 (Part A)
  try {
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@dardcorCode/open-sse/services/batchProcessor");
    initBatchProcessor();
  ...
  try {
    ...
    if (settings.systemPrompt) {
      const { setSystemPromptConfig } =
        await import("@dardcorCode/open-sse/services/systemPrompt.ts");
    ...
    const { hydrateThinkingBudgetConfig } =
      await import("@dardcorCode/open-sse/services/thinkingBudget.ts");
    ...
    const { hydrateTaskRoutingConfig } =
      await import("@dardcorCode/open-sse/services/taskAwareRouter.ts");
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    await Promise.allSettled([
      ...
      import("@dardcorCode/open-sse/services/autoRefreshDaemon")
      ...
      // Reads the schedule written by `dardcorCode backup auto enable` and fires
