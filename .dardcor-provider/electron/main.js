/**
 * DardcorCode Electron Desktop App - Main Process
 *
 ...
 * #14 Removed dead dardcorCode:// protocol (no handler existed)
...
// ── Remote Server Mode ──────────────────────────────────────
// Lets the desktop shell attach to an already-running DardcorCode server (e.g. a
...
function resolveNodeExecutable(env = process.env) {
  ...
  // (e.g. DardcorCode.app/Contents/MacOS/DardcorCode). Spawning it with
  ...
  if (process.platform === "darwin" && !isDev) {
    ...
    // ("DardcorCode"), whereas app.getName() reads package.json `name` ("dardcorCode-desktop")
...
function resolveDataDir(overridePath, env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return path.join(appData, "dardcorCode");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return path.join(path.resolve(xdg), "dardcorCode");

  return path.join(require("os").homedir(), ".dardcorCode");
}
    ...
    new Promise((r) =>
      setTimeout(() => {
        try {
          ...
          // bare SIGKILL of the direct child, keeping dardcorCode.exe locked).
  ...
  autoUpdater.on("update-downloaded", (info) => {
    ...
    if (Notification.isSupported()) {
      const notification = new Notification({
        title: "DardcorCode Update Ready",
...
function installUpdate() {
  if (nextServer) {
    ...
    // grandchildren) keeps dardcorCode.exe locked and the updater fails with "file in use".
...
function setupContentSecurityPolicy() {
  session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
    ...
    const csp = [
      ...
      `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* wss://localhost:* wss://127.0.0.1:* https://*.dardcorCode.online https://*.dardcorCode.dev`,
      scriptSrc,
...
function startNextServer() {
  ...
  if (changed) {
    ...
    try {
      ...
      const lines = [
        "# Auto-generated by DardcorCode bootstrap",
        "",
  ...
  nextServer.stdout?.on("data", (data) => {
    ...
    if (text.includes("Ready") || text.includes("started") || text.includes("listening")) {
      ...
      if (isHeadless && !global.loggedHeadlessReady) {
        ...
        console.log("\n\x1b[32m✔ DardcorCode Headless CLI Server is ready and listening!\x1b[0m");
...
function stopNextServer() {
  if (nextServer) {
    ...
    // (dardcorCode.exe-as-node) spawns grandchildren that a bare SIGTERM leaves alive,
    // holding a lock on dardcorCode.exe and blocking updates.
    killProcessTree(nextServer, { signal: "SIGTERM" });
    nextServer = null;
  }
}
  ...
  try {
    ...
    const desktopFileContent =
      [
        ...
        "Type=Application",
        "Name=DardcorCode",
        "Comment=DardcorCode Desktop Client",
    ...
    fs.writeFileSync(path.join(autostartDir, "dardcorCode-desktop.desktop"), desktopFileContent, {
      mode: 0o644,
    });
    return true;
  } catch (err) {
...
function isLinuxDesktopAutostartEnabled() {
  try {
    ...
    return fs.existsSync(
      path.join(os.homedir(), ".config", "autostart", "dardcorCode-desktop.desktop")
    );
  } catch {
    return false;
  }
