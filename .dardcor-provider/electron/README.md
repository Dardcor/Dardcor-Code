# Dardcor Code Electron Desktop App

This directory contains the Electron desktop application wrapper for Dardcor Code.
...
1. Download the latest `.dmg` from the [Releases](https://github.com/diegosouzapw/Dardcor Code/releases) page.
2. Open the `.dmg` file.
3. Drag `Dardcor Code.app` to the Applications folder.
4. Launch from Applications.

> ⚠️ **Note:** The app is not signed with an Apple Developer certificate yet. If macOS blocks the app, run:
>
> ```bash
> xattr -cr /Applications/Dardcor Code.app
> ```
>
...
1. Download `Dardcor Code.Setup.*.exe` from [Releases](https://github.com/diegosouzapw/Dardcor Code/releases).
2. Run the installer.
3. Launch from Start Menu or Desktop shortcut.

**Portable (No Installation):**

1. Download `Dardcor Code.exe` from [Releases](https://github.com/diegosouzapw/Dardcor Code/releases).
2. Run directly from any folder.

### Linux

1. Download the `.AppImage` from [Releases](https://github.com/diegosouzapw/Dardcor Code/releases).
2. Make it executable:
   ```bash
   chmod +x Dardcor Code-*.AppImage
   ```
3. Run:
   ```bash
   ./Dardcor Code-*.AppImage
   ```
...
- **Remote Server Mode** — Point the shell at an already-running Dardcor Code server (e.g. a Docker/OrbStack container, or another machine) instead of spawning a local one — see below
...
already run Dardcor Code elsewhere — most commonly in a Docker/OrbStack container, so
