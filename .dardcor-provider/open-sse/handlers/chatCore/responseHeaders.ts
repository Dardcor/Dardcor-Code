...
} from "@/domain/dardcorCodeResponseMeta";
import { OMNIROUTE_RESPONSE_HEADERS } from "@/shared/constants/headers";
import { defaultLogger } from "@dardcorCode/open-sse/utils/logger";
 ...
 * This budget includes each header name, separator, value, and trailing CRLF. DardcorCode's own
...
function isDardcorCodeInternalHeader(headerName: string): boolean {
  return headerName.toLowerCase().startsWith("x-dardcorCode-");
}
 ...
 *
 * If DardcorCode re-emits those headers from an App Router route handler, Next
...
export function buildStreamingResponseHeaders(
  ...
  providerHeaders.forEach((value, key) => {
    ...
    if (
      ...
      isDardcorCodeInternalHeader(normalized)
    ) {
      return;
    }
    candidates.push({
  ...
  attachDardcorCodeMetaHeaders(responseHeaders, meta);
