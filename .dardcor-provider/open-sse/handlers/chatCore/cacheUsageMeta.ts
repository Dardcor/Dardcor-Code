 ...
 * shapes), and attach an `_dardcorCode` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _dardcorCode: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._dardcorCode &&
    typeof payload._dardcorCode === "object" &&
    !Array.isArray(payload._dardcorCode)
      ? payload._dardcorCode
      : {};
  return {
    ...payload,
