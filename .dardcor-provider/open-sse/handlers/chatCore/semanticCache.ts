...
export async function checkSemanticCache({
  ...
  if (semanticCacheEnabled && isCacheableForRead(body, clientRawRequest?.headers)) {
    ...
    if (cached) {
      ...
      // the client is 0 (consumers that sum X-DardcorCode-Response-Cost must not charge for
      // hits). The original/would-have-been cost is surfaced via X-DardcorCode-Cost-Saved.
