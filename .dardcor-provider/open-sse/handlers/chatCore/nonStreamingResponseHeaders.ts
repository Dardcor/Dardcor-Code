 ...
 * cache-MISS JSON response — the static Content-Type + cache marker, the DardcorCode meta headers
...
import { attachDardcorCodeMetaHeaders as defaultAttachMeta } from "@/domain/dardcorCodeResponseMeta";
...
export function buildNonStreamingResponseHeaders(
  ...
  deps: { attachDardcorCodeMetaHeaders: typeof defaultAttachMeta; now: () => number } = {
    attachDardcorCodeMetaHeaders: defaultAttachMeta,
    now: Date.now,
  }
  ...
  deps.attachDardcorCodeMetaHeaders(responseHeaders, {
