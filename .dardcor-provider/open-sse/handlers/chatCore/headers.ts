 ...
 * `x-dardcorCode-no-memory` header. Mirrors the existing `x-dardcorCode-no-cache`
...
export function isNoMemoryRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-dardcorCode-no-memory") || "")
    .trim()
    .toLowerCase();
...
/**
 * Per-request compression override via the `x-dardcorCode-compression` header. Mirrors the
 * `x-dardcorCode-no-memory` convention (#4290). Returns the raw trimmed value, or null when
...
export function resolveCompressionHeader(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-dardcorCode-compression") || "").trim();
  return value || null;
}

/**
 * Per-request opt-in to unconditionally strip `reasoning_content` from the
 * non-streaming JSON response via the `x-dardcorCode-strip-reasoning` header.
...
export function isStripReasoningRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-dardcorCode-strip-reasoning") || "")
    .trim()
    .toLowerCase();
  return value === "true" || value === "1" || value === "yes";
}
