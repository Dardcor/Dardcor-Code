...
export async function handleChatGptWebImageGeneration({
  ...
  for (let i = 0; i < requestedCount; i++) {
    ...
    if (urls.length === 0) {
      // Distinguish "image was generated upstream but DardcorCode could not
      ...
      const error = imageResolutionFailed
        ? `ChatGPT Web generated an image but DardcorCode could not retrieve it (the image asset could not be downloaded — the URL may have expired or ChatGPT changed its image delivery format). Please retry; if it persists, report it. Assistant text: ${content.slice(0, 200)}`
