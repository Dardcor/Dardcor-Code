...
import {
  checkTokenLimits,
  recordTokenUsage,
} from "@dardcorCode/open-sse/services/tokenLimitCounter.ts";
import {
  COOLDOWN_MS,
  HTTP_STATUS,
...
import { wasRefreshTokenRotated } from "@dardcorCode/open-sse/services/refreshSerializer.ts";
...
export async function handleChatCore({
  ...
  if (webSearchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webSearchFallbackPlan.convertedToolCount} web_search tool(s) to DardcorCode fallback for ${provider}`
    );
  }
  ...
  if (webFetchFallbackPlan.enabled) {
    ...
    log?.info?.(
      "TOOLS",
      `Converted ${webFetchFallbackPlan.convertedToolCount} web_fetch tool(s) to DardcorCode fallback for ${provider}`
    );
  }
  ...
  const explicitSessionIdHeader =
    (clientRawRequest?.headers && typeof clientRawRequest.headers.get === "function"
      ? clientRawRequest.headers.get("x-dardcorCode-session-id")
      : getHeaderValueCaseInsensitive(
          clientRawRequest?.headers ?? null,
          "x-dardcorCode-session-id"
        )) || null;
  ...
  // (#5312 / #5245): `x-dardcorCode-thinking-marker: off` suppresses it for
  ...
  // They are accepted for compatibility at the DardcorCode API boundary only.
  if (body && typeof body === "object") {
  ...
  const reasoningRouteDecision =
    body && typeof body === "object"
      ? (body as Record<string, unknown>)._dardcorCodeReasoningRouteTrace
      : null;
  if (reasoningRouteDecision) {
    reqLogger.logRouteDecision(reasoningRouteDecision);
    body = { ...(body as Record<string, unknown>) };
    delete (body as Record<string, unknown>)._dardcorCodeReasoningRouteTrace;
  }
  ...
  // `x-dardcorCode-no-memory: true` to skip memory+skills injection (a null owner
  ...
  if (body && Array.isArray(allMessages) && allMessages.length > 0) {
    ...
    try {
      ...
      if (compressionHeader) {
        log?.debug?.("COMPRESSION", `x-dardcorCode-compression header: ${compressionHeader}`);
      }
  ...
  // rejects unknown/extra params with a strict "400 Param Incorrect". Map DardcorCode's
  ...
  if (!stream) {
    ...
    if (clientResponseFormat === FORMATS.OPENAI_RESPONSES) {
      ...
      // Port of decolua/9router#517: opt-in `x-dardcorCode-strip-reasoning` header
    ...
    // #6426: align response body `model` with the `X-DardcorCode-Model` header
  ...
  if (needsResponsesTranslation) {
    ...
    transformStream = createSSETransformStreamWithLogger(
      ...
      // (e.g. OpenCode by UA; any client via `x-dardcorCode-thinking-marker: off`);
