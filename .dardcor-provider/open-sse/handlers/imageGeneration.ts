...
async function handleGeminiImageGeneration({ model, providerConfig, body, credentials, log }) {
  ...
  if (!projectId || typeof projectId !== "string") {
    return saveImageErrorResult({
      ...
      startTime,
      error:
        "Missing Google projectId for Antigravity account. Please reconnect OAuth in Providers so DardcorCode can fetch your Cloud Code project.",
...
export async function handleOpenAIImageEdit({
  ...
  const boundary = `----DardcorCodeImageEdit${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
...
export async function handleImageEdit({
  ...
  if (!cached?.entry.context) {
    ...
    return saveImageErrorResult({
      ...
      startTime,
      error:
        "chatgpt-web image edit only works for images recently generated through this DardcorCode instance " +
