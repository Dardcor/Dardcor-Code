...
/**
 * Map DardcorCode's reasoning-effort inputs onto the binary thinking switch exposed by
...
export function supportsMaxEffortForProvider(provider: string, model: string): boolean {
  ...
  // normalized to xhigh (the DardcorCode-internal top tier) and rejected by the
...
// DardcorCode carries the requested effort on up to three shapes:
//   1. top-level `reasoning_effort`        — OpenAI / DardcorCode-internal
...
export function sanitizeReasoningEffortForProvider(
  ...
  // ONLY as {high, max} (its own top tier is literally "max"). DardcorCode's internal
