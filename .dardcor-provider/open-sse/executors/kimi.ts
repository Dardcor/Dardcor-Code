...
function resolveKimiProtocol(
  ...
  const targetFormat = credentials?.providerSpecificData?._dardcorCodeKimiTargetFormat;
...
function getThinkingPolicy(credentials: ProviderCredentials): KimiThinkingPolicy {
  return (asRecord(credentials.providerSpecificData?._dardcorCodeKimiThinking) ||
