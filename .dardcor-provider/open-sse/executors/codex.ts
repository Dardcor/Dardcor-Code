...
function consumeResponsesStoreMarker(body: Record<string, unknown>): unknown {
  const marker = body._dardcorCodeResponsesStore;
  delete body._dardcorCodeResponsesStore;
  return marker;
}
...
export function isCodexResponsesWebSocketRequired(_model: string, credentials: unknown): boolean {
  ...
  // DardcorCode is an HTTP→SSE gateway — WebSocket transport is unnecessary and
...
export class CodexExecutor extends BaseExecutor {
  ...
  transformRequest(
    ...
    // These are often injected by DardcorCode's fallback logic for store=true,
    ...
    const RESPONSES_API_ALLOWLIST = new Set([
      ...
      "text",
      // Internal markers used by DardcorCode pipeline
      "_dardcorCodeResponsesStore",
    ]);
