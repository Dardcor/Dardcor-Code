...
/**
 * Strip the DardcorCode provider prefix from tool model fields (e.g.
 ...
 * DardcorCode prefix makes Anthropic reject the request.
 *
      ...
      try {
        ...
        if (
          ...
          if (Array.isArray(tb.tools)) {
            ...
            // Also strip DardcorCode provider prefix from versioned built-in tool
          ...
          //   x-dardcorCode-effort:   low | medium | high | xhigh | max | off
          //   x-dardcorCode-thinking: adaptive | off
          // A header value applies only when the corresponding body field is
          // not already set; "off" force-strips the field.
          const headerEffort = (
            clientHeaders?.["x-dardcorCode-effort"] ?? clientHeaders?.["X-DardcorCode-Effort"]
          )
            ?.trim()
            .toLowerCase();
          const headerThinking = (
            clientHeaders?.["x-dardcorCode-thinking"] ?? clientHeaders?.["X-DardcorCode-Thinking"]
          )
            ?.trim()
            .toLowerCase();
          ...
          if (effThinking === "adaptive") {
            ...
            // must opt in with x-dardcorCode-thinking; force-injecting adaptive thinking
          ...
          // Anthropic correlate accounts behind one DardcorCode.
          ...
          // is pinned to the captured CLI wire image, not DardcorCode's Node.
