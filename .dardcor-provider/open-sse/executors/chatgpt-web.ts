...
// DardcorCode model ID → ChatGPT internal slug. The public ChatGPT Web catalog
// keeps DardcorCode's historical dot-form IDs (e.g. "gpt-5.5-pro"), while
 ...
 * dozen "DardcorCode" entries clutter the sidebar and can interact with
 ...
 * chat but returned to DardcorCode as a bare "completed without image markdown".
 */
 ...
 * often reach DardcorCode from a container while the user's browser needs a
...
/**
 * Download a chatgpt.com signed image URL and re-serve it from DardcorCode's
 ...
 * and return a browser-fetchable DardcorCode URL.
 */
...
function makeImageResolver(ctx: ResolverContext): ImageResolver {
  ...
  return async (assetPointer, conversationId, parentMessageId) => {
    ...
    if (fileId) {
      ...
      // DardcorCode's short-lived image cache.
      //
    ...
    if (signedUrl) {
      ...
      // so we materialize the bytes into our own cache and emit an DardcorCode
