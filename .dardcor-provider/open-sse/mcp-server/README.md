...
> **Model Context Protocol server** that exposes Dardcor Code's gateway intelligence as **104 tools** for AI agents.
>
...
The MCP Server allows any AI agent (Claude Desktop, Cursor, VS Code Copilot, custom agents) to **monitor, control, and optimize** the Dardcor Code AI gateway programmatically.
...
└─────────────────────────────┼────────────────────────────────────┘
                              ...
                              ▼
┌──────────────────────────────────────────────────────────────────┐
│                    Dardcor Code Gateway (port 20128)                 │
...
```bash
# Required: Dardcor Code base URL
...
```json
{
  "mcpServers": {
    "Dardcor Code": {
      "command": "node",
      "args": ["path/to/Dardcor Code/open-sse/mcp-server/server.ts"],
      "env": {
...
| --- | ------------------------------- | --------------------- | -------------------------------------------------------------------------- |
| 1   | `Dardcor Code_get_health`          | `read:health`         | Gateway health, uptime, memory, circuit breakers, rate limits, cache stats |
| 2   | `Dardcor Code_list_combos`         | `read:combos`         | List all combos (model chains) with strategies and optional metrics        |
| 3   | `Dardcor Code_get_combo_metrics`   | `read:combos`         | Performance metrics for a specific combo                                   |
| 4   | `Dardcor Code_switch_combo`        | `write:combos`        | Activate or deactivate a combo for routing                                 |
| 5   | `Dardcor Code_check_quota`         | `read:quota`          | Remaining API quota per provider with token health status                  |
| 6   | `Dardcor Code_route_request`       | `execute:completions` | Send a chat completion through intelligent routing                         |
| 7   | `Dardcor Code_cost_report`         | `read:usage`          | Cost report by period (session/day/week/month) with per-provider breakdown |
| 8   | `Dardcor Code_list_models_catalog` | `read:models`         | List all available models across providers with capabilities and pricing   |

### Phase 2: Advanced Tools (8)

| #   | Tool                               | Scopes                               | Description                                                                                    |
| --- | ---------------------------------- | ------------------------------------ | ---------------------------------------------------------------------------------------------- |
| 9   | `Dardcor Code_simulate_route`         | `read:health`, `read:combos`         | Dry-run routing simulation showing fallback tree and estimated costs                           |
| 10  | `Dardcor Code_set_budget_guard`       | `write:budget`                       | Set session budget with action on exceed: `degrade`, `block`, or `alert`                       |
| 11  | `Dardcor Code_set_resilience_profile` | `write:resilience`                   | Apply resilience profile: `aggressive`, `balanced`, or `conservative`                          |
| 12  | `Dardcor Code_test_combo`             | `execute:completions`, `read:combos` | Test each provider in a combo with a real prompt and a real upstream call, report latency/cost |
| 13  | `Dardcor Code_get_provider_metrics`   | `read:health`                        | Per-provider metrics with latency percentiles (p50/p95/p99), circuit breaker                   |
| 14  | `Dardcor Code_best_combo_for_task`    | `read:combos`, `read:health`         | AI-powered combo recommendation by task type with budget/latency constraints                   |
| 15  | `Dardcor Code_explain_route`          | `read:health`, `read:usage`          | Explain why a request was routed to a provider (scoring factors, fallbacks)                    |
| 16  | `Dardcor Code_get_session_snapshot`   | `read:usage`                         | Full session snapshot: cost, tokens, top models, errors, budget status                         |

### Cache and Compression Tools

| #   | Tool                                | Scopes              | Description                                                                  |
| --- | ----------------------------------- | ------------------- | ---------------------------------------------------------------------------- |
| 21  | `Dardcor Code_cache_stats`             | `read:cache`        | Semantic cache, prompt-cache, and idempotency statistics                     |
| 22  | `Dardcor Code_cache_flush`             | `write:cache`       | Flush cache entries globally or by signature/model                           |
| 23  | `Dardcor Code_compression_status`      | `read:compression`  | Compression settings, analytics summary, and provider-aware cache statistics |
| 24  | `Dardcor Code_compression_configure`   | `write:compression` | Configure compression mode and trigger thresholds at runtime                 |
| 25  | `Dardcor Code_set_compression_engine`  | `write:compression` | Set Caveman, RTK, or stacked compression mode and pipeline                   |
| 26  | `Dardcor Code_list_compression_combos` | `read:compression`  | List named compression combos and routing assignments                        |
| 27  | `Dardcor Code_compression_combo_stats` | `read:compression`  | Read analytics grouped by compression combo and engine                       |
| 28  | `Dardcor Code_ccr_store`               | `write:compression` | Store content in the caller-isolated in-memory CCR store                     |
| 29  | `Dardcor Code_ccr_retrieve`            | `read:compression`  | Retrieve full or ranged caller-owned CCR content                             |
| 30  | `Dardcor Code_ccr_inspect`             | `read:compression`  | Inspect CCR metadata without returning content                               |
| 31  | `Dardcor Code_ccr_list`                | `read:compression`  | List paginated caller-owned CCR metadata                                     |
| 32  | `Dardcor Code_ccr_delete`              | `write:compression` | Delete a caller-owned CCR block                                              |
| 33  | `Dardcor Code_ccr_stats`               | `read:compression`  | Report caller usage, bounded-store limits, and lifecycle counters            |
...
compression is enabled. `Dardcor Code_compression_status` exposes those savings separately as
...
```python
"""
Dardcor Code MCP Client — Python example using the mcp SDK.
...
async def main():
    ...
    async with stdio_client(server) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            health = await session.call_tool("Dardcor Code_get_health", {})
            print("Health:", health.content[0].text)

            # 2. List available combos with metrics
            combos = await session.call_tool("Dardcor Code_list_combos", {
                "includeMetrics": True
            })
            print("Combos:", combos.content[0].text)

            # 3. Find the best combo for a coding task
            best = await session.call_tool("Dardcor Code_best_combo_for_task", {
                "taskType": "coding",
            ...
            budget = await session.call_tool("Dardcor Code_set_budget_guard", {
                "maxCost": 1.00,
                "action": "degrade",
                "degradeToTier": "cheap",
            })
            ...
            response = await session.call_tool("Dardcor Code_route_request", {
            ...
            snapshot = await session.call_tool("Dardcor Code_get_session_snapshot", {})
...
async function main() {
  ...
  const simulation = await client.callTool({
    name: "Dardcor Code_simulate_route",
    arguments: {
      model: "claude-sonnet-4",
  ...
  const result = await client.callTool({
    name: "Dardcor Code_route_request",
    arguments: {
      model: "claude-sonnet-4",
...
An agent that monitors Dardcor Code health and auto-switches combos when providers degrade.
...
async def auto_healing_loop(session):
    ...
    while True:
        ...
        if broken:
            ...
            await session.call_tool("Dardcor Code_set_resilience_profile", {
            ...
            # Activate it
            await session.call_tool("Dardcor Code_switch_combo", {
...
async def budget_aware_coding(session, task: str, max_budget: float):
    ...
    # Set budget guard
    await session.call_tool("Dardcor Code_set_budget_guard", {
        "maxCost": max_budget,
        "action": "degrade",
        "degradeToTier": "cheap",
    })
    ...
    sim = await session.call_tool("Dardcor Code_simulate_route", {
        "model": "claude-sonnet-4",
    ...
    # Send request
    result = await session.call_tool("Dardcor Code_route_request", {
        "model": "claude-sonnet-4",
...
async def benchmark_combos(session):
    ...
    for combo in combo_list:
        ...
        test = await session.call_tool("Dardcor Code_test_combo", {
            "comboId": combo["id"],
...
async function debugRouting(client: Client, requestId: string) {
  ...
  const explanation = await client.callTool({
    name: "Dardcor Code_explain_route",
    arguments: { requestId },
  });
...
async def find_cheapest_models(session, capability="chat"):
    ...
    catalog = await session.call_tool("Dardcor Code_list_models_catalog", {
...
Part of [Dardcor Code](https://github.com/diegosouzapw/Dardcor Code) — MIT License.
