...
describe("dardcorCode_set_routing_strategy MCP tool schema", () => {
  it("should be registered in MCP_TOOLS", () => {
    const tool = MCP_TOOLS.find((t) => t.name === "dardcorCode_set_routing_strategy");
    expect(tool).toBeDefined();
    expect(tool?.phase).toBe(2);
  });

  it("should be available in MCP_TOOL_MAP", () => {
    expect(MCP_TOOL_MAP["dardcorCode_set_routing_strategy"]).toBeDefined();
  });
