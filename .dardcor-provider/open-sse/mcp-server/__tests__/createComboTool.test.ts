...
describe("dardcorCode_create_combo MCP tool schema", () => {
  it("should be registered in MCP_TOOLS and MCP_TOOL_MAP", () => {
    const tool = MCP_TOOLS.find((t) => t.name === "dardcorCode_create_combo");
    expect(tool).toBeDefined();
    expect(MCP_TOOL_MAP["dardcorCode_create_combo"]).toBeDefined();
  });
...
describe("dardcorCode_create_combo handler (via MCP dispatch)", () => {
  let client: Client;
  ...
  it("should appear in tools/list after registration", async () => {
    ...
    const tool = tools.find((t) => t.name === "dardcorCode_create_combo");
    expect(tool).toBeDefined();
  ...
  it("should POST to /api/combos and return the created combo on success", async () => {
    ...
    const result = await client.callTool({ name: "dardcorCode_create_combo", arguments: args });
