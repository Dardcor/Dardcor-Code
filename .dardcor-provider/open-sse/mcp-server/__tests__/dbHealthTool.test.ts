...
describe("dardcorCode_db_health_check MCP tool", () => {
  let client: Client;
  ...
  it("is registered in the MCP tool map", () => {
    expect(MCP_TOOL_MAP["dardcorCode_db_health_check"]).toBeDefined();
    expect(MCP_TOOL_MAP["dardcorCode_db_health_check"]?.phase).toBe(2);
  });
