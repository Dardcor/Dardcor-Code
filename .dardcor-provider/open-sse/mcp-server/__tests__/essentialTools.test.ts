 ...
 * The dardcorCode_web_search tests use InMemoryTransport + Client to exercise
...
describe("MCP Essential Tools", () => {
  ...
  describe("Tool schema validation", () => {
    it("should have exactly 12 essential tools (includes web_search + web_fetch + tool_search)", () => {
      // 11 -> 12: #8925 shipped dardcorCode_create_combo as a phase-1 tool.
      const schemas = MCP_ESSENTIAL_TOOLS;
      expect(schemas).toHaveLength(12);
    });

    it("all tools should have dardcorCode_ prefix", () => {
      const schemas = MCP_ESSENTIAL_TOOLS;
      for (const schema of schemas) {
        expect(schema.name).toMatch(/^dardcorCode_/);
      }
    });
  });
...
// ── dardcorCode_web_search: handler dispatch tests ──────────────────────────────
...
describe("dardcorCode_web_search handler (via MCP dispatch)", () => {
  let client: Client;
  ...
  it("should appear in tools/list after registration", async () => {
    ...
    const webSearch = tools.find((t) => t.name === "dardcorCode_web_search");
