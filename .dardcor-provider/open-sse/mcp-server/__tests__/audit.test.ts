...
// mock through the audit connection cache (globalThis.__dardcorCodeMcpAuditDb),
...
describe("MCP audit shutdown", () => {
  ...
  beforeEach(() => {
    vi.resetModules();
    globalThis.__dardcorCodeMcpAuditDb = undefined;
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-mcp-audit-"));
  ...
  it(
    ...
    async () => {
      ...
      globalThis.__dardcorCodeMcpAuditDb = mockDb as unknown as typeof globalThis.__dardcorCodeMcpAuditDb;

      await audit.logToolCall("dardcorCode_get_health", { ok: true }, { ok: true }, 12, true);
  ...
  it("still closes the audit database when checkpoint fails", async () => {
    ...
    await audit.logToolCall("dardcorCode_get_health", {}, {}, 5, true);
    expect(audit.closeAuditDb()).toBe(true);
    expect(mockDb.close).toHaveBeenCalledTimes(1);
  });
  ...
  it("falls back to node:sqlite when better-sqlite3 binding is missing", async () => {
    ...
    try {
      await audit.logToolCall("dardcorCode_get_health", { ok: true }, { ok: true }, 4, true);
