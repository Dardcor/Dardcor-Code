...
describe("dardcorCode_sync_pricing MCP tool schema", () => {
  it("should be registered in MCP_TOOLS", () => {
    const tool = MCP_TOOLS.find((t) => t.name === "dardcorCode_sync_pricing");
    expect(tool).toBeDefined();
    expect(tool?.phase).toBe(2);
  });

  it("should be in MCP_TOOL_MAP", () => {
    expect(MCP_TOOL_MAP["dardcorCode_sync_pricing"]).toBeDefined();
  });
