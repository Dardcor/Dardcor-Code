...
describe("cache MCP tools", () => {
  it("should be registered in MCP_TOOLS and MCP_TOOL_MAP", () => {
    expect(MCP_TOOLS.find((tool) => tool.name === "dardcorCode_cache_stats")).toBeDefined();
    expect(MCP_TOOLS.find((tool) => tool.name === "dardcorCode_cache_flush")).toBeDefined();
    expect(MCP_TOOL_MAP.dardcorCode_cache_stats).toBeDefined();
    expect(MCP_TOOL_MAP.dardcorCode_cache_flush).toBeDefined();
  });
...
describe("cache MCP tools registration", () => {
  ...
  it("should appear in tools/list after registration", async () => {
    ...
    expect(names).toContain("dardcorCode_cache_stats");
    expect(names).toContain("dardcorCode_cache_flush");
