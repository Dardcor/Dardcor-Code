...
declare global {
  var __dardcorCodeMcpAuditDb: AuditDatabase | null | undefined;
}
...
function getCachedAuditDb(): AuditDatabase | null {
  return globalThis.__dardcorCodeMcpAuditDb ?? null;
}

function setCachedAuditDb(database: AuditDatabase | null): void {
  globalThis.__dardcorCodeMcpAuditDb = database;
}
  ...
  if (!nodeSqliteFallbackAvailable()) {
    console.error(
      `[MCP Audit] better-sqlite3 native binding unavailable and Node ${process.version} ` +
        ...
        "`npm rebuild better-sqlite3` in the dardcorCode install root."
    );
    return null;
  }
 ...
 * Uses the same SQLite database as the main DardcorCode app.
 *
 * Driver priority:
...
async function getDb(): Promise<AuditDatabase | null> {
  ...
  try {
    ...
    const dbPath = process.env.DATA_DIR
      ...
      : join(homedir(), ".dardcorCode", "storage.sqlite");
