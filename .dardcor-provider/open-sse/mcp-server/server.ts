...
import { resolveDardcorCodeBaseUrl } from "../../src/shared/utils/resolveDardcorCodeBaseUrl.ts";
import { sanitizeErrorMessage } from "../utils/error.ts";
import { getMcpModelsCatalog } from "./catalog.ts";
export { getMcpModelsCatalog } from "./catalog.ts";

const OMNIROUTE_BASE_URL = resolveDardcorCodeBaseUrl();
...
function getDardcorCodeApiKey(): string {
...
export async function omniRouteFetch(path: string, options: RequestInit = {}): Promise<unknown> {
  ...
  const apiKey = getDardcorCodeApiKey();
  ...
  if (!response.ok) {
    ...
    throw new Error(`DardcorCode API error [${response.status}]: ${errorText}`);
  }
...
async function handleGetHealth() {
  ...
  try {
    ...
    await logToolCall("dardcorCode_get_health", {}, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_get_health", {}, null, Date.now() - start, false, msg);
...
async function handleListCombos(args: { includeMetrics?: boolean }) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_list_combos", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_list_combos", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleGetComboMetrics(args: { comboId: string }) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_get_combo_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_get_combo_metrics", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleSwitchCombo(args: { comboId: string; active: boolean }) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_switch_combo", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_switch_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCreateCombo(args: {
  ...
  try {
    ...
    await logToolCall("dardcorCode_create_combo", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_create_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCheckQuota(args: { provider?: string; connectionId?: string }) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_check_quota", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_check_quota", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleCostReport(args: { period?: string }) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_cost_report", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_cost_report", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleListModelsCatalog(args: { provider?: string; capability?: string }) {
  ...
  try {
    ...
    await logToolCall(
      "dardcorCode_list_models_catalog",
      args,
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_list_models_catalog", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebSearch(args: {
  ...
  try {
    ...
    await logToolCall("dardcorCode_web_search", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_web_search", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
async function handleWebFetch(args: {
  ...
  try {
    ...
    await logToolCall("dardcorCode_web_fetch", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_web_fetch", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export function createMcpServer(): McpServer {
  ...
  server.registerTool(
    "dardcorCode_get_health",
    {
      description:
        "Returns DardcorCode health status including uptime, memory, circuit breakers, rate limits, and cache stats",
      inputSchema: getHealthInput,
    },
    withScopeEnforcement("dardcorCode_get_health", async (args) => {
  ...
  server.registerTool(
    "dardcorCode_route_request",
    {
      description: "Sends a chat completion request through DardcorCode intelligent routing",
      inputSchema: routeRequestInput,
    },
    withScopeEnforcement("dardcorCode_route_request", (args) =>
      handleRouteRequest(routeRequestInput.parse(args))
    )
  );

  server.registerTool(
    "dardcorCode_cost_report",
    {
      description: "Generates a cost report for the specified period",
      inputSchema: costReportInput,
    },
    withScopeEnforcement("dardcorCode_cost_report", (args) =>
  ...
  server.registerTool(
    ...
    withScopeEnforcement("dardcorCode_set_resilience_profile", (args) =>
      handleSetResilienceProfile(setResilienceProfileInput.parse(args))
    )
  );

  server.registerTool(
    "dardcorCode_test_combo",
    {
      description:
    ...
    withScopeEnforcement("dardcorCode_test_combo", (args) =>
      handleTestCombo(testComboInput.parse(args))
    )
  );

  server.registerTool(
    "dardcorCode_get_provider_metrics",
    {
      description:
  ...
  server.registerTool(
    ...
    withScopeEnforcement("dardcorCode_get_session_snapshot", async (args) => {
  ...
  server.registerTool(
    "dardcorCode_db_health_check",
    {
      description:
        "Diagnoses or repairs DardcorCode database drift, including broken combo references and orphan quota/domain rows",
      inputSchema: dbHealthCheckInput,
    },
    withScopeEnforcement("dardcorCode_db_health_check", (args) =>
      handleDbHealthCheck(dbHealthCheckInput.parse(args ?? {}))
    )
  );

  server.registerTool(
    "dardcorCode_sync_pricing",
    {
      description:
        "Syncs pricing data from external sources (LiteLLM) into DardcorCode without overwriting user-set prices",
      inputSchema: syncPricingInput,
    },
    withScopeEnforcement("dardcorCode_sync_pricing", (args) =>
      handleSyncPricing(syncPricingInput.parse(args))
    )
  );

  server.registerTool(
    "dardcorCode_web_search",
    {
      description:
        "Performs a web search using DardcorCode's search gateway. Supports multiple providers (Serper, Brave, Perplexity, Exa, Tavily) with automatic failover. Returns search results with titles, URLs, snippets, and position data.",
      inputSchema: webSearchInput,
    },
    withScopeEnforcement("dardcorCode_web_search", (args) =>
      handleWebSearch(webSearchInput.parse(args))
    )
  );

  server.registerTool(
    "dardcorCode_web_fetch",
    {
      description:
        "Fetches and extracts content from a URL using DardcorCode's web fetch gateway. Supports multiple providers (Firecrawl, Jina Reader, Tavily) with automatic failover. Returns the page content as markdown, HTML, links, or screenshot, along with metadata.",
      inputSchema: webFetchInput,
    },
    withScopeEnforcement("dardcorCode_web_fetch", (args) => handleWebFetch(webFetchInput.parse(args)))
  );

  server.registerTool(
    "dardcorCode_cache_stats",
    {
      description:
    ...
    withScopeEnforcement("dardcorCode_cache_stats", () => handleCacheStats())
  );

  server.registerTool(
    "dardcorCode_cache_flush",
    {
      description:
    ...
    withScopeEnforcement("dardcorCode_cache_flush", (args) =>
      handleCacheFlush(cacheFlushInput.parse(args))
    )
  );

  server.registerTool(
    "dardcorCode_oneproxy_fetch",
    {
      description:
    ...
    withScopeEnforcement("dardcorCode_oneproxy_fetch", (args) =>
      handleOneproxyFetch(oneproxyFetchInput.parse(args))
    )
  );

  server.registerTool(
    "dardcorCode_oneproxy_rotate",
    {
      description:
    ...
    withScopeEnforcement("dardcorCode_oneproxy_rotate", (args) =>
      handleOneproxyRotate(oneproxyRotateInput.parse(args))
    )
  );

  server.registerTool(
    "dardcorCode_oneproxy_stats",
    {
      description:
    ...
    withScopeEnforcement("dardcorCode_oneproxy_stats", (args) =>
 ...
 * Called when `dardcorCode --mcp` is used.
 */
export async function startMcpStdio(): Promise<void> {
  ...
  console.error("[MCP] DardcorCode MCP Server starting (stdio transport)...");
  try {
    await server.connect(transport);
    console.error("[MCP] DardcorCode MCP Server connected and ready.");
  } finally {
    if (closeAuditDb()) {
