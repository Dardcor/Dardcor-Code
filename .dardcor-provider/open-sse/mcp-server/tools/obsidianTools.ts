...
export const obsidianTools = [
  ...
  },
  {
    ...
    description: "Get the DardcorCode sync plugin status: whether the server is running, vault name, port, uptime, and last sync results. Requires the sync auth token to be configured in DardcorCode settings.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({}),
    handler: async (_args: unknown, extra?: McpExtra) => {
  ...
  },
  {
    ...
    description: "Trigger an immediate bidirectional sync between desktop and mobile Obsidian vaults. Returns the sync result (files pulled, pushed, deleted, conflicts). Requires the sync auth token to be configured in DardcorCode settings.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({}),
    handler: async (_args: unknown, extra?: McpExtra) => {
      const token = getSyncToken() || requireToken(extractApiKeyId(extra));
  ...
  },
  {
    ...
    description: "List unresolved sync conflicts. Each conflict shows the file path, conflict file path, and when it was detected. Requires the sync auth token to be configured in DardcorCode settings.",
    scopes: ["read:obsidian"],
    inputSchema: z.object({}),
    handler: async (_args: unknown, extra?: McpExtra) => {
      const token = getSyncToken() || requireToken(extractApiKeyId(extra));
  ...
  },
  {
    ...
    description: "Resolve a sync conflict by choosing which version to keep. Use 'local' for the mobile version, 'remote' for the desktop version, or 'keep-both' to preserve both. Requires the sync auth token to be configured in DardcorCode settings.",
    scopes: ["write:obsidian"],
    inputSchema: z.object({
