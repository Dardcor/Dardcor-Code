/**
 * DardcorCode MCP Advanced Tools — 13 intelligence tools that differentiate
 * DardcorCode from all other AI gateways.
 *
 * Tools:
 *   1. dardcorCode_simulate_route     — Dry-run routing simulation
 *   2. dardcorCode_set_budget_guard   — Session budget with degrade/block/alert
 *   3. dardcorCode_set_routing_strategy — Runtime strategy switch for combos
 *   4. dardcorCode_set_resilience_profile — Circuit breaker/retry profiles
 *   5. dardcorCode_test_combo         — Live test each provider in a combo
 *   6. dardcorCode_get_provider_metrics — Detailed per-provider metrics
 *   7. dardcorCode_best_combo_for_task — AI-powered combo recommendation
 *   8. dardcorCode_explain_route      — Post-hoc routing decision explainer
 *   9. dardcorCode_get_session_snapshot — Full session state snapshot
 *  10. dardcorCode_db_health_check   — Diagnose and repair DB state drift
 *  11. dardcorCode_sync_pricing      — Sync provider pricing from external source
 */
...
export async function handleSimulateRoute(args: {
  ...
  try {
    ...
    await logToolCall("dardcorCode_simulate_route", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_simulate_route", args, null, Date.now() - start, false, msg);
...
export async function handleSetBudgetGuard(args: {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_set_budget_guard", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleSetRoutingStrategy(args: {
  ...
  try {
    ...
    await logToolCall("dardcorCode_set_routing_strategy", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_set_routing_strategy", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleSetResilienceProfile(args: {
  ...
  try {
    ...
    // Apply to DardcorCode via API using the plan-aligned resilience structure.
    ...
    await logToolCall("dardcorCode_set_resilience_profile", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall(
...
export async function handleTestCombo(args: { comboId: string; testPrompt: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_test_combo", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleGetProviderMetrics(args: { provider: string }) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_get_provider_metrics", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_get_provider_metrics", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleBestComboForTask(args: {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_best_combo_for_task", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleExplainRoute(args: { requestId: string }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_explain_route", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleSyncPricing(args: { sources?: string[]; dryRun?: boolean }) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_sync_pricing", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_sync_pricing", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleGetSessionSnapshot() {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_get_session_snapshot", {}, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleDbHealthCheck(args: { autoRepair?: boolean }) {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_db_health_check", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleCacheStats() {
  ...
  try {
    ...
    await logToolCall("dardcorCode_cache_stats", {}, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_cache_stats", {}, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleCacheFlush(args: { signature?: string; model?: string }) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_cache_flush", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_cache_flush", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleOneproxyFetch(
  ...
  try {
    ...
    await logToolCall("dardcorCode_oneproxy_fetch", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_oneproxy_fetch", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleOneproxyRotate(
  ...
  try {
    ...
    await logToolCall("dardcorCode_oneproxy_rotate", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_oneproxy_rotate", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
...
export async function handleOneproxyStats(args: Record<string, never> = {}) {
  ...
  try {
    ...
    await logToolCall("dardcorCode_oneproxy_stats", args, result, Date.now() - start, true);
    return { content: [{ type: "text" as const, text: JSON.stringify(result, null, 2) }] };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    await logToolCall("dardcorCode_oneproxy_stats", args, null, Date.now() - start, false, msg);
    return { content: [{ type: "text" as const, text: `Error: ${msg}` }], isError: true };
  }
}
