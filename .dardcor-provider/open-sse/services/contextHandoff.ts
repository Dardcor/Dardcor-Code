...
export const SKIP_UNIVERSAL_HANDOFF_FLAG = "_dardcorCodeSkipUniversalHandoff";
...
async function generateHandoffAsync(options: {
  ...
  const summaryBody = {
    ...
    temperature: 0.1,
    _dardcorCodeSkipContextRelay: true,
    _dardcorCodeInternalRequest: "context-handoff",
  };
...
async function generateUniversalHandoffAsync(options: {
  ...
  const summaryBody: Record<string, unknown> = {
    ...
    temperature: 0.1,
    _dardcorCodeSkipContextRelay: true,
    _dardcorCodeInternalRequest: "universal-handoff",
  };
