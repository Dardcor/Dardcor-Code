 ...
 *
 * Reference: DardcorCode issue #2260 + comment 4459544580 (Open WebUI bypass).
 */
 ...
 * verified against opencode→DardcorCode→Anthropic with claude-opus-4-7 OAuth.
 */
...
export const DEFAULT_SYSTEM_TRANSFORMS_CONFIG: SystemTransformsConfig = {
  providers: {
    [PROVIDER_CLAUDE]: {
      ...
      // usage` when opencode (or any non-claude-cli client) hits DardcorCode's
 ...
 *
 * `providerId` matches DardcorCode's provider key (`claude`,
...
const GLOBAL_KEY = "__dardcorCode_systemTransforms_config__";
