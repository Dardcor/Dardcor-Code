...
/**
 * Isolated `--config-dir` for DardcorCode-driven qodercli runs. Keeping it separate
...
export function getQoderCliConfigDir(): string {
  ...
  const base = dataDir || path.join(os.homedir() || os.tmpdir(), ".dardcorCode");
...
export async function runQoderCli(options: QoderCliRunOptions): Promise<QoderCliRunResult> {
  ...
  const args = [
    ...
    "--model",
    level,
    // Disable all built-in tools — DardcorCode only wants a plain LM reply, never
...
/**
 * Resolve an DardcorCode model id to the exact value to pass to `qodercli -m`.
...
export function buildQoderPrompt(body: unknown): string {
  ...
  const lines = [
    "You are answering an DardcorCode OpenAI-compatible request through the Qoder CLI transport.",
...
export function buildCosyHeadersForValidation(bodyStr: string, token: string) {
  ...
  const uid = "dardcorCode.user@qoder.sh";
  const userInfo = {
    uid: uid,
...
// DardcorCode mis-surfaced as "PAT may not be valid for the chat API". We mirror the
...
export async function validateQoderCliPat({
  ...
  if (run.timedOut) {
    return {
      valid: false,
      error:
        "qodercli timed out while validating the token. Check network/proxy access from the DardcorCode host.",
      unsupported: false,
    };
  }
