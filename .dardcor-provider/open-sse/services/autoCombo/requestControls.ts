 ...
 *
 *   X-DardcorCode-Mode:            fast | balanced | quality | <raw mode-pack name>  (#6024/#6025)
 *   X-DardcorCode-Budget:          <max USD per request>                             (#6023)
 *   X-DardcorCode-Budget-Fallback: cheapest | strict                                 (#3470)
 *
...
/**
 * Resolve the `X-DardcorCode-Mode` header value into a mode-pack override.
 *
...
/**
 * Parse the `X-DardcorCode-Budget` header into a hard per-request cost ceiling (USD).
...
/**
 * Parse the `X-DardcorCode-Budget-Fallback` header into a budget-fallback policy override.
...
export function resolveRequestAutoControls(headers: {
  ...
  const modeHeader = headers.get("x-dardcorCode-mode")?.trim() || null;
  const budgetHeader = headers.get("x-dardcorCode-budget")?.trim() || null;
  const budgetFallbackHeader = headers.get("x-dardcorCode-budget-fallback")?.trim() || null;
