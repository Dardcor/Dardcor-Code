 ...
 * not reliable here because DardcorCode may force a streaming envelope internally.
 */
 ...
 *
 * Supports BOTH wire formats DardcorCode may emit:
 ...
 * `x-dardcorCode-chaos-error` header and an error final chunk (status stays 200
...
export async function handleChaosChat(opts: {
  ...
  return new Response(stream, {
    ...
    headers: {
      ...
      "X-DardcorCode-Chaos": "true",
      "X-DardcorCode-Chaos-Panel": String(panel.length),
      "X-DardcorCode-Chaos-Primary": primaryModel ?? "",
    },
  });
}
