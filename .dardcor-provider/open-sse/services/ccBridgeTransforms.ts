 ...
 *
 * Goal: ensure the final request body DardcorCode sends to Anthropic's
 ...
 *
 * Related: DardcorCode issue #2260.
 */
...
export interface InjectBillingHeaderOp {
   ...
   *   - dardcorCode-daystamp: sha256(YYYY-MM-DD + version).slice(0,3)
   */
  versionFormat: "ex-machina" | "dardcorCode-daystamp";
  /**
 ...
 * proven against the live DardcorCode deployment (call log
...
/**
 * Compute the `cc_version` suffix per the DardcorCode native-OAuth algorithm:
...
export function buildBillingHeaderValue(
  ...
  const suffix =
    ...
    (options.versionFormat === "dardcorCode-daystamp"
