 ...
 * self-track pattern — DardcorCode sums the tokens it itself routed to this
 ...
 * MiMo self-track pattern above — DardcorCode sums the tokens it itself routed
...
export async function getXaiUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "xAI / Grok (DardcorCode-tracked)",
      quotas: {
        monthly: {
          used,
          total: 0,
