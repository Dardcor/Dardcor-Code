 ...
 * cannot reach the `tokenPlan/usage` endpoint), so DardcorCode self-tracks the
 ...
 * API to call. Instead we count the tokens DardcorCode itself routed to this
 ...
 * through DardcorCode, not the provider's own dashboard figure.
 */
export async function getXiaomiMimoUsage(connectionId: string) {
  ...
  try {
    ...
    return {
      plan: "Xiaomi MiMo Token Plan (DardcorCode-tracked)",
      quotas: {
