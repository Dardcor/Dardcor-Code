 ...
 * with the connection's OAuth access token, falling back to DardcorCode's own
...
export async function getXaiOauthUsage(
  ...
  try {
    ...
    const used =
      ...
      0;
    return {
      plan: "xAI OAuth (Grok) · DardcorCode-tracked",
      message: "Live weekly quota unavailable; showing DardcorCode-routed token totals only.",
      quotas: {
        monthly: {
          used,
          total: 0,
