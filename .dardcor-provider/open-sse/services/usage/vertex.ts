 ...
 * DardcorCode self-tracks the USD it spent through the connection (summed from
 ...
 * the USD that DardcorCode has spent through this connection since the account was added — summed
...
export async function getVertexUsage(connectionId: string, provider: string) {
  ...
  try {
    ...
    if (requests === 0) {
      return {
        plan: "Vertex AI",
        message: "Vertex connected. No usage recorded through DardcorCode yet for this account.",
        quotas: { spend },
      };
    }
