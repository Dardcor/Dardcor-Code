 ...
 * can fall back without treating DardcorCode's own deadline as a provider-connection failure.
...
/** Stable internal classification for DardcorCode's own combo per-target timer. */
  ...
  return async (
    ...
    const timeoutPromise = new Promise<Response>((resolve) => {
      timeoutId = setTimeout(() => {
        ...
        // HTTP 504 (not proprietary 524): this is DardcorCode's own per-target timer.
