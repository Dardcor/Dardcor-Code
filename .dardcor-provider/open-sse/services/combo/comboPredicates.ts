...
// #1731v2 guard: a provider circuit-breaker-open response (503 + `X-DardcorCode-Provider-Breaker`
// header / `provider_circuit_open` error code, see providerCircuitOpenResponse) is an DardcorCode
...
export function isProviderCircuitOpenResult(
  ...
  const breakerHeader = result.headers?.get?.("x-dardcorCode-provider-breaker");
