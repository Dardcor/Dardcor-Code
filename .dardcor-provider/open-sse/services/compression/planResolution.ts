...
/**
 * Interprets the `x-dardcorCode-compression` request header into a plan, or null when the
...
/** Renders the X-DardcorCode-Compression response header value. */
export function formatCompressionMeta(plan: DerivedPlan): string {
  return `${plan.mode}; source=${plan.source ?? "off"}`;
}

/**
 * Builds the annotation suffix for X-DardcorCode-Compression from compression stats.
 ...
 * ASCII-only: this string is appended to the X-DardcorCode-Compression HTTP header,
