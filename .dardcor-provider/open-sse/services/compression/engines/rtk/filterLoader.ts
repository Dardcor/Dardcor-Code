...
export interface RtkFilterLoadDiagnostic {
  ...
  format?: "dardcorCode-json" | "rtk-toml-v1";
  path?: string;
  level: "warning" | "error";
  message: string;
}
...
function getModuleDir(): string {
  ...
  return path.join(os.homedir(), ".dardcorCode");
}
...
function getDataDir(): string {
  return process.env.DATA_DIR || path.join(os.homedir(), ".dardcorCode");
}
...
function collectProjectFilterSources(sources: FilterSource[], options: RtkFilterLoadOptions): void {
  const projectCandidates = [
    ...
    { path: path.join(process.cwd(), ".rtk", "filters.json"), format: "dardcorCode-json" as const },
  ];
...
function collectGlobalFilterSources(sources: FilterSource[]): void {
  const globalCandidates = [
    ...
    { path: path.join(getDataDir(), "rtk", "filters.json"), format: "dardcorCode-json" as const },
  ];
...
export function loadRtkFilters(options: RtkFilterLoadOptions = {}): RtkFilterDefinition[] {
  ...
  const formatRank = { "rtk-toml-v1": 2, "dardcorCode-json": 1 } as const;
  const sorted = filters.sort(
    (a, b) =>
      sourceRank[b.source ?? "builtin"] - sourceRank[a.source ?? "builtin"] ||
      formatRank[b.sourceFormat ?? "dardcorCode-json"] -
        formatRank[a.sourceFormat ?? "dardcorCode-json"] ||
