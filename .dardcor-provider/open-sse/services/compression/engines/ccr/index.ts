...
/**
 * Handler for the `dardcorCode_ccr_retrieve` MCP tool.
 *
 ...
 * ENTIRE prompt for any caller that cannot resolve `dardcorCode_ccr_retrieve`
...
function processMessages(
  ...
  const result = messages.map((msg) => {
    ...
    // DardcorCode is used as a chat-completion PROVIDER, the upstream LLM has no
    // way to call `dardcorCode_ccr_retrieve` and expand markers — replacing tool
...
export const ccrEngine: CompressionEngine = {
  ...
  description:
    ...
    "retrievable via the `dardcorCode_ccr_retrieve` MCP tool (H4). " +
  ...
  apply(body: Record<string, unknown>, options?: CompressionEngineApplyOptions): CompressionResult {
    ...
    // #8033: teach MCP-capable callers the marker → dardcorCode_ccr_retrieve contract
