 ...
 * content — it can only call `dardcorCode_ccr_retrieve` if it (a) knows the tool
 ...
 * `dardcorCode_ccr_retrieve` (an MCP-capable caller). Plain OpenAI-compatible
 * callers that cannot reach the tool must never be told to call it.
 */

const CCR_RETRIEVE_TOOL_NAME = "dardcorCode_ccr_retrieve";
 ...
 * but only when:
 *  - the caller can actually reach `dardcorCode_ccr_retrieve` (see
