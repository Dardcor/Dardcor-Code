...
export function openaiResponsesToOpenAIRequest(
    ...
    for (const toolValue of tools) {
      ...
      if (
        ...
        throw unsupportedFeature(
          `Unsupported Responses API feature: ${toolType} tool type is not supported by dardcorCode`
        );
      }
    }
  }
  ...
  // DardcorCode is a forward proxy that streams responses synchronously —
  ...
  if (
    ...
    if (tcType === "function" && tc.name !== undefined && !tc.function) {
      ...
      if (mode !== "auto" && mode !== "required") {
        throw unsupportedFeature(
          `Unsupported Responses API feature: allowed_tools mode '${mode || "missing"}' is not supported by dardcorCode`
        );
      }
      ...
      throw unsupportedFeature(
        `Unsupported Responses API feature: tool_choice type '${tcType}' is not supported by dardcorCode`
      );
    }
  }
