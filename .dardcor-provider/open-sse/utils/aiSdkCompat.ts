 ...
 * `stream`. Without this hint, DardcorCode previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // DardcorCode accumulates the provider stream and converts it to JSON for the client
  ...
  // DardcorCode fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve DardcorCode's streaming default unless
