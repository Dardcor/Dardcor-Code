 ...
 *
 * When DardcorCode translates a Claude-native streamed response to OpenAI Chat
 ...
 *   - Explicit opt-in: `x-dardcorCode-thinking-marker: on` force-keeps it for the
 *     shrinking set of content-scanning clients (#4633).
 *   - Explicit opt-out: `x-dardcorCode-thinking-marker: off` (same as default).
...
export const THINKING_MARKER_HEADER = "x-dardcorCode-thinking-marker";
...
/**
 * Interpret the explicit `x-dardcorCode-thinking-marker` request header.
 ...
 *   2. Explicit `x-dardcorCode-thinking-marker` header → honor on/off (#5312)
...
export function resolveSuppressThinkClose(opts: {
  ...
  // Legacy content-scanning clients opt in with x-dardcorCode-thinking-marker: on.
  return true;
}
