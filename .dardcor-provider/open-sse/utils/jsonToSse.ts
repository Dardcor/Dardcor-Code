 ...
 * SSE stream. DardcorCode's streaming readiness check only recognizes SSE `data:`
...
export function synthesizeOpenAiSseFromJson(jsonText: string): string {
  ...
  const id = typeof parsed.id === "string" && parsed.id ? parsed.id : "chatcmpl-dardcorCode-sse";
