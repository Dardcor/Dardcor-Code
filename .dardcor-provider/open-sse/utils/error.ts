 ...
 * `x-dardcorCode-combo-*` headers and a `diagnostics` field in the OpenAI-shaped
 ...
 * carries a `recovery` hint it is mirrored as `x-dardcorCode-recovery-action` /
 * `x-dardcorCode-recovery-next-step` / `x-dardcorCode-retry-after-seconds` headers and as a
...
export function errorResponseWithComboDiagnostics(
  ...
  const headers: Record<string, string> = {
    ...
    "x-dardcorCode-combo-pool-size": String(safe.poolSize),
    "x-dardcorCode-combo-attempted": String(safe.attempted),
    "x-dardcorCode-combo-excluded": excludedHeader,
    "x-dardcorCode-combo-terminal-reason": toHeaderSafeAscii(safe.terminalReason.slice(0, 200)),
  };

  if (safe.recovery) {
    headers["x-dardcorCode-recovery-action"] = safe.recovery.action;
    ...
    headers["x-dardcorCode-recovery-next-step"] = toHeaderSafeAscii(safe.recovery.next_step).slice(
      0,
      128
    );
    if (
      ...
      headers["x-dardcorCode-retry-after-seconds"] = String(safe.recovery.retry_after_seconds);
    }
  }
  ...
  return new Response(
    ...
    }),
    {
      ...
      headers: {
        "Content-Type": "application/json",
        "Retry-After": String(retryAfterSec),
        "X-DardcorCode-Provider-Breaker": "open",
      },
    }
  );
}
