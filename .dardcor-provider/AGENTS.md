...
**Dardcor Code** — unified AI proxy/router. One endpoint, 291 LLM providers, auto-fallback.
...
Dardcor Code has three related but distinct temporary-failure mechanisms. Keep their
...
- **Imports**: external → internal (`@/`, `@Dardcor Code/open-sse`) → relative
...
repo** instead of polluting the main Dardcor Code tree.
...
```bash
gh issue list --repo diegosouzapw/Dardcor Code --state open \
...
This checkout is a fork of `diegosouzapw/Dardcor Code`. Keep fork-only deployment and personal
...
- **Path aliases**: `@/*` → `src/`, `@Dardcor Code/open-sse` → `open-sse/`, `@Dardcor Code/open-sse/*` → `open-sse/*`
- **Default port**: 20128 (API + dashboard on same port)
- **Data directory**: `DATA_DIR` env var, defaults to `~/.Dardcor Code/`
...
Dardcor Code has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
