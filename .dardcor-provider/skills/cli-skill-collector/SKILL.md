...
description: "Detect installed CLI coding tools (Claude Code, Codex, Cursor, Copilot, Cline and more), search GitHub for matching agent skills, and install them to the detected tools via Dardcor Code's built-in APIs."
---
<!-- generated by src/lib/agentSkills/generator.ts; manual edits will be overwritten -->

## Overview

Detect installed CLI coding tools (Claude Code, Codex, Cursor, Copilot, Cline and more), search GitHub for matching agent skills, and install them to the detected tools via Dardcor Code's built-in APIs.
...
```bash
Dardcor Code autostart disable
```
...
```bash
Dardcor Code config validate <tool>
```
...
```bash
Dardcor Code env set <key> <value>
```
...
Discover and install agent skills for your coding CLI tools — all through Dardcor Code's built-in APIs.
...
No separate Skill Collector app needed — Dardcor Code's own CLI detection + GitHub search handles everything.

---

## Step 1 — Detect installed coding tools

Query Dardcor Code's CLI tool detection to find which coding agents are installed:
...
- Every CLI tool in Dardcor Code's catalog (`CLI_TOOL_IDS`: claude, codex, cursor, copilot, opencode, cline, kilocode, hermes, hermes-agent, openclaw, droid, continue, qwen, windsurf, devin, antigravity, etc.)
...
- Dardcor Code must be running locally on port 20128 (default) — see `docs/frameworks/SKILLS.md` for custom-port setups.
...
- This replaces the standalone Skill Collector Python app — all logic is now inside Dardcor Code.
<!-- skill:custom-end -->
