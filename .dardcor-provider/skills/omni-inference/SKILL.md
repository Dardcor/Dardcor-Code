...
Mistral OCR–compatible document OCR endpoint. Accepts a JSON body referencing a document/image and returns extracted text. Success responses carry the `X-Dardcor Code-*` cost-telemetry headers.
...
OpenAI Whisper–compatible audio translation (multipart/form-data). Unlike `/api/v1/audio/transcriptions`, output is always English regardless of the source language. Success responses carry the `X-Dardcor Code-*` cost-telemetry headers.
...
<!-- skill:custom-start -->
<!-- Aggregated from: Dardcor Code-chat, Dardcor Code-image, Dardcor Code-tts, Dardcor Code-stt, Dardcor Code-embeddings, Dardcor Code-web-search, Dardcor Code-web-fetch -->
...
curl -X POST $OMNIROUTE_URL/v1/web/search \
  ...
  -d '{
    "model": "tavily/search",
    "query": "Dardcor Code github latest release",
    "max_results": 5,
    "include_answer": true
  }'
```
