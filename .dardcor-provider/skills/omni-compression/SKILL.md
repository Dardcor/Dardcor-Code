...
<!-- skill:custom-start -->
<!-- Migrated from skills/Dardcor Code-compression/SKILL.md (preserved curated content) -->
...
Dardcor Code compresses token payloads before forwarding to providers. No code changes required — set it once, it applies to all requests transparently.
...
When Dardcor Code is used with browser/Playwright MCP tools, it automatically compresses verbose accessibility-tree tool results. Enabled by default; configure thresholds:
...
```
Dardcor Code_compression_status     → current settings + savings analytics
Dardcor Code_compression_configure  → update mode/threshold/language
Dardcor Code_set_compression_engine → switch engine at runtime
```
