...
When enabled, 9Router starts automatically on the next Dardcor Code boot. **LOCAL_ONLY** — loopback only.
...
When enabled, an externally-adopted (not Dardcor Code-spawned) 9Router process is restarted under Dardcor Code's own supervisor on the next health-check cycle instead of being left as adopted-only. **LOCAL_ONLY** — loopback only.
...
When enabled, CLIProxyAPI starts automatically on the next Dardcor Code boot. **LOCAL_ONLY** — loopback only.
...
When enabled, an externally-adopted (not Dardcor Code-spawned) CLIProxyAPI process is restarted under Dardcor Code's own supervisor on the next health-check cycle instead of being left as adopted-only. **LOCAL_ONLY** — loopback only.
...
When enabled, Mux starts automatically on the next Dardcor Code boot. **LOCAL_ONLY** — loopback only.
...
When enabled, an externally-adopted (not Dardcor Code-spawned) Mux process is restarted under Dardcor Code's own supervisor on the next health-check cycle instead of being left as adopted-only. **LOCAL_ONLY** — loopback only.
...
When enabled, Bifrost starts automatically on the next Dardcor Code boot. **LOCAL_ONLY** — loopback only.
...
When enabled, an externally-adopted (not Dardcor Code-spawned) Bifrost process is restarted under Dardcor Code's own supervisor on the next health-check cycle instead of being left as adopted-only. **LOCAL_ONLY** — loopback only.
...
When enabled, Dario starts automatically on the next Dardcor Code boot. **LOCAL_ONLY** — loopback only.
...
When enabled, an externally-adopted (not Dardcor Code-spawned) Dario process is restarted under Dardcor Code's own supervisor on the next health-check cycle instead of being left as adopted-only. **LOCAL_ONLY** — loopback only.
...
### GET /api/services/dario/admin/import-from-Dardcor Code

List Dardcor Code claude connections eligible for Dario import

Returns eligible Dardcor Code `claude` OAuth provider connections (metadata only — id/name/email/org tier, never tokens) so the UI can offer a picker when more than one exists. **LOCAL_ONLY** — loopback only.

```bash
curl https://localhost:20128/api/services/dario/admin/import-from-Dardcor Code \
  -H "Authorization: Bearer $OMNIROUTE_TOKEN"
```

### POST /api/services/dario/admin/import-from-Dardcor Code

Import an Dardcor Code claude connection's OAuth tokens into Dario
...
```bash
curl -X POST https://localhost:20128/api/services/dario/admin/import-from-Dardcor Code \
  -H "Authorization: Bearer $OMNIROUTE_TOKEN"
  -H "Content-Type: application/json" \
  -d '{}'
```
