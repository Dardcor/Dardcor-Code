---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend Dardcor Code's automation surface."
---
<!-- generated by src/lib/agentSkills/generator.ts; manual edits will be overwritten -->

## Overview

Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend Dardcor Code's automation surface.
...
Read Crush CLI Dardcor Code config

Local-only. Reads the Dardcor Code provider block in Crush's config.
...
Write Crush CLI Dardcor Code config

Local-only. Registers Dardcor Code as an `openai-compat` provider in Crush's config.
...
Remove Dardcor Code from Crush CLI config

Local-only. Removes the Dardcor Code provider block from Crush's config.
...
Read CodeWhale CLI Dardcor Code config

Local-only. Reads the Dardcor Code config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI Dardcor Code config

Local-only. Writes the Dardcor Code config block in CodeWhale TOML format.
...
Remove Dardcor Code from CodeWhale CLI config

Local-only. Removes the Dardcor Code config block from CodeWhale's config.
