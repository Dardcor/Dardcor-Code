...
consume Dardcor Code via OpenAI-compatible REST in one fetch.
...
- **MCP tool**: `Dardcor Code_agent_skills_list` (scope `read:catalog`) — returns the full 42-skill catalog in one call.
...
`https://raw.githubusercontent.com/diegosouzapw/Dardcor Code/main/skills/<id>/SKILL.md`
...
| `omni-api-keys`        | API Keys                      | Create, list, rotate, and revoke Dardcor Code API keys. Control per-key scopes, spending limits, and expiration.                                                                                  |
...
| `omni-cli-tools`       | CLI Tools                     | Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend Dardcor Code's automation surface.                                                     |
| `omni-tunnels`         | Tunnels                       | Create and manage secure tunnels (ngrok, Cloudflare Tunnel, custom) to expose Dardcor Code to the internet or share access with remote agents and CI pipelines.                                   |
| `omni-sync-cloud`      | Cloud Sync                    | Synchronise Dardcor Code configuration, provider connections, and settings to/from cloud storage. Manage cloud worker authentication and remote backup targets.                                   |
...
| `omni-mcp`             | MCP Server                    | Connect to the Dardcor Code MCP server (37 tools, 3 transports: SSE/stdio/HTTP). Covers routing, cache, compression, memory, skills, providers, and audit tools across 16 permission scopes.      |
| `omni-agents-a2a`      | Agents & A2A Protocol         | Interact with Dardcor Code via JSON-RPC 2.0 agent-to-agent protocol. 6 built-in A2A skills: smart-routing, quota-management, provider-discovery, cost-analysis, health-report, list-capabilities. |
...
| ID                    | Name                          | Description                                                                                                                                                                                    |
| --------------------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cli-serve`           | CLI: Serve                    | Start, stop, and restart the Dardcor Code server from the CLI. Manage daemon mode, port configuration, auto-recovery, system tray integration, and the dashboard open shortcut.                   |
...
| `cli-keys`            | CLI: API Keys                 | Create, list, rotate, and revoke Dardcor Code API keys from the CLI. Manage OAuth flows for provider authentication and inspect key scopes and expiration.                                        |
...
| `cli-chat`            | CLI: Chat                     | Send chat completions, stream responses, and start an interactive REPL session from the CLI. Supports all Dardcor Code providers, combo routing, and system prompt configuration.                 |
...
| `cli-a2a`             | CLI: A2A Protocol             | Interact with the Dardcor Code A2A server from the CLI. Send tasks, inspect skill execution history, and test the JSON-RPC 2.0 agent-to-agent protocol interactively.                             |
...
| `cli-backup-sync`     | CLI: Backup & Sync            | Backup and restore Dardcor Code data from the CLI. Trigger incremental snapshots, sync to cloud storage, manage backup schedules, and restore from archive files.                                 |
...
- API entry: `https://raw.githubusercontent.com/diegosouzapw/Dardcor Code/main/skills/omni-auth/SKILL.md`
- CLI entry: `https://raw.githubusercontent.com/diegosouzapw/Dardcor Code/main/skills/cli-serve/SKILL.md`
...
## What makes Dardcor Code skills unique
