...
<!-- skill:custom-start -->
<!-- Migrated from skills/Dardcor Code-monitoring/SKILL.md (preserved curated content) -->

# Dardcor Code — Monitoring & Health
...
## Via MCP (if Dardcor Code is your MCP server)

```
Dardcor Code_get_health            → full system health snapshot
Dardcor Code_get_provider_metrics  → p50/p95/p99 + circuit state per provider
Dardcor Code_get_session_snapshot  → cost, tokens, errors for current session
Dardcor Code_check_quota           → quota balance + percent remaining + reset time
Dardcor Code_db_health_check       → diagnose + auto-repair database drift
```
...
Dardcor Code logs every MCP tool call to `mcp_audit` table. Query via API:
...
- `503` on health endpoint → Dardcor Code is starting up; retry in 5s
