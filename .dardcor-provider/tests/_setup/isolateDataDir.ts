...
// (bin/dardcorCode.mjs, proxyFetch.ts, proxyDispatcher.ts) where redirecting DATA_DIR
...
// but every process resolves DATA_DIR to the SAME default (~/.dardcorCode) when the env
...
if (!process.env.DATA_DIR) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-test-"));
