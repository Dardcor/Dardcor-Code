...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-crush-settings-"));
...
test("crush-settings POST: writes crush.json with an openai-compat providers.dardcorCode block", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        const provider = written.providers?.dardcorCode;
        assert.ok(provider, "providers.dardcorCode must be written");
...
test("crush-settings DELETE: removes only the dardcorCode provider entry", async () => {
  ...
  try {
      ...
      JSON.stringify({
        providers: {
          dardcorCode: {
            ...
            api_key: "sk-test",
            models: [
              { id: "openai/gpt-5", name: "DardcorCode: openai/gpt-5", context_window: 128000 },
            ],
          },
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        const written = JSON.parse(fs.readFileSync(configPath, "utf-8"));
        assert.equal(written.providers?.dardcorCode, undefined);
