 ...
 *
 * Plan 21 F8 — D16: dardcorCode_memory_search reads retrievalStrategy from settings.
 *
...
test("dardcorCode_memory_search: strategy=hybrid in DB → handler returns success", async () => {
  ...
  const handler = memoryTools.dardcorCode_memory_search.handler;
...
test("dardcorCode_memory_search: strategy=recent in DB → handler maps to exact, returns success", async () => {
  const db = core.getDbInstance();
...
test("dardcorCode_memory_search: hardcoded fallback config has retrievalStrategy=exact", async () => {
