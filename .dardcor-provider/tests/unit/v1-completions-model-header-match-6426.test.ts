...
// `X-DardcorCode-Model` header. Some upstreams (notably legacy text-completions)
...
test("#6426 body.model rewritten to resolved backend model aligns with header value", () => {
  ...
  const resolvedModel = "gpt-5.5"; // this is what attachDardcorCodeMetaHeaders wrote to X-DardcorCode-Model
  ...
  assert.equal(
    ...
    resolvedModel,
    "body.model must equal the resolved backend model that is in X-DardcorCode-Model header"
  );
});
