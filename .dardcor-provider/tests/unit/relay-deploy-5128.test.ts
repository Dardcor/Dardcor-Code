...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-5128-"));
...
test("#6416: Cloudflare worker script body is Service Worker syntax (no top-level export) + metadata uses body_part", async () => {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-cf-worker-"));
