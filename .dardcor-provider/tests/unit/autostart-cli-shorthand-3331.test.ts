 ...
 * (`serve --tray`) or the Electron Appearance tab; a plain `dardcorCode serve`
 * user had no way to enable it. The `autostart` command now exposes the
 * shorthand the reporter asked for (`dardcorCode autostart on` / `... true`) via
...
test("status is the default action (bare `dardcorCode autostart` is a safe read-only)", () => {
