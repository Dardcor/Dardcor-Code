...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-api-key-policy-"));
...
test("enforceApiKeyPolicy returns Anthropic error envelope for /v1/messages model denials", async () => {
  ...
  assert.doesNotMatch(body.error.message, /login|authenticate|api key|credential|dardcorCode/i);
