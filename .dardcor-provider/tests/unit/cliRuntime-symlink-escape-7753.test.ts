...
const sandboxHome = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-7753-home-"));
const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-7753-outside-"));
...
test("#7753: a genuinely unsafe symlink whose ORIGINAL location is also untrusted must still be rejected", async () => {
  ...
  const untrustedDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-7753-untrusted-"));
