/**
 * #6422 — X-DardcorCode-Compression response header echo.
 *
 * Docs promise that when a request supplies `x-dardcorCode-compression`, the response
 * echoes `X-DardcorCode-Compression: <mode>; source=<source>`. Internal early-returns
...
function makeRequest(headerValue: string | null): {
  ...
  return {
    headers: {
      get(name: string) {
        if (name.toLowerCase() === "x-dardcorCode-compression") return headerValue;
        return null;
      },
    },
  };
...
describe("compressionHeaderEcho (#6422)", () => {
  ...
  it("echoes the request header value onto the response when missing", () => {
    ...
    assert.equal(wrapped.headers.get("X-DardcorCode-Compression"), "engine:rtk; source=request-header");
    assert.equal(wrapped.status, 200);
  });

  it("normalizes off / default / engine:* to lowercase", () => {
    assert.equal(
      withCompressionHeaderEcho(new Response(""), "OFF").headers.get("X-DardcorCode-Compression"),
      "off; source=request-header"
    );
    assert.equal(
      withCompressionHeaderEcho(new Response(""), "Default").headers.get("X-DardcorCode-Compression"),
      "default; source=request-header"
    );
    assert.equal(
      withCompressionHeaderEcho(new Response(""), "Engine:Rtk").headers.get(
        "X-DardcorCode-Compression"
      ),
  ...
  it("preserves operator casing on named-combo values", () => {
    ...
    assert.equal(wrapped.headers.get("X-DardcorCode-Compression"), "MyCombo; source=request-header");
  });

  it("never overwrites an existing X-DardcorCode-Compression header set by the inner pipeline", () => {
    const inner = new Response("", {
      headers: {
        "X-DardcorCode-Compression": "stacked; source=routing; tokens=100->42; rules: rtk-nl x2",
      },
    });
    const wrapped = withCompressionHeaderEcho(inner, "engine:rtk");
    assert.equal(
      wrapped.headers.get("X-DardcorCode-Compression"),
  ...
  it("is a no-op when the request did not supply the header", () => {
    ...
    assert.equal(wrapped.headers.get("X-DardcorCode-Compression"), null);
