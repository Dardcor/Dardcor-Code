...
//
// The fix is a local helper (`dardcorCode login antigravity`) that runs the OAuth
...
test("blob is a single paste-safe line with the recognizable prefix", () => {
  const blob = encodeCredentialBlob(SAMPLE);
  assert.ok(blob.startsWith(CREDENTIAL_BLOB_PREFIX), "must carry the dardcorCode prefix");
