...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-context-handoff-"));
...
test("maybeGenerateHandoff persists a structured handoff once the threshold is reached", async () => {
  ...
  assert.equal(calls[0].body._dardcorCodeSkipContextRelay, true);
  assert.equal(calls[0].body._dardcorCodeInternalRequest, "context-handoff");
});
