...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-d1-discovery-"));
...
test("7. the 500 error path is sanitized — no stack trace or absolute source path leaks into the body", async () => {
  ...
  const rawMessage =
    "Query failed at /home/diegosouzapw/dev/proxys/DardcorCode-Enterprise/secret/catalog.ts:42:1";
