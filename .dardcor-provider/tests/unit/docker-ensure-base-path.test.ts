...
function makeStandaloneRoot(basePath = "") {
  const appRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-docker-base-"));
...
test("ensureDockerBasePath patches a root image when a subpath is configured", () => {
  const appRoot = makeStandaloneRoot("");
  const result = ensureDockerBasePath({ appRoot, env: { OMNIROUTE_BASE_PATH: "/dardcorCode" } });
  assert.equal(result.action, "patched");
  assert.equal(fs.readFileSync(path.join(appRoot, "BUILD_OMNIROUTE_BASE_PATH"), "utf8"), "/dardcorCode\n");
  const manifest = JSON.parse(
    fs.readFileSync(path.join(appRoot, ".build", "next", "routes-manifest.json"), "utf8")
  );
  assert.equal(manifest.basePath, "/dardcorCode");
});

test("ensureDockerBasePath rejects unset runtime on subpath images", () => {
  const appRoot = makeStandaloneRoot("/dardcorCode");
  assert.throws(
