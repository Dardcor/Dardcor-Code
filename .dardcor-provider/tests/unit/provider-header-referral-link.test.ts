...
test("resolveProviderHeaderLink: referral present => overrides the static website, isReferralLink=true", () => {
  const result = resolveProviderHeaderLink("https://groq.com", "https://groq.com/?ref=dardcorCode");
  assert.deepEqual(result, {
    website: "https://groq.com/?ref=dardcorCode",
    isReferralLink: true,
  });
});

test("resolveProviderHeaderLink: referral present even when catalog has no static website => still links out", () => {
  const result = resolveProviderHeaderLink(undefined, "https://groq.com/?ref=dardcorCode");
  assert.deepEqual(result, {
    website: "https://groq.com/?ref=dardcorCode",
    isReferralLink: true,
  });
