 ...
 * (`dardcorCode-tproxy-ca.crt`) so it never clobbers the static MITM cert slot.
...
test("installTproxyCa stages the PEM, copies it into the dedicated slot, refreshes, cleans up", async () => {
  ...
  assert.match(f.writes[0].path, /dardcorCode-tproxy-ca\.crt$/);
...
test("installTproxyCa never touches the static MITM cert slot", async () => {
  ...
  assert.ok(!joined.includes("dardcorCode-mitm.crt"), "must not collide with the MITM cert");
  assert.ok(joined.includes("dardcorCode-tproxy-ca.crt"));
});
