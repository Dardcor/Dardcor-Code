...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-sweep-reentrancy-"));
...
function isSweeping(): boolean {
  const hc = (globalThis as Record<string, unknown>).__dardcorCodeTokenHC as
