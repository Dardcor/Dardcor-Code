...
describe("dashboard guard public LLM API access", () => {
  ...
  it("allows loopback public LLM API without API key", async () => {
    const response = await proxy(request("/v1/chat/completions", { host: "localhost:21128" }));
  ...
  it("allows loopback rewritten public LLM API without API key", async () => {
    const response = await proxy(request("/api/v1/chat/completions", { host: "localhost:21128" }));
...
describe("dashboard guard local-only access", () => {
  ...
  it.each(["/api/agents", "/api/cloud-agents/status"])("protects agent path %s", async (pathname) => {
...
describe("dashboard guard always-protected routes", () => {
  ...
  it("rejects /api/auth/oidc/test without JWT or CLI token even when requireLogin=false", async () => {
  ...
  it("rejects /api/auth/oidc/test from loopback without JWT or CLI token", async () => {
    const response = await proxy(request("/api/auth/oidc/test", {
      host: "localhost:21128",
      origin: "http://localhost:21128",
    }));
  ...
  it("allows /api/auth/oidc/test with a valid JWT", async () => {
