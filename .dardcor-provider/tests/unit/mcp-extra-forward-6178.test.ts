...
// forwards. On the stdio transport `dardcorCode_ccr_retrieve` therefore fell back
...
// the registered dardcorCode_ccr_retrieve handler with an `extra` carrying that
...
test("static tool loops forward `extra` so stdio callers keep their scope/identity (#6178)", async () => {
  ...
  const retrieve = getRegisteredHandler(server, "dardcorCode_ccr_retrieve");
