 ...
 * (~/.omp/agent/agent.db) directly, per request — NOT DardcorCode's own DB.
...
describe("db/omp.ts — getOmpCredentials", () => {
  it("returns hasDardcorCode:false without throwing when the omp DB file does not exist", () => {
    assert.ok(!fs.existsSync(getOmpDbPath()), "precondition: no DB file yet");
    const creds = getOmpCredentials(PROVIDER_ID);
    assert.deepEqual(creds, { hasDardcorCode: false, baseUrl: null, apiKey: null });
  });

  it("returns hasDardcorCode:false when the DB exists but has no matching row", () => {
    seedOmpDb();
    const creds = getOmpCredentials(PROVIDER_ID);
    assert.deepEqual(creds, { hasDardcorCode: false, baseUrl: null, apiKey: null });
  });

  it("returns hasDardcorCode:false gracefully when the schema itself is missing (corrupt/foreign DB)", () => {
    const dbPath = getOmpDbPath();
    fs.mkdirSync(path.dirname(dbPath), { recursive: true });
...
describe("db/omp.ts — saveOmpCredentials + getOmpCredentials round trip", () => {
  it("persists apiKey/baseUrl so a subsequent read sees them", () => {
    ...
    const creds = getOmpCredentials(PROVIDER_ID);
    assert.equal(creds.hasDardcorCode, true);
...
describe("db/omp.ts — deleteOmpCredentials", () => {
  it("removes the row so a subsequent get reports hasDardcorCode:false", () => {
    seedOmpDb();
    saveOmpCredentials(PROVIDER_ID, "sk-test-omp-key", "http://localhost:20128/v1");
    assert.equal(getOmpCredentials(PROVIDER_ID).hasDardcorCode, true);

    deleteOmpCredentials(PROVIDER_ID);

    assert.deepEqual(getOmpCredentials(PROVIDER_ID), {
      hasDardcorCode: false,
      baseUrl: null,
      apiKey: null,
    });
  });
