...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-weak-rng-"));
...
test("migrationRunner probe table format uses crypto.randomUUID", async () => {
  const probeName = `__dardcorCode_fts5_probe_${crypto.randomUUID().replace(/-/g, "_")}`;
  assert.match(
    probeName,
    /^__dardcorCode_fts5_probe_[0-9a-f]{8}_[0-9a-f]{4}_[0-9a-f]{4}_[0-9a-f]{4}_[0-9a-f]{12}$/i
  );
});
