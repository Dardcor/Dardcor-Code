 ...
 *   (ROUTER_API_KEY is required). Set a router API key in DardcorCode and retry."
 *
 ...
 * exports it before launching DardcorCode. On a normal built-from-source install
 * neither is ever set, so `startMitm()` always received `""` even though
 * DardcorCode already had a usable API key sitting in its own DB. The fix falls
 ...
 * AgentBridge reuses an existing DardcorCode key instead of hard-failing.
 */
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-ab-routerkey-"));
...
// DardcorCode already has a usable key in its own DB. Before the fix this
...
test("resolveRouterApiKey: falls back to an existing DardcorCode API key (#6403)", async () => {
  const created = await createApiKey("AgentBridge Default", "machine-1");
  const resolved = await resolveRouterApiKey("");
  assert.equal(resolved, created.key, "must reuse the existing DardcorCode API key, not fail empty");
