...
test("runFabricatedDocsCheck: index contains real DardcorCode routes", () => {
  const result = runFabricatedDocsCheck();
...
test("cli-cmd: an arg-bearing `.command('connect <host>')` registration is NOT flagged", () => {
  ...
  const found = findingsFor({
    ...
    docs: { "guides/remote.md": "You log in once with `dardcorCode connect <host>`.\n" },
  });
  assert.ok(
    !found.has("cli-cmd::dardcorCode connect"),
...
test("ANTI-OVER-SUPPRESSION: an unregistered subcommand IS still flagged", () => {
  ...
  const found = findingsFor({
    ...
    docs: { "guides/remote.md": "Then run `dardcorCode teleport <host>` to finish.\n" },
  });
  assert.ok(
    found.has("cli-cmd::dardcorCode teleport"),
