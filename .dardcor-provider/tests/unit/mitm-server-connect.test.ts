...
test("C2 header contract — server.cjs intercept must inject x-dardcorCode-source and x-dardcorCode-agent", async () => {
  ...
  assert.match(
    src,
    /"x-dardcorCode-source":\s*"agent-bridge"/,
    'server.cjs must inject "x-dardcorCode-source: agent-bridge"'
  );
  assert.match(
    src,
    /"x-dardcorCode-agent":\s*agentId/,
    'server.cjs must inject "x-dardcorCode-agent: <id>" from the host→agent map'
  );
