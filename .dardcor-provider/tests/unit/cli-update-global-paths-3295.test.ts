...
// script, not process.cwd(). When DardcorCode is installed globally, the user's
...
test("getCurrentVersion resolves the real version from a foreign cwd (#3295)", async () => {
  ...
  const foreignCwd = mkdtempSync(path.join(tmpdir(), "dardcorCode-cwd-"));
  try {
...
test("createBackup resolves bin/ from a foreign cwd and copies cli/ recursively (#3295)", async () => {
  ...
  const foreignCwd = mkdtempSync(path.join(tmpdir(), "dardcorCode-cwd-"));
  const fakeHome = mkdtempSync(path.join(tmpdir(), "dardcorCode-home-"));
  try {
    process.chdir(foreignCwd); // no bin/ here → cwd-relative binPath would be missing
    process.env.HOME = fakeHome; // redirect ~/.dardcorCode/backups
    mkdirSync(fakeHome, { recursive: true });

    const backupDir = await update.createBackup();

    assert.ok(backupDir, "createBackup must return a path (not null)");
    // dardcorCode.mjs is a real file in bin/ and must be copied
    assert.ok(existsSync(path.join(backupDir, "dardcorCode.mjs")), "dardcorCode.mjs copied");
