...
test("discoverPlugins descobre plugin com package.json válido", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `dardcorCode-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "dardcorCode-cmd-test-hello");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "dardcorCode-cmd-test-hello",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
  ...
  try {
    const plugins = await discoverPlugins();
    assert.ok(
      plugins.some((p) => p.name === "dardcorCode-cmd-test-hello"),
...
test("discoverPlugins ignora pacotes sem prefixo dardcorCode-cmd-", async () => {
  const { discoverPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `dardcorCode-plugins-test-${Date.now()}`);
...
test("loadPlugins não quebra CLI quando plugin tem erro de load (try/catch)", async () => {
  ...
  const pluginDir = join(tmpdir(), `dardcorCode-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "dardcorCode-cmd-broken");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "dardcorCode-cmd-broken",
      version: "1.0.0",
      type: "module",
      main: "broken.mjs",
    })
...
test("loadPlugins carrega plugin válido e chama register()", async () => {
  const { loadPlugins } = await import("../../bin/cli/plugins.mjs");
  const pluginDir = join(tmpdir(), `dardcorCode-plugins-test-${Date.now()}`);
  const pkgDir = join(pluginDir, "dardcorCode-cmd-valid");
  mkdirSync(pkgDir, { recursive: true });
  writeFileSync(
    join(pkgDir, "package.json"),
    JSON.stringify({
      name: "dardcorCode-cmd-valid",
      version: "1.0.0",
      type: "module",
      main: "index.mjs",
    })
...
test("exemplo dardcorCode-cmd-hello existe e tem register()", () => {
  const examplePath = join(ROOT, "examples", "dardcorCode-cmd-hello", "index.mjs");
...
test("docs/frameworks/PLUGINS.md existe", () => {
  ...
  assert.ok(src.includes("dardcorCode-cmd"), "deve mencionar dardcorCode-cmd");
