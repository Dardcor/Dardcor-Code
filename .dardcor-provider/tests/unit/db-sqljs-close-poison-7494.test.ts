...
test(
  ...
  async () => {
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-7494-mech-"));
...
test(
  ...
  async () => {
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-7494-guard-"));
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    try {
      const { preInitSqlJs, getSqlJsAdapter } = await import(
        "../../src/lib/db/adapters/driverFactory"
...
test(
  ...
  async () => {
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-7494-real-"));
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    try {
