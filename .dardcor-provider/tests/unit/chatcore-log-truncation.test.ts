...
test("cloneBoundedChatLogPayload tail-truncates long arrays with a marker entry", () => {
  ...
  assert.equal(marker._dardcorCode_truncated_array, true);
...
test("cloneBoundedChatLogPayload caps object keys and records the dropped count", () => {
  ...
  // first maxKeys keys retained + the synthetic _dardcorCode_truncated_keys field
  assert.equal(cloned._dardcorCode_truncated_keys, 5);
