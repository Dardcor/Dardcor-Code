...
test("getDbInstance() eventually caps a persistently-OOMing sql.js probe (#6835)", async () => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-6835-oom-"));
  ...
  (
    globalThis as unknown as { __dardcorCodeSqlJsAdapters: Map<string, unknown> }
  ).__dardcorCodeSqlJsAdapters = new Map([[sqliteFile, fakeAdapter]]);
  const errors: string[] = [];
  for (let i = 0; i < 8; i++) {
    try {
      core.getDbInstance();
