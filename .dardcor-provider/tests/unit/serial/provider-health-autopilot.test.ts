...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-health-autopilot-"));
...
const accountFallback = await import("@dardcorCode/open-sse/services/accountFallback");
...
test("provider health autopilot action accepts LAN dashboard requests (#6277)", async () => {
  ...
  assert.equal(request.headers.get("x-dardcorCode-peer-ip"), null);
