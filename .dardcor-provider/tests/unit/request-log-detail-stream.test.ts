...
test("status discrepancy shows both DardcorCode and provider statuses", () => {
  const html = renderToStaticMarkup(
    React.createElement(RequestLoggerDetail, {
      log: {
        status: 504,
  ...
  assert.notEqual(
    html.indexOf("DardcorCode returned 504"),
    -1,
    "Should indicate DardcorCode returned its own status"
  );
});
