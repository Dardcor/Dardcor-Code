 ...
 *
 * `dardcorCode setup opencode` wires the bundled @dardcorCode/opencode-plugin into a
 * local OpenCode install: copies the built plugin into `<config>/plugins/dardcorCode/`
 * and registers a tuple entry in `opencode.json` (idempotent, replacing the legacy
 * `opencode-dardcorCode-auth` entry from issue #3711).
 *
...
function makeFakePluginDist() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "@dardcorCode/opencode-plugin", version: "0.0.0-test" })
  );
...
describe("dardcorCode setup opencode", () => {
  before(() => {
    console.log = () => {};
    console.info = () => {};
    console.warn = () => {};
  ...
  it("installs the plugin and registers a tuple entry honouring --base-url (camelCased by Commander)", async () => {
    ...
    assert.ok(fs.existsSync(path.join(CONFIG_DIR, "plugins", "dardcorCode", "dist", "index.js")));
    assert.ok(fs.existsSync(path.join(CONFIG_DIR, "plugins", "dardcorCode", "package.json")));

    const cfg = readConfig();
    assert.ok(Array.isArray(cfg.plugin));
    assert.equal(cfg.plugin.length, 1);
    const [modulePath, options] = cfg.plugin[0];
    assert.equal(modulePath, "./plugins/dardcorCode/dist/index.js");
    assert.equal(options.providerId, "dardcorCode");
    assert.equal(
      options.baseURL,
      "http://10.0.0.5:20128",
  ...
  it("is idempotent: re-running updates the entry in place instead of duplicating it", async () => {
    ...
    const omniEntries = cfg.plugin.filter(
      (p: unknown) =>
        Array.isArray(p) && (p[1] as { providerId?: string })?.providerId === "dardcorCode"
    );
  ...
  it("removes the legacy opencode-dardcorCode-auth entry (#3711) and preserves unrelated plugins", async () => {
    const cfgPath = path.join(CONFIG_DIR, "opencode.json");
    fs.writeFileSync(
      cfgPath,
      JSON.stringify({
        plugin: [
          "opencode-dardcorCode-auth",
    ...
    assert.ok(!flat.includes("opencode-dardcorCode-auth"), "legacy entry must be dropped");
    assert.ok(flat.includes('"providerId":"other"'), "unrelated plugin entries must survive");
    assert.equal(cfg.plugin.length, 2, "other + dardcorCode");
  });
