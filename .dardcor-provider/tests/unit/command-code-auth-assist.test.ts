...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-command-code-auth-"));
...
test("Command Code auth assist keeps auth URL callback on CLI localhost contract", async () => {
  process.env.OMNIROUTE_PUBLIC_BASE_URL = "https://dardcorCode.example.com/base-path";
