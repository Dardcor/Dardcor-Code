...
describe("installBasePathFetch", () => {
  ...
  it("prefixes absolute paths on fetch when basePath is set", async () => {
    ...
    globalThis.fetch = native as unknown as typeof fetch;
    try {
      const uninstall = installBasePathFetch("/dardcorCode");
      await fetch("/api/health/ping");
      assert.deepEqual(native.mock.calls[0].arguments, ["/dardcorCode/api/health/ping", undefined]);
      uninstall();
    } finally {
      globalThis.fetch = originalFetch;
    }
  ...
  it("does not double-prefix already-prefixed paths", async () => {
    ...
    try {
      const uninstall = installBasePathFetch("/dardcorCode");
      await fetch("/dardcorCode/api/health/ping");
      assert.deepEqual(native.mock.calls[0].arguments, ["/dardcorCode/api/health/ping", undefined]);
      uninstall();
    } finally {
      globalThis.fetch = originalFetch;
  ...
  it("restores native fetch after last uninstall", async () => {
    ...
    globalThis.fetch = native as unknown as typeof fetch;
    try {
      const a = installBasePathFetch("/dardcorCode");
      const b = installBasePathFetch("/dardcorCode");
      a();
      await fetch("/api/x");
      assert.deepEqual(native.mock.calls[0].arguments, ["/dardcorCode/api/x", undefined]);
      b();
