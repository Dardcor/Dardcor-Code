...
process.env.DATA_DIR = path.join(os.tmpdir(), "dardcorCode-check-tool-test");
...
async function writeTempFile(filename: string, content: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dardcorCode-clicheck-"));
...
async function writeCodexConfig(opts: {
  hasDardcorCode: boolean;
  authApiKey?: string;
}): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dardcorCode-codex-"));
  const configPath = path.join(tmpDir, "config.toml");

  const tomlContent = opts.hasDardcorCode
...
test("codex: returns 'configured' when TOML has DardcorCode URL + valid auth key", async () => {
  const configPath = await writeCodexConfig({
    hasDardcorCode: true,
    authApiKey: "sk_dardcorCode_testkey_1234567890abcdef",
  });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "configured");
});

test("codex: returns 'not_configured' when TOML has DardcorCode URL but auth key is masked", async () => {
  const configPath = await writeCodexConfig({
    hasDardcorCode: true,
    authApiKey: "sk_****",
  });
...
test("codex: returns 'not_configured' when TOML does not mention DardcorCode", async () => {
  const configPath = await writeCodexConfig({ hasDardcorCode: false });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "not_configured");
});
...
test("hermes: returns 'configured' when config contains DardcorCode", async () => {
  const configPath = await writeTempFile(
    "hermes.toml",
    `[openai]\nbase_url = "http://localhost:20128/v1"\n`
  );
...
test("droid: returns 'configured' when JSON config contains sk_dardcorCode marker", async () => {
  const configPath = await writeTempFile(
    "droid.json",
    JSON.stringify({ apiKey: "sk_dardcorCode_somekey", baseUrl: "http://localhost:20128/v1" })
  );
  const result = await checkToolConfigStatus("droid", configPath);
  assert.equal(result, "configured");
});

test("openclaw: returns 'configured' when JSON config contains dardcorCode text", async () => {
  const configPath = await writeTempFile(
    "openclaw.json",
    JSON.stringify({ openAiBaseUrl: "http://dardcorCode.local/v1", openAiApiKey: "sk-test" })
  );
...
test("kilo: returns 'not_configured' when no DardcorCode markers present", async () => {
  const configPath = await writeTempFile(
    "kilo.json",
...
test("qwen: returns 'configured' only for an DardcorCode-managed model entry", async () => {
  const configPath = await writeTempFile(
    "settings.json",
    JSON.stringify({
      modelProviders: {
