...
const RETRIEVE_TOOL_OPENAI = { type: "function", function: { name: "dardcorCode_ccr_retrieve" } };
const RETRIEVE_TOOL_FLAT = { name: "dardcorCode_ccr_retrieve" };
const RETRIEVE_TOOL_CLAUDE = { name: "dardcorCode_ccr_retrieve", input_schema: {} };
...
describe("ccr protocol instruction (#8033)", () => {
  ...
  it("instruction text contains the tool name, the marker shape, and the verbatim-24-char warning", () => {
    ...
    assert.ok(instruction.includes("dardcorCode_ccr_retrieve"), "must mention the tool name");
    assert.ok(
