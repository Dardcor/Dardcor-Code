...
test("custom Google OAuth callbacks preserve the requested callback path and query", () => {
  const redirectUri = resolveBrowserOAuthRedirectUri(
    ...
    { ...publicBaseEnv, NEXT_PUBLIC_BASE_URL: "https://dardcorCode.example.com/base" }
  );

  assert.equal(redirectUri, "https://dardcorCode.example.com/base/auth/callback?source=popup");
});
