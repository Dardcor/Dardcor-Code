...
test("media routes bypass DardcorCode's configured body-size limit", () => {
...
test("media routes never return DardcorCode's PAYLOAD_TOO_LARGE response", () => {
...
test("image edit body reader does not enforce an DardcorCode media limit", async () => {
  const request = new Request("http://localhost/api/v1/images/edits", {
    method: "POST",
    headers: { "content-length": String(Number.MAX_SAFE_INTEGER) },
    body: new Uint8Array([1, 2, 3, 4]),
