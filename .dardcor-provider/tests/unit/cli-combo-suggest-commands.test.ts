...
test("combo suggest chama dardcorCode_best_combo_for_task via MCP", async () => {
  ...
  assert.equal(capturedBody.name, "dardcorCode_best_combo_for_task");
...
test("combo suggest --switch chama /api/combos/switch com melhor combo", async () => {
  ...
  await (globalThis.fetch as any)("/api/mcp/tools/call", {
    method: "POST",
    body: '{"name":"dardcorCode_best_combo_for_task","arguments":{"task":"x"}}',
  });
