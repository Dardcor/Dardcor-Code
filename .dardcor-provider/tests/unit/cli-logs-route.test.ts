...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-cli-logs-route-"));
...
test("GET /api/cli-tools/logs returns empty array when log file does not exist", async () => {
  ...
  process.env.APP_LOG_FILE_PATH = "/tmp/dardcorCode-nonexistent-cli-logs-test.log";
