 ...
 * Helper.app bundles from build.productName ("DardcorCode"). The old code used
 * app.getName() (package.json `name` = "dardcorCode-desktop"); the two diverged, so it
...
const EXEC = "/Applications/DardcorCode.app/Contents/MacOS/DardcorCode";

describe("#7941 — resolveDarwinHelperExecutable()", () => {
  it("resolves the productName-derived framework Helper even though app.getName() would say 'dardcorCode-desktop'", () => {
    const frameworkHelper = path.join(
      path.dirname(EXEC),
      "..",
      "Frameworks",
      "DardcorCode Helper.app",
      "Contents",
      "MacOS",
      "DardcorCode Helper"
    );
  ...
  it("prefers the sibling (unsuffixed) Helper next to the main binary when present", () => {
    const sibling = path.join(path.dirname(EXEC), "DardcorCode Helper");
    const result = resolveDarwinHelperExecutable({
      execPath: EXEC,
