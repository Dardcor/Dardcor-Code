...
// Repro for #9455: dardcorCode stop reports success but supervisor respawns child.
//
...
function createTempDataDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-stop-sup-"));
}
...
test("Defect 1b: pid.mjs SERVICES array must include supervisor so killAllSubprocesses reaches it (#9455)", async () => {
  const tmpDir = os.tmpdir() + "/dardcorCode-sup-pid-" + Date.now();
