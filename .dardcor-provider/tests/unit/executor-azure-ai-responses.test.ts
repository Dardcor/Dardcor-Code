 ...
 *      providerSpecificData._dardcorCodeForceResponsesUpstream === true.
 *   3. The default chat path is preserved for non-responses requests.
 */
import test from "node:test";
import assert from "node:assert/strict";

import { DefaultExecutor } from "@dardcorCode/open-sse/executors/default.ts";
...
test("DefaultExecutor.buildUrl forces /responses path when _dardcorCodeForceResponsesUpstream=true", () => {
  const executor = new DefaultExecutor("azure-ai");

  const forced = executor.buildUrl("gpt-4.1", true, 0, {
    providerSpecificData: {
      baseUrl: "https://example-resource.services.ai.azure.com/openai/v1",
      _dardcorCodeForceResponsesUpstream: true,
    },
  });
