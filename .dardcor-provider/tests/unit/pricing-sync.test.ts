...
// ─── transformToDardcorCode ────────────────────────────────

describe("transformToDardcorCode", () => {
  test("converts LiteLLM per-token pricing to DardcorCode per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToDardcorCode(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToDardcorCode(raw);
