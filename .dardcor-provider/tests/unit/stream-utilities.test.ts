...
test("wantsProgress detects X-DardcorCode-Progress header correctly", () => {
  const headersObj = { "x-dardcorCode-progress": "true" };
  assert.equal(wantsProgress(new Headers(headersObj)), true);

  const headersMap = new Map([["x-dardcorCode-progress", "true"]]);
  assert.equal(wantsProgress(headersMap), true);

  const headersPlain = { "x-dardcorCode-progress": "true" };
  assert.equal(wantsProgress(headersPlain), true);

  assert.equal(wantsProgress({ "x-dardcorCode-progress": "false" }), false);
