...
test("Test 3: handleChat returns cached response directly for Semantic Cache hits", async () => {
  ...
  assert.equal(res2.headers.get("X-DardcorCode-Cache"), "HIT");
});

test("Test 4: handleChat supports X-DardcorCode-Progress tracking header for streams", async () => {
  ...
  assert.equal(response.status, 200);
  assert.equal(response.headers.get("X-DardcorCode-Progress"), "enabled");
...
test("handleChat returns cached response directly for Idempotency hits", async () => {
  ...
  assert.equal(response2.headers.get("X-DardcorCode-Idempotent"), "true");
...
test("handleChat returns Semantic Cache hit", async () => {
  ...
  const json2 = (await response2.json()) as any;
  assert.equal(response2.status, 200);
  assert.equal(response2.headers.get("X-DardcorCode-Cache"), "HIT");
