...
test("createRecordedTriageRun returns deterministic dry-run metadata", () => {
  ...
  assert.equal(run.repository, "KooshaPari/DardcorCode");
...
test("createRecordedTriageRun redacts URL credentials before returning metadata", () => {
  const run = createRecordedTriageRun({
    issueUrl: "https://user:token12345678901234567890@github.com/KooshaPari/DardcorCode/issues/1",
  });

  assert.equal(run.issueUrl, "https://[REDACTED]@github.com/KooshaPari/DardcorCode/issues/1");
...
test("createRecordedTriageRun summarizes recorded context without leaking secrets", () => {
  const run = createRecordedTriageRun({
    issueUrl: "https://github.com/KooshaPari/DardcorCode/issues/42",
    recordedContext: {
