...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-sem-cache-"));
...
const { formatDardcorCodeCost } = await import("../../src/domain/dardcorCodeResponseMeta.ts");
...
test("checkSemanticCache returns null when the x-dardcorCode-no-cache header forces a bypass", async () => {
  // The no-cache header makes isCacheableForRead return false even with temperature:0.
  const { args } = makeBaseArgs({
    semanticCacheEnabled: true,
    body: { model: "gpt-4o", messages: [{ role: "user", content: "hi" }], temperature: 0 },
    clientRawRequest: { headers: { "x-dardcorCode-no-cache": "true" } },
  });
...
test("checkSemanticCache returns a non-streaming JSON HIT with cache headers + logging side effects", async () => {
  ...
  assert.equal(res.headers.get(OMNIROUTE_RESPONSE_HEADERS.cache), "HIT", "X-DardcorCode-Cache: HIT");
  assert.equal(
...
// X-DardcorCode-Response-Cost must therefore be 0 (so billing consumers don't charge
// for cache hits), while the original cost is surfaced via X-DardcorCode-Cost-Saved.

test("checkSemanticCache HIT bills 0 incremental cost and reports the original cost in X-DardcorCode-Cost-Saved", async () => {
  clearCache();
  ...
  const expectedSaved = formatDardcorCodeCost(
  ...
  assert.equal(
    ...
    expectedSaved,
    "X-DardcorCode-Cost-Saved reflects the original cost the cache avoided"
  );
});
