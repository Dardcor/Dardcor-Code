...
test("peer routing stays disabled without an instance id", () => {
  ...
  assert.equal(
    inspectPeerRequest(new Headers({ "X-DardcorCode-Peer-Trace": "gateway-a" }), {}),
    null
  );
});

test("peer URL matching requires the configured origin and path boundary", () => {
  assert.equal(isConfiguredDardcorCodePeer("http://gateway-b:20128/v1/chat/completions", env), true);
  assert.equal(isConfiguredDardcorCodePeer("https://peer.example/api/v1/responses", env), true);
  assert.equal(
    isConfiguredDardcorCodePeer("http://gateway-b:20128/v10/chat/completions", env),
    false
  );
  assert.equal(
    isConfiguredDardcorCodePeer("http://gateway-b.evil:20128/v1/chat/completions", env),
    false
  );
});
...
test("outbound peer calls append the local instance to the existing trace", () => {
  ...
  const applied = applyPeerTraceHeader(
    headers,
    { "x-dardcorCode-peer-trace": "edge,gateway-z" },
    "http://gateway-b:20128/v1/chat/completions",
    env
  );
  assert.equal(applied, true);
  assert.equal(headers["X-DardcorCode-Peer-Trace"], "edge,gateway-z,gateway-a");
...
test("non-peer providers never receive peer metadata", () => {
  ...
  assert.equal(headers["X-DardcorCode-Peer-Trace"], undefined);
});

test("ingress rejects a repeated instance and an exhausted hop budget", () => {
  assert.deepEqual(
    inspectPeerRequest(new Headers({ "X-DardcorCode-Peer-Trace": "edge,gateway-a" }), env),
    {
      code: "peer_loop_detected",
      message: "DardcorCode peer routing loop detected",
    }
  );
  assert.deepEqual(
    inspectPeerRequest(
      { "X-DardcorCode-Peer-Trace": "gateway-w,gateway-x,gateway-y,gateway-z" },
      env
    ),
    {
      code: "peer_hop_limit_exceeded",
      message: "DardcorCode peer routing hop limit exceeded",
    }
  );
});
...
test("BaseExecutor adds the trace only on an allowlisted peer dispatch", async () => {
  ...
  const server = createServer((request, response) => {
    capturedTrace = request.headers["x-dardcorCode-peer-trace"];
  ...
  try {
    ...
    await executor.execute({
      ...
      clientHeaders: { "x-dardcorCode-peer-trace": "edge" },
      upstreamExtraHeaders: { "x-dardcorCode-peer-trace": "overridden" },
    });
    ...
    const response = await handleChat(
      new Request("http://gateway-a:20128/v1/chat/completions", {
        ...
        headers: {
          ...
          "X-DardcorCode-Peer-Trace": "gateway-b,gateway-a",
        },
        body: JSON.stringify({
          model: "steady-free",
