// Regression coverage for #4605 — `dardcorCode server --tray` showed no tray on
...
// ~/.dardcorCode/runtime by trayRuntime.ts), the icon was read from a
...
test("initSystrayUnix loads the injected SysTray ctor and builds the menu (#4605)", async () => {
  ...
  assert.ok(
    titles.includes("Quit DardcorCode"),
