...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "dardcorCode-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@dardcorCode\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@dardcorCode\/open-sse\/services\/model\.ts/);
