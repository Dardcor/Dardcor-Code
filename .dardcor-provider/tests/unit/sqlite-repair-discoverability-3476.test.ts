...
// ─── #3476: discoverability of `dardcorCode runtime repair` ──────────────────────
//
// When better-sqlite3's native binding mismatches the Node ABI, DardcorCode prints
...
// A self-heal command already exists (`dardcorCode runtime repair`); these tests
...
test("serve.mjs ABI-mismatch hint source mentions `runtime repair`", () => {
  ...
  assert.match(
    src,
    /runtime repair/,
    "serve.mjs should surface `dardcorCode runtime repair` as a recovery hint"
  );
});
