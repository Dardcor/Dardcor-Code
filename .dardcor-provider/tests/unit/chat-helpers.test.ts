...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-chat-helpers-"));
...
test("checkPipelineGates blocks providers with an open circuit breaker", async () => {
  ...
  assert.equal(response.headers.get("X-DardcorCode-Provider-Breaker"), "open");
});
...
test("withSessionHeader adds headers to mutable and immutable responses", async () => {
  ...
  assert.equal(mutable.headers.get("X-DardcorCode-Session-Id"), "sess_mutable");
  assert.equal(immutable.headers.get("X-DardcorCode-Session-Id"), "sess_redirect");
