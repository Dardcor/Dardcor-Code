...
function runCli(dataDir: string): { code: number | null; stderr: string } {
  ...
  // bin/dardcorCode.mjs picks up STORAGE_ENCRYPTION_KEY from the repo .env and
  // the bootstrap skips writing DATA_DIR/.env (the behaviour the test exercises).
  const isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-key-home-"));
  try {
...
// the CLI must persist the key into DATA_DIR (not just ~/.dardcorCode) so Docker/custom-DATA_DIR
...
test("CLI generates STORAGE_ENCRYPTION_KEY into DATA_DIR on first run (#1622)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-key-a-"));
  try {
    runCli(dir);
...
test("CLI refuses to auto-generate a key when a database already exists (#1622)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-key-b-"));
  try {
