...
// shapes (null when no cache fields), and the _dardcorCode meta attachment/merge.
...
test("attachLogMeta merges compact meta into _dardcorCode", () => {
  const out = attachLogMeta({ a: 1, _dardcorCode: { keep: true } }, { added: 2, drop: null });
  assert.deepEqual(out, { a: 1, _dardcorCode: { keep: true, added: 2 } });
});

test("attachLogMeta wraps non-object payloads", () => {
  const out = attachLogMeta(null, { added: 2 });
  assert.deepEqual(out, { _dardcorCode: { added: 2 }, _payload: null });
});
