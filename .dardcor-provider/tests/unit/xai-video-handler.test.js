 ...
 *  - connection id surfaced via x-dardcor-connection-id
...
describe("handleVideoCreate", () => {
  ...
  it("returns the serving connection id in x-dardcor-connection-id", async () => {
    ...
    expect(res.headers.get("x-dardcor-connection-id")).toBe("conn-77");
  ...
  it("rotates to the next account on 401 (auth errors cannot have created a job)", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.headers.get("x-dardcor-connection-id")).toBe("conn-2");
