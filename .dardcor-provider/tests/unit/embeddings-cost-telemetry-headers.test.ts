...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-embed-telemetry-"));
...
test("createEmbeddingResponse emits X-DardcorCode-* cost telemetry headers on success", async () => {
  ...
  try {
    ...
    assert.ok(cost, "X-DardcorCode-Response-Cost header must be present");
    assert.match(
      cost,
      /^\d+\.\d{10}$/,
      `X-DardcorCode-Response-Cost must be a 10-decimal cost string, got "${cost}"`
    );

    assert.equal(
      res.headers.get(OMNIROUTE_RESPONSE_HEADERS.tokensIn),
      String(PROMPT_TOKENS),
      "X-DardcorCode-Tokens-In must equal the upstream prompt_tokens"
    );

    const version = res.headers.get(OMNIROUTE_RESPONSE_HEADERS.version);
    assert.ok(
      version && version.length > 0,
      "X-DardcorCode-Version header must be present and non-empty"
    );
