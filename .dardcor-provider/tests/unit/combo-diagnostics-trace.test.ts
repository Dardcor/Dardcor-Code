 ...
 * both `x-dardcorCode-combo-*` headers and a `diagnostics` body field, while the
...
test("combo diagnostics: headers + body carry the sanitized trace (code override preserved)", async () => {
  ...
  assert.equal(res.headers.get("x-dardcorCode-combo-pool-size"), "3");
  assert.equal(res.headers.get("x-dardcorCode-combo-attempted"), "2");
  assert.match(res.headers.get("x-dardcorCode-combo-excluded") || "", /openai\/gpt-x:exhausted/);
  assert.equal(res.headers.get("x-dardcorCode-combo-terminal-reason"), "all_accounts_inactive");
...
test("combo diagnostics: JSON body keeps the original non-Latin1 text even though headers are ASCII-sanitized (#6612)", async () => {
  ...
  assert.equal(res.headers.get("x-dardcorCode-combo-terminal-reason"), terminalReason.replace("—", "?"));
  const body = await res.json();
