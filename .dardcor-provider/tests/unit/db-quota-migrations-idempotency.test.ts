...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-quota-mig-idem-"));
...
test("running migration runner a second time produces zero errors and identical schema", async () => {
  ...
  // migrations are already recorded in _dardcorCode_migrations — should be no-op.
  core.resetDbInstance();
