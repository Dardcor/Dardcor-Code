...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-settings-ext-"));
...
function cleanupGlobalDb() {
  try {
    if ((globalThis as any).__dardcorCodeDb?.open) {
      (globalThis as any).__dardcorCodeDb.close();
    }
  } catch {}
  delete (globalThis as any).__dardcorCodeDb;
}
