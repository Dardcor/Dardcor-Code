...
// coletava tests/unit/docs/) e apodreceu — filtrava dirs `dardcorCode*`, mas os skills
...
// comandos da CLI (`dardcorCode …`)" — 3 skills (health/keys/batches) usam só a CLI.
...
test("each skill dir has SKILL.md with frontmatter", async () => {
  ...
  for (const dir of dirs) {
    ...
    if (!dir.startsWith("omni-")) {
      assert.ok(
        content.includes("OMNIROUTE_") || content.includes("dardcorCode "),
        `${dir}: missing usage references (OMNIROUTE_* env vars or dardcorCode CLI commands)`
      );
    }
  }
});
