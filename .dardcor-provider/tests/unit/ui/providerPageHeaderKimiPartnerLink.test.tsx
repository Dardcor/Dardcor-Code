...
describe("ProviderPageHeader — Kimi partner-link note", () => {
  ...
  it.each([
    ["moonshot", "Kimi", "https://platform.kimi.ai?aff=dardcorCode"],
    ["kimi-coding", "Kimi Code CLI", "https://www.kimi.com/code?aff=dardcorCode"],
    ["kimi-web", "Kimi Web", "https://www.kimi.com/code?aff=dardcorCode"],
    ...
    expect(link?.getAttribute("title")).toBe(
      "Partner link — supports DardcorCode at no extra cost to you"
    );
    expect(link?.getAttribute("aria-label")).toBe(
      `${name} — Partner link — supports DardcorCode at no extra cost to you`
    );
    expect(el.textContent).toContain("Partner link — supports DardcorCode at no extra cost to you");
  });
