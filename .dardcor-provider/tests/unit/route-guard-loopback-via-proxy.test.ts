/**
 * Security regression: when DardcorCode itself runs behind an external reverse
...
test("classifyStampedPeerLocality: loopback socket WITH a proxy stamp is REMOTE (fail closed)", () => {
  // DardcorCode is behind nginx/Caddy/Cloudflare; the socket peer is the proxy.
...
test("classifyStampedPeerLocality: private-LAN socket WITH a proxy stamp is still REMOTE", () => {
  // Caddy/nginx running on a LAN box in front of DardcorCode. We do not know how
