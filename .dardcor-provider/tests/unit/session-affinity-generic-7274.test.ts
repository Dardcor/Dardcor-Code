 ...
 *    `x-session-id`, `x-dardcorCode-session`) are forwarded upstream by
 *    providers that use custom executors with no client-header passthrough.
 *    `x-codex-session-id` and `x-dardcorCode-session` are never forwarded by
...
// the real ~/.dardcorCode data dir instead of this test's isolated tmp dir.

const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-session-affinity-7274-"));
...
test("#7274 x-codex-session-id and x-dardcorCode-session are never forwarded upstream by DefaultExecutor", () => {
  ...
  const headers = executor.buildHeaders({ accessToken: "test-key" }, true, {
    ...
    "x-dardcorCode-session": "another-internal-correlation-id",
  ...
  assert.ok(
    !lowerKeys.includes("x-dardcorCode-session"),
    "x-dardcorCode-session must never reach the upstream request"
  );
});
