...
// `X-DardcorCode-Compression: off` so a globally-enabled Output Style (e.g. "Ultra terse") never
...
test("buildInternalChatRequest sends X-DardcorCode-Compression: off", () => {
  const controller = new AbortController();
  const request = buildInternalChatRequest({ model: "openai/gpt-4" }, controller.signal);
  assert.equal(request.headers.get("X-DardcorCode-Compression"), "off");
  assert.equal(request.headers.get("X-DardcorCode-No-Cache"), "true");
});

test("buildInternalRerankRequest sends X-DardcorCode-Compression: off", () => {
  const controller = new AbortController();
