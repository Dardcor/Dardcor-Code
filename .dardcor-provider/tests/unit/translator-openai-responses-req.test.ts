...
test("Responses -> Chat strips background flag and degrades to synchronous execution", () => {
  // Previously this threw 400 unsupported_feature. DardcorCode is a forward proxy
...
test("Chat -> Responses preserves prompt_cache_key and session affinity fields", () => {
  const result = openaiToOpenAIResponsesRequest(
    "gpt-5.3-codex",
    {
      ...
      session_id: "dardcorCode-session-abc",
  ...
  (assert as any).equal((result as any).session_id, "dardcorCode-session-abc");
...
test("Responses -> Chat: tool_search does not throw (issue #2766)", () => {
  ...
  // DardcorCode must not return 400 — it should silently drop the tool_search entry.
  assert.doesNotThrow(() =>
    openaiResponsesToOpenAIRequest(
      "gpt-4o",
      {
