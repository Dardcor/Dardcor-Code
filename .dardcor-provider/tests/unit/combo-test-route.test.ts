...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-combo-test-route-"));
...
test("combo test route marks a model healthy only when it returns assistant text", async () => {
  ...
  assert.equal(fetchCalls[0].init.headers["X-DardcorCode-No-Cache"], "true");
...
test("combo test route preserves structured step metadata for repeated model/account targets", async () => {
  ...
  assert.equal(fetchCalls[0].init.headers["X-DardcorCode-Connection"], "conn-openai-a");
  assert.equal(fetchCalls[1].init.headers["X-DardcorCode-Connection"], "conn-openai-b");
