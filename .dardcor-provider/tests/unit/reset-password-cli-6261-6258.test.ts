...
const OMNIROUTE_BIN = path.join(ROOT, "bin", "dardcorCode.mjs");
...
// ~/.dardcorCode so DATA_DIR is the only data directory in play.
function baseEnv(dataDir: string, isolatedHome: string): NodeJS.ProcessEnv {
  ...
  return {
    ...
    DATA_DIR: dataDir,
    HOME: isolatedHome,
    // Give the CLI a key so bin/dardcorCode.mjs never warns/provisions.
...
function mkHome(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-reset-home-"));
}
function mkDataDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-reset-data-"));
}

// #6261: `dardcorCode reset-password` must be a real subcommand (not "unknown
...
test("dardcorCode reset-password subcommand applies the reset over piped stdin (#6261, #6258)", async () => {
...
test("dardcorCode-reset-password applies the reset over piped two-line stdin (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
...
test("dardcorCode-reset-password --password-stdin reads the whole stdin as the password (#6258)", async () => {
  const dataDir = mkDataDir();
  const home = mkHome();
  try {
    const dbPath = seedDb(dataDir);
