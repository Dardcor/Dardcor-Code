...
// server binds the SERVER's loopback (localhost:PORT). When DardcorCode runs on a
...
test("remote Host → returns the reverse-tunnel hint with the exact callback port", () => {
  ...
  assert.equal(hint.tunnelCommand, "ssh -L 1455:127.0.0.1:1455 <user>@<dardcorCode-host>");
...
test("remote Host honours a random callback port (Windsurf/Devin OS-assigned port)", () => {
  const hint = buildRemoteOAuthHint("dardcorCode.example.com", 54321);
  assert.ok(hint.remoteHost === true);
  assert.equal(hint.tunnelCommand, "ssh -L 54321:127.0.0.1:54321 <user>@<dardcorCode-host>");
});
