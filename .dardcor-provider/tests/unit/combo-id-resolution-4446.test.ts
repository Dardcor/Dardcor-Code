...
// for a combo provisioned as "MASTER-LIGHT". DardcorCode's combo resolver only
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-combo-id-4446-"));
...
test("#4446 getComboForModel resolves a combo by a case-insensitive name (lowercased slug)", async () => {
  ...
  // The opencode `--model opencode-dardcorCode/master-light` path arrives as the
