...
test("mcp call envia name e arguments no body", async () => {
  ...
  await (globalThis.fetch as any)("/api/mcp/tools/call", {
    method: "POST",
    body: JSON.stringify({ name: "dardcorCode_get_health", arguments: {} }),
  });

  globalThis.fetch = origFetch;
  assert.ok(capturedUrl.includes("/api/mcp/tools/call"));
  assert.equal(capturedBody.name, "dardcorCode_get_health");
...
test("mcp call com --args passa argumentos como JSON", async () => {
  ...
  await (globalThis.fetch as any)("/api/mcp/tools/call", {
    method: "POST",
    body: JSON.stringify({ name: "dardcorCode_check_quota", arguments: { provider: "openai" } }),
  });
...
test("mcp tools list busca /api/mcp/tools", async () => {
  const TOOLS = [
    { name: "dardcorCode_get_health", scopes: ["read:health"], auditLevel: "low", phase: 1 },
    { name: "dardcorCode_list_combos", scopes: ["read:combos"], auditLevel: "low", phase: 1 },
  ];
  const origFetch = globalThis.fetch;
