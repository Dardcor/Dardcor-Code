...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-search-route-"));
...
test("v1 search POST uses stored Linkup credentials and returns normalized results", async () => {
  ...
  try {
    ...
    assert.equal(body.query, "dardcorCode linkup");
...
test("v1 search POST uses firecrawl credentials for unified firecrawl search", async () => {
  ...
  try {
    ...
    assert.equal(requestBody.query, "dardcorCode firecrawl");
