...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-model-sync-"));
...
test("modelSyncScheduler resolves only loopback origins and uses the dashboard port", async () => {
  ...
  process.env.OMNIROUTE_BASE_PATH = "/dardcorCode/";
  ...
  try {
    ...
    assert.equal(scheduler.getModelSyncInternalBaseUrl(), "http://127.0.0.1:22128/dardcorCode");
    assert.equal(
      scheduler.resolveModelSyncInternalBaseUrl("https://attacker.example/steal"),
      "http://127.0.0.1:22128/dardcorCode"
    );
    assert.equal(
...
test("modelSyncScheduler uses the listener-declared TLS scheme without trusting candidates", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      "https://localhost:22128/dardcorCode"
    );
    assert.equal(
