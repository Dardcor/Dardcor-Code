...
// kept listening (hence "DardcorCode is running") but every DB-touching route
...
test("issue #7773: a fatal (non-transient) boot-time DB init failure must be logged before it propagates", async () => {
  ...
  const fatalMessage =
    "[DB] Nenhum driver SQLite disponível para '/data/data/com.termux/files/home/.dardcorCode/storage.sqlite'. " +
