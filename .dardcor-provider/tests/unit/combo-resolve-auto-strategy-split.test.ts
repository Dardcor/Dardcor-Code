...
import { resolveAutoStrategyOrder } from "@dardcorCode/open-sse/services/combo/resolveAutoStrategy.ts";
...
// also supports a per-request `X-DardcorCode-Mode` override (relayOptions.mode) that can
...
test("per-request X-DardcorCode-Mode override changes the EFFECTIVE weights used for fallback ranking, not just selection", async () => {
