...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-combo-context-relay-"));
...
test("handleComboChat context-relay persists a handoff when codex quota reaches the warning threshold", async () => {
  ...
  const result = await handleComboChat({
    ...
    handleSingleModel: async (body) => {
      if (body._dardcorCodeInternalRequest === "context-handoff") {
        summaryCalls += 1;
        return okResponse({
          choices: [
            {
...
test("handleComboChat universal handoff detects model switch before recording current model", async () => {
  ...
  const result = await handleComboChat({
    ...
    handleSingleModel: async (body) => {
      if (body._dardcorCodeInternalRequest === "universal-handoff") {
        summaryCalls += 1;
        return okResponse({
          choices: [
            {
