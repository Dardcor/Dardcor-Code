...
test("ensureAndroidCacheDir: creates ~/.cache when missing on android", () => {
  const home = mkdtempSync(join(tmpdir(), "dardcorCode-android-cache-home-"));
...
test("ensureAndroidCacheDir: does not recreate when ~/.cache already exists", () => {
  const home = mkdtempSync(join(tmpdir(), "dardcorCode-android-cache-existing-"));
  const cacheDir = join(home, ".cache");
  mkdirSync(cacheDir);
  const env = {};
  let mkdirCalls = 0;
...
test("ensureAndroidCacheDir: respects an existing XDG_CACHE_HOME and creates that path", () => {
  const root = mkdtempSync(join(tmpdir(), "dardcorCode-android-cache-xdg-"));
...
test("ensureAndroidCacheDir: Termux-on-linux still prepares ~/.cache", () => {
  const home = mkdtempSync(join(tmpdir(), "dardcorCode-android-cache-termux-"));
...
test("CLI entrypoint calls ensureAndroidCacheDir before Commander/Next load", () => {
  const src = readFileSync(join(ROOT, "bin/dardcorCode.mjs"), "utf8");
  ...
  assert.ok(callIdx > 0, "dardcorCode.mjs must call ensureAndroidCacheDir()");
  assert.ok(programImportIdx > 0, "dardcorCode.mjs must still load program.mjs");
  assert.ok(
