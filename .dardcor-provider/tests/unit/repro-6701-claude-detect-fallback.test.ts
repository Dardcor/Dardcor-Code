/**
 * Repro for #6701 — Claude Code CLI reported "not found" in DardcorCode's
 ...
 * DardcorCode process, binary moved, etc.) it unconditionally reports
 ...
 * installed:true when the settings file exists on disk. DardcorCode's rewrite
...
describe("#6701 — claude detection should fall back to settings.json when binary is unresolvable", () => {
  ...
  before(() => {
    ...
    configHome = fs.mkdtempSync(path.join(os.homedir(), ".dardcorCode-test-6701-"));
