...
// failure was translated into "Run: dardcorCode runtime repair" guidance and aborted.
...
const { openDardcorCodeDb } = await import("../../bin/cli/sqlite.mjs");

test("#8826: openDardcorCodeDb() falls back to node:sqlite when better-sqlite3 native binding is missing (construction-time failure)", async (t) => {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-8826-"));
  t.after(() => {
  ...
  const result = await openDardcorCodeDb();

  assert.ok(result.db, "openDardcorCodeDb() should return a working db adapter");
  assert.equal(
    result.db.driver,
    "node:sqlite",
