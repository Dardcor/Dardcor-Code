...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-chatcore-translation-"));
...
test("chatCore normalizes native Claude Code messages for native Claude OAuth passthrough", async () => {
  ...
  const { call, result } = await invokeChatCore({
    ...
    body: {
      model: "dardcorCode/alias-that-should-resolve",
      max_tokens: 64,
...
test("chatCore serves a cached idempotent response without hitting the provider twice", async () => {
  ...
  assert.equal(second.result.response.headers.get("X-DardcorCode-Idempotent"), "true");
...
test("chatCore returns a semantic cache HIT for repeated deterministic requests", async () => {
  ...
  assert.equal(first.calls.length, 1);
  assert.equal(first.result.response.headers.get("X-DardcorCode-Cache"), "MISS");
  assert.equal(second.calls.length, 0);
  assert.equal(second.result.response.headers.get("X-DardcorCode-Cache"), "HIT");
...
test("chatCore skips semantic cache when disabled in settings", async () => {
  ...
  assert.equal(first.result.response.headers.get("X-DardcorCode-Cache"), "MISS");
  assert.equal(second.result.response.headers.get("X-DardcorCode-Cache"), "MISS");

  const payload = (await second.result.response.json()) as any;
  assert.equal(payload.choices[0].message.content, "fresh-2");
});

test("chatCore attaches DardcorCode response metadata headers to non-stream responses", async () => {
  const { result } = await invokeChatCore({
    provider: "claude",
    model: "claude-sonnet-4-6",
    body: {
  ...
  assert.equal(result.success, true);
  assert.equal(result.response.headers.get("X-DardcorCode-Provider"), "cc");
  assert.equal(result.response.headers.get("X-DardcorCode-Model"), "claude-sonnet-4-6");
  assert.equal(result.response.headers.get("X-DardcorCode-Cache-Hit"), "false");
  assert.equal(result.response.headers.get("X-DardcorCode-Tokens-In"), "12");
  assert.equal(result.response.headers.get("X-DardcorCode-Tokens-Out"), "3");
  assert.ok(Number(result.response.headers.get("X-DardcorCode-Latency-Ms")) >= 0);
  assert.match(String(result.response.headers.get("X-DardcorCode-Response-Cost")), /^\d+\.\d{10}$/);
});
...
test("chatCore does not expose provider request credentials in non-stream response headers", async () => {
  ...
  assert.equal(result.response.headers.get("X-DardcorCode-Cache"), "MISS");
});
...
test("chatCore records Claude prompt cache and cache usage metadata in call logs", async () => {
  ...
  assert.equal(result.success, true);
  assert.ok(detail);
  assert.equal(detail.requestBody._dardcorCode.claudePromptCache.applied, true);
  // Breakpoints: system[2] (1), message content (1), assistant response (1). Tools cache_control is stripped by base.ts.
  assert.equal(detail.requestBody._dardcorCode.claudePromptCache.totalBreakpoints, 3);
  assert.equal(detail.responseBody._dardcorCode.claudePromptCache.applied, true);
  assert.equal(detail.responseBody._dardcorCode.claudePromptCache.totalBreakpoints, 3);
  assert.equal(typeof detail.responseBody._dardcorCode.claudePromptCache.anthropicBeta, "string");
  assert.match(detail.responseBody._dardcorCode.claudePromptCache.anthropicBeta, /prompt-caching/i);
  assert.deepEqual(detail.responseBody._dardcorCode.claudePromptCacheUsage, {
    cacheReadTokens: 4,
...
test("chatCore injects progress events into streaming responses when requested", async () => {
  const { result } = await invokeChatCore({
    ...
    model: "gpt-4o-mini",
    accept: "text/event-stream",
    requestHeaders: { "x-dardcorCode-progress": "true" },
    body: {
      model: "gpt-4o-mini",
      stream: true,
  ...
  assert.equal(result.success, true);
  assert.equal(result.response.headers.get("X-DardcorCode-Progress"), "enabled");
...
test("chatCore emits final SSE metadata comments before [DONE] on streaming responses", async () => {
  ...
  assert.equal(result.success, true);
  assert.equal(result.response.headers.get("X-DardcorCode-Provider"), "openai");
  assert.equal(result.response.headers.get("X-DardcorCode-Model"), "gpt-4o-mini");
  assert.match(streamText, /: x-dardcorCode-response-cost=\d+\.\d{10}/);
  assert.match(streamText, /: x-dardcorCode-tokens-in=\d+/);
  assert.match(streamText, /: x-dardcorCode-tokens-out=\d+/);
  assert.ok(
    streamText.indexOf(": x-dardcorCode-response-cost=") < streamText.indexOf("data: [DONE]")
  );
});
...
test("buildStreamingResponseHeaders drops upstream compression and framing headers", () => {
  ...
  assert.equal(headers.get("X-DardcorCode-Cache"), "MISS");
});
...
test("chatCore skips streaming cache when X-DardcorCode-No-Cache header is set", async () => {
  let upstreamHits = 0;
  const sharedBody = {
    model: "gpt-4o-mini",
    stream: true,
  ...
  const first = await invokeChatCore({
    ...
    model: "gpt-4o-mini",
    accept: "text/event-stream",
    requestHeaders: { "x-dardcorCode-no-cache": "true" },
    body: sharedBody,
    responseFormat: "openai",
    responseFactory() {
      upstreamHits += 1;
