...
describe("Electron URL Validation", () => {
  ...
  it("should allow https URLs", () => {
    assert.equal(validateExternalUrl("https://github.com/diegosouzapw/DardcorCode").allowed, true);
  });
...
describe("Electron SQLite credential inspection", () => {
  ...
  function withTempDb(fn) {
    const dir = mkdtempSync(join(tmpdir(), "dardcorCode-electron-db-"));
  ...
  it("should return false when the database file does not exist", () => {
    assert.equal(hasEncryptedCredentials(join(tmpdir(), "missing-dardcorCode.sqlite")), false);
  });
});
