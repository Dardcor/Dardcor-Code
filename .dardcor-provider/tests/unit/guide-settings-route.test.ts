...
const DUMMY_HOME = path.join(os.tmpdir(), "dardcorCode-guide-settings-test-" + Date.now());
...
type HermesConfig = {
  ...
  providers?: { dardcorCode?: { base_url?: string; api_key?: string } };
};
...
test("guide-settings POST creates new hermes config.yaml if it doesn't exist", async () => {
  ...
  assert.equal(content.model?.provider, "dardcorCode");
  assert.equal(content.model?.base_url, "http://my-omni/v1");
  assert.equal(content.providers?.dardcorCode?.base_url, "http://my-omni/v1");
  assert.ok(String(content.providers?.dardcorCode?.api_key || "").startsWith("sk-"));
});
...
test("guide-settings POST writes OpenCode config with current schema and multi-model selection", async () => {
  ...
  assert.equal(content.provider.dardcorCode.npm, "@ai-sdk/openai-compatible");
  assert.equal(content.provider.dardcorCode.options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider.dardcorCode.options.apiKey.startsWith("sk-"));
  assert.deepEqual(Object.keys(content.provider.dardcorCode.models), [
...
test("guide-settings POST preserves existing OpenCode config fields while only updating provider.dardcorCode", async () => {
  await fs.mkdir(path.dirname(OPENCODE_CONFIG_PATH), { recursive: true });
  await fs.writeFile(
    OPENCODE_CONFIG_PATH,
    `{
  ...
  assert.equal(content.provider.dardcorCode.options.baseURL, "http://my-omni/v1");
  assert.ok(content.provider.dardcorCode.options.apiKey.startsWith("sk-"));
  assert.deepEqual(content.provider.dardcorCode.models, {
