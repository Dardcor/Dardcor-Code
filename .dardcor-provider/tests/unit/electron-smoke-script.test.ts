...
test("electron smoke discovers the default Linux executable name", () => {
  assert.ok(LINUX_EXECUTABLE_NAMES.includes("dardcorCode-desktop"));
});

test("electron smoke env allowlists runtime variables and drops secrets", () => {
  const env = buildSmokeEnv({
    currentPlatform: "linux",
    dataDir: "/tmp/dardcorCode-electron-smoke-test",
    parentEnv: {
      DISPLAY: ":99",
  ...
  assert.equal(env.DATA_DIR, "/tmp/dardcorCode-electron-smoke-test");
  assert.equal(env.DISPLAY, ":99");
  assert.equal(env.PATH, "/usr/bin");
  assert.equal(env.HOME, "/tmp/dardcorCode-electron-smoke-test/home");
  assert.equal(env.XDG_CONFIG_HOME, "/tmp/dardcorCode-electron-smoke-test/config");
