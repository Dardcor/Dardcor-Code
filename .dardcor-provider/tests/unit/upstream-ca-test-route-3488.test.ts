...
const DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-ca-datadir-"));
...
// A throwaway self-signed cert (CN=DardcorCode Test CA), valid to 2036.
...
const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-ca-test-"));
...
test("#3488 valid PEM cert → 200 ok with subject", async () => {
  ...
  assert.match(json.subject, /DardcorCode Test CA/);
});
