...
//   objects : 80 keys + _dardcorCode_truncated_keys is 81, so a second pass evicted a real key to
...
test("objects: second pass keeps the real keys and the true dropped count", () => {
  ...
  assert.deepEqual(twice, once, "re-bounding must be a no-op");
  assert.equal(once._dardcorCode_truncated_keys, 20, "100 keys minus the 80 retained");
  assert.equal(twice._dardcorCode_truncated_keys, 20, "the dropped count must not be recomputed");
  assert.equal(
    Object.keys(twice).filter((k) => k !== "_dardcorCode_truncated_keys").length,
    MAX_KEYS,
