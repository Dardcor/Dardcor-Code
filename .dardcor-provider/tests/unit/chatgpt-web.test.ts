...
test("Executor MODEL_MAP: DardcorCode IDs translate to ChatGPT backend slugs", async () => {
  reset();
  const m = installMockFetch();
  try {
...
test("Image gen: file-service:// pointer resolves to download URL and is appended as markdown (non-streaming)", async () => {
  ...
  try {
    ...
    // from the DardcorCode cache; clients see a stable /v1/chatgpt-web/image
...
test("Image gen: sediment:// pointer prefers /files/<id>/download over /attachment", async () => {
  ...
  try {
    ...
    // bytes that are cached behind the emitted DardcorCode image URL.
    assert.match(
      content,
...
test("Image gen: signed URL bytes are cached and exposed via /v1/chatgpt-web/image URL", async () => {
  ...
  // so we fetch the bytes, cache them locally, and emit an DardcorCode image URL.
  ...
  tls.__setTlsFetchOverrideForTesting(async (url, opts = {}) => {
    ...
    if (u.startsWith(downloadUrl)) {
      ...
      // back into bytes before putting the image in DardcorCode's cache.
      return {
        status: 200,
        headers: makeHeaders({ "Content-Type": "image/png" }),
...
test("Image edit: cached DardcorCode image URL continues the saved ChatGPT conversation", async () => {
  reset();
...
test("Image edit handler: no cached match returns 400 (does not silently generate unrelated image)", async () => {
  ...
  try {
    ...
    assert.match(
      ...
      /generated through this DardcorCode instance/
    );
