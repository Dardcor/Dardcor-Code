...
const argv = (...args: string[]) => ["node", "dardcorCode", ...args];
...
test("dardcorCode CLI --version fast-path prints ONLY the version, skipping bootstrap output", async () => {
  const pkg = JSON.parse(
    readFileSync(join(process.cwd(), "package.json"), "utf8")
  ) as { version: string };

  const { stdout } = await execFileAsync(process.execPath, ["bin/dardcorCode.mjs", "--version"], {
    cwd: process.cwd(),
