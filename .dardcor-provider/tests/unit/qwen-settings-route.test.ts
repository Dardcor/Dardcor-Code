...
const TEST_HOME = path.join(os.tmpdir(), `dardcorCode-qwen-route-${process.pid}-${Date.now()}`);
...
test("POST writes the shared V4 contract and preserves unrelated credentials", async () => {
  ...
  assert.deepEqual(settings.modelProviders.openai[1], {
    ...
    name: "qwen/qwen3.8-max-preview (DardcorCode)",
...
test("DELETE removes only DardcorCode-owned settings and env lines", async () => {
  const configured = {
    ui: { theme: "dark" },
