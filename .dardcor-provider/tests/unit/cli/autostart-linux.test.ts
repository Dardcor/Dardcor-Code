 ...
 *
 * `disableLinux()` runs `systemctl --user disable --now dardcorCode.service` and
 ...
 * HOME, so on any Linux developer machine that actually runs dardcorCode as a user
...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "dardcorCode-autostart-linux-"));
...
test("resolveCliPath finds dardcorCode.mjs from argv", async () => {
  ...
  const unitPath = join(tmpDir, ".config", "systemd", "user", "dardcorCode.service");
  const desktopPath = join(tmpDir, ".config", "autostart", "dardcorCode.desktop");
  ...
  if (existsSync(unitPath)) {
    ...
    assert.match(unit, /ExecStart=.*dardcorCode\.mjs.*serve --no-open/m);
