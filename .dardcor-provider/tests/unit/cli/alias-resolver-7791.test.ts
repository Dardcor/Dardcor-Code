 ...
 * `Cannot find package '@/shared'` when DardcorCode is installed globally
 * (issue #7791).
 *
...
describe("aliasResolver.resolveAlias — @dardcorCode/open-sse aliases", () => {
  it("exposes ALIAS_MAP with three entries matching tsconfig paths", () => {
    ...
    // @dardcorCode/open-sse/ (subpath)
    assert.equal(ALIAS_MAP[1].prefix, "@dardcorCode/open-sse/");
    assert.equal(ALIAS_MAP[1].target, "open-sse");
    assert.equal(ALIAS_MAP[1].exact, false);
    // @dardcorCode/open-sse (exact package name)
    assert.equal(ALIAS_MAP[2].prefix, "@dardcorCode/open-sse");
    assert.equal(ALIAS_MAP[2].target, "open-sse");
    assert.equal(ALIAS_MAP[2].exact, true);
  });

  it("resolves @dardcorCode/open-sse (bare) to open-sse/index.ts", () => {
    const got = resolveAlias("@dardcorCode/open-sse", REPO_ROOT);
    assert.ok(got, "expected non-null URL for @dardcorCode/open-sse");
    assert.ok(got.startsWith("file://"), "must be a file URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("resolves @dardcorCode/open-sse/services/usage to open-sse/services/usage.ts", () => {
    const got = resolveAlias("@dardcorCode/open-sse/services/usage", REPO_ROOT);
  ...
  it("resolves @dardcorCode/open-sse/utils/proxyFetch to open-sse/utils/proxyFetch.ts", () => {
    const got = resolveAlias("@dardcorCode/open-sse/utils/proxyFetch", REPO_ROOT);
    assert.ok(got, "expected non-null URL");
    const fsPath = fileURLToPath(got);
    assert.ok(
  ...
  it("returns null for non-existent @dardcorCode/open-sse/* paths", () => {
    assert.equal(resolveAlias("@dardcorCode/open-sse/does/not/exist", REPO_ROOT), null);
  });

  it("returns null for @dardcorCode/other (unmatched scope)", () => {
    assert.equal(resolveAlias("@dardcorCode/other", REPO_ROOT), null);
    assert.equal(resolveAlias("@dardcorCode/other/pkg", REPO_ROOT), null);
  });

  it("rejects path-traversal via @dardcorCode/open-sse/../../etc/passwd", () => {
    assert.equal(resolveAlias("@dardcorCode/open-sse/../../etc/passwd", REPO_ROOT), null);
  });
});
...
describe("aliasResolver end-to-end (#7791 regression)", () => {
  ...
  it("resolves @/shared/network/outboundUrlGuard from a child process", () => {
    ...
    // bin/dardcorCode.mjs does (await import("tsx/esm") first, then register).
    const script = `
