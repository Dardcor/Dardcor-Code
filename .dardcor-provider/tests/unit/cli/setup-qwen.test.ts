...
test("setup-qwen writes current V4 settings and only its dedicated env key", async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dardcorCode-setup-qwen-"));
...
test("setup-qwen does not overwrite an invalid settings file", async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "dardcorCode-setup-qwen-bad-"));
  const settingsPath = path.join(tempDir, "settings.json");
