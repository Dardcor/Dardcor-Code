...
test("postProcessOpencodeConfig replaces the literal API key with an env ref (no secret on disk)", () => {
  ...
  assert.equal(cfg.provider.dardcorCode.options.apiKey, "{env:OMNIROUTE_API_KEY}");
  assert.equal(cfg.provider.dardcorCode.options.baseURL, "http://vps:20128/v1");
});
...
test("postProcessOpencodeConfig --only filters the model map by substring", () => {
  ...
  assert.ok(cfg.provider.dardcorCode.models["glm/glm-5.2"]);
  assert.ok(cfg.provider.dardcorCode.models["kmc/kimi-k2.7"]);
  assert.equal("openai/gpt-4o" in cfg.provider.dardcorCode.models, false);
});
...
test("postProcessOpencodeConfig preserves $schema, provider name and npm", () => {
  ...
  assert.equal(cfg.provider.dardcorCode.npm, "@ai-sdk/openai-compatible");
});
