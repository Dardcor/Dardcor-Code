...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "dardcorCode-autostart-win-"));
...
test("Windows enable/disable writes and removes VBS in Startup folder", async () => {
  ...
  const vbsPath = join(startupDir, "DardcorCode.vbs");
  ...
  assert.match(vbs, /serve --no-open --tray/, "launches DardcorCode tray server");
