...
test("buildRooImport produces an openai-compatible provider profile (baseUrl /v1, model)", () => {
  ...
  const cfg = d.providerProfiles.apiConfigs.DardcorCode;
  ...
  assert.equal(d.providerProfiles.currentApiConfigName, "DardcorCode");
});
test("buildRooImport falls back to a placeholder key", () => {
  assert.equal(buildRooImport({ baseUrl: "http://x/v1", apiKey: "", model: "m" }).providerProfiles.apiConfigs.DardcorCode.openAiApiKey, "sk_dardcorCode");
});
test("buildRooVscodeAutoImport sets the pointer, preserving other settings", () => {
  const s = buildRooVscodeAutoImport({ "editor.tabSize": 2 }, "/home/u/.dardcorCode/roo-settings.json");
  assert.equal(s["editor.tabSize"], 2);
  assert.equal(s["roo-cline.autoImportSettingsPath"], "/home/u/.dardcorCode/roo-settings.json");
});
