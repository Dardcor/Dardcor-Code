...
test("quoteCodexArgs: exact win32 encoding (golden)", () => {
  const golden: Array<[string, string]> = [
    ["-c", '^^^"-c^^^"'],
    [
      'model_providers.dardcorCode.base_url="http://localhost:20128/v1"',
      '^^^"model_providers.dardcorCode.base_url=\\^^^"http://localhost:20128/v1\\^^^"^^^"',
    ],
    [
      "model_providers.dardcorCode.requires_openai_auth=false",
      '^^^"model_providers.dardcorCode.requires_openai_auth=false^^^"',
    ],
...
test("quoteCodexArgs does not mutate the caller's array", () => {
  const input = ["-c", 'model_provider="dardcorCode"'];
  quoteCodexArgs(input, "win32");
  assert.deepEqual(input, ["-c", 'model_provider="dardcorCode"']);
});
...
test(
  ...
  async () => {
    const dir = mkdtempSync(join(tmpdir(), "dardcorCode-codex-argv-"));
    try {
