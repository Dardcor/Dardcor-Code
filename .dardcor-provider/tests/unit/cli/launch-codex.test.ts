...
test("buildCodexEnv uses a no-auth sentinel when no token is given", () => {
  ...
  assert.equal(env.OMNIROUTE_API_KEY, "dardcorCode-no-auth");
});
...
test("buildCodexProviderArgs defines the dardcorCode provider inline (works without config.toml)", () => {
  const args = buildCodexProviderArgs("http://vps:20128");
  const joined = args.join(" ");
  assert.ok(joined.includes('model_provider="dardcorCode"'));
  assert.ok(joined.includes('model_providers.dardcorCode.base_url="http://vps:20128/v1"'));
  assert.ok(joined.includes('model_providers.dardcorCode.env_key="OMNIROUTE_API_KEY"'));
  assert.ok(joined.includes('model_providers.dardcorCode.wire_api="responses"'));
  assert.ok(joined.includes("model_providers.dardcorCode.requires_openai_auth=false"));
