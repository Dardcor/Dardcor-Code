...
const { AuggieExecutor, buildAuggiePrompt, resolveAuggieBin, resolveAuggieModel } =
  await import("@dardcorCode/open-sse/executors/auggie");

const TMP_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-auggie-test-"));
...
test("refreshCredentials() is a no-op (auggie has no DardcorCode-managed credentials)", async () => {
  const executor = new AuggieExecutor();
...
test("execute() spawns a model discovered via auto-fetch", async () => {
  const { __resetAuggieModels } = await import("@dardcorCode/open-sse/executors/auggie");
  __resetAuggieModels();
  const bin = writeFakeBin(
