...
const FIXTURE_DIR = join(tmpdir(), `dardcorCode_plugin_test_${Date.now()}`);
...
test("welcome banner PoC plugin lifecycle", async (t) => {
  ...
  await t.test("onResponse injects banner into response", async () => {
    ...
    assert.ok(result.choices[0].message.content.includes("[Welcome to DardcorCode"));
  ...
  await t.test("onResponse handles streaming delta", async () => {
    ...
    const result = await mod.plugin.onResponse({}, response);
    assert.ok(result.choices[0].delta.content.includes("[Welcome to DardcorCode"));
  });
