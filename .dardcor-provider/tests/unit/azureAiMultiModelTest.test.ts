...
test("buildInternalChatRequest sets X-DardcorCode-Connection header when connectionId is provided", () => {
  const reqWithConn = buildInternalChatRequest(
    { model: "azure-ai/gpt-4o" },
    new AbortController().signal,
    "conn-azure-123"
  );
  assert.equal(reqWithConn.headers.get("X-DardcorCode-Connection"), "conn-azure-123");

  const reqWithoutConn = buildInternalChatRequest(
    { model: "azure-ai/gpt-4o" },
    new AbortController().signal
  );
  assert.equal(reqWithoutConn.headers.get("X-DardcorCode-Connection"), null);
});
