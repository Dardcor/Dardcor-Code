...
import { IMAGE_PROVIDERS, parseImageModel } from "@dardcorCode/open-sse/config/imageRegistry.ts";
...
test("cheaperinference image models are NOT in the chat registry", async () => {
  const { REGISTRY } = await import("@dardcorCode/open-sse/config/providers/index.ts");
