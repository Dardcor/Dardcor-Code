...
test("builds compact OpenAI stream summary for detailed logs", () => {
  ...
  assert.equal(compact._dardcorCode_stream.stage, "provider_response");
  assert.equal(compact._dardcorCode_stream.eventCount, 3);
...
test("builds compact Claude stream summary for detailed logs", () => {
  ...
  assert.equal(compact._dardcorCode_stream.eventCount, 4);
});
