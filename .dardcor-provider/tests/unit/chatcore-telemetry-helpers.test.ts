...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-telemetry-"));
...
test("forwardDashboardEventToLiveWs POSTs event+payload+timestamp as JSON to the default port", async () => {
  ...
  assert.equal(capturedUrl, "http://127.0.0.1:20132/__dardcorCode_event");
...
test("forwardDashboardEventToLiveWs honors LIVE_WS_PORT override", async () => {
  ...
  assert.equal(capturedUrl, "http://127.0.0.1:31337/__dardcorCode_event");
});
