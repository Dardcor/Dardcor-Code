...
test("Fable 5 catalog exposes claude-fable-5 in cc — but NOT via Kiro (fabricated)", () => {
  ...
  // registry from DardcorCode's own Anthropic catalog and returned upstream 400
...
test("Kiro catalog does NOT expose Claude Opus (fabricated — Kiro upstream has no Opus)", () => {
  ...
  // copied into the Kiro registry from DardcorCode's Anthropic catalog and returned upstream
