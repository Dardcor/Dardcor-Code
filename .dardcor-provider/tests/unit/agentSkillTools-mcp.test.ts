...
test("dardcorCode_agent_skills_list is registered in MCP_TOOL_MAP", () => {
  const tool = MCP_TOOL_MAP["dardcorCode_agent_skills_list"];
  assert.ok(tool, "Tool should exist in MCP_TOOL_MAP");
  assert.equal(tool.name, "dardcorCode_agent_skills_list");
  assert.deepEqual(tool.scopes, ["read:catalog"]);
});

test("dardcorCode_agent_skills_get is registered in MCP_TOOL_MAP", () => {
  const tool = MCP_TOOL_MAP["dardcorCode_agent_skills_get"];
  assert.ok(tool, "Tool should exist in MCP_TOOL_MAP");
  assert.equal(tool.name, "dardcorCode_agent_skills_get");
  assert.deepEqual(tool.scopes, ["read:catalog"]);
});

test("dardcorCode_agent_skills_coverage is registered in MCP_TOOL_MAP", () => {
  const tool = MCP_TOOL_MAP["dardcorCode_agent_skills_coverage"];
  assert.ok(tool, "Tool should exist in MCP_TOOL_MAP");
  assert.equal(tool.name, "dardcorCode_agent_skills_coverage");
  assert.deepEqual(tool.scopes, ["read:catalog"]);
});
...
test("dardcorCode_agent_skills_list with no filters returns all 45 skills", async () => {
  const result = await agentSkillTools.dardcorCode_agent_skills_list.handler({});
  assert.equal(result.count, 45, `Expected 45 but got ${result.count}`);
  assert.ok(Array.isArray(result.skills));
  assert.equal(result.skills.length, 45);
});

test("dardcorCode_agent_skills_list({category:'api'}) returns exactly 23 entries", async () => {
  const result = await agentSkillTools.dardcorCode_agent_skills_list.handler({ category: "api" });
...
test("dardcorCode_agent_skills_list({category:'cli'}) returns exactly 21 entries", async () => {
  const result = await agentSkillTools.dardcorCode_agent_skills_list.handler({ category: "cli" });
...
test("dardcorCode_agent_skills_list result includes coverage shape", async () => {
  const result = await agentSkillTools.dardcorCode_agent_skills_list.handler({});
...
test("dardcorCode_agent_skills_list skill entries have required fields", async () => {
  const result = await agentSkillTools.dardcorCode_agent_skills_list.handler({});
...
// NOTE: dardcorCode_agent_skills_get calls fetchSkillMarkdown which tries:
...
test("dardcorCode_agent_skills_get({id:'omni-providers'}) returns correct skill metadata before markdown fetch", async () => {
...
test("dardcorCode_agent_skills_get({id:'cli-serve'}) resolves correct cli skill metadata", async () => {
  const { getSkillById } = await import("../../src/lib/agentSkills/catalog.ts");
...
test("dardcorCode_agent_skills_get with invalid id throws Error", async () => {
  await assert.rejects(
    () => agentSkillTools.dardcorCode_agent_skills_get.handler({ id: "non-existent-skill-xyz" }),
    (err: unknown) => {
...
test("dardcorCode_agent_skills_coverage({}) returns coverage shape", async () => {
  const result = await agentSkillTools.dardcorCode_agent_skills_coverage.handler({});
