...
// credential at all — so `dardcorCode contexts use <remote>` silently failed to
...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "dardcorCode-remote-test-"));
...
test("buildHeaders: active-context token wins over an opts.apiKey echoing the ambient env", async () => {
  ...
  // `dardcorCode connect <remote>` the active context holds the scoped token; an
...
test("createProgram wires the remote-mode commands into the real CLI program", async () => {
  ...
  // registerContexts() was never called by registry.mjs — so `dardcorCode contexts`
  // fell through to `serve`, and `connect`'s own advice ("dardcorCode contexts use
