...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-proxy-registry-"));
...
test("createProxyRegistrySchema accepts type:vercel and source:vercel-relay (schema gap-06)", async () => {
  ...
  // tests run with CWD=/DardcorCode, so `@/` aliases resolve to the main branch's src/.
  ...
  const result = createProxyRegistrySchema.safeParse({
    ...
    type: "vercel",
    host: "dardcorCode-relay-abc123.vercel.app",
    port: 443,
    source: "vercel-relay",
...
test("createProxy persists type:vercel and source:vercel-relay to DB (schema gap-06)", async () => {
  ...
  const created = await proxiesDb.createProxy({
    ...
    type: "vercel",
    host: "dardcorCode-relay-xyz.vercel.app",
    port: 443,
    source: "vercel-relay",
