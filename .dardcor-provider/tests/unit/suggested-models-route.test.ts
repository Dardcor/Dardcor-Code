...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-suggested-models-route-"));
...
test("route source: imports and uses buildErrorBody (Hard Rule #12)", async () => {
  ...
  assert.match(
    ...
    "must import buildErrorBody from @dardcorCode/open-sse/utils/error"
  );
