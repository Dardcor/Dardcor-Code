...
test("groups/route.ts: uses buildErrorBody from @dardcorCode/open-sse/utils/error", () => {
  assert.ok(listSrc.includes("buildErrorBody"), "route must use buildErrorBody — Hard Rule #12");
  assert.ok(
    listSrc.includes("@dardcorCode/open-sse/utils/error"),
    "route must import buildErrorBody from @dardcorCode/open-sse/utils/error",
  );
});
...
test("groups/[id]/route.ts: uses buildErrorBody from @dardcorCode/open-sse/utils/error", () => {
  assert.ok(idSrc.includes("buildErrorBody"), "route must use buildErrorBody — Hard Rule #12");
  assert.ok(
    idSrc.includes("@dardcorCode/open-sse/utils/error"),
    "route must import buildErrorBody from @dardcorCode/open-sse/utils/error",
  );
});
