...
test("a 'latest' path segment is NOT a test run (the bug that disabled backups)", () => {
  assert.equal(isAutomatedTestProcess(["node", "/opt/dardcorCode/latest/server.js"], NO_ENV), false);
  assert.equal(isAutomatedTestProcess(["node", "C:\\apps\\latest\\bin\\dardcorCode.mjs", "serve"], NO_ENV), false);
});
...
test("a production serve is never a test run", () => {
  assert.equal(
    isAutomatedTestProcess(["C:\\Program Files\\nodejs\\node.exe", "bin/dardcorCode.mjs", "serve"], { NODE_ENV: "production" } as NodeJS.ProcessEnv),
    false
  );
});
