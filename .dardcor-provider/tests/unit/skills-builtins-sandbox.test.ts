...
test("builtin skill handlers validate required fields and perform real sandboxed work", async () => {
  const dataDir = makeTempDir("dardcorCode-skills-builtins-");
...
test("sandboxRunner kill/killAll fallback naming matches containerProvider's SANDBOX_NAME convention", async () => {
  ...
  await withSandboxModule(
    ...
    async ({ sandboxRunner }) => {
      ...
      // containerProvider.ts's SANDBOX_NAME (`dardcorCode-${id}`), not the
      // pre-PR `dardcorCode-sandbox-${id}` convention.
      const proc = createFakeProcess();
      ...
      assert.equal(killCall.args[1], "dardcorCode-fallback-id");
      ...
      assert.ok(killAllNames.includes("dardcorCode-fallback-a"));
      assert.ok(killAllNames.includes("dardcorCode-fallback-b"));
    }
  );
});
...
test("containerProvider: buildKillCommand utility", () => {
  return importFresh("src/lib/skills/containerProvider.ts").then((mod) => {
    ...
    assert.equal(result.args[1], "dardcorCode-test-id");
  });
});
