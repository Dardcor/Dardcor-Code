...
test("generateCert() issues a cert whose SAN list covers all 4 antigravity hosts", async (t) => {
  // Isolate DATA_DIR so this test never touches (or reuses) a real ~/.dardcorCode cert.
  const tmpDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-mitm-cert-6494-"));
