...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-radar-inertia-"));
...
test("Radar inertia — flag off means zero behavioral delta", async (t) => {
  ...
  await t.test(
    ...
    () => {
      ...
      // cada release do DardcorCode, e um total fixo quebraria o teste pelo motivo
