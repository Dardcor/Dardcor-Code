...
function createTempRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-sync-env-"));
}
...
test("syncEnv creates .env from .env.example and generates install-time secrets", () => {
  ...
  // find the user's real DB at ~/.dardcorCode/ during tests
  ...
  try {
    ...
    assert.match(envContent, /^MACHINE_ID_SALT=dardcorCode-/m);
