...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "dardcorCode-tray-test-"));
  ...
  // dardcorCode.service`, which reach the caller's real systemd bus (XDG_RUNTIME_DIR,
  // not HOME) and therefore stopped and disabled the developer's REAL dardcorCode
