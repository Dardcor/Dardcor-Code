...
const BIN = path.join(ROOT, "bin", "dardcorCode.mjs");
...
test("CLI data-dir resolver preserves an existing legacy ~/.dardcorCode before XDG/app data", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-cli-data-dir-"));
  const home = path.join(tmp, "home");
  const legacyDir = path.join(home, ".dardcorCode");
...
test("CLI startup loads later non-conflicting .env files without overriding earlier values", () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-cli-env-layers-"));
  const home = path.join(tmp, "home");
  const dataDir = path.join(tmp, "data");
  const cwd = path.join(tmp, "cwd");
  const appDataDir =
    process.platform === "win32"
      ? path.join(tmp, "appdata", "dardcorCode")
      : path.join(home, ".dardcorCode");
