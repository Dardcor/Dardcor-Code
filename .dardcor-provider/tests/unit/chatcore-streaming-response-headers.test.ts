...
// x-dardcorCode-request-id, and the compression header only when meta is present.
...
test("merges upstream headers and sets x-dardcorCode-request-id", () => {
  const { build } = makeBuild();
  const h = assembleStreamingResponseHeaders(baseArgs(), build);
  assert.equal(h["x-upstream"], "kept");
  assert.equal(h["x-dardcorCode-request-id"], "preq-1");
});
