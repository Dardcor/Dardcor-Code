...
test("Gemini stream: converts textual Tool call block to structured tool_calls", () => {
      ...
      candidates: [
        {
          content: {
            parts: [
              {
                text: '[Tool call: terminal]\nArguments: {"command":"sqlite3 ~/.dardcorCode/storage.sqlite \\"SELECT name FROM sqlite_master WHERE type=\'table\';\\""}',
              },
            ],
          },
          finishReason: "STOP",
  ...
  assert.equal(
    ...
    JSON.stringify({
      command:
        "sqlite3 ~/.dardcorCode/storage.sqlite \"SELECT name FROM sqlite_master WHERE type='table';\"",
    })
  );
  assert.equal(
...
test("Gemini stream: converts prefixed textual Tool call block with zero-width chars", () => {
  ...
  assert.equal(
    toolCall.function.arguments,
    JSON.stringify({
      command: "sqlite3 ~/.dardcorCode/storage.sqlite",
    })
  );
  assert.equal(
    result.some((event: any) => event.choices?.[0]?.delta?.content?.includes("[Tool call:")),
...
test("Gemini stream: unwraps native functionCall args when emitted as JSON string", () => {
            ...
            parts: [
              {
                functionCall: {
                  name: "terminal",
                  args: JSON.stringify({
                    command: 'ssh test-vps "systemctl cat dardcorCode.service"',
                  }),
                },
              },
            ],
  ...
  assert.equal(
    toolCall.function.arguments,
    JSON.stringify({ command: 'ssh test-vps "systemctl cat dardcorCode.service"' })
  );
});
      ...
      candidates: [
        {
          content: {
            parts: [
              {
                text: '[Tool call: terminal]\nArguments: "{\\\"command\\\":\\\"ssh test-vps \\\\\\\"systemctl cat dardcorCode.service\\\\\\\"\\\"}"',
              },
            ],
          },
          finishReason: "STOP",
