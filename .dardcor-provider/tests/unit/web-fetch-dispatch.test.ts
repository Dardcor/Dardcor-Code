...
// #7339 — proves the synthetic dardcorCode_web_fetch tool call (emitted by
// webFetchInterception.ts, Phase 3-4 of #3384) is routed through the same
// generic dispatch path already proven for dardcorCode_web_search
// (handleToolCallExecution -> builtinSkills.web_fetch), including the
// error/abort-not-swallowed case (Hard Rule #6).
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-web-fetch-dispatch-"));
...
test("handleToolCallExecution routes dardcorCode_web_fetch to the web_fetch builtin handler and returns the tool-result envelope", async () => {
