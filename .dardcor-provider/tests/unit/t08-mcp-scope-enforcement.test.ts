...
test("evaluateToolScopes allows requests when enforcement is disabled", () => {
  const check = evaluateToolScopes("dardcorCode_switch_combo", [], false);
  assert.equal(check.allowed, true);
  assert.deepEqual(check.missing, []);
});

test("evaluateToolScopes denies tool execution when required scope is missing", () => {
  const check = evaluateToolScopes("dardcorCode_switch_combo", ["read:combos"], true);
...
test("evaluateToolScopes supports wildcard scopes", () => {
  const check = evaluateToolScopes("dardcorCode_get_health", ["read:*"], true);
  assert.equal(check.allowed, true);
  assert.deepEqual(check.missing, []);
});

test("evaluateToolScopes denies unknown tool names", () => {
  const check = evaluateToolScopes("dardcorCode_unknown_tool", ["*"], true);
  assert.equal(check.allowed, false);
