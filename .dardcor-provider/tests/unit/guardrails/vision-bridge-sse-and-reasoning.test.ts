/**
 * Regression tests for the vision-bridge describe call against DardcorCode's own
 ...
 *   DardcorCode's resolveStreamFlag() defaulted the request to `stream=true` and
...
test("vision-bridge: parses the diagnostics envelope { _streamed, summary }", async () => {
  // Some DardcorCode capture paths wrap the provider response in
