...
test(
  ...
  { timeout: 5_000 },
  async (t) => {
    const pluginDir = await mkdtemp(join(tmpdir(), "dardcorCode-plugin-loader-"));
...
test(
  ...
  { timeout: 5_000 },
  async (t) => {
    const pluginDir = await mkdtemp(join(tmpdir(), "dardcorCode-plugin-lifecycle-"));
    const entryPoint = join(pluginDir, "index.mjs");
    let loaded: LoadedPlugin | undefined;
...
// the promise settled, so every plugin load leaked one dardcorCode-plugin-host-*.mjs into
...
test(
  ...
  async (t) => {
    ...
    const hostScriptDir = await mkdtemp(join(tmpdir(), "dardcorCode-hostscript-"));
    ...
    assert.deepEqual(
      readdirSync(hostScriptDir).filter((f) => f.startsWith("dardcorCode-plugin-host-")).length,
      1,
    ...
    assert.deepEqual(
      readdirSync(hostScriptDir).filter((f) => f.startsWith("dardcorCode-plugin-host-")),
      [],
