...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-mgmt-auth-"));
...
test("internal service token requires the trusted loopback locality marker", async () => {
  ...
  const tokenHeader = "x-dardcorCode-internal-service-token";
  const local = new Request(`${BASE}/api/combos`, {
    headers: {
      [tokenHeader]: "internal-service-token-0123456789",
      "x-dardcorCode-peer-locality": "loopback",
    },
  });
  ...
  const remote = new Request(`${BASE}/api/combos`, {
    headers: {
      [tokenHeader]: "internal-service-token-0123456789",
      "x-dardcorCode-peer-locality": "remote",
    },
  });
