...
const dir = mkdtempSync(join(tmpdir(), "dardcorCode-interceptor-8181-"));
...
test("a non-EPIPE stream error is still fatal: it must be re-raised (#8181)", async () => {
  ...
  const childDir = mkdtempSync(join(tmpdir(), "dardcorCode-interceptor-8181-child-"));
...
test("the stdio guard is installed even when file logging is disabled (#8181)", () => {
  ...
  const childDir = mkdtempSync(join(tmpdir(), "dardcorCode-interceptor-8181-nofile-"));
