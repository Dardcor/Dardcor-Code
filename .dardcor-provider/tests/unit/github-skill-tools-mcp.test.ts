 ...
 *
 * - dardcorCode_github_skills_search
 * - dardcorCode_github_skills_scan
 * - dardcorCode_github_skills_install
 *
...
test("dardcorCode_github_skills_search: returns a well-shaped result for a valid search", async () => {
  const args = GitHubSkillsSearchSchema.parse({ minStars: 1, maxResults: 5 });
  const result = await githubSkillTools.dardcorCode_github_skills_search.handler(args);
...
test("dardcorCode_github_skills_scan: flags a blocked pattern as unclean", async () => {
  ...
  const result = await githubSkillTools.dardcorCode_github_skills_scan.handler(args);
...
test("dardcorCode_github_skills_scan: reports clean for benign content", async () => {
  const args = GitHubSkillsScanSchema.parse({
...
test("dardcorCode_github_skills_install: reports action 'planned' (honest — no file is actually cloned)", async () => {
  const args = GitHubSkillsInstallSchema.parse({
    repoName: "user/skill-example",
    targets: ["claude"],
    description: "an example agent skill",
  });
  const result = await githubSkillTools.dardcorCode_github_skills_install.handler(args);
...
test("dardcorCode_github_skills_install: error path never leaks a stack trace", async () => {
