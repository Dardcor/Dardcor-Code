...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-poe-api-8969-"));
...
test("#8969: resolveExecutionCredentials forces responses upstream for poe", () => {
  ...
  assert.equal(out.providerSpecificData._dardcorCodeForceResponsesUpstream, true);
});
