...
test("compose isolates guard audit mounts and waits for healthy guards", () => {
  ...
  assert.equal(
    services["dardcorCode-live"].depends_on?.["network-guard"].condition,
    "service_healthy"
  );
...
test("compose keeps role-separated credentials and proxy settings", () => {
  ...
  for (const [name, service] of Object.entries(services)) {
    ...
    assert.equal(sources.includes("devin-auth"), name === "dardcorCode-live", `${name} Devin auth`);
  }
  assert.equal(
    services["dardcorCode-live"].environment?.DEVIN_BRIDGE_PROXY_URL,
  ...
  for (const name of ["claude", "claude-live"]) {
    ...
    assert.equal(services[name].environment?.NO_PROXY, "dardcorCode");
  }
});
...
test("model discovery accepts only explicit uid/id fields and detects catalog ambiguity", () => {
  ...
  assert.throws(
    ...
    /Ambiguous DardcorCode catalog normalization/
  );
});
