...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-radar-api-"));
...
test("GET /api/radar/settings: flag on, authenticated, default state => optIn false, no key", async () => {
  ...
  assert.equal(body.contributorClaimUrl, "https://radar.dardcorCode.online/auth/github");
  assert.equal(body.supporterPlansUrl, "https://radar.dardcorCode.online/planos");
});
