...
test("G14 rejects executor implementation imports from src/app", async () => {
  for (const importPath of [
    "@dardcorCode/open-sse/executors/default.ts",
...
test("G14 allows imports through the intended boundaries", async () => {
  const cases = [
    ...
    {
      code: 'import { DefaultExecutor } from "@dardcorCode/open-sse/executors/default.ts";\nvoid DefaultExecutor;',
      filePath: "src/sse/handlers/example.ts",
    },
    {
      code: 'import { handleChat } from "@dardcorCode/open-sse/handlers/chat";\nvoid handleChat;',
