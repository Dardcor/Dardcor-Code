...
test("installCert under the guard skips the OS mutation but keeps input contracts", async () => {
  ...
  await assert.rejects(() => installCert("", "/nonexistent/dardcorCode-guard-test.pem"));
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-trust-guard-"));
  const pem = path.join(dir, "dardcorCode-guard-test.pem");
  fs.writeFileSync(
    pem,
