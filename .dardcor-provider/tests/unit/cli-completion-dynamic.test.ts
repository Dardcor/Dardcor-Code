...
test("runCompletionCommand bash retorna 0 e string não-vazia", async () => {
  ...
  assert.ok(out.includes("dardcorCode"), "bash script should mention dardcorCode");
  assert.ok(out.includes("_dardcorCode"), "bash script should define _dardcorCode function");
});
...
test("runCompletionCommand fish retorna 0", async () => {
  ...
  const out = chunks.join("");
  assert.ok(out.includes("dardcorCode"), "fish script should mention dardcorCode");
});
...
test("completion scripts incluem combos/providers/models no cache dinamicamente", async () => {
  ...
  const out = chunks.join("");
  assert.ok(
    out.includes("completion-cache.json") || out.includes("dardcorCode_get_cache"),
