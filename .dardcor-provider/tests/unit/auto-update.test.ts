...
test("auto update config normalizes env values and local source installs to source mode", () => {
  const config = autoUpdate.getAutoUpdateConfig({
    DATA_DIR: "/tmp/dardcorCode-data",
    AUTO_UPDATE_MODE: "npm",
    AUTO_UPDATE_REPO_DIR: "/workspace/custom-dardcorCode",
    AUTO_UPDATE_COMPOSE_FILE: "/workspace/custom-dardcorCode/compose.yml",
    AUTO_UPDATE_COMPOSE_PROFILE: "desktop",
    AUTO_UPDATE_SERVICE: "dardcorCode-desktop",
    AUTO_UPDATE_GIT_REMOTE: "upstream",
    AUTO_UPDATE_PATCH_COMMITS: "abc123, def456 ghi789",
    AUTO_UPDATE_LOG_PATH: "/tmp/dardcorCode-data/auto-update.log",
  });

  assert.equal(config.mode, "source");
  assert.equal(config.repoDir, "/workspace/custom-dardcorCode");
  assert.equal(config.composeFile, "/workspace/custom-dardcorCode/compose.yml");
  assert.equal(config.composeProfile, "desktop");
  assert.equal(config.composeService, "dardcorCode-desktop");
  assert.equal(config.gitRemote, "upstream");
  assert.deepEqual(config.patchCommits, ["abc123", "def456", "ghi789"]);
  assert.equal(config.logPath, "/tmp/dardcorCode-data/auto-update.log");
});
  ...
  const sourceValidation = await autoUpdate.validateAutoUpdateRuntime(
    {
      ...
      composeProfile: "cli",
      composeService: "dardcorCode-cli",
      gitRemote: "origin",
      patchCommits: [],
      logPath: "/tmp/log",
    },
...
test("auto update script builders generate npm, source, and docker-compose scripts with quoting and patch commits", () => {
  ...
  assert.match(npmScript, /npm install -g dardcorCode@3.6.0/);
  ...
  assert.match(npmScript, /pm2 restart dardcorCode \|\| true/);
...
test("launchAutoUpdate returns validation failures and starts detached update scripts when runtime is supported", async () => {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-autoupdate-"));
  ...
  const started = await autoUpdate.launchAutoUpdate({
    ...
    env: {
      ...
      AUTO_UPDATE_SERVICE: "dardcorCode-cli",
...
test("resolveProjectRoot walks up from start dir to nearest package.json or .git", () => {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-root-"));
  const subDir = path.join(tempRoot, "sub", "deep");
  fs.mkdirSync(subDir, { recursive: true });
  fs.writeFileSync(path.join(tempRoot, "package.json"), JSON.stringify({ name: "dardcorCode" }));
