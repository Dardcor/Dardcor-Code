 ...
 *
 * The Intel dmg is `DardcorCode-X.Y.Z.dmg` — no arch suffix. On Intel `process.arch` is `"x64"`,
...
files:
  - url: DardcorCode-3.8.49.dmg
    sha512: FSRnh09fSyOSeB7VXXe==
    size: 392717602
path: DardcorCode-3.8.49.dmg
sha512: FSRnh09fSyOSeB7VXXe==
...
files:
  - url: DardcorCode-3.8.49-arm64.dmg
    sha512: 2eVsFVsY0JiCKgNkNYg==
    size: 390627465
path: DardcorCode-3.8.49-arm64.dmg
sha512: 2eVsFVsY0JiCKgNkNYg==
...
test("parses electron-builder's manifest shape", () => {
  ...
  assert.equal(m.files[0].url, "DardcorCode-3.8.49.dmg");
  assert.equal(m.files[0].size, "392717602");
  assert.equal(m.path, "DardcorCode-3.8.49.dmg");
...
test("checksums and sizes travel untouched", () => {
  ...
  assert.equal(byUrl["DardcorCode-3.8.49.dmg"].sha512, "FSRnh09fSyOSeB7VXXe==");
  assert.equal(byUrl["DardcorCode-3.8.49-arm64.dmg"].size, "390627465");
});
...
test("hasArchSuffix recognizes only real arch markers", () => {
  assert.equal(hasArchSuffix("DardcorCode-3.8.49-arm64.dmg"), true);
  assert.equal(hasArchSuffix("DardcorCode-3.8.49-x64.dmg"), true);
  assert.equal(hasArchSuffix("DardcorCode-3.8.49-universal.dmg"), true);
  assert.equal(hasArchSuffix("DardcorCode-3.8.49.dmg"), false);
  // "arm64" must be a suffix segment, not any substring of the name.
  assert.equal(hasArchSuffix("DardcorCode-arm64beta-3.8.49.dmg"), false);
});
...
test("the rendered manifest round-trips", () => {
  ...
  assert.equal(reparsed.files[0].url, "DardcorCode-3.8.49.dmg", "order survives serialization");
  assert.equal(reparsed.version, "3.8.49");
  assert.equal(reparsed.path, "DardcorCode-3.8.49.dmg");
});
