...
describe("RTK custom filter lookup and trust", () => {
  it("skips untrusted project filters and reports a diagnostic", () => {
    const temp = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-rtk-project-"));
    process.chdir(temp);
  ...
  it("loads global filters before builtin fallback filters", () => {
    const tempProject = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-rtk-project-"));
    const tempData = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-rtk-data-"));
