...
import { smartFilterText } from "@dardcorCode/open-sse/services/compression/engines/mcpAccessibility/index.ts";
import { DEFAULT_MCP_ACCESSIBILITY_CONFIG } from "@dardcorCode/open-sse/services/compression/engines/mcpAccessibility/constants.ts";
...
test("collapse fires on interleaved tree AND no [ref=eXX] anchor is lost", () => {
  ...
  assert.ok(
    out.includes('items omitted by DardcorCode MCP filter'),
