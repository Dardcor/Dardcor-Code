...
describe("RTK raw output retention", () => {
  it("redacts secrets before raw output persistence and exposes a pointer", () => {
    const tempData = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-rtk-raw-"));
  ...
  it("retains only configured failure output and enforces byte caps", () => {
    const tempData = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-rtk-raw-cap-"));
    process.env.DATA_DIR = tempData;
  ...
  it("propagates raw output pointers from stacked RTK runs", () => {
    const tempData = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-rtk-stacked-raw-"));
    process.env.DATA_DIR = tempData;
  ...
  it("never throws when the raw-output write fails (disk error → null pointer)", () => {
    ...
    const blocker = path.join(os.tmpdir(), `dardcorCode-rtk-blocked-${process.pid}-${Date.now()}`);
