...
describe("formatCompressionAnnotation", () => {
  ...
  it("is ASCII-only so it survives HTTP header (X-DardcorCode-Compression) construction", () => {
    // Regression: the annotation is appended to the X-DardcorCode-Compression response
    ...
    assert.doesNotThrow(() => new Headers({ "X-DardcorCode-Compression": value }));
    const res = new Response(null, { headers: { "X-DardcorCode-Compression": value } });
    assert.equal(res.headers.get("X-DardcorCode-Compression"), value);
  });
  ...
  it("bounds high-cardinality rule telemetry before it reaches an HTTP response header", () => {
    ...
    assert.doesNotThrow(
      () => new Response(null, { headers: { "X-DardcorCode-Compression": annotation } })
    );
  });
