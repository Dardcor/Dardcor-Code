...
// T07 — dardcorCode_rtk_discover / dardcorCode_rtk_learn MCP tools (read-only; audited).
...
describe("RTK MCP tools (T07)", () => {
  it("dardcorCode_rtk_discover returns ranked noise candidates from the sample store", async () => {
    seedSamples();
  ...
  it("dardcorCode_rtk_learn returns a filter draft for a specific command", async () => {
    seedSamples();
  ...
  it("both tools are audited in mcp_tool_audit", async () => {
    ...
    assert.ok(names.includes("dardcorCode_rtk_discover"), "discover must be audited");
    assert.ok(names.includes("dardcorCode_rtk_learn"), "learn must be audited");
  });
});
