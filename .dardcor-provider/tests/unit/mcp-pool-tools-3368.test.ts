...
// imported or registered in open-sse/mcp-server/server.ts, so dardcorCode_pool_*
...
test("read tools require read:health; lifecycle tools require write:resilience", () => {
  ...
  assert.deepEqual(tools.dardcorCode_pool_status.scopes, ["read:health"]);
  assert.deepEqual(tools.dardcorCode_pool_sessions.scopes, ["read:health"]);
  assert.deepEqual(tools.dardcorCode_pool_health.scopes, ["read:health"]);
  assert.deepEqual(tools.dardcorCode_browser_pool_status.scopes, ["read:health"]);
  assert.deepEqual(tools.dardcorCode_pool_reset.scopes, ["write:resilience"]);
  assert.deepEqual(tools.dardcorCode_pool_warm.scopes, ["write:resilience"]);
});
