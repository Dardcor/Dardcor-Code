...
test("webFetchTool has the required McpToolDefinition shape", () => {
  assert.equal(webFetchTool.name, "dardcorCode_web_fetch");
...
test("webFetchTool is registered in MCP_TOOLS and MCP_TOOL_MAP", () => {
  ...
  assert.ok(toolNames.includes("dardcorCode_web_fetch"), "webFetchTool must be in MCP_TOOLS array");
  assert.ok("dardcorCode_web_fetch" in MCP_TOOL_MAP, "webFetchTool must be in MCP_TOOL_MAP");
});

// ── Scope mapping ──

test("dardcorCode_web_fetch is mapped in MCP_TOOL_SCOPES with execute:search", () => {
  const scopes = MCP_TOOL_SCOPES["dardcorCode_web_fetch"];
  assert.ok(scopes != null, "dardcorCode_web_fetch must have a scope mapping");
  assert.ok(
    scopes.includes("execute:search"),
    "dardcorCode_web_fetch must require execute:search scope"
  );
});
