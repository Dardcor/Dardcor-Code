...
const TEST_DATA_DIR =
  ...
  fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-issue-agent-execution-"));
...
test("issue-agent live triage traverses the normal chat-completions POST route", async () => {
  ...
  const response = await issueAgentRoute.POST(
    new Request("http://localhost/api/issue-agent/runs", {
      ...
      body: JSON.stringify({
        ...
        dryRun: false,
        issueUrl: "https://github.com/KooshaPari/DardcorCode/issues/5980",
        recordedContext: {
  ...
  assert.equal(body.runner, "dardcorCode-chat-completions");
