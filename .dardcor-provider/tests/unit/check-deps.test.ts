...
test("6A.8: discoverManifests finds root and workspace package.json files", () => {
  ...
  assert.ok(
    manifests.includes("@dardcorCode/opencode-plugin/package.json"),
    "@dardcorCode/opencode-plugin/package.json must be included"
  );
  assert.ok(
    manifests.includes("@dardcorCode/opencode-provider/package.json"),
    "@dardcorCode/opencode-provider/package.json must be included"
  );
});
