...
// `auto enable` no fallback legacy ("dardcorCode backup" sem subcomando) — o
...
test("backup auto enable — nenhuma opção é sombreada pelo parent backup", async () => {
  ...
  const dataDir = mkdtempSync(join(tmpdir(), "dardcorCode-backup-test-"));
...
// Regressão de documentação: `dardcorCode backup` sem subcomando é o uso
// canônico documentado em USER_GUIDE.md / CLI-TOOLS.md / AGENT-SKILLS.md
// ("dardcorCode backup # Snapshot config + DB"). Remover só as opções
...
test("backup — sem subcomando ainda cria um backup (uso legado documentado)", async () => {
  ...
  const dataDir = mkdtempSync(join(tmpdir(), "dardcorCode-backup-bare-test-"));
  const origDataDir = process.env.DATA_DIR;
  process.env.DATA_DIR = dataDir;
  try {
    const prog = new Command().exitOverride();
    ...
    assert.ok(
      ...
      "dardcorCode backup deve criar o diretório de backups"
    );
  } finally {
    if (origDataDir === undefined) delete process.env.DATA_DIR;
    else process.env.DATA_DIR = origDataDir;
