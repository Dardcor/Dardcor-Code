...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-codex-ws-policy-6564-"));
...
type CodexWsPrepareBody = {
  ...
  response: {
    ...
    _dardcorCodeReasoningRule?: unknown;
    _dardcorCodeReasoningRouteTrace?: unknown;
  };
  reasoningRouting: {
    ruleId: string;
    sourceModel: string;
...
function buildPrepareRequest(apiKey: string, model: string): Request {
  return new Request("http://localhost/api/internal/codex-responses-ws", {
    ...
    headers: {
      ...
      "x-dardcorCode-ws-bridge-secret": process.env.OMNIROUTE_WS_BRIDGE_SECRET as string,
    },
    body: JSON.stringify({
      action: "prepare",
...
test("WS reasoning rules apply Codex-to-Codex effort and reject non-Codex targets", async () => {
  ...
  assert.equal(allowedBody.response._dardcorCodeReasoningRule, undefined);
  assert.equal(allowedBody.response._dardcorCodeReasoningRouteTrace, undefined);
