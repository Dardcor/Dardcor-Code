...
const TEST_DATA_DIR = await fs.mkdtemp(path.join(os.tmpdir(), "dardcorCode-tailscale-"));
...
test("getTailscaleUrlFromStatusPayload normalizes DNS names into HTTPS URLs", () => {
  assert.equal(
    tailscaleTunnel.getTailscaleUrlFromStatusPayload({
      Self: { DNSName: "dardcorCode-demo.tail123.ts.net." },
    }),
    "https://dardcorCode-demo.tail123.ts.net"
  );
});
...
test("getTailscaleTunnelStatus reflects a logged-in running funnel", async () => {
  ...
  assert.equal(status.tunnelUrl, "https://dardcorCode-demo.tail123.ts.net");
  assert.equal(status.apiUrl, "https://dardcorCode-demo.tail123.ts.net/v1");
});
...
test("enableTailscaleTunnel stores the funnel URL and disableTailscaleTunnel clears it", async () => {
  ...
  process.env.TAILSCALE_TEST_FUNNEL_OUTPUT = "Available at https://dardcorCode-demo.tail123.ts.net";
  ...
  if (enabled.success) {
    assert.equal(enabled.tunnelUrl, "https://dardcorCode-demo.tail123.ts.net");
  }
  assert.equal(settingsAfterEnable.tailscaleEnabled, true);
  assert.equal(settingsAfterEnable.tailscaleUrl, "https://dardcorCode-demo.tail123.ts.net");
