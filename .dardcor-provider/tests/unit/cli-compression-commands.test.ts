...
test("compression status chama dardcorCode_compression_status via mcp", async () => {
  ...
  assert.equal(capturedBody.name, "dardcorCode_compression_status");
});
...
test("compression configure envia configuração via mcp", async () => {
  ...
  globalThis.fetch = origFetch;
  assert.equal(capturedBody.name, "dardcorCode_compression_configure");
...
test("compression engine set chama dardcorCode_set_compression_engine", async () => {
  let capturedBody: any = null;
  const origFetch = globalThis.fetch;
  globalThis.fetch = ((_url: string, opts: any) => {
    if (opts?.body) capturedBody = JSON.parse(opts.body);
  ...
  globalThis.fetch = origFetch;
  assert.equal(capturedBody.name, "dardcorCode_set_compression_engine");
