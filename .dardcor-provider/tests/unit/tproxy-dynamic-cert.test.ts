...
test("generateMitmCa produces a CA certificate (basicConstraints CA, key+cert PEM)", async () => {
  const ca = await generateMitmCa("DardcorCode MITM CA (test)");
  ...
  assert.match(x.subject, /DardcorCode MITM CA \(test\)/);
});
...
test("DynamicCertStore caches one SecureContext per hostname", async () => {
  const store = new DynamicCertStore("DardcorCode MITM CA (test)");
