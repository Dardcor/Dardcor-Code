...
test("T40: OpenCode config path resolves per-platform", () => {
  ...
  // Windows (NOT %APPDATA%) — DardcorCode must write where OpenCode reads.
...
test("T40: OpenCode config generator includes endpoint and selected API key", () => {
  ...
  assert.equal(mergedConfig.provider.dardcorCode.options.baseURL, "http://localhost:20128/v1");
  assert.equal(mergedConfig.provider.dardcorCode.options.apiKey, "sk_test_opencode");
});
...
test("T40: OpenCode config document uses current provider schema", () => {
  ...
  assert.ok(configDocument.provider.dardcorCode);
  assert.equal(configDocument.provider.dardcorCode.npm, "@ai-sdk/openai-compatible");
  assert.equal(configDocument.provider.dardcorCode.options.baseURL, "http://localhost:20128/v1");
  assert.equal(configDocument.provider.dardcorCode.options.apiKey, "sk_test_opencode");
  assert.deepEqual(Object.keys(configDocument.provider.dardcorCode.models), [
    "cc/claude-sonnet-4-20250514",
    "gg/gemini-2.5-pro",
  ]);
  assert.equal(
    configDocument.provider.dardcorCode.models["cc/claude-sonnet-4-20250514"].name,
    "Claude Sonnet 4.5"
  );
  assert.equal(
    configDocument.provider.dardcorCode.models["gg/gemini-2.5-pro"].name,
    "Gemini 2.5 Pro"
  );
...
test("T40: OpenCode merge preserves unrelated config and updates only provider.dardcorCode", () => {
  const mergedConfig = mergeOpenCodeConfig(
    {
  ...
  assert.deepEqual(mergedConfig.provider.dardcorCode.models, {
