...
// `x-dardcorCode-strip-reasoning` header to unconditionally strip
...
test("isStripReasoningRequested is true for truthy header values", () => {
  for (const v of ["true", "1", "yes", "TRUE", "Yes", " true "]) {
    assert.equal(
      isStripReasoningRequested({ "x-dardcorCode-strip-reasoning": v }),
      true,
...
test("isStripReasoningRequested is case-insensitive on the header NAME", () => {
  assert.equal(isStripReasoningRequested({ "X-DardcorCode-Strip-Reasoning": "true" }), true);
});

test("isStripReasoningRequested works with a Headers instance", () => {
  const h = new Headers();
  h.set("x-dardcorCode-strip-reasoning", "1");
...
test("isStripReasoningRequested is false when absent / empty / falsy", () => {
  ...
  assert.equal(isStripReasoningRequested({ "x-dardcorCode-strip-reasoning": "" }), false);
  assert.equal(isStripReasoningRequested({ "x-dardcorCode-strip-reasoning": "false" }), false);
  assert.equal(isStripReasoningRequested({ "x-dardcorCode-strip-reasoning": "0" }), false);
  assert.equal(isStripReasoningRequested({ "x-dardcorCode-strip-reasoning": "no" }), false);
});

test("getHeaderValueCaseInsensitive still resolves the header (sanity)", () => {
  assert.equal(
    getHeaderValueCaseInsensitive(
      { "x-dardcorCode-strip-reasoning": "true" },
      "x-dardcorCode-strip-reasoning"
    ),
    "true"
  );
});
