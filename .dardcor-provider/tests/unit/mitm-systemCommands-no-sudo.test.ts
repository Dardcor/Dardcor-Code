 ...
 *
 * DardcorCode funnels every privileged MITM command through the single choke
 ...
 * DardcorCode starts cleanly in a root-less / user-namespace deployment where
...
test("resolveSudoSpawn: a non-sudo command is never touched, regardless of the flag", () => {
  ...
  try {
    ...
    assert.deepEqual(finalArgs, ["-A", "-n", "DardcorCode"]);
  } finally {
    if (prev === undefined) delete process.env.OMNIROUTE_NO_SUDO;
    else process.env.OMNIROUTE_NO_SUDO = prev;
  }
