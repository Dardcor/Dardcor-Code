...
function cleanupGlobalDb() {
  try {
    if (globalThis.__dardcorCodeDb?.open) {
      globalThis.__dardcorCodeDb.close();
    }
  } catch {}

  delete globalThis.__dardcorCodeDb;
}
...
test("getDbInstance creates sqlite schema, metadata and applies migrations", serial, async () => {
  const dataDir = makeTempDir("dardcorCode-db-core-");
  ...
  try {
    await withEnv({ DATA_DIR: dataDir, NEXT_PHASE: undefined }, async () => {
      ...
      const versions = db
        .prepare("SELECT version FROM _dardcorCode_migrations ORDER BY version")
        .all()
        .map((row) => row.version);
...
test("module exports honor DATA_DIR from the environment", serial, async () => {
  const dataDir = makeTempDir("dardcorCode-db-core-env-");
...
test(
  ...
  serial,
  async () => {
    const fakeHome = makeTempDir("dardcorCode-home-");
        ...
        async () => {
          ...
          const expectedDir =
            process.platform === "win32"
              ? path.join(fakeHome, "AppData", "Roaming", "dardcorCode")
              : path.join(fakeHome, ".dardcorCode");
...
test("build phase uses an in-memory database without creating sqlite files", serial, async () => {
  const dataDir = makeTempDir("dardcorCode-db-build-");
...
test("invalid DATA_DIR (a file where a dir is expected) surfaces as a startup failure", serial, async () => {
  const sandboxDir = makeTempDir("dardcorCode-db-bad-path-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("dardcorCode-db-legacy-empty-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("dardcorCode-db-legacy-data-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    createLegacySchemaDb(sqliteFile, { withData: true });
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("dardcorCode-db-missing-max-concurrent-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    const seedDb = new Database(sqliteFile);
    const now = new Date().toISOString();
    ...
    seedDb.exec(`
      ...
      INSERT INTO _dardcorCode_migrations (version, name) VALUES ('001', 'initial_schema');
      INSERT INTO _dardcorCode_migrations (version, name) VALUES ('029', 'webhooks_templates');
    `);
    seedDb
      .prepare(
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("dardcorCode-db-legacy-call-logs-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("dardcorCode-db-probe-recover-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("dardcorCode-db-probe-latest-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("dardcorCode-db-probe-abort-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
