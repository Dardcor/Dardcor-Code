...
function selfLoopChatRequest(
  ...
  const headers: Record<string, string> = {
    ...
    "x-dardcorCode-admission-bypass": "internal",
...
test("resolveSelfLoopBearer falls back to sk_dardcorCode when no env key is set", () => {
  const restore = withSelfLoopEnv({});
  try {
    assert.equal(resolveSelfLoopBearer(), "sk_dardcorCode");
  } finally {
    restore();
  }
});
...
test("sk_dardcorCode sentinel is rejected once an env key is configured (REQUIRE_API_KEY hardening)", async () => {
  const restore = withSelfLoopEnv({ OMNIROUTE_API_KEY: "env-key" });
  try {
    const controller = new ChatAdmissionController(1);
    ...
    const request = new Request("http://x/v1/chat/completions", {
      ...
      headers: {
        "content-type": "application/json",
        "x-dardcorCode-admission-bypass": "internal",
        authorization: "Bearer sk_dardcorCode",
        "content-length": String(body.length),
      },
      body,
    });
