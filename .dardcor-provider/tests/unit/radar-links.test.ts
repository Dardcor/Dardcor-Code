...
test("getContributorClaimUrl: defaults to the radar.dardcorCode.online GitHub OAuth entry point", async () => {
  const { getContributorClaimUrl } = await import("../../src/lib/radar/links.ts");
  assert.equal(getContributorClaimUrl(), "https://radar.dardcorCode.online/auth/github");
});
...
test("getSupporterPlansUrl: defaults to the radar.dardcorCode.online plans page", async () => {
  const { getSupporterPlansUrl } = await import("../../src/lib/radar/links.ts");
  assert.equal(getSupporterPlansUrl(), "https://radar.dardcorCode.online/planos");
});
