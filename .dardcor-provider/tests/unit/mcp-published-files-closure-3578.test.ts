...
// #3578 — `dardcorCode --mcp` crashed on npm installs with ERR_MODULE_NOT_FOUND for
...
function resolveImport(fromFile: string, spec: string): string | null {
  ...
  else if (spec.startsWith("@dardcorCode/open-sse/"))
    base = path.join("open-sse", spec.slice("@dardcorCode/open-sse/".length));
  else if (spec === "@dardcorCode/open-sse") base = path.join("open-sse", "index");
