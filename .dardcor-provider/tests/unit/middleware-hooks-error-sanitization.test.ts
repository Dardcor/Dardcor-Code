 ...
 * "SQLITE_ERROR: near /var/lib/dardcorCode/..."). Guard against
...
for (const path of ROUTES) {
  test(`${path} imports sanitizeErrorMessage from open-sse/utils/error`, () => {
    ...
    assert.ok(
      /import\s*\{[^}]*\bsanitizeErrorMessage\b[^}]*\}\s*from\s*["']@dardcorCode\/open-sse\/utils\/error["']/.test(
        source
      ),
      `expected ${path} to import sanitizeErrorMessage from "@dardcorCode/open-sse/utils/error"`
    );
  });
