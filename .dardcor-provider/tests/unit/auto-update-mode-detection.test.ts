...
test("npm + git repo → source (a source checkout self-updates via git)", () => {
  assert.equal(
    resolveAutoUpdateMode("npm", {
      isGitRepo: true,
      currentDir: "/home/me/dardcorCode/dist/lib/system",
    }),
    "source"
  );
});
...
test("npm + global install under node_modules → npm", () => {
  assert.equal(
    resolveAutoUpdateMode("npm", {
      isGitRepo: false,
      currentDir: "/usr/lib/node_modules/dardcorCode/dist/lib/system",
    }),
    "npm"
  );
});
...
test("npm + no git + not under node_modules → source (downloaded build/zip)", () => {
  assert.equal(
    resolveAutoUpdateMode("npm", {
      isGitRepo: false,
      currentDir: "/opt/dardcorCode/dist/lib/system",
    }),
    "source"
  );
});
...
test("isUnderNodeModules matches real segments on both path separators", () => {
  assert.equal(isUnderNodeModules("/usr/lib/node_modules/dardcorCode"), true);
  assert.equal(isUnderNodeModules("C:\\Users\\me\\node_modules\\dardcorCode"), true);
