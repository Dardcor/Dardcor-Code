...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-memory-tools-"));
...
test.beforeEach(async () => {
  ...
  // within the memory subsystem (dardcorCode_memory_search → retrieveMemories, which
...
test("memory add stores entries with default session and metadata", async () => {
  const result = await memoryTools.dardcorCode_memory_add.handler({
    apiKeyId: "key-add",
    type: "factual",
    key: "pref:language",
...
test("memory search filters by type, enforces limit, and reports token totals", async () => {
  await memoryTools.dardcorCode_memory_add.handler({
    apiKeyId: "key-search",
    sessionId: "search",
    type: "factual",
    key: "pref:stack",
  ...
  const result = await memoryTools.dardcorCode_memory_search.handler({
    apiKeyId: "key-search",
...
test("memory clear deletes only older filtered entries and reports the deleted count", async () => {
  ...
  const result = await memoryTools.dardcorCode_memory_clear.handler({
    apiKeyId: "key-clear",
    type: "factual",
