...
test("x-dardcorCode-recovery-action header emitted when recovery present", async () => {
  const diag = emptyDiag({
    recovery: {
      action: "try-auto" as RecoveryAction,
  ...
  const res = errorResponseWithComboDiagnostics(503, "exhausted", diag);
  assert.equal(res.headers.get("x-dardcorCode-recovery-action"), "try-auto");
  assert.equal(
    res.headers.get("x-dardcorCode-recovery-next-step"),
    "Auto-select available providers."
  );
});

test("x-dardcorCode-retry-after-seconds header emitted when retry_after_seconds set", async () => {
  const diag = emptyDiag({
    recovery: { action: "wait" as RecoveryAction, next_step: "wait", retry_after_seconds: 45 },
  });
  const res = errorResponseWithComboDiagnostics(429, "rate limited", diag);
  assert.equal(res.headers.get("x-dardcorCode-retry-after-seconds"), "45");
});

test("x-dardcorCode-retry-after-seconds omitted when retry_after_seconds is 0", async () => {
  const diag = emptyDiag({
  ...
  assert.equal(res.headers.get("x-dardcorCode-retry-after-seconds"), null);
});

test("x-dardcorCode-recovery-* headers omitted when recovery is undefined", async () => {
  const diag = emptyDiag();
  const res = errorResponseWithComboDiagnostics(503, "generic", diag);
  assert.equal(res.headers.get("x-dardcorCode-recovery-action"), null);
  assert.equal(res.headers.get("x-dardcorCode-recovery-next-step"), null);
});
...
test("next_step sanitized (newlines → spaces, max 128 chars)", async () => {
  ...
  const res = errorResponseWithComboDiagnostics(503, "exhausted", diag);
  const sanitized = res.headers.get("x-dardcorCode-recovery-next-step")!;
