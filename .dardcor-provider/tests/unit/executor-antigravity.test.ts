...
test("AntigravityExecutor.buildHeaders includes native headers without DardcorCode internals", () => {
  const executor = new AntigravityExecutor();
  ...
  assert.equal(headers["X-DardcorCode-Source"], undefined);
});

test("Antigravity header scrub removes DardcorCode internal headers", () => {
  const headers = scrubProxyAndFingerprintHeaders({
    Authorization: "Bearer ag-token",
    "X-DardcorCode-Source": "dardcorCode",
  ...
  assert.equal(headers.Authorization, "Bearer ag-token");
  assert.equal(headers["X-DardcorCode-Source"], undefined);
  assert.equal(headers["X-DardcorCode-No-Cache"], undefined);
