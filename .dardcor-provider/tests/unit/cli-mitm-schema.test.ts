...
// Regression test: null apiKey + unresolvable keyId must yield the sentinel 'sk_dardcorCode',
...
test("sentinel guard condition catches sk_dardcorCode and null", () => {
  const SENTINEL = "sk_dardcorCode";
  // Simulate what the route guard checks: (!apiKey || apiKey === 'sk_dardcorCode')
  ...
  assert.equal(shouldReject("sk_dardcorCode"), true, "sentinel must be rejected");
