...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-plugins-edge-"));
...
function writeTestPlugin(opts: {
  ...
  const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-edge-src-"));
...
test("scanner: empty directory returns empty results", async () => {
  const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-scan-empty-"));
...
test("scanner: directory with no manifest reports error", async () => {
  const noManifestDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-scan-nomanifest-"));
...
test("scanner: invalid JSON manifest reports error", async () => {
  const badDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-scan-badjson-"));
...
test("scanner: missing required fields reports error", async () => {
  const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-scan-missing-"));
...
test("scanner: non-directory entries are skipped", async () => {
  const mixedDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-scan-mixed-"));
