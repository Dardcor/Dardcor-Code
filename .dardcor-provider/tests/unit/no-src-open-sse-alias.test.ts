 ...
 * workspace (alias `@dardcorCode/open-sse/*`). So an import of `@/open-sse/...`
...
test("src/ never imports the open-sse workspace via the @/ alias (use @dardcorCode/open-sse)", () => {
  ...
  assert.deepEqual(
    offenders,
    [],
    `@/open-sse/* resolves to the non-existent src/open-sse/* and throws "Cannot find module" at runtime. Use @dardcorCode/open-sse/* instead. Offenders:\n${offenders.join("\n")}`
  );
});
