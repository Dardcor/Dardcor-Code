...
// #7586: `dardcorCode doctor`'s "Database" and "Storage/encryption" checks call
...
test("#7586: readDatabaseHealth() falls back to node:sqlite when better-sqlite3 is unavailable", async (t) => {
  ...
  seed.exec(
    "CREATE TABLE _dardcorCode_migrations (version TEXT PRIMARY KEY, name TEXT NOT NULL, applied_at TEXT);" +
      "INSERT INTO _dardcorCode_migrations (version, name, applied_at) VALUES ('001', 'initial_schema', datetime('now'));"
  );
  seed.close();

  // Exercise the CLI helper's own health check — the exact function
  // `dardcorCode doctor` calls (bin/cli/commands/doctor.mjs:checkDatabase ->
