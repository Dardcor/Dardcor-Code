...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-bifrost-installer-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-bifrost-fake-bin-"));
...
test("install creates host package.json structure", async () => {
  ...
  assert.equal(parsedHost.name, "dardcorCode-bifrost-host");
