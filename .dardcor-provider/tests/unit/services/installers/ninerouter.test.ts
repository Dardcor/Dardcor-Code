...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-installer-"));
const FAKE_BIN_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-fake-bin-"));
...
test("install creates package.json structure", async () => {
  ...
  assert.equal(parsedHost.name, "dardcorCode-9router-host");
