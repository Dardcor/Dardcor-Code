...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-reasoning-routing-"));
...
test("default does not override a budget-only signal and force replaces discrete effort", async () => {
  ...
  assert.equal(forced._dardcorCodeReasoningRule, undefined);
