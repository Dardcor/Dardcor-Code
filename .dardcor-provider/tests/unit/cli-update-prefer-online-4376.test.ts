...
// #4376: `dardcorCode update` reported "Latest version: 3.8.30" while npm's `latest`
// dist-tag was already 3.8.31, so it told users on an old build they were "running
// the latest version". Root cause: getLatestVersion() ran `npm view dardcorCode version`
...
test("getLatestVersion passes --prefer-online to bypass the stale npm cache (#4376)", async () => {
  ...
  assert.ok(capturedArgs.args.includes("dardcorCode"));
