 ...
 *   2. Import and use `buildErrorBody` from `@dardcorCode/open-sse/utils/error`.
 *
...
for (const { rel, label } of PLUGIN_ROUTES) {
  ...
  test(`${label}: imports buildErrorBody from @dardcorCode/open-sse/utils/error`, () => {
    const src = readRoute(rel);
    assert.match(
      src,
      /import \{[^}]*buildErrorBody[^}]*\} from ["']@dardcorCode\/open-sse\/utils\/error["']/,
      `${rel}: must import buildErrorBody from @dardcorCode/open-sse/utils/error`
    );
  });
