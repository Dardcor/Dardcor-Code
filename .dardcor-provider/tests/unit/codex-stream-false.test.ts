...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-codex-stream-false-"));
...
test("non-stream chat success carries cost-telemetry meta headers (cost/version/tokens)", async () => {
  ...
  // through attachDardcorCodeMetaHeaders, which must always emit the cost-telemetry
