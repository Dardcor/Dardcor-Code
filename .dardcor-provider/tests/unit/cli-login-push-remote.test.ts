...
// `dardcorCode login antigravity` already runs the OAuth on the operator's OWN machine —
...
//   - `dardcorCode connect <host>` saves an admin-scoped token in the active context;
...
test("pushCredentialBlob POSTs the blob to the provider's paste-credentials route", async () => {
  ...
  const result = await pushCredentialBlob("antigravity", "dardcorCode-cred-v1.abc", { fetchImpl });

  assert.equal(result.ok, true);
  assert.equal(calls.length, 1);
  assert.equal(calls[0].path, "/api/oauth/antigravity/paste-credentials");
  assert.equal(calls[0].opts.method, "POST");
  assert.deepEqual(calls[0].opts.body, { blob: "dardcorCode-cred-v1.abc" });
});
...
test("pushCredentialBlob surfaces a server rejection instead of pretending success", async () => {
  ...
  const result = await pushCredentialBlob("agy", "dardcorCode-cred-v1.abc", { fetchImpl });
...
test("a remote context auto-pushes and does NOT print the blob", async () => {
  ...
  assert.match(pushed!.blob, /^dardcorCode-cred-v1\./);
  // The blob carries a refresh token; don't spray it on a terminal when it already landed.
  assert.equal(
    printed.join("").includes("dardcorCode-cred-v1."),
    false,
...
test("a FAILED push falls back to printing the blob — the firewall case still works", async () => {
  ...
  assert.match(blob, /^dardcorCode-cred-v1\./);
  assert.ok(
...
test("a LOCAL context prints as before — no surprise network call", async () => {
  ...
  assert.ok(printed.join("").includes("dardcorCode-cred-v1."));
});
