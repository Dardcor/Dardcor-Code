...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-image-route-auth-"));
...
test("v1 image generation POST rejects an invalid presented API key", async () => {
  const originalDardcorCodeApiKey = process.env.OMNIROUTE_API_KEY;
  const originalRequireApiKey = process.env.REQUIRE_API_KEY;
  ...
  try {
    ...
    if (originalDardcorCodeApiKey === undefined) {
      delete process.env.OMNIROUTE_API_KEY;
    } else {
      process.env.OMNIROUTE_API_KEY = originalDardcorCodeApiKey;
    }
    if (originalRequireApiKey === undefined) {
      delete process.env.REQUIRE_API_KEY;
    } else {
