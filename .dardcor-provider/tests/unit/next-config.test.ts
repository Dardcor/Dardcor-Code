...
test("optimizePackageImports excludes the internal @dardcorCode/open-sse workspace (build-OOM guard)", async () => {
  // Regression guard: adding the internal `@dardcorCode/open-sse` workspace to
  ...
  assert.ok(
    !list.includes("@dardcorCode/open-sse"),
    "do NOT add the internal @dardcorCode/open-sse workspace to optimizePackageImports — it OOMs the production build"
  );
