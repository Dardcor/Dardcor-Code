...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/dardcorCode" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/dardcorCode");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/dardcorCode",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/dardcorCode/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "dardcorCode-base",
    ...
    assert.equal(config.repoDir, "/srv/dardcorCode");
    assert.equal(config.composeFile, "/srv/dardcorCode/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "dardcorCode-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart dardcorCode --update-env \|\| true/);
  });
});
