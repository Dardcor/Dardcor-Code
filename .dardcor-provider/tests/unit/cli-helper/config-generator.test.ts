...
describe("config-generator", () => {
  ...
  describe("assertSafeCatalogUrl (SSRF guard, CodeQL #326)", () => {
    it("allows the loopback DardcorCode target (the legitimate default) and returns a URL", async () => {
      const { assertSafeCatalogUrl } =
        await import("../../../src/lib/cli-helper/config-generator/opencode.ts");
      // The catalog source IS the user's own DardcorCode — localhost must stay allowed.
    ...
    it("allows a public DardcorCode Cloud target", async () => {
      const { assertSafeCatalogUrl } =
        await import("../../../src/lib/cli-helper/config-generator/opencode.ts");
      assert.doesNotThrow(() => assertSafeCatalogUrl("https://api.dardcorCode.online/v1/models"));
    });
  ...
  describe("generateConfig", () => {
    ...
    it("returns success for valid hermes config", async () => {
      ...
      assert.ok(String(result.content || "").includes("dardcorCode"));
    });
  ...
  describe("hermes-agent (rich multi-role)", () => {
    ...
    it("generateHermesAgentConfig returns yaml string for valid payload", async () => {
      ...
      assert.ok(result.yaml.includes("provider: dardcorCode"));
    });
  ...
  describe("opencode (context-aware)", () => {
    ...
    it("emits limit.context from the catalog (no hardcoded fallback)", async () => {
      ...
      try {
        ...
        const models = cfg.provider.dardcorCode.models;
    ...
    it("does NOT fabricate a default context when the catalog has no entry", async () => {
      ...
      try {
        ...
        const noCtx = cfg.provider.dardcorCode.models["NO_CTX_COMBO"];
        assert.strictEqual(
          noCtx.limit?.context,
          undefined,
    ...
    it("prefers max_context_window_tokens when context_length is absent", async () => {
      ...
      try {
        ...
        const cfg = JSON.parse(out);
        assert.strictEqual(cfg.provider.dardcorCode.models.llama3.limit.context, 8192);
      } finally {
        stub.restore();
      }
    });
    ...
    it("writes a top-level model prefixed with provider id when options.model is supplied", async () => {
      ...
      try {
        ...
        const cfg = JSON.parse(out);
        assert.strictEqual(cfg.model, "dardcorCode/MASTER");
      } finally {
        stub.restore();
      }
    });
    ...
    it("propagates vision capability from the live catalog for issue #8960", async () => {
      ...
      try {
        ...
        const cfg = JSON.parse(out);
        const model = cfg.provider.dardcorCode.models[modelId];
    ...
    it("auto-pulls the Opencode FREE Omni combo context (the user-reported case)", async () => {
      ...
      try {
        ...
        const cfg = JSON.parse(out);
        assert.strictEqual(
          cfg.provider.dardcorCode.models["Opencode FREE Omni"].limit.context,
          200000,
