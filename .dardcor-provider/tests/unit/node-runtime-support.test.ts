...
test("getNodeRuntimeSupport rejects unsupported major lines", () => {
  ...
  assert.match(getNodeRuntimeWarning("18.20.8") || "", /outside DardcorCode's approved secure/i);
...
test("package.json engines.node equals SUPPORTED_NODE_RANGE (install gate == runtime gate)", () => {
  ...
  // DardcorCode on a Node version the runtime then refuses to run (the Node 22.0.0–22.22.1 gap).
