...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".dardcorCode") });
  } finally {
...
test("bootstrapEnv prefers ~/.dardcorCode/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
