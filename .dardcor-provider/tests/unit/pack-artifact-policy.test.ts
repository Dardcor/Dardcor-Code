...
test("findUnexpectedArtifactPaths flags node_modules even inside an allowed prefix", () => {
  // Regression guard: the allowlist grants the whole `@dardcorCode/opencode-provider/`
  ...
  const unexpectedPaths = findUnexpectedArtifactPaths(
    [
      "@dardcorCode/opencode-provider/node_modules/tsup/package.json",
      "@dardcorCode/opencode-provider/node_modules/esbuild/lib/main.js",
      "@dardcorCode/opencode-provider/dist/index.js",
      "@dardcorCode/opencode-provider/package.json",
    ],
    {
      exactPaths: [],
      prefixPaths: ["@dardcorCode/opencode-provider/"],
    }
  );
...
test("package.json files[] excludes nested node_modules from the published package", () => {
  ...
  assert.ok(
    ...
    'package.json "files" must keep the "!**/node_modules/**" negation — without it, ' +
      "a nested install inside @dardcorCode/* ships ~79 MB of devDependencies."
  );
});
...
test("setupPolyfill.ts is allowed in the tarball (bin/dardcorCode.mjs imports it at startup)", () => {
...
test("findMissingArtifactPaths flags missing root runtime files in the tarball", () => {
  ...
  // above (dist/server.js, bin/dardcorCode.mjs, package.json, the postinstall scripts).
