...
test("oneproxy status chama dardcorCode_oneproxy_stats via MCP", async () => {
  ...
  await (globalThis.fetch as any)("/api/mcp/tools/call", {
    method: "POST",
    body: JSON.stringify({ name: "dardcorCode_oneproxy_stats", arguments: {} }),
  });

  globalThis.fetch = origFetch;
  assert.equal(capturedBody.name, "dardcorCode_oneproxy_stats");
});
...
test("oneproxy fetch chama dardcorCode_oneproxy_fetch com count e type", async () => {
  let capturedBody: any = null;
  const origFetch = globalThis.fetch;
  globalThis.fetch = ((_url: string, opts: any) => {
    if (opts?.body) capturedBody = JSON.parse(opts.body);
  ...
  globalThis.fetch = origFetch;
  assert.equal(capturedBody.name, "dardcorCode_oneproxy_fetch");
  assert.equal(capturedBody.arguments.count, 5);
  assert.equal(capturedBody.arguments.type, "http");
});

test("oneproxy rotate chama dardcorCode_oneproxy_rotate com provider", async () => {
  let capturedBody: any = null;
  const origFetch = globalThis.fetch;
  globalThis.fetch = ((_url: string, opts: any) => {
    if (opts?.body) capturedBody = JSON.parse(opts.body);
  ...
  globalThis.fetch = origFetch;
  assert.equal(capturedBody.name, "dardcorCode_oneproxy_rotate");
