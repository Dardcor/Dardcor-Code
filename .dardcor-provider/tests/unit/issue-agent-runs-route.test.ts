  ...
  try {
    const response = await POST(
      new Request("http://localhost/api/issue-agent/runs", {
        ...
        body: JSON.stringify({
          mode: "recorded-triage",
          issueUrl: "https://github.com/KooshaPari/DardcorCode/issues/6059",
          dryRun: true,
        }),
      })
    );
    ...
    assert.equal(body.repository, "KooshaPari/DardcorCode");
  ...
  try {
    const response = await POST(
      new Request("http://localhost/api/issue-agent/runs", {
        ...
        body: JSON.stringify({
          mode: "recorded-triage",
          issueUrl: "https://github.com/KooshaPari/DardcorCode/issues/7",
          recordedContext: {
...
test("issue-agent run accepts recorded GitHub export payloads", async () => {
  ...
  try {
        ...
        body: JSON.stringify({
          ...
          githubExport: {
            issue: {
              ...
              html_url: "https://github.com/KooshaPari/DardcorCode/issues/88",
            },
    ...
    assert.equal(response.status, 200);
    assert.equal(body.issueUrl, "https://github.com/KooshaPari/DardcorCode/issues/88");
  ...
  try {
    const response = await POST(
      new Request("http://localhost/api/issue-agent/runs", {
        ...
        body: JSON.stringify({
          mode: "recorded-triage",
          issueUrl: "https://github.com/KooshaPari/DardcorCode/issues/1",
        }),
      })
    );
    const body = await json(response);
