...
test("testImportsModule matches static, dynamic and require imports of the module path", () => {
  ...
  // static import-from
  assert.equal(
    testImportsModule(`import { x } from "@dardcorCode/open-sse/handlers/chatCore/headers";`, frag),
    true
  );
  ...
  assert.equal(
    testImportsModule(`import { a } from "@dardcorCode/open-sse/handlers/chatCore/idempotency";`, frag),
    false
  );
...
test("findCoverageDrift flags covering unit tests absent from tap.testFiles", () => {
  ...
  const unitTests = [
    ...
    { path: "tests/unit/idempo.test.ts", content: `import { x } from "@dardcorCode/open-sse/handlers/chatCore/idempotency";` },
