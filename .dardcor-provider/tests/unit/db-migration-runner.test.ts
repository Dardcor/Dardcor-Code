...
test("runMigrations applies pending files sequentially in version order", serial, async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.prepare("SELECT version FROM _dardcorCode_migrations ORDER BY version").all(),
...
test("runMigrations skips versions that are already tracked as applied", serial, async () => {
  ...
  try {
    ...
    assert.equal(
      (
        db
          .prepare("SELECT COUNT(*) AS count FROM _dardcorCode_migrations WHERE version = ?")
          .get("001") as any
      ).count,
      1
    );
...
test(
  ...
  async () => {
    ...
    `);
      ...
      assert.deepEqual(
        db.prepare("SELECT version, name FROM _dardcorCode_migrations WHERE version = ?").get("032"),
...
test("getMigrationStatus reports applied and pending migrations", serial, async () => {
  ...
  try {
    db.exec(`
      CREATE TABLE _dardcorCode_migrations (
...
test("invalid file names are ignored while valid migrations still run", serial, async () => {
  ...
  try {
    ...
    assert.deepEqual(
      db.prepare("SELECT version, name FROM _dardcorCode_migrations ORDER BY version").all(),
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(
        db.prepare("SELECT name FROM _dardcorCode_migrations WHERE version = ?").get("041")?.name,
        "compression_receipts"
      );
      assert.equal(
        db.prepare("SELECT name FROM _dardcorCode_migrations WHERE version = ?").get("050")?.name,
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.equal(
          db.prepare("SELECT name FROM _dardcorCode_migrations WHERE version = ?").get("056")?.name,
          "mcp_accessibility_compression"
        );
        assert.equal(
          db.prepare("SELECT name FROM _dardcorCode_migrations WHERE version = ?").get("059")?.name,
          "manifest_routing"
        );
  ...
  async () => {
    ...
    try {
      ...
      try {
        ...
        assert.equal(
          db.prepare("SELECT name FROM _dardcorCode_migrations WHERE version = ?").get("051")?.name,
          "hot_path_db_indexes"
        );
        assert.equal(
          db.prepare("SELECT name FROM _dardcorCode_migrations WHERE version = ?").get("054")?.name,
          "usage_history_service_tier"
        );
  ...
  async () => {
    ...
    try {
      ...
      for (const [v, n] of oldMigrations) {
        db.prepare("INSERT INTO _dardcorCode_migrations (version, name) VALUES (?, ?)").run(v, n);
      }
      ...
      try {
        ...
        const row034 = db
          .prepare("SELECT name FROM _dardcorCode_migrations WHERE version = ?")
