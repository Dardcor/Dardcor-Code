  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        expect(toolNames).toContain("dardcorCode_get_health");
        expect(toolNames).toContain("dardcorCode_list_combos");
      ...
      const auditRes = await apiFetch("/api/mcp/audit?limit=50&tool=dardcorCode_get_health");
      ...
      if (auditRes.status === 200) {
        ...
        expect(entries.some((entry: any) => entry.toolName === "dardcorCode_get_health")).toBe(true);
      }
    },
    TEST_TIMEOUT_MS * 2
  );
