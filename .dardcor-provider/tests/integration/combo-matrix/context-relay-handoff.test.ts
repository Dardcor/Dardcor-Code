...
//      - request carries x-dardcorCode-session-id header
...
// reads "x-session-id" (not "x-dardcorCode-session-id") and prefixes the result with "ext:".
...
//     internal summary request (identified by _dardcorCodeInternalRequest flag in body)
...
function installHandoffAwareFetch() {
  ...
  globalThis.fetch = async (url: any, init: any = {}) => {
    ...
    if (bodyObj._dardcorCodeInternalRequest === "universal-handoff") {
...
// Without the x-dardcorCode-session-id header, sessionId = null → block is skipped.
test("context-relay universal handoff: does NOT fire when x-dardcorCode-session-id header is absent", async () => {
  ...
  assert.equal(
    handoff,
    null,
    "handoff must NOT be written when x-dardcorCode-session-id header is absent (sessionId gate)"
  );
