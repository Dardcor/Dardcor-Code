...
// completely independent of the per-request `x-dardcorCode-compression` header/mode. The internal
...
// a request carrying `x-dardcorCode-compression: off` must NOT get the styles system message
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-testmodel-compression-"));
...
test("chatCore: x-dardcorCode-compression: off suppresses Output Styles injection even with the operator's global style enabled (#6240)", async () => {
  ...
  assert.match(plainFirstMessage?.content ?? "", /DardcorCode Output Styles/);

  // The "Test model" connection test sends x-dardcorCode-compression: off — must be clean.
  const testModelBody = await runChatCore({
    provider,
    model,
    connectionId: connection.id,
    headers: new Headers({ "x-dardcorCode-compression": "off" }),
  });
  ...
  const anyMessageHasMarker = (testModelBody?.messages ?? []).some((m) =>
    (m?.content ?? "").includes("DardcorCode Output Styles")
  );
  assert.equal(
    anyMessageHasMarker,
    false,
