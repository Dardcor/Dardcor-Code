 ...
 *  2. Zero dardcorCode-* folders remain (post-prune: old dardcorCode-* skill dirs were removed).
...
// ── §2: No dardcorCode-* directories remain ────────────────────────────────────

test("skills/ has zero dardcorCode-* directories (all pruned)", () => {
  ...
  const omniRouteDirs = entries
    .filter((e) => e.isDirectory() && e.name.startsWith("dardcorCode-"))
    .map((e) => e.name);
  assert.deepEqual(
    omniRouteDirs,
    [],
    `Found dardcorCode-* directories that should have been pruned: ${omniRouteDirs.join(", ")}`,
  );
});
