...
test("successful responses extract facts and persist them as memories", async () => {
  ...
  const response = await handleChat(
    buildRequest({
      authKey: apiKey.key,
      headers: { "x-dardcorCode-session-id": "session-extract" },
      body: {
        model: "openai/gpt-4o-mini",
        stream: false,
...
test("later requests inject retrieved memories into upstream messages", async () => {
  ...
  const response = await handleChat(
    buildRequest({
      authKey: apiKey.key,
      headers: { "x-dardcorCode-session-id": "session-inject" },
      body: {
        model: "openai/gpt-4o-mini",
        stream: false,
...
test("memory clear removes all stored memories for an API key", async () => {
  ...
  const cleared = await memoryTools.dardcorCode_memory_clear.handler({
    apiKeyId: apiKey.id,
  });
...
test("extracted memories remain isolated by session id", async () => {
  ...
  await handleChat(
    buildRequest({
      authKey: apiKey.key,
      headers: { "x-dardcorCode-session-id": "session-a" },
      body: {
        model: "openai/gpt-4o-mini",
        stream: false,
  ...
  await handleChat(
    buildRequest({
      authKey: apiKey.key,
      headers: { "x-dardcorCode-session-id": "session-b" },
      body: {
        model: "openai/gpt-4o-mini",
        stream: false,
