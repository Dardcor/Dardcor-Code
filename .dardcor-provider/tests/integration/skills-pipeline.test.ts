...
test("web_search fallback converts built-in tools for unsupported providers and executes search", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (urlStr.includes("google.serper.dev/search")) {
      ...
      return new Response(
        JSON.stringify({
          organic: [
            {
              title: "DardcorCode Search Result",
              link: "https://example.com/dardcorCode",
              snippet: "Fresh DardcorCode web search fallback result",
            },
          ],
        }),
    ...
    return buildOpenAIToolCallResponse({
      ...
      argumentsObject: {
        query: "latest dardcorCode release notes",
        max_results: 3,
      },
    });
  };
  ...
  const response = await handleChat(
    buildRequest({
      ...
      body: {
        ...
        stream: false,
        messages: [
          { role: "user", content: "Search the web for the latest DardcorCode release notes" },
        ],
  ...
  assert.equal(output.results[0].title, "DardcorCode Search Result");
});
...
test("web_search fallback preserves Responses API output by appending function_call_output", async () => {
  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    return buildOpenAIToolCallResponse({
      ...
      argumentsObject: {
        query: "latest dardcorCode roadmap",
      },
    });
  };
        ...
        input: [
          {
            ...
            role: "user",
            content: [
              { type: "input_text", text: "Search the web for the latest DardcorCode roadmap" },
            ],
          },
        ],
