...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-codex-chat-http-"));
...
test("chat completions streams Codex Responses reasoning through real route HTTP", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(raw, /DardcorCode cannot recover|Codex is reasoning/i);
