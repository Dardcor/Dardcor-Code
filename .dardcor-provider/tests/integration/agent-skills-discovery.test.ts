 ...
 *  3. MCP tool dardcorCode_agent_skills_list handler returns 44 entries.
...
// ── §3: MCP tool dardcorCode_agent_skills_list ─────────────────────────────────

test("MCP dardcorCode_agent_skills_list handler returns count 45 (44 + config)", async () => {
...
test("MCP dardcorCode_agent_skills_list result has all 42 IDs", async () => {
  const result = await agentSkillTools.dardcorCode_agent_skills_list.handler({});
