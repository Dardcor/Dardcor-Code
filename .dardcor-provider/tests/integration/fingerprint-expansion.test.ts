...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-fingerprint-e2e-"));
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 120_000) {
    if (logs.exitInfo) {
      throw new Error(
        [
          `DardcorCode exited before it became ready (code=${logs.exitInfo.code}, signal=${logs.exitInfo.signal})`,
          "--- stdout ---",
  ...
  throw new Error(
    [
      `Timed out waiting for DardcorCode to start: ${lastError}`,
      "--- stdout ---",
      ...logs.stdoutLines.slice(-40),
      "--- stderr ---",
      ...logs.stderrLines.slice(-40),
...
test("round-robin combo with 3 fingerprints: all requests succeed", async () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    // X-DardcorCode-Model header (echoRequestedModelName/#1311 is opt-in and off
