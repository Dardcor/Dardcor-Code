...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-omp-settings-"));
...
test("omp-settings GET: treats an existing agent.db as installed", async () => {
  ...
  assert.equal(body.hasDardcorCode, false);
});
...
test("omp-settings POST: writes models.yml and persists credentials for a seeded DB", async () => {
  ...
  assert.equal(getBody.hasDardcorCode, true);
});

// ── Test 6: DELETE → removes DardcorCode provider entry ────────────────────────

test("omp-settings DELETE: removes the DardcorCode provider from models.yml and credentials", async () => {
  seedOmpDb();
  await POST(
    req({
      method: "POST",
  ...
  const getRes = await GET(req());
  const getBody = await getRes.json();
  assert.equal(getBody.hasDardcorCode, false);
});
