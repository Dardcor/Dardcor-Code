...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-codewhale-settings-"));
...
test("codewhale-settings POST: writes primary ~/.codewhale/config.toml for a fresh install", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      assert.ok(content.includes("managed by DardcorCode"), "Config should have DardcorCode marker");
...
test("codewhale-settings GET: falls back to legacy ~/.deepseek/config.toml when primary is absent", async () => {
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      '# managed by DardcorCode (plan 14)\n[openai]\nbase_url = "http://localhost:20128"\n'
    );
    ...
    const body = await res.json();
    if (body.config) {
      assert.ok(body.config.includes("managed by DardcorCode"));
      assert.equal(body.hasDardcorCode, true);
    }
  } finally {
    process.env.HOME = origHome;
    fs.rmSync(tmpHome, { recursive: true, force: true });
