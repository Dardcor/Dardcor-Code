 ...
 *
 * Letta configures DardcorCode as its "lmstudio" provider (localModelDiscovery:
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-letta-settings-"));
...
// ── Test 6: POST refuses to overwrite an existing non-DardcorCode lmstudio config ──
...
// ── Test 7: DELETE → removes the DardcorCode lmstudio config ──────────────────
