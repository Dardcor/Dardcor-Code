...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-allstatuses-"));
...
test("cache hit: pre-populated cache is returned without re-executing", async () => {
  ...
  const knownStatus = {
    ...
    config: { status: "configured" as const, endpoint: "http://cached.dardcorCode.local" },
  };
