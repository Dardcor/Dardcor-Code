...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-smelt-settings-"));
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        assert.equal(written._managedBy, "dardcorCode");
...
// ── Test 5: DELETE → removes DardcorCode fields ────────────────────────────────

test("smelt-settings DELETE: removes DardcorCode fields from existing config", async () => {
