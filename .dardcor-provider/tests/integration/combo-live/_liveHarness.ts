...
export async function createLiveHarness(prefix: string): Promise<LiveHarness> {
  ...
  // -------------------------------------------------------------------------
  const snapshotDir = fs.mkdtempSync(path.join(os.tmpdir(), `dardcorCode-live-${prefix}-`));
  ...
  try {
    const output = execFileSync(
      ...
      [
        "root@192.168.0.15",
        'grep -E "^(STORAGE_ENCRYPTION_KEY|API_KEY_SECRET)=" ~/.dardcorCode/.env',
      ],
  ...
  try {
    execFileSync(
      "scp",
      ["root@192.168.0.15:/root/.dardcorCode/storage.sqlite", snapshotDbPath],
      { timeout: 60_000 }
    );
  } catch (err: any) {
    fs.rmSync(snapshotDir, { recursive: true, force: true });
   ...
   * `X-DardcorCode-Selected-Connection-Id` on the response, but only on the
  ...
  function servedProvider(response: Response): string | undefined {
    const connectionId = response.headers.get("X-DardcorCode-Selected-Connection-Id");
  ...
  async function cleanup(): Promise<void> {
    ...
    // Destroy the snapshot — targets only the temp dir, NEVER /root/.dardcorCode.
    fs.rmSync(snapshotDir, { recursive: true, force: true });
  }
