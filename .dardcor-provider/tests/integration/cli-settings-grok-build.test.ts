 ...
 * rebuilt on top of DardcorCode's existing "custom" configType settings pattern
 ...
 * the `[model.dardcorCode]` section and preserves the rest of the file.
 */
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-grok-build-settings-"));
...
// ── Test 4: POST with valid body → surgically upserts [model.dardcorCode] ─────

test("grok-build-settings POST: writes [model.dardcorCode] section and preserves existing content", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      assert.ok(content.includes("[model.dardcorCode]"), "Config should have [model.dardcorCode]");
      assert.ok(content.includes("http://localhost:20128/v1"), "Config should contain base URL");
      assert.ok(content.includes('default = "dardcorCode"'), "Default should point at our slot");
      ...
      assert.ok(
        content.includes('dardcorCode-prev-default = "grok-build"'),
...
test("grok-build-settings DELETE: removes our section, preserves the rest, restores default", async () => {
  ...
  try {
    ...
    const preConfigured = [
      ...
      'default = "dardcorCode"',
      "",
      "# dardcorCode-prev-default = \"grok-build\"",
      "[model.dardcorCode]",
      'model = "grok-4.5"',
    ...
    if (res.status === 200) {
      ...
      const configPath = path.join(tmpHome, ".grok", "config.toml");
      const content = fs.readFileSync(configPath, "utf-8");
      assert.ok(!content.includes("[model.dardcorCode]"), "Our section should be removed");
