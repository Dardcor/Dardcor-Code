...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-plugins-lifecycle-"));
...
function writeTestPlugin(opts?: { name?: string; onRequest?: boolean; enabledByDefault?: boolean }) {
  ...
  const sourceDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-plugin-src-"));
...
test("install: throws on invalid source directory", async () => {
  const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-empty-"));
