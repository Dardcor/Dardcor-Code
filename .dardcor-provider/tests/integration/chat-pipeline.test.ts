...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-chat-pipeline-"));
...
test("chat pipeline serves repeated /v1/responses requests as MISS then HIT and logs cache hits separately", async () => {
  ...
  assert.equal(firstResponse.headers.get("X-DardcorCode-Cache"), "MISS");
  assert.equal(secondResponse.headers.get("X-DardcorCode-Cache"), "HIT");
  assert.equal(thirdResponse.headers.get("X-DardcorCode-Cache"), "HIT");
...
test("chat pipeline treats Accept text/event-stream as streaming mode and returns a session header", async () => {
  ...
  assert.equal(response.status, 200);
  assert.equal(response.headers.get("Content-Type"), "text/event-stream");
  assert.ok(response.headers.get("X-DardcorCode-Session-Id"));
