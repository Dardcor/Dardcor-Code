...
function dardcorCodeCompress(text: string): string {
  return dardcorCodePromptCompression(text).text;
}

function dardcorCodePromptCompression(text: string): { text: string; fallbackApplied: boolean } {
...
describe("upstream Caveman parity benchmark", () => {
  it("matches core upstream shrink protections and savings direction", () => {
    for (const input of parityCases) {
      const ours = dardcorCodeCompress(input);
      const theirs = upstream.compress(input).compressed;
      assert.ok(ours.length <= input.length, `DardcorCode did not reduce: ${input}`);
  ...
  it("stays within upstream token budget on representative prose", () => {
    ...
    assert.ok(
      ...
      `Expected DardcorCode within 20% of upstream shrink length. ours=${ours.length}, upstream=${theirs.length}`
    );
  });
  ...
  it("runs offline parity against upstream Caveman fixture files", () => {
    ...
    for (const fixture of fixturePairs) {
      ...
      const ours = dardcorCodePromptCompression(original);
      ...
      if (ours.fallbackApplied) {
        ...
        assert.ok(ours.text.length < original.length, `${fixture.name}: DardcorCode did not reduce`);
        assert.ok(
          ours.text.length <= Math.ceil(Math.max(expected.length, upstreamShrink.length) * 1.35),
          `${fixture.name}: DardcorCode drifted too far from upstream fixture budget`
        );
      }
