...
import {
  ...
  diskSnapshotPath,
  type DardcorCodeFetchCacheEntry,
} from "../src/index.js";

function makeEntry(): Omit<DardcorCodeFetchCacheEntry, "expiresAt"> {
  return {
    rawModels: [],
    rawCombos: [],
...
test("defaultDiskSnapshotWriter writes an owner-only (no group/other) snapshot", async (t) => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "dardcorCode-disk-perms-"));
