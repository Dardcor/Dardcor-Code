 ...
 *
 * Covers `createDardcorCodeFetchInterceptor` (URL-prefix gating, header merge,
...
import { createDardcorCodeAuthHook, createDardcorCodeFetchInterceptor } from "../src/index.js";
...
test("createDardcorCodeFetchInterceptor: targets baseURL → Authorization header injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDardcorCodeFetchInterceptor: path-prefixed baseURL scopes auth to its normalized inference paths", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const prefixedBase = "https://or.example.com/tenant-a/v1";
    const f = createDardcorCodeFetchInterceptor({
      apiKey: KEY,
    ...
    const suffixingInterceptor = createDardcorCodeFetchInterceptor({
      apiKey: KEY,
...
test("createDardcorCodeFetchInterceptor: targets baseURL → Authorization OVERRIDES caller-supplied Bearer", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createDardcorCodeFetchInterceptor: targets baseURL + body → Content-Type defaults to application/json", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
    await f(`${BASE}/chat/completions`, {
...
test("createDardcorCodeFetchInterceptor: caller-set Content-Type is NOT overwritten", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDardcorCodeFetchInterceptor: non-baseURL host → passthrough, no Authorization injected", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDardcorCodeFetchInterceptor: refuses suffix-spoof — `${base}-attacker.evil` does NOT match baseURL", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDardcorCodeFetchInterceptor: URL object input is handled", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDardcorCodeFetchInterceptor: Request input is handled (reads .url)", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("createDardcorCodeFetchInterceptor: trailing slash in baseURL is normalized", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({
      apiKey: KEY,
...
test("createDardcorCodeFetchInterceptor: GET without body does NOT set Content-Type", async () => {
  const { calls, restore } = installFetchRecorder();
  try {
    const f = createDardcorCodeFetchInterceptor({ apiKey: KEY, baseURL: BASE });
...
test("loader: returns fetch fn when apiKey + baseURL both present (via opts)", async () => {
  const hook = createDardcorCodeAuthHook({ baseURL: BASE });
...
test("loader: returns fetch fn when baseURL is stashed on the auth credential", async () => {
  ...
  const hook = createDardcorCodeAuthHook();
...
test("loader: omits fetch fn when baseURL missing (apiKey-only return)", async () => {
  const hook = createDardcorCodeAuthHook(); // no baseURL opt
  const result = await hook.loader!(async () => ({ type: "api", key: KEY }) as never, {} as never);
