 ...
 *
 * Covers `createDardcorCodeConfigHook(opts, deps)`:
 *   - happy path: valid auth.json → mutates input.provider[id] with the
 *     stripped per-model shape (mirroring `@dardcorCode/opencode-provider`).
...
import {
  ...
  createDardcorCodeConfigHook,
  createDardcorCodeProviderHook,
  DardcorCodePlugin,
  resolveDardcorCodePluginOptions,
  type DardcorCodeCombosFetcher,
  type DardcorCodeEnrichmentEntry,
  type DardcorCodeEnrichmentFetcher,
  type DardcorCodeEnrichmentMap,
  type DardcorCodeFetchCache,
  type DardcorCodeModelsFetcher,
  type DardcorCodeProviderConnection,
  type DardcorCodeProvidersFetcher,
  type dardcorCodeRawCombo,
  type dardcorCodeRawModelEntry,
  type dardcorCodeReadAuthJson,
  type DardcorCodeStaticProviderEntry,
} from "../src/index.js";
...
const MODEL_CLAUDE: dardcorCodeRawModelEntry = {
...
const MODEL_GEMINI: dardcorCodeRawModelEntry = {
  id: "gemini-3-flash",
...
const COMBO_CLAUDE_TIER: dardcorCodeRawCombo = {
...
): dardcorCodeReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: dardcorCodeReadAuthJson = async () => {
    n++;
...
function throwingReadAuthJson(): dardcorCodeReadAuthJson & { callCount: () => number } {
  let n = 0;
  const f: dardcorCodeReadAuthJson = async () => {
    n++;
...
function stubModelsFetcher(
  payload: dardcorCodeRawModelEntry[]
): DardcorCodeModelsFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DardcorCodeModelsFetcher = async (baseURL, apiKey) => {
    n++;
...
function stubCombosFetcher(
  payload: dardcorCodeRawCombo[]
): DardcorCodeCombosFetcher & { callCount: () => number; callsBy: () => Array<[string, string]> } {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DardcorCodeCombosFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingModelsFetcher(): DardcorCodeModelsFetcher & { callCount: () => number } {
  let n = 0;
  const f: DardcorCodeModelsFetcher = async () => {
    n++;
...
function throwingCombosFetcher(): DardcorCodeCombosFetcher & { callCount: () => number } {
  let n = 0;
  const f: DardcorCodeCombosFetcher = async () => {
    n++;
...
function stubEnrichmentFetcher(payload: DardcorCodeEnrichmentMap): DardcorCodeEnrichmentFetcher & {
  callCount: () => number;
  callsBy: () => Array<[string, string]>;
} {
  let n = 0;
  const calls: Array<[string, string]> = [];
  const f: DardcorCodeEnrichmentFetcher = async (baseURL, apiKey) => {
    n++;
    calls.push([baseURL, apiKey]);
    return payload;
  };
...
function throwingEnrichmentFetcher(): DardcorCodeEnrichmentFetcher & { callCount: () => number } {
  let n = 0;
  const f: DardcorCodeEnrichmentFetcher = async () => {
    n++;
...
test("config: with valid auth.json + apiKey + baseURL → mutates input.provider[id] with stripped models block", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "api", key: "sk-test-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createDardcorCodeConfigHook(
  ...
  const provider = (input as { provider: Record<string, DardcorCodeStaticProviderEntry> }).provider;
  const entry = provider["opencode-dardcorCode"];
  assert.ok(entry, "input.provider['opencode-dardcorCode'] set");
  assert.equal(entry.npm, "@ai-sdk/openai-compatible");
  assert.equal(entry.name, "DardcorCode");
  ...
  const claude = entry.models["opencode-dardcorCode/claude-sonnet-4-6"];
  ...
  const combo = entry.models["dardcorCode/claude-tier"];
...
//     prefixed (`opencode-dardcorCode`). Without the fallback the lookup misses
...
test("config: auth.json under bare key (pre-prefix login) resolves via dual-key fallback", async () => {
  // Stored under bare `dardcorCode` (the key OC wrote before the auto-prefix fix),
  // but the resolved providerId is now `opencode-dardcorCode`.
  const readAuthJson = stubReadAuthJson({
    dardcorCode: { type: "api", key: "sk-bare-1", baseURL: "https://or.example.com/v1" },
  });
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode" }, // resolves to opencode-dardcorCode internally
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: prefixed key wins over bare key when both present (dual-key precedence)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "api", key: "sk-prefixed", baseURL: "https://pref.example/v1" },
    dardcorCode: { type: "api", key: "sk-bare", baseURL: "https://bare.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const entry = (input as { provider: Record<string, DardcorCodeStaticProviderEntry> }).provider[
    "opencode-dardcorCode"
  ];
  assert.ok(entry);
  assert.equal(
...
test("config: existing input.provider[id] → no overwrite (respect manual override)", async () => {
  const manual = {
    ...
    name: "Manual DardcorCode",
    options: { baseURL: "http://manual/v1", apiKey: "manual-key" },
    models: { "manual-model": { name: "manual-model" } },
  };
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "api", key: "sk-test", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const input = makeInput({ "opencode-dardcorCode": manual });
  await hook(input);

  const provider = (input as { provider: Record<string, unknown> }).provider;
  assert.equal(provider["opencode-dardcorCode"], manual, "manual override preserved by reference");
...
test("config: fetchers throw → warn + emit stub entry with models: {}", async () => {
  ...
  // test below).
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", features: { diskCache: false } },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: combos fetcher throws → emit models-only catalog (no combos in models block)", async () => {
  ...
  assert.deepEqual(ids, [
    "opencode-dardcorCode/claude-sonnet-4-6",
    "opencode-dardcorCode/gemini-3-flash",
  ]);
  assert.equal(entry.models["dardcorCode/claude-tier"], undefined, "no combo entry");
  assert.ok(
    logger.entries.some((e) => String(e[0]).includes("/api/combos fetch failed")),
    "combos-fetch breadcrumb emitted"
  );
...
test("config: baseURL from auth.json takes precedence when opts.baseURL absent", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "api", key: "sk-test", baseURL: "https://creds.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode" }, // NO opts.baseURL
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: opts.baseURL wins over auth.json's stored baseURL", async () => {
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", baseURL: "https://opts.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: no baseURL resolvable (no opts, no auth.json baseURL) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "api", key: "sk-test" }, // NO baseURL on the credential
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
...
test("config: multi-instance — two plugins with different providerIds publish to their own keys without collision", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode-prod": {
      type: "api",
      key: "sk-prod",
      baseURL: "https://prod.example/v1",
    },
    "opencode-dardcorCode-preprod": {
      type: "api",
      key: "sk-preprod",
  ...
  const hookA = createDardcorCodeConfigHook(
    { providerId: "dardcorCode-prod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const hookB = createDardcorCodeConfigHook(
    { providerId: "dardcorCode-preprod" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  ...
  const provider = (input as { provider: Record<string, DardcorCodeStaticProviderEntry> }).provider;
  assert.ok(provider["opencode-dardcorCode-prod"], "prod block present");
  assert.ok(provider["opencode-dardcorCode-preprod"], "preprod block present");
  assert.equal(provider["opencode-dardcorCode-prod"].options.apiKey, "sk-prod");
  assert.equal(provider["opencode-dardcorCode-preprod"].options.apiKey, "sk-preprod");
  assert.equal(provider["opencode-dardcorCode-prod"].options.baseURL, "https://prod.example/v1");
  assert.equal(
    provider["opencode-dardcorCode-preprod"].options.baseURL,
    "https://preprod.example/v1"
  );
  assert.notEqual(
    provider["opencode-dardcorCode-prod"],
    provider["opencode-dardcorCode-preprod"],
...
test("config + provider share cache: second call uses cached fetch result (single fetch per TTL)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "api", key: "sk-shared", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([COMBO_CLAUDE_TIER]);
  ...
  const configHook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, cache: sharedCache, logger }
  );
  const providerHook = createDardcorCodeProviderHook(
    { providerId: "dardcorCode", baseURL: "https://or.example/v1", modelCacheTtl: 60_000 },
...
test("provider → config order also dedupes (cache populated by provider, consumed by config)", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "api", key: "sk-reverse", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const sharedCache: DardcorCodeFetchCache = new Map();
...
test("buildStaticProviderEntry: stripped per-model shape matches sibling @dardcorCode/opencode-provider", () => {
  const resolved = resolveDardcorCodePluginOptions({
    providerId: "dardcorCode",
    displayName: "DardcorCode",
  });
  ...
  assert.equal(block.name, "DardcorCode");
  ...
  const claude = block.models["opencode-dardcorCode/claude-sonnet-4-6"];
...
test("buildStaticProviderEntry: hidden combos are excluded", () => {
  ...
  assert.equal(block.models["dardcorCode/claude-tier"], undefined);
  assert.ok(block.models["opencode-dardcorCode/claude-sonnet-4-6"]);
});
...
test("buildStaticProviderEntry: emits release_date when raw carries it; omits when null", () => {
  const resolved = resolveDardcorCodePluginOptions({ providerId: "dardcorCode" });
  const withDate: dardcorCodeRawModelEntry = {
    ...MODEL_CLAUDE,
    id: "claude-with-date",
  ...
  assert.equal(block.models["opencode-dardcorCode/claude-with-date"].release_date, "2026-02-19");
  assert.equal(block.models["opencode-dardcorCode/gemini-3-flash"].release_date, undefined);
});

test("buildStaticProviderEntry: combo modalities = intersection of members (LCD)", () => {
  const resolved = resolveDardcorCodePluginOptions({ providerId: "dardcorCode" });
  const TEXT_ONLY: dardcorCodeRawModelEntry = {
    id: "text-only",
  ...
  const combo = block.models["dardcorCode/mixed-tier"];
...
// ────────────────────────────────────────────────────────────────────────────
// Integration: DardcorCodePlugin factory now exposes config hook
// ────────────────────────────────────────────────────────────────────────────

test("DardcorCodePlugin factory exposes config hook alongside auth + provider", async () => {
  const hooks = await DardcorCodePlugin({} as never, { providerId: "dardcorCode" });
...
test("config: auth.json entry of wrong type (oauth) → no-op", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "oauth", refresh: "r", access: "a", expires: 0 },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", baseURL: "https://or.example/v1" },
    { readAuthJson, fetcher, combosFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: initialises input.provider when undefined", async () => {
  const readAuthJson = stubReadAuthJson({
    "opencode-dardcorCode": { type: "api", key: "sk", baseURL: "https://or.example/v1" },
  });
  const fetcher = stubModelsFetcher([MODEL_CLAUDE]);
  const combosFetcher = stubCombosFetcher([]);
  const logger = captureWarn();
  ...
  assert.ok(provider!["opencode-dardcorCode"]);
});
...
test("config: enrichment fetched + name overlaid on raw-model entries", async () => {
  ...
  const enrichmentFetcher = stubEnrichmentFetcher(
    new Map<string, DardcorCodeEnrichmentEntry>([
  ...
  assert.ok(entry);
  assert.equal(entry.models["opencode-dardcorCode/claude-sonnet-4-6"].name, "Claude Sonnet 4.6");
  assert.equal(entry.models["opencode-dardcorCode/gemini-3-flash"].name, "Gemini 3 Flash");
  // Combo names still come from /api/combos — enrichment overlay does NOT touch combos.
  assert.equal(entry.models["dardcorCode/claude-tier"].name, "Claude Tier");
...
test("config: features.enrichment=false skips enrichment fetch + keeps raw-id names", async () => {
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", features: { enrichment: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
  ...
  assert.equal(
    entry.models["opencode-dardcorCode/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id retained"
  );
});
...
function stubProvidersFetcher(payload: DardcorCodeProviderConnection[]): DardcorCodeProvidersFetcher & {
  callCount: () => number;
} {
  let n = 0;
  const f: DardcorCodeProvidersFetcher = async () => {
    n++;
    return payload;
  };
  return Object.assign(f, { callCount: () => n });
...
function throwingProvidersFetcher(): DardcorCodeProvidersFetcher & { callCount: () => number } {
  let n = 0;
  const f: DardcorCodeProvidersFetcher = async () => {
    n++;
    throw new Error("ETIMEDOUT");
...
const MODEL_CC_OPUS: dardcorCodeRawModelEntry = {
  id: "cc/claude-opus-4-7",
  capabilities: { tool_calling: true, reasoning: true, temperature: true },
  context_length: 200_000,
};
const MODEL_NV_LLAMA: dardcorCodeRawModelEntry = {
...
test("config: usableOnly=true → drops models for non-usable providers, keeps usable + unknown", async () => {
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", baseURL: "https://or.example/v1", features: { usableOnly: true } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, providersFetcher }
  );
...
test("config: diskCache hydrates stale snapshot when /v1/models throws", async () => {
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", features: { diskCache: true } },
    {
      readAuthJson,
      fetcher,
      combosFetcher,
  ...
  assert.ok(
    entry.models["opencode-dardcorCode/claude-sonnet-4-6"],
...
test("config: providerTag (default-on) prepends '<provider> - ' to enriched raw-model names", async () => {
  ...
  assert.equal(entry.models["opencode-dardcorCode/gemini-3-flash"].name, "Gemini - Gemini 3 Flash");
...
test("config: providerTag=false suppresses the suffix", async () => {
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", features: { providerTag: false } },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, logger }
  );
  const input = makeInput();
  await hook(input);
...
test("config: providerTag falls back to UPPER(alias) when providerDisplayName missing", async () => {
  ...
  // a slot DardcorCode hasn't curated a human label for yet. We still
  ...
  assert.equal(entry.models["opencode-dardcorCode/claude-sonnet-4-6"].name, "CC - Claude Sonnet 4.6");
});
...
test("config: providerTag is idempotent — second hook call doesn't double-suffix", async () => {
  ...
  const hook = createDardcorCodeConfigHook(
    { providerId: "dardcorCode", modelCacheTtl: 60_000 },
    { readAuthJson, fetcher, combosFetcher, enrichmentFetcher, cache: sharedCache, logger }
  );

  const inputA = makeInput();
  await hook(inputA);
  const entryA = (inputA as { provider: Record<string, DardcorCodeStaticProviderEntry> }).provider[
    "opencode-dardcorCode"
  ];
  assert.equal(
    entryA.models["opencode-dardcorCode/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
  ...
  await hook(inputB);
  const entryB = (inputB as { provider: Record<string, DardcorCodeStaticProviderEntry> }).provider[
    "opencode-dardcorCode"
  ];
  assert.equal(
    entryB.models["opencode-dardcorCode/claude-sonnet-4-6"].name,
    "Claude - Claude Sonnet 4.6"
  );
});
...
test("buildStaticProviderEntry: nested combo-ref context is the bottleneck across the graph", () => {
  const resolved = resolveDardcorCodePluginOptions({ providerId: "dardcorCode" });
  const rawModels: dardcorCodeRawModelEntry[] = [
    {
      id: "raw-big",
      context_length: 200_000,
      max_output_tokens: 64_000,
  ...
  const rawCombos: dardcorCodeRawCombo[] = [
    {
      id: "tiny-combo",
      name: "TinyCombo",
  ...
  const parent = block.models["dardcorCode/parent"];
