/**
 * OpenCode plugin for the DardcorCode AI Gateway.
 *
 * Implements the official `@opencode-ai/plugin` Plugin contract (auth +
 * provider + config hooks) to drive a running DardcorCode instance from
 ...
 *                  pass-through (DardcorCode is the source of truth — no
 ...
 *
 *  1. Single-instance (default `providerId: "dardcorCode"`):
 *
 *     ```json
 *     {
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": ["@dardcorCode/opencode-plugin"]
 *     }
 *     ```
 *
 ...
 *       "$schema": "https://opencode.ai/config.json",
 *       "plugin": [
 *         ["@dardcorCode/opencode-plugin", { "providerId": "dardcorCode" }],
 *         ["@dardcorCode/opencode-plugin", { "providerId": "dardcorCode-preprod" }]
 *       ]
 *     }
 *     ```
 *
 ...
 *
 * Companion library: `@dardcorCode/opencode-provider` (build-time config generator)
 ...
 * @see https://github.com/diegosouzapw/DardcorCode for the AI Gateway.
 */
 ...
 *                     different `providerId` ("dardcorCode", "dardcorCode-preprod",
 *                     "dardcorCode-local"). Maps to `ProviderHook.id` and
 ...
 *  - `baseURL`        Override base URL for this DardcorCode instance. When
 ...
 *                           JSONL file at
 *                           `~/.local/share/opencode/plugins/dardcorCode-debug-{providerId}.jsonl`.
 ...
 *                           (covers DardcorCode's canonical Anthropic aliases).
 *
 ...
 * import `DardcorCodePluginOptions` as before; only the source of truth
 * shifted from a hand-written interface to `z.infer<typeof optionsSchema>`.
 */
export type DardcorCodePluginOptions = z.infer<typeof optionsSchema>;
...
export type DardcorCodeFeatureFlag = keyof typeof OMNIROUTE_FEATURE_DEFAULTS;
...
export function resolveEffectiveFeatureFlags(
  features?: DardcorCodePluginOptions["features"]
): Record<DardcorCodeFeatureFlag, boolean> {
  const f: Partial<Record<DardcorCodeFeatureFlag, boolean>> = features ?? {};
  const out = {} as Record<DardcorCodeFeatureFlag, boolean>;
  for (const key of Object.keys(OMNIROUTE_FEATURE_DEFAULTS) as DardcorCodeFeatureFlag[]) {
    const val = f[key];
...
export const OMNIROUTE_PROVIDER_KEY = "dardcorCode" as const;
 ...
 */
export function resolveDardcorCodePluginOptions(opts?: DardcorCodePluginOptions): Required<
  Pick<
    DardcorCodePluginOptions,
    "providerId" | "displayName" | "modelCacheTtl" | "autoSyncIntervalMs"
  >
> & {
  /**
   * #6859: the UNPREFIXED provider id ("dardcorCode", "dardcorCode-preprod", …).
   ...
   * `provider.<id>` block). `dardcorCodeProviderId` MUST be used everywhere an
   * identifier reaches or represents something DardcorCode's own server parses
   * (model `id` prefix, `ModelV2.providerID`, combo catalog keys in the
   * dynamic provider hook) — DardcorCode's `parseModel()` has no alias for
   * "opencode-<x>", so a prefixed id there is unrecoverable and credential
   * lookup fails with "No credentials for opencode-<x>".
   */
  dardcorCodeProviderId: string;
} & Pick<DardcorCodePluginOptions, "baseURL" | "managementReadToken" | "features"> {
  const rawProviderId = opts?.providerId ?? OMNIROUTE_PROVIDER_KEY;
  const dardcorCodeProviderId = trimLeadingOpencodePrefix(rawProviderId);
  // OC 1.17.8+ native-adapter gate rejects providerID not in
  // {openai, anthropic, opencode*}. Silently prefix so existing
  // configs (providerId: "dardcorCode") keep working.
...
export type ResolvedDardcorCodePluginOptions = ReturnType<typeof resolveDardcorCodePluginOptions>;

/**
 * Strip a leading "opencode-" prefix (added only for the OC native-adapter
 * gate — see `resolveDardcorCodePluginOptions`) so the returned id is safe to
 * embed in anything DardcorCode's own server parses. A user-supplied
 * `providerId: "opencode-dardcorCode"` (already prefixed) resolves to the same
 * unprefixed "dardcorCode" as the default, matching `providerId`'s own
 ...
 * direct factory call) into the validated `DardcorCodePluginOptions` shape.
 *
 ...
 *
 * Validation happens at plugin invocation time (inside `DardcorCodePlugin`),
 ...
 */
export function parseDardcorCodePluginOptions(opts: unknown): DardcorCodePluginOptions {
  ...
  if (!result.success) {
    const errs = result.error.issues
      ...
      .join("; ");
    throw new Error(`Invalid @dardcorCode/opencode-plugin options: ${errs}`);
  }
  return result.data;
}

/**
 * Internal coercion shim. Delegates to `parseDardcorCodePluginOptions` to keep
 ...
 */
function coercePluginOptions(opts?: PluginOptions): DardcorCodePluginOptions {
  return parseDardcorCodePluginOptions(opts);
}
 ...
 * Covers DardcorCode's canonical Anthropic aliases: `cc/`, `claude/`,
 ...
 *     the multi-instance bug in opencode-dardcorCode-auth@1.2.1 which pinned
 *     `OMNIROUTE_PROVIDER_ID = "dardcorCode"` at module scope).
 *   - `methods[0]` is the `api` flavor (no OAuth flow; DardcorCode issues bearer
 ...
 */
export function createDardcorCodeAuthHook(opts?: DardcorCodePluginOptions): AuthHook {
  const { providerId, displayName, baseURL, features } = resolveDardcorCodePluginOptions(opts);
        ...
        prompts: [
          {
            type: "text",
            key: "apiKey",
            message: `DardcorCode API key (${providerId})`,
          },
        ],
      },
    ],
    loader: async (getAuth, _provider) => {
      ...
      if (
        ...
        if (wantFetchInterceptor) {
          composedFetch = createDardcorCodeFetchInterceptor({
            apiKey,
 ...
 */
export function invalidateDardcorCodeFetchCache(
  cache: DardcorCodeFetchCache,
  baseURL?: string
): number {
  if (!baseURL) {
    const n = cache.size;
 ...
 */
export async function resolveDardcor CodeuntimeAuth(
  resolved: ResolvedDardcorCodePluginOptions,
  readAuthJson?: dardcorCodeReadAuthJson
  ...
  if (resolved.dardcorCodeProviderId && !lookupKeys.includes(resolved.dardcorCodeProviderId)) {
    lookupKeys.push(resolved.dardcorCodeProviderId);
  }
...
/**
 * Force-refresh DardcorCode catalog: clear memory + disk cache, re-fetch /v1/models
 * (and optional management endpoints), and repopulate the shared cache.
 * OpenCode equivalent of Pi `/omni sync`.
 */
export async function forceSyncDardcorCodeModels(args: {
  resolved: ResolvedDardcorCodePluginOptions;
  cache: DardcorCodeFetchCache;
  readAuthJson?: dardcorCodeReadAuthJson;
  fetcher?: DardcorCodeModelsFetcher;
  combosFetcher?: DardcorCodeCombosFetcher;
  autoCombosFetcher?: DardcorCodeAutoCombosFetcher;
  enrichmentFetcher?: DardcorCodeEnrichmentFetcher;
  compressionMetaFetcher?: DardcorCodeCompressionMetaFetcher;
  providersFetcher?: DardcorCodeProvidersFetcher;
  now?: () => number;
}): Promise<{
  ok: boolean;
  count: number;
  ...
  const fetcher = args.fetcher ?? defaultDardcorCodeModelsFetcher;
  const combosFetcher = args.combosFetcher ?? defaultDardcorCodeCombosFetcher;
  const autoCombosFetcher = args.autoCombosFetcher ?? defaultDardcorCodeAutoCombosFetcher;
  const enrichmentFetcher = args.enrichmentFetcher ?? defaultDardcorCodeEnrichmentFetcher;
  const compressionMetaFetcher =
    args.compressionMetaFetcher ?? defaultDardcorCodeCompressionMetaFetcher;
  const providersFetcher = args.providersFetcher ?? defaultDardcorCodeProvidersFetcher;
  ...
  const auth = await resolveDardcor CodeuntimeAuth(
    resolved,
  ...
  if (!auth) {
    return {
      ...
      count: 0,
      combos: 0,
      provider: resolved.dardcorCodeProviderId,
      clearedMemory: 0,
      clearedDisk: false,
      error:
        "No DardcorCode credentials/baseURL available. Run `opencode connect dardcorCode` or set plugin baseURL.",
    };
  }

  const clearedMemory = invalidateDardcorCodeFetchCache(cache, auth.baseURL);
  // Clear residual entries from prior baseURL history as well.
  const clearedAll = invalidateDardcorCodeFetchCache(cache);
  let clearedDisk = false;
  if (wantDiskCache) {
    clearedDisk = await clearDiskSnapshot(resolved.providerId);
    if (resolved.dardcorCodeProviderId !== resolved.providerId) {
      clearedDisk = (await clearDiskSnapshot(resolved.dardcorCodeProviderId)) || clearedDisk;
    }
  }

  try {
    const rawModels = await fetcher(auth.baseURL, auth.apiKey, 10_000);
    let rawCombos: dardcorCodeRawCombo[] = [];
    if (wantCombos) {
      try {
        rawCombos = await combosFetcher(auth.baseURL, auth.managementReadToken, 10_000);
      } catch (err) {
        console.warn("[dardcorCode-plugin] force sync: combos fetch failed", err);
      }
    }
    let rawAutoCombos: dardcorCodeRawAutoCombo[] = [];
    if (wantAutoCombos) {
      try {
    ...
    let rawEnrichment: DardcorCodeEnrichmentMap = new Map();
    if (wantEnrichment) {
      try {
    ...
    let rawCompressionCombos: DardcorCodeCompressionCombo[] = [];
    ...
    let rawConnections: DardcorCodeProviderConnection[] = [];
    if (wantUsableOnly) {
      try {
    ...
    console.warn(
      `[dardcorCode-plugin] force sync ok providerId=${resolved.providerId} ` +
...
export function createDardcorCodeSyncModelsTool(args: {
  resolved: ResolvedDardcorCodePluginOptions;
  cache: DardcorCodeFetchCache;
  ...
  return tool({
    description:
      "Force-refresh the DardcorCode model catalog (OpenCode equivalent of Pi `/omni sync`). " +
    ...
    async execute(toolArgs) {
      const result = await forceSyncDardcorCodeModels({ resolved, cache });
      const reason = toolArgs.reason ? ` reason=${toolArgs.reason}` : "";
      if (!result.ok) {
        return {
          title: "DardcorCode sync failed",
          output:
            `DardcorCode model sync failed for provider=${result.provider}.${reason}\n` +
            `${result.error ?? "unknown error"}`,
          metadata: result,
        };
      }
      return {
        title: `DardcorCode sync: ${result.count} models`,
        output:
          `DardcorCode models synced.` +
 ...
 */
export function startDardcorCodeAutoSync(args: {
  resolved: ResolvedDardcorCodePluginOptions;
  cache: DardcorCodeFetchCache;
  intervalMs?: number;
}): () => void {
  ...
  const tick = () => {
    ...
    inFlight = (async () => {
      ...
      if (!result.ok) {
        console.warn(
          `[dardcorCode-plugin] auto-sync failed providerId=${resolved.providerId}: ${result.error}`
        );
        return;
      }
      ...
      if (result.count !== lastCount) {
        console.warn(
          `[dardcorCode-plugin] auto-sync catalog size changed ${lastCount} → ${result.count} ` +
      ...
      .catch((err) => {
        console.warn("[dardcorCode-plugin] auto-sync tick error", err);
      })
      .finally(() => {
        inFlight = null;
      });
  ...
  console.warn(
    `[dardcorCode-plugin] auto-sync enabled intervalMs=${intervalMs} providerId=${resolved.providerId}`
  );
...
export const DardcorCodePlugin: Plugin = async (_input, options) => {
  const resolved = resolveDardcorCodePluginOptions(coercePluginOptions(options));
  ...
  // Each `DardcorCodePlugin(...)` invocation gets its OWN cache via closure,
  // so prod + preprod side-by-side instances do NOT collide.
  const sharedCache: DardcorCodeFetchCache = new Map();
  ...
  startDardcorCodeAutoSync({ resolved, cache: sharedCache });

  const syncTool = createDardcorCodeSyncModelsTool({ resolved, cache: sharedCache });
  const bareProviderId = resolved.dardcorCodeProviderId;
  ...
  const baseConfigHook = createDardcorCodeConfigHook(resolved, { cache: sharedCache });
  const configWithSyncCommand = async (input: Config) => {
    ...
    if (!cfg.command["omni-sync"]) {
      cfg.command["omni-sync"] = {
        description: "Force-refresh DardcorCode model catalog (like Pi /omni sync)",
        template:
          `Force-refresh the DardcorCode model catalog now using the dardcorCode_sync_models tool ` +
    ...
    if (!cfg.command["omni-autosync"]) {
      cfg.command["omni-autosync"] = {
        description: "Show DardcorCode auto-sync / cache status",
        template:
          `Report DardcorCode discovery status for provider ${bareProviderId}: ` +
          `autoSyncIntervalMs=${resolved.autoSyncIntervalMs}, modelCacheTtl=${resolved.modelCacheTtl}. ` +
          `If the user asked to refresh now, call dardcorCode_sync_models.`,
      };
    }
  };

  return {
    auth: createDardcorCodeAuthHook(resolved),
    provider: createDardcorCodeProviderHook(resolved, { cache: sharedCache }),
    config: configWithSyncCommand,
    tool: {
      dardcorCode_sync_models: syncTool,
    },
  };
};
 ...
 */
const DardcorCodeV1Plugin = {
  id: "@dardcorCode/opencode-plugin",
  server: DardcorCodePlugin,
};

export default DardcorCodeV1Plugin;
...
/**
 * Raw shape of a `/v1/models` entry from DardcorCode. Captured verbatim from
 ...
 */
export interface dardcorCodeRawModelEntry {
  id: string;
  object?: string;
  owned_by?: string;
  root?: string | null;
 ...
 * DardcorCode instance. Surfaced as a dependency so unit tests can inject a
 * stub without monkey-patching global `fetch`.
 *
 * Why we inline this instead of using `@dardcorCode/opencode-provider`'s
 ...
 */
export type DardcorCodeModelsFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<dardcorCodeRawModelEntry[]>;

/**
 * Default fetcher: `GET <baseURL>/v1/models` with bearer auth + AbortController
 * timeout. Accepts both the `{object:"list", data:[…]}` envelope DardcorCode
 * emits today and a bare-array envelope (defensive — keeps the plugin
 * working if a future DardcorCode build trims the wrapper). Anything that
 * isn't an object with a string `id` is filtered out silently.
 */
export const defaultDardcorCodeModelsFetcher: DardcorCodeModelsFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@dardcorCode/opencode-plugin: apiKey required to fetch /v1/models");
  if (!baseURL) throw new Error("@dardcorCode/opencode-plugin: baseURL required to fetch /v1/models");
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error(
        `@dardcorCode/opencode-plugin: GET ${url} failed: ${res.status} ${res.statusText}`
      );
    }
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { data?: unknown }).data)
        ...
        : [];
    const out: dardcorCodeRawModelEntry[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as dardcorCodeRawModelEntry);
      }
    }
    return out;
  } finally {
 ...
 *   5. `thinking` from DardcorCode has no 1:1 ModelV2 slot. We OR it into
 *      `reasoning` so thinking-only models still surface a non-false
 *      reasoning flag.
 *   6. `last_updated` from DardcorCode has no ModelV2 slot — dropped (the
 ...
 *   7. `temperature: true` per DardcorCode convention (OpenAI-compat mode
 ...
 *      Unknown strings (future DardcorCode additions) are ignored — when the
 *      server adds new modalities we can map them here without breaking
 *      existing entries.
 *   9. `status: "active"` — DardcorCode doesn't tier models alpha/beta on
...
export function mapRawModelToModelV2(
  ...
  return {
    ...
    // `(dardcorCode, claude-opus-4)` and the credentials resolve correctly.
    id: raw.id.includes("/") ? raw.id : `${ctx.providerId}/${raw.id}`,
    /**
     * Display name. Falls back to raw.id when no enrichment is available;
     * the caller (`createDardcorCodeProviderHook`) overlays
...
/**
 * Raw shape of a single combo entry as returned by DardcorCode's `/api/combos`.
 *
 * Schema established via a live probe against
 * an DardcorCode `/api/combos` endpoint with a management-scoped key
 ...
 * bare array envelope so the plugin keeps working if a future DardcorCode
 * build trims the wrapper (mirrors the same pattern in the sibling
 * `@dardcorCode/opencode-provider#listCombos`).
 *
 ...
 */
export interface dardcorCodeRawComboMemberRef {
...
export interface dardcorCodeRawCombo {
  id: string;
  name?: string;
  ...
  models?: dardcorCodeRawComboMemberRef[];
  /** Hidden combos are excluded from the OC model picker. */
  isHidden?: boolean;
  /** When DardcorCode attaches a lifecycle hint we forward it; today it doesn't. */
  release_date?: string;
  /**
 ...
 * `DardcorCodeModelsFetcher` so unit tests can inject a stub instead of
 * monkey-patching global `fetch`.
 */
export type DardcorCodeCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<dardcorCodeRawCombo[]>;
 ...
 * plugin working if a future DardcorCode build trims the wrapper).
 *
 * Differences from `defaultDardcorCodeModelsFetcher`:
 ...
 */
export const defaultDardcorCodeCombosFetcher: DardcorCodeCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  if (!apiKey) throw new Error("@dardcorCode/opencode-plugin: apiKey required to fetch /api/combos");
  if (!baseURL)
    throw new Error("@dardcorCode/opencode-plugin: baseURL required to fetch /api/combos");
  ...
  try {
    ...
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ...
        : [];
    const out: dardcorCodeRawCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as dardcorCodeRawCombo);
      }
    }
    return out;
  } finally {
...
export function mapComboToModelV2(
  combo: dardcorCodeRawCombo,
  members: dardcorCodeRawModelEntry[],
  providerId: string,
  baseURL: string,
  apiFormat?: { anthropicPrefixes?: string[] }
): ModelV2 {
...
/**
 * Raw shape of an auto combo entry as returned by DardcorCode's
 ...
 */
export interface dardcorCodeRawAutoCombo {
  ...
  /** MAX of candidates' context windows, served by newer DardcorCode builds.
 ...
 */
export type DardcorCodeAutoCombosFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<dardcorCodeRawAutoCombo[]>;
 ...
 */
export const defaultDardcorCodeAutoCombosFetcher: DardcorCodeAutoCombosFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 5_000
) => {
  ...
  try {
    ...
    if (res.status === 404) {
      console.warn(
        `[dardcorCode-plugin] /api/combos/auto not available (404) — auto combos disabled`
      );
      return [];
    }
    if (!res.ok) {
      console.warn(
        `[dardcorCode-plugin] /api/combos/auto failed: ${res.status} ${res.statusText} — auto combos disabled`
      );
      return [];
    }
    const body = (await res.json()) as unknown;
    const rawList: unknown[] = Array.isArray(body)
      ...
      : body && typeof body === "object" && Array.isArray((body as { combos?: unknown }).combos)
        ? ((body as { combos: unknown[] }).combos as unknown[])
        : [];
    const out: dardcorCodeRawAutoCombo[] = [];
    for (const r of rawList) {
      if (r && typeof r === "object" && typeof (r as { id?: unknown }).id === "string") {
        out.push(r as dardcorCodeRawAutoCombo);
      }
    }
    return out;
  } catch (err) {
    ...
    console.warn(
      `[dardcorCode-plugin] /api/combos/auto fetch failed: ${err instanceof Error ? err.message : String(err)} — auto combos disabled`
    );
    return [];
  } finally {
    clearTimeout(timer);
 ...
 * DardcorCode builds). MUST be positive: OpenCode's overflow guard treats
...
export function mapAutoComboToStaticEntry(
  autoCombo: dardcorCodeRawAutoCombo
): DardcorCodeStaticModelEntry {
...
/**
 * Per-model enrichment overlay derived from DardcorCode's
 ...
 */
export interface DardcorCodeEnrichmentEntry {
   ...
   * Populated by `defaultDardcorCodeEnrichmentFetcher` from
...
export type DardcorCodeEnrichmentMap = Map<string, DardcorCodeEnrichmentEntry>;

export type DardcorCodeEnrichmentFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DardcorCodeEnrichmentMap>;
 ...
 * other.
 */
export const defaultDardcorCodeEnrichmentFetcher: DardcorCodeEnrichmentFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const out: DardcorCodeEnrichmentMap = new Map();
    ...
    if (res.ok) {
      ...
      if (providers && typeof providers === "object") {
        for (const [providerAlias, slot] of Object.entries(providers)) {
          ...
          // DardcorCode hasn't curated a label for this slot.
          ...
          for (const m of models) {
            ...
            const entry: DardcorCodeEnrichmentEntry = {
              providerAlias,
              providerCanonical,
            };
      ...
      if (body && typeof body === "object" && !Array.isArray(body)) {
        for (const [providerAlias, slot] of Object.entries(body as Record<string, unknown>)) {
          ...
          for (const [modelId, raw] of Object.entries(slot as Record<string, unknown>)) {
            ...
            const parsed: NonNullable<DardcorCodeEnrichmentEntry["pricing"]> = {};
            // DardcorCode `/api/pricing` keys:
            //   input         → cost.input
            //   output        → cost.output
...
async function writeStartupDiagnostics(params: {
  ...
  enrichment: DardcorCodeEnrichmentMap;
  autoCombos: dardcorCodeRawAutoCombo[];
  features?: DardcorCodePluginOptions["features"];
}): Promise<void> {
  const {
    providerId,
  ...
  lines.push(
    `features(effective): ` +
      (Object.keys(effectiveFlags) as DardcorCodeFeatureFlag[])
  ...
  try {
    ...
    const diagPath = path.join(diagDir, "plugins", "dardcorCode-startup-diagnostics.log");
    let existing = "";
    try {
...
// Re-export from naming.ts — thin wrapper preserving DardcorCodeEnrichmentEntry signature
export function shortProviderLabel(
  enrichment: DardcorCodeEnrichmentEntry | undefined
): string | undefined {
 ...
 *
 * DardcorCode's `/api/pricing/models` is keyed by short ALIAS (`cc`, `cx`,
...
export function buildCanonicalToAliasMap(
  enrichment: DardcorCodeEnrichmentMap | undefined
): Map<string, string> {
...
): DardcorCodeEnrichmentEntry | undefined {
...
export function canonicalDedupSet(
  rawModels: ReadonlyArray<dardcorCodeRawModelEntry>,
  canonicalToAlias: Map<string, string>
): Set<string> {
 ...
 *
 * Real example: DardcorCode's `pricing['cohere']` slot lists 10 curated
 ...
 * a minimal `DardcorCodeEnrichmentEntry` whenever the direct lookup
...
export function buildAliasIndex(
  enrichment: DardcorCodeEnrichmentMap | undefined
): Map<string, DardcorCodeEnrichmentEntry> {
  const out = new Map<string, DardcorCodeEnrichmentEntry>();
  if (!enrichment) return out;
  for (const entry of enrichment.values()) {
    const alias = typeof entry.providerAlias === "string" ? entry.providerAlias.trim() : "";
...
export function resolveProviderTagEntry(
  rawId: string,
  direct: DardcorCodeEnrichmentEntry | undefined,
  aliasIndex: Map<string, DardcorCodeEnrichmentEntry>,
  canonicalToAlias?: Map<string, string>
): DardcorCodeEnrichmentEntry | undefined {
  if (direct) {
...
export interface DardcorCodeCompressionStep {
...
export interface DardcorCodeCompressionCombo {
  id: string;
  name?: string;
  description?: string;
  pipeline: DardcorCodeCompressionStep[];
  isDefault?: boolean;
}

export type DardcorCodeCompressionMetaFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DardcorCodeCompressionCombo[]>;
 ...
 */
export const defaultDardcorCodeCompressionMetaFetcher: DardcorCodeCompressionMetaFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: DardcorCodeCompressionCombo[] = [];
  ...
  try {
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { combos?: unknown[] })?.combos)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ...
          : [];
    const out: DardcorCodeCompressionCombo[] = [];
    for (const raw of list) {
      ...
      const steps: DardcorCodeCompressionStep[] = [];
      for (const step of pipeline) {
        ...
        const entry: DardcorCodeCompressionStep = { engine };
        if (typeof intensity === "string" && intensity.length > 0) {
          entry.intensity = intensity;
        }
        steps.push(entry);
      }
      const combo: DardcorCodeCompressionCombo = { id, pipeline: steps };
 ...
 * DardcorCode knows but the plugin doesn't.
 *
 ...
 */
export function formatCompressionPipeline(pipeline: DardcorCodeCompressionStep[]): string {
...
// `features.usableOnly` filter. Returns the operator's installed DardcorCode
...
export interface DardcorCodeProviderConnection {
  /** Connection UUID. */
  id: string;
...
export type DardcorCodeProvidersFetcher = (
  baseURL: string,
  apiKey: string,
  timeoutMs?: number
) => Promise<DardcorCodeProviderConnection[]>;
 ...
 */
export const defaultDardcorCodeProvidersFetcher: DardcorCodeProvidersFetcher = async (
  baseURL,
  apiKey,
  timeoutMs = 10_000
) => {
  const empty: DardcorCodeProviderConnection[] = [];
  if (!baseURL || !apiKey) return empty;
  const root = baseURL.replace(/\/v1\/?$/, "").replace(/\/$/, "");
    ...
    const list = Array.isArray(body)
      ...
      : Array.isArray((body as { connections?: unknown[] })?.connections)
        ...
        : Array.isArray((body as { data?: unknown[] })?.data)
          ? (body as { data: unknown[] }).data
          : [];
    const out: DardcorCodeProviderConnection[] = [];
    for (const raw of list) {
      if (!raw || typeof raw !== "object") continue;
 ...
 * (`claude/sonnet-4`) depending on the DardcorCode deployment's `/v1/models`
 * surface shape.
 *
...
export function usableProviderAliasSet(
  connections: DardcorCodeProviderConnection[],
  enrichment: DardcorCodeEnrichmentMap | undefined
): {
  aliases: Set<string>;
  ...
  if (enrichment) {
    ...
    // populated by `defaultDardcorCodeEnrichmentFetcher`. Every entry carries
...
export function isUsableCombo(
  ...
  // The provider id is folded INTO the full model string by DardcorCode's
 ...
 * (e.g. `dardcorCode/MASTER`, `dardcorCode/MASTER-LIGHT`), guaranteeing uniqueness
 ...
 *
 * NOTE: the key MUST carry the OWNING provider prefix (`dardcorCode/…`), never a
 ...
 * "Unable to determine provider", whereas `dardcorCode/MASTER` resolves provider=
 * `dardcorCode` and the openai-compatible adapter strips the prefix and sends the
 ...
 * OC user may register prod + preprod DardcorCode side-by-side with distinct
 ...
 *     DardcorCodePlugin instance — sharing the cache means /v1/models +
 *     /api/combos each hit the gateway exactly ONCE per TTL refresh, not
 *     twice.
 */
export interface DardcorCodeFetchCacheEntry {
  rawModels: dardcorCodeRawModelEntry[];
  rawCombos: dardcorCodeRawCombo[];
  rawAutoCombos: dardcorCodeRawAutoCombo[];
  /** Display-name + pricing overlay from /api/pricing/models. Empty Map when feature is disabled or fetch failed. */
  rawEnrichment: DardcorCodeEnrichmentMap;
  /** Compression combos from /api/context/combos. Empty array when feature is disabled or fetch failed. */
  rawCompressionCombos: DardcorCodeCompressionCombo[];
  /** Provider connections from /api/providers. Empty array when feature is disabled or fetch failed. */
  rawConnections: DardcorCodeProviderConnection[];
  expiresAt: number;
}

export type DardcorCodeFetchCache = Map<string, DardcorCodeFetchCacheEntry>;
 ...
 *   - **Combo / model ID collisions: combos win.** DardcorCode treats combos
 ...
 *     `/api/combos` requires a management-scoped key and DardcorCode may
 ...
 *             outside tests today).
 */
export function createDardcorCodeProviderHook(
  opts?: DardcorCodePluginOptions,
  deps: {
    fetcher?: DardcorCodeModelsFetcher;
    combosFetcher?: DardcorCodeCombosFetcher;
    autoCombosFetcher?: DardcorCodeAutoCombosFetcher;
  ...
  const resolved = resolveDardcorCodePluginOptions(opts);
  const fetcher = deps.fetcher ?? defaultDardcorCodeModelsFetcher;
  ...
  const combosFetcher = deps.combosFetcher ?? defaultDardcorCodeCombosFetcher;
  const autoCombosFetcher = deps.autoCombosFetcher ?? defaultDardcorCodeAutoCombosFetcher;
  const enrichmentFetcher = deps.enrichmentFetcher ?? defaultDardcorCodeEnrichmentFetcher;
  const compressionMetaFetcher =
    deps.compressionMetaFetcher ?? defaultDardcorCodeCompressionMetaFetcher;
  const providersFetcher = deps.providersFetcher ?? defaultDardcorCodeProvidersFetcher;
  ...
  const cache: DardcorCodeFetchCache = deps.cache ?? new Map();
  ...
  return {
    ...
    async models(_provider, ctx) {
      ...
      if (!baseURL) {
        console.warn(
          `[dardcorCode-plugin] provider.models(${resolved.providerId}): ` +
      ...
      let rawModels: dardcorCodeRawModelEntry[];
      let rawCombos: dardcorCodeRawCombo[];
      let rawAutoCombos: dardcorCodeRawAutoCombo[];
      let rawEnrichment: DardcorCodeEnrichmentMap;
      let rawCompressionCombos: DardcorCodeCompressionCombo[];
      let rawConnections: DardcorCodeProviderConnection[];
      if (cached && cached.expiresAt > t) {
        ...
        // management-scoped key and DardcorCode may not have any combos
        ...
        if (wantCombos) {
          try {
            ...
            console.warn(
              "[dardcorCode-plugin] combos fetch failed, falling back to models-only catalog",
              err
            );
          }
        }
        ...
        // may not exist yet on older DardcorCode versions).
        rawAutoCombos = [];
        if (wantAutoCombos) {
          try {
        ...
        if (wantEnrichment) {
          try {
            ...
            console.warn(
              "[dardcorCode-plugin] enrichment fetch failed, falling back to raw ids",
              err
            );
          }
        }
        ...
        if (wantCompressionMeta) {
          try {
            ...
            console.warn("[dardcorCode-plugin] compression-metadata fetch failed", err);
          }
        }
        ...
        if (wantUsableOnly) {
          try {
            ...
            console.warn(
              "[dardcorCode-plugin] /api/providers fetch failed; usableOnly filter disabled for this refresh",
              err
            );
          }
        }
        ...
        // the dynamic pipeline fired and how much catalog DardcorCode returned.
        // Emitted once per cache miss (TTL refresh) — quiet on cache hits.
        console.warn(
          `[dardcorCode-plugin] catalog refreshed for providerId=${resolved.providerId} baseURL=${baseURL}: ` +
      ...
      const rawModelById = new Map<string, dardcorCodeRawModelEntry>();
      ...
      for (const entry of rawModels) {
        ...
        const model = mapRawModelToModelV2(entry, {
          ...
          providerId: resolved.dardcorCodeProviderId,
          baseURL,
        ...
        // prefixed id on `model.id` (e.g. `dardcorCode/claude-primary`), so we
      ...
      // compression metadata is present). DardcorCode returns at most one
      ...
      for (let pass = 0; pass < MAX_COMBO_PASSES && pending.length > 0; pass++) {
        ...
        for (const combo of pending) {
          ...
          const memberEntries: dardcorCodeRawModelEntry[] = [];
          ...
          for (const step of memberSteps) {
            ...
            if (stepKind === "combo-ref") {
              ...
              } as unknown as dardcorCodeRawModelEntry);
              continue;
            }
          ...
          // Apply enrichment overlay to combos too (DardcorCode's
          ...
          const comboKey = buildComboKey(combo, usedComboKeys, resolved.dardcorCodeProviderId);
          ...
          if (Object.prototype.hasOwnProperty.call(models, comboKey)) {
            ...
            if (!isIntentionalDedup) {
              ...
              if (!collisionWarned.has(dedupeKey)) {
                ...
                console.warn(
                  `[dardcorCode-plugin] combo key "${comboKey}" collides with a model id; combo wins.`
                );
              }
            }
          }
      ...
      if (pending.length > 0) {
        console.warn(
          `[dardcorCode-plugin] ${pending.length} combo(s) could not resolve all nested combo-refs after ${MAX_COMBO_PASSES} passes; they will advertise context=0 to avoid over-claiming.`
        );
      }
...
// same-origin DardcorCode inference requests only
// ────────────────────────────────────────────────────────────────────────────
 ...
 * a secret bound to both the configured DardcorCode origin and these intended
 * inference endpoints, and MUST NOT leak elsewhere.
 *
 * Ported from Alph4d0g's `opencode-dardcorCode-auth@1.2.1` `createFetchInterceptor`
 ...
 *     any caller-supplied Bearer for the same DardcorCode provider.
 *
 ...
 */
export function createDardcorCodeFetchInterceptor(config: {
  apiKey: string;
  baseURL: string;
}): typeof fetch {
 ...
 * validator throws on them, breaking DardcorCode → Gemini chains transparently.
 *
 * Source: behavioural reverse-engineering from Alph4d0g's
 * opencode-dardcorCode-auth@1.2.1 (dist/src/plugin.js:517).
 */
 ...
 *              interceptor from `createDardcorCodeFetchInterceptor`).
 */
    ...
    try {
      ...
      if (typeof initBody === "string") {
        ...
        if (!geminiStreamingWarningEmitted) {
          ...
          console.warn(
            "[dardcorCode-plugin] sanitizeGemini: streaming Request body, skipping schema strip (Gemini may reject)"
          );
        }
        return inner(input, init);
      } else if (
...
// `@dardcorCode/opencode-provider/src/index.ts` (`createDardcorCodeProvider`,
 ...
 * `OpenCodeModelEntry` exported by `@dardcorCode/opencode-provider`. Stripped
 ...
 * when DardcorCode didn't surface a value, NOT emitted as `undefined` — the
 * resulting JSON must be diffable across DardcorCode deployments without
 * `undefined` noise.
 */
/** Modalities accepted by OC's static catalog reader (see `@opencode-ai/sdk`). */
export type DardcorCodeModalityKind = "text" | "audio" | "image" | "video" | "pdf";

const STATIC_MODALITY_VALUES: ReadonlySet<DardcorCodeModalityKind> = new Set([
  "text",
  "audio",
  "image",
  "video",
...
function normaliseModalities(raw: unknown): DardcorCodeModalityKind[] {
  if (!Array.isArray(raw)) return [];
  const out: DardcorCodeModalityKind[] = [];
  const seen = new Set<string>();
  for (const v of raw) {
    if (typeof v !== "string") continue;
    const lower = v.toLowerCase() as DardcorCodeModalityKind;
...
export interface DardcorCodeStaticModelEntry {
   ...
   * model KEY already carries the provider prefix (e.g. `dardcorCode/MASTER`),
  ...
  tool_call?: boolean;
  /**
   * Per-million-token cost. Maps from DardcorCode `/api/pricing` shape:
   ...
   * DardcorCode's `input_modalities` / `output_modalities` on `/v1/models`.
   ...
   */
  modalities?: {
    input: DardcorCodeModalityKind[];
    output: DardcorCodeModalityKind[];
  };
}

/**
 * Static `provider.<id>` block written to `input.provider` by the config hook.
 * Mirrors `OpenCodeProviderEntry` from `@dardcorCode/opencode-provider`.
 *
 *   - `npm` is always `"@ai-sdk/openai-compatible"` — DardcorCode exposes an
 ...
 */
export interface DardcorCodeStaticProviderEntry {
  npm: "@ai-sdk/openai-compatible";
  name: string;
  options: {
    baseURL: string;
  ...
  models: Record<string, DardcorCodeStaticModelEntry>;
}
 ...
 *
 * Mapping rules (per the sibling `createDardcorCodeProvider` output spec):
 *
...
export function buildStaticProviderEntry(
  rawModels: dardcorCodeRawModelEntry[],
  rawCombos: dardcorCodeRawCombo[],
  opts: ReturnType<typeof resolveDardcorCodePluginOptions>,
  baseURL: string,
  apiKey: string,
  enrichment?: DardcorCodeEnrichmentMap,
  compressionCombos?: DardcorCodeCompressionCombo[],
  connections?: DardcorCodeProviderConnection[],
  rawAutoCombos?: dardcorCodeRawAutoCombo[]
): DardcorCodeStaticProviderEntry {
  const models: Record<string, DardcorCodeStaticModelEntry> = {};
  ...
  // Build a name-set of every non-hidden combo from `/api/combos`. DardcorCode
  ...
  for (const raw of rawModels) {
    ...
    // model KEY (e.g. `dardcorCode/claude-opus-4`) is what OC uses to recover
    // (providerID, modelID) when the user selects a model.
    const entry: DardcorCodeStaticModelEntry = { name: displayName };
    ...
    // Modalities — emit when DardcorCode surfaced any. Without this field
    ...
    // DardcorCode field names to OC's static-schema field names.
    const pricing = enrichmentEntry?.pricing;
    if (pricing && (typeof pricing.input === "number" || typeof pricing.output === "number")) {
      const cost: NonNullable<DardcorCodeStaticModelEntry["cost"]> = {
  ...
  const resolvedStaticCombosByName = new Map<string, DardcorCodeStaticModelEntry>();
  ...
  for (let pass = 0; pass < MAX_STATIC_COMBO_PASSES && pendingStatic.length > 0; pass++) {
    ...
    for (const combo of pendingStatic) {
      ...
      // provider and the KEY prefix (`dardcorCode/<slug>`) is what OC parses.
      const entry: DardcorCodeStaticModelEntry = { name: displayName };
      ...
      if (hasMembers) {
        ...
        const intersect = (sets: Set<DardcorCodeModalityKind>[]): DardcorCodeModalityKind[] => {
      ...
      // `opts.providerId` reaches DardcorCode's server doubled
      // (`opencode-dardcorCode/opencode-dardcorCode/<slug>`), and `parseModel()`
      // resolves credentials for the nonexistent provider `opencode-dardcorCode`
      // instead of `dardcorCode`. See #7976.
      models[buildComboKey(combo, usedComboKeys, opts.dardcorCodeProviderId).split("/").pop()!] =
        entry;
  ...
  if (pendingStatic.length > 0) {
    console.warn(
      `[dardcorCode-plugin] ${pendingStatic.length} combo(s) in the static catalog could not resolve all nested combo-refs after ${MAX_STATIC_COMBO_PASSES} passes; they will be omitted.`
    );
  }
    ...
    for (const autoCombo of rawAutoCombos) {
      ...
      if (models[key]) {
        ...
        if (!reportedCollisions.has(key)) {
          ...
          console.warn(
            `[dardcorCode-plugin] auto combo key "${key}" collides with an existing model; auto combo wins.`
          );
        }
      }
      models[key] = entry;
 ...
 *
 * Exported as a dependency-injectable function on `createDardcorCodeConfigHook`
...
// `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/dardcorCode-<providerId>.json`.
...
interface DardcorCodeDiskSnapshot {
  v: 2;
  ...
  rawEnrichment: Array<[string, DardcorCodeEnrichmentEntry]>;
  rawCompressionCombos: DardcorCodeCompressionCombo[];
  rawConnections: DardcorCodeProviderConnection[];
...
export function diskSnapshotPath(providerId: string): string {
  ...
  return path.join(dir, "plugins", `dardcorCode-${providerId}.json`);
}
...
export type DardcorCodeDiskSnapshotWriter = (
  providerId: string,
  entry: Omit<DardcorCodeFetchCacheEntry, "expiresAt">,
  identityFingerprint: string
) => Promise<void>;

export type DardcorCodeDiskSnapshotReader = (
  providerId: string,
  identityFingerprint: string
) => Promise<Omit<DardcorCodeFetchCacheEntry, "expiresAt"> | undefined>;
...
export const defaultDiskSnapshotWriter: DardcorCodeDiskSnapshotWriter = async (
  providerId,
  entry,
  identityFingerprint
) => {
  try {
    ...
    const snapshot: DardcorCodeDiskSnapshot = {
      v: 2,
      identityFingerprint,
...
export const defaultDiskSnapshotReader: DardcorCodeDiskSnapshotReader = async (
  providerId,
  identityFingerprint
) => {
  try {
    ...
    const parsed = JSON.parse(body) as Partial<DardcorCodeDiskSnapshot>;
    if (
      !parsed ||
      parsed.v !== 2 ||
...
export const noopDiskSnapshotWriter: DardcorCodeDiskSnapshotWriter = async () => {};
...
function debugLogPath(providerId: string): string {
  return join(debugLogDir(), `dardcorCode-debug-${providerId}.jsonl`);
}

function debugStatePath(providerId: string): string {
  return join(debugLogDir(), `dardcorCode-debug-${providerId}.state.json`);
}
...
export function debugLogSetEnabled(providerId: string, enabled: boolean): void {
  try {
    ...
    console.warn(`[dardcorCode-plugin] debugLogSetEnabled failed: ${(err as Error).message}`);
  }
}
...
export function debugLogAppend(entry: DebugLogEntry): void {
  try {
    ...
    console.warn(`[dardcorCode-plugin] debugLogAppend failed: ${(err as Error).message}`);
  }
}
...
export function debugLogClear(providerId: string): void {
  try {
    ...
    console.warn(`[dardcorCode-plugin] debugLogClear failed: ${(err as Error).message}`);
  }
}
...
export const noopDiskSnapshotReader: DardcorCodeDiskSnapshotReader = async () => undefined;

export type dardcorCodeReadAuthJson = () => Promise<AuthJsonShape | undefined | null>;

export const defaultReadAuthJson: dardcorCodeReadAuthJson = async () => {
  const dir = process.env.OPENCODE_DATA_DIR ?? path.join(os.homedir(), ".local/share/opencode");
 ...
 *   - `fetcher`          — replaces `defaultDardcorCodeModelsFetcher`.
 *   - `combosFetcher`    — replaces `defaultDardcorCodeCombosFetcher`.
 *   - `now`              — clock for cache TTL (default `Date.now`).
 *   - `cache`            — shared fetch-result cache (see
 *                          `DardcorCodeFetchCache`). Pass the same Map the
 *                          provider hook owns to dedupe round-trips.
 *   - `logger`           — `{warn}` sink for breadcrumb capture in tests.
 *                          Defaults to `console`.
 */
export function createDardcorCodeConfigHook(
  opts?: DardcorCodePluginOptions,
  deps: {
    readAuthJson?: dardcorCodeReadAuthJson;
    fetcher?: DardcorCodeModelsFetcher;
    ...
    compressionMetaFetcher?: DardcorCodeCompressionMetaFetcher;
    providersFetcher?: DardcorCodeProvidersFetcher;
    diskSnapshotReader?: DardcorCodeDiskSnapshotReader;
    diskSnapshotWriter?: DardcorCodeDiskSnapshotWriter;
    now?: () => number;
    cache?: DardcorCodeFetchCache;
  ...
  return async (input: Config) => {
    ...
    if (existingProviders && existingProviders[resolved.providerId] !== undefined) {
      logger.warn(
        `[dardcorCode-plugin] config shim skipped: provider.${resolved.providerId} already set by user`
      );
      return;
    }
    ...
    if (authJson === null) {
      logger.warn("[dardcorCode-plugin] config shim: auth.json failed to parse; treating as missing");
      authJson = undefined;
    }

    // Try both prefixed (e.g. opencode-dardcorCode) and unprefixed (e.g. dardcorCode)
    // keys so a user who ran `/connect dardcorCode` before the auto-prefix fix
    ...
    if (!apiKey) {
      ...
      logger.warn(
        `[dardcorCode-plugin] config shim skipped: no apiKey for providerId=${resolved.providerId}`
      );
      return;
    }
    // Management-plane catalog reads may use a narrower read-only token.
    ...
    if (!baseURL) {
      logger.warn(
        `[dardcorCode-plugin] config shim skipped: no baseURL for providerId=${resolved.providerId}`
      );
      return;
    }
    ...
    if (cached && cached.expiresAt > t) {
      ...
      try {
        ...
        logger.warn(
          "[dardcorCode-plugin] config shim: /v1/models fetch failed; publishing stub provider entry",
          err
        );
        rawModels = [];
      ...
      try {
        ...
        logger.warn(
          "[dardcorCode-plugin] config shim: /api/combos fetch failed; publishing models-only static catalog",
          err
        );
      }
      ...
      if (wantEnrichment) {
        try {
          ...
          logger.warn(
            "[dardcorCode-plugin] config shim: /api/pricing/models fetch failed; publishing raw-id static catalog",
            err
          );
        }
      }
      ...
      if (wantCompressionMeta) {
        try {
          ...
          logger.warn(
            "[dardcorCode-plugin] config shim: /api/context/combos fetch failed; publishing combos without compression suffix",
            err
          );
        }
      }
      ...
      if (wantUsableOnly) {
        try {
          ...
          logger.warn(
            "[dardcorCode-plugin] config shim: /api/providers fetch failed; usableOnly filter disabled for this refresh",
            err
          );
        }
      }
      ...
      if (modelsFetchThrew && wantDiskCache) {
        ...
        if (snapshot && snapshot.rawModels.length > 0) {
          logger.warn(
            `[dardcorCode-plugin] config shim: /v1/models unreachable; using stale disk cache (${snapshot.rawModels.length} models)`
          );
    ...
    if (features.mcpAutoEmit === true) {
      ...
      if (!mcpKey) {
        logger.warn(
          `[dardcorCode-plugin] mcp auto-emit skipped: no Bearer token for providerId=${resolved.providerId}`
        );
      } else {
        ...
        if (inputWithMcp.mcp[resolved.providerId] !== undefined) {
          logger.warn(
            `[dardcorCode-plugin] mcp auto-emit skipped: mcp.${resolved.providerId} already set by user`
          );
        } else {
