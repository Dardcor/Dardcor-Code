...
Run Dardcor Code with **podman compose** on Linux, macOS, or Windows, or with
...
```bash
cd /path/to/Dardcor Code
podman build --target runner-base -t Dardcor Code:base .
# For web-cookie providers (gemini-web, claude-web, claude-turnstile):
podman build --target runner-web -t Dardcor Code:web .
# For CLI tool support:
podman build --target runner-cli -t Dardcor Code:cli .
```

### 2. Copy Quadlet files to the systemd directory

```bash
mkdir -p ~/.config/containers/systemd/Dardcor Code
cp contrib/podman/*.container ~/.config/containers/systemd/Dardcor Code/
cp contrib/podman/*.network ~/.config/containers/systemd/Dardcor Code/
cp contrib/podman/*.volume ~/.config/containers/systemd/Dardcor Code/
```

### 3. Mount the project .env for secrets

Edit `~/.config/containers/systemd/Dardcor Code/Dardcor Code.container` and
uncomment/replace the `EnvironmentFile` line with the absolute path to
your project `.env`:

```
EnvironmentFile=/home/USER/code/docker/Dardcor Code/.env
```
...
systemctl --user start Dardcor Code-redis
systemctl --user start Dardcor Code
```

### 5. Verify

```bash
systemctl --user status Dardcor Code
...
```bash
journalctl --user -u Dardcor Code -f
```
...
The application profiles use local image names such as `Dardcor Code:base`; those
...
```bash
podman volume create Dardcor Code-data
podman run -d --name Dardcor Code \
  --env-file .env \
  -e DATA_DIR=/app/data \
  -p 20128:20128 \
  -v Dardcor Code-data:/app/data \
  docker.io/diegosouzapw/Dardcor Code:latest
```
