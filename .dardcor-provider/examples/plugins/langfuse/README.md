...
Emits [Langfuse](https://langfuse.com/) generation traces for every LLM completion routed through Dardcor Code.
...
Copy the `examples/plugins/langfuse/` directory to your Dardcor Code plugins path, or install directly from the marketplace UI.
...
- **Trace:** `Dardcor Code:<model>` with `userId`, `provider`, `requestId` metadata
...
- API keys are stored in the plugin config, encrypted at rest by Dardcor Code
...
- Dardcor Code plugin SDK: `docs/frameworks/PLUGIN_SDK.md`
