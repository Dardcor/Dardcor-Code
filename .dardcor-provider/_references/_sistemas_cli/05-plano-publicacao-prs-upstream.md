# Plano de publicacao de integracoes Dardcor Code nos repositorios upstream
...
**Documentos-base:** `01-relatorio-pesquisa-clis-Dardcor Code.md`,
...
1. **PR upstream de integracao nominal:** adiciona provider/preset `Dardcor Code`, configuracao,
...
2. **PR upstream de compatibilidade generica:** melhora suporte a endpoint customizado sem acoplar
   o projeto ao nome Dardcor Code, acompanhado de documentacao comprovando o uso com Dardcor Code.
3. **PR somente de documentacao:** registra uma configuracao funcional quando o codigo ja suporta
   Dardcor Code e o upstream aceita guias de terceiros.
...
5. **Configuracao sem PR:** documenta no Dardcor Code um fluxo que ja funciona e para o qual uma mudanca
...
- Procurar issues e PRs abertas/fechadas sobre custom provider, base URL, OpenAI-compatible,
  Anthropic-compatible, Gemini endpoint, proxy, gateway e Dardcor Code antes de propor uma mudanca.
...
```md
# CLI-NNN - <projeto> - integracao Dardcor Code upstream

- Repositorio canonico: <URL>
- Prioridade/lote: <P0-P4 / lote>
- Estado no catalogo Dardcor Code: <integrated/not-in-catalog/parcial>
...
10. Ja existe issue, PR, discussao ou documentacao para endpoints customizados ou Dardcor Code?
...
|---|---|---|
| `pr-provider` | O upstream possui catalogo/presets de providers | Provider/preset Dardcor Code, docs e testes |
| `pr-generic` | Falta uma capacidade generica necessaria, como base URL customizavel | Patch generico, docs e teste com Dardcor Code |
...
| `config-only` | Tudo funciona por configuracao e um PR seria redundante | Guia no Dardcor Code e smoke test |
...
- Pesquisar issues/PRs por termos de compatibilidade e pelo nome Dardcor Code.
...
- identificador e nome de exibicao `Dardcor Code`, se presets nomeados forem aceitos;
...
### Fase PR-5 - Validar contra Dardcor Code
...
- `feat/Dardcor Code-provider` para provider/preset nominal;
- `feat/custom-base-url` para capacidade generica;
- `docs/Dardcor Code-setup` para documentacao validada;
...
- `<sanitized Dardcor Code smoke test and result>`
...
upstream: `Add Dardcor Code provider preset`, `Support configurable OpenAI-compatible base URLs` ou
`Document Dardcor Code as a custom endpoint`.
...
- manter o guia/catalogo Dardcor Code coerente com o estado real do upstream.
...
- status no catalogo Dardcor Code;
...
- `CLI-005` Trae Agent: confirmar `model_providers` e propor entrada Dardcor Code/documentacao.
...
- `CLI-067` nanobot: `config-only` pelo provider dinâmico OpenAI-compatible. A base correta inclui
  `/api/v1`; `Dardcor Code/auto` seleciona o provider custom e envia `auto` no wire. Chat, SSE, tools,
...
- `CLI-069` NanoClaw: `config-only` pelo provider Claude existente, apontando a raiz Anthropic do
  Dardcor Code sem `/v1/messages` e usando OneCLI para a credencial. Codex e OpenCode têm bloqueios
...
- `CLI-073` Moltis: `config-only`, provider `custom-Dardcor Code`, `/api/v1`, `auto`, Chat/SSE/tools e capacidades multimodais. 401 testes e fmt passaram. Sem publicação.
- `CLI-074` GitClaw/GitAgent: `config-only`, loader OpenAI-compatible com `GITAGENT_MODEL_BASE_URL`, `OPENAI_API_KEY` e `Dardcor Code:auto`. Build, 65 testes e smoke passaram. Sem publicação.
