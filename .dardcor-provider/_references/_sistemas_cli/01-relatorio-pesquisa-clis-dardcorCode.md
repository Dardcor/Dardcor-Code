# Relatorio de pesquisa: repositorios de CLI integraveis com Dardcor Code
...
O Dardcor Code ja possui uma integracao funcional com o jcode e um catalogo local de ferramentas CLI. O proximo ganho de maior valor e transformar o Dardcor Code em um endpoint reconhecido pelos principais agentes de terminal, priorizando configuracao nativa e PR upstream quando o projeto aceitar contribuicoes.
...
Conclusao: devemos pesquisar e tentar todos os candidatos tecnicamente viaveis, mas separar claramente `suporte no catalogo Dardcor Code`, `configuracao generica`, `adaptacao upstream publicada` e `PR/issue aceita`. O tracker acompanha essas dimensoes separadamente.
...
6. Classificacao por adocao, manutencao, licenca, evidencia de endpoint, maturidade, potencial de PR e utilidade para o ecossistema Dardcor Code.
...
## 3. Baseline do Dardcor Code

### 3.1 Superficie que o Dardcor Code oferece
...
- Branch: `feat/Dardcor Code-provider`
- Commit: `ee4f904e6`
...
- Pendencia prometida: adicionar no README do Dardcor Code a secao "Tools & repositories that work with Dardcor Code".

Licao: o trabalho deve comecar descobrindo o mecanismo real de providers do upstream. Nem todos os clientes precisam de mudanca no Dardcor Code; alguns precisam somente de um perfil local, e outros exigirao um adaptador especifico.
...
| Crush | `charmbracelet/crush` | `base_url`, provider compativel | ja catalogado no Dardcor Code; validar upstream |
...
Estes podem consumir Dardcor Code como backend, mas a task deve confirmar se a interface de configuracao e realmente uma CLI de codigo ou apenas um gateway de agente.
...
Nao sao todos alvos de um provider Dardcor Code. Devem ser avaliados para launcher, ACP, MCP, observabilidade ou configuracao de seus agentes filhos.
...
## 6. Evidencias tecnicas e mapeamento para Dardcor Code

### 6.1 Padroes de endpoint encontrados

| Padrao observado | Exemplos | Acao Dardcor Code |
|---|---|---|
...
| ACP/MCP sem URL direta | Agentty, Kimi CLI, Goose, OpenCode | avaliar se Dardcor Code deve ser provider ou backend ACP |
...
- **Anthropic:** clientes que mandam `/v1/messages` esperam `ANTHROPIC_BASE_URL` sem `/v1` no valor. A traducao Anthropic do Dardcor Code deve ser validada com streaming e tool use.
...
7. **Mudancas no checkout:** usar worktree em `.claude/worktrees/` por projeto; nao editar o checkout compartilhado do Dardcor Code nem usar `git stash`.
...
- Dardcor Code CLI catalogo: `src/shared/constants/cliTools.ts`
- Dardcor Code CLI reference: `docs/reference/CLI-TOOLS.md`
- Dardcor Code setup guide: `docs/guides/CLI-INTEGRATIONS.md`
