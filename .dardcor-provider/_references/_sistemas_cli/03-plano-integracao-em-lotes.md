...
**Data:** 2026-08-01  
**Objetivo:** pesquisar, integrar, validar e publicar suporte ao Dardcor Code em todos os projetos tecnicamente possiveis, mantendo uma fila que permite ate tres subagentes simultaneos.
...
Criar uma task com nome do projeto, URL canonica, prioridade, evidencia inicial, estado no catalogo Dardcor Code e objetivo de integrar. Definir a worktree e o agente responsavel.
...
- perfil/preset `Dardcor Code` ou provider custom;
...
- smoke request com Dardcor Code;
...
### Fase 7 - Catalogo e integracao Dardcor Code

Quando houver valor para usuarios Dardcor Code:

- criar worktree propria do Dardcor Code;
...
Registrar commit, branch, PR/issue, testes, limitacoes, status do upstream, status do catalogo Dardcor Code e proximo passo. O agente principal faz uma revisao final de seguranca, licenca e factualidade.
...
- `CLI-000` - jcode - manter a issue #704, validar resposta do mantenedor e concluir a secao do README Dardcor Code.

### Lote P0.1

- `CLI-001` - Gemini CLI - integrar provider/base URL Gemini.
- `CLI-002` - Claw Code - integrar `OPENAI_BASE_URL`/provider Dardcor Code.
...
6. Entrada no catalogo Dardcor Code quando aplicavel.
