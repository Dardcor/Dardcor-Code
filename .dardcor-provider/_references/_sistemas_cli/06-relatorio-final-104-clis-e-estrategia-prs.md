# Relatório final — campanha de 104 integrações CLI Dardcor Code
...
- estado no catálogo Dardcor Code;
...
7. Consolidação de cada lote com commit separado no Dardcor Code e no repositório `_tasks`.
...
5. Implementar o menor patch genérico possível — preferir `openai-compatible`, `base_url` ou provider abstrato a um provider nominal Dardcor Code.
6. Executar GREEN: testes focados, suite upstream, lint, format, typecheck/build e smoke com fake server ou Dardcor Code local usando placeholder.
...
- eventual documentação externa/local do Dardcor Code;
- issue somente se houver pedido de documentação ou descoberta de bug real.

Não criar provider nominal ou PR apenas para adicionar a palavra “Dardcor Code”.
...
- MCP remoto/stdio do Dardcor Code;
...
Nunca mascarar Dardcor Code como Claude/Codex, falsificar executável, interceptar TLS ou reutilizar tokens privados para fabricar uma PR upstream.
