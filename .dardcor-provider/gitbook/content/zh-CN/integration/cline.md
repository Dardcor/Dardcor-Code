...
将 Dardcor Code 与 Cline VSCode 扩展集成,通过 Dardcor Code 的智能路由系统转发你的 AI 请求。
...
将 base URL 设为你的 Dardcor Code endpoint:
...
**步骤:**
1. 在 **Base URL** 字段中输入你的 Dardcor Code endpoint
2. 末尾必须包含 `/v1`
...
3. key 应以 `sk-dardcor-code-` 开头

### 5. 选择模型

1. 在 **Model** 下拉菜单中,可以:
   - 从可用模型中选择(若 Cline 自动检测)
   - 或手动输入 Dardcor Code 配置中的模型名
...
2. 在聊天输入框中输入消息
3. 按 Enter 发送
4. Cline 会通过 Dardcor Code 处理你的请求

### 生成代码

1. 让 Cline 生成代码:"创建一个登录表单的 React 组件"
2. Cline 会通过 Dardcor Code 生成代码
3. 检查并接受生成的代码
...
1. 在编辑器中选中代码
2. 让 Cline:"解释这段代码"
3. 通过 Dardcor Code 获得 AI 驱动的解释

### 文件操作

1. 让 Cline 创建、修改或删除文件
2. Cline 会通过 Dardcor Code 理解上下文并进行修改
3. 在接受前检查变更
...
2. 确认 Dardcor Code 实例正在运行且健康
3. 重新加载 VSCode 窗口(Cmd/Ctrl + Shift + P → "Reload Window")
4. 检查 Dardcor Code 日志是否有错误
...
## 与 Dardcor Code 功能的集成

### 模型路由

Dardcor Code 会根据以下因素自动将请求路由到最佳提供商:
- 模型可用性
- 提供商健康状态
- 成本优化
- 负载均衡
...
某个提供商失败时,Dardcor Code 会自动回退到仪表盘中配置的备用提供商。

### 使用跟踪

通过 Dardcor Code 仪表盘监控你的 Cline 使用:
- 请求总数
- Token 使用
- 每个模型的成本
- 提供商分布
