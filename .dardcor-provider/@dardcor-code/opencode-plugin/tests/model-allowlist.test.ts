...
function combo(models: dardcorCodeRawCombo["models"]): dardcorCodeRawCombo {
...
test("parseDardcorCodePluginOptions: visibleModels string[] → preserved", () => {
...
test("parseDardcorCodePluginOptions: hiddenModels string[] → preserved", () => {
  const r = parseDardcorCodePluginOptions({
...
test("parseDardcorCodePluginOptions: both lists together → preserved", () => {
  const r = parseDardcorCodePluginOptions({
    features: {
...
test("parseDardcorCodePluginOptions: empty string in visibleModels → rejects", () => {
  assert.throws(
    () =>
      parseDardcorCodePluginOptions({
...
test("parseDardcorCodePluginOptions: empty string in hiddenModels → rejects", () => {
  assert.throws(
    () =>
      parseDardcorCodePluginOptions({
...
test("parseDardcorCodePluginOptions: unknown features key still rejects (strict invariant)", () => {
  assert.throws(
    () =>
      parseDardcorCodePluginOptions({
...
const FAKE_RAW_MODELS: dardcorCodeRawModelEntry[] = [
...
test("buildStaticProviderEntry: no allowlist → all models emitted", () => {
  const opts = resolveDardcorCodePluginOptions({ features: {} });
...
test("buildStaticProviderEntry: visibleModels filters to only listed IDs", () => {
  const opts = resolveDardcorCodePluginOptions({
