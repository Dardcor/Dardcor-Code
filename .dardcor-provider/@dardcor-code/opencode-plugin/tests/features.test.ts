 ...
 *
 * Surfaces tested:
 *   - `parseDardcorCodePluginOptions({ features: ... })`  → schema accept/reject
 *   - `applyEnrichment(model, entry)`                   → mutation semantics
 *   - `formatCompressionPipeline(steps)`                → display formatting
 *   - `createDardcorCodeProviderHook` with mocked
 *     `enrichmentFetcher` / `compressionMetaFetcher`    → overlay applied,
 *                                                         off-by-default
 *                                                         gating works.
 *   - `createDardcorCodeConfigHook` with `features.mcpAutoEmit:true`
 *                                                       → emits mcp entry
 *                                                       → falls back to
 *                                                         provider apiKey
 *                                                         when mcpToken
...
import {
  ...
  defaultDardcorCodeEnrichmentFetcher,
  defaultDardcorCodeCompressionMetaFetcher,
  ...
  type DardcorCodeCompressionCombo,
...
test("parseDardcorCodePluginOptions: empty features object → preserved", () => {
  const r = parseDardcorCodePluginOptions({ features: {} });
  assert.deepEqual(r, { features: {} });
});

test("parseDardcorCodePluginOptions: all boolean features set → preserved", () => {
  const r = parseDardcorCodePluginOptions({
    features: {
      combos: true,
      enrichment: true,
...
test("parseDardcorCodePluginOptions: mcpToken string → preserved", () => {
  const r = parseDardcorCodePluginOptions({
...
test("parseDardcorCodePluginOptions: unknown features key → throws (strict)", () => {
  assert.throws(
    () =>
      parseDardcorCodePluginOptions({
        features: { combos: true, unknown_field: "oops" },
      }),
    /Invalid @dardcorCode\/opencode-plugin options/
  );
});

test("parseDardcorCodePluginOptions: non-boolean for boolean feature → throws", () => {
  assert.throws(
    () =>
      parseDardcorCodePluginOptions({
...
test("parseDardcorCodePluginOptions: empty mcpToken → throws (min 1)", () => {
  assert.throws(
    () => parseDardcorCodePluginOptions({ features: { mcpToken: "" } }),
    /Invalid @dardcorCode\/opencode-plugin options/
  );
});
...
const SAMPLE_RAW: dardcorCodeRawModelEntry[] = [
  {
    id: "claude-sonnet-4-6",
    object: "model",
    created: 0,
...
test("provider hook: enrichment fetcher called when features.enrichment !== false", async () => {
  ...
  // #6859: dynamic-hook catalog keys use the unprefixed dardcorCodeProviderId.
  const m = out["dardcorCode/claude-sonnet-4-6"];
...
test("provider hook: enrichment fetcher NOT called when features.enrichment:false", async () => {
  ...
  assert.equal(
    out["dardcorCode/claude-sonnet-4-6"].name,
    "claude-sonnet-4-6",
    "raw id preserved"
  );
});
...
test("provider hook: compression metadata fetcher called when opted in", async () => {
  let called = 0;
  const compressionCombos: DardcorCodeCompressionCombo[] = [
    {
      id: "default-caveman",
  ...
  const combo = out["dardcorCode/claude-primary"];
...
const stubAuthJson = (apiKey: string) => async () => ({
  "opencode-dardcorCode": { type: "api" as const, key: apiKey },
});
...
test("config hook: MCP auto-emit OFF by default (no mcp entry)", async () => {
  ...
  assert.ok(input.provider?.["opencode-dardcorCode"], "provider block written");
...
test("config hook: features.mcpAutoEmit:true writes mcp entry with provider apiKey", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-dardcorCode"] as
...
test("config hook: features.mcpToken overrides provider apiKey in mcp Bearer", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-dardcorCode"] as { headers: Record<string, string> };
  assert.equal(
...
test("config hook: existing operator mcp.<providerId> wins (no overwrite)", async () => {
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {
    mcp: { "opencode-dardcorCode": { type: "custom-user-entry", url: "https://manual.example/mcp" } },
  };
  await hook(input as never);
  assert.deepEqual(
    input.mcp?.["opencode-dardcorCode"],
...
test("config hook: features.mcpAutoEmit:true with /v1 in baseURL → strips correctly", async () => {
  const hook = createDardcorCodeConfigHook(
    ...
    {
      readAuthJson: async () => ({
        "opencode-dardcorCode-preprod": { type: "api" as const, key: "sk-preprod" },
      }),
      fetcher: async () => SAMPLE_RAW,
      combosFetcher: async () => [],
      logger: { warn: () => {} },
  ...
  const input: { provider?: Record<string, unknown>; mcp?: Record<string, unknown> } = {};
  await hook(input as never);
  const entry = input.mcp?.["opencode-dardcorCode-preprod"] as { url: string };
  assert.equal(
    entry.url,
...
test("defaultDardcorCodeEnrichmentFetcher: empty baseURL → empty map", async () => {
  const m = await defaultDardcorCodeEnrichmentFetcher("", "sk", 100);
  assert.equal(m.size, 0);
});

test("defaultDardcorCodeEnrichmentFetcher: empty apiKey → empty map", async () => {
  const m = await defaultDardcorCodeEnrichmentFetcher("https://or.example.com", "", 100);
  assert.equal(m.size, 0);
});

test("defaultDardcorCodeCompressionMetaFetcher: empty baseURL → empty array", async () => {
  const arr = await defaultDardcorCodeCompressionMetaFetcher("", "sk", 100);
...
test("defaultDardcorCodeEnrichmentFetcher: merges names from /api/pricing/models and prices from /api/pricing", async () => {
  ...
  try {
    const map = await defaultDardcorCodeEnrichmentFetcher(
...
test("defaultDardcorCodeEnrichmentFetcher: name-only when pricing endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing/models")) {
  ...
  try {
    const map = await defaultDardcorCodeEnrichmentFetcher("https://or.example.com", "sk-test", 5_000);
    const opus = map.get("cc/claude-opus-4-7");
...
test("defaultDardcorCodeEnrichmentFetcher: pricing-only when catalog endpoint 5xxs", async () => {
  const origFetch = globalThis.fetch;
  globalThis.fetch = (async (input: unknown) => {
    const url = typeof input === "string" ? input : (input as { url: string }).url;
    if (url.endsWith("/api/pricing")) {
...
): DardcorCodeEnrichmentMap {
  const map: DardcorCodeEnrichmentMap = new Map();
...
test("canonicalDedupSet: drops canonical row when alias twin present", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: dardcorCodeRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as dardcorCodeRawModelEntry,
    { id: "claude/claude-opus-4-7" } as dardcorCodeRawModelEntry,
  ];
...
test("canonicalDedupSet: keeps standalone canonical row (no alias twin) — never hides a model", () => {
  ...
  const c2a = buildCanonicalToAliasMap(map);
  const raw: dardcorCodeRawModelEntry[] = [
    { id: "claude/claude-opus-99" } as dardcorCodeRawModelEntry, // canonical only — no `cc/claude-opus-99`
  ];
  const drop = canonicalDedupSet(raw, c2a);
...
test("canonicalDedupSet: multi-provider — drops all canonical twins where alias exists", () => {
  ...
  const raw: dardcorCodeRawModelEntry[] = [
    { id: "cc/claude-opus-4-7" } as dardcorCodeRawModelEntry,
    { id: "claude/claude-opus-4-7" } as dardcorCodeRawModelEntry,
    { id: "cx/gpt-5.5" } as dardcorCodeRawModelEntry,
    { id: "codex/gpt-5.5" } as dardcorCodeRawModelEntry,
    { id: "pol/openai-large" } as dardcorCodeRawModelEntry,
    { id: "pollinations/openai-large" } as dardcorCodeRawModelEntry,
  ];
  const drop = canonicalDedupSet(raw, c2a);
  assert.equal(drop.has("claude/claude-opus-4-7"), true);
