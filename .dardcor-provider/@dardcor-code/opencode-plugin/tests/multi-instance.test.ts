 ...
 *
 * Validates that two `DardcorCodePlugin(input, opts)` invocations with
 ...
 *
 *   "plugin": [
 *     ["@dardcorCode/opencode-plugin", {"providerId": "dardcorCode-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@dardcorCode/opencode-plugin", {"providerId": "dardcorCode-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
import { DardcorCodePlugin } from "../src/index.js";

const fakeInput = {} as Parameters<typeof DardcorCodePlugin>[0];

test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  const a = await DardcorCodePlugin(fakeInput, {
    providerId: "dardcorCode-prod",
    baseURL: "https://a.example/v1",
  });
  const b = await DardcorCodePlugin(fakeInput, {
    providerId: "dardcorCode-preprod",
    baseURL: "https://b.example/v1",
  });

  assert.equal(a.auth?.provider, "opencode-dardcorCode-prod");
  assert.equal(b.auth?.provider, "opencode-dardcorCode-preprod");
});
...
test("multi-instance: identical opts twice still yield independent objects", async () => {
  ...
  const first = await DardcorCodePlugin(fakeInput, { ...opts });
  const second = await DardcorCodePlugin(fakeInput, { ...opts });
...
test("multi-instance: invalid opts on one instance does not poison the other", async () => {
  ...
  await assert.rejects(
    () => DardcorCodePlugin(fakeInput, { providerId: "bad id!" } as never),
    /providerId/
  );
  const ok = await DardcorCodePlugin(fakeInput, {
