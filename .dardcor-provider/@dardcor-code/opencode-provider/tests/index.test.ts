...
import {
  buildDardcorCodeOpenCodeConfig,
  createDardcorCodeAgentBlock,
  createDardcorCodeComboConfig,
  createDardcorCodeMCPEntry,
  createDardcorCodeModesBlock,
  createDardcorCodeProvider,
  fetchLiveModels,
  listCombos,
...
test("createDardcorCodeProvider validates required fields", () => {
  assert.throws(
    () => createDardcorCodeProvider({ baseURL: "", apiKey: "x" } as never),
    /baseURL is required/
  );
  assert.throws(
    () => createDardcorCodeProvider({ baseURL: "http://x", apiKey: "" } as never),
    /apiKey is required/
  );
});

test("createDardcorCodeProvider produces the OpenCode-compatible shape", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
  });

  assert.equal(provider.npm, OMNIROUTE_PROVIDER_NPM);
  assert.equal(provider.name, "DardcorCode");
  assert.equal(provider.options.baseURL, "http://localhost:20128/v1");
  assert.equal(provider.options.apiKey, "sk_dardcorCode");
  assert.equal(typeof provider.models, "object");
});

test("createDardcorCodeProvider seeds the default model catalog", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
  });
...
test("createDardcorCodeProvider honours a custom models list and labels", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
...
test("createDardcorCodeProvider deduplicates and trims model ids", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
...
test("createDardcorCodeProvider honours displayName override", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
    displayName: "Local DardcorCode",
  });
  assert.equal(provider.name, "Local DardcorCode");
});

test("buildDardcorCodeOpenCodeConfig wraps the provider with the OpenCode schema", () => {
  const doc = buildDardcorCodeOpenCodeConfig({
    baseURL: "http://localhost:20128/v1",
    apiKey: "sk_dardcorCode",
  });

  assert.equal(doc.$schema, OPENCODE_CONFIG_SCHEMA);
  assert.equal(typeof doc.provider.dardcorCode, "object");
  assert.equal(doc.provider.dardcorCode.options.baseURL, "http://localhost:20128/v1");
});
...
test("buildDardcorCodeOpenCodeConfig emits model and small_model prefixed with provider key", () => {
  const doc = buildDardcorCodeOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
  ...
  assert.equal(doc.model, "dardcorCode/claude-sonnet-4-5-thinking");
  assert.equal(doc.small_model, "dardcorCode/gemini-3-flash");
});

test("buildDardcorCodeOpenCodeConfig omits model and small_model when not supplied", () => {
  const doc = buildDardcorCodeOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
  });
...
test("buildDardcorCodeOpenCodeConfig ignores blank model strings", () => {
  const doc = buildDardcorCodeOpenCodeConfig({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
    model: "   ",
...
test("mergeIntoExistingConfig preserves existing provider entries", () => {
  ...
  assert.ok("dardcorCode" in (result.provider as Record<string, unknown>));
  assert.deepEqual((result as Record<string, unknown>).keybinds, { submit: "enter" });
});

test("mergeIntoExistingConfig overwrites existing dardcorCode entry", () => {
  const existing = {
    provider: {
      dardcorCode: {
  ...
  const dardcorCode = (result.provider as Record<string, unknown>).dardcorCode as { name: string };
  assert.equal(dardcorCode.name, "NEW");
});
...
test("mergeIntoExistingConfig writes model and small_model when supplied", () => {
  ...
  assert.equal(result.model, "dardcorCode/claude-sonnet-4-5-thinking");
  assert.equal(result.small_model, "dardcorCode/gemini-3-flash");
});

test("mergeIntoExistingConfig does not add model keys when not supplied", () => {
  const result = mergeIntoExistingConfig(
    {},
    { baseURL: "http://localhost:20128", apiKey: "sk_dardcorCode" }
  );
...
test("createDardcorCodeMCPEntry defaults to tsx runtime", () => {
  const entry = createDardcorCodeMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_dardcorCode",
  });
  assert.equal(entry.command, "npx");
  assert.deepEqual(entry.args, ["tsx", "/path/to/server.ts"]);
  assert.equal(entry.env.OMNIROUTE_API_KEY, "sk_dardcorCode");
  assert.ok(!("OMNIROUTE_MCP_ENFORCE_SCOPES" in entry.env));
  assert.ok(!("OMNIROUTE_MANAGEMENT_API_KEY" in entry.env));
});

test("createDardcorCodeMCPEntry uses node runtime when specified", () => {
  const entry = createDardcorCodeMCPEntry({
...
test("createDardcorCodeMCPEntry sets management key and scopes when supplied", () => {
  const entry = createDardcorCodeMCPEntry({
    serverPath: "/path/to/server.ts",
    apiKey: "sk_dardcorCode",
...
test("createDardcorCodeMCPEntry rejects missing required fields", () => {
  assert.throws(
    () => createDardcorCodeMCPEntry({ serverPath: "", apiKey: "x" }),
    /serverPath is required/
  );
  assert.throws(
    () => createDardcorCodeMCPEntry({ serverPath: "/p", apiKey: "" }),
    /apiKey is required/
  );
});
...
test("createDardcorCodeComboConfig builds minimal payload", () => {
  const payload = createDardcorCodeComboConfig({ name: "my-combo", strategy: "priority" });
...
test("createDardcorCodeComboConfig includes optional fields when supplied", () => {
...
test("createDardcorCodeProvider emits limit.context on default model entries", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
  });
...
test("createDardcorCodeProvider omits limit.context for unknown model ids", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
...
test("createDardcorCodeProvider reads contextLength from a live model entry for ids absent from the static map", () => {
...
test("createDardcorCodeProvider: a live model contextLength wins over the static default map", () => {
...
test("createDardcorCodeProvider serialises limit.context to JSON", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
  });
...
test("createDardcorCodeProvider emits default capability flags inline with the model entry", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
  });
...
test("createDardcorCodeProvider modelCapabilities overrides defaults and merges per id", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
    modelCapabilities: {
...
test("createDardcorCodeProvider applies capability overrides to non-default model ids", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
    models: ["custom-model"],
...
test("createDardcorCodeProvider modelLabels still works when modelCapabilities omits label", () => {
  const provider = createDardcorCodeProvider({
    baseURL: "http://localhost:20128",
    apiKey: "sk_dardcorCode",
...
test("createDardcorCodeAgentBlock builds provider-prefixed entries per role", () => {
  const block = createDardcorCodeAgentBlock({
    roles: {
  ...
  assert.equal(block.build.model, "dardcorCode/claude-sonnet-4-5-thinking");
  assert.equal(block.build.temperature, 0.2);
  assert.equal(block.plan.model, "dardcorCode/claude-opus-4-5-thinking");
  assert.equal(block.plan.top_p, 0.95);
  assert.equal(block.review.model, "dardcorCode/gemini-3-flash");
  assert.equal(block.review.temperature, 0.0);
});

test("createDardcorCodeAgentBlock omits optional fields when not supplied", () => {
  const block = createDardcorCodeAgentBlock({
...
test("createDardcorCodeAgentBlock skips roles with empty modelId", () => {
  const block = createDardcorCodeAgentBlock({
    roles: {
...
test("createDardcorCodeAgentBlock emits tools as Record<string, boolean> per OC schema", () => {
  const block = createDardcorCodeAgentBlock({
    roles: {
      build: {
...
test("createDardcorCodeAgentBlock filters invalid tool entries and omits empty maps", () => {
  const block = createDardcorCodeAgentBlock({
    roles: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
test("createDardcorCodeModesBlock builds provider-prefixed mode entries", () => {
  const block = createDardcorCodeModesBlock({
    modes: {
...
test("createDardcorCodeModesBlock skips modes with empty modelId", () => {
  const block = createDardcorCodeModesBlock({
    modes: {
      build: { modelId: "claude-sonnet-4-5-thinking" },
      plan: { modelId: "" },
...
test("createDardcorCodeModesBlock honours numeric overrides limited to OC schema", () => {
  const block = createDardcorCodeModesBlock({
    modes: {
      build: {
        modelId: "claude-sonnet-4-5-thinking",
...
// #3419 — soft-deprecation in favour of @dardcorCode/opencode-plugin. Guard the
...
test("package is marked deprecated in favour of @dardcorCode/opencode-plugin (#3419)", () => {
  const here = dirname(fileURLToPath(import.meta.url));
  const pkg = JSON.parse(readFileSync(join(here, "..", "package.json"), "utf8"));
  assert.match(pkg.description, /DEPRECATED/);
  assert.match(pkg.description, /@dardcorCode\/opencode-plugin/);

  const readme = readFileSync(join(here, "..", "README.md"), "utf8");
  assert.match(readme, /Deprecated/i);
  assert.match(readme, /@dardcorCode\/opencode-plugin/);
});
