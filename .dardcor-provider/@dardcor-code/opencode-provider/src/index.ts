/**
 * OpenCode provider plugin for DardcorCode AI Gateway.
 *
 * Generates an OpenCode-compatible provider object that points to a running
 * DardcorCode instance. The output follows the OpenCode config schema
 * (https://opencode.ai/config.json) and delegates the runtime to
 * `@ai-sdk/openai-compatible` so OpenCode can drive any DardcorCode-exposed
 ...
 *
 *     ```ts
 *     import { buildDardcorCodeOpenCodeConfig } from "@dardcorCode/opencode-provider";
 *     const config = buildDardcorCodeOpenCodeConfig({
 *       baseURL: "http://localhost:20128",
 *       apiKey: "sk_dardcorCode",
 *     });
 *     // config -> { $schema, provider: { dardcorCode: { npm, name, options, models } } }
 *     ```
 *
 *  2. As a single-provider entry to merge into an existing opencode.json:
 *
 *     ```ts
 *     import { createDardcorCodeProvider } from "@dardcorCode/opencode-provider";
 *     const provider = createDardcorCodeProvider({ baseURL, apiKey });
 *     // provider -> the value to place under provider.dardcorCode in opencode.json
 *     ```
 *
 ...
 *
 * Curated set covering the most commonly deployed DardcorCode models. Synced
 * with the Alph4d0g/opencode-dardcorCode-auth OMNIROUTE_DEFAULT_MODELS constant
 * (https://github.com/Alph4d0g/opencode-dardcorCode-auth, MIT) and extended
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-dardcorCode-auth
 * (https://github.com/Alph4d0g/opencode-dardcorCode-auth, MIT).
 */
 ...
 * Matches the context lengths used by DardcorCode's provider registry.
 */
 ...
 * model via `DardcorCodeProviderOptions.modelCapabilities`.
 */
...
export interface DardcorCodeProviderOptions {
  /** DardcorCode base URL, with or without trailing `/v1`. Required. */
  baseURL: string;
  /** DardcorCode API key. Required. Use `sk_dardcorCode` for local instances without REQUIRE_API_KEY. */
  apiKey: string;
  /** Override the display name shown in OpenCode. Default: `"DardcorCode"`. */
  displayName?: string;
   ...
   * Emitted as `"dardcorCode/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.dardcorCode.models[id]`. */
...
export interface OpenCodeProviderEntry {
  /** Identifier of the OpenCode runtime package that will speak to DardcorCode. */
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"dardcorCode/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"dardcorCode/gemini-3-flash"`. */
  small_model?: string;
  provider: {
...
function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@dardcorCode/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@dardcorCode/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@dardcorCode/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
/**
 * Build the `provider.dardcorCode` entry for an OpenCode config document.
 * The returned object is JSON-serialisable and safe to embed verbatim.
 */
export function createDardcorCodeProvider(options: DardcorCodeProviderOptions): OpenCodeProviderEntry {
  ...
  return {
    ...
    name: options.displayName?.trim() || "DardcorCode",
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.dardcorCode`).
 * Useful when scaffolding a fresh `opencode.json`.
 *
 * When `options.model` / `options.smallModel` are supplied they are emitted as
 * top-level `model` / `small_model` keys prefixed with `"dardcorCode/"` so
 * OpenCode resolves them through the configured provider.
 */
export function buildDardcorCodeOpenCodeConfig(
  options: DardcorCodeProviderOptions
): OpenCodeConfigDocument {
  const doc: OpenCodeConfigDocument = {
    $schema: OPENCODE_CONFIG_SCHEMA,
    provider: {
      [OMNIROUTE_PROVIDER_KEY]: createDardcorCodeProvider(options),
    },
  };
...
/**
 * Merge the DardcorCode provider entry (and optional `model` / `small_model`
 ...
 * present are not removed. If `existing.provider.dardcorCode` already exists it
...
export function mergeIntoExistingConfig(
  ...
  const partial = buildDardcorCodeOpenCodeConfig(options);
...
export type DardcorCodeMCPScope = (typeof OMNIROUTE_MCP_DEFAULT_SCOPES)[number] | string;

export interface DardcorCodeMCPOptions {
  /** Absolute path to the MCP server entry point (TypeScript or compiled JS). */
  serverPath: string;
  /** DardcorCode API key forwarded to the MCP server as `OMNIROUTE_API_KEY`. */
  apiKey: string;
  /**
...
/**
 * Build the `mcp.servers.dardcorCode` entry for an OpenCode config document.
 *
 * @example
 * ```ts
 * const mcpEntry = createDardcorCodeMCPEntry({
 *   serverPath: "/home/user/.local/share/dardcorCode/open-sse/mcp-server/server.ts",
 *   apiKey: "sk_dardcorCode",
 *   managementApiKey: "sk_manage_...",
 *   scopes: ["read:health", "read:combos", "execute:completions"],
 * });
 * // Place at config.mcp.servers.dardcorCode
 * ```
 */
export function createDardcorCodeMCPEntry(options: DardcorCodeMCPOptions): OpenCodeMCPServerEntry {
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@dardcorCode/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
 ...
 * The shape mirrors the subset of fields that DardcorCode's `/v1/models`
 * endpoint reliably provides across versions, normalised from both
 * camelCase and snake_case variants used by different DardcorCode releases.
 *
 * Attribution: field-variant normalisation logic adapted from
 * https://github.com/Alph4d0g/opencode-dardcorCode-auth (MIT).
 */
export interface DardcorCodeLiveModel {
  id: string;
  name: string;
...
/**
 * Fetch the live model catalog from a running DardcorCode instance.
 *
 ...
 * `display_name`) field variants across DardcorCode versions.
 *
 * Useful for dynamically populating the `models` option of
 * `createDardcorCodeProvider` / `buildDardcorCodeOpenCodeConfig` instead of
 * relying on `OMNIROUTE_DEFAULT_OPENCODE_MODELS`.
 *
 * @param baseURL   - DardcorCode base URL (with or without `/v1`).
 * @param apiKey    - DardcorCode API key.
 * @param timeoutMs - Request timeout in milliseconds (default 5000).
 *
 * @example
 * ```ts
 * const models = await fetchLiveModels("http://localhost:20128", "sk_dardcorCode");
 * const config = buildDardcorCodeOpenCodeConfig({
 *   baseURL: "http://localhost:20128",
 *   apiKey: "sk_dardcorCode",
 *   models,                    // DardcorCodeLiveModel[] — contextLength auto-extracted
...
export async function fetchLiveModels(
  ...
  const models: DardcorCodeLiveModel[] = [];
  for (const raw of rawList) {
    ...
    // Extract context_length from DardcorCode's /v1/models response.
    // DardcorCode returns context_length in snake_case for both synced
 ...
 */
export type DardcorCodeCompressionOverride =
  | ""
  | "off"
  | "lite"
  | "standard"
...
export interface DardcorCodeCombo {
  id: string;
  name: string;
  strategy: string;
  active: boolean;
  compressionOverride: DardcorCodeCompressionOverride;
}

/**
 * Fetch the active routing combo list from a running DardcorCode instance.
 *
...
export async function listCombos(
  ...
  for (const raw of rawList) {
    ...
    const compressionOverride = VALID_COMPRESSION_OVERRIDES.has(rawOverride)
      ? (rawOverride as DardcorCodeCompressionOverride)
      : "";
...
/**
 * Options for `createDardcorCodeComboConfig`.
 * Mirrors the subset of combo fields exposed by the DardcorCode `/api/combos`
 * PATCH / POST payload that are safe to set programmatically.
 */
export interface DardcorCodeComboConfigOptions {
...
/**
 * Build a typed combo payload suitable for DardcorCode's management API.
 *
 ...
 * @example
 * ```ts
 * const payload = createDardcorCodeComboConfig({
 *   name: "claude-primary",
 *   strategy: "priority",
 ...
 * ```
 */
export function createDardcorCodeComboConfig(
  options: DardcorCodeComboConfigOptions
): Record<string, unknown> {
 ...
 */
export interface dardcorCodeRoleOverrides {
...
/** Per-role binding used by `createDardcorCodeAgentBlock`. */
export interface DardcorCodeAgentRole extends dardcorCodeRoleOverrides {
  /** DardcorCode model id, e.g. `"claude-sonnet-4-5-thinking"`. */
  modelId: string;
...
/** Options for `createDardcorCodeAgentBlock`. */
export interface DardcorCodeAgentBlockOptions {
  /** Per-role bindings. Keys become entries under OpenCode's `agent` block. */
  roles: Record<string, DardcorCodeAgentRole>;
}

/** Single entry inside the emitted OpenCode `agent` block. */
export interface OpenCodeAgentEntry extends dardcorCodeRoleOverrides {
  /** Always emitted as `"dardcorCode/<modelId>"`. */
  model: string;
...
function buildAgentEntry(role: DardcorCodeAgentRole): OpenCodeAgentEntry | undefined {
 ...
 * specific DardcorCode model. Useful for `.opencode/agent/*.md` defaults and
 ...
 * @example
 * ```ts
 * const agentBlock = createDardcorCodeAgentBlock({
 *   roles: {
 ...
 *   },
 * });
 * // -> { build: { model: "dardcorCode/claude-sonnet-4-5-thinking", temperature: 0.2 }, ... }
 * ```
 */
export function createDardcorCodeAgentBlock(
  options: DardcorCodeAgentBlockOptions
...
/**
 * Per-mode binding used by `createDardcorCodeModesBlock`.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `DardcorCodeAgentRole` + `createDardcorCodeAgentBlock`. This
 * type and the corresponding helper are kept for back-compat with configs
 * still using `mode:`.
 */
export interface DardcorCodeMode extends DardcorCodeAgentRole {}

/**
 * Options for `createDardcorCodeModesBlock`.
 *
 * @deprecated See `DardcorCodeMode`.
 */
export interface DardcorCodeModesBlockOptions {
  /** Per-mode bindings. Keys become entries under OpenCode's deprecated top-level `mode` block. */
  modes: Record<string, DardcorCodeMode>;
}
 ...
 * a specific DardcorCode model. Emits the same shape as the `agent` block since
 * OpenCode's schema treats them identically (both reference `AgentConfig`).
 *
 * Modes with empty / missing `modelId` are skipped.
 *
 * @deprecated OpenCode's top-level `mode` block is deprecated in favour of
 * `agent`. Prefer `createDardcorCodeAgentBlock`. This helper is kept for
 * back-compat with configs still using `mode:`.
 *
 * @example
 * ```ts
 * const modesBlock = createDardcorCodeModesBlock({
 *   modes: {
 ...
 * ```
 */
export function createDardcorCodeModesBlock(
  options: DardcorCodeModesBlockOptions
...
export default createDardcorCodeProvider;
