...
function getSessionsHTML(host, port, cssModules, useMock) {
	...
	const additionalBuiltinExtensions = useMock
		? `additionalBuiltinExtensions: [{ scheme: 'http', authority: '${host}:${port}', path: '/src/dc/sessions/test/e2e/extensions/sessions-e2e-mock' }],`
		: '';
...
<body aria-label="">
	<script type="module">
		import { create, URI } from '${fileRoot}/dc/sessions/${useMock ? 'test/sessions.web.test.internal' : 'sessions.web.main.internal'}.js';
