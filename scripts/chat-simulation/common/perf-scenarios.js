...
const CONTENT_SCENARIOS = {
	...
	'giant-codeblock': {
		...
		chunks: (() => {
			...
			b.stream([
				'import { Disposable, DisposableStore } from "dc/base/common/lifecycle";\n',
				'import { Emitter, Event } from "dc/base/common/event";\n',
				'import { URI } from "dc/base/common/uri";\n\n',
			], 10);
			for (let i = 0; i < 40; i++) {
				b.stream([
	...
	'file-links': {
		...
		chunks: (() => {
			const files = [
				'src/dc/workbench/contrib/chat/browser/chatListRenderer.ts',
				'src/dc/workbench/contrib/chat/common/chatService/chatServiceImpl.ts',
				'src/dc/workbench/contrib/chat/browser/widget/input/chatInputPart.ts',
				'src/dc/workbench/contrib/chat/common/chatPerf.ts',
				'src/dc/base/common/lifecycle.ts',
				'src/dc/base/common/event.ts',
				'src/dc/platform/instantiation/common/instantiation.ts',
				'src/dc/workbench/services/extensions/common/abstractExtensionService.ts',
				'src/dc/workbench/api/common/extHostLanguageModels.ts',
				'src/dc/workbench/contrib/chat/common/languageModels.ts',
				'src/dc/editor/browser/widget/codeEditor/editor.ts',
				'src/dc/workbench/browser/parts/editor/editorGroupView.ts',
			];
			const b = new ScenarioBuilder();
			...
			b.wait(10, '\nAdditionally, the following files import from `dc/base/common/lifecycle`:\n\n');
			for (let i = 0; i < 20; i++) {
				...
				const file = `src/dc/${depth}/${area}/${name}${i}.ts`;
				b.stream([
					`- [${file}](${file}#L${i * 10 + 5})`,
				...
				turns: [
					...
					},
					{
						...
						chunks: new ScenarioBuilder()
							...
							.stream([
								'I read 8 core utility files from `src/dc/base/common/`. Here is a summary:\n\n',
								'### lifecycle.ts\n',
					...
					// Round 3: final content
					{
						...
						chunks: new ScenarioBuilder()
							...
							.stream([
								...
								'### Files read:\n',
								'1. `src/dc/base/common/lifecycle.ts` — Disposable pattern and lifecycle management\n',
								'2. `src/dc/base/common/event.ts` — Event emitter and observer pattern\n',
								'3. `src/dc/base/common/errors.ts` — Error handling utilities\n\n',
								'### Edits applied:\n',
