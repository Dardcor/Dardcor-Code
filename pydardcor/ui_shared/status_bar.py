...
class StatusBar(QStatusBar):
    ...
    def set_selection_count(self, chars: int, lines: int = 0):
        if chars <= 0:
            self._selection_btn.hide()
            return
        if lines > 1:
            self._selection_btn.setText(f"{lines} lines selected")
        else:
            self._selection_btn.setText(f"{chars} selected")
    ...
    def resizeEvent(self, event):
        ...
        self._selection_btn.setVisible(w > 900 and not self._selection_btn.isHidden())
