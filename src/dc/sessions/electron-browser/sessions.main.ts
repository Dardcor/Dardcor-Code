...
import { INativeManagedSettingsService, IFileManagedSettingsService, IManagedSettingsService, NullManagedSettingsService } from '../../platform/policy/common/copilotManagedSettings.js';
...
export class SessionsMain extends Disposable {
	...
	private async initServices(): Promise<{ serviceCollection: ServiceCollection; logService: ILogService; storageService: NativeWorkbenchStorageService; configurationService: ConfigurationService }> {
		...
		serviceCollection.set(IManagedSettingsService, new NullManagedSettingsService());
		if (policyChannel) {
