...
`dc/sessions` implements the Agents Window as a top-level layer above `dc/workbench`. This directory contains both the implementation and the durable design specifications that describe its boundaries.
...
- `dc/sessions` may import from `dc/workbench` and lower layers. `dc/workbench` must not import from `dc/sessions`.
