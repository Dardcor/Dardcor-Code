...
| `contributions/mobileDiffColors.ts` | Registers the `agentsMobileDiff.*` color tokens (icons, A/M/D pills, +N/−N counters, edge bars) used by the phone changes/diff overlays. Imported as a side-effect; needed because `dc/workbench/contrib/scm` (and its `gitDecoration.*` tokens) is not loaded in the agents window. |
