...
**Folder:** `src/dc/sessions/contrib/providers/agentHost/`
...
Agent host providers implement `IAgentHostSessionsProvider` (defined in sessions core at `src/dc/sessions/common/agentHostSessionsProvider.ts`), which extends `ISessionsProvider` with:
...
| `agentHostCodexApprovalsPicker.ts` | Codex-specific permissions-preset picker with Default Permissions, Auto-Review, and Full Access choices. Its bounded, wrapped action-list layout is shared with the editor composer through `dc/platform/agentHost/browser/codexApprovalsPicker.ts`. |
