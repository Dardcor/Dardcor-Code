...
export class SessionsChatAccessibilityHelp implements IAccessibleViewImplementation {
	...
	getProvider(accessor: ServicesAccessor) {
		...
		content.push(localize('sessionsChat.syncChanges', "When available, Sync Changes appears as a separate sync icon with incoming and outgoing commit counts beside the branch picker below the input. Use Tab and the arrow keys to reach it, then Enter or Space to synchronize the session's repository. The action is disabled while synchronization is running."));
		...
		content.push(localize('sessionsChat.dccodePet', "Use the checked Pet item in the new-session view context menu, or type /dccode-pet, to show or hide the dccode pet above the input. Drag it horizontally to reposition it, or use Tab to focus it and the left and right arrow keys to move it. Press Enter or Space to show it some love."));
		...
		content.push(localize('sessionsChat.navigateChatTabs', "When a chat tab is focused, use the left and right arrow keys to move to and activate an adjacent chat tab, or Home and End to activate the first and last tabs."));
