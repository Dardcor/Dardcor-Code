...
export class Dialog extends Disposable {
	...
	private getIconAriaLabel(): string {
		...
		switch (this.options.type) {
			case 'error': typeLabel = 'Error'; break;
			case 'warning': typeLabel = 'Warning'; break;
			case 'pending': typeLabel = 'In Progress'; break;
		}
		return typeLabel;
	}
	...
	async show(): Promise<IDialogResult> {
		...
		return new Promise<IDialogResult>(resolve => {
				...
				if (evt.equals(KeyCode.Tab) || evt.equals(KeyCode.RightArrow) || evt.equals(KeyMod.Shift | KeyCode.Tab) || evt.equals(KeyCode.LeftArrow)) {
					...
					if (this.messageContainer) {
						...
						for (const link of links) {
							focusableElements.push(link as unknown as { focus: () => void });
			...
			if (!this.options.disableCloseAction && !this.options.disableCloseButton) {
				...
				const action = this._register(new Action('dialog.close', 'Close Dialog', ThemeIcon.asClassName(Codicon.dialogClose), true, async () => {
					resolve({
						button: this.options.cancelId || 0,
						checkboxChecked: this.checkbox ? this.checkbox.checked : undefined
					});
	...
	private applyStyles() {
		...
		if (linkFgColor) {
			for (const el of [...this.messageContainer.getElementsByTagName('a'), ...this.footerContainer?.getElementsByTagName('a') ?? []]) {
				(el as HTMLElement).style.color = linkFgColor;
		...
		switch (this.options.type) {
			case 'none': break;
			case 'error': color = style.errorIconForeground; break;
			case 'warning': color = style.warningIconForeground; break;
			default: color = style.infoIconForeground; break;
		}
		if (color) {
