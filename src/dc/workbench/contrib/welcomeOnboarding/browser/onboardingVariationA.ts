...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
		...
		if (this.footerLeft) {
			if (this._isLastStep()) {
				...
				if (!this._footerSignInBtn && !this._userSignedIn) {
					...
					this._footerSignInBtn.textContent = localize('onboarding.sessions.signInNudge', "Sign in to use Dardcor Code");
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to Dardcor Code");

		const subtitle = append(contentMain, $('p.onboarding-a-signin-subtitle'));
		subtitle.textContent = localize('onboarding.signIn.heroSubtitle', "Sign in to use Dardcor Code.");
