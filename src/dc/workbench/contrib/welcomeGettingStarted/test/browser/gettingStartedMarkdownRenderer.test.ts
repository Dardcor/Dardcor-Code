...
suite('Getting Started Markdown Renderer', () => {
	...
	test('renders theme picker markdown with images', async () => {
		...
		const mdPath = convertInternalMediaPathToFileURI('theme_picker').with({ query: JSON.stringify({ moduleId: 'dc/workbench/contrib/welcomeGettingStarted/common/media/theme_picker' }) });
		const mdBase = FileAccess.asFileUri('dc/workbench/contrib/welcomeGettingStarted/common/media/');
		...
		for (const src of imageSrcs) {
			const targetSrcFormat = /^img src=".*\/dc\/workbench\/contrib\/welcomeGettingStarted\/common\/media\/.*.png"$/;
