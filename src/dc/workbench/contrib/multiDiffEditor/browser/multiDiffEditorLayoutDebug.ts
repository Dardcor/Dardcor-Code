...
export function createMultiDiffEditorLayoutDebugModel(state: IObservable<IMultiDiffEditorLayoutDebugState>, modelService: IModelService, languageService: ILanguageService): ITextModel {
    let previousState = state.get();
    const id = generateUuid();
    const logModel = modelService.createModel(`${JSON.stringify({ timestamp: Date.now(), set: previousState })}\n`, languageService.createById('jsonl'), URI.from({ scheme: Schemas.inMemory, path: `/multi-diff-editor-layout-${id}.jsonl` }));
...
function isJsonObject(value: JsonValue): value is {
