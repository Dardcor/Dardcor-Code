...
export class ScmHistoryItemResolver implements IMultiDiffSourceResolver {
    ...
    async resolveDiffSource(uri: URI): Promise<IResolvedMultiDiffSource> {
        ...
        const repository = await waitForState(observableFromEvent(this, this._scmService.onDidAddRepository, () => this._scmService.getRepository(repositoryId)));
        ...
        const resources = ValueWithChangeEvent.const<readonly MultiDiffEditorItem[]>(historyItemChanges.map(change => {
...
class ScmResolvedMultiDiffSource implements IResolvedMultiDiffSource {
    ...
    constructor(private readonly _group: ISCMResourceGroup, private readonly _repository: ISCMRepository) {
        this._resources = observableFromEvent<MultiDiffEditorItem[]>(this._group.onDidChangeResources, () => this._group.resources.map(e => new MultiDiffEditorItem(e.multiDiffEditorOriginalUri, e.multiDiffEditorModifiedUri, e.sourceUri)));
