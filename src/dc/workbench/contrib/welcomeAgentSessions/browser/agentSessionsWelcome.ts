...
import { ChatViewId, IChatWidgetService, ISessionTypePickerDelegate } from '../../chat/browser/chat.js';
...
export class AgentSessionsWelcomePage extends EditorPane {
	...
	private async buildStartEntries(container: HTMLElement): Promise<void> {
		const openEntry = { icon: Codicon.folderOpened, label: localize('openFolder', "Open Folder..."), command: 'workbench.action.files.openFolder' };
		const entries = [
			openEntry,
	...
	private buildPrivacyNotice(container: HTMLElement): void {
		...
		const title = append(content, $('.agentSessionsWelcome-walkthroughCard-title'));
		title.textContent = localize('tosTitle', "Try Dardcor Code for free, no sign-in required!");
