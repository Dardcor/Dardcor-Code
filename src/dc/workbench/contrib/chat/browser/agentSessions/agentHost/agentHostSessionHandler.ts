...
/** Whether `err` reports that the host has no such resource (AHP `NotFound` or deleted/missing session). */
export function isNotFoundError(err: unknown): boolean {
	if (err instanceof ProtocolError && (err.code === AHP_NOT_FOUND || err.code === AHP_SESSION_NOT_FOUND)) {
		return true;
	}
	const message = err instanceof Error ? err.message : (typeof err === 'string' ? err : undefined);
	if (message && /explicitly deleted|session not found|not found|NotFound|ENOENT/i.test(message)) {
		return true;
	}
	return false;
}
...
export function unwrapSessionLoadErrorMessage(err: unknown): string | undefined {
	...
	if (/explicitly deleted|session not found/i.test(message)) {
		return undefined;
	}
