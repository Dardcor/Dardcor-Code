...
class QuickChat extends Disposable {
	...
	private setupDisclaimer(parent: HTMLElement): void {
		...
		this._register(autorun(reader => {
			...
			if (showDisclaimer) {
				const renderedMarkdown = disposables.add(this.markdownRendererService.render(new MarkdownString(localize({ key: 'termsDisclaimer', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0} Dardcor AI, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", product.defaultChatAgent?.provider?.default?.name ?? '', product.defaultChatAgent?.provider?.default?.name ?? '', product.defaultChatAgent?.termsStatementUrl ?? '', product.defaultChatAgent?.privacyStatementUrl ?? ''), { isTrusted: true })));
	...
	async openChatView(): Promise<void> {
		...
		if (value) {
			widget.viewModel.model.inputModel.setState(value as any);
		}
		widget.focusInput();
	}
