...
export async function clearChatEditor(accessor: ServicesAccessor, chatEditorInput?: ChatEditorInput, resolvedSessionType?: IResolvedNewChatSessionType | string): Promise<void> {
	...
	if (chatEditorInput instanceof ChatEditorInput) {
		...
		const resolved = typeof resolvedSessionType === 'string'
			? { sessionType: resolvedSessionType, selectionReason: 'explicitOverride' as const }
			: resolvedSessionType ?? getDefaultNewChatSessionTypeAndReason(accessor, {
				currentSessionType,
			});
