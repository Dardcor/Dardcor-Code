...
export class ChatStatusDashboard extends DomWidget {
	...
	private render(): void {
		...
		if (hasUsageSection && !this.options?.compactQuotaLayout) {
			...
			const header = this.renderHeader(headerHost, this._store, planName, toAction({
				...
				label: localize('quotaLabel', "Manage Dardcor AI Settings"),
				tooltip: localize('quotaTooltip', "Manage Dardcor AI Settings"),
	...
	private renderSetupSection(): void {
		...
		if (newUser && anonymousUser) {
			descriptionText = new MarkdownString(localize({ key: 'activeDescriptionAnonymous', comment: ['{Locked="]({2})"}', '{Locked="]({3})"}'] }, "By continuing with {0} Dardcor AI, you agree to {1}'s [Terms]({2}) and [Privacy Statement]({3})", defaultChat.provider.default.name, defaultChat.provider.default.name, defaultChat.termsStatementUrl, defaultChat.privacyStatementUrl), { isTrusted: true });
			descriptionClass = `${descriptionClass}.terms`;
		} else if (newUser) {
			descriptionText = localize('activateDescription', "Set up Dardcor AI to use AI features.");
		} else if (anonymousUser) {
			descriptionText = localize('enableMoreDescription', "Sign in to enable more Dardcor AI features.");
		} else if (disabled) {
			descriptionText = localize('enableDescription', "Enable Dardcor AI to use AI features.");
		} else {
			descriptionText = localize('signInDescription', "Sign in to use Dardcor AI features.");
		}
		...
		if (newUser) {
			...
			buttonLabel = localize('signInToUseAIFeatures', "Sign in to use Dardcor AI");
		}
	...
	private createGlobalQuotaCallout(container: HTMLElement): () => { calloutVisible: boolean; additionalUsageEnabled: boolean } {
		...
		const update = () => {
			...
			if (isEnterpriseUser && isPooledQuotaExhausted) {
				...
				calloutText.textContent = localize('quotaBudgetExceededEnterprise', "Your organization or enterprise has exceeded its Dardcor AI budget. Contact your admin to resume usage.");
			} else if (maxUsedPercentage >= 100 && additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaAdditionalUsageActiveEnterprise', "Dardcor AI has paused because your limits are reached. Please contact your admin to increase your limits.")
					: isUsageBasedBilling
				...
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaAdditionalUsageApproachingEnterprise', "Dardcor AI will pause when your limits are reached. Please contact your admin to increase your limits.")
					: isUsageBasedBilling
				...
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaPausedEnterprise', "Dardcor AI is paused until the limit resets. Contact your administrator for more information.")
					: localize('quotaPaused', "Dardcor AI is paused until the limit resets.");
			} else if (maxUsedPercentage >= 75 && !additionalUsageEnabled) {
				quotaCallout.style.display = '';
				quotaCallout.className = 'quota-callout info';
				calloutIcon.className = `callout-icon ${ThemeIcon.asClassName(Codicon.info)}`;
				calloutText.textContent = isEnterpriseUser
					? localize('quotaWarningEnterprise', "Dardcor AI will pause when the limit is reached. Contact your administrator for more information.")
					: localize('quotaWarning', "Dardcor AI will pause when the limit is reached.");
			} else {
				quotaCallout.style.display = 'none';
			}
