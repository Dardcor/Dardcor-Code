...
export class InlineChatUntitledSaveHandler {
    constructor(private readonly _isEligible: (resource: URI) => boolean, private readonly _editorService: IEditorService, private readonly _onSaved: (source: URI, target: URI, message: string) => Promise<void>, private readonly _logService: ILogService) { }
...
export class InlineChatController implements IEditorContribution {
    ...
    chatService: IChatService) {
        ...
        this.#untitledSaveHandler = new InlineChatUntitledSaveHandler(resource => this.#configurationService.getValue<boolean>(ChatConfiguration.InlineChatAgentHostEnabled) === true
            && !this.#notebookEditorService.getNotebookForPossibleCell(this.#editor)
            && resource.scheme === Schemas.untitled, editorService, async (source, target, message) => {
        ...
        this.#zone = new Lazy<InlineChatZoneWidget>(() => {
            ...
            }, { editor: this.#editor, notebookEditor }, () => Promise.resolve());
        ...
        const lastResponseProgressObs = lastResponseObs.map((response, r) => {
            ...
            return observableFromEvent(this, response.onDidChange, () => response.response.value.findLast(part => part.kind === 'progressMessage' || part.kind === 'toolInvocation' || part.kind === 'toolInvocationSerialized')).read(r);
        });
        this.#store.add(autorun(r => {
            const session = visibleSessionObs.read(r);
    ...
    #waitForSavedEditorController(resource: URI): Promise<InlineChatController | undefined> {
        ...
        return new Promise(resolve => {
            ...
            store.add(disposableTimeout(() => complete(undefined), 5000));
            const controller = findController();
            if (controller) {
                complete(controller);
            }
