...
class FixtureEditorInput extends EditorInput {
    constructor(readonly resource: URI, private readonly _options: IFixtureEditorInputOptions = {}) {
        super();
    }
...
function defaultEditorSpecs(): IEditorSpec[] {
    return [
        ...
        { resource: URI.from({ scheme: Schemas.untitled, path: 'Untitled-1' }), typeId: 'workbench.editors.untitledFixture', icon: ThemeIcon.fromId(Codicon.file.id), pinned: false },
...
function createFixtureEditorTitleActions(store: DisposableStore, menuId: MenuId): IToolbarActions {
    ...
    return {
        primary: [
            store.add(new Action('fixture.splitEditorRight', localize('fixtureSplitEditorRight', "Split Editor Right"), ThemeIcon.asClassName(Codicon.splitHorizontal)))
        ],
        secondary: [
            store.add(new Action('fixture.openEditor', localize('fixtureOpenEditor', "Open Editor..."), ThemeIcon.asClassName(Codicon.goToFile)))
        ]
    };
}
...
export function renderEditorTabBarFixture(ctx: ComponentFixtureContext, options: IEditorTabBarFixtureOptions): void {
    ...
    const groupView = new class extends mock<IEditorGroupView>() {
        ...
        override getEditors(order: EditorsOrder, opts?: {
    ...
    const titleControl = disposableStore.add(instantiationService.createInstance(EditorTitleControl, titleContainer, editorPartsView, groupsView, groupView, model, options.headerMenuIds, options.showHeader ?? false));
    const layout = () => {
        titleControl.layout({
