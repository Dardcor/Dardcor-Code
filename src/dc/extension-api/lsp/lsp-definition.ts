...
export class LspDefinition {
	...
	public async getDefinition(uri: string, position: Position | ILspPosition): Promise<Location[]> {
		const result = await this._requireClient().request<any>('textDocument/definition', {
			textDocument: { uri },
	...
	private _toLocations(result: any): Location[] {
		...
		for (const item of items) {
			if (item.targetUri && item.targetRange) {
				locations.push(new Location(URI.parse(item.targetUri), lspRangeToApiRange(item.targetRange)));
