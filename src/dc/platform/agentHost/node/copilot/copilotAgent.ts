...
export class CopilotAgent extends Disposable implements IAgent {
	...
	getDescriptor(): IAgentDescriptor {
		return {
			provider: 'copilotcli',
			displayName: localize('copilotAgent.displayName', "Dardcor Code"),
			description: localize('copilotAgent.description', "Dardcor Code agent running in the local agent host process"),
			capabilities: {
	...
	private async _ensureClientOnce(): Promise<CopilotClient> {
		...
		const startClient = async () => {
			...
			const clientOptions: CopilotClientOptions & { clientInfo?: unknown } = {
	...
	private _createModelPickerMeta(modelInfo: CopilotModelInfo, billing: ICAPIModelBilling | undefined): Record<string, unknown> | undefined {
		...
		const notices = isAutoModel(modelInfo.id) ? undefined : createAgentModelNoticesMeta(modelInfo as any);
