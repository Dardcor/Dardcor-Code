...
import type { CopilotSession, CurrentToolMetadata, PermissionRequest, SessionEvent, SessionEventHandler, SessionEventPayload, SessionEventType, Tool, ToolResultObject, TypedSessionEventHandler } from '@github/copilot-sdk';
...
Use the attached image as context.
	...
	suite('event mapping', () => {
		...
		test('tool approval waits for permission outcome and falls back only at completion', async () => {
			...
			} as unknown as SessionEventPayload<'permission.requested'>['data']);
			assert.strictEqual(telemetryService.events.length, 0);
