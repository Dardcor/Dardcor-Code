...
import type { CopilotClient, CopilotClientOptions, CopilotSession, GitHubTelemetryNotification, PermissionRequest, SessionConfig, SessionEvent, SessionEventHandler, SessionEventPayload, SessionEventType, TypedSessionEventHandler } from '@github/copilot-sdk';
type PermissionMode = 'manual' | 'allow-all' | string;
...
suite('CopilotAgent', () => {
	...
	test('installs client identity and the GitHub telemetry callback in CopilotClientOptions', async () => {
		...
		try {
			...
			assert.deepStrictEqual({
				clientInfo: (clientOptions as any)?.clientInfo,
