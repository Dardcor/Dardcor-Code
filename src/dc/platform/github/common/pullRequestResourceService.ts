...
import { FragmentState, GitHubFragmentError, PullRequestChecks, PullRequestComment, PullRequestCore, PullRequestFragment, PullRequestInlineComment, PullRequestMergeability, PullRequestParticipants, PullRequestRef, PullRequestRefreshOptions, PullRequestResource, PullRequestReview, PullRequestReviewThread, PullRequestSnapshot, PullRequestSubscription, PullRequestSubscriptionOptions, } from './githubPullRequestService.js';
...
const defaultPollingPolicy: PullRequestPollingPolicy = {
    dormantGrace: 120000,
    fragmentBodyGrace: 30000,
    maximumDormantEntries: 50,
    coreVisible: 60000,
    coreBackground: 300000,
    conversationVisible: 60000,
    conversationBackground: 300000,
    checksPendingVisible: 15000,
    checksPendingBackground: 60000,
    checksBackstop: 300000,
    mergeabilityVisible: 30000,
    mergeabilityBackground: 120000,
    participants: 300000,
    failureBackoff: { immediateRetries: 0, base: 30000, maximum: 300000, jitter: 5000 },
    jitter: 5000,
};
...
type AnyFragmentState = FragmentState<PullRequestCore> | FragmentState<readonly PullRequestComment[]> | FragmentState<readonly PullRequestReview[]> | FragmentState<readonly PullRequestInlineComment[]> | FragmentState<readonly PullRequestReviewThread[]> | FragmentState<PullRequestChecks> | FragmentState<PullRequestMergeability> | FragmentState<PullRequestParticipants>;
...
class PullRequestEntry {
    ...
    disposed = false;
    constructor(readonly id: number, ref: PullRequestRef) {
        this.ref = ref;
...
class PullRequestSubscriptionImpl implements PullRequestSubscription {
    ...
    constructor(readonly resource: PullRequestResource, entry: PullRequestEntry, private readonly _service: PullRequestResourceService, options: PullRequestSubscriptionOptions) {
        this.entry = entry;
    ...
    refresh(fragment?: PullRequestFragment, token: CancellationToken = CancellationToken.None, options?: PullRequestRefreshOptions): Promise<void> {
        if (this._disposed) {
...
export class PullRequestResourceService extends Disposable implements IPullRequestResources {
    ...
    private _entryId = 0;
    constructor(scheduler: IGitHubScheduler = systemGitHubScheduler, private readonly _policy: PullRequestPollingPolicy = defaultPollingPolicy, private readonly _credentials: IGitHubCredentials, private readonly _queries: IPullRequestQuery, private readonly _logService: ILogService) {
        super();
    ...
    async refreshSubscription(subscription: PullRequestSubscriptionImpl, fragment: PullRequestFragment | undefined, token: CancellationToken, options?: PullRequestRefreshOptions): Promise<void> {
        if (!subscription.entry.subscriptions.has(subscription)) {
            throw new Error('Pull request subscription is no longer active');
        }
        if (fragment) {
    ...
    private async _refreshFragment(entry: PullRequestEntry, fragment: PullRequestFragment, token: CancellationToken, authoritative = false): Promise<void> {
        ...
        const operation: IFragmentOperation = {
            ...
            interest,
            promise: this._runFragmentFetch(entry, fragment, interest, entryGeneration, fragmentGeneration, headAtStart, controller).finally(() => {
    ...
    private async _runFragmentFetch(entry: PullRequestEntry, fragment: PullRequestFragment, interest: EffectivePullRequestFragmentInterest, entryGeneration: number, fragmentGeneration: number, headAtStart: string | undefined, controller: AbortController): Promise<void> {
        ...
        try {
            ...
            const result = await this._queries.fetch(fragment, entry.ref, entry.snapshot.get().core.value, pullRequestOptionsForFragment(fragment, interest), credential, AbortSignal.any([controller.signal, credential.signal]));
    ...
    private _canCommit(entry: PullRequestEntry, fragment: PullRequestFragment, entryGeneration: number, fragmentGeneration: number, credential: GitHubCredential | undefined, headAtStart: string | undefined): boolean {
        if (entry.disposed
    ...
    private _scheduleAfterFailure(entry: PullRequestEntry, fragment: PullRequestFragment, interest: EffectivePullRequestFragmentInterest, error: unknown): void {
