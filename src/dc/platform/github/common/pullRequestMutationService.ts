...
import { CreatedPullRequest, CreatePullRequestOptions, EnablePullRequestAutoMergeOptions, GitHubCheckAnnotation, GitHubWorkflowJob, GitHubWorkflowLog, GitHubWorkflowRerunOptions, GitHubWorkflowRun, PullRequestBranchUpdateOptions, PullRequestCommentOptions, PullRequestEnqueueResult, PullRequestMergeAuthorization, PullRequestMergeOptions, PullRequestMergePreparation, PullRequestMergeResult, PullRequestMutationApi, PullRequestMutationResult, PullRequestNodeOptions, PullRequestReplyAndResolveOptions, PullRequestReplyAndResolveResult, PullRequestReplyOptions, } from './githubPullRequestMutationService.js';
import { GitHubRepositoryRef } from './githubQueryService.js';
import { PullRequestComment, GitHubFragmentError, PullRequestInlineComment, PullRequestRef, PullRequestResource, PullRequestSnapshot, PullRequestSubscription, } from './githubPullRequestService.js';
...
const workflowLogTimeout = 30000;
const mergePreparationLifetime = 5 * 60000;
...
export class PullRequestMutationService extends Disposable implements IPullRequestMutations {
    ...
    constructor(scheduler: IGitHubScheduler | undefined, private readonly _credentials: IGitHubCredentials, private readonly _transport: IGitHubTransport, private readonly _resources: IPullRequestResources, private readonly _endpoint: IGitHubEndpointProvider, private readonly _logService?: ILogService) {
        super();
    ...
    enableAutoMerge(ref: GitHubRepositoryRef, options: EnablePullRequestAutoMergeOptions, signal: AbortSignal): Promise<void> {
        return this._serializeRepository(ref, 'enableAutoMerge', async () => {
            await this._withCredential(ref, signal, async (credential, combinedSignal) => {
                const response = await this._transport.graphql(credential.account, credential.token, this._endpoint.getGraphQlUri(), enableAutoMergeMutation, { pullRequestId: options.pullRequestId, mergeMethod: options.method }, combinedSignal, 'mutation');
    ...
    listWorkflowRuns(ref: PullRequestRef, headSha: string, signal: AbortSignal): Promise<readonly GitHubWorkflowRun[]> {
        return this._withCredential(ref, signal, async (credential, combinedSignal) => {
            const values = await this._fetchRestArray(ref, credential, `actions/runs?head_sha=${encodeURIComponent(headSha)}&per_page=100`, combinedSignal, 'workflow_runs');
    ...
    listWorkflowJobs(ref: PullRequestRef, runId: string, signal: AbortSignal): Promise<readonly GitHubWorkflowJob[]> {
        return this._withCredential(ref, signal, async (credential, combinedSignal) => {
            const values = await this._fetchRestArray(ref, credential, `actions/runs/${encodeURIComponent(runId)}/jobs?per_page=100`, combinedSignal, 'jobs');
    ...
    listCheckAnnotations(ref: PullRequestRef, checkRunId: string, signal: AbortSignal): Promise<readonly GitHubCheckAnnotation[]> {
        return this._withCredential(ref, signal, async (credential, combinedSignal) => {
            const values = await this._fetchRestArray(ref, credential, `check-runs/${encodeURIComponent(checkRunId)}/annotations?per_page=100`, combinedSignal);
            ...
            try {
                await this._withCredential(ref, signal, async (credential, combinedSignal) => {
                    await this._transport.rest(credential.account, credential.token, {
                        method: 'POST',
                        url: this._restUrl(ref, `actions/runs/${encodeURIComponent(options.runId)}/${options.failedJobsOnly ? 'rerun-failed-jobs' : 'rerun'}`),
                        priority: 'mutation',
                    }, combinedSignal);
                });
                this._resources.invalidatePullRequest(ref, ['checks']);
    ...
    enqueue(preparation: PullRequestMergePreparation, authorization: PullRequestMergeAuthorization, signal: AbortSignal): Promise<PullRequestEnqueueResult> {
        return this._serialize(preparation.ref, 'enqueue', async () => {
            ...
            try {
                ...
                try {
                    const entryId = await this._enqueuePullRequest(preparation.ref, pullRequestId, preparation.expectedHeadSha, signal);
    ...
    private async _addComment(ref: PullRequestRef, options: PullRequestCommentOptions, signal: AbortSignal): Promise<PullRequestMutationResult<PullRequestComment>> {
    ...
    private async _replyToThread(ref: PullRequestRef, options: PullRequestReplyOptions, signal: AbortSignal): Promise<PullRequestMutationResult<PullRequestInlineComment>> {
        const body = withOperationMarker(options.body, options.operationId);
        try {
    ...
    private async _retryComment(ref: PullRequestRef, body: string, signal: AbortSignal): Promise<PullRequestMutationResult<PullRequestComment>> {
        try {
            const value = await this._postComment(ref, body, signal);
            this._resources.invalidatePullRequest(ref, ['topLevelComments']);
            return { outcome: 'succeeded', value };
    ...
    private async _retryThreadReply(ref: PullRequestRef, threadId: string, body: string, signal: AbortSignal): Promise<PullRequestMutationResult<PullRequestInlineComment>> {
        try {
    ...
    private _postThreadReply(ref: PullRequestRef, threadId: string, body: string, signal: AbortSignal): Promise<PullRequestInlineComment> {
        return this._withCredential(ref, signal, async (credential, combinedSignal) => {
            const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), addReviewThreadReplyMutation, { threadId, body }, combinedSignal, 'mutation');
            throwGraphQLErrors(response.errors);
    ...
    private _resolveThread(ref: PullRequestRef, threadId: string, signal: AbortSignal): Promise<void> {
        return this._withCredential(ref, signal, async (credential, combinedSignal) => {
            const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), resolveReviewThreadMutation, { threadId }, combinedSignal, 'mutation');
            throwGraphQLErrors(response.errors);
    ...
    private async _reconcileComment(ref: PullRequestRef, fragment: 'topLevelComments' | 'reviewThreads', operationId: string, signal: AbortSignal): Promise<{
        readonly proven: boolean;
        readonly value?: PullRequestComment | PullRequestInlineComment;
    }> {
        const subscription = this._resources.subscribePullRequest(ref, {
            priority: 'interactive',
    ...
    private async _enqueuePullRequest(ref: PullRequestRef, pullRequestId: string, expectedHeadOid: string, signal: AbortSignal): Promise<string> {
        return this._withCredential(ref, signal, async (credential, combinedSignal) => {
            const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), enqueuePullRequestMutation, { pullRequestId, expectedHeadOid }, combinedSignal, 'mutation');
            throwGraphQLErrors(response.errors);
            return requiredString(objectAt(response.data, 'enqueuePullRequest', 'mergeQueueEntry'), 'id');
        });
    }
    private async _fetchRestArray(ref: PullRequestRef, credential: GitHubCredential, route: string, signal: AbortSignal, arrayPropertyName?: string): Promise<readonly unknown[]> {
    ...
    private async _pullRequestNodeMutation(ref: PullRequestRef, options: PullRequestNodeOptions, mutation: string, signal: AbortSignal): Promise<void> {
        if (!options.pullRequestId) {
            throw new GitHubRequestError('Pull request node ID is required for this mutation', 'validation');
        }
        await this._withCredential(ref, signal, async (credential, combinedSignal) => {
            const response = await this._transport.graphql(credential.account, credential.token, this._endpoint.getGraphQlUri(), mutation, { pullRequestId: options.pullRequestId }, combinedSignal, 'mutation');
            throwGraphQLErrors(response.errors);
        });
    }
    private async _withCredential<T>(ref: GitHubRepositoryRef, signal: AbortSignal, task: (credential: GitHubCredential, combinedSignal: AbortSignal) => Promise<T>): Promise<T> {
...
function toMergeResult(value: unknown): {
    readonly sha?: string;
            ...
            : types.some(type => type?.includes('NOT_FOUND'))
                ...
                : types.some(type => type?.includes('VALIDATION') || type?.includes('UNPROCESSABLE'))
                    ...
                    : types.every(type => type === undefined)
                        ? 'schema'
                        : 'unknown';
    throw new GitHubRequestError(`GitHub GraphQL mutation failed: ${errors.map(error => error.message ?? error.type ?? 'unknown error').join('; ')}`, kind, 200, undefined, errors);
}
function cancellationTokenFromSignal(signal: AbortSignal): {
...
function toActor(value: object | undefined): {
    readonly id?: string;
