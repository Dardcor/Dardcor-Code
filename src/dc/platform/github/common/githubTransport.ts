...
export class GitHubRequestError extends Error {
    constructor(message: string, readonly kind: GitHubRequestErrorKind, readonly statusCode?: number, readonly responseBody?: string, readonly graphQLErrors?: readonly GitHubGraphQLError[]) {
        super(message);
...
export class GitHubTransport extends Disposable implements IGitHubTransport {
    ...
    constructor(fetchFn: FetchFunction | undefined, private readonly _scheduler: IGitHubScheduler = systemGitHubScheduler, private readonly _allowInsecureLoopbackDownloads = false, private readonly _logService?: ILogService) {
        super();
    ...
    async rest<T>(account: GitHubAccountHandle, token: string, request: GitHubRestRequest, signal: AbortSignal): Promise<GitHubRestResponse<T>> {
        ...
        if (!shared) {
            ...
            void promise.then(() => this._deleteRestRequest(coalescingKey, created), () => this._deleteRestRequest(coalescingKey, created));
        }
        else {
    ...
    async graphql<T>(account: GitHubAccountHandle, token: string, url: string, query: string, variables: Readonly<Record<string, unknown>>, signal: AbortSignal, priority: GitHubRequestPriority = 'interactive'): Promise<GitHubGraphQLResponse<T>> {
    ...
    async download(account: GitHubAccountHandle, token: string, request: GitHubDownloadRequest, signal: AbortSignal): Promise<GitHubDownloadResponse> {
        ...
        return this._logRequest('download', formatDownloadUrl(request.url), account, priority, signal, () => this._enqueueWithRateLimit(account, 'core', priority, signal, async () => {
            const controller = new AbortController();
            const timeout = this._scheduler.schedule(() => controller.abort(new GitHubRequestError('GitHub download timed out', 'network')), Math.max(0, request.timeout));
    ...
    private async _graphqlRead<T>(account: GitHubAccountHandle, token: string, url: string, query: string, variables: Readonly<Record<string, unknown>>, signal: AbortSignal, priority: GitHubRequestPriority): Promise<GitHubGraphQLResponse<T>> {
        ...
        if (!shared) {
            ...
            const created = shared;
            void promise.then(() => this._deleteGraphQLRequest(key, created), () => this._deleteGraphQLRequest(key, created));
        }
        else {
    ...
    private async _executeGraphQL<T>(account: GitHubAccountHandle, token: string, url: string, query: string, variables: Readonly<Record<string, unknown>>, signal: AbortSignal, priority: GitHubRequestPriority): Promise<GitHubGraphQLResponse<T>> {
        ...
        return this._logRequest('GraphQL', operation, account, priority, signal, () => this._enqueueWithRateLimit(account, 'graphql', priority, signal, async () => {
            ...
            const json = this._parseJson<{
                data?: T;
                errors?: readonly GitHubGraphQLError[];
            }>(body, 'GitHub GraphQL response was not valid JSON');
    ...
    private async _executeRest<T>(account: GitHubAccountHandle, token: string, request: GitHubRestRequest, signal: AbortSignal, cacheKey: string): Promise<GitHubRestResponse<T>> {
    ...
    private async _enqueueWithRateLimit<T>(account: GitHubAccountHandle, resource: string, priority: GitHubRequestPriority, signal: AbortSignal, task: () => Promise<T>): Promise<T> {
        while (true) {
    ...
    private async _logRequest<T>(kind: string, operation: string, account: GitHubAccountHandle, priority: GitHubRequestPriority, signal: AbortSignal, task: () => Promise<T>): Promise<T> {
    ...
    private _waitForShared<T>(shared: ISharedRequest, signal: AbortSignal): Promise<GitHubRestResponse<T>> {
        ...
        return new Promise((resolve, reject) => {
            ...
            void shared.promise.then(response => {
...
function readGraphQLRateLimit(data: unknown): {
    limit?: number;
    remaining?: number;
    used?: number;
    resetAt?: string;
...
async function readBoundedResponse(response: Response, maximumBytes: number, signal: AbortSignal): Promise<{
