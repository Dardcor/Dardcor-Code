...
import { GitHubChangedFile, GitHubComparison, GitHubComparisonCommit, GitHubHydratableResourceRef, GitHubIssue, GitHubIssueRef, GitHubIssueResource, GitHubIssueSubscription, GitHubPullRequestContext, GitHubPullRequestContextComment, GitHubPullRequestLookup, GitHubPullRequestsPage, GitHubPullRequestSummary, GitHubQueryApi, GitHubRecentIssue, GitHubRecentPullRequest, GitHubRecentPullRequestReviewThread, GitHubRepository, GitHubRepositoryRef, GitHubRepositoryResource, GitHubRepositorySubscription, GitHubResourcePriority, GitHubResourceSubscriptionOptions, } from './githubQueryService.js';
...
const defaultPollingPolicy: GitHubEntityPollingPolicy = {
    ...
    visible: 60000,
    background: 300000,
    failureBackoff: { immediateRetries: 0, base: 60000, maximum: 900000, jitter: 5000 },
    jitter: 5000,
};
...
class EntityEntry<TRef extends EntityRef, TValue extends EntityValue> {
    ...
    generation = 0;
    disposed = false;
    constructor(readonly id: number, readonly kind: EntityKind, ref: TRef) {
        this.ref = ref;
...
class EntitySubscription<TRef extends EntityRef, TValue extends EntityValue> {
    ...
    constructor(readonly resource: TRef extends GitHubIssueRef ? GitHubIssueResource : GitHubRepositoryResource, readonly entry: EntityEntry<TRef, TValue>, private readonly _service: GitHubQueryService, options: GitHubResourceSubscriptionOptions) {
...
export class GitHubQueryService extends Disposable implements IGitHubQuery {
    ...
    private _entryId = 0;
    constructor(scheduler: IGitHubScheduler | undefined, private readonly _policy: GitHubEntityPollingPolicy = defaultPollingPolicy, private readonly _credentials: IGitHubCredentials, private readonly _transport: IGitHubTransport, private readonly _endpoint: IGitHubEndpointProvider, private readonly _capabilities: IGitHubCapabilities, private readonly _logService: ILogService) {
        super();
    ...
    async findPullRequestByHeadBranch(ref: GitHubRepositoryRef, branch: string, headOwner: string | undefined, signal: AbortSignal): Promise<GitHubPullRequestLookup | undefined> {
        const normalized = normalizeRepositoryRef(ref);
    ...
    async getIssuesWithLinkedPullRequests(ref: GitHubRepositoryRef, issueNumbers: readonly number[], signal: AbortSignal): Promise<readonly number[]> {
    ...
    private async _searchPullRequests(ref: GitHubRepositoryRef, query: string, reviewRequested: boolean, assigned: boolean, signal: AbortSignal): Promise<readonly GitHubPullRequestSummary[]> {
        return this._graphqlWithCredential(ref, searchPullRequestsQuery, { query }, signal, data => arrayProperty(objectProperty(asObject(data, 'GitHub pull request search response was malformed'), 'search'), 'nodes')
            .filter(isObject)
            .map(value => toPullRequestSummary(value, reviewRequested, assigned)));
    }
    private async _graphqlRaw(ref: GitHubRepositoryRef, query: string, variables: Readonly<Record<string, unknown>>, signal: AbortSignal): Promise<unknown> {
        return this._withCredential(ref, signal, async (credential, combinedSignal) => {
            ...
            try {
                const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), query, variables, combinedSignal, 'interactive');
    ...
    private async _graphqlWithCredential<T>(ref: GitHubRepositoryRef, query: string, variables: Readonly<Record<string, unknown>>, signal: AbortSignal, map: (data: unknown) => T): Promise<T> {
        return map(await this._graphqlRaw(ref, query, variables, signal));
    }
    private async _fetchRestPages(ref: GitHubRepositoryRef, credential: GitHubCredential, route: string, signal: AbortSignal): Promise<readonly unknown[]> {
    ...
    private async _withCredential<T>(ref: GitHubRepositoryRef, signal: AbortSignal, task: (credential: GitHubCredential, signal: AbortSignal) => Promise<T>): Promise<T> {
...
function sameAccount(ref: GitHubRepositoryRef, credential: {
    readonly account: {
            ...
            : types.some(type => type?.includes('NOT_FOUND'))
                ...
                : types.some(type => type?.includes('VALIDATION'))
                    ...
                    : codes.some(code => code === 'UNDEFINEDFIELD' || code === 'ARGUMENTNOTACCEPTED' || code === 'VARIABLEMISMATCH')
                        ? 'schema'
                        : 'server';
    throw new GitHubRequestError(`GitHub GraphQL query failed: ${errors.map(error => error.message ?? error.type ?? 'unknown error').join('; ')}`, kind, 200, undefined, errors);
}
...
function toFragmentError(error: unknown): {
    readonly message: string;
    readonly kind: import('./githubTypes.js').GitHubRequestErrorKind;
