import { PullRequestCheck, PullRequestChecks, PullRequestCheckSuite, PullRequestComment, PullRequestCore, PullRequestFragment, PullRequestInlineComment, PullRequestMergeability, PullRequestParticipant, PullRequestParticipants, PullRequestRef, PullRequestReview, PullRequestReviewThread, PullRequestSubscriptionOptions, } from './githubPullRequestService.js';
...
export type PullRequestFragmentResult = {
    readonly fragment: 'core';
    readonly value: PullRequestCore;
    readonly complete: true;
} | {
    readonly fragment: 'topLevelComments';
    readonly value: readonly PullRequestComment[];
    readonly complete: true;
} | {
    readonly fragment: 'submittedReviews';
    readonly value: readonly PullRequestReview[];
    readonly complete: true;
} | {
    readonly fragment: 'inlineComments';
    readonly value: readonly PullRequestInlineComment[];
    readonly complete: true;
} | {
    readonly fragment: 'reviewThreads';
    readonly value: readonly PullRequestReviewThread[];
    readonly complete: boolean;
    readonly headSha: string;
} | {
    readonly fragment: 'checks';
    readonly value: PullRequestChecks;
    readonly complete: boolean;
    readonly headSha: string;
} | {
    readonly fragment: 'mergeability';
    readonly value: PullRequestMergeability;
    readonly complete: boolean;
    readonly headSha: string;
} | {
    readonly fragment: 'participants';
    readonly value: PullRequestParticipants;
    readonly complete: true;
};
export interface IPullRequestQuery {
    fetch(fragment: PullRequestFragment, ref: PullRequestRef, core: PullRequestCore | undefined, options: PullRequestSubscriptionOptions, credential: GitHubCredential, signal: AbortSignal): Promise<PullRequestFragmentResult>;
}
...
export class PullRequestQueryService implements IPullRequestQuery {
    ...
    constructor(private readonly _transport: IGitHubTransport, private readonly _capabilities: IGitHubCapabilities, private readonly _endpoint: IGitHubEndpointProvider, private readonly _logService?: ILogService) { }
    async fetch(fragment: PullRequestFragment, ref: PullRequestRef, core: PullRequestCore | undefined, options: PullRequestSubscriptionOptions, credential: GitHubCredential, signal: AbortSignal): Promise<PullRequestFragmentResult> {
        ...
        switch (fragment) {
            ...
            case 'checks':
                ...
                return {
                    fragment,
                    value: await this._fetchChecks(ref, core, credential, signal, plan.priority, capabilities.checkContextRequiredness, options.checks?.required === true, options.checks?.includeOptional === true),
    ...
    private async _fetchRestArray(ref: PullRequestRef, credential: GitHubCredential, route: string, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority): Promise<readonly unknown[]> {
    ...
    private async _fetchReviewThreads(ref: PullRequestRef, core: PullRequestCore, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority, includeBodies: boolean): Promise<readonly PullRequestReviewThread[]> {
        const result: PullRequestReviewThread[] = [];
        let after: string | undefined;
        for (let page = 0; page < maximumPaginationPages; page++) {
            const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), reviewThreadsQuery, { owner: ref.owner, repo: ref.repo, number: ref.number, after }, signal, priority);
    ...
    private async _toReviewThread(value: unknown, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority, includeBodies: boolean): Promise<PullRequestReviewThread> {
        ...
        for (let page = 1; pageInfo.hasNextPage && page < maximumPaginationPages; page++) {
            const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), reviewThreadCommentsQuery, { threadId: id, after: requiredCursor(after) }, signal, priority);
            throwGraphQLErrors(response.errors);
    ...
    private async _fetchChecks(ref: PullRequestRef, core: PullRequestCore, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority, includeRequiredness: boolean, loadExpectedSuites: boolean, includeOptional: boolean): Promise<PullRequestChecks> {
    ...
    private async _fetchCheckRollupWithWorkflowNames(ref: PullRequestRef, core: PullRequestCore, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority, includeRequiredness: boolean, includeOptional: boolean): Promise<{
        readonly headSha: string;
    ...
    private async _fetchExpectedCheckSuitesWhenPermitted(ref: PullRequestRef, headSha: string, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority): Promise<{
        readonly suites: readonly PullRequestCheckSuite[];
        readonly complete: boolean;
    }> {
        try {
    ...
    private async _fetchCheckRollup(ref: PullRequestRef, core: PullRequestCore, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority, includeRequiredness: boolean, includeOptional: boolean, includeWorkflowNames: boolean): Promise<{
        readonly headSha: string;
        readonly checks: readonly PullRequestCheck[];
    }> {
        ...
        let after: string | undefined;
        for (let page = 0; page < maximumPaginationPages; page++) {
            const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), checksQuery(includeRequiredness, includeWorkflowNames), { owner: ref.owner, repo: ref.repo, number: ref.number, after }, signal, priority);
            throwGraphQLErrors(response.errors);
            const pullRequest = objectAt(response.data, 'repository', 'pullRequest');
    ...
    private async _fetchExpectedCheckSuites(ref: PullRequestRef, headSha: string, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority): Promise<readonly PullRequestCheckSuite[]> {
        const suites: PullRequestCheckSuite[] = [];
        let after: string | undefined;
        for (let page = 0; page < maximumPaginationPages; page++) {
            const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), expectedCheckSuitesQuery, { owner: ref.owner, repo: ref.repo, headSha, after }, signal, priority);
            throwGraphQLErrors(response.errors);
    ...
    private async _fetchChecksFallback(ref: PullRequestRef, core: PullRequestCore, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority): Promise<PullRequestChecks> {
        const checks: PullRequestCheck[] = [];
    ...
    private async _fetchMergeability(ref: PullRequestRef, core: PullRequestCore, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority, mergeQueueSupported: boolean): Promise<PullRequestMergeability> {
        const response = await this._transport.graphql<unknown>(credential.account, credential.token, this._endpoint.getGraphQlUri(), mergeabilityQuery(mergeQueueSupported), mergeQueueSupported
            ? { owner: ref.owner, repo: ref.repo, number: ref.number, baseBranch: core.baseRef }
            : { owner: ref.owner, repo: ref.repo, number: ref.number }, signal, priority);
        throwGraphQLErrors(response.errors);
    ...
    private async _fetchMergeabilityFallback(ref: PullRequestRef, core: PullRequestCore, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority): Promise<PullRequestMergeability> {
        const response = await this._transport.rest<unknown>(credential.account, credential.token, {
            method: 'GET',
            url: this._restUrl(ref, `pulls/${ref.number}`),
            unconditional: true,
    ...
    private async _fetchParticipants(ref: PullRequestRef, core: PullRequestCore | undefined, credential: GitHubCredential, signal: AbortSignal, priority: import('./githubTypes.js').GitHubRequestPriority): Promise<PullRequestParticipants> {
        ...
        const participants = new Map<string, {
            actor: PullRequestParticipant;
            roles: Set<'author' | 'commenter' | 'reviewer'>;
        }>();
        if (core?.author) {
...
function toActor(value: object | undefined, association?: string): {
    readonly id?: string;
...
function addParticipant(participants: Map<string, {
    actor: PullRequestParticipant;
    roles: Set<'author' | 'commenter' | 'reviewer'>;
}>, actor: {
    readonly id?: string;
    readonly login: string;
}, role: 'author' | 'commenter' | 'reviewer'): void {
        ...
        : kinds.some(type => type === 'FORBIDDEN' || type === 'UNAUTHORIZED')
            ...
            : kinds.some(type => type?.includes('NOT_FOUND'))
                ...
                : kinds.some(type => type?.includes('VALIDATION'))
                    ? 'schema'
                    : 'server';
    throw new GitHubRequestError(`GitHub GraphQL request failed: ${errors.map(error => error.message ?? error.type ?? 'unknown error').join('; ')}`, kind, 200, undefined, errors);
}
...
function pageInfoFrom(connection: object): {
