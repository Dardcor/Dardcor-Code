...
function hasExpandablePanel<T>(item: IActionListItem<T>): boolean {
...
class ActionItemRenderer<T> implements IListRenderer<IActionListItem<T>, IActionMenuTemplateData> {
    ...
    constructor(private readonly _supportsPreview: boolean, private readonly _onRemoveItem: ((item: IActionListItem<T>) => void) | undefined, private readonly _onShowSubmenu: ((item: IActionListItem<T>) => void) | undefined, private readonly _reservesSubmenuSpace: () => boolean, private readonly _groupTitleByIndex: ReadonlyMap<number, string>, private readonly _linkHandler: ((uri: URI, item: IActionListItem<T>) => void) | undefined, private readonly _hideDefaultKeybindingTooltip: boolean, private readonly _registerStandaloneToggle: (item: IActionListItem<T>, toggle: Switch) => IDisposable, 
    @IKeybindingService
...
export class ActionListWidget<T> extends Disposable {
    ...
    constructor(user: string, protected readonly _supportsPreview: boolean, items: readonly IActionListItem<T>[], protected readonly _delegate: IActionListDelegate<T>, accessibilityProvider: Partial<IListAccessibilityProvider<IActionListItem<T>>> | undefined, protected readonly _options: IActionListOptions | undefined, 
    @IKeybindingService
    private readonly _keybindingService: IKeybindingService, 
    @IOpenerService
    private readonly _openerService: IOpenerService, 
    ...
    private readonly _instantiationService: IInstantiationService) {
        ...
        if (this._options?.persistentHover) {
            this._register(dom.addDisposableListener(this.domNode, dom.EventType.MOUSE_MOVE, event => {
                if (dom.isHTMLElement(event.target) && this._expandedTrigger?.contains(event.target)) {
        ...
        this._list = this._register(new List(user, this.domNode, virtualDelegate, [
            ...
            accessibilityProvider: {
                ...
                getPosInSet: (_element, index) => Math.max(this._visibleMenuItems.slice(0, index + 1).filter(item => item.kind === ActionListItemKind.Action).length, 1),
                getRole: (e) => {
                    switch (e.kind) {
        ...
        if (this._options?.headerText) {
            ...
            this._register(dom.addDisposableListener(this._headerContainer, dom.EventType.MOUSE_ENTER, () => {
                if (this._options?.persistentHover) {
                    this._cancelSubmenuShow();
                    this._resetSubmenuPointer();
                }
    ...
    private _applyFilter(skipTextFilter = false, fireLayout = true): void {
        ...
        if (filterInputHasFocus) {
            ...
            if (focusedItem) {
                const focusedItemId = (focusedItem.item as {
                    id?: string;
                })?.id;
                if (focusedItemId) {
    ...
    showHoverForCheckedItem(): void {
        const element = this._allMenuItems.find(item => item.kind === ActionListItemKind.Action && (item.item as {
            checked?: boolean;
        } | undefined)?.checked)
        ...
        if (index >= 0) {
            ...
            this._list.reveal(index);
            this._showHoverForElement(element, index);
        }
    }
        ...
        try {
            ...
            if (initialFocusItemId) {
                for (let i = 0; i < this._list.length; i++) {
                    ...
                    if (element.kind === ActionListItemKind.Action && (element.item as {
                        id?: string;
                    })?.id === initialFocusItemId) {
                        this._list.setFocus([i]);
                        this._list.reveal(i);
                        return;
                    }
        ...
        if (hasSubmenuActions) {
            ...
            for (const action of element.submenuActions!) {
                if (!(action instanceof SubmenuAction)) {
                    const extendedAction = action as IAction & {
                        onRemove?: () => void;
                    };
                    submenuItems.push({
                        item: action,
            ...
            const submenuWidget = this._submenuDisposables.add(this._instantiationService.createInstance(ActionListWidget<IAction>, 'submenu', false, submenuItems, submenuDelegate, undefined, undefined));
    ...
    private _scheduleSubmenuShow(element: IActionListItem<T>, pointer: MouseEvent): void {
        ...
        if (this._options?.persistentHover) {
            ...
            if (origin && pointer.clientX !== origin.x && this._currentSubmenuElement && new dom.SafeTriangle(origin.x, origin.y, this._submenuContainer).contains(pointer.clientX, pointer.clientY)) {
                const now = Date.now();
...
export class ActionList<T> extends Disposable {
    ...
    constructor(user: string, preview: boolean, items: readonly IActionListItem<T>[], _delegate: IActionListDelegate<T>, accessibilityProvider: Partial<IListAccessibilityProvider<IActionListItem<T>>> | undefined, options: IActionListOptions | undefined, anchor: HTMLElement | StandardMouseEvent | IAnchor, 
    @IContextViewService
    ...
    instantiationService: IInstantiationService) {
        ...
        this._widget = this._register(instantiationService.createInstance(ActionListWidget<T>, user, preview, items, _delegate, accessibilityProvider, options));
