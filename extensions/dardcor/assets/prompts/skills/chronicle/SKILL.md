...
Analyze the user's Copilot session history using the `dardcor_sessionStoreSql` tool. This skill handles standup reports, usage analysis, session search, and session store maintenance.
...
**Prerequisite:** Chronicle requires the `github.dardcor.chat.localIndex.enabled` setting to be `true`. If the `dardcor_sessionStoreSql` tool is not available, tell the user to enable this setting in VS Code Settings.

## Available Tool Actions

The `dardcor_sessionStoreSql` tool supports two actions:
...
Use `dardcor_sessionStoreSql` with `action: "query"` and follow the SQL dialect shown in the tool description (SQLite locally, DuckDB on cloud — see the **Database Schema** and **Query Guidelines** sections below).
...
Use `dardcor_sessionStoreSql` with `action: "query"` to explore their recent sessions. The goal is to understand their patterns — how they prompt, what tools they use, and where they spend time.
...
If the current workspace has a `.github/` folder, check for `.github/dardcor-instructions.md`, `.github/skills/`, and `.github/agents/` to see what custom configuration exists. Do NOT look outside the workspace. Look for gaps between what's available and what the user actually uses.
...
- **sessions.agent_name** / **agent_description** (both backends): the interactive VS Code chat surface is stored as `'VS Code Chat'` on cloud and `'GitHub Copilot Chat'` on local. Other values include `Copilot CLI` / `dardcorcli`, `Copilot Coding Agent`, subagents (`Explore`, `summarizeConversationHistory`, `panel/editAgent`, …), and custom agents.
...
Use `dardcor_sessionStoreSql` with `action: "query"`. Every query in this step must filter `sessions.agent_name` to the interactive VS Code chat value for the active backend — `'VS Code Chat'` on cloud, `'GitHub Copilot Chat'` on local. What to investigate depends on the active backend.
...
- **Repeated work** — the same file/topic showing up in many sessions, or the same agent stumbling block recurring (suggesting a custom skill, agent, or `dardcor-instructions.md` entry would let the model do the work in one shot).
...
If the current workspace has a `.github/` folder, check `.github/dardcor-instructions.md`, `.github/skills/`, and `.github/agents/` to see what custom configuration already exists. Do NOT look outside the workspace. Cost-relevant capabilities to keep in mind:
...
- `.github/dardcor-instructions.md` to encode project conventions the model otherwise has to be told every session.
...
- **Stay within VS Code chat scope** — tips should target interactive VS Code chat usage (compaction, model picker, fresh chats, `.github/dardcor-instructions.md`, custom skills/agents, subagent delegation). Do not propose CLI- or Coding-Agent–specific changes unless the user has explicitly broadened scope.
...
Read whichever instructions file the project uses (`.github/dardcor-instructions.md` or `AGENTS.md`) to understand what already exists.
...
Use `dardcor_sessionStoreSql` to explore. Scope all queries to sessions from the current repository or working directory.
...
Call `dardcor_sessionStoreSql` with `action: "query"` and `description: "Search sessions for <query>"`. Follow the SQL dialect shown in the tool description (SQLite vs DuckDB).
...
1. Call `dardcor_sessionStoreSql` with `action: "reindex"` and `description: "Reindex sessions"`.
...
- Guide them to run the **Delete Session Sync Data** command from the Command Palette (`github.dardcor.sessionSync.deleteSessions`).
