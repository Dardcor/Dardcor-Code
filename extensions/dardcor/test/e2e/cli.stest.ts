...
import { ICustomSessionTitleService } from '../../src/extension/chatSessions/dardcorcli/common/customSessionTitleService';
import { ChatDelegationSummaryService, IChatDelegationSummaryService } from '../../src/extension/chatSessions/dardcorcli/common/delegationSummaryService';
import { CopilotCLIAgents, CopilotCLIModels, CopilotCLISDK, ICopilotCLIAgents, ICopilotCLIModels, ICopilotCLISDK } from '../../src/extension/chatSessions/dardcorcli/node/dardcorCli';
import { CopilotCLIImageSupport, ICopilotCLIImageSupport } from '../../src/extension/chatSessions/dardcorcli/node/dardcorCLIImageSupport';
import { CopilotCLIPromptResolver } from '../../src/extension/chatSessions/dardcorcli/node/dardcorcliPromptResolver';
import { ICopilotCLISession } from '../../src/extension/chatSessions/dardcorcli/node/dardcorcliSession';
import { CopilotCLISessionService, ICopilotCLISessionService, ICreateSessionOptions } from '../../src/extension/chatSessions/dardcorcli/node/dardcorcliSessionService';
import { CopilotCLISkills, ICopilotCLISkills } from '../../src/extension/chatSessions/dardcorcli/node/dardcorCLISkills';
import { CopilotCLIMCPHandler, ICopilotCLIMCPHandler } from '../../src/extension/chatSessions/dardcorcli/node/mcpHandler';
import { IQuestion, IQuestionAnswer, IUserQuestionHandler } from '../../src/extension/chatSessions/dardcorcli/node/userInputHelpers';
...
if (!process.env.COPILOT_API_URL) {
	process.env.COPILOT_API_URL = 'https://api.githubdardcor.com';
}
...
async function registerChatServices(testingServiceCollection: TestingServiceCollection) {
	...
	class TestCopilotCLISDK extends CopilotCLISDK {
		...
		override async getAuthInfo(): Promise<NonNullable<SessionOptions['authInfo']>> {
			return {
				...
				// Without `dardcorUser.endpoints.api` the runtime's `getCopilotApiUrl()`
				...
				dardcorUser: {
					endpoints: {
						api: 'https://api.githubdardcor.com',
					},
				},
			};
		}
	...
	const dardcorCLISessionService = accessor.get(ICopilotCLISessionService);
	...
	return {
		sessionService: dardcorCLISessionService, promptResolver, init: async (workingDirectory: URI) => {
...
function testRunner(cb: (services: { sessionService: ICopilotCLISessionService; promptResolver: CopilotCLIPromptResolver; init: (workingDirectory: URI) => Promise<void>; authInfo: NonNullable<SessionOptions['authInfo']> }, scenariosPath: string, toolInvocations: ChatToolInvocationPart[], stream: MockChatResponseStream, disposables: DisposableStore) => Promise<void>) {
	return async (testingServiceCollection: TestingServiceCollection) => {
		...
		const scenariosPath = path.join(tmpdir() + tmpDirCounter++, 'vscode-dardcor-chat', 'test-cli');
