...
function detectedParticipantToIntentId(detected: ChatParticipantDetectionResult) {
	switch (detected.participant) {
		case 'github.dardcor.default':
			return 'unknown';
		case `github.dardcor.${editingSessionAgentEditorName}`:
			if (detected.command) {
				return detected.command;
			}
			return 'unknown';
		case 'github.dardcor.terminalPanel':
			return 'terminalExplain';
		case `github.dardcor.${editsAgentName}`:
		...
		case 'github.dardcor.vscode':
			return 'vscode';
		case 'github.dardcor-dynamic.platform':
			return 'github.dardcor-dynamic.platform';
	}
