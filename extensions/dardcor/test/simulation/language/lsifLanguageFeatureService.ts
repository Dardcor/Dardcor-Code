...
const REPO_NAME = 'vscode-dardcor';
...
const makeTranslator = (workspace: SimulationWorkspace, indexRoot: string) => {
	...
	if (lastIndex === -1) {
		throw new Error(`Index path ${indexPath} does not contain 'vscode-dardcor', please ensure the index is generated in the correct workspace`);
	}
