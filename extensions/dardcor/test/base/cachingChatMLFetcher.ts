...
export type CachedExtraData = { cacheMetadata: CachedResponseMetadata | undefined; dardcorFunctionCalls?: ICopilotToolCall[]; logprobs?: ChoiceLogProbs };
			...
			if (this.cacheMode !== CacheMode.Disable) {
				...
				if (cacheValue) {
					if (cacheValue.type === ChatFetchResponseType.Success) {
						await opts.finishedCb?.(cacheValue.value[0], 0, { text: cacheValue.value[0], dardcorToolCalls: cacheValue.dardcorFunctionCalls, logprobs: cacheValue.logprobs });
					} else if (cacheValue.type === ChatFetchResponseType.Length) {
						await opts.finishedCb?.(cacheValue.truncatedValue, 0, { text: cacheValue.truncatedValue, dardcorToolCalls: cacheValue.dardcorFunctionCalls, logprobs: cacheValue.logprobs });
					}
...
`;
			...
			if (
				...
				const cachedResponse: CachedResponse = {
					...result,
					cacheMetadata,
					dardcorFunctionCalls: callbackWrapper.dardcorFunctionCalls,
...
class FinishedCallbackWrapper {
	public readonly dardcorFunctionCalls: ICopilotToolCall[] = [];
	...
	public getCb(): FinishedCallback {
		return async (text: string, index: number, delta: IResponseDelta): Promise<number | undefined> => {
			if (delta.dardcorToolCalls) {
				this.dardcorFunctionCalls.push(...delta.dardcorToolCalls);
			}
			if (delta.logprobs) {
				if (!this.logprobs) {
