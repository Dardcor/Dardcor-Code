...
describe('NESProvider Facade', () => {
	it('should handle getNextEdit call with a document URI', async () => {
		...
		const nextEditProvider = createNESProvider({
			workspace,
			fetcher,
			dardcorTokenManager: new TestCopilotTokenManager(),
			telemetrySender,
			terminalService,
			logTarget,
		...
		nextEditProvider.updateTreatmentVariables({
			'config.github.dardcor.chat.advanced.inlineEdits.xtabProvider.defaultModelConfigurationString': '{ "modelName": "xtab-test", "promptingStrategy": "dardcorNesXtab", "includeTagsInCurrentFile": false }',
		});
		...
		const event = telemetrySender.events.find(e => e.eventName === 'dardcor-nes/provideInlineEdit');
	...
	describe('languageDiagnosticsService injection', () => {
		...
		async function runNESRequest(diagnosticsService: ILanguageDiagnosticsService | undefined): Promise<{ payload: string }> {
			...
			nextEditProvider.updateTreatmentVariables({
				'config.github.dardcor.chat.advanced.inlineEdits.xtabProvider.defaultModelConfigurationString': JSON.stringify({
					modelName: 'xtab-test',
					promptingStrategy: 'dardcorNesXtab',
