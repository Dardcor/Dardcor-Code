...
ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.dardcor.chat.experimental.setupTests.enabled',
	migrateFn: async (value: any) => {
		return [
			['github.dardcor.chat.setupTests.enabled', { value }],
			['github.dardcor.chat.experimental.setupTests.enabled', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.dardcor.chat.experimental.codeGeneration.instructions',
	migrateFn: async (value: any) => {
		return [
			['github.dardcor.chat.codeGeneration.instructions', { value }],
			['github.dardcor.chat.experimental.codeGeneration.instructions', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.dardcor.chat.experimental.codeGeneration.useInstructionFiles',
	migrateFn: async (value: any) => {
		return [
			['github.dardcor.chat.codeGeneration.useInstructionFiles', { value }],
			['github.dardcor.chat.experimental.codeGeneration.useInstructionFiles', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.dardcor.chat.experimental.testGeneration.instructions',
	migrateFn: async (value: any) => {
		return [
			['github.dardcor.chat.testGeneration.instructions', { value }],
			['github.dardcor.chat.experimental.testGeneration.instructions', { value: undefined }]
		];
	}
}]);

ConfigurationMigrationRegistry.registerConfigurationMigrations([{
	key: 'github.dardcor.chat.planAgent.model',
	migrateFn: async (value: any) => {
		return [
			['chat.planAgent.defaultModel', { value }],
			['github.dardcor.chat.planAgent.model', { value: undefined }]
		];
	}
}]);

const oldCursorJumpKey = 'github.dardcor.chat.advanced.inlineEdits.nextCursorPrediction.enabled';
const newCursorJumpKey = 'github.dardcor.nextEditSuggestions.extendedRange';
ConfigurationMigrationRegistry.registerConfigurationMigrations([{
