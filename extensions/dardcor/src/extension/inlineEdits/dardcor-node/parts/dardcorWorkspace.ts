...
import { asPromise, raceCancellation, raceTimeout } from '../../../../util/dardcor/base/common/async';
import { diffMaps } from '../../../../util/dardcor/base/common/collections';
import { onUnexpectedError } from '../../../../util/dardcor/base/common/errors';
...
import { autorun, derived, IObservable, IReader, ISettableObservable, mapObservableArrayCached, observableFromEvent, observableValue, transaction } from '../../../../util/dardcor/base/common/observableInternal';
...
export class VSCodeWorkspace extends ObservableWorkspace implements IDisposable {
	...
	private readonly _textDocsWithShouldTrackFlag = mapObservableArrayCached(this, this._vscodeTextDocuments, (doc, store) => {
		...
		const updateShouldTrack = () => {
			...
			// 	also not sure if we should be enforcing local dardcor-ignore rules (vs only remote-exclusion rules)
	...
	*/
	 ...
	 * per-language `github.dardcor.enable` setting. See {@link DocumentFilter.isLanguageEnabled}.
	 */
