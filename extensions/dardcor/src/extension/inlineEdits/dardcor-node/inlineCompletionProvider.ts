...
import { BugIndicatingError, onUnexpectedError } from '../../../util/dardcor/base/common/errors';
import { Emitter } from '../../../util/dardcor/base/common/event';
import { Disposable } from '../../../util/dardcor/base/common/lifecycle';
import { clamp } from '../../../util/dardcor/base/common/numbers';
import { autorun, IObservable, observableFromEvent } from '../../../util/dardcor/base/common/observable';
...
export class InlineCompletionProviderImpl extends Disposable implements InlineCompletionItemProvider {
	...
	private async _provideInlineCompletionItems(
		...
		// Respect the per-language `github.dardcor.enable` setting for NES, just like ghost text does.
		...
		try {
			...
			// per-language `github.dardcor.enable` setting for that target too. The active document was
	...
	private async _trackSurvivalRate(item: LlmCompletionInfo) {
		...
		this._instantiationService.createInstance(
			...
			res => {
				...
				*/
				this._telemetryService.sendTelemetryEvent('reportInlineEditSurvivalRate', { microsoft: true, github: { eventNamePrefix: 'dardcor-nes/' } },
					{
