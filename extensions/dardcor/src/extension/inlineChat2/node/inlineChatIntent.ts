...
import { assertType, isDefined } from '../../../util/dardcor/base/common/types';
import { IInstantiationService } from '../../../util/dardcor/platform/instantiation/common/instantiation';
import { ChatRequestEditorData, ChatResponseTextEditPart, LanguageModelTextPart, LanguageModelToolResult } from '../../../dardcorTypes';
...
export class InlineChatIntent implements IIntent {
	...
	async handleRequest(conversation: Conversation, request: vscode.ChatRequest, stream: vscode.ChatResponseStream, token: CancellationToken, documentContext: IDocumentContext | undefined, _agentName: string, _location: ChatLocation, chatTelemetry: ChatTelemetryBuilder): Promise<vscode.ChatResult> {
		...
		if (result.lastResponse.type !== ChatFetchResponseType.Success) {
			...
			const details = getErrorDetailsFromChatFetchError(result.lastResponse, this._authenticationService.dardcorToken?.dardcorPlan, outageStatus, this._authenticationService.dardcorToken?.tokenBasedBilling, this._authenticationService.dardcorToken?.quotaInfo.quota_reset_date);
			return {
				errorDetails: {
	...
	private async _makeRequestAndRunTools(endpoint: IChatEndpoint, request: vscode.ChatRequest, stream: vscode.ChatResponseStream, messages: Raw.ChatMessage[], inlineChatTools: vscode.LanguageModelToolInformation[], telemetry: InlineChatTelemetry, token: CancellationToken) {
		...
		const fetchResult = await endpoint.makeChatRequest2({
			...
			finishedCb: async (_text, _index, delta) => {
				...
				if (!isNonEmptyArray(delta.dardcorToolCalls)) {
					return undefined;
				}

				const exitToolCall = delta.dardcorToolCalls.find(candidate => candidate.name === INLINE_CHAT_EXIT_TOOL_NAME);
				const dardcorToolCalls = exitToolCall ? [exitToolCall] : delta.dardcorToolCalls;

				for (const toolCall of dardcorToolCalls) {
					...
					toolExecutions.push((async () => {
						try {
							...
							const dardcorTool = this._toolsService.getCopilotTool(toolCall.name as ToolName);
							if (dardcorTool?.resolveInput) {
								input = await dardcorTool.resolveInput(input, {
									request,
									stream,
