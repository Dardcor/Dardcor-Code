...
import { ChatRequestEditorData } from '../../../../dardcorTypes';
...
suite('InlineChatIntent', () => {
	...
	test('Inline tool loop aggregates tokens, tool rounds, and metrics across recovered tool failures', async () => {
		...
		expect(harness.otelService.metrics).toEqual(expect.arrayContaining([
			{
				name: 'dardcor_chat.agent.invocation.duration',
				value: expect.closeTo(0.5, 2),
				attributes: { [GenAiAttr.AGENT_NAME]: 'Inline Chat' },
			},
			{
				name: 'dardcor_chat.agent.turn.count',
				value: 2,
				attributes: { [GenAiAttr.AGENT_NAME]: 'Inline Chat' },
			},
		]));
...
function createInlineChatHarness(options: InlineChatHarnessOptions = {}) {
		...
		const mockEndpoint = {
			...
			makeChatRequest2: vi.fn().mockImplementation(async (requestOptions: { finishedCb?: (text: string, index: number, delta: { dardcorToolCalls?: readonly TestToolCall[] }) => Promise<undefined> }) => {
				...
				if (response.toolCalls) {
					await requestOptions.finishedCb?.('', 0, { dardcorToolCalls: response.toolCalls });
				}
				return {
