...
export class SearchPanelCommands extends Disposable {
	constructor(
		...
		super();
		this._register(vscode.commands.registerCommand('github.dardcor.search.markHelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Helpful);
		}));
		this._register(vscode.commands.registerCommand('github.dardcor.search.markUnhelpful', () => {
			this.sendFeedback(SearchFeedbackKind.Unhelpful);
		}));
		this._register(vscode.commands.registerCommand('github.dardcor.search.feedback', () => {
			this.sendFeedback(SearchFeedbackKind.Feedback);
			vscode.commands.executeCommand('github.dardcor.report', `Copilot search feedback: "${SemanticSearchTextSearchProvider.latestQuery}"`);
		}));
	}
