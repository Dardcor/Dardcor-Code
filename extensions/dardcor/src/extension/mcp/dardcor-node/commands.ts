...
import { DeferredPromise, raceCancellation } from '../../../util/dardcor/base/common/async';
...
import { ChatLocation as VsCodeChatLocation } from '../../../dardcorTypes';
...
export class McpSetupCommands extends Disposable {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('github.dardcor.chat.mcp.setup.flow', async (args: { name: string }) => {
		...
		this._register(vscode.commands.registerCommand('github.dardcor.chat.mcp.setup.validatePackage', async (args: IValidatePackageArgs): Promise<ValidatePackageResult> => {
		...
		this._register(vscode.commands.registerCommand('github.dardcor.chat.mcp.setup.check', () => {
			return 1;
		}));
	}
