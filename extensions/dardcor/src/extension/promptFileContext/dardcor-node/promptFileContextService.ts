...
export class PromptFileContextContribution extends Disposable {
	...
	private async register(): Promise<IDisposable> {
		...
		try {
			...
			if (dardcorAPI) {
				disposables.add(dardcorAPI.registerContextProvider(provider));
			}
			...
			case PromptFileLangageId.agent: {
				...
				return [
					...
					},
					{
						...
						value: `vscode, github-dardcor`,
					},
					{
						name: '`handoffs` is optional and is a sequence of mappings with `label`, `agent`, `prompt`, `send`, and `model` properties. The `model` property uses the format `Model Name (vendor)` (e.g., `GPT-4.1 (dardcor)`)',
						value: [
							`handoffs:`,
