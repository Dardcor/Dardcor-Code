...
export class RemoteSessionExporter extends Disposable implements IExtensionContribution, ISessionSyncStateService {
	...
	constructor(
		...
		this._register(vscode.commands.registerCommand('github.dardcor.sessionSync.deleteSessions', () => this._deleteCloudSessions()));

		// Register cloud-only delete for sessions window hook (fire-and-forget, no UI)
		this._register(vscode.commands.registerCommand('github.dardcor.sessionSync.deleteSessionFromCloud', (sessionIds: string[]) => this._deleteSessionsFromCloud(sessionIds)));

		// Register suggest session sync command (called from chronicleIntent when user runs /chronicle)
		this._register(vscode.commands.registerCommand('github.dardcor.sessionSync.suggest', () => this._suggestSessionSync()));

		// Register cloud reindex command (called from chronicleIntent after local reindex)
		this._register(vscode.commands.registerCommand('github.dardcor.sessionSync.reindex', (reportProgress: (msg: string) => void, token: vscode.CancellationToken) => this._reindexCloud(reportProgress, token)));

		// Register user-facing reindex command (Command Palette)
		this._register(vscode.commands.registerCommand('github.dardcor.chronicle.reindex', () => this._reindexFromCommandPalette()));
