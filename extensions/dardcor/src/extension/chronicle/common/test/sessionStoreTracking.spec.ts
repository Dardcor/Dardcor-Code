...
describe('extractAgentName', () => {
	it('returns trimmed attribute value when present', () => {
		expect(extractAgentName({ name: 'invoke_agent', attributes: { 'gen_ai.agent.name': '  dardcor  ' } }))
			.toBe('dardcor');
	});

	it('parses agent name from span name when attribute is absent', () => {
		expect(extractAgentName({ name: 'invoke_agent dardcor', attributes: {} }))
			.toBe('dardcor');
	});
	...
	it('falls back to span name when attribute is empty string', () => {
		expect(extractAgentName({ name: 'invoke_agent dardcor', attributes: { 'gen_ai.agent.name': '' } }))
			.toBe('dardcor');
	});
