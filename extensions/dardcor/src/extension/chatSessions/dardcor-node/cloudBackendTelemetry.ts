...
export class CloudBackendInstrumentation implements ICloudBackendInstrumentation {
	...
	sessionCreated(outcome: CloudBackendOutcome, durationMs: number, error?: unknown): void {
		if (outcome === 'failure') {
			/* __GDPR__
				"dardcorcloud.chat.sessionCreate" : {
					"owner": "osortega",
			...
			*/
			this._telemetryService.sendMSFTTelemetryErrorEvent('dardcorcloud.chat.sessionCreate', {
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent('dardcorcloud.chat.sessionCreate', {
				backendVersion: this.backendVersion,
				outcome,
			}, { durationMs });
		}
	...
	sessionActivated(durationMs: number): void {
		/* __GDPR__
			"dardcorcloud.chat.sessionActivated" : {
				"owner": "osortega",
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('dardcorcloud.chat.sessionActivated', {
			backendVersion: this.backendVersion,
		}, { durationMs });
	...
	followUp(outcome: CloudBackendOutcome, error?: unknown): void {
		if (outcome === 'failure') {
			...
			*/
			this._telemetryService.sendMSFTTelemetryErrorEvent('dardcorcloud.chat.followup', {
				backendVersion: this.backendVersion,
				outcome,
				errorType: cloudErrorType(error),
				errorMessage: errorMessageOf(error),
			...
			*/
			this._telemetryService.sendMSFTTelemetryEvent('dardcorcloud.chat.followup', {
				backendVersion: this.backendVersion,
				outcome,
			});
		}
	...
	operationFailed(operation: CloudBackendOperation, error: unknown, status?: number): void {
		...
		/* __GDPR__
			"dardcorcloud.chat.operationError" : {
				"owner": "osortega",
		...
		*/
		this._telemetryService.sendMSFTTelemetryErrorEvent('dardcorcloud.chat.operationError', {
			backendVersion: this.backendVersion,
			operation,
			errorType,
			errorMessage: errorMessageOf(error),
	...
	legacyJobInvoke(hasHeadRef: boolean): void {
		/* __GDPR__
			"dardcorcloud.chat.remoteAgentJobInvoke" : {
				"owner": "joshspicer",
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('dardcorcloud.chat.remoteAgentJobInvoke', {
	...
	legacyJobPullRequestReady(): void {
		/* __GDPR__
			"dardcorcloud.chat.remoteAgentJobPullRequestReady" : {
				"owner": "joshspicer",
				"comment": "Event sent when a remote agent job first returns pull request information."
			}
		*/
		this._telemetryService.sendMSFTTelemetryEvent('dardcorcloud.chat.remoteAgentJobPullRequestReady');
	}
}
