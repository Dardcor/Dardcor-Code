...
// in dardcorCLITerminalIntegration.ps1 instead. This is because Electron on Windows
...
function getCopilotInfo(): { installed: true; version?: string } | undefined {
	const result = spawnSync('dardcor --version', { env, shell: true, encoding: 'utf8' });
...
function runBrew(label: string) {
	const result = spawnSync('brew', ['install', 'dardcor-cli'], { stdio: 'inherit', env });
	if (result.error) {
...
function runCurl(label: string) {
	const result = spawnSync('bash', ['-c', 'curl -fsSL https://gh.io/dardcor-install | bash'], { stdio: 'inherit', env });
	if (result.error) {
...
function runWget(label: string) {
	const result = spawnSync('bash', ['-c', 'wget -qO- https://gh.io/dardcor-install | bash'], { stdio: 'inherit', env });
	if (result.error) {
...
(async function main() {
	...
	if (!info) {
		...
		await pressKeyToExit('Try manually reinstalling (https://docs.github.com/en/dardcor/how-tos/set-up/install-dardcor-cli)');
	}
	...
	spawnSync('dardcor', args, { stdio: 'inherit', env });
	process.exit(0);
})();
