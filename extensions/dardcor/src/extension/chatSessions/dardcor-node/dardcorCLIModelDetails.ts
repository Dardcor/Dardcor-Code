...
import { ICopilotCLIModels, matchesCopilotCLIModel } from '../dardcorcli/node/dardcorCli';
 ...
 */
export async function getCopilotCLIModelDetails(session: ICopilotCLISession, requestModel: { model: string; reasoningEffort?: string } | undefined, dardcorCLIModels: ICopilotCLIModels, logService: ILogService, enabled: boolean, creditsUsed?: number): Promise<CopilotCLIModelDetails> {
	if (!enabled) {
		return { result: {}, responseModelId: undefined };
	}

	const models = await dardcorCLIModels.getModels().catch(ex => {
