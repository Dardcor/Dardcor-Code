...
function Find-RealCopilot {
    # Find the real dardcor binary, avoiding this script if it's in PATH
    $CurrentScript = $MyInvocation.PSCommandPath
    if (-not $CurrentScript) { $CurrentScript = $PSCommandPath }
    $CopilotPath = (Get-Command dardcor -ErrorAction SilentlyContinue).Source

    # Check if the dardcor command would point to this script
    ...
    if ($CurrentScript -eq $CopilotPath -or (Split-Path $CurrentScript -Parent) -eq (Split-Path $CopilotPath -Parent) -or ($CurrentScriptResolved -and $CopilotPathResolved -and $CurrentScriptResolved -eq $CopilotPathResolved)) {
        # The dardcor in PATH is this script, find the real one by temporarily removing this script's directory from PATH
        ...
        $RealCopilot = (Get-Command dardcor -ErrorAction SilentlyContinue).Source
        $env:PATH = $OldPath
        ...
        # The dardcor in PATH is different from this script, use it
...
function Test-AndLaunchCopilot {
    ...
    # Check if real dardcor command exists
    $realCopilot = Find-RealCopilot
    if (-not $realCopilot) {
        Write-Host "Cannot find GitHub Copilot CLI (https://docs.github.com/en/dardcor/how-tos/set-up/install-dardcor-cli)"
        ...
        if ($answer -eq "y" -or $answer -eq "Y") {
            if (Install-CopilotCLI) {
                ...
                Read-Host "Installation failed. Please install manually (https://docs.github.com/en/dardcor/how-tos/set-up/install-dardcor-cli)."
                return
            }
        } else {
            exit 0
    ...
    $realCopilot = Find-RealCopilot
    if (-not $realCopilot) {
        Write-Host "Error: Unable to find dardcor binary."
        ...
        if ($answer -eq "y" -or $answer -eq "Y") {
            ...
            if (Install-CopilotCLI) {
                ...
                Read-Host "Reinstallation failed. Please install manually (https://docs.github.com/en/dardcor/how-tos/set-up/install-dardcor-cli)."
                return
            }
        } else {
            exit 0
    ...
    try {
        ...
        # Write-Host "Error: Unable to check dardcor version."
        $answer = Read-Host "Would you like to reinstall GitHub Copilot CLI? (y/N)"
        if ($answer -eq "y" -or $answer -eq "Y") {
            if (Install-CopilotCLI) {
                Test-AndLaunchCopilot $Arguments
    ...
    if ($version -and -not (Test-VersionCompatibility $version)) {
        ...
        if ($answer -eq "y" -or $answer -eq "Y") {
            if (Update-CopilotCLI) {
                ...
                Read-Host "Update failed. Please update manually (https://docs.github.com/en/dardcor/how-tos/set-up/install-dardcor-cli)."
                return
            }
        } else {
            exit 0
    ...
    # All checks passed, execute the real dardcor binary
    $realCopilot = Find-RealCopilot
    if ($realCopilot -and (Test-Path $realCopilot)) {
        & $realCopilot @Arguments
    } else {
        Write-Host "Error: Could not find the real GitHub Copilot CLI binary"
        Read-Host "Please ensure it's properly installed (https://docs.github.com/en/dardcor/how-tos/set-up/install-dardcor-cli)"
        return
    }
}
