...
import { DeferredPromise, IntervalTimer, retry, RunOnceScheduler, timeout } from '../../../util/dardcor/base/common/async';
import { Event } from '../../../util/dardcor/base/common/event';
import { Disposable, DisposableStore, toDisposable } from '../../../util/dardcor/base/common/lifecycle';
import { ResourceMap } from '../../../util/dardcor/base/common/map';
import { joinPath } from '../../../util/dardcor/base/common/resources';
...
import { CONTINUE_TRUNCATION, extractTitle, getAuthorDisplayName, getRepoId, isActiveTaskState, SessionIdForPr, SessionIdForTask, toOpenPullRequestWebviewUri, truncatePrompt } from '../dardcor/dardcorCodingAgentUtils';
import { CloudAgentBackend, CloudSessionData, PullArtifactRef, TaskCloudAgentBackend, TaskContent } from '../dardcor/cloudAgentBackend';
import { CopilotCloudGitOperationsManager } from './dardcorCloudGitOperationsManager';
import { ChatSessionContentBuilder, SessionResponseLogChunk } from './dardcorCloudSessionContentBuilder';
...
const OPEN_REPOSITORY_COMMAND_ID = 'github.dardcor.chat.cloudSessions.openRepository';
const CLEAR_CACHES_COMMAND_ID = 'github.dardcor.chat.cloudSessions.clearCaches';
const CREATE_PULL_REQUEST_FOR_TASK_COMMAND_ID = 'github.dardcor.chat.cloudSessions.createPullRequestForTask';
const OPEN_PULL_REQUEST_FOR_TASK_COMMAND_ID = 'github.dardcor.chat.cloudSessions.openPullRequestForTask';
...
const CAN_CREATE_PULL_REQUEST_CONTEXT_KEY = 'github.dardcor.chat.cloudTaskCanCreatePullRequest';
/** Context key gating the chat-input "Open pull request" toolbar action: true once the viewed cloud task has a pull request. */
const CAN_OPEN_PULL_REQUEST_CONTEXT_KEY = 'github.dardcor.chat.cloudTaskCanOpenPullRequest';
...
const HARDCODED_PARTNER_AGENTS: { id: string; name: string; at?: string; assignableActorLogin?: string; codiconId?: string }[] = [
	{ id: DEFAULT_PARTNER_AGENT_ID, name: 'Copilot', assignableActorLogin: 'dardcor-swe-agent', codiconId: 'dardcor' },
...
export class CopilotCloudSessionsProvider extends Disposable implements vscode.ChatSessionContentProvider, vscode.ChatSessionItemProvider {
	public static readonly TYPE = 'dardcor-cloud-agent';
	...
	private readonly WORKSPACE_CONTEXT_PREFIX = 'dardcor.cloudAgent';
	...
	constructor(
		...
		this._register(this._domainService.onDidChangeDomains(e => {
			if (e.capiUrlChanged) {
				this.logService.debug('dardcorCloudSessionsProvider: CAPI URL changed, refreshing sessions');
		...
		getRepoId(this._gitService).then(async repoIds => {
			...
			this.telemetry.sendTelemetryEvent('dardcorCloudSessions.refreshInterval', { microsoft: true, github: false }, telemetryObj);
		});
	}
	...
	private registerCommands() {
		...
		this._register(vscode.commands.registerCommand('github.dardcor.chat.checkoutPullRequestReroute', checkoutPullRequestReroute));
		...
		this._register(vscode.commands.registerCommand('github.dardcor.chat.openPullRequestReroute', openPullRequestReroute));
		...
		this._register(vscode.commands.registerCommand(CLEAR_CACHES_COMMAND_ID, () => {
			this.logService.debug('dardcorCloudSessionsProvider#clearCaches: clearing all cloud agent caches');
			this.clearOptionsCaches();
			this.refresh();
			this._onDidChangeChatSessionProviderOptions.fire();
		}));
	...
	private getRefreshIntervalTime(hasHistoricalSessions: boolean): number {
		...
		const expRefreshInterval = this._experimentationService.getTreatmentVariable<number>('dardcorCloudSessions.refreshInterval');
	...
	private async checkCCAEnabled(owner: string, repo: string): Promise<CCAEnabledResult> {
		...
		if (cached !== undefined) {
			this.logService.trace(`dardcorCloudSessionsProvider#checkCCAEnabled: using cached CCA enabled status for ${owner}/${repo}: ${cached.enabled}`);
			return cached;
		}
		...
		this.telemetry.sendTelemetryEvent('dardcor.codingAgent.CCAIsEnabledCheck', { microsoft: true, github: false }, {
		...
		if (result.statusCode !== undefined && !CCA_KNOWN_STATUS_CODES.has(result.statusCode)) {
			/* __GDPR__
				"dardcor.codingAgent.CCAIsEnabledUnexpectedStatus" : {
					"owner": "joshspicer",
			...
			*/
			this.telemetry.sendTelemetryErrorEvent('dardcor.codingAgent.CCAIsEnabledUnexpectedStatus', { microsoft: true, github: false }, {
				statusCode: String(result.statusCode),
				isRateLimited: String(result.statusCode === 429),
			});
		}

		this.logService.trace(`dardcorCloudSessionsProvider#checkCCAEnabled: fetched CCA enabled status for ${owner}/${repo}: ${result.enabled}`);
		return result;
	}
	...
	async provideChatSessionProviderOptions(token: vscode.CancellationToken): Promise<vscode.ChatSessionProviderOptions> {
		this.logService.trace('dardcorCloudSessionsProvider#provideChatSessionProviderOptions Start');
		...
		if (!isCcaEnabled && repoId) {
			this.logService.trace(`dardcorCloudSessionsProvider#provideChatSessionProviderOptions: CCA disabled for ${repoId.org}/${repoId.repo}, statusCode: ${ccaEnabledResult?.statusCode}`);
		...
		if (cachedOptions) {
			this.logService.trace('dardcorCloudSessionsProvider#provideChatSessionProviderOptions: using cached options');
		...
		try {
			...
			if (models.status === 'fulfilled' && models.value.length > 0) {
				const isUBB = !!this._authenticationService.dardcorToken?.isUsageBasedBilling;
			...
			this.logService.debug(`dardcorCloudSessionsProvider#provideChatSessionProviderOptions: Returning options: ${JSON.stringify(optionGroups, undefined, 2)}`);
			return result;
		} catch (error) {
			this.logService.error(`[dardcorCloudSessionsProvider#provideChatSessionProviderOptions] Error fetching options: ${error}`);
			return { optionGroups: [] };
		}
	}
	...
	async provideChatSessionItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionItem[]> {
		...
		this.chatSessionItemsPromise = (async () => {
			const repoIds = await getRepoId(this._gitService);
			this.logService.debug(`dardcorCloudSessionsProvider#provideChatSessionItems: repoIds=${JSON.stringify(repoIds?.map(r => ({ org: r.org, repo: r.repo, host: r.host })))}, isAgentSessionsWorkspace=${vscode.workspace.isAgentSessionsWorkspace}`);
			...
			if (!vscode.workspace.isAgentSessionsWorkspace && !this.isGitHubRepoOrEmpty(repoIds)) {
				this.logService.debug('dardcorCloudSessionsProvider#provideChatSessionItems: not a GitHub repo, returning empty');
				return [];
			}
			const sessionList = await this._backend.fetchSessionList(repoIds, vscode.workspace.isAgentSessionsWorkspace, true);
			this.logService.debug(`dardcorCloudSessionsProvider#provideChatSessionItems: fetched ${sessionList.length} grouped sessions`);
			...
			vscode.commands.executeCommand('setContext', 'github.dardcor.chat.cloudSessionsEmpty', filteredSessions.length === 0);
			this.logService.debug(`dardcorCloudSessionsProvider#provideChatSessionItems: returning ${filteredSessions.length} sessions (${sessionItems.length - filteredSessions.length} filtered out)`);
	...
	async provideChatSessionContent(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		...
		const getProblemStatement = async (repoOwner: string, repoName: string, sessions: SessionInfo[]) => {
			...
			return prompt.replace(/@dardcor\s*/gi, '').trim();
		};
		if (!pr) {
	...
	async delegate(
		...
		return {
			...
			command: {
				...
				command: 'github.dardcor.chat.openPullRequestReroute',
	...
	private async handleConfirmationData(request: vscode.ChatRequest, stream: vscode.ChatResponseStream, context: vscode.ChatContext, token: vscode.CancellationToken) {
		...
		if (!selection || selection === this.CANCEL.toUpperCase() || token.isCancellationRequested) {
			/* __GDPR__
				"dardcorcloud.chat.confirmationCancelled" : {
					"owner": "joshspicer",
			...
			*/
			this.telemetry.sendMSFTTelemetryEvent('dardcorcloud.chat.confirmationCancelled', {
		...
		try {
			await vscode.window.withProgress(
				...
				async () => {
					...
					/* __GDPR__
						"dardcorcloud.chat.createPRFromTask" : {
							"owner": "osortega",
					...
					*/
					this.telemetry.sendMSFTTelemetryEvent('dardcorcloud.chat.createPRFromTask', {
	...
	private async chatParticipantImpl(request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) {
		...
		*/
		this.telemetry.sendMSFTTelemetryEvent('dardcorcloud.chat.invoke', {
	...
	private async addFollowUpToExistingPR(pullRequestNumber: number, userPrompt: string, summary?: string, targetAgent = 'dardcor'): Promise<string | undefined> {
		const backend = this._backend;
		if (backend.kind !== 'pr') {
			return undefined;
		}
		try {
			/* __GDPR__
				"dardcorcloud.chat.followupComment" : {
					"owner": "joshspicer",
			...
			*/
			this.telemetry.sendMSFTTelemetryEvent('dardcorcloud.chat.followupComment', {
				targetAgent,
			});
	...
	> {
		...
		if (isTruncated) {
			...
			/* __GDPR__
				"dardcor.codingAgent.truncation" : {
					"isCancelled" : { "classification": "SystemMetaData", "purpose": "FeatureInsight" }
				}
			*/
			this.telemetry.sendTelemetryEvent('dardcor.codingAgent.truncation', { microsoft: true, github: false }, {
