...
import { DeferredPromise, IntervalTimer, raceCancellation } from '../../../util/dardcor/base/common/async';
...
import { basename, dirname, extUri } from '../../../util/dardcor/base/common/resources';
...
import { clearPendingCopilotCLIRequestContext, setPendingCopilotCLIRequestContext, takePendingCopilotCLIRequestContext } from '../dardcorcli/common/pendingRequestContext';
import { SessionIdForCLI } from '../dardcorcli/common/utils';
import { getCopilotCLISessionDir } from '../dardcorcli/node/cliHelpers';
import { ICopilotCLIModels, ICopilotCLISDK } from '../dardcorcli/node/dardcorCli';
...
import { ICopilotCLIChatSessionInitializer, SessionInitOptions } from '../dardcorcli/dardcor-node/dardcorCLIChatSessionInitializer';
...
export class CopilotCLIChatSessionContentProvider extends Disposable implements vscode.ChatSessionContentProvider, ICopilotCLIChatSessionItemProvider {
		...
		if (controller.onDidChangeChatSessionItemState) {
			this._register(controller.onDidChangeChatSessionItemState(async (item) => {
				...
				if (item.archived) {
					...
					const worktreePath = await this.dardcorCLIWorktreeManagerService.getWorktreePath(sessionId);
					if (worktreePath) {
					...
					try {
						const result = await this.dardcorCLIWorktreeManagerService.cleanupWorktreeOnArchive(sessionId);
					...
					try {
						const result = await this.dardcorCLIWorktreeManagerService.recreateWorktreeOnUnarchive(sessionId);
	...
	public async toChatSessionItem(session: ICopilotCLISessionItem, options?: { readonly includeChanges?: boolean }, token?: vscode.CancellationToken): Promise<vscode.ChatSessionItem> {
		...
		let worktreeProperties = await raceCancellation(this.dardcorCLIWorktreeManagerService.getWorktreeProperties(session.id), token);
		...
		if (options?.includeChanges || ((await this.hasCachedChanges(session.id, worktreeProperties)))) {
			...
			worktreeProperties = await raceCancellation(this.dardcorCLIWorktreeManagerService.getWorktreeProperties(session.id), token);
			if (token.isCancellationRequested) {
				return item;
			}
	...
	private async hasCachedChanges(sessionId: string, worktreeProperties: Awaited<ReturnType<IChatSessionWorktreeService['getWorktreeProperties']>>): Promise<boolean> {
		...
		const [hasCachedWorktreeChanges, hasCachedWorkspaceChanges] = await Promise.all([
			this.dardcorCLIWorktreeManagerService.hasCachedChanges(sessionId),
	...
	private async buildChanges(
		...
		if (worktreeProperties?.repositoryPath && await vscode.workspace.isResourceTrusted(vscode.Uri.file(worktreeProperties.repositoryPath))) {
			...
			changes.push(...(await raceCancellation(this.dardcorCLIWorktreeManagerService.getWorktreeChanges(sessionId), token) ?? []));
	...
	async provideChatSessionContent(resource: Uri, token: vscode.CancellationToken, context?: { readonly inputState: vscode.ChatSessionInputState }): Promise<vscode.ChatSession> {
		...
		try {
			...
			if (dardcorcliSessionId.startsWith('untitled:') || dardcorcliSessionId.startsWith('untitled-')) {
				return {
					history: [],
					requestHandler: undefined,
				};
			}
			if (this.sessionService.isNewSessionId(dardcorcliSessionId)) {
				...
				this._prDetectionService.detectPullRequest(dardcorcliSessionId);
				...
				const [history, title, optionGroups] = await Promise.all([
					...
					this.sessionService.getSessionTitle(dardcorcliSessionId, token),
...
export class CopilotCLIChatSessionParticipant extends Disposable {
	...
	private async authenticate(): Promise<NonNullable<SessionOptions['authInfo']>> {
		const authInfo = await this.dardcorCLISDK.getAuthInfo().catch((ex) => this.logService.error(ex, 'Authorization failed'));
		if (!authInfo) {
	...
	private async resolveInput(
		...
		if (request.command && !request.prompt && !isNewSession) {
			const input = (dardcorCLICommands as readonly string[]).includes(request.command)
		...
		const input = (request.command && (dardcorCLICommands as readonly string[]).includes(request.command))
			? { command: request.command as CopilotCLICommand, prompt }
			: { prompt };
		return { input, attachments };
	}
...
export function registerCLIChatCommands(
	...
	dardcorCLIFolderMruService: IChatFolderMruService,
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.delete', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			...
			const worktreePath = await dardcorCLIWorktreeManagerService.getWorktreePath(id);
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		if (newTitle) {
			...
			if (trimmedTitle) {
				await dardcorCLISessionService.renameSession(id, trimmedTitle);
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.newSession', async () => {
		await createCopilotCLITerminal('editor', l10n.t('Dardcor AI CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.newSessionToSide', async () => {
		await createCopilotCLITerminal('editorBeside', l10n.t('Dardcor AI CLI'));
	}));
	disposableStore.add(vscode.commands.registerCommand(OPEN_IN_COPILOT_CLI_COMMAND_ID, async (sourceControlContext?: unknown) => {
		...
		await createCopilotCLITerminal('editor', l10n.t('Dardcor AI CLI'), rootUri?.fsPath);
	}));
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.copyWorktreeBranchName', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		const id = SessionIdForCLI.parse(sessionItem.resource);
		const worktreeProperties = await dardcorCLIWorktreeManagerService.getWorktreeProperties(id);
	...
	disposableStore.add(vscode.commands.registerCommand(OPEN_REPOSITORY_COMMAND_ID, async ({ inputState }: { inputState: vscode.ChatSessionInputState; sessionResource: vscode.Uri | undefined }) => {
		...
		const mruItems = await dardcorCLIFolderMruService.getRecentlyUsedFolders(CancellationToken.None);
		...
		if (!(await checkPathExists(selectedFolderUri, fileSystemService))) {
			await dardcorCLIFolderMruService.deleteRecentlyUsedFolder(selectedFolderUri);
