...
import { disposableTimeout } from '../../../util/dardcor/base/common/async';
...
import { windowsToGitBashPath } from '../../../util/dardcor/workbench/contrib/terminalContrib/suggest/browser/terminalGitBashHelpers';
import { PythonTerminalService } from './dardcorCLIPythonTerminalService';
import { CopilotCLITerminalLinkProvider, SessionDirResolver } from './dardcorCLITerminalLinkProvider';

//@ts-ignore
import powershellScript from './dardcorCLIShim.ps1';

const COPILOT_CLI_SHIM_JS = 'dardcorCLIShim.js';
const COPILOT_CLI_COMMAND = 'dardcor';
const COPILOT_ICON = new ThemeIcon('dardcor');
...
export class CopilotCLITerminalIntegration extends Disposable implements ICopilotCLITerminalIntegration {
	...
	private async initialize(): Promise<void> {
		...
		const storageLocation = path.join(globalStorageUri.fsPath, 'dardcorCli');
		this.terminalService.contributePath('dardcor-cli', storageLocation, { command: COPILOT_CLI_COMMAND }, true);
		...
		if (process.platform === 'win32') {
			...
			const dardcorPowershellScript = `@echo off
...
`;
			...
			await fs.writeFile(this.shellScriptPath, dardcorPowershellScript);
			...
			// support console stdin (see dardcorCLIShim.ts header). Instead, delegate to
			...
			const dardcorBashScript = `#!/bin/sh\nexec "${posixBatPath}" "$@"\n`;
			this.posixShellScriptPath = path.join(storageLocation, COPILOT_CLI_COMMAND);
			await fs.writeFile(this.posixShellScriptPath, dardcorBashScript);
		} else {
			const dardcorShellScript = `#!/bin/sh
unset NODE_OPTIONS
ELECTRON_RUN_AS_NODE=1 "${process.execPath}" "${path.join(storageLocation, COPILOT_CLI_SHIM_JS)}" "$@"`;
			...
			this.powershellScriptPath = path.join(storageLocation, `dardcorCLIShim.ps1`);
			await fs.writeFile(this.shellScriptPath, dardcorShellScript);
			await fs.writeFile(this.powershellScriptPath, powershellScript);
		...
		this._register(window.registerTerminalProfileProvider('dardcor-cli', { provideTerminalProfile }));
	...
	public async openTerminal(name: string, cliArgs: string[] = [], cwd?: string, location: TerminalOpenLocation = 'editor'): Promise<Terminal | undefined> {
		...
		if (shellPathAndArgs && (shellPathAndArgs.shell !== 'powershell' && shellPathAndArgs.shell !== 'pwsh')) {
			...
			if (terminal) {
				...
				const command = this.buildCommandForPythonTerminal(shellPathAndArgs?.dardcorCommand, cliArgs, shellPathAndArgs);
	...
	private sendTerminalOpenTelemetry(sessionType: string, shell: string, terminalCreationMethod: string, location: TerminalOpenLocation): void {
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent('dardcorcli.terminal.open', {
			sessionType,
			shell,
	...
	private buildCommandForPythonTerminal(dardcorCommand: string, cliArgs: string[], shellInfo: IShellInfo) {
		let commandPrefix = '';
		...
		return `${commandPrefix}${quoteArgsForShell(dardcorCommand, [])} ${cliArgs.join(' ')} ${exitCommand}`;
	}

	private buildCommandForTerminal(terminal: Terminal, dardcorCommand: string, cliArgs: string[]) {
		return `${quoteArgsForShell(dardcorCommand, [])} ${cliArgs.join(' ')}`;
	}
	...
	private async sendCommandToTerminal(terminal: Terminal, command: string, waitForPythonActivation: boolean, shellInfo: IShellInfo | undefined = undefined): Promise<void> {
		...
		try {
			...
			if (waitForPythonActivation) {
				...
				// Else if we send too early, the dardcor command might not get executed properly.
	...
	private async getShellInfo(cliArgs: string[]): Promise<IShellInfo | undefined> {
		...
		if (shellBasename === 'zsh' && this.shellScriptPath) {
			return {
				...
				iconPath,
				dardcorCommand: this.shellScriptPath,
				exitCommand: `&& exit`
			};
			...
			return {
				...
				iconPath,
				dardcorCommand: this.powershellScriptPath,
				exitCommand: `&& exit`
			};
