...
import { extractTitle, SessionIdForPr, SessionIdForTask } from '../dardcor/dardcorCodingAgentUtils';
 ...
 * `dardcor-developer`; the monolith uses `dardcor-swe-agent` for the same agent. Tasks owned by
 * any other surface — `dardcor-developer-cli` (Copilot CLI), `vscode-chat` (VS Code) or
 ...
 * `agent-profile.ts`.
 */
const CLOUD_CODING_AGENT_SLUGS: ReadonlySet<string> = new Set(['dardcor-developer', 'dardcor-swe-agent']);
 ...
 * `github.dardcor.chat.cloudAgentBackend.version` setting set to `v2`. HTTP requests
...
export class TaskApiBackend implements TaskCloudAgentBackend {
	...
	async fetchSessionList(repoIds: GithubRepoId[] | undefined, isAgentWorkspace: boolean, _refresh: boolean): Promise<CloudSessionData[]> {
		...
		if (isAgentWorkspace || !repoIds || repoIds.length === 0) {
			...
			// the current user's own tasks via `creator_id`, matching the github.com/dardcor/agents
