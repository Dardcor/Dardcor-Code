...
import { autorun, derived, IObservable, ISettableObservable, observableFromEvent, observableValue } from '../../../util/dardcor/base/common/observable';
...
import { IClaudeSlashCommandService } from '../claude/dardcor-node/claudeSlashCommandService';
...
import '../claude/dardcor-node/toolPermissionHandlers/index';
...
import '../claude/dardcor-node/mcpServers/index';
...
export class ClaudeChatSessionItemController extends Disposable {
	...
	constructor(
		...
		this._autoPermissionsEnabled = observableFromEvent(
			...
			() => _configurationService.getExperimentBasedConfig(ConfigKey.ClaudeAgentAllowAutoPermissions, experimentationService)
				...
				&& (_authenticationService.dardcorToken?.isEditorPreviewFeaturesEnabled() ?? true),
		);
	...
	private _registerCommands(): void {
		this._register(vscode.commands.registerCommand('github.dardcor.claude.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		...
		this._registerPromptCommand('github.dardcor.claude.sessions.commit', builtinSlashCommands.commit);
		this._registerPromptCommand('github.dardcor.claude.sessions.commitAndSync', `${builtinSlashCommands.commit} and ${builtinSlashCommands.sync}`);
		this._registerPromptCommand('github.dardcor.claude.sessions.sync', builtinSlashCommands.sync);

		this._register(vscode.commands.registerCommand('github.dardcor.claude.sessions.initializeRepository', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
			const resource = getSessionResource(sessionItemOrResource);
			if (!resource) {
				return;
			}
