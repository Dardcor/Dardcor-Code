 ...
 * 4. `dardcor-worktree-*` branches excluded
...
import { isWelcomeView } from '../dardcorcli/node/dardcorCli';
...
export const OPEN_REPOSITORY_COMMAND_ID = 'github.dardcor.cli.sessions.openRepository';
...
const COPILOT_WORKTREE_PATTERN = 'dardcor-worktree-';
...
export class SessionOptionGroupBuilder implements ISessionOptionGroupBuilder {
	...
	constructor(
		...
		@IChatFolderMruService private readonly dardcorCLIFolderMruService: IChatFolderMruService,
	...
	async provideChatSessionProviderOptionGroups(previousInputState: vscode.ChatSessionInputState | undefined, selectedFolderUri?: vscode.Uri): Promise<vscode.ChatSessionProviderOptionGroup[]> {
		...
		if (isWelcomeView(this.workspaceService)) {
			...
			const repositories = await this.dardcorCLIFolderMruService.getRecentlyUsedFolders(CancellationToken.None);
	...
	async buildExistingSessionInputStateGroups(resource: vscode.Uri, token: vscode.CancellationToken): Promise<vscode.ChatSessionProviderOptionGroup[]> {
		...
		const optionGroups: vscode.ChatSessionProviderOptionGroup[] = [];
		const folderInfo = await this.folderRepositoryManager.getFolderRepository(dardcorcliSessionId, undefined, token);
		const repositories = isWelcomeView(this.workspaceService) ? folderMRUToChatProviderOptions(await this.dardcorCLIFolderMruService.getRecentlyUsedFolders(token)) : this.getRepositoryOptionItems();
		...
		const worktreeProperties = await this.chatSessionWorktreeService.getWorktreeProperties(dardcorcliSessionId);
