...
import { ICopilotCLIChatSessionInitializer } from '../../dardcorcli/dardcor-node/dardcorCLIChatSessionInitializer';
import { ICopilotCLISessionTracker } from '../../dardcorcli/dardcor-node/dardcorCLISessionTracker';
import { CopilotCLIChatSessionContentProvider, CopilotCLIChatSessionParticipant, resolveBranchLockState, resolveBranchSelection, resolveIsolationSelection, resolveSessionDirsForTerminal } from '../dardcorCLIChatSessions';
...
vi.mock('../dardcorCLIShim.ps1', () => ({ default: '# mock powershell script' }));
...
describe('CopilotCLIChatSessionContentProvider', () => {
	...
	it('triggers pull request detection when opening an existing session', async () => {
		...
		await provider.provideChatSessionContent(
			URI.from({ scheme: 'dardcorcli', path: '/session-1' }),
