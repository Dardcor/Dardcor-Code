...
import { Attachment, SessionOptions, SweCustomAgent } from '@github/dardcor/sdk';
...
import { sep } from '../../../../util/dardcor/base/common/path';
import { URI } from '../../../../util/dardcor/base/common/uri';
import { IInstantiationService, ServicesAccessor } from '../../../../util/dardcor/platform/instantiation/common/instantiation';
import { LanguageModelTextPart, LanguageModelToolResult2 } from '../../../../dardcorTypes';
...
import { type CopilotCLIModelInfo, type ICopilotCLIModels, type ICopilotCLISDK } from '../../dardcorcli/node/dardcorCli';
import { CopilotCLIPromptResolver } from '../../dardcorcli/node/dardcorcliPromptResolver';
import { CopilotCLISession, CopilotCLISessionInput, ICopilotCLISession } from '../../dardcorcli/node/dardcorcliSession';
import { CopilotCLISessionService, CopilotCLISessionWorkspaceTracker, ICopilotCLISessionService } from '../../dardcorcli/node/dardcorcliSessionService';
import { ICopilotCLIMCPHandler } from '../../dardcorcli/node/mcpHandler';
import { MockCliSdkSession, MockCliSdkSessionManager, MockSkillLocations, NullCopilotCLIAgents, NullICopilotCLIImageSupport } from '../../dardcorcli/node/test/testHelpers';
import { IQuestion, IQuestionAnswer, IUserQuestionHandler } from '../../dardcorcli/node/userInputHelpers';
import { CustomSessionTitleService } from '../../dardcorcli/dardcor-node/customSessionTitleServiceImpl';
import { CopilotCLIChatSessionContentProvider, CopilotCLIChatSessionItemProvider, CopilotCLIChatSessionParticipant } from '../dardcorCLIChatSessionsContribution';
...
vi.mock('../dardcorCLITerminalIntegration', () => {
...
function createChatContext(sessionId: string, isUntitled: boolean, ...requests: TestChatRequest[]): vscode.ChatContext {
	...
	const normalizedSessionId = isUntitled && !sessionId.startsWith('untitled-')
		...
		: sessionId;
	const resource = vscode.Uri.from({ scheme: 'dardcorcli', path: `/${normalizedSessionId}` });
...
describe('CopilotCLIChatSessionParticipant.handleRequest', () => {
	...
	beforeEach(async () => {
		...
		// re-invoking handleRequest with the dardcorcli:// resource. This matches
		...
		mockExecuteCommand.mockImplementation(async (command: string, args: any) => {
			if (command === 'workbench.action.chat.openSessionWithPrompt.dardcorcli') {
	...
	it('applies live permissionLevel option changes to an active session', async () => {
		...
		await provider.provideHandleOptionsChange(Uri.parse('dardcorcli:/untitled-session'), [
	...
	it('scopes live permissionLevel changes to the targeted session', async () => {
		...
		await provider.provideHandleOptionsChange(Uri.parse('dardcorcli:/resource-b'), [
			{ optionId: 'permissionLevel', value: 'autopilot' }
		], disposables.add(new CancellationTokenSource()).token);
	...
	it('defers worktree handleRequestCompleted until all steering requests complete', async () => {
		...
		const sessionResource = vscode.Uri.from({ scheme: 'dardcorcli', path: `/${sessionId}` });
		const stream = new MockChatResponseStream();
	...
	it('defers untitled session swap while a steering request is still pending', async () => {
		...
		const steeringSessionResource = vscode.Uri.from({ scheme: 'dardcorcli', path: '/untitled-temp-steering' });
		const stream = new MockChatResponseStream();
		...
		otherRequest.sessionResource = vscode.Uri.from({ scheme: 'dardcorcli', path: `/${otherSessionId}` });
	...
	it('starts a new chat session and submits the request', async () => {
		...
		expect(mockExecuteCommand).toHaveBeenCalledWith(
			'workbench.action.chat.openSessionWithPrompt.dardcorcli',
	...
	describe('chatSessionContext lost workaround (core bug)', () => {
		...
		//    then calls vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.dardcorcli', ...)
		// 3. VS Code core opens the new session and calls handleRequest again with the dardcorcli:// resource,
		//    but due to a core bug chatSessionContext may be undefined
		// 4. The workaround detects the dardcorcli:// scheme + stored contextForRequest data and
		...
		beforeEach(() => {
			...
			// calling handleRequest again with the dardcorcli:// resource but with chatSessionContext lost.
			mockExecuteCommand.mockImplementation(async (command: string, args: any) => {
				if (command === 'workbench.action.chat.openSessionWithPrompt.dardcorcli') {
		...
		it('full delegation round-trip: executeCommand triggers callback that uses workaround to reconstruct context and reuse session', async () => {
			...
			// the dardcorcli:// resource but chatSessionContext=undefined (the core bug).
			...
			expect(mockExecuteCommand).toHaveBeenCalledWith(
				'workbench.action.chat.openSessionWithPrompt.dardcorcli',
				expect.objectContaining({
					resource: expect.objectContaining({ scheme: 'dardcorcli' }),
		...
		it('does not attempt workaround for non-dardcorcli resource and proceeds with normal delegation', async () => {
			const request = new TestChatRequest('do some work');
			// Default sessionResource is test://session/... (not dardcorcli scheme),
	...
	describe('PR detection with retry', () => {
		...
		const v2WorktreeProperties: ChatSessionWorktreePropertiesV2 = {
			version: 2,
			baseCommit: 'abc123',
			branchName: 'dardcor/test-branch',
			baseBranchName: 'main',
			repositoryPath: `${sep}repo`,
			worktreePath: `${sep}worktree`,
		};
	...
	describe('sdkToUntitledUriMapping lifecycle', () => {
		...
		it('maps SDK session ID to the original untitled URI', async () => {
			...
			expect(capturedUri!.scheme).toBe('dardcorcli');
