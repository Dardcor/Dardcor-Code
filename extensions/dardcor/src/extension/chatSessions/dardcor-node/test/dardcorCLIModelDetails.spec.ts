...
import type { ICopilotCLISession } from '../../dardcorcli/node/dardcorcliSession';
import type { ICopilotCLIModels, CopilotCLIModelInfo } from '../../dardcorcli/node/dardcorCli';
...
describe('persistCopilotCLIResponseModelId', () => {
	it('persists responseModelId and creditsUsed so they are readable immediately after', async () => {
		...
		// Simulate dardcorcliSession.ts writing dardcorRequestId first (as happens in production)
		await store.updateRequestDetails('session-1', [{ vscodeRequestId: 'req-1', dardcorRequestId: 'sdk-1', toolIdEditMap: {} }]);
