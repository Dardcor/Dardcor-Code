...
vi.mock('../dardcorCLIPythonTerminalService', () => ({
...
vi.mock('../dardcorCLITerminalLinkProvider', () => ({
...
import { CopilotCLITerminalIntegration } from '../dardcorCLITerminalIntegration';
...
class TestExtensionContext {
	...
	extension = { id: 'GitHub.dardcor-chat' };
	extensionUri = { fsPath: '/tmp/extensions/dardcor-chat' };
...
describe('CopilotCLITerminalIntegration', () => {
	...
	describe('openTerminal', () => {
		...
		it('should set sessionType to "new" when no cliArgs provided', async () => {
			...
			const event = telemetryService.events.find(e => e.name === 'dardcorcli.terminal.open');
	...
	describe('initialize', () => {
		it('should contribute path to terminal service', async () => {
			expect(terminalService.contributePathSpy).toHaveBeenCalledWith(
				'dardcor-cli',
				expect.stringContaining('dardcorCli'),
				expect.objectContaining({ command: 'dardcor' }),
				true,
			);
		});
