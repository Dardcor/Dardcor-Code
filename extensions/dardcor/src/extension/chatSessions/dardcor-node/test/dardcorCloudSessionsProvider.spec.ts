...
import type { AgentTask, AgentTaskCreateRequest, AgentTaskGetResponse, AgentTaskListEventsResponse, AgentTaskListResponse, AgentTaskSessionEvent, AgentTaskState, AgentTaskSteerRequest, AgentTaskCreatePullRequestResponse } from '@vscode/dardcor-api';
import { GithubRepoId, IGitService } from '../../../../platform/git/common/gitService';
import { PullRequestSearchItem, SessionInfo } from '../../../../platform/github/common/githubAPI';
import { TestLogService } from '../../../../platform/testing/common/testLogService';
import { mock } from '../../../../util/common/test/simpleMock';
import { ChatRequestTurn2, ChatResponseMarkdownPart, ChatResponseTurn2, ChatToolInvocationPart } from '../../../../dardcorTypes';
import { ITaskApiClient, ListTaskEventsOptions, ListTasksOptions } from '../../common/taskApiTypes';
import { ChatSessionContentBuilder, extractTaskErrorDetail, formatTaskStoppedMessage } from '../dardcorCloudSessionContentBuilder';
import { getCloudSessionItemMetadata, normalizeInitialSessionOptions, parseSessionLogChunksSafely, taskStateToChatSessionStatus } from '../dardcorCloudSessionsProvider';
import { TaskApiBackend, parseRepoFromTaskUrl, isCloudCodingAgentTask } from '../taskApiBackend';
import { isActiveTaskState, isFailedTaskState } from '../../dardcor/dardcorCodingAgentUtils';
import { NullCloudBackendInstrumentation } from '../cloudBackendTelemetry';
import { MockOctoKitService } from '../../../agents/dardcor-node/test/mockOctoKitService';
...
function createPullRequest(): PullRequestSearchItem {
	return {
		...
		fullDatabaseId: 1,
		headRefOid: 'abc123',
		headRefName: 'dardcor/test-branch',
		baseRefName: 'main',
		body: 'Body',
	};
}
...
describe('dardcorCloudSessionsProvider helpers', () => {
	it('coerces object-shaped initialSessionOptions into option entries', () => {
		const logService = new RecordingLogService();
		const sessionResource = vscode.Uri.parse('dardcor-cloud-agent:/1');
	...
	it('includes the task branch in PR-less cloud session metadata', () => {
		expect(getCloudSessionItemMetadata(
			...
			{ owner: 'microsoft', repo: 'vscode', baseRef: 'main', headRef: 'dardcor/task-branch' },
		)).toEqual({
			owner: 'microsoft',
			name: 'vscode',
			host: 'github.com',
			branch: 'dardcor/task-branch',
		});
	});
});
...
describe('ChatSessionContentBuilder', () => {
	it('ignores malformed tool_calls payloads instead of throwing', async () => {
		const builder = new ChatSessionContentBuilder('dardcor-cloud-agent', new TestGitService(), new TestLogService());
		const logs = [
...
describe('ChatSessionContentBuilder Task API history', () => {
	const newBuilder = () =>
		new ChatSessionContentBuilder('dardcor-cloud-agent', new TestGitService(), new TestLogService());
...
describe('TaskApiBackend', () => {
	...
	it('fetchSessionList resolves a global-list task repo by numeric id when it has no html_url', async () => {
		...
		const client = new FakeTaskApiClient({
			globalTasks: [
				{ id: 'g1', state: 'completed', created_at: '2026-03-27T00:00:00Z', agent_collaborators: [{ slug: 'dardcor-developer' }], repository: { id: 123 } } as unknown as AgentTask,
				{ id: 'g2', state: 'completed', created_at: '2026-03-27T00:00:00Z', agent_collaborators: [{ slug: 'dardcor-developer' }], repository: { id: 123 } } as unknown as AgentTask,
			],
		});
		const octoKitService = new MockOctoKitService();
	...
	it('fetchSessionList carries the raw task lifecycle state so settled tasks are not shown as in_progress', async () => {
		...
		const client = new FakeTaskApiClient({ repoTasks: [{ id: 't-idle', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'dardcor-developer' }] } as unknown as AgentTask] });
		const octoKitService = new MockOctoKitService();
		octoKitService.getCurrentAuthedUser = async () => ({ id: 4242, login: 'octocat', name: 'The Octocat', avatar_url: '' });
		const backend = new TaskApiBackend(client, new TestLogService(), octoKitService, NullCloudBackendInstrumentation);
	...
	it('fetchSessionList shows only cloud coding agent tasks and excludes local-client tasks (CLI / VS Code / JetBrains)', async () => {
		const repoTasks = [
			{ id: 'cloud-dev', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'dardcor-developer' }] },
			{ id: 'cloud-swe', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'dardcor-swe-agent' }] },
			{ id: 'cli', state: 'idle', created_at: '2026-03-27T00:00:00Z', creator: { id: 4242 }, agent_collaborators: [{ slug: 'dardcor-developer-cli' }] },
...
describe('isCloudCodingAgentTask', () => {
	it('keeps cloud coding agent slugs and rejects local-client / missing / malformed slugs', () => {
		...
		expect({
			'dardcor-developer': classify([{ slug: 'dardcor-developer' }]),
			'dardcor-swe-agent': classify([{ slug: 'dardcor-swe-agent' }]),
			'dardcor-developer-cli': classify([{ slug: 'dardcor-developer-cli' }]),
			...
			'dardcor-developer': true,
			'dardcor-swe-agent': true,
			'dardcor-developer-cli': false,
			'vscode-chat': false,
