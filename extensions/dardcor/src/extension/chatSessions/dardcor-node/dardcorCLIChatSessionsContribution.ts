...
import type { Attachment, SessionOptions, SweCustomAgent } from '@github/dardcor/sdk';
...
import { DeferredPromise, disposableTimeout, IntervalTimer, raceCancellation, SequencerByKey } from '../../../util/dardcor/base/common/async';
...
import { Disposable, DisposableStore, IDisposable, IReference } from '../../../util/dardcor/base/common/lifecycle';
import { relative } from '../../../util/dardcor/base/common/path';
import { basename, dirname, extUri, isEqual } from '../../../util/dardcor/base/common/resources';
...
import { IChatDelegationSummaryService } from '../dardcorcli/common/delegationSummaryService';
import { getCopilotCLISessionDir } from '../dardcorcli/node/cliHelpers';
import { COPILOT_CLI_CONTEXT_SIZE_PROPERTY, COPILOT_CLI_REASONING_EFFORT_PROPERTY, ICopilotCLIAgents, ICopilotCLIModels, ICopilotCLISDK, isWelcomeView, resolveContextTier } from '../dardcorcli/node/dardcorCli';
import { CopilotCLIPromptResolver } from '../dardcorcli/node/dardcorcliPromptResolver';
import { builtinSlashSCommands, CopilotCLICommand, dardcorCLICommands, CopilotCLIQuotaExceededError, ICopilotCLISession } from '../dardcorcli/node/dardcorcliSession';
import { ICopilotCLISessionItem, ICopilotCLISessionService } from '../dardcorcli/node/dardcorcliSessionService';
import { buildMcpServerMappings } from '../dardcorcli/node/mcpHandler';
import { ICopilotCLISessionTracker } from '../dardcorcli/dardcor-node/dardcorCLISessionTracker';
import { ICopilotCLIChatSessionItemProvider } from './dardcorCLIChatSessions';
import { getCopilotCLIModelDetails, persistCopilotCLIResponseModelId } from './dardcorCLIModelDetails';
import { ICopilotCLITerminalIntegration, TerminalOpenLocation } from './dardcorCLITerminalIntegration';
...
import { convertReferenceToVariable } from '../dardcorcli/dardcor-node/dardcorCLIPromptReferences';
...
const LAST_USED_ISOLATION_OPTION_KEY = 'github.dardcor.cli.lastUsedIsolationOption';
const OPEN_REPOSITORY_COMMAND_ID = 'github.dardcor.cli.sessions.openRepository';
const OPEN_IN_COPILOT_CLI_COMMAND_ID = 'github.dardcor.cli.openInCopilotCLI';
...
namespace SessionIdForCLI {
	export function getResource(sessionId: string): vscode.Uri {
		return vscode.Uri.from({
			scheme: 'dardcorcli', path: `/${sessionId}`,
		});
	}
	...
	export function isCLIResource(resource: vscode.Uri): boolean {
		return resource.scheme === 'dardcorcli';
	}
}
...
export class CopilotCLIChatSessionItemProvider extends Disposable implements vscode.ChatSessionItemProvider, ICopilotCLIChatSessionItemProvider {
	...
	constructor(
		@ICopilotCLISessionService private readonly dardcorcliSessionService: ICopilotCLISessionService,
		...
		if (configurationService.getConfig(ConfigKey.Advanced.CLIChatLazyLoadSessionItem)) {
			this.resolveChatSessionItem = async (item: vscode.ChatSessionItem, token: vscode.CancellationToken): Promise<vscode.ChatSessionItem | undefined> => {
				...
				const session = await this.dardcorcliSessionService.getSessionItem(sessionId, token);
		...
		this._register(this.dardcorcliSessionService.onDidChangeSessions(() => {
	...
	public async provideChatSessionItems(token: vscode.CancellationToken): Promise<vscode.ChatSessionItem[]> {
		...
		const sessions = await this.dardcorcliSessionService.getAllSessions(token);
		...
		void this.commandExecutionService.executeCommand('setContext', 'github.dardcor.chat.cliSessionsEmpty', count === 0);
...
export class CopilotCLIChatSessionContentProvider extends Disposable implements vscode.ChatSessionContentProvider {
	...
	constructor(
		...
		@ICopilotCLIAgents private readonly dardcorCLIAgents: ICopilotCLIAgents,
		@ICopilotCLISessionService private readonly sessionService: ICopilotCLISessionService,
		@IChatSessionWorktreeService private readonly dardcorCLIWorktreeManagerService: IChatSessionWorktreeService,
		...
		this._register(this.dardcorCLIAgents.onDidChangeAgents(() => {
			this._onDidChangeChatSessionProviderOptions.fire();
		}));
	}
	...
	private async getDefaultUntitledSessionRepositoryOption(dardcorcliSessionId: string | undefined, token: vscode.CancellationToken) {
		...
		const folderInfo = dardcorcliSessionId ? await this.folderRepositoryManager.getFolderRepository(dardcorcliSessionId, undefined, token) : undefined;
	...
	async provideChatSessionContent(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		const stopwatch = new StopWatch();
		try {
			const dardcorcliSessionId = SessionIdForCLI.parse(resource);
			const isUntitled = this.sessionItemProvider.isNewSession(dardcorcliSessionId);
			if (isUntitled) {
	...
	async provideChatSessionContentForUntitledSession(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		const dardcorcliSessionId = SessionIdForCLI.parse(resource);
		this._currentSessionId = dardcorcliSessionId;
		const folderRepo = await this.folderRepositoryManager.getFolderRepository(dardcorcliSessionId, undefined, token);
		const isUntitled = this.sessionItemProvider.isNewSession(dardcorcliSessionId);
		const [history, title] = await Promise.all([
			isUntitled ? Promise.resolve([]) : this.getSessionHistory(dardcorcliSessionId, folderRepo, token),
			this.customSessionTitleService.getCustomSessionTitle(dardcorcliSessionId),
		]);

		const options: Record<string, string | vscode.ChatSessionProviderOptionItem> = {};

		// Use FolderRepositoryManager to get folder/repository info (no trust check needed for UI population)
		const defaultRepo = await this.getDefaultUntitledSessionRepositoryOption(dardcorcliSessionId, token);
		if (defaultRepo) {
			...
			if (defaultRepoIsTrusted) {
				...
				this.folderRepositoryManager.setNewSessionFolder(dardcorcliSessionId, defaultRepo);
				...
				if (repoInfo.repository && isIsolationOptionFeatureEnabled(this.configurationService)) {
					if (!_sessionIsolation.has(dardcorcliSessionId)) {
						const lastUsed = this.context.globalState.get<IsolationMode>(LAST_USED_ISOLATION_OPTION_KEY, IsolationMode.Workspace);
						_sessionIsolation.set(dardcorcliSessionId, lastUsed);
					}
					const isolationMode = _sessionIsolation.get(dardcorcliSessionId)!;
				...
				const shouldShowBranch = !isIsolationOptionFeatureEnabled(this.configurationService) || _sessionIsolation.get(dardcorcliSessionId) === IsolationMode.Worktree;
				const branchItems = await this.getBranchOptionItems();
				if (branchItems.length > 0 && shouldShowBranch) {
					_sessionBranch.set(dardcorcliSessionId, branchItems[0].id);
	...
	async provideChatSessionContentForExistingSession(resource: Uri, token: vscode.CancellationToken): Promise<vscode.ChatSession> {
		...
		void this.sessionItemProvider.detectPullRequestOnSessionOpen(dardcorcliSessionId);

		const folderRepo = await this.folderRepositoryManager.getFolderRepository(dardcorcliSessionId, undefined, token);
		const [history, title, folderInfo, worktreeProperties] = await Promise.all([
			this.getSessionHistory(dardcorcliSessionId, folderRepo, token),
			this.customSessionTitleService.getCustomSessionTitle(dardcorcliSessionId),
			this.folderRepositoryManager.getFolderRepository(dardcorcliSessionId, undefined, token),
			this.dardcorCLIWorktreeManagerService.getWorktreeProperties(dardcorcliSessionId)
		]);
		...
		_sessionIsolation.set(dardcorcliSessionId, IsolationMode.Workspace);
		if (worktreeProperties?.repositoryPath) {
			_sessionIsolation.set(dardcorcliSessionId, IsolationMode.Worktree);
		...
		_sessionTurnCount.set(dardcorcliSessionId, history.length);
...
export class CopilotCLIChatSessionParticipant extends Disposable {
	...
	constructor(
		...
		@IGitService private readonly gitService: IGitService,
		@ICopilotCLIModels private readonly dardcorCLIModels: ICopilotCLIModels,
		@ICopilotCLIAgents private readonly dardcorCLIAgents: ICopilotCLIAgents,
		@ICopilotCLISessionService private readonly sessionService: ICopilotCLISessionService,
		@IChatSessionWorktreeService private readonly dardcorCLIWorktreeManagerService: IChatSessionWorktreeService,
		@IChatSessionWorktreeCheckpointService private readonly dardcorCLIWorktreeCheckpointService: IChatSessionWorktreeCheckpointService,
		@IChatSessionWorkspaceFolderService private readonly workspaceFolderService: IChatSessionWorkspaceFolderService,
		...
		@IFolderRepositoryManager private readonly folderRepositoryManager: IFolderRepositoryManager,
		@IConfigurationService private readonly configurationService: IConfigurationService,
		@ICopilotCLISDK private readonly dardcorCLISDK: ICopilotCLISDK,
		@IChatSessionMetadataStore private readonly chatSessionMetadataStore: IChatSessionMetadataStore,
		@ICustomSessionTitleService private readonly customSessionTitleService: ICustomSessionTitleService,
		@IOctoKitService private readonly octoKitService: IOctoKitService,
	...
	private sendTelemetryForHandleRequest(request: vscode.ChatRequest, chatSessionContext: vscode.ChatSessionContext | undefined): void {
		...
		*/
		this.telemetryService.sendMSFTTelemetryEvent('dardcorcli.chat.invoke', {
	...
	private async handleRequestImpl(request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<vscode.ChatResult | void> {
		...
		try {
			...
			const [authInfo,] = await Promise.all([this.dardcorCLISDK.getAuthInfo().catch((ex) => this.logService.error(ex, 'Authorization failed')), this.lockRepoOptionForSession(context, token)]);
			if (!authInfo) {
			...
			if (context.history.length === 0) {
				...
				await this.dardcorCLIWorktreeCheckpointService.handleRequest(session.object.sessionId);
			}
			...
			const isCopilotCLICommand = !!request.command && (dardcorCLICommands as readonly string[]).includes(request.command);
			...
			const { result, responseModelId } = await getCopilotCLIModelDetails(session.object, model, this.dardcorCLIModels, this.logService, modelDetailsEnabled, creditsUsed);
		...
		try {
			if (session.status === vscode.ChatSessionStatus.Completed) {
				...
				if (isIsolationEnabled(session.workspace)) {
					...
					await this.dardcorCLIWorktreeManagerService.handleRequestCompleted(session.sessionId);
				} else if (workingDirectory) {
				...
				await this.dardcorCLIWorktreeCheckpointService.handleRequestCompleted(session.sessionId, request.id);
				if (workingDirectory) {
					void clearChangesCacheForAffectedSessions(workingDirectory, [session.sessionId], this.logService, this.chatSessionMetadataStore, this.workspaceFolderService, this.dardcorCLIWorktreeManagerService, this.sessionItemProvider).catch(ex => this.logService.error(ex, 'Failed to clear changes cache after request completion'));
				}
			}
	...
	private async handlePullRequestCreated(session: ICopilotCLISession): Promise<void> {
		...
		const worktreeProperties = await this.dardcorCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		...
		try {
			await this.dardcorCLIWorktreeManagerService.setWorktreeProperties(sessionId, {
				...worktreeProperties,
	...
	private async getAgent(sessionId: string | undefined, request: vscode.ChatRequest | undefined, token: vscode.CancellationToken): Promise<SweCustomAgent | undefined> {
		...
		if (request?.modeInstructions2) {
			const customAgent = request.modeInstructions2.uri ? await this.dardcorCLIAgents.resolveAgent(request.modeInstructions2.uri.toString()) : await this.dardcorCLIAgents.resolveAgent(request.modeInstructions2.name);
			if (customAgent) {
	...
	private async getOrCreateSession(request: vscode.ChatRequest, chatSessionContext: vscode.ChatSessionContext, stream: vscode.ChatResponseStream, options: { model: { model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined; agent: SweCustomAgent | undefined; newBranch?: Promise<string | undefined>; sessionParentId?: string; permissionLevel?: string }, disposables: DisposableStore, token: vscode.CancellationToken): Promise<{ session: IReference<ICopilotCLISession> | undefined; trusted: boolean }> {
		...
		if (isNewSession) {
			...
			if (worktreeProperties) {
				void this.dardcorCLIWorktreeManagerService.setWorktreeProperties(session.object.sessionId, worktreeProperties);
			}
		}
	...
	private async getModelId(request: vscode.ChatRequest | undefined, token: vscode.CancellationToken): Promise<{ model: string; reasoningEffort?: string; contextTier?: 'default' | 'long_context' } | undefined> {
		...
		const model = promptFile?.header?.model ? await getModelFromPromptFile(promptFile.header.model, this.dardcorCLIModels) : undefined;
		if (token.isCancellationRequested) {
			return undefined;
		}
		if (model) {
		...
		const preferredModelInRequest = request?.model?.id ? await this.dardcorCLIModels.resolveModel(request.model.id) : undefined;
		if (preferredModelInRequest) {
			...
			const resolvedModels = await this.dardcorCLIModels.getModels();
		...
		const defaultModel = await this.dardcorCLIModels.getDefaultModel();
		if (!defaultModel) {
			return undefined;
		}
	...
	private async handleDelegationToCloud(session: ICopilotCLISession, request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, token: vscode.CancellationToken) {
		...
		const worktreeProperties = await this.dardcorCLIWorktreeManagerService.getWorktreeProperties(session.sessionId);
	...
	private async handleDelegationFromAnotherChat(
		...
		void vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.dardcorcli', {
...
export function registerCLIChatCommands(
	dardcorcliSessionItemProvider: CopilotCLIChatSessionItemProvider,
	dardcorCLISessionService: ICopilotCLISessionService,
	dardcorCLIWorktreeManagerService: IChatSessionWorktreeService,
	dardcorCLIWorktreeCheckpointService: IChatSessionWorktreeCheckpointService,
	gitService: IGitService,
	gitCommitMessageService: IGitCommitMessageService,
	gitExtensionService: IGitExtensionService,
	toolsService: IToolsService,
	dardcorCliWorkspaceSession: IChatSessionWorkspaceFolderService,
	...
	async function shouldKeepWorktreeForOtherSessions(sessionId: string, worktreePath: vscode.Uri): Promise<boolean> {
		const siblings = await getBlockingSiblingSessionsForFolder(
			...
			sessionId,
			metadataStore,
			dardcorCliWorkspaceSession,
		);
		return siblings.length > 0;
	}
	async function deleteSessionById(sessionId: string, options?: { keepWorktree?: boolean; sessionLabel?: string }): Promise<void> {
		const worktree = await dardcorCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		const worktreePath = await dardcorCLIWorktreeManagerService.getWorktreePath(sessionId);

		await dardcorCLISessionService.deleteSession(sessionId);
		await dardcorCliWorkspaceSession.deleteTrackedWorkspaceFolder(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.delete', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			const id = SessionIdForCLI.parse(sessionItem.resource);
			const sessionId = dardcorcliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
			const worktreePath = await dardcorCLIWorktreeManagerService.getWorktreePath(sessionId);
			...
			if (result === deleteLabel) {
				...
				dardcorcliSessionItemProvider.notifySessionsChange();
			}
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('agents.github.dardcor.cli.deleteSessions', async (sessionItems?: vscode.ChatSessionItem[], options?: { skipConfirmation?: boolean }) => {
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.resumeInTerminal', async (sessionItem?: vscode.ChatSessionItem) => {
		if (sessionItem?.resource) {
			await dardcorcliSessionItemProvider.resumeCopilotCLISessionInTerminal(sessionItem);
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.rename', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		if (newTitle) {
			...
			if (trimmedTitle) {
				const id = SessionIdForCLI.parse(sessionItem.resource);
				const sessionId = dardcorcliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
				await dardcorCLISessionService.renameSession(sessionId, trimmedTitle);
				dardcorcliSessionItemProvider.notifySessionsChange();
			}
		}
	}));
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.setTitle', async (sessionItem?: vscode.ChatSessionItem, title?: string) => {
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.openWorktreeInTerminal', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.copyWorktreeBranchName', async (sessionItem?: vscode.ChatSessionItem) => {
		if (!sessionItem?.resource) {
			return;
		}
		...
		const id = SessionIdForCLI.parse(sessionItem.resource);
		const sessionId = dardcorcliSessionItemProvider.untitledSessionIdMapping.get(id) ?? id;
		const worktreeProperties = await dardcorCLIWorktreeManagerService.getWorktreeProperties(sessionId);
	...
	const applyChanges = async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		...
		try {
			// Apply changes
			const sessionId = SessionIdForCLI.parse(resource);
			await dardcorCLIWorktreeManagerService.applyWorktreeChanges(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.chat.applyCopilotCLIAgentSessionChanges', applyChanges));
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.chat.applyCopilotCLIAgentSessionChanges.apply', applyChanges));
	...
	const mergeChanges = async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri, syncWithRemote: boolean = false) => {
		...
		await vscode.commands.executeCommand('workbench.action.chat.openSessionWithPrompt.dardcorcli', {
			resource,
			prompt,
			attachedContext: [{
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.chat.mergeCopilotCLIAgentSessionChanges.merge', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await mergeChanges(sessionItemOrResource);
	}));

	disposableStore.add(vscode.commands.registerCommand('github.dardcor.chat.mergeCopilotCLIAgentSessionChanges.mergeAndSync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await mergeChanges(sessionItemOrResource, true);
	}));

	disposableStore.add(vscode.commands.registerCommand('github.dardcor.sessions.refreshChanges', async (resource?: vscode.Uri) => {
		if (!resource) {
			return;
		}
		...
		const sessionId = SessionIdForCLI.parse(resource);
		const worktreeProperties = await dardcorCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		const workspaceFolder = await dardcorCliWorkspaceSession.getSessionWorkspaceFolder(sessionId);
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.sessions.initializeRepository', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		await dardcorCliWorkspaceSession.trackSessionWorkspaceFolder(sessionId, workspaceFolder.fsPath, repositoryProperties);
		dardcorCliWorkspaceSession.clearWorkspaceChanges(sessionId);
	...
	const setHasGitOperationInProgress = async (sessionId: string, inProgress: boolean, commandId = '') => {
		...
		if (worktreeProperties) {
			...
			if (!inProgress) {
				...
				await dardcorCLIWorktreeManagerService.refreshWorktreeChanges(sessionId);
			}

			await dardcorCLIWorktreeManagerService.updateWorktreeProperties(sessionId, {
				hasGitOperationInProgress: inProgress
			});

			await dardcorcliSessionItemProvider.refreshSession({ reason: 'update', sessionId });
		...
		// Workspace
		const workspaceFolder = await dardcorCliWorkspaceSession.getSessionWorkspaceFolder(sessionId);
		const repositoryProperties = await dardcorCliWorkspaceSession.getRepositoryProperties(sessionId);
		...
		if (!inProgress) {
			// Refresh the changes after the git operation is complete
			await dardcorCliWorkspaceSession.refreshWorkspaceChanges(sessionId);
		}

		await dardcorCliWorkspaceSession.setRepositoryProperties(sessionId, {
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.sessions.commit', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'github.dardcor.sessions.commit');
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.sessions.commitAndSync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'github.dardcor.sessions.commitAndSync');
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.sessions.sync', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		const resource = sessionItemOrResource instanceof vscode.Uri
			? sessionItemOrResource
			: sessionItemOrResource?.resource;
		...
		try {
			await setHasGitOperationInProgress(sessionId, true, 'github.dardcor.sessions.sync');
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.sessions.discardChanges', async (sessionResource: vscode.Uri, ref: string, ...resources: vscode.Uri[]) => {
		...
		await dardcorCLIWorktreeCheckpointService.updateLastCheckpoint(sessionId);
	...
	const createPullRequest = async (sessionItemOrResource: vscode.ChatSessionItem | vscode.Uri | undefined, isDraft: boolean) => {
		...
		const sessionId = SessionIdForCLI.parse(resource);
		let worktreeProperties = await dardcorCLIWorktreeManagerService.getWorktreeProperties(sessionId);
		if (!worktreeProperties || worktreeProperties.version !== 2) {
		...
		try {
			const commandId = isDraft
				? 'github.dardcor.chat.createDraftPullRequestCopilotCLIAgentSession.createDraftPR'
				: 'github.dardcor.chat.createPullRequestCopilotCLIAgentSession.createPR';
			...
			worktreeProperties = await dardcorCLIWorktreeManagerService.getWorktreeProperties(sessionId);
			if (worktreeProperties && worktreeProperties.version === 2) {
				await dardcorCLIWorktreeManagerService.setWorktreeProperties(sessionId, {
					...worktreeProperties,
					changes: undefined,
					pullRequestUrl
				});
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.chat.createPullRequestCopilotCLIAgentSession.createPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await createPullRequest(sessionItemOrResource, false);
	}));

	disposableStore.add(vscode.commands.registerCommand('github.dardcor.chat.createDraftPullRequestCopilotCLIAgentSession.createDraftPR', async (sessionItemOrResource?: vscode.ChatSessionItem | vscode.Uri) => {
		await createPullRequest(sessionItemOrResource, true);
	}));

	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.commitToWorktree', async (args?: { worktreeUri?: vscode.Uri; fileUri?: vscode.Uri }) => {
		...
		try {
			...
			const sessionIds = await dardcorcliSessionItemProvider.getAssociatedSessions(worktreeUri);
			await Promise.all(sessionIds.map(async sessionId => {
				const props = await dardcorCLIWorktreeManagerService.getWorktreeProperties(sessionId);
				if (props) {
					await dardcorCLIWorktreeManagerService.setWorktreeProperties(sessionId, { ...props, changes: undefined });
				} else {
	...
	disposableStore.add(vscode.commands.registerCommand('github.dardcor.cli.sessions.commitToRepository', async (args?: { repositoryUri?: vscode.Uri; fileUri?: vscode.Uri }) => {
...
async function getModelFromPromptFile(models: readonly string[], dardcorCLIModels: ICopilotCLIModels): Promise<string | undefined> {
	for (const model of models) {
		let modelId = await dardcorCLIModels.resolveModel(model);
		if (modelId) {
			return modelId;
		}
		...
		modelId = await dardcorCLIModels.resolveModel(model.substring(0, model.indexOf('(')).trim());
		if (modelId) {
			return modelId;
		}
	}
