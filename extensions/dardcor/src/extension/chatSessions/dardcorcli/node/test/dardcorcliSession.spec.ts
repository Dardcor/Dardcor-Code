...
import { FakeToolsService, ToolCall } from '../../common/dardcorCLITools';
import { Session } from '../../common/utils';
import { CopilotCLISession } from '../dardcorcliSession';
...
describe('CopilotCLISession', () => {
	...
	it('emits languageModelToolInvoked telemetry for each completed tool invocation', async () => {
		...
		const session = await createSession();
		session.attachStream(new MockChatResponseStream());
		const sessionResource = Uri.parse('dardcorcli:/test-session');
		...
		expect(invokedSnapshot).toEqual([
			{ result: 'success', chatSessionId: 'dardcorcli:/test-session', toolId: 'bash', toolExtensionId: undefined, toolSourceKind: 'dardcorCli', hasInvocationTimeMs: true },
			{ result: 'error', chatSessionId: 'dardcorcli:/test-session', toolId: 'mcp_tool', toolExtensionId: undefined, toolSourceKind: 'mcp', hasInvocationTimeMs: true },
			{ result: 'userCancelled', chatSessionId: 'dardcorcli:/test-session', toolId: 'bash', toolExtensionId: undefined, toolSourceKind: 'dardcorCli', hasInvocationTimeMs: true },
			{ result: 'success', chatSessionId: 'dardcorcli:/test-session', toolId: 'completion_tool', toolExtensionId: undefined, toolSourceKind: 'dardcorCli', hasInvocationTimeMs: false },
		]);
	});
