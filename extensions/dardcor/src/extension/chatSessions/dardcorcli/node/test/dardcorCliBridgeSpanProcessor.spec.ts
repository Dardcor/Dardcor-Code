...
describe('CopilotCliBridgeSpanProcessor', () => {
	...
	it('forwards spans with registered traceId', () => {
		...
		expect(otelService.injectedSpans[0].attributes['dardcor_chat.chat_session_id']).toBe('session-1');
	});
	...
	it('does not overwrite existing CHAT_SESSION_ID', () => {
		...
		bridge.onEnd(makeReadableSpan({
			traceId: 'trace-abc',
			attributes: { 'dardcor_chat.chat_session_id': 'existing-session' },
		}));

		expect(otelService.injectedSpans[0].attributes['dardcor_chat.chat_session_id']).toBe('existing-session');
	});
	...
	it('enriches SDK hook spans with stashed event data', () => {
		...
		bridge.onEnd(makeReadableSpan({
			...
			traceId: 'trace-abc',
			attributes: {
				'github.dardcor.hook.type': 'sessionEnd',
				'github.dardcor.hook.invocation_id': 'inv-123',
			},
		}));
		...
		expect(span.attributes['dardcor_chat.hook_type']).toBe('sessionEnd');
		expect(span.attributes['dardcor_chat.hook_input']).toBe('{"reason":"complete"}');
		expect(span.attributes['dardcor_chat.hook_result_kind']).toBe('success');
	});
	...
	it('holds SDK hook span until hook.end data arrives', () => {
		...
		bridge.onEnd(makeReadableSpan({
			...
			traceId: 'trace-abc',
			attributes: {
				'github.dardcor.hook.type': 'preToolUse',
				'github.dardcor.hook.invocation_id': 'inv-456',
			},
		}));
		...
		expect(otelService.injectedSpans).toHaveLength(1);
		const span = otelService.injectedSpans[0];
		expect(span.attributes['dardcor_chat.hook_input']).toBe('{"tool":"bash"}');
		expect(span.attributes['dardcor_chat.hook_output']).toBe('{"decision":"allow"}');
		expect(span.attributes['dardcor_chat.hook_result_kind']).toBe('success');
	});
