...
describe('Copilot CLI shims', () => {
	...
	beforeEach(async () => {
		testDir = await mkdtemp(join(tmpdir(), 'dardcor-cli-shims-'));
	});
	...
	it('creates runtime ripgrep and node-pty shims for separate extension installs before SDK import', async () => {
		...
		await expect(readFile(join(extensionPath, 'node_modules', '@github', 'dardcor', 'shims.txt'), 'utf8')).resolves.toBe('Shims created successfully');
		const sdkRipgrepDir = join(extensionPath, 'node_modules', '@github', 'dardcor', 'sdk', 'ripgrep', 'bin', process.platform + '-' + process.arch);
		await expect(readFile(join(sdkRipgrepDir, 'rg'), 'utf8')).resolves.toBe('ripgrep');
		const sdkNodePtyDir = join(extensionPath, 'node_modules', '@github', 'dardcor', 'sdk', 'prebuilds', process.platform + '-' + process.arch);
	...
	it('skips runtime node-pty shimming for bundled installs when shims.txt already exists', async () => {
		const extensionPath = join(testDir, 'extension');
		const vscodeAppRoot = join(testDir, 'app');
		const placeholder = join(extensionPath, 'node_modules', '@github', 'dardcor', 'shims.txt');
		...
		await mkdir(join(extensionPath, 'node_modules', '@github', 'dardcor'), { recursive: true });
		await writeFile(placeholder, 'already created');

		await createSdk(extensionPath, vscodeAppRoot).runEnsureShims();

		await expect(readFile(placeholder, 'utf8')).resolves.toBe('already created');
		await expect(stat(join(extensionPath, 'node_modules', '@github', 'dardcor', 'sdk'))).rejects.toMatchObject({ code: 'ENOENT' });
	});
});
