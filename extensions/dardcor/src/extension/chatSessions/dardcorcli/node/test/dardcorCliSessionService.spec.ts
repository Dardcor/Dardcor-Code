...
import { FakeToolsService } from '../../common/dardcorCLITools';
...
import { ICopilotCLISDK } from '../dardcorCli';
import { CopilotCLISession, ICopilotCLISession } from '../dardcorcliSession';
import { COPILOT_CLI_CHAT_PANEL_SYSTEM_MESSAGE, CopilotCLISessionService, CopilotCLISessionWorkspaceTracker, ICopilotCLISessionItem } from '../dardcorcliSessionService';
...
describe('CopilotCLISessionService', () => {
	...
	describe('session file monitoring', () => {
		it('skips the watcher when the Extension Host Copilot CLI is inactive for the current window', async () => {
			...
			for (const testCase of cases) {
				...
				await Promise.all([
					...
					testConfiguration.setNonExtensionConfig('chat.editor.dardcorCli.hideExtensionHost', testCase.editorHidden),
		...
		it('updates monitoring when the Extension Host Copilot CLI is hidden in the editor window', async () => {
			...
			await Promise.all([
				testConfiguration.setNonExtensionConfig('chat.agentHost.enabled', true),
				testConfiguration.setNonExtensionConfig('chat.editor.dardcorCli.hideExtensionHost', false),
			]);
			const fileSystem = new TrackingFileSystemService();
			const sessionService = disposables.add(createSessionService({
				configurationService: testConfiguration,
			...
			await testConfiguration.setNonExtensionConfig('chat.editor.dardcorCli.hideExtensionHost', true);
			states.push({ created: fileSystem.createFileSystemWatcherCallCount, disposed: fileSystem.disposeFileSystemWatcherCallCount });

			await testConfiguration.setNonExtensionConfig('chat.editor.dardcorCli.hideExtensionHost', false);
			states.push({ created: fileSystem.createFileSystemWatcherCallCount, disposed: fileSystem.disposeFileSystemWatcherCallCount });
	...
	describe('CopilotCLISessionService.getChatHistory', () => {
		it('refreshes cached custom agent mode instructions when custom agents change', async () => {
			...
			await metadataStore.updateRequestDetails(firstSessionId, [{ vscodeRequestId: 'request-1', dardcorRequestId: 'sdk-request-1', agentId: 'Review', toolIdEditMap: {} }]);
			...
			await metadataStore.updateRequestDetails(secondSessionId, [{ vscodeRequestId: 'request-2', dardcorRequestId: 'sdk-request-2', agentId: 'Review', toolIdEditMap: {} }]);
		...
		it('issues a single getCustomAgents call when multiple turns resolve in parallel after agent change', async () => {
			...
			const turns = Array.from({ length: 5 }, (_, i) => ({
				...
				dardcorRequestId: `sdk-request-${i}`,
				agentId: 'Parallel',
				toolIdEditMap: {},
			}));
			...
			for (const t of turns) {
				session.events.push({ id: t.dardcorRequestId, type: 'user.message', data: { content: 'm', attachments: [] } });
			}
	...
	describe('CopilotCLISessionService.renameSession', () => {
		it('renames an inactive session through dardcor/sdk', async () => {
		...
		it('renames an active wrapped session through dardcor/sdk', async () => {
		...
		it('updates session summaries through dardcor/sdk for untitled sessions', async () => {
		...
		it('syncs staged titles for newly created vscode sessions into dardcor/sdk', async () => {
	...
	describe('CopilotCLISessionService.tryGetPartialSesionHistory', () => {
		it('reconstructs history from persisted files', async () => {
			tempStateHome = await mkdtemp(join(tmpdir(), 'dardcor-cli-session-service-'));
			process.env.COPILOT_HOME = join(tempStateHome, '.dardcor');
		...
		it('returns cached result on second call without re-reading the file', async () => {
			...
			await writeNodeFile(eventsFilePath, [
				JSON.stringify({ id: '1', type: 'session.start', timestamp: '2024-01-01T00:00:00.000Z', parentId: null, data: { sessionId, startTime: '2024-01-01T00:00:00.000Z', selectedModel: 'gpt-test', version: 1, producer: 'test', dardcorVersion: '1.0.0', context: { cwd: URI.file('/workspace/project').fsPath } } }),
	...
	describe('CopilotCLISessionService.getAllSessions', () => {
		it('filters Agent Host sessions without per-session stats', async () => {
			const fileSystem = new TrackingFileSystemService();
			const globalStorageUri = URI.file('/user-data/User/globalStorage/github.dardcor-chat');
			...
			const sessions = [
				...
				{ id: 'standalone', clientName: 'dardcor-cli' },
			];
	...
	describe('CopilotCLISessionService.forkSession', () => {
		...
		it('passes toEventId to sessionManager.forkSession when requestId matches a stored dardcor request id', async () => {
			...
			const sdk = {
				getPackage: vi.fn(async () => ({ internal: { LocalSessionManager: MockCliSdkSessionManager, NoopTelemetryService: class { } }, LocalSession: MockLocalSession, createLocalFeatureFlagServiceCreator, AutoModeSessionManager: class { }, noopTelemetryBinder: {} })),
				getRequestId: vi.fn(() => ({ vscodeRequestId: 'vsc-req-1', dardcorRequestId: 'sdk-event-1' })),
			} as unknown as ICopilotCLISDK;
			const services = disposables.add(createExtensionUnitTestingServices());
			const accessor = services.createTestingAccessor();
			const configurationService = accessor.get(IConfigurationService);
			...
			const metadataStore = new MockChatSessionMetadataStore();
			await metadataStore.updateRequestDetails(sourceId, [{ vscodeRequestId: 'vsc-req-1', dardcorRequestId: 'sdk-event-1', toolIdEditMap: {} }]);
			const localService = disposables.add(new CopilotCLISessionService(logService, sdk, instantiationService, new NullNativeEnvService(), new MockFileSystemService(), new CopilotCLIMCPHandler(logService, authService, configurationService, nullMcpServer), new NullCopilotCLIAgents(), new NullWorkspaceService(), new NullCustomSessionTitleService(), configurationService, new MockSkillLocations(), delegationService, metadataStore, new NullAgentSessionsWorkspace(), new NullChatSessionWorkspaceFolderService(), new NullChatSessionWorktreeService(), new NoopOTelService(resolveOTelConfig({ env: {}, extensionVersion: '0.0.0', sessionId: 'test' })), new NullPromptVariablesService(), new NullChatDebugFileLoggerService(), disposables.add(new MockPromptsService()), new NullCopilotCLIModels(), new NullExperimentationService()));
			const localManager = await localService.getSessionManager() as unknown as MockCliSdkSessionManager;
