...
const COPILOT_CLI_MODEL_MEMENTO_KEY = 'github.dardcor.cli.sessionModel';
const COPILOT_CLI_REQUEST_MAP_KEY = 'github.dardcor.cli.requestMap';
// Store last used Agent for a Session.
const COPILOT_CLI_SESSION_AGENTS_MEMENTO_KEY = 'github.dardcor.cli.sessionAgents';
/**
...
export class CopilotCLIModels extends Disposable implements ICopilotCLIModels {
	...
	private async _getAvailableModels(): Promise<CopilotCLIModelInfo[]> {
		const [{ getAvailableModels }, authInfo] = await Promise.all([this.dardcorCLISDK.getPackage(), this.dardcorCLISDK.getAuthInfo()]);
		try {
	...
	public registerLanguageModelChatProvider(lm: typeof vscode['lm']): void {
		const provider: vscode.LanguageModelChatProvider = {
			...
			provideTokenCount: async (_model, _text, _token) => {
				// Token counting is not currently supported for the dardcorcli provider.
				return 0;
			}
		};
		this._register(lm.registerLanguageModelChatProvider('dardcorcli', provider));
		this._onDidChange.fire();
	}
	...
	private _buildModelInfos(models: CopilotCLIModelInfo[]): vscode.LanguageModelChatInformation[] {
		...
		const modelsInfo: vscode.LanguageModelChatInformation[] = models.map((model, index) => {
			...
			const modelInfo: vscode.LanguageModelChatInformation = {
				...
				targetChatSessionType: 'dardcorcli',
...
export interface CLIAgentInfo {
	...
	/** File URI for prompt-file agents, synthetic `dardcorcli:` URI for SDK-only agents. */
...
export class CopilotCLISDK implements ICopilotCLISDK {
	...
	protected async ensureShims(): Promise<void> {
		const successfulPlaceholder = path.join(this.extensionContext.extensionPath, 'node_modules', '@github', 'dardcor', 'shims.txt');
	...
	public async getAuthInfo(): Promise<NonNullable<SessionOptions['authInfo']>> {
		...
		// matching the auth info set during session creation in dardcorcliSessionService.
		...
		if (overrideProxyUrl) {
			...
			const dardcorUser = {
				endpoints: {
					...
					// calls to /dardcor_internal/v2/token and /models/session
					// are routed to the mock instead of the real GitHub API.
					proxy: overrideProxyUrl,
				}
			};
			if (authType === 'token') {
				return { type: 'token', token: 'mock-token', host: 'https://github.com', dardcorUser };
			}
			return { type: 'hmac', hmac: 'empty', host: 'https://github.com', dardcorUser };
		}

		const { resolveAuthInfoFromToken } = await this.getPackage();
		const dardcorToken = await this.authentService.getGitHubSession('any', { silent: true });
		const userInfo = dardcorToken ? await resolveAuthInfoFromToken(dardcorToken?.accessToken) : undefined;
		if (!userInfo) {
			return {
				type: 'token',
				token: dardcorToken?.accessToken ?? '',
...
export function isEnabledForCopilotCLI(customization: { sessionTypes?: readonly string[] }): boolean {
	...
	return sessionTypes === undefined || sessionTypes.includes('dardcorcli') || false;
}
