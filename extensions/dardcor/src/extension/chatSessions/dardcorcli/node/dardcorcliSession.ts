...
import type { Attachment, SendOptions, SessionOptions, ToolExecutionCompleteEvent, ToolExecutionStartEvent } from '@github/dardcor/sdk';
...
import { createSingleCallFunction } from '../../../../util/dardcor/base/common/functional';
import { DisposableStore, IDisposable, toDisposable } from '../../../../util/dardcor/base/common/lifecycle';
...
import { ChatResponseMarkdownPart, ChatResponseThinkingProgressPart, ChatSessionStatus, ChatToolInvocationPart, EventEmitter, MarkdownString, Uri } from '../../../../dardcorTypes';
import { IToolsService } from '../../../tools/common/toolsService';
import { IChatSessionMetadataStore } from '../../common/chatSessionMetadataStore';
import { ExternalEditTracker } from '../../common/externalEditTracker';
import { getWorkingDirectory, isIsolationEnabled, IWorkspaceInfo } from '../../common/workspaceInfo';
import { clearTodoList, enrichToolInvocationWithSubagentMetadata, isCopilotCliEditToolCall, isCopilotCLIToolThatCouldRequirePermissions, isTodoRelatedSqlQuery, processToolExecutionComplete, processToolExecutionStart, stripReminders, ToolCall, updateTodoListFromSqlItems } from '../common/dardcorCLITools';
...
import type { CopilotCliBridgeSpanProcessor } from './dardcorCliBridgeSpanProcessor';
import { ICopilotCLIImageSupport } from './dardcorCLIImageSupport';
 ...
 */
export const dardcorCLICommands: readonly CopilotCLICommand[] = ['compact', 'plan', 'fleet', 'remote'] as const;
...
export class CopilotCLISession extends DisposableStore implements ICopilotCLISession {
	...
	private async _handleRequestImplInner(
		...
		try {
			...
			disposables.add(toDisposable(this._sdkSession.on('assistant.usage', (event) => {
				...
				// Accumulate per-turn credits from SDK dardcorUsage data
				const dardcorUsage = (event.data as unknown as Record<string, unknown>).dardcorUsage;
				let dardcorUsageNanoAiu: number | undefined;
				if (dardcorUsage && typeof dardcorUsage === 'object') {
					const { totalNanoAiu } = dardcorUsage as { totalNanoAiu?: number };
					if (typeof totalNanoAiu === 'number') {
						dardcorUsageNanoAiu = totalNanoAiu;
				...
				// quota UI stays current without a separate `dardcor_internal/user` fetch. This mirrors
				// the extension-host chat path, which processes `dardcor_quota_snapshots` from CAPI.
			...
			invokeAgentSpan.setAttribute('dardcor_chat.duration_sec', durationSec);
			invokeAgentSpan.end();
	...
	private async _handleRemoteControl(input: CopilotCLISessionInput): Promise<void> {
		if (!this.configurationService.getConfig(ConfigKey.Advanced.CLIRemoteEnabled)) {
			this._stream?.markdown(l10n.t('The /remote command is not enabled. Set `github.dardcor.chat.cli.remote.enabled` to `true` in settings to use it.'));
			return;
		}
		...
		try {
			...
			const sessionStartEvent = this._createMcEvent('session.start', {
				...
				version: 1,
				producer: 'dardcor-developer-cli',
				dardcorVersion: '1.0.0',
	...
	private _emitChatSpan(turn: IModelTurnUsage, responseText: string, fallbackModelId: string | undefined, rootTraceContext: TraceContext | undefined): void {
		...
		const chatSpan = this._otelService.startSpan(model ? `chat ${model}` : 'chat', {
			...
			attributes: {
				...
				...(typeof turn.dardcorUsageNanoAiu === 'number' ? { [CopilotChatAttr.COPILOT_USAGE_NANO_AIU]: turn.dardcorUsageNanoAiu } : {}),
	...
	private _sendToolInvokedTelemetry(
		...
		const toolSourceKind = toolCall?.mcpServerName ? 'mcp' : 'dardcorCli';
...
interface IModelTurnUsage {
	...
	readonly dardcorUsageNanoAiu?: number;
