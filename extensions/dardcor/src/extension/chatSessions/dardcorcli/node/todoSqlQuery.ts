...
import type { TodoItem, TodoSqlWorkerApi } from './dardcorCLITodoWorker';
...
export class TodoSqlQuery {
	...
	private ensureWorker(): RpcProxy<TodoSqlWorkerApi> {
		if (!this._proxy) {
			this._worker = new WorkerWithRpcProxy<TodoSqlWorkerApi>(
				join(__dirname, 'dardcorCLITodoWorker.js')
			);
	 ...
	 * @param sessionDir - The session state directory (e.g. ~/.dardcor/session-state/{session-id})
