...
import type { internal, LocalSessionMetadata, SessionContext, SessionEvent, SessionOptions, SweCustomAgent } from '@github/dardcor/sdk';
...
import { disposableTimeout, raceCancellation, raceCancellationError, SequencerByKey, ThrottledDelayer } from '../../../../util/dardcor/base/common/async';
import { CancellationToken } from '../../../../util/dardcor/base/common/cancellation';
import { Emitter, Event } from '../../../../util/dardcor/base/common/event';
import { Lazy } from '../../../../util/dardcor/base/common/lazy';
import { Disposable, DisposableMap, DisposableStore, IDisposable, IReference, MutableDisposable, RefCountedDisposable, toDisposable } from '../../../../util/dardcor/base/common/lifecycle';
import { basename, dirname, joinPath } from '../../../../util/dardcor/base/common/resources';
import { URI } from '../../../../util/dardcor/base/common/uri';
import { generateUuid } from '../../../../util/dardcor/base/common/uuid';
import { IInstantiationService } from '../../../../util/dardcor/platform/instantiation/common/instantiation';
import { ChatRequestTurn2, ChatResponseTurn2, ChatSessionStatus, Uri } from '../../../../dardcorTypes';
...
import { buildChatHistoryFromEvents, RequestIdDetails, stripReminders } from '../common/dardcorCLITools';
...
import { getAgentFileNameFromFilePath, ICopilotCLIAgents, ICopilotCLIModels, ICopilotCLISDK, isEnabledForCopilotCLI } from './dardcorCli';
import { CopilotCliBridgeSpanProcessor } from './dardcorCliBridgeSpanProcessor';
import { CopilotCLISession, ICopilotCLISession } from './dardcorcliSession';
import { ICopilotCLISkills } from './dardcorCLISkills';
import { ICopilotCLIMCPHandler, McpServerMappings, remapCustomAgentTools } from './mcpHandler';


const COPILOT_CLI_WORKSPACE_JSON_FILE_KEY = 'github.dardcor.cli.workspaceSessionFile';
...
const COPILOT_CLI_HIDE_EXTENSION_HOST_EDITOR_SETTING_ID = 'chat.editor.dardcorCli.hideExtensionHost';
...
export class CopilotCLISessionService extends Disposable implements ICopilotCLISessionService {
	...
	constructor(
		...
		@ICopilotCLISkills private readonly dardcorCLISkills: ICopilotCLISkills,
		...
		@ICopilotCLIModels private readonly _dardcorCLIModels: ICopilotCLIModels,
	...
	private async getSDKPackage(): Promise<SDKPackage> {
		return this.dardcorCLISDK.getPackage();
	}
	...
	protected monitorSessionFiles(): void {
		...
		try {
			const sessionDir = joinPath(this.nativeEnv.userHome, '.dardcor', 'session-state');
	...
	async _getAllSessions(token: CancellationToken): Promise<readonly ICopilotCLISessionItem[]> {
		...
		try {
			...
			// Skip sessions the agent host already lists (both surfaces share `~/.dardcor/session-state/`).
	...
	public async createSession(options: ICreateSessionOptions, token: CancellationToken): Promise<RefCountedSession> {
		const resource = options.sessionId ? SessionIdForCLI.getResource(options.sessionId) : URI.from({ scheme: 'dardcor-cli', path: `mcp-gateway-${generateUuid()}` });
		...
		try {
			...
			if (sessionOptions.dardcorUrl) {
				...
				const dardcorUser = { endpoints: { api: sessionOptions.dardcorUrl } };
				const host = 'https://github.com' as const;
				const authInfo = authType === 'token'
					? { type: 'token' as const, token: 'mock-token', host, dardcorUser }
					: { type: 'hmac' as const, hmac: 'empty', host, dardcorUser };
	...
	private _installBridgeIfNeeded(): void {
		...
		try {
			...
			// synthesized directly from the SDK event stream in `dardcorcliSession.ts`.
			if ((delegate as Record<string, unknown>).__dardcorChatOwnProvider) {
	...
	protected async createSessionsOptions(options: ICreateSessionOptions & { mcpServers?: SessionOptions['mcpServers'] }): Promise<Readonly<SessionOptions>> {
		const [agentInfos, skillLocations] = await Promise.all([
			...
			this.dardcorCLISkills.getSkillsLocations(CancellationToken.None),
		]);
		...
		const dardcorUrl = this.configurationService.getConfig(ConfigKey.Shared.DebugOverrideProxyUrl) || undefined;
		if (dardcorUrl) {
			allOptions.dardcorUrl = dardcorUrl;
		}
	...
	private async getChatHistoryImpl({ sessionId, workspace }: { sessionId: string; workspace: IWorkspaceInfo }, token: CancellationToken): Promise<{ history: (ChatRequestTurn2 | ChatResponseTurn2)[]; events: readonly SessionEvent[] }> {
		...
		// Build lookup from dardcorRequestId → RequestDetails for the callback
		...
		for (const d of storedDetails) {
			if (d.dardcorRequestId) {
				...
				detailsByCopilotId.set(d.dardcorRequestId, { requestId: d.vscodeRequestId, toolIdEditMap: d.toolIdEditMap, modeInstructions, responseModelId: d.responseModelId, creditsUsed: d.creditsUsed, isUsingAutoModel: d.isUsingAutoModel });
			}
		}
		const getVSCodeRequestId = (sdkRequestId: string) => {
			...
			const mapping = this.dardcorCLISDK.getRequestId(sdkRequestId);
			if (mapping) {
				detailsByCopilotId.set(sdkRequestId, mapping);
				legacyMappings.push({
					dardcorRequestId: sdkRequestId,
	...
	private async getModelDetailsById(): Promise<ReadonlyMap<string, ModelDetailsInfo>> {
		const models = await this._dardcorCLIModels.getModels().catch(ex => {
	...
	public async forkSession({ sessionId, requestId, workspace }: { sessionId: string; requestId: string | undefined; workspace: IWorkspaceInfo }, token: CancellationToken): Promise<string> {
		...
		if (requestId) {
			...
			if (requestToTruncateTo) {
				...
				const translatedSDKEvent = storedDetails.find(d => d.vscodeRequestId === requestToTruncateTo.id || d.dardcorRequestId === requestToTruncateTo.id)?.dardcorRequestId;
...
export class CopilotCLISessionWorkspaceTracker {
	...
	constructor(
		...
		this._initializeSessionStorageFiles = new Lazy<Promise<{ global: Uri; workspace: Uri }>>(async () => {
			const globalFile = joinPath(this.context.globalStorageUri, 'dardcor.cli.oldGlobalSessions.json');
			let workspaceFile = joinPath(this.context.globalStorageUri, 'dardcor.cli.workspaceSessions.json');
			...
			if (this.workspaceService.getWorkspaceFolders().length) {
				...
				if (!workspaceFileName) {
					workspaceFileName = `dardcor.cli.workspaceSessions.${generateUuid()}.json`;
