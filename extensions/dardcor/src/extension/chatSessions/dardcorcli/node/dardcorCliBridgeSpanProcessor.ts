...
/**
 * Hook event data stashed by dardcorcliSession for bridge enrichment.
 */
 ...
 *
 * The processor injects `dardcor_chat.chat_session_id` on each forwarded span
...
export class CopilotCliBridgeSpanProcessor implements SpanProcessor {
	 ...
	 * Populated when dardcorcliSession.ts creates its root `invoke_agent dardcorcli` span.
	 */
	private readonly _traceIdToSessionId = new Map<string, string>();
	private _disposed = false;

	/**
	 * Hook event data stashed by dardcorcliSession for enriching SDK hook spans.
	 ...
	 * Called by dardcorcliSession before the SDK span ends.
	 */
