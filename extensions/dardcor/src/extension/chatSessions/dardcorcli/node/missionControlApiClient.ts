...
export class MissionControlApiClient {
	...
	async createSession(
		...
		const res = await this._fetcherService.fetch(url, {
			callSite: 'dardcorcli.mc.createSession',
			method: 'POST',
			headers,
			json: {
				owner_id: ownerId,
	...
	async submitEvents(
		...
		try {
			...
			const res = await this._fetcherService.fetch(url, {
				callSite: 'dardcorcli.mc.submitEvents',
				method: 'POST',
				headers,
				json: {
					events,
	...
	async getPendingCommands(sessionId: string): Promise<McCommand[]> {
		try {
			...
			const res = await this._fetcherService.fetch(url, {
				callSite: 'dardcorcli.mc.getPendingCommands',
				method: 'GET',
				headers,
				timeout: REQUEST_TIMEOUT_MS,
			});
	...
	async deleteSession(sessionId: string): Promise<void> {
		try {
			...
			await this._fetcherService.fetch(url, {
				callSite: 'dardcorcli.mc.deleteSession',
	...
	private async _buildRequest(
		...
		const dardcorToken = await this._authService.getCopilotToken();
		const baseUrl = dardcorToken.endpoints?.api;
		if (!baseUrl) {
