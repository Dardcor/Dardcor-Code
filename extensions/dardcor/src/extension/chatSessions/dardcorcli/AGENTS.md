...
│  (node/dardcorcliSessionService.ts)                              │
...
│  - Discoverable by CLI via lock files in ~/.dardcor/ide/          │
└─────────────────────────────────────────────────────────────────┘
```
...
│   ├── dardcorCLITools.ts      # Tool type definitions and processing helpers
│   ├── dardcorCLIPrompt.ts     # Prompt reference extraction and parsing
│   ├── customSessionTitleService.ts
│   ├── delegationSummaryService.ts
│   ├── utils.ts                # SessionIdForCLI namespace (URI scheme: 'dardcorcli')
│   └── test/
│
├── node/                       # Node.js-specific (SDK integration, filesystem, permissions)
│   ├── dardcorCli.ts           # ICopilotCLISDK, CopilotCLIModels, CopilotCLIAgents
│   ├── dardcorcliSession.ts    # CopilotCLISession — main session wrapper
│   ├── dardcorcliSessionService.ts  # Session lifecycle management
│   ├── permissionHelpers.ts    # Permission request handlers
│   ├── dardcorcliPromptResolver.ts  # Resolves prompts with variables and attachments
│   ├── dardcorCLISkills.ts     # Skills location resolution
│   ├── dardcorCLIImageSupport.ts    # Image attachment handling
...
└── vscode-node/                # VS Code API-dependent (commands, MCP tools, UI)
    ├── dardcorCLIFolderMru.ts  # Folder MRU (most-recently-used) service
    └── test/
```
...
- Registers a `LanguageModelChatProvider` with `targetChatSessionType: 'dardcorcli'` so VS Code's model picker shows CLI models
...
### `node/dardcorCLIImageSupport.ts`

**ICopilotCLIImageSupport / CopilotCLIImageSupport**
- Stores image data as files in extension global storage (`dardcor-cli-images/`)
...
Copilot CLI sessions are persisted to `~/.dardcor/session-state/<sessionId>/` directories containing:
...
2. Add a `case '<kind>':` in the permission switch in `node/dardcorcliSession.ts` (~line 468)
...
1. Add a listener in `node/dardcorcliSession.ts` using `this._sdkSession.on(eventName, handler)`
...
**VS Code Session commands** (registered via `registerCLIChatCommands` in `vscode-node/dardcorCLIChatSessions.ts`):

## Configuration

The integration respects these VS Code settings (all under `github.dardcor.chat.cli.*`):
...
V1 registration in `../vscode-node/dardcorCLIChatSessionsContribution.ts` and `registerCopilotCLIServicesV1` are deprecated. All new development should use `CopilotCLISessionService` and the controller-based V2 API.
