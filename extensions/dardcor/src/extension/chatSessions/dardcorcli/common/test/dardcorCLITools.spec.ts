...
describe('CopilotCLITools', () => {
	...
	describe('buildChatHistoryFromEvents', () => {
		it('builds turns with user and assistant messages including PR metadata', () => {
			...
			if (prPart) {
				...
				expect((prPart as any).command.command).toBe('github.dardcor.chat.openPullRequestReroute');
			}
			if (markdownPart) {
		...
		it('filters out instruction file attachments', () => {
			const events: any[] = [
				{
					type: 'user.message', data: {
						content: 'Hello',
						attachments: [
							{ type: 'file', path: '/workspace/.github/dardcor-instructions.md', displayName: 'dardcor-instructions.md' },
			...
			expect(paths).not.toContain('/workspace/.github/dardcor-instructions.md');
	...
	describe('createCopilotCLIToolInvocation', () => {
		...
		it('creates invocation for fetch_dardcor_cli_documentation', () => {
			const part = createCopilotCLIToolInvocation({ toolName: 'fetch_dardcor_cli_documentation', toolCallId: 'fd1', arguments: {} });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
		});
		...
		it('formats mcp_validate invocation with path', () => {
			const part = createCopilotCLIToolInvocation({ toolName: 'mcp_validate', toolCallId: 'mv1', arguments: { path: '/home/user/.dardcor/config/mcp-config.json' } });
			expect(part).toBeInstanceOf(ChatToolInvocationPart);
