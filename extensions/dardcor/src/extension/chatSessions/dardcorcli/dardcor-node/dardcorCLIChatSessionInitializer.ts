...
import { DisposableStore, IReference } from '../../../../util/dardcor/base/common/lifecycle';
...
import { COPILOT_CLI_CONTEXT_SIZE_PROPERTY, COPILOT_CLI_REASONING_EFFORT_PROPERTY, ICopilotCLIAgents, ICopilotCLIModels, resolveContextTier } from '../../dardcorcli/node/dardcorCli';
import { ICopilotCLISession } from '../../dardcorcli/node/dardcorcliSession';
import { ICopilotCLISessionService } from '../../dardcorcli/node/dardcorcliSessionService';
import { buildMcpServerMappings, McpServerMappings } from '../../dardcorcli/node/mcpHandler';
...
export class CopilotCLIChatSessionInitializer implements ICopilotCLIChatSessionInitializer {
	...
	private async getModelFromPromptFile(models: readonly string[]): Promise<string | undefined> {
		for (const model of models) {
			let modelId = await this.dardcorCLIModels.resolveModel(model);
			if (modelId) {
				return modelId;
			}
			...
			modelId = await this.dardcorCLIModels.resolveModel(model.substring(0, model.indexOf('(')).trim());
			if (modelId) {
				return modelId;
			}
		}
