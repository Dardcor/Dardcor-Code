...
import { copyRipgrepShim } from '../../../dardcorcli/node/ripgrepShim';

describe('CopilotCLI SDK Upgrade', function () {
	const extensionPath = path.join(__dirname, '..', '..', '..', '..', '..', '..');
	const dardcorSDKPath = path.join(extensionPath, 'node_modules', '@github', 'dardcor');
	...
	it('should not contain new native binaries nor removed native binaries', async function () {
		...
		const existingBinaries = new Set(await findAllBinaries(dardcorSDKPath));
		...
		].map(p => path.join(dardcorSDKPath, p)));

		// Exclude ripgrep files that we copy over in src/extension/chatSessions/dardcorcli/node/ripgrepShim.ts (until we get better API/solution from SDK)
		const ripgrepFilesWeCopy = path.join(dardcorSDKPath, 'sdk', 'ripgrep', 'bin');
		...
		for (const binary of existingBinaries) {
			...
			if (!knownBinaries.has(binary)) {
				errors.push(`Unexpected native binary found in Copilot CLI SDK: ${path.relative(dardcorSDKPath, binary)}`);
			}
		}
		...
		for (const binary of knownBinaries) {
			...
			if (!existingBinaries.has(binary)) {
				if (isNonCurrentCopilotPlatformBinary(dardcorSDKPath, binary) || isNonCurrentPvRecorderBinary(dardcorSDKPath, binary)) {
					continue;
				}
				errors.push(`Expected native binary missing from Copilot CLI SDK: ${path.relative(dardcorSDKPath, binary)}`);
			}
		}
...
const dardcorPlatformArchs = new Set([
	'darwin-arm64',
	'darwin-x64',
	'linux-arm64',
	'linux-x64',
...
function isNonCurrentCopilotPlatformBinary(dardcorSDKPath: string, binary: string): boolean {
	const relativeSegments = path.relative(dardcorSDKPath, binary).split(path.sep);
	const platformArch = relativeSegments.find(segment => dardcorPlatformArchs.has(segment));
	return platformArch !== undefined && platformArch !== currentCopilotPlatformArch();
}

function isNonCurrentPvRecorderBinary(dardcorSDKPath: string, binary: string): boolean {
	const relative = path.relative(dardcorSDKPath, binary).split(path.sep).join(path.posix.sep);
...
async function copyBinaries(extensionPath: string) {
	const dardcorSDKPath = path.join(extensionPath, 'node_modules', '@github', 'dardcor');
	const vscodeRipgrepPath = path.join(dardcorSDKPath, 'ripgrep', 'bin', process.platform + '-' + process.arch);
