...
describe('createLockFile', () => {
	...
	beforeEach(() => {
		...
		process.env.COPILOT_HOME = path.join(testDir, '.dardcor');
	});
	...
	it('should create lock file in .dardcor directory', async () => {
		...
		expect(handle.path).toMatch(/\.dardcor[/\\]ide.*\.lock$/);
	...
	it('should create .dardcor directory if it does not exist', async () => {
		const mockServerUri = { path: '/tmp/server.sock', scheme: 'http' } as any;
		const handle = await createLockFile(mockServerUri, {}, logger);
		createdLockFile = handle.path;

		const dardcorDir = path.dirname(handle.path);
		const stat = await fs.stat(dardcorDir);
...
describe('cleanupStaleLockFiles', () => {
	...
	const dardcorDir = path.join(testDir, '.dardcor', 'ide');
	let originalEnv: string | undefined;

	beforeEach(async () => {
		originalEnv = process.env.COPILOT_HOME;
		process.env.COPILOT_HOME = path.join(testDir, '.dardcor');
		await fs.mkdir(dardcorDir, { recursive: true });
	});
	...
	it('should remove lockfiles for non-running processes', async () => {
		const staleLockFile = path.join(dardcorDir, 'stale.lock');
		const staleLockInfo = {
	...
	it('should keep lockfiles for running processes', async () => {
		const activeLockFile = path.join(dardcorDir, 'active.lock');
	...
	it('should return 0 when dardcor directory does not exist', async () => {
		await fs.rm(dardcorDir, { recursive: true, force: true });
