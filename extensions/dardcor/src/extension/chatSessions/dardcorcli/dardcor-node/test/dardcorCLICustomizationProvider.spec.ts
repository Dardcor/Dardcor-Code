...
import { CLIAgentInfo, ICopilotCLIAgents } from '../../../dardcorcli/node/dardcorCli';
import { CopilotCLICustomizationProvider } from '../dardcorCLICustomizationProvider';
...
/** Creates a CLIAgentInfo with a synthetic dardcorcli: URI (SDK-only agent). */
function makeAgentInfo(name: string, description = '', displayName?: string): CLIAgentInfo {
	return {
		...
		sourceUri: URI.from({ scheme: 'dardcorcli', path: `/agents/${name}` }),
		source: 'local',
...
describe('CopilotCLICustomizationProvider', () => {
	const testSessionResource = URI.parse('dardcorcli:///test-session');
	...
	describe('metadata', () => {
		it('has correct label and icon', () => {
			...
			expect(CopilotCLICustomizationProvider.metadata.iconId).toBe('dardcor');
		});
	...
	describe('provideChatSessionCustomizations', () => {
		...
		it('uses synthetic URI for SDK-only agents', async () => {
			...
			const agentItems = items.filter((i: vscode.ChatSessionCustomizationItem) => i.type === FakeChatSessionCustomizationType.Agent);
			expect(agentItems).toHaveLength(1);
			expect(agentItems[0].uri.scheme).toBe('dardcorcli');
		...
		it('returns instructions with on-demand groupKey when no applyTo pattern', async () => {
			const uri = URI.file('/workspace/.github/dardcor-instructions.md');
			mockPromptsService.setInstructions([makeInstruction(uri, 'dardcor-instructions', undefined)]);
		...
		it('derives skill name from parent directory for SKILL.md files', async () => {
			const uri = URI.file('/workspace/.dardcor/skills/my-skill/SKILL.md');
		...
		it('returns all matching types combined', async () => {
			...
			mockPromptsService.setHooks([makeHook(URI.file('/workspace/.dardcor/hooks/pre-commit.json'))]);
			mockPromptsService.setPlugins([makePlugin(URI.file('/workspace/.dardcor/plugins/my-plugin'))]);
		...
		it('returns hooks with correct type and name', async () => {
			const uri = URI.file('/workspace/.dardcor/hooks/diagnostics.json');
		...
		it('strips .json extension from hook file name', async () => {
			mockPromptsService.setHooks([makeHook(URI.file('/workspace/.dardcor/hooks/security-checks.json'))]);
		...
		it('returns multiple hooks', async () => {
			mockPromptsService.setHooks([
				makeHook(URI.file('/workspace/.dardcor/hooks/hooks.json')),
				makeHook(URI.file('/workspace/.dardcor/hooks/diagnostics.json')),
			]);
		...
		it('returns plugins with correct type and name derived from URI', async () => {
			const uri = URI.file('/workspace/.dardcor/plugins/lint-rules');
	...
	describe('instruction groupKeys and badges', () => {
		it('uses agent-instructions groupKey for dardcor-instructions.md files', async () => {
			const uri = URI.file('/workspace/.github/dardcor-instructions.md');
			mockPromptsService.setInstructions([makeInstruction(uri, 'dardcor-instructions', undefined)]);
		...
		it('emits agent instructions not in chatPromptFileService.instructions', async () => {
			...
			const dardcorUri = URI.file('/workspace/.github/dardcor-instructions.md');
			...
			mockCustomInstructionsService.setAgentInstructionUris([agentsUri, claudeUri, dardcorUri]);

			const items = await provider.provideChatSessionCustomizations(testSessionResource, undefined!);
			const instrItems = items.filter(i => i.type === FakeChatSessionCustomizationType.Instructions);
			expect(instrItems).toHaveLength(3);
			expect(instrItems.every(i => i.groupKey === 'agent-instructions')).toBe(true);
			expect(instrItems.map(i => i.name)).toEqual(['AGENTS.md', 'CLAUDE.md', 'dardcor-instructions.md']);
		});
		...
		it('categorizes mixed instructions correctly', async () => {
			const agentUri = URI.file('/workspace/.github/dardcor-instructions.md');
			...
			mockPromptsService.setInstructions([makeInstruction(agentUri, 'dardcor instructions', undefined), makeInstruction(contextUri, 'style instructions', 'src/**'), makeInstruction(onDemandUri, 'refactor instructions', undefined)]);
