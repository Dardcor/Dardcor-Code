...
export class InProcHttpServer extends Disposable {
	...
	private async _handlePost(mcpOptions: McpProviderOptions, req: express.Request, res: express.Response): Promise<void> {
		const sessionId = req.headers['mcp-session-id'] ?? req.headers['x-dardcor-session-id'];
		...
		if (sessionId && existingTransport) {
			...
			const clientPid = parseInt(req.headers['x-dardcor-pid'] as string, 10);
			const clientPpid = parseInt(req.headers['x-dardcor-parent-pid'] as string, 10);
