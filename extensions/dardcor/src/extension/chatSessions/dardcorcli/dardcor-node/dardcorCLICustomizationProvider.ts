...
import { ICopilotCLIAgents, isEnabledForCopilotCLI } from '../../dardcorcli/node/dardcorCli';
...
export class CopilotCLICustomizationProvider extends Disposable implements vscode.ChatSessionCustomizationProvider {
	...
	constructor(
		...
		this._register(this.dardcorCLIAgents.onDidChangeAgents(() => this._onDidChange.fire()));
	}
	...
	private async getAgentItems(_token: vscode.CancellationToken): Promise<vscode.ChatSessionCustomizationItem[]> {
		const agentInfos = await this.dardcorCLIAgents.getAgents();
	 ...
	 * implementation:
	 * - agent-instructions: AGENTS.md, CLAUDE.md, dardcor-instructions.md
	...
	private async getInstructionItems(token: CancellationToken): Promise<vscode.ChatSessionCustomizationItem[]> {
		...
		// (dardcor-instructions.md) plus workspace-root AGENTS.md and CLAUDE.md
		...
		// Emit agent instruction files (AGENTS.md, CLAUDE.md, dardcor-instructions.md)
		...
		for (const instruction of await this.promptsService.getInstructions(token)) {
			...
			if (!isEnabledForCopilotCLI(instruction)) {
				continue; // only include instructions that are relevant for dardcorcli
			}
...
function getSearchRoots() {
	return {
		...
		user: [
			{ path: ['.dardcor', 'agents'], type: vscode.ChatSessionCustomizationType.Agent },
			{ path: ['.agents', 'skills'], recursive: true, type: vscode.ChatSessionCustomizationType.Skill },
			{ path: ['.dardcor', 'instructions'], recursive: true, type: vscode.ChatSessionCustomizationType.Instructions },
			{ path: ['.dardcor', 'hooks'], recursive: true, type: vscode.ChatSessionCustomizationType.Hook },
		],
	};
}
