...
import { disposableTimeout, SequencerByKey, ThrottledDelayer } from '../../../../util/dardcor/base/common/async';
...
import { getCopilotBulkMetadataFile, getCopilotCLISessionDir } from '../../dardcorcli/node/cliHelpers';
import { ICopilotCLIAgents } from '../../dardcorcli/node/dardcorCli';

// const WORKSPACE_FOLDER_MEMENTO_KEY = 'github.dardcor.cli.sessionWorkspaceFolders';
// const WORKTREE_MEMENTO_KEY = 'github.dardcor.cli.sessionWorktrees';
const LEGACY_BULK_METADATA_FILENAME = 'dardcorcli.session.metadata.json';
const LEGACY_BULK_MIGRATED_KEY = 'github.dardcor.cli.legacyBulkMigrated';
 ...
 * (`~/.dardcor/vscode.session.metadata.cache.json`). Older entries (by `modified`)
 * are evicted from the file but remain available via the per-session metadata files
 * (`~/.dardcor/session-state/{id}/vscode.metadata.json`) and the JSONL worktree index.
 */
...
export class ChatSessionMetadataStore extends Disposable implements IChatSessionMetadataStore {
	...
	/** Path of the shared bulk metadata cache file in `~/.dardcor/`. */
	...
	private async initializeStorage(): Promise<void> {
		...
		// globalStorageUri to the shared `~/.dardcor/` location.
	...
	async getSessionAgent(sessionId: string): Promise<string | undefined> {
		...
		return findLast(details, d => !!d.modeInstructions)?.modeInstructions?.uri ?? findLast(details, d => !!d.agentId)?.agentId ?? this.dardcorCLIAgents.getSessionAgent(sessionId);
	}
	...
	private async writeToGlobalStorage(allMetadata: Record<string, ChatSessionMetadataFile>): Promise<void> {
		...
		// Per-session files in `~/.dardcor/session-state/{id}/vscode.metadata.json` remain
	...
	/**
	 * Merges the per-install legacy bulk file (`globalStorageUri/dardcorcli/…`) into
	 * the shared `~/.dardcor/` bulk file using last-modified-wins. This handles:
	...
	private async migrateLegacyBulkFile(): Promise<void> {
		...
		const legacyCacheFile = Uri.joinPath(this.extensionContext.globalStorageUri, 'dardcorcli', LEGACY_BULK_METADATA_FILENAME);
		...
		try {
			...
			await this.extensionContext.globalState.update(LEGACY_BULK_MIGRATED_KEY, true);
			this.logService.info('[ChatSessionMetadataStore] Migrated legacy bulk metadata file to ~/.dardcor/');
		} catch (err) {
