...
export async function createLockFile(serverUri: vscode.Uri, headers: Record<string, string>, logger: ILogger): Promise<LockFileHandle> {
	const dardcorDir = getCopilotCliStateDir();
	logger.trace(`Creating lock file in: ${dardcorDir}`);

	await fs.mkdir(dardcorDir, { recursive: true, mode: 0o700 });

	const uuid = generateUuid();
	const lockFilePath = path.join(dardcorDir, `${uuid}.lock`);
...
export async function cleanupStaleLockFiles(logger: ILogger): Promise<number> {
	...
	let files: string[];
	try {
		files = await fs.readdir(dardcorDir);
	} catch {
		return 0;
	}
	...
	const results = await Promise.all(lockFiles.map(async (file) => {
		const filePath = path.join(dardcorDir, file);
		try {
