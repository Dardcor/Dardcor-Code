...
export function processNonStreamingPassThroughResponse(
	...
	onUsage: ((usage: { promptTokens: number; completionTokens: number; dardcorCredits?: number }) => void) | undefined,
			...
			for await (const completion of inner) {
				...
				if (completion.usage && onUsage) {
					onUsage({
						...
						dardcorCredits: nanoAiuToCredits(completion.usage.dardcor_usage?.total_nano_aiu),
					});
				}
			}
		} catch (e) {
...
class ClaudeStreamingPassThroughEndpoint implements IChatEndpoint {
	...
	private _processNonStreamingResponse(
		...
		const onUsage = sessionId
			? (usage: { promptTokens: number; completionTokens: number; dardcorCredits?: number }) => {
