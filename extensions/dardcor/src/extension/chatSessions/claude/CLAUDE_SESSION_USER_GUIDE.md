...
```json
{
  "github.dardcor.chat.claudeAgent.enabled": false
}
```
...
| `auto` | **Auto (model classifier)** | Uses a model classifier to automatically approve or deny tool operations. Visible by default; controlled by `github.dardcor.chat.claudeAgent.allowAutoPermissions`. |
| `bypassPermissions` | **Bypass all permissions** | All tools run without confirmation. ⚠️ Only visible when `github.dardcor.chat.claudeAgent.allowDangerouslySkipPermissions` is enabled. **Recommended only for sandboxed environments with no internet access.** |
...
|---------|------|---------|-------------|
| `github.dardcor.chat.claudeAgent.enabled` | boolean | `true` | Enable Claude Agent sessions in VS Code |
| `github.dardcor.chat.claudeAgent.allowDangerouslySkipPermissions` | boolean | `false` | Show "Bypass all permissions" option. ⚠️ Sandboxes only |
| `github.dardcor.chat.claudeAgent.allowAutoPermissions` | boolean | `true` | Show "Auto (model classifier)" permission mode option |
