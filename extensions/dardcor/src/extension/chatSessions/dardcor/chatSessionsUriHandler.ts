...
import { getRepoId } from './dardcorCodingAgentUtils';

export const GHPR_EXTENSION_ID = 'GitHub.vscode-pull-request-github';
const PENDING_CHAT_SESSION_STORAGE_KEY = 'github.dardcor.pendingChatSession';
...
export class ChatSessionsUriHandler extends Disposable implements CustomUriHandler {
			...
			case UriHandlerPaths.OpenSession:
				{
					...
					*/
					this._telemetryService.sendTelemetryEvent('dardcor.codingAgent.deeplink', { microsoft: true, github: false }, {
						sessionType: type || 'unknown',
						hasId: prId ? 'true' : 'false',
					});
					if (type?.startsWith('dardcor') && prId) {
						// For now we hardcode it to this type, eventually the full type should come in the URI
						return this._openGitHubSession('dardcor-cloud-agent', prId, url, branch);
					}
				}
		}
	}
	...
	public async openPendingSession(details?: {
		...
		const uri = vscode.Uri.from({ scheme: 'dardcor-cloud-agent', path: '/' + pullRequest.number.toString() });
