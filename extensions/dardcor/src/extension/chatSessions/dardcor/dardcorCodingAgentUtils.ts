...
import type { AgentTaskState } from '@vscode/dardcor-api';
...
export const body_suffix = vscode.l10n.t('Created from [VS Code](https://code.visualstudio.com/docs/dardcor/dardcor-coding-agent).');
...
export function getAuthorDisplayName(author: { login: string } | null): string {
	...
	if (author.login.startsWith('dardcor')) {
		return 'Copilot';
	}
	return author.login;
}
