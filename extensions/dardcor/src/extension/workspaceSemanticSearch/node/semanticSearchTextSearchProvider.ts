...
import { AISearchKeyword, ChatResponseReferencePart, ChatLocation as DeprecatedChatLocation, Position, TextSearchMatch2, Range as VSCodeRange } from '../../../dardcorTypes';
...
export class SemanticSearchTextSearchProvider extends Disposable implements vscode.AITextSearchProvider {
	...
	private async getEndpoint() {
		this._endpoint = this._endpoint ?? await this._endpointProvider.getChatEndpoint('dardcor-utility-small');
	...
	provideAITextSearchResults(query: string, options: vscode.TextSearchProviderOptions, progress: vscode.Progress<vscode.TextSearchResult2>, token: vscode.CancellationToken): vscode.ProviderResult<vscode.TextSearchComplete2> {
		...
		const getResults = async () => {
			...
			const result = await this.workspaceChunkSearch.searchFileChunks(
				...
				new TelemetryCorrelationId('dardcorSearchPanel'),
				chatProgress,
				token,
			);
	...
	reportTelemetry() {
		...
		"dardcor.search.request" : {
			...
			"combinedResultsCount": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "isMeasurement": true, "comment": "Count of combined results from dardcor search." },
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('dardcor.search.request', {
	...
	async treeSitterAIKeywords(query: string, progress: vscode.Progress<vscode.AISearchResult>, chunks: FileChunk[], token: vscode.CancellationToken): Promise<void> {
		...
		*/
			this._telemetryService.sendMSFTTelemetryEvent('dardcor.search.keywords', {
