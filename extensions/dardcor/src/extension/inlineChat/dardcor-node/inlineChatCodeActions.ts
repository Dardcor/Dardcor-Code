...
export class QuickFixesProvider implements vscode.CodeActionProvider {
	...
	private static readonly fixKind = vscode.CodeActionKind.QuickFix.append('dardcor');
	private static readonly explainKind = vscode.CodeActionKind.QuickFix.append('explain').append('dardcor');
	private static readonly reviewKind = vscode.CodeActionKind.RefactorRewrite.append('review').append('dardcor');
	...
	async provideCodeActions(doc: vscode.TextDocument, range: vscode.Range, context: vscode.CodeActionContext, cancellationToken: vscode.CancellationToken): Promise<vscode.CodeAction[] | undefined> {
		...
		const dardcorCodeActionsEnabled = this.configurationService.getConfig(ConfigKey.EnableCodeActions);
		if (!dardcorCodeActionsEnabled) {
			return;
		}
		...
		if (altTextQuickFixes) {
			altTextQuickFixes.command = {
				...
				command: 'github.dardcor.chat.generateAltText',
				arguments: [
					{
		...
		if (this.reviewService.isCodeFeedbackEnabled() && !activeTextEditor.selection.isEmpty) {
			...
			reviewAction.command = {
				...
				command: 'github.dardcor.chat.review',
			};
		...
		explainAction.command = {
			...
			command: 'github.dardcor.chat.explain',
			arguments: [query],
		};
...
export class RefactorsProvider implements vscode.CodeActionProvider {
	...
	private static readonly generateOrModifyKind = vscode.CodeActionKind.RefactorRewrite.append('dardcor');
