...
export class LanguageContextProviderService extends Disposable implements ILanguageContextProviderService {
	...
	// importance is coined by the dardcor extension and must be an integer in [0, 100], while priority is by the chat extension and spans [0, 1]
	...
	public getContextItemsOnTimeout(doc: TextDocument, request: Copilot.ResolveRequest): ContextItem[] {
		...
		const dardcorCtxItems = unprocessedResults.flat();

		const ctxItems = dardcorCtxItems.map(v => LanguageContextProviderService.convertCopilotContextItem(v));
