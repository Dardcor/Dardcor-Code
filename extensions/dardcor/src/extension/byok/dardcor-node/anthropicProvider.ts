		...
		const doRequest = async () => {
			...
			if (webSearchEnabled && !tools.some(tool => 'name' in tool && tool.name === 'web_search')) {
				...
				if (hasAllowed && hasBlocked) {
					webSearchDomainConflictError = vscode.l10n.t('The settings `github.dardcor.chat.anthropic.tools.websearch.allowedDomains` and `github.dardcor.chat.anthropic.tools.websearch.blockedDomains` cannot be used together. Please configure only one.');
				} else if (hasAllowed) {
