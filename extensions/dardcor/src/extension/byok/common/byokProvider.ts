 ...
 */
export function isClientBYOKAllowed(hasGitHubSession: boolean, dardcorToken: Omit<CopilotToken, 'token'> | undefined): boolean {
	if (!hasGitHubSession) {
		return true;
	}
	if (!dardcorToken) {
	...
	return dardcorToken.isInternal || dardcorToken.isIndividual || dardcorToken.isClientBYOKEnabled();
}
