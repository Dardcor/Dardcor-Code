...
export async function renderPromptElementJSON<P extends BasePromptElementProps>(
	...
	// todo@connor4312: we don't know what model the tool call will use, just assume dardcor base
	...
	const endpoint = await instantiationService.invokeFunction(async (accessor) => {
		...
		try {
			return await endpointProvider.getChatEndpoint('dardcor-utility');
		} catch {
