...
import { DiagnosticSeverity } from '../../../../util/dardcor/workbench/api/common/extHostTypes/diagnostic';
import { ChatReferenceBinaryData, ChatReferenceDiagnostic, LanguageModelToolResult2, Range, Uri } from '../../../../dardcorTypes';
...
export class ChatToolReferences extends PromptElement<ChatToolCallProps, void> {
	...
	override async render(state: void, sizing: PromptSizing, _progress: unknown, token?: CancellationToken): Promise<PromptPiece<any, any> | undefined> {
		...
		for (const toolReference of tools.toolReferences) {
			...
			const toolArgsEndpoint = await this.endpointProvider.getChatEndpoint('dardcor-utility-small');
		...
		const fetchResult = await endpoint.makeChatRequest(
			...
			async (text, _, delta) => {
				if (delta.dardcorToolCalls) {
					fnCall = delta.dardcorToolCalls[0];
				}
				return undefined;
			},
