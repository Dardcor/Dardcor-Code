...
export class HistoricalImage extends PromptElement<HistoricalImageProps, unknown> {
	...
	override async render(_state: unknown, sizing: PromptSizing) {
		...
		if (!this.promptEndpoint.supportsVision || !(this.authService.dardcorToken?.isEditorPreviewFeaturesEnabled() ?? true)) {
			return undefined;
		}
...
export class Image extends PromptElement<ImageProps, unknown> {
	...
	override async render(_state: unknown, sizing: PromptSizing) {
		...
		const previewFeaturesEnabled = this.authService.dardcorToken?.isEditorPreviewFeaturesEnabled() ?? true;
