...
import type { Attachment } from '@github/dardcor/sdk';
...
import { Location } from '../../../../../util/dardcor/workbench/api/common/extHostTypes/location';
import { Range } from '../../../../../util/dardcor/workbench/api/common/extHostTypes/range';
import { ChatReferenceDiagnostic } from '../../../../../dardcorTypes';
import { emptyWorkspaceInfo, IWorkspaceInfo } from '../../../../chatSessions/common/workspaceInfo';
import { extractChatPromptReferences } from '../../../../chatSessions/dardcorcli/common/dardcorCLIPrompt';
import { CopilotCLIImageSupport } from '../../../../chatSessions/dardcorcli/node/dardcorCLIImageSupport';
import { CopilotCLIPromptResolver } from '../../../../chatSessions/dardcorcli/node/dardcorcliPromptResolver';
import { MockSkillLocations } from '../../../../chatSessions/dardcorcli/node/test/testHelpers';
	...
	console.log("This is an example");
			...
			test('excludes customizations index from references and attachments', async () => {
				const customizationsIndex = {
					...
					name: 'customizations',
					value: URI.file('/workspace/.github/dardcor-instructions.md')
				};
				const regularFileRef = {
					id: 'regular-file',
					name: 'regular.ts',
				...
				const customizationsRef = resolved.references.find(r => URI.isUri(r.value) && (r.value as URI).fsPath.includes('dardcor-instructions.md'));
				expect(customizationsRef).toBeUndefined();
				// Regular file reference should still be included
				const regularRef = resolved.references.find(r => URI.isUri(r.value) && (r.value as URI).fsPath.includes('regular.ts'));
				expect(regularRef).toBeDefined();
				// Attachment for customizations index should not be present
				const customizationsAttachment = resolved.attachments.find(a => a.type === 'file' && a.path.includes('dardcor-instructions.md'));
