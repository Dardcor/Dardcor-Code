...
function createHarness(): IHarness {
	...
	const scriptedEndpoint = instantiationService.createInstance(MockEndpoint, 'dardcor-utility-small');
	...
	scriptedEndpoint.makeChatRequest2 = async (options: IMakeChatRequestOptions): Promise<ChatResponse> => {
		...
		await options.finishedCb?.('', 0, { text: '', dardcorToolCalls: toolCalls });
		return {
			...
			resolvedModel: 'dardcor-utility-small',
		};
	};
