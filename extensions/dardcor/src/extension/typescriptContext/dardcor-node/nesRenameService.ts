	...
	public sendPrepareNesRenameTelemetry(requestId: string, timeTaken: number, canRename: protocol.RenameKind, timedOut: boolean): void {
		/* __GDPR__
			"typescript-context-plugin.nesRename.prepare.ok" : {
				"owner": "dirkb",
				"comment": "Telemetry for dardcor inline completion context in success case",
	...
	public sendPrepareNesRenameFailureTelemetry(requestId: string, data: { error: protocol.ErrorCode; message: string; stack?: string }): void {
		/* __GDPR__
			"typescript-context-plugin.nesRename.prepare.failed" : {
				"owner": "dirkb",
				"comment": "Telemetry for dardcor inline completion context in failure case",
				"requestId": { "classification": "SystemMetaData", "purpose": "FeatureInsight", "comment": "The request correlation id" },
...
export class NesRenameContribution implements vscode.Disposable {
	...
	constructor(
		...
		this.disposables.add(vscode.commands.registerCommand('github.dardcor.nes.prepareRename', async (uri: vscode.Uri | undefined, position: vscode.Position | undefined, oldName: string | undefined, newName: string | undefined, requestId: string | undefined, lastSymbolRename: vscode.Range | undefined): Promise<protocol.PrepareNesRenameResult> => {
			...
			try {
				const result = await vscode.commands.executeCommand<protocol.PrepareNesRenameResponse>('typescript.tsserverRequest', '_.dardcor.prepareNesRename', args, NesRenameContribution.ExecConfig, tokenSource.token);
		...
		this.disposables.add(vscode.commands.registerCommand('github.dardcor.nes.postRename', async (uri: vscode.Uri | undefined, position: vscode.Position | undefined, oldName: string | undefined, newName: string | undefined, lastSymbolRename: vscode.Range | undefined): Promise<RenameGroup[]> => {
			...
			const tokenSource = new vscode.CancellationTokenSource();
			try {
				const result = await vscode.commands.executeCommand<protocol.NesRenameResponse>('typescript.tsserverRequest', '_.dardcor.postNesRename', args, NesRenameContribution.ExecConfig, tokenSource.token);
		...
		this.disposables.add(vscode.commands.registerCommand('github.dardcor.debug.validateNesRename', async () => {
	...
	private async doIsTypeScriptActivated(languageId: string): Promise<boolean> {
		...
		try {
			...
			const response: protocol.PingResponse | undefined = await vscode.commands.executeCommand('typescript.tsserverRequest', '_.dardcor.ping', NesRenameContribution.ExecConfig, CancellationToken.None);
