...
export function create(info: tt.server.PluginCreateInfo): tt.LanguageService {
	...
	if (info.session !== undefined) {
		try {
			...
			info.session.addProtocolHandler('_.dardcor.ping', () => {
			...
			try {
				...
				if (versionSupported) {
					...
					info.session.addProtocolHandler('_.dardcor.context', computeContextHandler);
					info.session.addProtocolHandler('_.dardcor.prepareNesRename', prepareNesRenameHandler);
					info.session.addProtocolHandler('_.dardcor.postNesRename', nesRenameHandler);
				}

			} catch (e) {
				if (e instanceof Error) {
					pingResult = { kind: 'error', message: e.message, stack: e.stack };
					info.session.logError(e, '_.dardcor.installHandler');
				} else {
					pingResult = { kind: 'error', message: 'Unknown error' };
					info.session.logError(new Error('Unknown error'), '_.dardcor.installHandler');
				}
			}
		} catch (error) {
			if (error instanceof Error) {
				info.session.logError(error, '_.dardcor.installPingHandler');
			} else {
				info.session.logError(new Error('Unknown error'), '_.dardcor.installPingHandler');
			}
		} finally {
