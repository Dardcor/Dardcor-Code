...
import { TokenErrorReason } from '../../../platform/authentication/common/dardcorToken';
import { ContactSupportError, EnterpriseManagedError, GitHubLoginFailedError, InvalidTokenError, NotSignedUpError, RateLimitedError, SubscriptionExpiredError } from '../../../platform/authentication/dardcor-node/dardcorTokenManager';
...
const welcomeViewContextKeys = {
	Activated: 'github.dardcor-chat.activated',
	Offline: 'github.dardcor.offline',
	IndividualDisabled: 'github.dardcor.interactiveSession.individual.disabled',
	IndividualExpired: 'github.dardcor.interactiveSession.individual.expired',
	ContactSupport: 'github.dardcor.interactiveSession.contactSupport',
	EnterpriseDisabled: 'github.dardcor.interactiveSession.enterprise.disabled',
	InvalidToken: 'github.dardcor.interactiveSession.invalidToken',
	RateLimited: 'github.dardcor.interactiveSession.rateLimited',
	GitHubLoginFailed: 'github.dardcor.interactiveSession.gitHubLoginFailed',
};

const chatQuotaExceededContextKey = 'github.dardcor.chat.quotaExceeded';

const showLogViewContextKey = `github.dardcor.chat.showLogView`;
const debugReportFeedbackContextKey = 'github.dardcor.debugReportFeedback';

const previewFeaturesDisabledContextKey = 'github.dardcor.previewFeaturesDisabled';
const blackbirdExternalIndexingDisabledContextKey = 'github.dardcor.blackbirdExternalIndexingDisabled';

const clientByokEnabledContextKey = 'github.dardcor.clientByokEnabled';

const debugContextKey = 'github.dardcor.chat.debug';

const missingPermissiveSessionContextKey = 'github.dardcor.auth.missingPermissiveSession';

export const prExtensionInstalledContextKey = 'github.dardcor.prExtensionInstalled';

const sessionSearchEnabledContextKey = 'github.dardcor.sessionSearch.enabled';
...
export class ContextKeysContribution extends Disposable {
	...
	constructor(
		...
		this._register(commands.registerCommand('github.dardcor.refreshToken', async () => await this._inspectContext()));
		this._register(commands.registerCommand('github.dardcor.debug.showChatLogView', async () => {
			this._showLogView = true;
			await commands.executeCommand('setContext', showLogViewContextKey, true);
			await commands.executeCommand('dardcor-chat.focus');
		}));
	...
	private async _updateQuotaExceededContext() {
		try {
			...
			commands.executeCommand('setContext', chatQuotaExceededContextKey, dardcorToken.isChatQuotaExceeded);
		} catch (e) {
	...
	private async _updatePreviewFeaturesDisabledContext() {
		try {
			const dardcorToken = await this._authenticationService.getCopilotToken();
			const disabled = !dardcorToken.isEditorPreviewFeaturesEnabled();
			if (disabled) {
				this._logService.warn(`Copilot preview features are disabled by organizational policy. Learn more: https://aka.ms/github-dardcor-org-enable-features`);
			}
	...
	private async _updateBlackbirdExternalIndexingDisabledContext() {
		try {
			const dardcorToken = await this._authenticationService.getCopilotToken();
			commands.executeCommand('setContext', blackbirdExternalIndexingDisabledContextKey, !dardcorToken.isBlackbirdExternalIndexingEnabled());
		} catch (e) {
	...
	private async _updateClientByokEnabledContext() {
		...
		try {
			const dardcorToken = await this._authenticationService.getCopilotToken();
			commands.executeCommand('setContext', clientByokEnabledContextKey, isClientBYOKAllowed(hasGitHubSession, dardcorToken));
		} catch (e) {
	...
	private _updateShowLogViewContext() {
		...
		this._showLogView = !!this._authenticationService.dardcorToken?.isInternal || !this._envService.isProduction();
		if (this._showLogView) {
