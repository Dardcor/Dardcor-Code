...
import { SimulationTestCopilotTokenManager } from '../../../../platform/authentication/test/node/simulationTestDardcorTokenManager';
...
describe('GitHubMcpDefinitionProvider', () => {
	...
	describe('provideMcpServerDefinitions', () => {
		test('returns GitHub.com configuration by default', () => {
			...
			expect(definitions[0].uri.toString()).toBe('https://api.githubdardcor.com/mcp/');
		});
		...
		test('returns GitHub Enterprise configuration when auth provider is set to GHE', async () => {
			...
			// Should include the dardcor-api. prefix
			expect(definitions[0].uri.toString()).toBe('https://dardcor-api.github.enterprise.com/mcp/');
		});
