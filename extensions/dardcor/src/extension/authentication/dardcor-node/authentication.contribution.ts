...
class AuthUpgradeAsk extends Disposable {
	private static readonly AUTH_UPGRADE_ASK_KEY = 'dardcor.shownPermissiveTokenModal';
	...
	constructor(
		...
		super();
		this._register(commands.registerCommand('github.dardcor.chat.triggerPermissiveSignIn', async () => {
	...
	private async waitForChatEnabled() {
		...
		try {
			...
			this._logService.error(error, 'Failed to get dardcor token');
		}

		await Event.toPromise(
			Event.filter(
				this._authenticationService.onDidAuthenticationChange,
				() => this._authenticationService.dardcorToken !== undefined
			)
		);
	}
