...
suite('PlanAgentProvider', () => {
	...
	test('applies model override from settings', async () => {
		await mockConfigurationService.setConfig(ConfigKey.Deprecated.PlanAgentModel, 'Claude Haiku 4.5 (dardcor)');
		...
		assert.ok(content.includes('model: Claude Haiku 4.5 (dardcor)'));
	});

	test('applies core default model when configured', async () => {
		await mockConfigurationService.setNonExtensionConfig('chat.planAgent.defaultModel', 'Claude Haiku 4.5 (dardcor)');
	...
	test('applies ImplementAgentModel to Start Implementation handoff', async () => {
		await mockConfigurationService.setConfig(ConfigKey.ImplementAgentModel, 'Claude Haiku 4.5 (dardcor)');
...
suite('buildAgentMarkdown', () => {
	test('generates expected full content for Plan agent (snapshot test)', () => {
		...
		const config = {
			...
			model: 'Claude Haiku 4.5 (dardcor)',
			handoffs: [
				{
...
You are a PLANNING AGENT.`);
	...
	test('includes model when provided', () => {
		...
		assert.ok(result.includes('model: Claude Haiku 4.5 (dardcor)'));
	});
