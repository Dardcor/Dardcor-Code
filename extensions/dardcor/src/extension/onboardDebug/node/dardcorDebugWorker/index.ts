...
// in dardcorDebugWorker.ps1 instead. This is because Electron on Windows
...
if (!args.length || flagConfig[Flags.Help]) {
	console.log(`Usage: dardcor-debug [${Object.keys(flagConfig).join('] [')}] <command> <args...>`);
	console.log('');
	console.log('Options:');
...
const pipeName = `dardcor-dbg.${process.pid}-${randomBytes(4).toString('hex')}.sock`;
