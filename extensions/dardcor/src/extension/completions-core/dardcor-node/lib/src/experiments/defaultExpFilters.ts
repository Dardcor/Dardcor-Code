...
import { CopilotToken } from '../auth/dardcorTokenManager';
...
export function setupCompletionsExperimentationService(accessor: ServicesAccessor): IDisposable {
	...
	// ensuring dardcorTrackingId is available for experiment assignment.
	const disposable = authService.onDidCopilotTokenChange(() => {
		instantiationService.invokeFunction(updateCompletionsFilters, authService.dardcorToken);
	});

	updateCompletionsFilters(accessor, authService.dardcorToken);
...
export function createCompletionsFilters(accessor: ServicesAccessor, token: Omit<CopilotToken, 'token'> | undefined) {
	...
	if (token) {
		...
		const dardcorTrackingId = token.getTokenValue('tid') ?? '';
		...
		filters.set(Filter.CopilotTrackingId, dardcorTrackingId);
