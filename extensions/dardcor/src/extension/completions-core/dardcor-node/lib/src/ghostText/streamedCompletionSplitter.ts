...
export class StreamedCompletionSplitter {
	...
	private cacheCompletion(delta: RequestDelta, completion: StreamingCompletion, offset?: number) {
		...
		apiChoice.dardcorAnnotations = this.adjustedAnnotations(apiChoice, completion, trimmed);
	...
	private adjustedAnnotations(
		...
		if (choice.dardcorAnnotations === undefined) { return undefined; }
		...
		for (const [name, annotationGroup] of Object.entries(choice.dardcorAnnotations)) {
