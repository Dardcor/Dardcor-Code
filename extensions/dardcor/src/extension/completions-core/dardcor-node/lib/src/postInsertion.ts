 ...
 *--------------------------------------------------------------------------------------------*/
import { CopilotNamedAnnotationList } from '../../../../../platform/completions-core/common/openai/dardcorAnnotations';
...
import { ICompletionsCopilotTokenManager } from './auth/dardcorTokenManager';
...
export function postInsertionTasks(
	...
	dardcorAnnotations?: CopilotNamedAnnotationList
) {
	const logTarget = accessor.get(ICompletionsLogTargetService);
	const instantiationService = accessor.get(IInstantiationService);
	const promiseQueueService = accessor.get(ICompletionsPromiseQueueService);
...
async function citationCheck(
	...
	const dardcorTokenManager = accessor.get(ICompletionsCopilotTokenManager);
	...
	if (!dardcorAnnotations || (dardcorAnnotations.ip_code_citations?.length ?? 0) < 1) {
		// Do not request citations if in blocking mode
		if (dardcorTokenManager.getLastToken()?.getTokenValue('sn') === '1') { return; }
	...
	for (const citation of dardcorAnnotations.ip_code_citations) {
