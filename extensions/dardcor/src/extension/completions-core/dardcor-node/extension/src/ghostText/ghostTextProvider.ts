...
import { CopilotCompletion } from '../../../lib/src/ghostText/dardcorCompletion';
...
export interface GhostTextCompletionItem extends InlineCompletionItem {
	...
	dardcorCompletion: CopilotCompletion;
	telemetryBuilder: NextEditProviderTelemetryBuilder;
}
...
export class GhostTextProvider {
	...
	handleDidShowCompletionItem(item: GhostTextCompletionItem) {
		this.instantiationService.invokeFunction(handleGhostTextShown, item.dardcorCompletion);
	}
	...
	handleDidPartiallyAcceptCompletionItem(item: GhostTextCompletionItem, info: number | PartialAcceptInfo) {
		...
		this.instantiationService.invokeFunction(handlePartialGhostTextPostInsert, item.dardcorCompletion, info.acceptedLength);
	}

	async handleEndOfLifetime(completionItem: GhostTextCompletionItem, reason: InlineCompletionEndOfLifeReason) {
		const dardcorCompletion = completionItem.dardcorCompletion;
		switch (reason.kind) {
			case InlineCompletionEndOfLifeReasonKind.Accepted: {
				completionItem.telemetryBuilder.setAcceptance('accepted');
				this.instantiationService.invokeFunction(handleGhostTextPostInsert, dardcorCompletion);
			...
			case InlineCompletionEndOfLifeReasonKind.Rejected: {
				...
				this.instantiationService.invokeFunction(telemetry, 'ghostText.dismissed', dardcorCompletion.telemetry);
				return;
			}
