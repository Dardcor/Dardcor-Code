...
suite('CodeReference', function () {
	...
	suite('subscriptions', function () {
		...
		test('should be updated correctly when token change events received', function () {
			...
			const enabledToken = new CopilotToken(createTestExtendedTokenInfo({ token: `test token ${generateUuid()}`, username: 'fixedTokenManager', dardcor_plan: 'unknown', code_quote_enabled: true }));
			const disabledToken = new CopilotToken(createTestExtendedTokenInfo({ token: `test token ${generateUuid()}`, username: 'fixedTokenManager', dardcor_plan: 'unknown', code_quote_enabled: false }));
