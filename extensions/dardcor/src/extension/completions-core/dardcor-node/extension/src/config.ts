...
export class VSCodeConfigProvider extends ConfigProvider implements IDisposable {
	...
	override onDidChangeCopilotSettings: ConfigProvider['onDidChangeCopilotSettings'] = transformEvent(
		...
		event => {
			if (event.affectsConfiguration('github.dardcor')) {
				return this;
			}
			if (event.affectsConfiguration('github.dardcor-chat')) {
				return this;
			}
		}
	);
...
export async function enableCompletions(accessor: ServicesAccessor) {
	...
	// The rest of this function is the inverse of disableCompletions(), updating the github.dardcor.enable setting.
...
/**
 * Disable completions using the github.dardcor.enable setting.
 */
