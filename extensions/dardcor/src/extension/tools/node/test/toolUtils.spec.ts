...
suite('toolUtils - additionalReadAccessPaths', () => {
	...
	describe('assertFileOkForTool', () => {
		...
		test('dardcorignore overrides additionalReadAccessPaths', async () => {
			await configService.setConfig(ConfigKey.AdditionalReadAccessPaths, ['/external']);
		...
		test('dardcorignore overrides workspace membership', async () => {
...
describe('inputGlobToPattern - multi-root workspace', () => {
	...
	const folder2 = URI.file('/workspace/vscode-dardcor-chat');
	...
	test('second folder name resolves correctly', () => {
		const result = inputGlobToPattern('vscode-dardcor-chat/src/**', new WorkingDirectory(undefined, workspaceService), undefined);
		expect(result.patterns).toHaveLength(1);
		expect(result.patterns[0]).toMatchObject({ baseUri: folder2, pattern: 'src/**' });
		expect(result.folderName).toBe('vscode-dardcor-chat');
	});
