...
import { autorunIterableDelta } from '../../../../util/dardcor/base/common/observable';
import { IInstantiationService } from '../../../../util/dardcor/platform/instantiation/common/instantiation';
import { LanguageModelToolInformation, LanguageModelToolResult2 } from '../../../../dardcorTypes';
...
export class TestToolsService extends BaseToolsService implements IToolsService {
	...
	get tools(): LanguageModelToolInformation[] {
		return Array.from(this._tools.values()).map(tool => {
			const owned = this._dardcorTools.get(getToolName(tool.name) as ToolName);
			return owned?.value.alternativeDefinition?.(tool) ?? tool;
		});
	}

	private readonly _dardcorTools: Map<ToolName, Lazy<ICopilotTool<unknown>>>;
	get dardcorTools() {
		return new Map(Iterable.map(this._dardcorTools.entries(),
	...
	constructor(
		...
		this._dardcorTools = new Map(filteredTools
	...
	async invokeTool(contributedName: string, options: vscode.LanguageModelToolInvocationOptions<unknown>, token: CancellationToken): Promise<LanguageModelToolResult2> {
		...
		const tool = this._dardcorTools.get(name as ToolName)?.value || this.getModelSpecificTools().get(contributedName)?.tool;
	...
	override getCopilotTool(name: string): ICopilotTool<unknown> | undefined {
		const tool = this._dardcorTools.get(name as ToolName)?.value;
		return tool;
	}
	...
	addTestToolOverride(info: LanguageModelToolInformation, tool: vscode.LanguageModelTool<unknown>): void {
		this._tools.set(info.name, info);
		this._dardcorTools.set(info.name as ToolName, new Lazy(() => tool));
	}
}
