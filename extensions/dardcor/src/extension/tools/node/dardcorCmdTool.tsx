...
const noConfirmationCommandsWithoutArgs = new Set([
	'github.dardcor.debug.collectDiagnostics',
	'breadcrumbs.toggle',
	...
	'git.stashStaged',
	'git.stashView',
	'github.dardcor.chat.completions.disable',
	'github.dardcor.chat.completions.enable',
	'github.dardcor.chat.completions.toggle',
	'github.dardcor.chat.tools.memory.showMemories',
	'github.dardcor.open.walkthrough',
	'github.dardcor.report',
...
class VSCodeCmdTool implements vscode.LanguageModelTool<IVSCodeCmdToolToolInput> {
	...
	async prepareInvocation(options: vscode.LanguageModelToolInvocationPrepareOptions<IVSCodeCmdToolToolInput>, token: vscode.CancellationToken): Promise<vscode.PreparedToolInvocation> {
		...
		markdownString.appendMarkdown(l10n.t(`Dardcor AI will execute the [{0}]({1}) (\`{2}\`) command.`, options.input.name, commandStr, options.input.commandId));
		if (hasArguments) {
