...
import { ChatResponseReferencePart2, Uri } from '../../../dardcorTypes';
...
export class RemoteAgentContribution implements IDisposable {
	...
	private async _doRefreshRemoteAgents(): Promise<void> {
		...
		try {
			...
			this.logService.error(e, 'Failed to load remote dardcor agents');
		}
	...
	private checkAuthorized(agent: string) {
		...
		const key = `dardcor.agent.${agent}.authorized`;
	...
	private async setAuthorized(agent: string, isGlobal = false) {
		...
		await memento.update(`dardcor.agent.${agent}.authorized`, true);
	}

	private registerAgent(agentData: IAgent | null): Disposable {
		const store = new DisposableStore();
		const participantId = `github.dardcor-dynamic.${agentData?.slug ?? GITHUB_PLATFORM_AGENT_ID}`;
		...
		const agent = store.add(chat.createDynamicChatParticipant(participantId, dynamicProps, async (request, context, responseStream, token): Promise<ICopilotChatResult> => {
			...
			if (!agentData?.slug) {
				...
				if (!accessToken) {
					if (this.authenticationService.isMinimalMode) {
						responseStream.markdown(l10n.t('Minimal mode is enabled. You will need to change `github.dardcor.advanced.authPermissions` to `default` to use this feature.'));
						responseStream.button({
							title: l10n.t('Open Settings (JSON)'),
							command: 'workbench.action.openSettingsJson',
							arguments: [{ revealSetting: { key: 'github.dardcor.advanced.authPermissions' } }]
						});
					} else {
			...
			try {
				...
				// Collect dardcor skills and references to be sent in the request
				const dardcorReferences = [];
				const { dardcor_skills } = await this.resolveCopilotSkills(slug, request);
				...
				try {
					...
					if (result.clientReferences) {
						dardcorReferences.push(...result.clientReferences);
					}
				...
				const response = await endpoint.makeChatRequest(
					...
					[
						...prepareRemoteAgentHistory(participantId, context),
						{
							...
							...(dardcorReferences.length ? { dardcor_references: dardcorReferences } : undefined),
							...(confirmations?.length ? { dardcor_confirmations: confirmations } : undefined),
						}
					],
					async (result, _, delta) => {
						if (delta.dardcorReferences) {
							...
							for (const reference of delta.dardcorReferences) {
						...
						if (delta.dardcorErrors && typeof responseStream.warning === 'function') {
							hadCopilotErrorsOrConfirmations = true;
							for (const error of delta.dardcorErrors) {
								if (reportedProgress) {
						...
						if (delta.dardcorConfirmation) {
							hadCopilotErrorsOrConfirmations = true;
							const confirm = delta.dardcorConfirmation;
					...
					undefined,
					{
						...
						dardcor_thread_id: sessionId,
						...(dardcor_skills ? { dardcor_skills } : undefined)
					},
					true,
					{
				...
				metadata['dardcor_references'] = [...new Set(reportedReferences.values()).values(), ...agentReferences];
				...
				provider: {
					provideCompletionItems: async (query, token) => {
						...
						return items.map<ChatCompletionItem>(i => {
							const item = new ChatCompletionItem(`dardcor.${i.name}`, '#' + i.name, [{ value: i.insertText, level: ChatVariableLevel.Full, description: i.description }]);
	...
	private async resolveCopilotSkills(agent: string, request: ChatRequest): Promise<{ dardcor_skills: string[] }> {
		if (agent === GITHUB_PLATFORM_AGENT_NAME) {
			...
			return { dardcor_skills: [...skills] };
		}

		return { dardcor_skills: [] };
	}
...
function prepareRemoteAgentHistory(agentId: string, context: ChatContext): Raw.ChatMessage[] {
	...
	for (const h of context.history) {
		...
		if (h instanceof ChatResponseTurn) {
			const dardcor_references = h.result.metadata?.['dardcor_references'];
			...
			result.push({
				...
				...(dardcor_references ? { dardcor_references } : undefined)
			});
		}
	}
