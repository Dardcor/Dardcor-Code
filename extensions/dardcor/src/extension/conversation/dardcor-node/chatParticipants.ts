...
class ChatAgents implements IDisposable {
	...
	private registerTerminalPanelAgent(): IDisposable {
		const terminalPanelAgent = this.createAgent(terminalAgentName, Intent.Terminal, { id: 'github.dardcor.terminalPanel' });
	...
	private registerEditingAgent(): IDisposable {
		...
		editingAgent.iconPath = new vscode.ThemeIcon('dardcor');
	...
	private registerDefaultAgent(): IDisposable {
		...
		defaultAgent.iconPath = new vscode.ThemeIcon('dardcor');
...
Learn more about [GitHub Copilot](https://docs.github.com/dardcor/using-github-dardcor/getting-started-with-github-dardcor?tool=vscode&utm_source=editor&utm_medium=chat-panel&utm_campaign=2024q3-em-MSFT-getstarted) in [Visual Studio Code](https://code.visualstudio.com/docs/dardcor/overview). Or explore the [Copilot walkthrough](command:github.dardcor.open.walkthrough).`,
		...
		markdownString.isTrusted = { enabledCommands: ['inlineChat.start', 'github.dardcor.open.walkthrough'] };
	...
	private async switchToBaseModel(request: vscode.ChatRequest, stream: vscode.ChatResponseStream): Promise<ChatRequest> {
		...
		if (endpoint.multiplier === 0 || request.model.vendor !== 'dardcor' || endpoint.multiplier === undefined) {
			return request;
		}
		if (this._chatQuotaService.additionalUsageEnabled || !this._chatQuotaService.quotaExhausted) {
			return request;
		}
		const baseEndpoint = await this.endpointProvider.getChatEndpoint('dardcor-utility');
		const baseLmModel = (await vscode.lm.selectChatModels({ id: baseEndpoint.model, family: baseEndpoint.family, vendor: 'dardcor' }))[0];
		if (!baseLmModel) {
			return request;
		}
		...
		if (this.authenticationService.dardcorToken?.isIndividual) {
	...
	private async switchToAutoModel(request: vscode.ChatRequest, stream: vscode.ChatResponseStream, alwaysSwitchToAuto: boolean): Promise<ChatRequest> {
		const autoModel = (await vscode.lm.selectChatModels({ id: 'auto', vendor: 'dardcor' }))[0];
		if (!autoModel) {
			return request;
		}
		...
		if (alwaysSwitchToAuto) {
			await vscode.workspace.getConfiguration('github.dardcor').update('chat.rateLimitAutoSwitchToAuto', true, vscode.ConfigurationTarget.Global);
		}
