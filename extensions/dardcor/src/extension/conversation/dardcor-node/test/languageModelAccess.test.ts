...
suite('CopilotLanguageModelWrapper', () => {
	...
	suite('validateRequest - invalid', () => {
		...
		setup(async () => {
			createAccessor();
			endpoint = await accessor.get(IEndpointProvider).getChatEndpoint('dardcor-utility');
...
suite('LanguageModelAccess model info', () => {
	test('does not wait for utility alias endpoint resolution', async () => {
		...
		const dardcorToken = new CopilotToken(createTestExtendedTokenInfo({ token: 'token', username: 'fake', dardcor_plan: 'unknown' }));
		const testingServiceCollection = createExtensionTestingServices();
		testingServiceCollection.define(ICopilotTokenManager, {
			_serviceBrand: undefined,
			onDidCopilotTokenRefresh: Event.None,
			getCopilotToken: async () => dardcorToken,
			resetCopilotToken: () => { },
	...
	test('refreshes utility aliases when an override uses the same model id from another provider', async () => {
		...
		internals._utilityAliasEndpoints.set('dardcor-utility-small', publishedEndpoint);
		internals._promptBaseCountCache = { getBaseCount: async () => 0 };
		try {
			await internals._refreshUtilityOverrides();
			assert.strictEqual(internals._resolvedUtilityEndpoints.get('dardcor-utility-small')?.endpoint, resolvedEndpoint);
		} finally {
			languageModelAccess.dispose();
		}
	});
...
suite('buildUtilityAliasModelInfo', () => {
	...
	test('clones an existing dardcor-provider entry, overriding id/family/selectable/default', () => {
		...
		const result = buildUtilityAliasModelInfo('dardcor-utility-small', endpoint, [base], /* baseCount */ 50, undefined);

		assert.strictEqual(result.synthesized, false);
		assert.deepStrictEqual(result.info, {
			...base,
			id: 'dardcor-utility-small',
			family: 'dardcor-utility-small',
			isUserSelectable: false,
			isDefault: false,
		});
	});

	test('synthesizes for non-dardcor endpoints when a dardcor model shares the same id', () => {
		...
		const base = makeBaseModelInfo({ id: 'gpt-4o-mini', requiresAuthorization: { label: 'octocat' } });
		const result = buildUtilityAliasModelInfo('dardcor-utility-small', endpoint, [base], /* baseCount */ 50, { label: 'octocat' });
	...
	test('synthesizes when no matching base entry exists, subtracting baseCount and completion reserve', () => {
		...
		const result = buildUtilityAliasModelInfo('dardcor-utility', endpoint, [], /* baseCount */ 100, { label: 'octocat' });

		assert.strictEqual(result.synthesized, true);
		assert.strictEqual(result.info.id, 'dardcor-utility');
		assert.strictEqual(result.info.name, 'Unknown Model');
		assert.strictEqual(result.info.family, 'dardcor-utility');
		...
		// `vscode.lm.selectChatModels({ vendor: 'dardcor', id: 'dardcor-utility' })`
