...
suite('Conversation feature test suite', function () {
	...
	test('The feature is enabled and activated in test mode', function () {
		...
		try {
			...
			setCopilotToken(accessor.get(IAuthenticationService), dardcorToken);
	...
	test('The feature activates without a Copilot token when a non-dardcor (BYOK) language model is available', async function () {
