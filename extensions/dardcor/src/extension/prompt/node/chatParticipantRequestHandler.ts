...
import { ChatRequestEditorData, ChatRequestNotebookData, ChatRequestTurn, ChatResponseAnchorPart, ChatResponseFileTreePart, ChatResponseMarkdownPart, ChatResponseProgressPart2, ChatResponseReferencePart, ChatResponseTurn, ChatLocation as VSChatLocation } from '../../../dardcorTypes';
		...
		try {
			...
			if (!result) {
				...
				const hideAutoModelName = isAutoModel(endpoint) === 1
					&& this._experimentationService.getTreatmentVariable<boolean>('dardcorchat.hideAutoModelName') === true;
				if (hideAutoModelName) {
					result.details = formatAutoModeDetails(creditsUsed, endpoint.multiplier);
				} else if (this._authService.dardcorToken?.isNoAuthUser) {
...
export function addHistoryToConversation(accessor: ServicesAccessor, history: ReadonlyArray<ChatRequestTurn | ChatResponseTurn>): { turns: Turn[]; sessionId: string | undefined } {
	...
	for (const entry of history) {
		...
		if (entry instanceof ChatRequestTurn) {
			...
			const dardcorResult = entry.result as ICopilotChatResultIn;
			if (typeof dardcorResult.metadata?.sessionId === 'string') {
				sessionId = dardcorResult.metadata.sessionId;
			}
		}
	}
