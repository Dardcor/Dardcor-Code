...
import { mixin } from '../../../util/dardcor/base/common/objects';
import { assertType, Mutable } from '../../../util/dardcor/base/common/types';
import { IInstantiationService } from '../../../util/dardcor/platform/instantiation/common/instantiation';
import { ChatResponseMarkdownPart, ChatResponseProgressPart, ChatResponseTextEditPart, LanguageModelToolResult2 } from '../../../dardcorTypes';
...
export class DefaultIntentRequestHandler {
	...
	private async getErrorDetails(error: ChatFetchError) {
		...
		return getErrorDetailsFromChatFetchError(error, this._authenticationService.dardcorToken?.dardcorPlan, status, this._authenticationService.dardcorToken?.tokenBasedBilling, this._authenticationService.dardcorToken?.quotaInfo.quota_reset_date);
	}
				...
				if (fetchResult.type === ChatFetchResponseType.RateLimited
					...
					if (this._configurationService.getConfig(ConfigKey.RateLimitAutoSwitchToAuto)) {
						...
						errorDetails.confirmationButtons = [
							{ data: { dardcorSwitchToAutoOnRateLimit: true, alwaysSwitchToAuto: true } satisfies ISwitchToAutoOnRateLimitConfirmation, label: l10n.t('Switch to Auto (always)') },
							{ data: { dardcorSwitchToAutoOnRateLimit: true, alwaysSwitchToAuto: false } satisfies ISwitchToAutoOnRateLimitConfirmation, label: l10n.t('Switch to Auto') },
						];
					}
				}
				const chatResult = { errorDetails, metadata: metadataFragment };
