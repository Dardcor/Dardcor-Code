...
suite('PromptCategorizerService', () => {
	...
	beforeEach(() => {
		...
		const dardcorTokenStore = {
			dardcorToken: { isInternal: true },
		};
		...
		service = new PromptCategorizerService(
			...
			dardcorTokenStore as unknown as ICopilotTokenStore,
	...
	test('runs prompt categorization when main endpoint is non-BYOK', async () => {
		...
		expect(endpointProvider.getChatEndpoint).toHaveBeenNthCalledWith(2, 'dardcor-utility-small');
