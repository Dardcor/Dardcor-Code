...
function processDeltasToMessage(deltas: IResponseDelta[]): string {
	return deltas.map((d, i) => {
		...
		if (d.dardcorToolCalls) {
			if (i > 0) {
				text += '\n';
			}

			text += d.dardcorToolCalls.map(c => {
...
class LoggedRequestInfo implements ILoggedRequestInfo {
	...
	toJSON(): object {
		...
		const metadata = {
			...
			dardcorUsageAic: this.entry.type === LoggedRequestKind.ChatMLSuccess && typeof this.entry.usage?.dardcor_usage?.total_nano_aiu === 'number'
				? this.entry.usage.dardcor_usage.total_nano_aiu / 1_000_000_000 : undefined,
...
`;
	...
	private _renderRequestToMarkdown(id: string, entry: LoggedRequest): string {
		...
		if (entry.type === LoggedRequestKind.ChatMLSuccess) {
			...
			if (typeof entry.usage?.dardcor_usage?.total_nano_aiu === 'number') {
				const aic = entry.usage.dardcor_usage.total_nano_aiu / 1_000_000_000;
				result.push(`dardcorUsage    : ${aic.toFixed(2)} AIC (${entry.usage.dardcor_usage.total_nano_aiu} nano-AIU)`);
			}
