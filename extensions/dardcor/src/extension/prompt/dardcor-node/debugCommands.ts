...
export class DebugCommandsContribution extends Disposable {
	constructor(
		...
		this._register(vscode.commands.registerCommand('github.dardcor.debug.generateSTest', async () => {
		...
		this._register(vscode.commands.registerCommand('github.dardcor.createLaunchJsonFileWithContents', async (launchConfig: IStartDebuggingParsedResponse) => {
		...
		this._register((vscode.commands.registerCommand('github.dardcor.startDebugging', async (config: IStartDebuggingParsedResponse, progress) => {
