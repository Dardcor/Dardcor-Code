...
import { ChatResponseFileTreePart, MarkdownString, Uri } from '../../../dardcorTypes';
...
export const CreateProjectCommand = 'github.dardcor.createProject';
export const CreateFileCommand = 'github.dardcor.createFile';
export const OpenFileCommand = 'github.dardcor.openFile';

export class NewWorkspacePreviewContentManagerImpl implements INewWorkspacePreviewContentManager {
	declare readonly _serviceBrand: undefined;
	private readonly dardcorContentManager: NewWorkspaceCopilotContentManager;
	...
	constructor(
		...
		this.dardcorContentManager = instantiationService.createInstance(NewWorkspaceCopilotContentManager);
	...
	set(responseId: string, projectName: string, fileTree: ChatResponseFileTreePart, serviceArgs: any) {
		...
		if (isGithubWorkspaceUri(fileTree.baseUri)) {
			...
			this.dardcorContentManager.set(responseId, projectName, fileTree, serviceArgs);
		}
	}
	...
	get(uri: Uri): FileTreeDataWithContent | undefined {
		...
		if (isGithubWorkspaceUri(uri)) {
			...
			fileContents = this.dardcorContentManager.get(uri.authority, uri.path, this.prevFileContents);
		}
...
export class NewWorkspaceCopilotContentManager {
	...
	private _getProjectMetadata(fullPath: string) {
		// Format: vscode-dardcor-workspace://<sessionId>/<projectName>/<filePath>
...
class NewWorkspaceGitHubContentManager {
	...
	private _getProjectMetadata(fullPath: string) {
		// Format: vscode-dardcor-github-workspace://<sessionId>/<projectName>/<filePath>
		const [, projectName, ...path] = fullPath.split('/');
		return { projectName, path };
	}
...
class NewWorkspaceFileContentManager {
	...
	private _getFileMetadata(fullPath: string) {
		// Format: vscode-dardcor-file://<sessionId>/<projectName>/<filePath>
		const [, projectName, ...path] = fullPath.split('/');
		return { projectName, path };
	}
...
export const CopilotWorkspaceScheme = 'vscode-dardcor-workspace';
export const GithubWorkspaceScheme = 'vscode-dardcor-github-workspace';
export const CopilotFileScheme = 'vscode-dardcor-file';
...
class NewWorkspaceResponseProcessor {
	...
	async processResponse(context: IResponseProcessorContext, inputStream: AsyncIterable<IResponsePart>, outputStream: vscode.ChatResponseStream, token: vscode.CancellationToken): Promise<void> {
		...
		if (turn.id &&
			...
			const markdownString = new MarkdownString(l10n.t(`Hint: You can [regenerate this project without using this sample](command:workbench.action.chat.open?{0}) or use this [setting](command:workbench.action.openSettings?%5B%22github.dardcor.chat.useProjectTemplates%22%5D) to configure the behavior.`, query));
