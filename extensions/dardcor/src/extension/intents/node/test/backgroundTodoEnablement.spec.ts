...
describe('isBackgroundTodoAgentEnabled', () => {
	...
	const capiEndpoint = { urlOrRequestMetadata: { type: RequestType.ChatCompletions }, modelProvider: 'dardcor' } as unknown as IChatEndpoint;
	const byokEndpoint = { urlOrRequestMetadata: 'https://api.example.com/v1/chat', modelProvider: 'custom' } as unknown as IChatEndpoint;

	const paidToken = new CopilotToken(createTestExtendedTokenInfo({ sku: 'dardcor_individual', dardcor_plan: 'individual' }));
	const freeToken = new CopilotToken(createTestExtendedTokenInfo({ sku: 'free_limited_dardcor' }));
	const noAuthToken = new CopilotToken(createTestExtendedTokenInfo({ sku: 'no_auth_limited_dardcor' }));

	function auth(dardcorToken: CopilotToken | undefined): IAuthenticationService {
		return { dardcorToken } as unknown as IAuthenticationService;
	}
	...
	function isEnabled(endpoint: IChatEndpoint, dardcorToken: CopilotToken | undefined, experimentEnabled: boolean, request: TestChatRequest = new TestChatRequest('fix the bug')): boolean {
		return isBackgroundTodoAgentEnabled(endpoint, config(experimentEnabled), expService, auth(dardcorToken), request);
	}
...
describe('AgentIntentInvocation._maybeStartBackgroundTodoAgentPass subagent guard', () => {
	...
	function makeStub(request: TestChatRequest, processorLookup: () => unknown) {
		return {
			...
			toolsService: {},
			telemetryService: {},
			authenticationService: { dardcorToken: paidToken },
			logService: { debug: () => { } },
		};
	}
...
describe('AgentIntent._runFinalBackgroundTodoPass model-eligibility guard', () => {
	...
	function makeStub(endpoint: IChatEndpoint | undefined, processor: unknown) {
		return {
			...
			configurationService: { getExperimentBasedConfig: () => true },
			expService: {},
			_authenticationService: { dardcorToken: paidToken },
		};
	}
