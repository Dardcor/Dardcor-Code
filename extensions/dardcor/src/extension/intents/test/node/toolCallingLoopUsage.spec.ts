...
class UsageCapturingStream extends ChatResponseStreamImpl {
	public readonly usages: Array<{ promptTokens: number; completionTokens: number; dardcorCredits: number | undefined }>;

	constructor() {
		const usages: Array<{ promptTokens: number; completionTokens: number; dardcorCredits: number | undefined }> = [];
		super(
			() => { },
			() => { },
			undefined,
			...
			(usage) => {
				usages.push({
					...
					dardcorCredits: usage.dardcorCredits
				});
			}
		);
		this.usages = usages;
	...
	protected override async fetch(): Promise<ChatResponse> {
		return {
			...
			usage: {
				...
				completion_tokens: 20,
				total_tokens: 120,
				dardcor_usage: { total_nano_aiu: 5_000_000_000 }
			},
			resolvedModel: 'gpt-4.1'
		};
	}
...
describe('ToolCallingLoop usage reporting', () => {
	...
	it('reports usage for regular requests', async () => {
		...
		expect(stream.usages).toEqual([{ promptTokens: 100, completionTokens: 20, dardcorCredits: undefined }]);
	});
	...
	it('reports credits-only usage for subagent requests', async () => {
		...
		expect(stream.usages).toEqual([{ promptTokens: 0, completionTokens: 0, dardcorCredits: 5 }]);
	});

	it('accumulates dardcor credits across iterations within a turn', async () => {
		const request = createMockChatRequest();
		const loop = instantiationService.createInstance(
			CreditsTestToolCallingLoop,
			{
		...
		expect(stream.usages.map(u => u.dardcorCredits)).toEqual([5, 10]);
	});
});
