...
const showHtmlCommand = 'vscode.dardcor.chat.showRequestHtmlItem';
const exportLogItemCommand = 'github.dardcor.chat.debug.exportLogItem';
const exportPromptArchiveCommand = 'github.dardcor.chat.debug.exportPromptArchive';
...
const exportPromptLogsAsJsonCommand = 'github.dardcor.chat.debug.exportPromptLogsAsJson';
const exportAllPromptLogsAsJsonCommand = 'github.dardcor.chat.debug.exportAllPromptLogsAsJson';
const saveCurrentMarkdownCommand = 'github.dardcor.chat.debug.saveCurrentMarkdown';
const showRawRequestBodyCommand = 'github.dardcor.chat.debug.showRawRequestBody';
...
export class RequestLogTree extends Disposable implements IExtensionContribution {
	...
	constructor(
		...
		this._register(vscode.window.registerTreeDataProvider('dardcor-chat', this.chatRequestProvider));
		...
		// Save the currently opened chat log (ccreq:*.dardcormd) to a file
		...
		this._register(vscode.commands.registerCommand(exportPromptArchiveCommand, async (treeItem: ChatPromptItem) => {
			...
			try {
				...
				const tempDir = path.join(os.tmpdir(), `vscode-dardcor-export-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`);
				...
				for (const logEntry of logEntries) {
					...
					switch (logEntry.kind) {
						case LoggedInfoKind.Request: {
							const requestEntry = logEntry as ILoggedRequestInfo;
							const debugName = requestEntry.entry.debugName.replace(/\W/g, '_');
							filename = `${debugName}_${logEntry.id}.dardcormd`;
							break;
						}
						case LoggedInfoKind.ToolCall: {
							const toolEntry = logEntry as ILoggedToolCall;
							const toolName = toolEntry.name.replace(/\W/g, '_');
							filename = `tool_${toolName}_${logEntry.id}.dardcormd`;
							break;
						}
						default:
							continue;
		...
		this._register(vscode.commands.registerCommand(exportAllPromptLogsAsJsonCommand, async (savePath?: string) => {
			...
			if (savePath && typeof savePath === 'string') {
				...
				const defaultFilename = `dardcor_all_prompts_${timestamp}.json`;
		...
		this._register(vscode.commands.registerCommand('github.dardcor.debug.showOutputChannel', async () => {
			outputChannel.show();
		}));
	}
}
...
class ChatRequestItem extends vscode.TreeItem {
	...
	${startTimeStr}`;
		...
		this.iconPath ??= new vscode.ThemeIcon('dardcor');
	}
}
...
class LogTreeFilters extends Disposable {
	...
	private getStorageKey(name: string): string {
		return `github.dardcor.chat.debug.${name}Hidden`;
	}
	...
	private setShown(name: string, value: boolean): void {
		vscode.commands.executeCommand('setContext', `github.dardcor.chat.debug.${name}Hidden`, !value);
...
class LogTreeFilterCommands extends Disposable {
	constructor(filters: LogTreeFilters) {
		...
		this._register(vscode.commands.registerCommand('github.dardcor.chat.debug.showElements', () => filters.setElementsShown(true)));
		this._register(vscode.commands.registerCommand('github.dardcor.chat.debug.hideElements', () => filters.setElementsShown(false)));
		this._register(vscode.commands.registerCommand('github.dardcor.chat.debug.showTools', () => filters.setToolsShown(true)));
		this._register(vscode.commands.registerCommand('github.dardcor.chat.debug.hideTools', () => filters.setToolsShown(false)));
		this._register(vscode.commands.registerCommand('github.dardcor.chat.debug.showNesRequests', () => filters.setNesRequestsShown(true)));
		this._register(vscode.commands.registerCommand('github.dardcor.chat.debug.hideNesRequests', () => filters.setNesRequestsShown(false)));
		this._register(vscode.commands.registerCommand('github.dardcor.chat.debug.showGhostRequests', () => filters.setGhostRequestsShown(true)));
		this._register(vscode.commands.registerCommand('github.dardcor.chat.debug.hideGhostRequests', () => filters.setGhostRequestsShown(false)));
	}
}
