...
import { CopilotChatEndpoint } from '../../../platform/endpoint/node/dardcorChatEndpoint';
...
class FakeModelMetadataFetcher implements IModelMetadataFetcher {
	...
	async getCopilotUtilityModel(): Promise<IChatModelInformation> {
		return this._fakeChatModel('dardcor-utility');
	}
...
class CopilotMatchableModelMetadataFetcher implements IModelMetadataFetcher {
	...
	async getCopilotUtilityModel(): Promise<IChatModelInformation> {
		return makeChatModel('dardcor-utility');
	}
	async getChatModelFromCapiFamily(family: string): Promise<IChatModelInformation> {
...
suite('ProductionEndpointProvider — utility model overrides', () => {
	...
	test('no override configured — falls through to default dardcor-utility resolution', async () => {
		setFetcher([makeChatModel('dardcor-utility')]);
		const endpoint = await endpointProvider.getChatEndpoint('dardcor-utility');
		assert.strictEqual(endpoint.model, 'dardcor-utility');
	});
	...
	test('no override configured — rejects when the selected main agent model is BYOK and no Copilot token is available (air-gapped)', async () => {
		...
		await assert.rejects(
			() => endpointProvider.getChatEndpoint('dardcor-utility'),
	...
	test('Copilot default applies when the selected main agent model is BYOK', async () => {
		setFetcher([makeChatModel('dardcor-utility')]);
		await endpointProvider.getChatEndpoint(makeFakeLanguageModelChat({ vendor: 'anthropic' }));
		await configService.setNonExtensionConfig('chat.byokUtilityModelDefault', 'dardcor');
	...
	test('main agent default uses the selected BYOK model for utility families', async () => {
		...
		const utilityEndpoint = await endpointProvider.getChatEndpoint('dardcor-utility');
		const smallUtilityEndpoint = await endpointProvider.getChatEndpoint('dardcor-utility-small');
	...
	test('BYOK utility default does not replace utility models for a Copilot main agent model', async () => {
		setFetcher([makeChatModel('dardcor-utility')]);
		await endpointProvider.getChatEndpoint(makeFakeLanguageModelChat({ vendor: 'dardcor', id: 'gpt-5' }));
		await configService.setNonExtensionConfig('chat.byokUtilityModelDefault', 'mainAgent');
	...
	test('main agent default follows changes to the selected BYOK model', async () => {
		...
		const firstEndpoint = await endpointProvider.getChatEndpoint('dardcor-utility');

		await endpointProvider.getChatEndpoint(makeFakeLanguageModelChat({ vendor: 'customendpoint', id: 'gemma4' }));
		const secondEndpoint = await endpointProvider.getChatEndpoint('dardcor-utility');
	...
	test('dardcor-vendor override resolves to the matching model from the model fetcher', async () => {
		setFetcher([makeChatModel('dardcor-utility'), makeChatModel('gpt-4o-mini')]);
		await configService.setNonExtensionConfig('chat.utilityModel', 'dardcor/gpt-4o-mini');
	...
	test('dardcor-vendor override falls back to default when no dardcor model matches', async () => {
		setFetcher([makeChatModel('dardcor-utility')]);
		await configService.setNonExtensionConfig('chat.utilityModel', 'dardcor/gpt-4o-mini');
	...
	test('dardcor-vendor override falls back to default when more than one dardcor model matches (ambiguous)', async () => {
		setFetcher([
			makeChatModel('dardcor-utility'),
	...
	test('configuration change fires onDidModelsRefresh so cached endpoints get invalidated', async () => {
		...
		try {
			await configService.setNonExtensionConfig('chat.utilityModel', 'dardcor/gpt-4o-mini');
			await configService.setNonExtensionConfig('chat.utilitySmallModel', 'dardcor/gpt-4o-mini');
			await configService.setNonExtensionConfig('chat.byokUtilityModelDefault', 'mainAgent');
	...
	test('non-dardcor vendor override resolves to an extension-contributed endpoint', async () => {
		setFetcher([makeChatModel('dardcor-utility')]);
		const fakeModel = makeFakeLanguageModelChat({ vendor: 'anthropic', id: 'claude-haiku-4.5' });
		sandbox.stub(lm, 'selectChatModels').resolves([fakeModel]);
		await configService.setNonExtensionConfig('chat.utilityModel', 'anthropic/claude-haiku-4.5');
	...
	test('non-dardcor vendor override falls back when lm.selectChatModels returns no matches', async () => {
		setFetcher([makeChatModel('dardcor-utility')]);
	...
	test('non-dardcor vendor override falls back when lm.selectChatModels returns multiple matches (ambiguous)', async () => {
		setFetcher([makeChatModel('dardcor-utility')]);
	...
	test('non-dardcor vendor override falls back when lm.selectChatModels throws', async () => {
		setFetcher([makeChatModel('dardcor-utility')]);
	...
	test('arbitrary CAPI model family resolves to that model (execution/search subagent override)', async () => {
		...
		setFetcher([makeChatModel('dardcor-utility'), makeChatModel(subagentModel)]);
