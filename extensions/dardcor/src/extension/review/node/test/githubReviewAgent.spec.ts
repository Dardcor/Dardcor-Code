...
suite('githubReviewAgent', () => {
	...
	describe('parseLine', () => {
		...
		test('returns empty array when no dardcor_references', () => {
			const result = parseLine('data: {"choices":[]}');
			assert.deepStrictEqual(result, []);
		});

		test('returns empty array when dardcor_references is empty', () => {
			const result = parseLine('data: {"dardcor_references":[]}');
			assert.deepStrictEqual(result, []);
		});
