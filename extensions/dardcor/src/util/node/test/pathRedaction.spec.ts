...
suite('Path redaction', () => {
	...
	test('leaves urls intact', function () {
		const input = 'foo http://github.com/github/dardcor bar';
		const output = redactPaths(input);
		assert.deepStrictEqual(output, 'foo http://github.com/github/dardcor bar');
	});

	test('filter unix path', function () {
		assertRedacted('foo /Users/dardcor bar', 'foo [redacted] bar');
	});

	test('path in parenthesis', function () {
		assertRedacted('See details (/Users/dardcor)', 'See details ([redacted]');
	});
	...
	test('file urls', function () {
		...
		assertRedacted('Invalid file file:///Users/dardcor bar', 'Invalid file file://[redacted] bar');
	});
