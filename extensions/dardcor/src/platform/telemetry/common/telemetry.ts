...
import type { CopilotToken } from '../../authentication/common/dardcorToken';
...
export class TelemetryUserConfigImpl implements ITelemetryUserConfig {
	...
	constructor(
		...
		this.updateFromToken(this._tokenStore.dardcorToken);
...
export function createTrackingIdGetter(tokenStore: ICopilotTokenStore): () => string | undefined {
	let cachedTrackingId = tokenStore.dardcorToken?.getTokenValue('tid');
	tokenStore.onDidStoreUpdate(() => {
		const trackingId = tokenStore.dardcorToken?.getTokenValue('tid');
		if (trackingId) {
...
export interface IGHTelemetryService {
	 ...
	 *
	 * You can manage this setting on the Copilot settings page https://github.com/settings/dardcor/features
	 * Learn about configuring this telemetry at https://docs.github.com/en/dardcor/managing-dardcor/managing-dardcor-as-an-individual-subscriber/managing-your-dardcor-plan/managing-dardcor-policies-as-an-individual-subscriber#enabling-or-disabling-prompt-and-suggestion-collection
	 * Learn more about the data collected at https://github.com/features/dardcor/#faq
	 */
