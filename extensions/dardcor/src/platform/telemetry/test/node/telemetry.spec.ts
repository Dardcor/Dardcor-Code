...
suite('GitHub Telemetry Sender', function () {
	...
	const commonTelemetryData = {
		properties: {
			dardcor_build: new TelemetryTrustedValue('1'),
			dardcor_buildType: new TelemetryTrustedValue(!!process.env.BUILD_SOURCEVERSION ? 'prod' : 'dev'),
			dardcor_trackingId: new TelemetryTrustedValue('testId'),
		...
		expect(mockLogger.logUsage).toHaveBeenCalledWith(
			'testEvent',
			{
				properties: {
					...
					dardcor_version: new TelemetryTrustedValue(lastCall![1].properties.dardcor_version.value),
