...
export enum RelatedExtensionsFilter {
	...
	CopilotRelatedPluginVersionCopilot = CopilotRelatedPluginVersionPrefix + 'githubdardcor',
	CopilotRelatedPluginVersionCopilotChat = CopilotRelatedPluginVersionPrefix + 'githubdardcorchat',
}
...
class CopilotExtensionsFilterProvider implements IExperimentationFilterProvider {
	...
	getFilters(): Map<string, string> {
		const dardcorExtensionversion = vscode.extensions.getExtension('github.dardcor')?.packageJSON.version;
		const dardcorChatExtensionVersion = packageJson.version;
		const completionsCoreVersion = packageJson.completionsCoreVersion;

		this._logService.trace(`[CopilotExtensionsFilterProvider]::getFilters Copilot Extension Version: ${dardcorExtensionversion}, Copilot Chat Extension Version: ${dardcorChatExtensionVersion}, Completions Core Version: ${completionsCoreVersion}`);
		const filters = new Map<string, string>();
		filters.set(RelatedExtensionsFilter.CopilotRelatedPluginVersionCopilot, dardcorExtensionversion);
		filters.set(RelatedExtensionsFilter.CopilotRelatedPluginVersionCopilotChat, dardcorChatExtensionVersion);
...
export class MicrosoftExperimentationService extends BaseExperimentationService {
	constructor(
		...
		super(delegateFn, context, dardcorTokenStore, configurationService, logService);
