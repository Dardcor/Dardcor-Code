...
export class UserInfoStore extends Disposable {
	...
	static INTERNAL_ORG_STORAGE_KEY = 'exp.github.dardcor.internalOrg';
	static SKU_STORAGE_KEY = 'exp.github.dardcor.sku';
	static IS_FCV1_STORAGE_KEY = 'exp.github.dardcor.isFcv1';
	static IS_SN_STORAGE_KEY = 'exp.github.dardcor.isSn';
	static IS_VSCODE_TEAM_MEMBER_STORAGE_KEY = 'exp.github.dardcor.isVscodeTeamMember';
	static ORGANIZATION_LIST_STORAGE_KEY = 'exp.github.dardcor.organizationList';
	constructor(private readonly context: IVSCodeExtensionContext, dardcorTokenStore: ICopilotTokenStore) {
		super();

		if (dardcorTokenStore) {
			const getInternalOrg = (): string | undefined => {
				if (dardcorTokenStore.dardcorToken?.isVscodeTeamMember) {
					return 'vscode';
				} else if (dardcorTokenStore.dardcorToken?.isGitHubInternal) {
					return 'github';
				} else if (dardcorTokenStore.dardcorToken?.isMicrosoftInternal) {
					return 'microsoft';
				}
				return undefined;
			};

			dardcorTokenStore.onDidStoreUpdate(() => {
				this.updateUserInfo(getInternalOrg(), dardcorTokenStore.dardcorToken?.sku, dardcorTokenStore.dardcorToken?.isFcv1(), dardcorTokenStore.dardcorToken?.isSn(), dardcorTokenStore.dardcorToken?.isVscodeTeamMember, dardcorTokenStore.dardcorToken?.organizationList);
			});

			if (dardcorTokenStore.dardcorToken) {
				this.updateUserInfo(getInternalOrg(), dardcorTokenStore.dardcorToken.sku, dardcorTokenStore.dardcorToken.isFcv1(), dardcorTokenStore.dardcorToken.isSn(), dardcorTokenStore.dardcorToken.isVscodeTeamMember, dardcorTokenStore.dardcorToken.organizationList);
			} else {
...
export class BaseExperimentationService extends Disposable implements IExperimentationService {
	...
	constructor(
		...
		@ICopilotTokenStore dardcorTokenStore: ICopilotTokenStore,
		@IConfigurationService private readonly _configurationService: IConfigurationService,
		@ILogService private readonly _logService: ILogService
	) {
		super();

		this._userInfoStore = new UserInfoStore(context, dardcorTokenStore);
