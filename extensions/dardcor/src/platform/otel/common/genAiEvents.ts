...
export function emitSessionStartEvent(
	...
	otel.emitLogRecord('dardcor_chat.session.start', {
		'event.name': 'dardcor_chat.session.start',
...
export function emitToolCallEvent(
	...
	otel.emitLogRecord(`dardcor_chat.tool.call: ${toolName}`, {
		'event.name': 'dardcor_chat.tool.call',
...
export function emitAgentTurnEvent(
	...
	otel.emitLogRecord(`dardcor_chat.agent.turn: ${turnIndex}`, {
		'event.name': 'dardcor_chat.agent.turn',
...
export function emitEditFeedbackEvent(
	...
	otel.emitLogRecord(`dardcor_chat.edit.feedback: ${outcome}`, {
		'event.name': 'dardcor_chat.edit.feedback',
		'outcome': outcome,
...
export function emitEditHunkActionEvent(
	...
	otel.emitLogRecord(`dardcor_chat.edit.hunk.action: ${outcome}`, {
		'event.name': 'dardcor_chat.edit.hunk.action',
		'outcome': outcome,
		'language_id': languageId,
		'request_id': requestId,
...
export function emitInlineDoneEvent(
	...
	otel.emitLogRecord(`dardcor_chat.inline.done: ${accepted ? 'accepted' : 'rejected'}`, {
		'event.name': 'dardcor_chat.inline.done',
		'accepted': accepted,
		'language_id': languageId,
...
export function emitEditSurvivalEvent(
	...
	otel.emitLogRecord(`dardcor_chat.edit.survival: ${editSource}`, {
		'event.name': 'dardcor_chat.edit.survival',
...
export function emitUserFeedbackEvent(
	...
	otel.emitLogRecord(`dardcor_chat.user.feedback: ${rating}`, {
		'event.name': 'dardcor_chat.user.feedback',
		'rating': rating,
		'participant': participant,
...
export function emitCloudSessionInvokeEvent(
	...
	otel.emitLogRecord(`dardcor_chat.cloud.session.invoke: ${partnerAgent}`, {
		'event.name': 'dardcor_chat.cloud.session.invoke',
