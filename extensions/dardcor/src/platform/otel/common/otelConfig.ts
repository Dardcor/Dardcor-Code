...
export interface OTelConfigInput {
	...
	/** Service name from VS Code setting (`github.dardcor.chat.otel.serviceName`). */
	settingServiceName?: string;
	/** Enterprise-managed `service.name` (raw `telemetry.serviceName`). */
	policyServiceName?: string;
	/** Resource attributes from VS Code setting (`github.dardcor.chat.otel.resourceAttributes`). */
	...
	/** OTLP headers from VS Code setting (`github.dardcor.chat.otel.headers`). */
