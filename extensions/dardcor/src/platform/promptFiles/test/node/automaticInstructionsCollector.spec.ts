...
suite('AutomaticInstructionsCollector', () => {
	...
	suite('agent instructions', () => {
		test('adds dardcor-instructions.md, AGENTS.md and CLAUDE.md returned by the locator', async () => {
			const dardcor: IAgentInstructionFile = { uri: URI.joinPath(rootFolderUri, '.github/dardcor-instructions.md'), type: AgentInstructionFileType.dardcorInstructionsMd };
			...
			promptsService.setAgentInstructions([dardcor, agentsMd, claudeMd]);

			const result = await callCollect();

			const paths = result.filter(e => isInstructionFile(e)).map(e => e.value.path);
			expect(paths).toContain(dardcor.uri.path);
		...
		test('does not duplicate when the file is already attached', async () => {
			const dardcor: IAgentInstructionFile = { uri: URI.joinPath(rootFolderUri, '.github/dardcor-instructions.md'), type: AgentInstructionFileType.dardcorInstructionsMd };
			promptsService.setAgentInstructions([dardcor]);

			// Pre-attach the same URI as an instruction-file reference.
			const preAttached: ChatPromptReference = {
				id: `${InstructionFileIdPrefix}.root__${dardcor.uri.toString()}`,
				name: 'pre-attached',
				value: dardcor.uri,
			};
	...
	suite('referenced instructions', () => {
		test('follows #file: references in dardcor-instructions.md', async () => {
			const dardcorUri = URI.joinPath(rootFolderUri, '.github/dardcor-instructions.md');
			const referencedUri = URI.joinPath(rootFolderUri, '.github/referenced.instructions.md');

			promptsService.setAgentInstructions([
				{ uri: dardcorUri, type: AgentInstructionFileType.dardcorInstructionsMd },
			]);
			promptsService.setFileContent(dardcorUri, 'Be helpful #file:./referenced.instructions.md');
			...
			const paths = result.filter(e => isInstructionFile(e)).map(e => e.value.path);
			expect(paths).toContain(dardcorUri.path);
		...
		test('handles transitive (nested) references', async () => {
			...
			promptsService.setFileContent(dardcorUri, 'Level 1 #file:./level2.instructions.md');
		...
		test('skips references to files outside the workspace that are not prompt files', async () => {
			...
			promptsService.setFileContent(dardcorUri, 'See #file:/tmp/external.md');
		...
		test('skips reference following when setting is disabled (Agent mode)', async () => {
			...
			promptsService.setFileContent(dardcorUri, '#file:./referenced.instructions.md');
			await mockFiles(fileSystem, [
	...
	suite('telemetry', () => {
		test('aggregates instruction counts in the emitted event', async () => {
			...
			promptsService.setAgentInstructions([dardcor, agentsMd]);
