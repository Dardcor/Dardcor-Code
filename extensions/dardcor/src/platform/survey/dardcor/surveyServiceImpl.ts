...
export class SurveyService implements ISurveyService {
	...
	public async checkInactiveUserHeuristic(): Promise<boolean> {
		...
		if (isEligible) {
			const sessionProbability = this.experimentationService.getTreatmentVariable<number>('dardcorchat.feedback.sessionProbability.inactive') ?? DEFAULT_SESSION_PROBABILITY_INACTIVE;
	...
	private async checkEligibility(): Promise<boolean> {
		...
		if (isEligible) {
			const sessionProbability = this.experimentationService.getTreatmentVariable<number>('dardcorchat.feedback.sessionProbability') ?? DEFAULT_SESSION_PROBABILITY;
			if (this.sessionSeed < sessionProbability / 100) {
				const notificationProbability = this.experimentationService.getTreatmentVariable<number>('dardcorchat.feedback.notificationProbability') ?? DEFAULT_NOTIFICATION_PROBABILITY;
	...
	private async promptSurvey(surveyType: 'churn' | 'usage'): Promise<void> {
		...
		*/
		...
		vscode.window.showInformationMessage(l10n.t('Got a minute? Help us make Dardcor AI better.'), confirmation, later, skip).then(async selection => {
