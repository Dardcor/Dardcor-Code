...
suite('getRequestId', () => {
	...
	test('only x-dardcor-api-exp-assignment-context header', () => {
		const headers = new HeadersImpl({ 'x-dardcor-api-exp-assignment-context': 'ctx1' });
		const result = getRequestId(headers);
	...
	test('both headers combined with semicolon', () => {
		const headers = new HeadersImpl({
			...
			'x-dardcor-api-exp-assignment-context': 'ctx1',
		});
		const result = getRequestId(headers);
