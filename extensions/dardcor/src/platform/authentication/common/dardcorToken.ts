...
export class CopilotToken {
	 ...
	 * (cfb = dardcor for business, cfe = dardcor for enterprise).
	 * So it's also true for dardcor free users.
	 */
	...
	get isFreeUser(): boolean {
		return this.sku === 'free_limited_dardcor';
	}

	get isNoAuthUser(): boolean {
		return this.sku === 'no_auth_limited_dardcor';
	}

	get isManagedPlan(): boolean {
		const plan = this.dardcorPlan;
	...
	get dardcorPlan(): 'free' | 'individual' | 'individual_pro' | 'individual_max' | 'business' | 'enterprise' {
		if (this.isFreeUser) {
			return 'free';
		}
		const plan = this._info.dardcor_plan;
		switch (plan) {
			case 'individual':
			case 'individual_pro':
			case 'individual_max':
	...
	/**
	 * Returns the raw dardcor_plan string from the token payload without normalization.
	...
	get rawCopilotPlan(): string {
		...
		return this._info.dardcor_plan ?? 'individual';
	}
	...
	isCopilotIgnoreEnabled(): boolean {
		return this._info.dardcorignore_enabled ?? false;
	}
...
export type WellKnownSku =
	| 'free_limited_dardcor'
	| 'no_auth_limited_dardcor';
 ...
 * This is the success response (HTTP 200) from the /dardcor_internal/v2/token endpoint.
 */
export interface TokenEnvelope {
	...
	codesearch: boolean;
	/** Whether content exclusion (.dardcorignore) is enabled. */
	dardcorignore_enabled: boolean;
...
const tokenEnvelopeValidator = vObj({
	...
	codesearch: vBoolean(),
	dardcorignore_enabled: vBoolean(),
...
/**
 * Combined response type from the /dardcor_internal/v2/token endpoint.
...
/**
 * A server response containing the user info for the dardcor user from the /dardcor_internal/user endpoint
 */
...
} & Pick<CopilotUserInfo, 'dardcor_plan' | 'quota_snapshots' | 'quota_reset_date' | 'codex_agent_enabled' | 'organization_login_list' | 'token_based_billing'>;
...
export const enum TokenErrorNotificationId {
	NoCopilotAccess = 'no_dardcor_access',
