...
import { CopilotToken, ExtendedTokenInfo, TokenErrorNotificationId, TokenInfoOrError } from '../common/dardcorToken';
import { ErrorNoTelemetry } from '../../../util/dardcor/base/common/errors';
import { nowSeconds } from '../common/dardcorTokenManager';
import { BaseCopilotTokenManager } from '../node/dardcorTokenManager';
...
export class VSCodeCopilotTokenManager extends BaseCopilotTokenManager {
	...
	async getCopilotToken(force?: boolean): Promise<CopilotToken> {
		...
		if (!this.dardcorToken || this.dardcorToken.expires_at - (60 * 5 /* 5min */) < nowSeconds() || force) {
			try {
				this._logService.debug(`Getting CopilotToken (force: ${force})...`);
				this.dardcorToken = await this._authShowWarnings();
	...
	private async _authShowWarnings(): Promise<ExtendedTokenInfo> {
		...
		if (tokenResult.kind === 'failure') {
			throw Error('Failed to get dardcor token. reason: ' + tokenResult.reason);
		}
	...
	private sendTokenResultErrorTelemetry(tokenResult: TokenInfoOrError): void {
		...
		*/
		this._telemetryService.sendMSFTTelemetryEvent('dardcorTokenFetching.error', {
