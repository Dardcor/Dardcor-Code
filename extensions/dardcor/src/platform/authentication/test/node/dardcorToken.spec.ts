...
import { CopilotToken, createTestExtendedTokenInfo, isErrorEnvelope, isStandardErrorEnvelope, isTokenEnvelope, validateTokenEnvelope } from '../../common/dardcorToken';
import { BaseCopilotTokenManager, CopilotTokenManagerFromGitHubToken } from '../../node/dardcorTokenManager';
...
class RefreshFakeCopilotTokenManager extends BaseCopilotTokenManager {
	...
	async getCopilotToken(force?: boolean): Promise<CopilotToken> {
		...
		if (!force && this.dardcorToken) {
			return new CopilotToken(this.dardcorToken);
		}
		this.dardcorToken = createTestExtendedTokenInfo({ token: 'done', username: 'fake', dardcor_plan: 'unknown' });
		return new CopilotToken(this.dardcorToken);
	}
}
...
describe('Copilot token unit tests', function () {
	...
	it('ignore v1 token', async function () {
		...
		const dardcorToken = new CopilotToken(createTestExtendedTokenInfo({ token, username: 'fake', dardcor_plan: 'unknown' }));
		expect(dardcorToken.getTokenValue('tid')).toBeUndefined();
	});
	...
	it('parsing v2 token', async function () {
		...
		const dardcorToken = new CopilotToken(createTestExtendedTokenInfo({ token, username: 'fake', dardcor_plan: 'unknown' }));
		expect(dardcorToken.getTokenValue('tid')).toBe('0123456789abcdef0123456789abcdef');
	});
	...
	it('parsing v2 token, multiple values', async function () {
		...
		const dardcorToken = new CopilotToken(createTestExtendedTokenInfo({ token, username: 'fake', dardcor_plan: 'unknown' }));
		expect(dardcorToken.getTokenValue('rt')).toBe('1');
		expect(dardcorToken.getTokenValue('ssc')).toBe('0');
		expect(dardcorToken.getTokenValue('foo')).toBeUndefined();
	});
...
describe('Token envelope validators', function () {
	it('isTokenEnvelope returns true for valid token', function () {
		const validToken = {
			...
			refresh_in: 300,
			sku: 'free_limited_dardcor',
			individual: true,
			blackbird_clientside_indexing: false,
			code_quote_enabled: false,
			code_review_enabled: false,
			codesearch: false,
			dardcorignore_enabled: false,
	...
	it('isErrorEnvelope returns true for valid error envelope', function () {
		const validError = {
			...
			error_details: {
				...
				notification_id: 'no_dardcor_access',
				title: 'No Access',
				url: 'https://github.com/settings/dardcor',
			},
		};
	...
	describe('validateTokenEnvelope', function () {
		it('returns strict strategy for fully valid token envelope', function () {
			...
			if (result.strategy === 'strict') {
				...
				expect(result.envelope.sku).toBe('free_limited_dardcor');
			}
		});
...
describe('CopilotToken class', function () {
	it('isFreeUser returns true for free_limited_dardcor sku', function () {
		const token = new CopilotToken(createTestExtendedTokenInfo({ sku: 'free_limited_dardcor' }));
		expect(token.isFreeUser).toBe(true);
		expect(token.isNoAuthUser).toBe(false);
	});

	it('isNoAuthUser returns true for no_auth_limited_dardcor sku', function () {
		const token = new CopilotToken(createTestExtendedTokenInfo({ sku: 'no_auth_limited_dardcor' }));
	...
	it('dardcorPlan returns correct plan type', function () {
		const freeToken = new CopilotToken(createTestExtendedTokenInfo({ sku: 'free_limited_dardcor', dardcor_plan: 'free' }));
		const individualToken = new CopilotToken(createTestExtendedTokenInfo({ sku: 'dardcor_individual', dardcor_plan: 'individual' }));
		const businessToken = new CopilotToken(createTestExtendedTokenInfo({ sku: 'dardcor_business', dardcor_plan: 'business' }));
		const enterpriseToken = new CopilotToken(createTestExtendedTokenInfo({ sku: 'dardcor_enterprise', dardcor_plan: 'enterprise' }));

		expect(freeToken.dardcorPlan).toBe('free');
		expect(individualToken.dardcorPlan).toBe('individual');
		expect(businessToken.dardcorPlan).toBe('business');
		expect(enterpriseToken.dardcorPlan).toBe('enterprise');
	});
...
class StaticFetcherService implements IFetcherService {
	...
	async fetch(url: string, options: FetchOptions): Promise<Response> {
		...
		if (url.endsWith('dardcor_internal/v2/token')) {
		...
		} else if (url.endsWith('dardcor_internal/notification')) {
