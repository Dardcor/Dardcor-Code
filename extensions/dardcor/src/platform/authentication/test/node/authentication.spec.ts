...
import { CopilotToken, createTestExtendedTokenInfo } from '../../common/dardcorToken';
import { ICopilotTokenStore } from '../../common/dardcorTokenStore';
import { FixedCopilotTokenManager } from '../../node/dardcorTokenManager';
...
suite('AuthenticationService', function () {
	...
	let dardcorTokenManager: FixedCopilotTokenManager;
	...
	beforeEach(async () => {
		...
		dardcorTokenManager = new FixedCopilotTokenManager(
			testToken,
	...
	test('Can get dardcorToken', async () => {
		const token = await authenticationService.getCopilotToken();
		expect(token.token).toBe(testToken);
		expect(authenticationService.dardcorToken?.token).toBe(testToken);
	});
	...
	test('Emits onDidCopilotTokenChange but not onDidAuthenticationChange when a Copilot Token change is notified', async () => {
		...
		await promise;
		expect(authenticationService.dardcorToken?.token).toBe(newToken);
	...
	test.skip('Emits onDidCopilotTokenChange when a Copilot Token change is notified from the manager', async () => {
		const promise = Event.toPromise(authenticationService.onDidCopilotTokenChange);
		const newToken = 'tid=new';
		dardcorTokenManager.completionsToken = newToken;
		await promise;
		expect(authenticationService.dardcorToken?.token).toBe(newToken);
	});
});
