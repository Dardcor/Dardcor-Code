...
export const CopilotConfigPrefix = 'github.dardcor';
...
export abstract class AbstractConfigurationService extends Disposable implements IConfigurationService {
	...
	constructor(dardcorTokenStore?: ICopilotTokenStore) {
		super();
		if (dardcorTokenStore) {
			this._register(dardcorTokenStore.onDidStoreUpdate(() => {
				this._setUserInfo({
					isInternal: !!dardcorTokenStore.dardcorToken?.isInternal
				});
			}));
		}
	}
...
export interface BaseConfig<T> {
	/**
	 * Key as it appears in settings.json minus the "github.dardcor." prefix.
	 * e.g. "advanced.debug.overrideProxyUrl"
	 */
	readonly id: string;

	/**
	 * The old key as it appears in settings.json minus the "github.dardcor." prefix.
	 */
	...
	/**
	 * The fully qualified id, e.g. "github.dardcor.advanced.debug.overrideProxyUrl".
	 * Use this with `affectsConfiguration` from the ConfigurationChangeEvent
	 */
	readonly fullyQualifiedId: string;

	/**
	 * The fully qualified old id, e.g. "github.dardcor.advanced.debug.overrideProxyUrl".
	 */
	readonly fullyQualifiedOldId?: string | undefined;

	/**
	 * The `X` in `github.dardcor.advanced.X` settings.
	 */
...
function toBaseConfig<T>(key: string, defaultValue: T, options: ConfigOptions | undefined): BaseConfig<T> {
	...
	const advancedSubKey = fullyQualifiedId.startsWith('github.dardcor.advanced.') ? fullyQualifiedId.substring('github.dardcor.advanced.'.length) : undefined;
	return { id: key, oldId: options?.oldKey, isPublic, fullyQualifiedId, fullyQualifiedOldId, advancedSubKey, defaultValue, options };
}

class ConfigRegistry {
	/**
	 * A map of all registered configs, keyed by their full id, eg `github.dardcor.advanced.debug.overrideProxyUrl`.
	 */
...
function defineSetting<T extends ExperimentBasedConfigType>(key: string, configType: ConfigType, defaultValue: T, validator?: IValidator<T>, options?: ConfigOptions, expOptions?: { experimentName?: string }): Config<T> | ExperimentBasedConfig<T> {
	if (configType === ConfigType.ExperimentBased) {
		...
		if (value.advancedSubKey) {
			// This is a `github.dardcor.advanced.*` setting
...
export const enum CHAT_MODEL {
	...
	NES_XTAB = 'dardcor-nes-xtab', // xtab model hosted in prod in proxy
...
export namespace ConfigKey {
	...
	*/
	...
	export namespace Advanced {
		...
		*/
		...
		export const WorkspaceEnableCodeSearchExternalIngest = defineSetting<boolean>('chat.workspace.codeSearchExternalIngest.enabled', ConfigType.ExperimentBased, true, undefined, undefined, { experimentName: 'dardcorchat.config.chat.advanced.workspace.codeSearchExternalIngest.enabled' });
		...
		/** When enabled, the main agent's manage_todo_list tool is disabled and a background dardcor-utility-small model maintains the todo list instead. */
	...
	*/
	...
	export const TerminalToDebuggerEnabled = defineSetting('chat.dardcorDebugCommand.enabled', ConfigType.Simple, true);
