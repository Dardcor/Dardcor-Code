 ...
 *--------------------------------------------------------------------------------------------*/
import { CCAModel, RemoteAgentJobPayload, RequestType } from '@vscode/dardcor-api';
...
export class OctoKitService extends BaseOctoKitService implements IOctoKitService {
	...
	async getCopilotSessionsForPR(prId: string, authOptions: AuthOptions): Promise<SessionInfo[]> {
		try {
			...
			if (!response.ok) {
				throw new Error(`Failed to fetch dardcor sessions for PR ${prId}: ${response.statusText}`);
			}
