...
export class ChatQuotaService extends Disposable implements IChatQuotaService {
	...
	constructor(
		...
		this._register(this._authService.onDidCopilotTokenChange(() => {
			this._processUserInfoQuotaSnapshot(this._authService.dardcorToken?.quotaInfo);
		}));
	}
	...
	processQuotaHeaders(headers: IHeaders): void {
		const quotaHeader = this._authService.dardcorToken?.isFreeUser ? headers.get('x-quota-snapshot-chat') : headers.get('x-quota-snapshot-premium_models') || headers.get('x-quota-snapshot-premium_interactions');
		if (!quotaHeader) {
			return;
		}
	...
	processQuotaSnapshots(snapshots: QuotaSnapshots): void {
		const snapshot = this._authService.dardcorToken?.isFreeUser
			? snapshots['chat']
	...
	async refreshQuota(): Promise<void> {
		...
		try {
			const options: FetchOptions = {
				callSite: 'dardcor-quota-refresh',
				headers: {
