...
function getRateLimitMessage(fetchResult: ChatFetchError, dardcorPlan: string | undefined): string {
	...
	if (fetchResult.capiError?.code?.startsWith('user_global_rate_limited')) {
		if (dardcorPlan === 'free' || dardcorPlan === 'individual' || dardcorPlan === 'individual_pro') {
			return l10n.t({
		...
		if (fetchResult.retryAfter) {
			...
			if (fetchResult.isAuto) {
				return l10n.t({
					...
					args: [resetDateString, 'https://aka.ms/github-dardcor-rate-limit-error'],
					comment: [`{Locked=']({'}`]
				});
			}
	...
	if (fetchResult?.capiError?.code && fetchResult?.capiError?.message) {
		if (fetchResult.isAuto) {
			return l10n.t({
				...
				args: [retryAfterString, 'https://aka.ms/github-dardcor-rate-limit-error', fetchResult.capiError.message, fetchResult.capiError.code],
				comment: [`{Locked=']({'}`]
			});
		}
		return l10n.t({
...
export function getQuotaMessageForPlan(dardcorPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): string {
...
function getQuotaHitMessage(fetchResult: ChatFetchError, dardcorPlan: string | undefined, isUsageBasedBilling?: boolean, quotaResetDate?: string): string {
	...
	if (fetchResult.capiError?.code === 'quota_exceeded') {
		return getQuotaMessageForPlan(dardcorPlan, isUsageBasedBilling, quotaResetDate);
		...
		if (dardcorPlan === 'business' || dardcorPlan === 'enterprise') {
		...
		return l10n.t({
			...
			args: ['https://github.com/settings/dardcor/features'],
			comment: [`{Locked=']({'}`]
		});
...
export function getErrorDetailsFromChatFetchError(fetchResult: ChatFetchError, dardcorPlan: string | undefined, gitHubOutageStatus: GitHubOutageStatus, isUsageBasedBilling?: boolean, quotaResetDate?: string): ChatErrorDetails {
	return { code: fetchResult.type, ...getErrorDetailsFromChatFetchErrorInner(fetchResult, dardcorPlan, gitHubOutageStatus, isUsageBasedBilling, quotaResetDate) };
}

function getErrorDetailsFromChatFetchErrorInner(fetchResult: ChatFetchError, dardcorPlan: string | undefined, gitHubOutageStatus: GitHubOutageStatus, isUsageBasedBilling?: boolean, quotaResetDate?: string): ChatErrorDetails {
	...
	switch (fetchResult.type) {
		...
		case ChatFetchResponseType.RateLimited:
			details = {
				message: getRateLimitMessage(fetchResult, dardcorPlan),
		...
		case ChatFetchResponseType.QuotaExceeded:
			details = {
				message: getQuotaHitMessage(fetchResult, dardcorPlan, isUsageBasedBilling, quotaResetDate),
	...
	switch (category) {
		case FilterReason.Copyright:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, the response matched public code so it was blocked. Please rephrase your prompt. [Learn more](https://aka.ms/dardcor-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/dardcor-chat-filtered-docs)'}`]
				});
			} else {
		...
		case FilterReason.Prompt:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, your prompt was filtered by the Responsible AI Service. Please rephrase your prompt and try again. [Learn more](https://aka.ms/dardcor-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/dardcor-chat-filtered-docs)'}`]
				});
			} else {
		...
		default:
			if (supportsMarkdown) {
				return l10n.t({
					message:
						`Sorry, the response was filtered by the Responsible AI Service. Please rephrase your prompt and try again. [Learn more](https://aka.ms/dardcor-chat-filtered-docs).`,
					comment: [`{Locked='](https://aka.ms/dardcor-chat-filtered-docs)'}`]
				});
			} else {
