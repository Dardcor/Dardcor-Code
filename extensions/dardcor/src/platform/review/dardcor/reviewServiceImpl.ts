...
const reviewDiffContextKey = 'github.dardcor.chat.reviewDiff.enabled';
const reviewDiffReposContextKey = 'github.dardcor.chat.reviewDiff.enabledRootUris';
const numberOfReviewCommentsKey = 'github.dardcor.chat.review.numberOfComments';
...
export class ReviewServiceImpl implements IReviewService {
	...
	private _commentController = this._disposables.add(vscode.comments.createCommentController('github-dardcor-review', 'Code Review'));
	...
	constructor(
		...
		this._disposables.add(vscode.workspace.onDidChangeConfiguration(e => {
			...
			if (e.affectsConfiguration('github.dardcor.advanced') || e.affectsConfiguration('github.dardcor.advanced.review.intent')) {
	...
	isReviewDiffEnabled() {
		return this._configurationService.getConfig(ConfigKey.ReviewAgent) && this._authenticationService.dardcorToken?.isCopilotCodeReviewEnabled || false;
	}
	...
	getDiagnosticCollection(): ReviewDiagnosticCollection {
		return this._diagnosticCollection || this._disposables.add(this._diagnosticCollection = vscode.languages.createDiagnosticCollection('github.dardcor.chat.review'));
	}
	...
	addReviewComments(comments: ReviewComment[]) {
		for (const comment of comments) {
			...
			if (this._comments.length === 1) {
				vscode.commands.executeCommand('github.dardcor.chat.review.next');
				this._monitorActiveThread = setInterval(() => {
					...
					if (active !== this._activeThread) {
						...
						if (active) {
							vscode.commands.executeCommand('github.dardcor.chat.review.current', active);
						}
					}
				}, 500);
			}
		...
		const comments = [
			{
				...
				author: {
					...
					iconPath: URI.joinPath(this._contextService.extensionUri, 'assets', 'dardcor.png'),
				},
			}
		];
		return comments;
