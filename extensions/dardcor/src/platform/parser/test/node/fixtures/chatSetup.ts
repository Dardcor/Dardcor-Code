...
class SetupChatAgent extends Disposable implements IChatAgentImplementation {
	...
	private async forwardRequestToCopilot(requestModel: IChatRequestModel, progress: (part: IChatProgress) => void, chatService: IChatService, languageModelsService: ILanguageModelsService, chatAgentService: IChatAgentService, chatWidgetService: IChatWidgetService, languageModelToolsService: ILanguageModelToolsService): Promise<void> {
		try {
			...
			progress({
				kind: 'warning',
				content: new MarkdownString(localize('dardcorUnavailableWarning', "Copilot failed to get a response. Please try again."))
			});
		}
	}
				...
				if (ready === 'error' || ready === 'timedout') {
					progress({
						...
						content: new MarkdownString(ready === 'timedout' ?
							localize('dardcorTookLongWarning', "Copilot took too long to get ready. Please review the guidance in the Chat view.") :
							localize('dardcorFailedWarning', "Copilot failed to get ready. Please review the guidance in the Chat view.")
						)
					});
		...
		if (typeof result?.success === 'boolean') {
			if (result.success) {
				if (result.dialogSkipped) {
					progress({
						kind: 'progressMessage',
						content: new MarkdownString(localize('dardcorSetupSuccess', "Copilot setup finished successfully."))
					});
				...
				progress({
					kind: 'warning',
					content: new MarkdownString(localize('dardcorSetupError', "Copilot setup failed."))
				});
			}
		}
	...
	private replaceToolInRequestModel(requestModel: IChatRequestModel): IChatRequestModel {
		...
		const toolId = toolPart.toolId.replace(/setup.tools\./, `dardcor_`.toLowerCase());
...
class ChatSetup {
	...
	private getDialogTitle(): string {
		...
		if (this.context.state.entitlement === ChatEntitlement.Pro) {
			return localize('dardcorProTitle', "Start using Copilot Pro");
		}

		return this.context.state.registered ? localize('dardcorTitle', "Start using Copilot") : localize('dardcorFreeTitle', "Start using Copilot for free");
	}
	...
	private async doSetup(options: { forceSignIn?: boolean }, watch: StopWatch): Promise<boolean> {
		...
		try {
			...
			const trusted = await this.workspaceTrustRequestService.requestWorkspaceTrust({
				message: localize('dardcorWorkspaceTrust', "Copilot is currently only supported in trusted workspaces.")
			});
			if (!trusted) {
