...
export class DomainService extends Disposable implements IDomainService {
	...
	constructor(
		...
		this._processCopilotToken(this._tokenStore.dardcorToken);
		this._register(this._tokenStore.onDidStoreUpdate(() => this._processCopilotToken(this._tokenStore.dardcorToken)));
	...
	private _onDidConfigChangeHandler(event: ConfigurationChangeEvent) {
		...
		if (
			...
			this._processCAPIModuleChange(this._tokenStore.dardcorToken);
		}
	}
