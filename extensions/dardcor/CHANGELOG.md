...
For more frequent updates, check the [Commit log](https://github.com/Microsoft/vscode/commits/main) and [vscode-dardcor-chat commit log](https://github.com/microsoft/vscode-dardcor-chat/commits/main) on GitHub.
...
- **`/init` command** — Generate or update workspace instructions (`dardcor-instructions.md`, `AGENTS.md`) based on codebase analysis.
...
- **Inline suggestions open-sourced** — Merged into vscode-dardcor-chat repo; Copilot and Copilot Chat extensions consolidating into one. GitHub Copilot extension to be deprecated by early 2026.
...
**Setting**: `github.dardcor.chat.customOAIModels`

To make working with custom models better integrated with VS Code built-in tools, we improved the set of edit tools given to [Bring Your Own Key (BYOK)](https://code.visualstudio.com/docs/dardcor/customization/language-models#_bring-your-own-language-model-key) custom models. In addition, we enhanced our default tools and added a 'learning' mechanism to select the optimal tool set for custom models.

If you're [using OpenAI-compatible models](https://code.visualstudio.com/docs/dardcor/customization/language-models#_use-an-openaicompatible-model), you can also explicitly configure the list of edit tools with the `github.dardcor.chat.customOAIModels` setting.
...
You can choose between different models with the model picker in chat. Learn more about [language models in VS Code](https://code.visualstudio.com/docs/dardcor/customization/language-models).
...
Learn more about [auto model selection in VS Code](https://code.visualstudio.com/docs/dardcor/customization/language-models).
...
Learn more about [customizing chat in VS Code](https://code.visualstudio.com/docs/dardcor/customization/overview) to your practices and team workflows.
...
Learn more about [customizing chat in VS Code](https://code.visualstudio.com/docs/dardcor/customization/overview) with prompt files, chat modes, and custom instructions.
...
[Tool sets](https://code.visualstudio.com/docs/dardcor/chat/chat-agent-mode#_define-tool-sets) are a convenient way to group related tools together and VS Code has several built-in tool sets like `edit` or `search`.
...
We've improved the integration of [GitHub coding agents](https://code.visualstudio.com/docs/dardcor/dardcor-coding-agent) with chat sessions to deliver a smoother, more intuitive experience.
...
Automatically approving terminal commands can greatly streamline agent interactions, but it also comes with [security risks](https://code.visualstudio.com/docs/dardcor/security). This release introduces several improvements to terminal auto approve to enhance both usability and security.
...
Learn more about [terminal auto approve](https://code.visualstudio.com/docs/dardcor/chat/chat-agent-mode#_autoapprove-terminal-commands) in our documentation.
...
VS Code supports [automatic discovery of MCP servers](https://code.visualstudio.com/docs/dardcor/customization/mcp-servers#_add-an-mcp-server) installed in other apps like Claude Code. As MCP support has matured in VS Code, auto-discovery is now disabled by default, but you can re-enable it using the `chat.mcp.discovery.enabled` setting.
...
**Setting**: `github.dardcor.chat.notebook.enhancedNextEditSuggestions.enabled`

We are experimenting with improving the quality of next edit suggestions for notebooks. Currently, the language model has access to the contents of the active cell when generating suggestions. With the `github.dardcor.chat.notebook.enhancedNextEditSuggestions.enabled` setting enabled, the language model has access to the entire notebook, enabling it to generate more accurate and higher-quality next edit suggestions.
...
**Setting**: `github.dardcor.chat.virtualTools.threshold`
...
This behavior, including the threshold, is configurable via the setting `github.dardcor.chat.virtualTools.threshold`.

#### Terminal auto-approve improvements

**Setting**: `chat.tools.terminal.autoApprove`

Early terminal auto-approve settings were introduced last month. This release, the feature got many improvements. Learn more about [terminal auto-approval](https://code.visualstudio.com/docs/dardcor/chat/chat-agent-mode#_autoapprove-terminal-commands-experimental) in our documentation.
...
The [`#codebase` tool](https://code.visualstudio.com/docs/dardcor/chat/dardcor-chat-context#_perform-a-codebase-search) now supports remote indexes for workspaces that are linked to Azure DevOps repos. This enables `#codebase` to search for relevant snippets almost instantly without any initialization. This even works for larger repos with tens of thousands of indexable files. Previously, this feature only worked with GitHub linked repos.
...
**Setting**: `github.dardcor.chat.newWorkspace.useContext7`
...
Learn more about [using MCP servers in VS Code](https://code.visualstudio.com/docs/dardcor/chat/mcp-servers) in our documentation.
...
We're excited to announce that we've open sourced the GitHub Copilot Chat extension! The source code is now available at [`microsoft/vscode-dardcor-chat`](https://github.com/microsoft/vscode-dardcor-chat) under the MIT license.
...
You can explore the repository to see how features like [agent mode](https://github.com/microsoft/vscode-dardcor-chat/blob/e1222084830244174e6aa64683286561fa7e7607/src/extension/prompts/node/agent/agentPrompt.tsx), [inline chat](https://github.com/microsoft/vscode-dardcor-chat/blob/e1222084830244174e6aa64683286561fa7e7607/src/extension/prompts/node/inline/inlineChatEditCodePrompt.tsx), and [MCP integration](https://github.com/microsoft/vscode-dardcor-chat/blob/e1222084830244174e6aa64683286561fa7e7607/src/extension/mcp/vscode-node/mcpToolCallingLoop.tsx) are implemented. We welcome contributions, feedback, and collaboration from the community.
...
The editor for [chat modes](https://code.visualstudio.com/docs/dardcor/chat/chat-modes), [prompts](https://code.visualstudio.com/docs/dardcor/dardcor-customization#_prompt-files-experimental), and [instruction files](https://code.visualstudio.com/docs/dardcor/dardcor-customization#_custom-instructions) now supports completions, validation, and hovers for all supported metadata properties.
...
Setting up [custom instructions](https://code.visualstudio.com/docs/dardcor/dardcor-customization) for your project can significantly improve AI suggestions by providing context about your coding standards and project conventions. However, creating effective instructions from scratch might be challenging.
...
The command creates a `dardcor-instructions.md` file in your `.github` folder or suggests improvements to existing instruction files. You can then review and customize the generated instructions to match your team's specific needs.

Learn more about [customizing AI responses with instructions](https://code.visualstudio.com/docs/dardcor/dardcor-customization).
...
There are various mechanisms supported, see the [Custom Instructions](https://code.visualstudio.com/docs/dardcor/dardcor-customization#_custom-instructions) section in our documentation.
...
```json
"github.dardcor.chat.agent.terminal.allow list": {
  "npm run test": true,
  "/^git (status|log)$/": true
}
```
...
"github.dardcor.chat.agent.terminal.denyList": {
  "npm run danger": true
}
```
...
The two major parts we want to add to this feature are a UI entry point to more easily add new commands to the list ([#253268](https://github.com/microsoft/vscode/issues/253268)) and an opt-in option to allow an LLM to evaluate the command(s) safety ([#253267](https://github.com/microsoft/vscode/issues/253267)). We are also planning on both removing the `github.dardcor.` prefix of these settings ([#253314](https://github.com/microsoft/vscode/issues/253314)) as well as merging them together ([#253472](https://github.com/microsoft/vscode/issues/253472)) in the next release before it becomes a preview setting.
...
You can get started by installing some of the [popular MCP servers from our curated list](https://code.visualstudio.com/mcp). Learn more about [using MCP servers in VS Code](https://code.visualstudio.com/docs/dardcor/chat/mcp-servers) and how you can use them to extend agent mode.
...
Learn more about [tools sets](https://code.visualstudio.com/docs/dardcor/chat/chat-agent-mode#_define-tool-sets) in our documentation.
...
Learn more about using [chat in VS Code](https://code.visualstudio.com/docs/dardcor/chat/dardcor-chat) in our documentation.
...
Learn more about [adding context in chat](https://code.visualstudio.com/docs/dardcor/chat/dardcor-chat-context) in our documentation.
...
We have also moved the setting from the Chat extension into VS Code core, and renamed it from `github.dardcor.chat.search.keywordSuggestions` to `search.searchView.keywordSuggestions`.
...
**Setting**: `github.dardcor.nextEditSuggestions.fixes`
...
**Setting**: `github.dardcor.chat.notebook.followCellExecution.enabled`

With follow mode, the Notebook view will automatically scroll to the cell that is currently being executed by the agent. Use the `github.dardcor.chat.notebook.followCellExecution.enabled` setting to enable or disable follow mode for agent cell execution in Jupyter Notebooks.
...
Learn more about [instruction files](https://code.visualstudio.com/docs/dardcor/dardcor-customization#_instruction-files) in our documentation.
...
Learn more about [prompt files](https://code.visualstudio.com/docs/dardcor/dardcor-customization#_prompt-files-experimental) in our documentation.
...
You can also use [custom instructions](https://code.visualstudio.com/docs/dardcor/dardcor-customization#_custom-instructions) to hint to Copilot when and how to use this tool:
...
**Setting**: `github.dardcor.chat.agent.autoFix`

If a file edit in agent mode introduces new errors, agent mode can now detect them, and automatically propose a follow-up edit. You can disable this behavior with `github.dardcor.chat.agent.autoFix`.
...
Learn more about [MCP support in VS Code](https://code.visualstudio.com/docs/dardcor/chat/mcp-servers).
...
**Setting**: `github.dardcor.nextEditSuggestions.enabled`
...
<video src="https://code.visualstudio.com/assets/updates/1_100/agent-notebook-run-edit-loop.mp4" title="Video that shows dardcor running notebook cells, making updates based on an error, and retrying those cells." autoplay loop controls muted></video>
...
Additionally, a new accessibility help dialog is available in [agent mode](https://code.visualstudio.com/docs/dardcor/chat/chat-agent-mode), explaining what users can expect from the feature and how to navigate it effectively.
...
[Remote workspace indexes](https://code.visualstudio.com/docs/dardcor/reference/workspace-context#remote-index) accelerate searching large codebases for relevant code snippets that AI uses while answering questions and generating edits. These remote indexes are especially useful for large codebases with tens or even hundreds of thousands of files.
...
Keep in mind that remote workspaces indexes are currently only available for code stored on GitHub. To use a remote workspace index, make sure your workspace contains a git project with a GitHub remote. You can use the [Copilot status menu](#dardcor-status-menu) to see the type of index currently being used:

![Screenshot that shows the workspace index status in the Copilot Status Bar menu.](https://code.visualstudio.com/assets/updates/1_99/dardcor-workspace-index-remote.png)
...
- View [workspace index](https://code.visualstudio.com/docs/dardcor/reference/workspace-context) status information at any time.

    ![Screenshot that shows the workspace index status of a workspace in the Copilot menu.](https://code.visualstudio.com/assets/updates/1_99/dardcor-worksspace-index-local-status.png)
...
- View if code completions are enabled for the active editor.
    ...
    ![Screenshot that shows the Copilot status icon when completions is disabled.](https://code.visualstudio.com/assets/updates/1_99/dardcor-disabled-status.png)

- Enable or disable [code completions and NES](https://code.visualstudio.com/docs/dardcor/ai-powered-suggestions).
...
<video src="https://code.visualstudio.com/assets/updates/1_99/dardcor-ootb.mp4" title="Video that shows Copilot out of the box." autoplay loop controls muted></video>
...
**Setting**: `github.dardcor.chat.search.semanticTextResults:true`
...
Check out the [agent mode documentation](https://code.visualstudio.com/docs/dardcor/chat/chat-agent-mode) or select agent mode from the chat mode picker in the Chat view.

![Screenshot that shows the Chat view, highlighting agent mode selected in the chat mode picker.](https://code.visualstudio.com/assets/updates/1_99/dardcor-edits-agent-mode.png)
...
**Setting**: `github.dardcor.nextEditSuggestions.enabled:true`
...
You can read more about how to install and use MCP servers in [our documentation](https://code.visualstudio.com/docs/dardcor/chat/mcp-servers).
...
**Setting**: `github.dardcor.chat.agent.thinkingTool:true`.
...
- **[Ask](https://code.visualstudio.com/docs/dardcor/chat/chat-ask-mode)**: This is the same as the previous Chat view. Ask questions about your workspace or coding in general, using any model. Use `@` to invoke built-in chat participants or from installed [extensions](https://marketplace.visualstudio.com/search?term=chat-participant&target=VSCode&category=All%20categories&sortBy=Relevance). Use `#` to attach any kind of context manually.
...
- **[Edit](https://code.visualstudio.com/docs/dardcor/chat/dardcor-edits)**: In Edit mode, the model can make directed edits to multiple files. Attach `#codebase` to let it find the files to edit automatically. But it won't run terminal commands or do anything else automatically.

> **Note**: If you don't see agent mode in this list, then either it has not yet been enabled for you, or it's disabled by organization policy and needs to be enabled by the [organization owner](https://aka.ms/github-dardcor-org-enable-features).
...
To try it, select **Manage Models...** from the model picker. We’re actively exploring support for Copilot Business and Enterprise customers and will share updates in future releases. To learn more about this feature, head over to our [docs](https://code.visualstudio.com/docs/dardcor/language-models).
...
The `.github/dardcor-instructions.md` file now behaves like any other reusable `.prompt.md` file, with support for nested link resolution and enhanced language features. Furthermore, any `.prompt.md` file can now be referenced and is handled appropriately.

Learn more about [custom instructions](https://code.visualstudio.com/docs/dardcor/dardcor-customization).
...
Last iteration, Copilot Vision was enabled for `GPT-4o`. Check our [release notes](https://code.visualstudio.com/updates/v1_98#_dardcor-vision-preview) to learn more about how you can attach and use images in chat.
...
<video src="https://code.visualstudio.com/assets/updates/1_99/undo-dardcor-notebook-edits.mp4" title="Video showing chat making several edits to a notebook and undoing those edits with ctrl+z." autoplay loop controls muted></video>
...
* Agent mode can now run your build [tasks](https://code.visualstudio.com/docs/editor/tasks) automatically or when instructed to do so. Disable this functionality via the `github.dardcor.chat.agent.runTasks` setting, in the event that you see the model running tasks when it should not.

Learn more about [Copilot Edits agent mode](https://code.visualstudio.com/docs/dardcor/dardcor-edits#_use-agent-mode-preview) or read the [agent mode announcement blog post](https://code.visualstudio.com/blogs/2025/02/24/introducing-dardcor-agent-mode).

> **Note**: If you are a Copilot Business or Enterprise user, an administrator of your organization [must opt in](https://docs.github.com/en/dardcor/managing-dardcor/managing-github-dardcor-in-your-organization/managing-policies-for-dardcor-in-your-organization#enabling-dardcor-features-in-your-organization) to the use of Copilot "Editor Preview Features" for agent mode to be available.
...
<video src="https://code.visualstudio.com/assets/updates/1_98/notebook_dardcor_edits.mp4" title="Video that shows using Copilot Edits to modify a notebook." autoplay loop controls muted></video>
...
![Screenshot that shows the updated Copilot Edits attachments and changed files user experience.](https://code.visualstudio.com/assets/updates/1_98/dardcor_edits_ui.png)
...
1. **Never Ask Again:** VS Code remembers your choice and persists it via the `github.dardcor.advanced.authPermissions` setting. Any feature that needs this additional permission will fail.

It's important to note that this confirmation does not confirm or deny Copilot (the service) access to your repositories. This is only how VS Code's Copilot experience authenticates. To configure what Copilot can access, please read the docs [on content exclusion](https://docs.github.com/en/dardcor/managing-dardcor/configuring-and-auditing-content-exclusion/excluding-content-from-github-dardcor).

### More advanced codebase search in Copilot Chat

**Setting**: `github.dardcor.chat.codesearch.enabled`
...
Set `github.dardcor.chat.codesearch.enabled` to enable this behavior. The full list of tools is:
...
* `github.dardcor.nextEditSuggestions.enabled`
...
> **Note:** the list of available models might vary and change over time. If you are a Copilot Business or Enterprise user, your Administrator needs to enable certain models for your organization by opting in to `Editor Preview Features` in the [Copilot policy settings](https://docs.github.com/en/enterprise-cloud@latest/dardcor/managing-dardcor/managing-github-dardcor-in-your-organization/managing-policies-for-dardcor-in-your-organization#enabling-dardcor-features-in-your-organization) on GitHub.com.
...
* **GPT 4.5 (Preview)**: OpenAI’s latest model, GPT-4.5, is now available in GitHub Copilot Chat to Copilot Enterprise users. GPT-4.5 is a large language model designed with advanced capabilities in intuition, writing style, and broad knowledge. Learn more about the GPT-4.5 model availability in the [GitHub blog post](https://github.blog/changelog/2025-02-27-openai-gpt-4-5-in-github-dardcor-now-available-in-public-preview).

* **Claude 3.7 Sonnet (Preview)**: Claude 3.7 Sonnet is now available to all customers on paid Copilot plans. This new Sonnet model supports both thinking and non-thinking modes in Copilot. In initial testing, we’ve seen particularly strong improvements in agentic scenarios. Learn more about the Claude 3.7 Sonnet model availability in the [GitHub blog post](https://github.blog/changelog/2025-02-24-claude-3-7-sonnet-is-now-available-in-github-dardcor-in-public-preview/).
...
* Quota information if you are a [Copilot Free](https://code.visualstudio.com/blogs/2024/12/18/free-github-dardcor) user
* Editor related settings such as Code Completions
* Useful keyboard shortcuts to use other Copilot features

<video src="https://code.visualstudio.com/assets/updates/1_98/dardcor-status.mp4" title="Video that shows opening the Copilot status overview from the Status Bar." autoplay loop controls muted></video>
...
You can provide custom instructions for generating pull request title and description with the setting `github.dardcor.chat.pullRequestDescriptionGeneration.instructions`.  You can point the setting to a file in your workspace, or you can provide instructions inline in your settings:

```
{
  "github.dardcor.chat.pullRequestDescriptionGeneration.instructions": [
    {
