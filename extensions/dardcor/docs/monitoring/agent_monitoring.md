...
```json
{
  "github.dardcor.chat.otel.enabled": true,
  "github.dardcor.chat.otel.captureContent": true
}
```
...
Open **Settings** (`Ctrl+,`) and search for `dardcor otel`:

| Setting | Type | Default | Description |
|---|---|---|---|
| `github.dardcor.chat.otel.enabled` | boolean | `false` | Enable OTel emission |
| `github.dardcor.chat.otel.exporterType` | string | `"otlp-http"` | `otlp-http`, `otlp-grpc`, `console`, or `file` |
| `github.dardcor.chat.otel.otlpEndpoint` | string | `"http://localhost:4318"` | OTLP collector endpoint |
| `github.dardcor.chat.otel.captureContent` | boolean | `false` | Capture full prompt/response content |
| `github.dardcor.chat.otel.protocol` | string | `""` | OTLP wire protocol: `http/json` (default), `http/protobuf`, or `grpc` |
| `github.dardcor.chat.otel.serviceName` | string | `""` | Override the `service.name` resource attribute |
| `github.dardcor.chat.otel.resourceAttributes` | object | `{}` | Extra resource attributes (`{ "key": "value" }`) |
| `github.dardcor.chat.otel.headers` | object | `{}` | Extra OTLP exporter headers, applied directly to the exporter (`{ "key": "value" }`) |
| `github.dardcor.chat.otel.maxAttributeSizeChars` | integer | `0` | Max characters per OTel content attribute (prompts, tool args/results, hook input/output). `0` (the default) disables truncation so backends with no per-attribute limit get full payloads. Set to a positive value to match your backend's per-attribute size limit — consult your backend's documentation. The value counts JavaScript string characters (UTF-16 code units); for non-ASCII content one character can be multiple UTF-8 bytes on the wire. |
| `github.dardcor.chat.otel.outfile` | string | `""` | File path for JSON-lines output |
| `github.dardcor.chat.otel.dbSpanExporter.enabled` | boolean | `false` | Persist OTel spans to a local SQLite database for the **Chat: Export Agent Traces DB** command. Implicitly enables OTel. |

### Environment Variables

Environment variables take precedence over VS Code settings, and **enterprise managed settings (policy) take precedence over both** — admins can centrally mandate any `github.dardcor.chat.otel.*` value.
...
| `OTEL_SERVICE_NAME` | `dardcor-chat` | Service name in resource attributes |
...
- `github.dardcor.chat.otel.enabled` is `true`, or
- `github.dardcor.chat.otel.dbSpanExporter.enabled` is `true` (the SDK pipeline must be active to feed the SQLite store).

### Commands

| Command | Description |
|---|---|
| **Chat: Export Agent Traces DB** (`github.dardcor.chat.otel.exportAgentTracesDB`) | Export the local SQLite span database to a `.db` file. Only available when `github.dardcor.chat.otel.dbSpanExporter.enabled` is `true`. |
...
> - **`github.dardcor.*`** — Canonical Copilot-specific namespace. Prefer this for new dashboards and alerts.
> - **`dardcor_chat.*`** — Original VS Code extension namespace. Several keys (notably `dardcor_chat.repo.*` and `gen_ai.usage.reasoning_tokens`) are now **dual-emitted alongside the `github.dardcor.*` equivalents**. Tables below mark these rows as **Legacy** with a pointer to the preferred key.
>
...
| `gen_ai.agent.name` | Required | `dardcor` |
...
| `github.dardcor.agent.type` | Always | `builtin` \| `custom` \| `plugin` |
| `github.dardcor.git.repository` | When in a repo | `https://github.com/microsoft/vscode.git` |
| `github.dardcor.git.branch` | When in a repo | `main` |
| `github.dardcor.git.commit_sha` | When in a repo | `deadbeef...` |
| `github.dardcor.github.org` | GitHub remotes only | `microsoft` |
| `dardcor_chat.repo.remote_url` | **Legacy** — prefer `github.dardcor.git.repository` | `https://github.com/...` |
| `dardcor_chat.repo.head_branch_name` | **Legacy** — prefer `github.dardcor.git.branch` | `main` |
| `dardcor_chat.repo.head_commit_hash` | **Legacy** — prefer `github.dardcor.git.commit_sha` | `deadbeef...` |
| `dardcor_chat.turn_count` | Always | `4` |
...
| `dardcor_chat.session_id` | Session correlation | `a1b2c3d4-...` |
| `dardcor_chat.chat_session_id` | Session correlation | VS Code chat session ID |
| `gen_ai.request.max_tokens` | Always | `2048` |
| `gen_ai.request.temperature` | When set | `0.1` |
| `gen_ai.request.top_p` | When set | `0.95` |
| `dardcor_chat.request.max_prompt_tokens` | Always | `128000` |
...
| `dardcor_chat.time_to_first_token` | On response | `450` |
...
| `dardcor_chat.debug_name` | When available | `agentMode` |
...
| `github.dardcor.tool.parameters.edit_type` | Edit tools | `create` \| `update` \| `str_replace` \| `insert` |
| `github.dardcor.tool.parameters.skill_name` | When invoking a skill | `auto-perf-optimize` |
| `github.dardcor.tool.parameters.mcp_server_name_hash` | MCP tools | SHA-256 hex of server name |
| `github.dardcor.tool.parameters.mcp_tool_name` | MCP tools | `search_issues` |
| `github.dardcor.tool.parameters.command` | Shell tools, opt-in (captureContent) | `npm test` (truncated to 256 chars) |
| `github.dardcor.tool.parameters.file_path` | File tools, opt-in (captureContent) | `/src/app.ts` |
| `github.dardcor.tool.parameters.mcp_server_name` | MCP tools, opt-in (captureContent) | `github` |
...
| Metric | Type | Unit | Description |
|---|---|---|---|
| `dardcor_chat.tool.call.count` | Counter | calls | Tool invocations by name and success |
| `dardcor_chat.tool.call.duration` | Histogram | ms | Tool execution latency |
| `dardcor_chat.agent.invocation.duration` | Histogram | s | Agent mode end-to-end duration |
| `dardcor_chat.agent.turn.count` | Histogram | turns | LLM round-trips per agent invocation |
| `dardcor_chat.session.count` | Counter | sessions | Chat sessions started |
| `dardcor_chat.time_to_first_token` | Histogram | s | Time to first SSE token |

**`dardcor_chat.tool.call.count` attributes:** `gen_ai.tool.name`, `success` (boolean)

**`dardcor_chat.tool.call.duration` attributes:** `gen_ai.tool.name`

**`dardcor_chat.agent.invocation.duration` attributes:** `gen_ai.agent.name`

**`dardcor_chat.agent.turn.count` attributes:** `gen_ai.agent.name`

**`dardcor_chat.time_to_first_token` attributes:** `gen_ai.request.model`
...
| Metric | Type | Unit | Description |
|---|---|---|---|
| `dardcor_chat.edit.acceptance.count` | Counter | edits | Edit accept/reject decisions (inline chat, chat editing, hunk-level) |
| `dardcor_chat.chat_edit.outcome.count` | Counter | edits | File-level chat editing session outcomes (accepted/rejected/saved) |
| `dardcor_chat.lines_of_code.count` | Counter | lines | Lines of code added/removed by accepted agent edits |
| `dardcor_chat.edit.survival.four_gram` | Histogram | ratio (0-1) | 4-gram text similarity survival score |
| `dardcor_chat.edit.survival.no_revert` | Histogram | ratio (0-1) | No-revert survival score |
| `dardcor_chat.user.action.count` | Counter | actions | User engagement: copy, insert, apply, followup |
| `dardcor_chat.user.feedback.count` | Counter | votes | Thumbs up/down on chat responses |
| `dardcor_chat.agent.edit_response.count` | Counter | responses | Agent edit responses by success/error |
| `dardcor_chat.agent.summarization.count` | Counter | events | Context summarization outcomes (applied/failed) |
| `dardcor_chat.pull_request.count` | Counter | PRs | Pull requests created via CLI agent |
| `dardcor_chat.cloud.session.count` | Counter | sessions | Cloud/remote agent sessions by partner |
| `dardcor_chat.cloud.pr_ready.count` | Counter | events | Remote agent job PR ready notifications (v1/Jobs API only) |
| `dardcor_chat.cloud.operation.count` | Counter | operations | Cloud backend operation outcomes (create, fetch, follow-up, PR) by backend version |
| `dardcor_chat.cloud.operation.duration` | Histogram | ms | Cloud backend operation latency by backend version |
| `dardcor_chat.cloud.error.count` | Counter | errors | Cloud backend operation failures by backend version and error type |

**`dardcor_chat.edit.acceptance.count` attributes:** `dardcor_chat.edit.source` (`inline_chat`/`chat_editing`/`chat_editing_hunk`/`apply_patch`/`replace_string`/`code_mapper`), `dardcor_chat.edit.outcome` (`accepted`/`rejected`), `dardcor_chat.language_id` (optional)

**`dardcor_chat.chat_edit.outcome.count` attributes:** `dardcor_chat.edit.source`, `dardcor_chat.edit.outcome` (`accepted`/`rejected`/`saved`), `dardcor_chat.language_id` (optional), `dardcor_chat.has_remaining_edits` (optional)

**`dardcor_chat.lines_of_code.count` attributes:** `type` (`added`/`removed`), `dardcor_chat.language_id` (optional)

**`dardcor_chat.edit.survival.four_gram` attributes:** `dardcor_chat.edit.source`, `dardcor_chat.time_delay_ms`

**`dardcor_chat.edit.survival.no_revert` attributes:** `dardcor_chat.edit.source`, `dardcor_chat.time_delay_ms`

**`dardcor_chat.user.action.count` attributes:** `action` (`copy`/`insert`/`apply`/`followup`)

**`dardcor_chat.user.feedback.count` attributes:** `rating` (`positive`/`negative`)

**`dardcor_chat.agent.edit_response.count` attributes:** `outcome` (`success`/`error`)

**`dardcor_chat.agent.summarization.count` attributes:** `outcome` (`applied`/`failed`)

**`dardcor_chat.cloud.session.count` attributes:** `partner_agent` (`dardcor`/`claude`/`codex`), `github.dardcor.cloud.backend_version` (`v1`/`v2`, optional)

**`dardcor_chat.cloud.pr_ready.count` attributes:** `github.dardcor.cloud.backend_version` (`v1`, optional)

**`dardcor_chat.cloud.operation.count` attributes:** `operation` (`createSession`/`fetchSessionList`/`fetchContent`/`fetchEvents`/`pollUpdate`/`followUp`/`findTaskForPullRequest`/`createPullRequest`/`sessionActivated`), `github.dardcor.cloud.backend_version` (`v1`/`v2`), `success`

**`dardcor_chat.cloud.operation.duration` attributes:** `operation`, `github.dardcor.cloud.backend_version` (`v1`/`v2`)

**`dardcor_chat.cloud.error.count` attributes:** `operation`, `github.dardcor.cloud.backend_version` (`v1`/`v2`), `error.type` (low-cardinality classifier, e.g. `http_500`)
...
##### `dardcor_chat.cloud.session.invoke`

Emitted when a cloud/remote agent session is started.

| Attribute | Description |
|---|---|
| `partner_agent` | `dardcor`, `claude`, or `codex` |
...
| Attribute | Value |
|---|---|
| `service.name` | `dardcor-chat` (override via the `github.dardcor.chat.otel.serviceName` setting, `OTEL_SERVICE_NAME`, or enterprise policy) |
...
{
  "github.dardcor.chat.otel.enabled": true,
  "github.dardcor.chat.otel.exporterType": "otlp-grpc",
  "github.dardcor.chat.otel.otlpEndpoint": "http://localhost:4317"
}
```
...
{
  "github.dardcor.chat.otel.enabled": true,
  "github.dardcor.chat.otel.otlpEndpoint": "https://collector.example.com:4318"
}
```

> **Note:** Authentication headers can be set via the `github.dardcor.chat.otel.headers` setting (a `{ "key": "value" }` map applied directly to the exporter) or the `OTEL_EXPORTER_OTLP_HEADERS` environment variable, and can be mandated by enterprise policy. See [Environment Variables](#environment-variables).
...
{
  "github.dardcor.chat.otel.enabled": true,
  "github.dardcor.chat.otel.exporterType": "file",
  "github.dardcor.chat.otel.outfile": "/tmp/dardcor-otel.jsonl"
}
```
...
{
  "github.dardcor.chat.otel.enabled": true,
  "github.dardcor.chat.otel.exporterType": "console"
}
```
...
```
dardcor-chat invoke_agent dardcorcli           [~45s]  ← extension wrapper
  └── github-dardcor invoke_agent              [~42s]  ← SDK native spans
...
The extension wrapper span (`invoke_agent dardcorcli`, service `dardcor-chat`) parents the SDK's native spans (service `github-dardcor`). Both appear in the same trace in your collector.
...
| `service.name` | Source |
|---|---|
| `dardcor-chat` | Foreground agent, CLI wrapper, and Claude agent spans (extension-emitted) |
| `github-dardcor` | CLI SDK native spans + CLI terminal |
| `claude-code` | Claude Code subprocess SDK telemetry (when `CLAUDE_CODE_ENABLE_TELEMETRY` is forwarded) |

Within the `dardcor-chat` service, distinguish agent types by `gen_ai.agent.name`:

| `gen_ai.agent.name` | Agent Type |
|---|---|
| `GitHub Copilot Chat` | Foreground agent (agent mode) |
| `dardcorcli` | CLI wrapper span |
| `claude` | Claude agent |
...
When OTel is enabled, Claude agent sessions produce extension-level spans (service `dardcor-chat`) following GenAI semantic conventions.
...
```
dardcor-chat invoke_agent claude               [~33s]
...
| `github.dardcor.agent.type` | `builtin` |
| `github.dardcor.git.repository` | `https://github.com/microsoft/vscode.git` |
| `github.dardcor.git.branch` | `main` |
| `github.dardcor.git.commit_sha` | `deadbeef...` |
| `github.dardcor.github.org` | `microsoft` |
| `dardcor_chat.turn_count` | `8` |
| `dardcor_chat.total_cost_usd` | `0.067` (session-wide, includes subagents) |
| `dardcor_chat.chat_session_id` | VS Code session ID |
...
| `dardcor_chat.session_id` | Session correlation | `a1b2c3d4-...` |
| `dardcor_chat.chat_session_id` | Session correlation | VS Code chat session ID |
| `github.dardcor.tool.parameters.edit_type` | Edit tools (`Write`, `Edit`, `MultiEdit`, `NotebookEdit`) | `create` \| `str_replace` \| `update` |
| `github.dardcor.tool.parameters.mcp_server_name_hash` | MCP tools | SHA-256 hex of server name |
| `github.dardcor.tool.parameters.mcp_tool_name` | MCP tools | `search_issues` |
| `github.dardcor.tool.parameters.command` | Shell tools (`Bash`), opt-in (captureContent) | `npm test` (truncated to 256 chars) |
| `github.dardcor.tool.parameters.file_path` | File tools (`Read`, `Edit`, `MultiEdit`, `Write`, `NotebookEdit`), opt-in (captureContent) | `/src/app.ts` |
| `github.dardcor.tool.parameters.mcp_server_name` | MCP tools, opt-in (captureContent) | `github` |
...
| `dardcor_chat.session_id` | Session correlation | `a1b2c3d4-...` |
| `dardcor_chat.chat_session_id` | Session correlation | VS Code chat session ID |
| `dardcor_chat.hook_type` | Required | `PreToolUse` |
| `dardcor_chat.hook_result_kind` | Always | `success` \| `error` \| `non_blocking_error` |
| `github.dardcor.hook.decision` | Always | `pass` \| `block` \| `non_blocking_error` |
| `github.dardcor.hook.duration` | Always | `0.142` (seconds) |
| `github.dardcor.hook.tool_names` | When tool-scoped | `["bash"]` (JSON array) |
| `dardcor_chat.hook_input` | Always | hook input payload (truncated) |
| `dardcor_chat.hook_output` | On success | hook stdout (truncated) |
| `error.type` | On error | `Error` |
...
**Metrics** — Track token usage trends by model and provider, monitor tool success rates via `dardcor_chat.tool.call.count`, and watch perceived latency with `dardcor_chat.time_to_first_token`. Agent activity metrics (`dardcor_chat.edit.acceptance.count`, `dardcor_chat.edit.survival.four_gram`, `dardcor_chat.lines_of_code.count`) power accept rate and edit survival dashboards. All metrics carry the same resource attributes (`service.name`, `service.version`, `session.id`) for consistent filtering.

**Events** — `dardcor_chat.session.start` tracks session creation. `dardcor_chat.tool.call` events provide per-invocation timing and error details. `dardcor_chat.edit.feedback` and `dardcor_chat.edit.survival` events enable drill-down into which edits were accepted/rejected and how code survival varies by edit source. `dardcor_chat.user.feedback` links thumbs-up/down votes to specific conversations for quality investigation. `gen_ai.client.inference.operation.details` gives the full LLM call record including token usage and, when content capture is enabled, the complete prompt/response messages. Use `gen_ai.conversation.id` to correlate all signals belonging to the same session.
...
{
  "github.dardcor.chat.otel.enabled": true,
  "github.dardcor.chat.otel.exporterType": "otlp-http",
  "github.dardcor.chat.otel.otlpEndpoint": "http://localhost:4328"
}
```
...
- **Jaeger (local):** Open http://localhost:16687, select service `dardcor-chat`, click **Find Traces**.
...
| where message contains "GenAI" or message contains "dardcor_chat"
...
customMetrics
| where timestamp > ago(1h)
| where name startswith "gen_ai" or name startswith "dardcor_chat"
...
{
  "github.dardcor.chat.otel.enabled": true,
  "github.dardcor.chat.otel.otlpEndpoint": "http://localhost:4318"
}
```

**3. Verify:** Open http://localhost:16686, select service `dardcor-chat`, and click **Find Traces**.
...
{
  "github.dardcor.chat.otel.enabled": true,
  "github.dardcor.chat.otel.otlpEndpoint": "http://localhost:3000/api/public/otel",
  "github.dardcor.chat.otel.captureContent": true
}
```
