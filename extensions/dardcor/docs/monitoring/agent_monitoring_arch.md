...
Extension Root (tracer A):
  invoke_agent dardcorcli → traceparent → SDK
...
```
invoke_agent dardcor (INTERNAL)          ← toolCallingLoop.ts
...
```
invoke_agent dardcorcli (INTERNAL)       ← dardcorcliSession.ts (tracer A)
└── [traceparent linked]
...
```
invoke_agent (CLIENT)                    ← standalone dardcor binary
│   service.name = github-dardcor
├── chat gpt-4o (CLIENT)
...
src/extension/chatSessions/dardcorcli/node/
├── dardcorCliBridgeSpanProcessor.ts  # Bridge: SDK spans → IOTelService (+ hook span enrichment)
├── dardcorcliSession.ts              # Root invoke_agent span + traceparent + hook event stash
└── dardcorcliSessionService.ts       # Bridge installation + env var setup
...
| `dardcorcliSession.ts` | `invoke_agent dardcorcli` wrapper span + traceparent propagation + hook event stash |
| `dardcorCliBridgeSpanProcessor.ts` | Bridge: SDK `ReadableSpan` → `ICompletedSpanData` (with hook-span enrichment) |
| `dardcorcliSessionService.ts` | Bridge installation + OTel env vars for SDK |
| `dardcorCLITerminalIntegration.ts` | OTel env vars forwarded to terminal process |
...
4. VS Code settings (`github.dardcor.chat.otel.*`)
5. Defaults (lowest)
...
| `setting` | `github.dardcor.chat.otel.enabled` is `true` |
| `otlpEndpointEnvVar` | `OTEL_EXPORTER_OTLP_ENDPOINT` is set without an explicit enable |
| `dbSpanExporterOnly` | Only `github.dardcor.chat.otel.dbSpanExporter.enabled` is on — implicitly turns OTel on so the SDK pipeline can feed the SQLite store |
...
| Source | `service.name` |
|---|---|
| Extension (Provider A) | `dardcor-chat` |
| Copilot CLI SDK / terminal | `github-dardcor` |
...
| `dardcor_chat.*` | Extension-specific (legacy; several keys dual-emit alongside `github.dardcor.*`) | `dardcor_chat.session_id`, `dardcor_chat.chat_session_id` |
| `github.dardcor.*` | Canonical Copilot namespace — extension-emitted enrichment (foreground agent, Claude agent, CLI bridge) + CLI SDK internal metrics | `github.dardcor.agent.type`, `github.dardcor.git.repository`, `github.dardcor.tool.parameters.edit_type`, `github.dardcor.hook.decision`, `github.dardcor.cost`, `github.dardcor.aiu` |
...
For **extension-emitted** spans (`invoke_agent`, `chat`, `execute_tool`, `execute_hook` produced by the foreground agent, Claude agent, BYOK providers, and `chatMLFetcher`), `gen_ai.conversation.id` carries the **VS Code chat session id** on every exported span, so a vendor-agnostic OTLP backend can group all spans of a session on the standard GenAI key without a traceId → root join. The vendor keys `dardcor_chat.session_id` / `dardcor_chat.chat_session_id` are dual-emitted for the Agent Debug Log and SQLite span store (`COALESCE(conversation_id, chat_session_id)`).
...
- BYOK `chat` spans (`anthropicProvider.ts`, `geminiNativeProvider.ts`) read the session id from the in-scope `CapturingToken.chatSessionId` and emit `gen_ai.conversation.id` + `dardcor_chat.session_id` + `dardcor_chat.chat_session_id`.
...
- **Debug panel copy** (via the bridge → `injectCompletedSpan` → `onDidCompleteSpan`): `dardcorCliBridgeSpanProcessor.ts` injects `dardcor_chat.chat_session_id` **and** `gen_ai.conversation.id` (session id) onto the copied span for any span that lacks them, keyed by a traceId → session map. This affects only the debug-panel / file-logger view.
- **SDK's own OTLP export** (via the runtime's `BatchSpanProcessor`): the bridge **cannot** mutate these spans. Their `gen_ai.conversation.id` is whatever the runtime stamps — currently `invoke_agent` and `chat` (and `execute_tool` once [github/dardcor-agent-runtime#12383](https://github.com/github/dardcor-agent-runtime/pull/12383) lands). Closing this gap for other CLI child spans must happen in the runtime.
...
src/extension/chatSessions/dardcorcli/node/test/
└── dardcorCliBridgeSpanProcessor.spec.ts   # Bridge processor tests
