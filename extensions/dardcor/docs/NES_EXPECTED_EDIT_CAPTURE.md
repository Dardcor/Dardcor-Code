...
```json
{
  ...
  "github.dardcor.chat.advanced.inlineEdits.recordExpectedEdit.enabled": true
}
```
...
```json
{
  "github.dardcor.chat.advanced.inlineEdits.recordExpectedEdit.onReject": false
}
```
...
Recordings are stored in your workspace under `.dardcor/nes-feedback/`:
...
|------------|-------------|
| `github.dardcor.nes.captureExpected.start` | Start capture manually |
| `github.dardcor.nes.captureExpected.confirm` | Confirm and save |
| `github.dardcor.nes.captureExpected.abort` | Cancel capture |
| `github.dardcor.nes.captureExpected.submit` | Submit to `microsoft/dardcor-nes-feedback` |
...
1. **Start Capture**: Create a bookmark in DebugRecorder, store the current document ID, set context key `dardcorNesCaptureMode` to enable keybindings, and show status bar indicator.
...
#### Location
Recordings are stored in the **first workspace folder** under the `.dardcor/nes-feedback/` directory:

- **Full path**: `<workspaceFolder>/.dardcor/nes-feedback/capture-<timestamp>.recording.w.json`
- **Timestamp format**: ISO 8601 with colons/periods replaced by hyphens (e.g., `2025-12-04T14-30-45`)
- **Example**: `.dardcor/nes-feedback/capture-2025-12-04T14-30-45.recording.w.json`
...
1. All captures from `.dardcor/nes-feedback/` are collected
2. A preview dialog shows which files will be included
3. You can exclude specific files if needed
4. A pull request is created in `microsoft/dardcor-nes-feedback`
...
**Requirements:** GitHub authentication with repo access to `microsoft/dardcor-nes-feedback`
