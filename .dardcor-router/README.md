...
[🚀 Quick Start](#-quick-start) • [💡 Features](#-key-features) • [📖 Setup](#-setup-guide) • [🌐 Website](https://dardcor-code.web.id)
...
This repository package is private (`dardcor-code-app`), so source/Docker execution is the expected local development path.
  ...
  <tr>
    ...
    <td align="center" width="320">
      <a href="https://www.youtube.com/watch?v=hPusYX-5Pmw">
        <img src="https://img.youtube.com/vi/hPusYX-5Pmw/maxresdefault.jpg" alt="Hướng Dẫn Setup OpenClaw + Dardcor Code: Tạo Bot Zalo AI Tự Động Từ A-Z" width="300"/>
      </a><br/>
      <b>🇻🇳 Tiếng Việt</b><br/>
      <sub>Hướng Dẫn Setup OpenClaw + Dardcor Code: Tạo Bot Zalo AI Tự Động Từ A-Z<br/>by <a href="https://github.com/tuanminhhole">tuanminhhole</a></sub>
    </td>
...
> 🎬 **Made a video about Dardcor Code?** Submit a [Pull Request](https://github.com/dardcor/dardcor-code/pulls) adding your video to this section — we'll merge it!

---

## 🛠️ Supported CLI Tools

Dardcor Code works seamlessly with all major AI coding tools:
...
Set `X-Dardcor Code-Token-Saver: off` to bypass all token savers for one chat request.
...
Headroom is optional and runs separately. Dardcor Code calls Headroom's local `/v1/compress` endpoint, then keeps normal routing, fallback, auth, and usage tracking:
...
If Headroom is down or returns an error, Dardcor Code fails open and sends the original request.
...
- Your CLI tool sends OpenAI format → Dardcor Code translates → Provider receives native format
...
> Dardcor Code itself **never charges** you anything. You only pay providers directly (if using paid services).
>
...
### 📊 Understanding Dardcor Code Costs & Billing

**Dardcor Code Billing Reality:**

✅ **Dardcor Code software = FREE forever** (open source, never charges)  
...
❌ **Dardcor Code never sends invoices** or charges your card
...
- **Cheap providers** (GLM, MiniMax): Pay them directly, Dardcor Code just routes
- **FREE providers** (iFlow, Kiro, Qwen): Genuinely free forever, no hidden charges
- **Dardcor Code**: Never charges anything, ever
...
The dashboard tracks your token usage and displays **estimated costs** as if you were using paid APIs directly. This is **not actual billing** - it's a reference to show how much you're saving by using free models or existing subscriptions through Dardcor Code.
...
<details>
<summary><b>💳 Will I be charged by Dardcor Code?</b></summary>

**No.** Dardcor Code is free, open-source software that runs on your own computer. It never charges you anything.

**You only pay:**

- ✅ **Subscription providers** (Claude Code $20/mo, Codex $20-200/mo) → Pay them directly on their websites
- ✅ **Cheap providers** (GLM, MiniMax) → Pay them directly, Dardcor Code just routes your requests
- ❌ **Dardcor Code itself** → **Never charges anything, ever**

Dardcor Code is a local proxy/router. It doesn't have your credit card, can't send invoices, and has no billing system. It's completely free software.
...
Dardcor Code just routes your requests to them - there's no "catch" or future billing from Dardcor Code itself. They're truly free services, and Dardcor Code makes them easy to use with fallback support.
...
3. **Use subscription providers last:**
   ...
   - Dardcor Code helps maximize their value through quota tracking
...
Dardcor Code's smart fallback prevents surprise charges:

**Scenario:** You're on a coding sprint and blow through your quotas

**Without Dardcor Code:**
...
**You're in control:** Set spending limits per provider in dashboard, and Dardcor Code respects them.
...
docker pull dardcor/dardcor-code:latest   # update to latest
```

**Data persistence:** `$HOME/.dardcor-code/db/data.sqlite` on host ↔ `/app/data/db/data.sqlite` in container.

### Environment Variables

| Variable                                             | Default                                  | Description                                                                         |
| ---------------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------------------------------- |
| `JWT_SECRET`                                         | Auto-generated (`~/.dardcor-code/jwt-secret`) | JWT signing secret for dashboard auth cookie (override to share across instances)   |
...
| `BASE_URL`                                           | `http://localhost:21128`                 | Server-side internal base URL used by cloud sync jobs                               |
| `CLOUD_URL`                                          | `https://dardcor-code.web.id`                    | Server-side cloud sync endpoint base URL                                            |
| `NEXT_PUBLIC_BASE_URL`                               | `http://localhost:3000`                  | Backward-compatible/public base URL (prefer `BASE_URL` for server runtime)          |
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://dardcor-code.web.id`                    | Backward-compatible/public cloud URL (prefer `CLOUD_URL` for server runtime)        |
...
- Both `${DATA_DIR}` and `~/.dardcor-code` resolve to the same location in a Docker container — the symlink `/root/.dardcor-code -> /app/data` is created at build time.
...
- Set `PORT=21128` and `NEXT_PUBLIC_BASE_URL=http://localhost:21128`
...
Thanks to all contributors who helped make Dardcor Code better!
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — A full-featured TypeScript fork of Dardcor Code. Adds 36+ providers, 4-tier auto-fallback, multi-modal APIs (images, embeddings, audio, TTS), circuit breaker, semantic cache, LLM evaluations, and a polished dashboard. 368+ unit tests. Available via npm and Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — Rust token-saver. Dardcor Code ports its compression pipeline to JS → **−20-40% input tokens** on every request.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) by **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. Dardcor Code adapts its prompt → **−65% output tokens**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) by **[@DietrichGebert](https://github.com/DietrichGebert)** — _"lazy senior dev"_ skill. Dardcor Code injects its YAGNI-first ladder → **fewer tokens, less code, shorter diffs**.

Huge thanks to these authors — without their work, Dardcor Code's token-saving features wouldn't exist. ⭐ them on GitHub!
