...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.dardcor-code/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/dardcor-code/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[dardcor-code][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[dardcor-code][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[dardcor-code][runtime] chmod tray bin failed: ${e.message}`);
  }
}
