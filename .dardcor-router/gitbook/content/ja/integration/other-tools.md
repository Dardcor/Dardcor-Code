...
Dardcor CodeはOpenAI API形式をサポートする任意のツールと互換性があります。このガイドでは、様々なツールやカスタムアプリケーション向けの汎用統合パターンを説明します。

## 概要

Dardcor CodeはOpenAI互換APIエンドポイントを提供し、以下と動作します:
- カスタムスクリプトとアプリケーション
- APIクライアントとテストツール
- CLIツールとユーティリティ
- サードパーティ統合
...
任意のOpenAI互換ツールは以下の設定でDardcor Codeに接続できます:
...
llm = ChatOpenAI(
    ...
    openai_api_base="http://localhost:21128/v1",
    temperature=0.7
)
...
llm = OpenAI(
    model="cc/claude-sonnet-4-20250514",
    api_key="your-api-key-from-dashboard",
    api_base="http://localhost:21128/v1"
)
...
openai.api_base = "http://localhost:21128/v1"
...
ROUTER_BASE_URL=http://localhost:21128/v1
...
- 利用可能なモデルを確認: `curl http://localhost:21128/v1/models`
