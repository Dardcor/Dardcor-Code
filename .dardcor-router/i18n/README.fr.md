<div align="center">
  <img src="../images/dardcor-code.png?1" alt="Tableau de bord Dardcor Code" width="800"/>
  
  # Dardcor Code - Routeur IA GRATUIT et économiseur de tokens
...
[🚀 Démarrage rapide](#-démarrage-rapide) • [💡 Fonctionnalités](#-fonctionnalités-clés) • [📖 Configuration](#-guide-dinstallation) • [🌐 Site web](https://dardcor-code.com)
...
Le paquet de ce dépôt est privé (`dardcor-code-app`), donc l'exécution depuis le code source/Docker est le chemin de développement local attendu.
...
> 🎬 **Vous avez réalisé une vidéo sur Dardcor Code ?** Envoyez une [Pull Request](https://github.com/decolua/dardcor-code/pulls) en ajoutant votre vidéo à cette section — nous la fusionnerons !

---

## 🛠️ Outils CLI pris en charge

Dardcor Code fonctionne parfaitement avec tous les principaux outils de codage IA :
...
Headroom est optionnel et s'exécute séparément. Dardcor Code appelle l'endpoint local `/v1/compress` de Headroom, puis conserve le routage, le repli, l'authentification et le suivi d'utilisation normaux :

```
Client → Dardcor Code → Headroom /v1/compress → Dardcor Code → fournisseur
```
...
Si Headroom est indisponible ou renvoie une erreur, Dardcor Code bascule en mode dégradé et envoie la requête originale.
...
- Votre outil CLI envoie le format OpenAI → Dardcor Code le traduit → Le fournisseur reçoit le format natif
...
> Dardcor Code lui-même **ne vous facture jamais** quoi que ce soit. Vous ne payez que les fournisseurs directement (si vous utilisez des services payants).
>
...
### 📊 Comprendre les coûts et la facturation de Dardcor Code

**La réalité de la facturation de Dardcor Code :**

✅ **Le logiciel Dardcor Code = GRATUIT pour toujours** (open source, ne facture jamais)  
...
❌ **Dardcor Code n'envoie jamais de factures** et ne débite pas votre carte
...
- **Fournisseurs bon marché** (GLM, MiniMax) : Payez-les directement, Dardcor Code se contente de router
- **Fournisseurs GRATUITS** (iFlow, Kiro, Qwen) : Vraiment gratuits pour toujours, sans frais cachés
- **Dardcor Code** : Ne facture jamais rien, jamais
...
<details>
<summary><b>💳 Dardcor Code me facturera-t-il ?</b></summary>

**Non.** Dardcor Code est un logiciel gratuit et open source qui s'exécute sur votre propre ordinateur. Il ne vous facture jamais rien.

**Vous ne payez que :**

- ✅ **Fournisseurs par abonnement** (Claude Code 20 $/mois, Codex 20 à 200 $/mois) → Payez-les directement sur leurs sites web
- ✅ **Fournisseurs bon marché** (GLM, MiniMax) → Payez-les directement, Dardcor Code route simplement vos requêtes
- ❌ **Dardcor Code lui-même** → **Ne facture jamais rien, jamais**

Dardcor Code est un proxy/routeur local. Il n'a pas votre carte bancaire, ne peut pas envoyer de factures et ne possède aucun système de facturation. C'est un logiciel entièrement gratuit.
...
Dardcor Code se contente de router vos requêtes vers eux — il n'y a pas de "piège" ni de facturation future. Ce sont de véritables services gratuits, et Dardcor Code les rend faciles à utiliser avec le support du repli.
...
3. **Utilisez les fournisseurs par abonnement en dernier :**
   ...
   - Dardcor Code vous aide à maximiser leur valeur grâce au suivi de quota
...
Le repli intelligent de Dardcor Code évite les frais surprises :
...
**Vous gardez le contrôle :** Définissez des limites de dépenses par fournisseur dans le tableau de bord, et Dardcor Code les respecte.
...
**Astuce pro :** Utilisez Opus pour les tâches complexes et Sonnet pour la vitesse. Dardcor Code suit le quota par modèle !
...
> **Remarque :** OpenClaw ne fonctionne qu'avec un Dardcor Code local. Utilisez `127.0.0.1` au lieu de `localhost` pour éviter les problèmes de résolution IPv6.
...
docker pull decolua/dardcor-code:latest   # mise à jour vers la dernière version
```

**Persistance des données :** `$HOME/.dardcor-code/db/data.sqlite` sur l'hôte ↔ `/app/data/db/data.sqlite` dans le conteneur.
...
| `CLOUD_URL`                                          | `https://dardcor-code.com`                       | URL de base de l'endpoint de synchronisation cloud côté serveur                               |
| `NEXT_PUBLIC_BASE_URL`                               | `http://localhost:3000`                     | URL de base publique/rétrocompatible (préférez `BASE_URL` pour le runtime serveur)            |
| `NEXT_PUBLIC_CLOUD_URL`                              | `https://dardcor-code.com`                       | URL cloud publique/rétrocompatible (préférez `CLOUD_URL` pour le runtime serveur)             |
...
- `${DATA_DIR}` et `~/.dardcor-code` résolvent tous deux le même emplacement dans un conteneur Docker — le lien symbolique `/root/.dardcor-code -> /app/data` est créé au moment de la compilation.
...
- Renouvelé automatiquement par Dardcor Code
...
- **Site web** : [dardcor-code.com](https://dardcor-code.com)
- **GitHub** : [github.com/decolua/dardcor-code](https://github.com/decolua/dardcor-code)
- **Problèmes** : [github.com/decolua/dardcor-code/issues](https://github.com/decolua/dardcor-code/issues)

---

## 👥 Contributeurs

Merci à tous les contributeurs qui ont contribué à améliorer Dardcor Code !
...
**[OmniRoute](https://github.com/diegosouzapw/OmniRoute)** — Un fork TypeScript complet de Dardcor Code. Ajoute plus de 36 fournisseurs, un repli automatique à 4 niveaux, des API multimodales (images, embeddings, audio, TTS), un disjoncteur, un cache sémantique, des évaluations de LLM et un tableau de bord soigné. Plus de 368 tests unitaires. Disponible via npm et Docker.
...
- **[RTK](https://github.com/rtk-ai/rtk)** ![Stars](https://img.shields.io/github/stars/rtk-ai/rtk?style=flat&color=yellow) — économiseur de tokens en Rust. Dardcor Code porte son pipeline de compression en JS → **−20 à 40 % de tokens d'entrée** sur chaque requête.
- **[Caveman](https://github.com/JuliusBrussee/caveman)** ![Stars](https://img.shields.io/github/stars/JuliusBrussee/caveman?style=flat&color=yellow) par **[@JuliusBrussee](https://github.com/JuliusBrussee)** — viral _"why use many token when few token do trick"_. Dardcor Code adapte son prompt → **−65 % de tokens de sortie**.
- **[Ponytail](https://github.com/DietrichGebert/ponytail)** ![Stars](https://img.shields.io/github/stars/DietrichGebert/ponytail?style=flat&color=yellow) par **[@DietrichGebert](https://github.com/DietrichGebert)** — compétence de _"dev sénior fainéant"_. Dardcor Code injecte sa hiérarchie YAGNI-d'abord → **moins de tokens, moins de code, des diffs plus courts**.

Un immense merci à ces auteurs — sans leur travail, les fonctionnalités d'économie de tokens de Dardcor Code n'existeraient pas. Mettez-leur une ⭐ sur GitHub !
