...
vi.mock("@/shared/constants/config", () => ({ UPDATER_CONFIG: { appPort: 21127 } }));
...
describe("pingModelByKind reasoning models (#3010)", () => {
  ...
  it("uses a 1024-token budget for the chat completions probe", async () => {
    ...
    await pingModelByKind("cline-pass/kimi-k3", "llm", "http://127.0.0.1:21127");
  ...
  it("treats a reasoning-only (length-limited) response as ok:true", async () => {
    ...
    const result = await pingModelByKind("cline-pass/kimi-k3", "llm", "http://127.0.0.1:21127");
  ...
  it("still fails when there are no choices and no reasoning", async () => {
    ...
    const result = await pingModelByKind("some/model", "llm", "http://127.0.0.1:21127");
  ...
  it("passes a normal answer with the larger budget", async () => {
    ...
    const result = await pingModelByKind("openai/gpt-4o", "llm", "http://127.0.0.1:21127");
    expect(result.ok).toBe(true);
  });
});
