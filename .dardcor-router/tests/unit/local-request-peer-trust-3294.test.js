...
describe("peer header trust", () => {
  ...
  afterEach(() => {
    ...
    delete process.env.DARDCORROUTER_PEER_TOKEN;
  });
  ...
  it("accepts the legacy Host fallback only in development", async () => {
    ...
    const response = await proxy(request("/api/v1/models", { host: "localhost:21127" }));
...
describe("login limiter client IP", () => {
  ...
  it("still honours TRUST_PROXY for operators fronting dardcor-code with a reverse proxy", () => {
