...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="flex h-screen h-[100dvh] w-full overflow-hidden bg-bg">
      ...
      {/* Sidebar - Mobile */}
      <div
        className={`fixed inset-y-0 left-0 z-50 transform lg:hidden transition-transform duration-300 ease-in-out max-w-[85vw] ${
      ...
      <main className="flex flex-col flex-1 h-full min-w-0 relative transition-colors duration-300 isolate">
        ...
        <div className={`flex-1 overflow-y-auto custom-scrollbar ${pathname === "/dashboard/basic-chat" ? "" : "p-3.5 sm:p-6 lg:p-8 xl:p-10"} ${pathname === "/dashboard/basic-chat" ? "flex flex-col overflow-hidden" : ""}`}>
          <div className={`${pathname === "/dashboard/basic-chat" ? "flex-1 w-full h-full flex flex-col" : "max-w-7xl mx-auto w-full min-w-0"}`}>{children}</div>
        </div>
      </main>
    </div>
  );
