...
- `POST $DARDCORROUTER_URL/v1/chat/completions` — OpenAI format
- `POST $DARDCORROUTER_URL/v1/messages` — Anthropic format

## Discover

```bash
curl $DARDCORROUTER_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$DARDCORROUTER_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $DARDCORROUTER_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.DARDCORROUTER_URL}/v1`, apiKey: process.env.DARDCORROUTER_KEY });
...
```bash
curl -X POST $DARDCORROUTER_URL/v1/messages \
  -H "Authorization: Bearer $DARDCORROUTER_KEY" \
