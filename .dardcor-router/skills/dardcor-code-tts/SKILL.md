---
name: dardcor-code-tts
description: Text-to-speech via Dardcor Code /v1/audio/speech using OpenAI / ElevenLabs / Deepgram / Edge TTS / Google TTS / Hyperbolic / Inworld voices. Use when the user wants to convert text to speech, generate audio, voiceover, narrate, or read text aloud.
---

# Dardcor Code — Text-to-Speech
...
```bash
# 1) List models
curl $DARDCORROUTER_URL/v1/models/tts | jq '.data[].id'
# 2) Per-model metadata (params, voicesUrl if voice-by-id)
curl "$DARDCORROUTER_URL/v1/models/info?id=el/eleven_multilingual_v2"
# 3) List voices (elevenlabs, edge-tts, deepgram, inworld, local-device). Optional ?lang=vi
curl "$DARDCORROUTER_URL/v1/audio/voices?provider=edge-tts&lang=vi" | jq '.data[].model'
```
...
`POST $DARDCORROUTER_URL/v1/audio/speech`
...
```bash
curl -X POST "$DARDCORROUTER_URL/v1/audio/speech" \
...
const r = await fetch(`${process.env.DARDCORROUTER_URL}/v1/audio/speech`, {
  method: "POST",
  headers: { "Authorization": `Bearer ${process.env.DARDCORROUTER_KEY}`, "Content-Type": "application/json" },
