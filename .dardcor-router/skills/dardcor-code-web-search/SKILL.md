...
```bash
curl $DARDCORROUTER_URL/v1/models/web | jq '.data[] | select(.kind=="webSearch") | .id'
# Per-provider params (searchTypes, maxResults, required options like cx for google-pse)
curl "$DARDCORROUTER_URL/v1/models/info?id=tavily/search"
```
...
`POST $DARDCORROUTER_URL/v1/search`
...
```bash
curl -X POST $DARDCORROUTER_URL/v1/search \
...
```js
const r = await fetch(`${process.env.DARDCORROUTER_URL}/v1/search`, {
  method: "POST",
