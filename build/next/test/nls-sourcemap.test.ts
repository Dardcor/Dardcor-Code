 ...
 * `dc/nls.ts` so test files can import from `../dc/nls` (when placed in `test/`).
 */
async function bundleWithNLS(
	...
	// Write source files (always include the NLS stub at dc/nls.ts)
	const allFiles = { 'dc/nls.ts': NLS_STUB, ...files };
