...
The runtime side (`src/dc/platform/agentHost/`) downloads and caches
the SDK tarball at first use. See `IAgentSdkProductConfig` in
`src/dc/base/common/product.ts` for the contract.
...
(See `src/dc/platform/agentHost/common/agentService.ts` for env var names.)
