...
export function getResource(sourceFile: string): Resource {
	...
	if (/^dc\/platform/.test(sourceFile)) {
		return { name: 'dc/platform', project: editorProject };
	} else if (/^dc\/editor\/contrib/.test(sourceFile)) {
		return { name: 'dc/editor/contrib', project: editorProject };
	} else if (/^dc\/editor/.test(sourceFile)) {
		return { name: 'dc/editor', project: editorProject };
	} else if (/^dc\/base/.test(sourceFile)) {
		return { name: 'dc/base', project: editorProject };
	} else if (/^dc\/code/.test(sourceFile)) {
		return { name: 'dc/code', project: workbenchProject };
	} else if (/^dc\/server/.test(sourceFile)) {
		return { name: 'dc/server', project: serverProject };
	} else if (/^dc\/workbench\/contrib/.test(sourceFile)) {
		resource = sourceFile.split('/', 4).join('/');
		return { name: resource, project: workbenchProject };
	} else if (/^dc\/workbench\/services/.test(sourceFile)) {
		resource = sourceFile.split('/', 4).join('/');
		return { name: resource, project: workbenchProject };
	} else if (/^dc\/workbench/.test(sourceFile)) {
		return { name: 'dc/workbench', project: workbenchProject };
	} else if (/^dc\/sessions\/contrib/.test(sourceFile)) {
		resource = sourceFile.split('/', 4).join('/');
		return { name: resource, project: sessionsProject };
	} else if (/^dc\/sessions/.test(sourceFile)) {
		return { name: 'dc/sessions', project: sessionsProject };
	}
