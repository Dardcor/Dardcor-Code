...
suite('layersChecker', () => {
	...
	test('ignores skipped files', async () => {
		...
		writeSource('dc/feature/browser/test/feature.test.ts', `
	...
	test('detects aliased forbidden types', async () => {
		writeForbiddenService();
		writeSource('dc/feature/browser/feature.ts', `
			import { ForbiddenService as Service } from '../../platform/common/service.js';
			export const service: Service | undefined = undefined;
		`);

		assert.deepStrictEqual(await getViolations(), [
			{ type: 'ForbiddenService', fileName: 'dc/feature/browser/feature.ts', line: 2, character: 13 },
			{ type: 'ForbiddenService', fileName: 'dc/feature/browser/feature.ts', line: 2, character: 33 },
			{ type: 'ForbiddenService', fileName: 'dc/feature/browser/feature.ts', line: 3, character: 26 },
		]);
	});
	...
	test('detects members of inferred forbidden types', async () => {
		...
		assert.deepStrictEqual(await getViolations(), [
			{ type: 'ForbiddenService', fileName: 'dc/feature/browser/feature.ts', line: 3, character: 17 },
		]);
	});

	function writeForbiddenService(): void {
		writeSource('dc/platform/common/service.ts', `
