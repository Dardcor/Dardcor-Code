	...
	return es.through(function (this, file: FileWithLines) {
		...
		lines.forEach((line, i) => {
			...
			if (isRestrictedPath && !isLayerCheckerDisabled && monacoWorkbenchPattern.test(line)) {
				reporter(file.relative + '(' + (i + 1) + ',1): The class .monaco-workbench cannot be used in files under src/dc/{base,platform,editor} because only src/dc/workbench applies it', true);
				errorCount++;
			}
		});
 ...
 * default `src/dc/sessions` scope to follow the requested path).
 */
