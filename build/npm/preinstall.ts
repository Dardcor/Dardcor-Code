...
if (!process.env['VSCODE_SKIP_NODE_VERSION_CHECK']) {
	...
	const isRequiredNodeVersion = majorNodeVersion === requiredMajor &&
		(minorNodeVersion > requiredMinor ||
			(minorNodeVersion === requiredMinor && patchNodeVersion >= requiredPatch));
	const isCompatibleNewerNodeVersion = majorNodeVersion === requiredMajor + 2;

	if (!isRequiredNodeVersion && !isCompatibleNewerNodeVersion) {
		console.error(`\x1b[1;31m*** Please use Node.js v${requiredVersion}+ or a compatible Node.js ${requiredMajor + 2}.x release. Currently using v${process.versions.node}.\x1b[0;0m`);
		throw new Error();
	}
}
...
if (process.platform === 'win32') {
	if (!process.env['VSCODE_SKIP_PREINSTALL_CHECK'] && !hasSupportedVisualStudioVersion()) {
...
function installHeaders() {
	...
	const cmd = (process.env.npm_command === 'install' || process.env.npm_command === 'ci') ? process.env.npm_command : 'install';
