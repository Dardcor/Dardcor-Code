...
const extractEditorSrcTask = task.define('extract-editor-src', () => {
	...
	const codiconDest = path.join(root, 'src', 'dc', 'base', 'browser', 'ui', 'codicons', 'codicon', 'codicon.ttf');
	...
	standalone.extractEditor({
		...
		entryPoints: [
			'dc/editor/editor.main.ts',
			'dc/editor/editor.worker.start.ts',
			'dc/editor/common/services/editorWebWorkerMain.ts',
		],
		inlineEntryPoints: [
			apiusages,
			extrausages
		...
		additionalFilesToCopyOut: [
			'dc/base/browser/dompurify/dompurify.js',
			'dc/base/common/marked/marked.js',
		],
...
const finalEditorResourcesTask = task.define('final-editor-resources', () => {
	return es.merge(
		...
		gulp.src('src/dc/monaco.d.ts')
			...
			.pipe(gulp.dest('out-monaco-editor-core/esm/dc/editor')),
		...
		gulp.src('build/monaco/package.json')
			.pipe(es.through(function (data) {
				...
				try {
					const markedManifestPath = path.join(root, 'src/dc/base/common/marked/cgmanifest.json');
					const dompurifyManifestPath = path.join(root, 'src/dc/base/browser/dompurify/cgmanifest.json');
...
task.task('monacodts', task.define('monacodts', () => {
	...
	fs.writeFileSync(path.join(root, 'src/dc/editor/common/standalone/standaloneEnums.ts'), result.enums);
			...
			child.stdout.on('data', data => {
				...
				if (str.indexOf('Starting compilation') >= 0 || str.indexOf('File change detected') >= 0) {
					...
					if (match) {
						...
						// e.g. src/dc/base/common/strings.ts(663,5): error TS2322: Type '1234' is not assignable to type 'string'.
